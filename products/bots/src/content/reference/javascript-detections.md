...
- If your CSP doesn't use `nonce` for script tags and **JavaScript Detection** feature is enabled, you may see a console error such as `Refused to execute inline script because it violates the following Content Security Policy directive: "script-src 'self'". Either the 'unsafe-inline' keyword, a hash ('sha256-b123b8a70+4jEj+d6gWI9U6IilUJIrlnRJbRR/uQl2Jc='), or a nonce ('nonce-...') is required to enable inline execution.` We recommend the use of CSP `nonce` for script tags which does auto-parsing instead of using `unsafe-inline`.
