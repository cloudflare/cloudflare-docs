...
```bash
curl -s -H "X-Auth-Email: <REDACTED>" -H "Authorization: Bearer <REDACTED>" GET "https://api.cloudflare.com/client/v4/zones/<ZONE_ID>/logs/control/retention/flag" | jq .
```
...
```bash
curl -s -H "X-Auth-Email: <REDACTED>" -H "Authorization: Bearer <REDACTED>" POST "https://api.cloudflare.com/client/v4/zones/<ZONE_ID>/logs/control/retention/flag" -d'{"flag":true}' | jq .
```
