...
**Logpush** is designed to retry in case of errors. If your destination is temporarily unavailable, Logpush will keep trying until the destination is online again and the destination receives the logs. Logpush will also automatically catch up so that you do not miss any logs. However, if Cloudflare persistently receives errors from your destination, we will take that as a sign that the destination is permanently unavailable and disable your push job. The job can always be re-enabled later.
