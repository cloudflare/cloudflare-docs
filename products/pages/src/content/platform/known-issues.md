...
- When adding a custom domain, the domain may get stuck verifying due to being unable to validate a request for a SSL on that hostname. In order to ensure the SSL can validiate, ensure Cloudflare Access or a Cloudflare Worker is allowing request to the validation path: `http://{domain_name}/.well-known/pki-validation/*`.
