...
When a filename includes a placeholder, the `name` must be alphanumeric and cannot contain spaces. The URL segment(s) that match the placeholder will be available under the `context.params` object using the filename placeholder as the key.
...
Once your binding is created in the Workers dashboard, go to **your Pages project** > **Settings** > **Functions**. Select the *namespace* from the dropdown and set a name for your binding. 
