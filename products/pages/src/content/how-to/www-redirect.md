...
To do this we will make a rule with the target of `www.example.com/*`, that `/*` is what matches all paths. Then for the Forwarding URL location, we want to set it to `example.com/$1`, that `$1` will be replaced by our wildcard match in target (the `*`).
