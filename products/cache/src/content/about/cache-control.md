...
- `stale-while-revalidate=seconds` — When present in an HTTP response, indicates caches may serve the response in which it appears after it becomes stale, up to the indicated number of seconds since the resource expired. If [Always Online](/about/always-online) is enabled, then the `stale-while-revalidate` and `stale-if-error` directive are ignored. This directive is not supported when using the Cache API'S `cache.match` or `cache.put`. For more information, refer to the [Worker's documentation for Cache API](https://developers.cloudflare.com/workers/platform/limits#cache-api).
