...
# Magic WAN and Secure Web Gateway
...
- Use site routers that support GRE tunnels and Policy-based Routing (PBR) so that specific Internet-bound traffic from the sites' private networks can be routed through Secure Web Gateway to enforce a user's specific web access policies.
...
- Headquarters is assigned a 192.168.0.0/16 network
- Branch office is assigned 10.0.1.0/24 network
- Data center is assigned with 172.16.0.0/12 network
...
After setting up the GRE tunnels and static routes, navigate to the Teams dashboard to configure the policies for Secure Web Gateway. To set up the policies, refer to [Secure Web Gateway policies](https://developers.cloudflare.com/cloudflare-one/policies/filtering).

After Secure Web Gateway is configured, enterprise users and devices from each of the sites mentioned in the example scenario would be able to safely browse or access Internet resources under the protection of the Cloudflare edge network.
