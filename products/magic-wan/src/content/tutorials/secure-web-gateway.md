...
In this tutorial, you will learn how to configure the Anycast GRE on-ramp to Magic WAN, which connects to Secure Web Gateway (SWG), from enterprise site routers. 

## Prerequisites

Before you can configure the Anycast GRE on-ramp to Magic WAN, the following items should already be completed:
...
- Use site routers that support Anycast GRE and Policy-based Routing (PBR) so that specific Internet-bound traffic from the sites' private networks can be routed over the Anycast GRE to Magic WAN, and subsequently Secure Web Gateway, to enforce a user's specific web access policies.
...
- Headquarters is assigned a `192.168.0.0/16` network, and Router A is the site router terminating the Anycast GRE
- Branch office is assigned `10.0.1.0/24` network, and Router B is the site router terminating the Anycast GRE
- Data center is assigned with `172.16.0.0/12` network, and Router C is the site router terminating the Anycast GRE

Each site's private network has an on-ramp to Cloudflare's Anycast network using Anycast GRE, and the Cloudflare tunnel endpoint IP address is `192.0.2.10`. The table below summarizes the Anycast GRE configuration and route table entries for the Magic WAN topology.
...
1. On the **Add GRE tunnels** page, fill out the information for the Anycast GRE. 
