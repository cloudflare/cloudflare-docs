...
Google Cloud supports configuring [outbound server policy](https://cloud.google.com/dns/docs/server-policies-overview#dns-server-policy-out) within Cloud DNS. Policies are applied per Virtual Private Cloud (VPC) network, and will apply to all resources within that VPC network, including any existing virtual machines.
...
1. Provide a name for your Policy (such as `cloudflare-1-1-1-1`), and select the VPC network or networks that this policy applies to.
