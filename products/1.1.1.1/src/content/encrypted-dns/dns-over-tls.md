...
Cloudflare supports DNS over TLS on `1.1.1.1` and `1.0.0.1` on port 853. The resolver can also be reached by hostname on 1`dot1dot1dot1.cloudflare-dns.com` and `one.one.one.one`. The certificate presented is for `cloudflare-dns.com`. A stub resolver (the DNS client on a device that talks to the DNS resolver) connects to the resolver over a TLS connection:
