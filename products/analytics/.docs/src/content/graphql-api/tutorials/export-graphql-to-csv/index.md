...
This tutorial assumes that you already have a Cloudflare API token for authentication to the Analytics GraphQL API (`Account Analytics: read` permission). If you do not already have one, refer to [Configure an Analytics API token](/graphql-api/getting-started/authentication/api-token-auth).

The scripts also require Python 3.6 or higher.
...
GraphQL also requires a query to be a single-line text,
therefore we should remove all newline symbols before sending a query.
