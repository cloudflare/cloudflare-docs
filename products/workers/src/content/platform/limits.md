...
If a Worker processes a request that pushes the Worker over the 128MB limit, the runtime may return an error. To review whether this error is linked to memory limitation, go to [**Workers**](https://dash.cloudflare.com/?to=/:account/workers) on the Cloudflare dashboard > **Manage Workers** > select the Worker you would like to investigate > scroll down to **Invocation Statuses** and examine *Exceeded Resources*. 
