...
The example outlined in this guide is just a starting point. There are many Workers [runtime APIs](/runtime-apis) available to manipulate requests and generate responses. For example, you can use [HTMLRewriter](/runtime-apis/html-rewriter) to parse and transform HTML on the fly, use [Cache](/runtime-apis/cache) to retrieve data from and put data into  [the Cloudflare cache](/learning/how-the-cache-works), compute a custom response right from the edge, redirect the request to another service, and so much more.
