...
- Call `cache.delete` within a worker to invalidate the cache for the asset with a matching request variable
  ...
  - Assets purged in this way are only purged locally to the Point of Presence the worker runtime was executed.
...
- In order to purge an asset globally, the standard cache purge options must be used. Based on cache API implementation, not all cache purge endpoints function for purging assets stored by the Cache API.
  ...
  - All assets on a zone can be purged by using the [Purge Everything](https://developers.cloudflare.com/cache/how-to/purge-cache#purge-everything) cache operation. This purge will remove all assets associated with a Cloudflare zone from cache in all datacenters regardless of the method set.
