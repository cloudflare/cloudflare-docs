...
You don’t need to use a wildcard operator to block domains. For example, if you want to block all the subdomains for `example.com` then you only have to block `example.com`. It will not only block dns requests to `example.com` but also all subdomains for `example.com`. You can read more about it on our [policies page](/gateway/reference/policy).
...
Visit the [SafeSearch page](/gateway/getting-started/configuring-policies/safesearch) to see how you can test if SafeSearch is working.

### In what order does Gateway apply the rules inside a policy?
Visit the [Policies page](/gateway/reference/policy) to see in what order Gateway applies its rules inside a policy.
