...
A URL Redirect has a source URL, a target URL, a status code, and some additional parameters that affect the URL matching behavior and the runtime behavior.
...
In the following reference table, the provided URL component examples are based on this URL:

```txt
https://user:password@www.example.com:443/search?q=term#results
```

Component | Supported in source URL | Supported in target URL
---|---|---
...
Query string<br/>(for example, `q=term`) | No | Yes, if _Preserve query string_ is false (optional)
Fragment<br/>(for example, `results`) | No | Yes (optional)

Mass Redirects also support target URLs without an authority component, like the following URL:
