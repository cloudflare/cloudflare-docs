...
Use the [Rulesets API](https://developers.cloudflare.com/firewall/cf-rulesets/rulesets-api) to create URL Rewrite Rules via API. Define the rewrite configuration in the `action_parameters` field. See [URL rewrite examples](/transform/url-rewrite/examples) for example rule definitions.
