...
- **`gravity`** or **`g`**
  - Cropping with `fit=cover` specifies the most important side or point in the image that should not be cropped off.
    ...
    - **`gravity=side`** and **`gravity=XxY`**
      - When used with `side` (`left`, `right`, `top`, `bottom`), specify coordinates on a scale from `0.0` (top or left) to `1.0` (bottom or right), `0.5` being the center. The X and Y coordinates are separated by lowercase `x`. For example, `0x1` means left and bottom, `0.5x0.5` is the center, `0.5x0.33` is a point in the top third of the image.
