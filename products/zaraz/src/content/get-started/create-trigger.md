...
1. In **Match Rule**, define the condition that will activate this trigger. 
    ...
    1. In **Variable name**, input the variable you want as the trigger. For example, `{{ client.__zarazTrack }}` is the name of an event a user sends using `zaraz.track()`. Refer to the [Zaraz event and system properties](/properties-reference) table for a list of variables you can use as triggers. 
