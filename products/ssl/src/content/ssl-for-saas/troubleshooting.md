...
To move a custom hostname back to an Active status, send a PATCH (see "Requesting an immediate validation check" section above) to restart the hostname verification.  A Custom Hostname in a Moved status is deleted after 7 days.
