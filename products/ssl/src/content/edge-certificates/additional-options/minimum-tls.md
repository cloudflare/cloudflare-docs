...
For example, if TLS 1.1 is selected, visitors attempting to connect with TLS 1.0 will be rejected while visitors attempting to connect using TLS 1.1, 1.2, or 1.3 (if enabled) will be allowed to connect.

To configure cipher suites, use the API. [Learn More](https://developers.cloudflare.com/ssl/ssl-tls/cipher-suites).
...
Results on supported TLS versions from tools like SSL Labs are not accurate because such tools do not attempt a full TLS connection to your Cloudflare domain.
...
It is not recommended to set the minimum TLS to 1.3 as this will likely cause issues with search engine crawlers and certain browsers. It should only be set to TLS 1.3 where there is a specific use case.

## Related resources

### More articles on PCI compliance

* [PCI compliance and Cloudflare SSL/TLS](https://support.cloudflare.com/hc/en-us/articles/205043158) - Learn how to configure Cloudflare to meet PCI scan requirements and understand what mitigations Cloudflare has in place for earlier versions of TLS/SSL.

### TLS and PCI compliance
