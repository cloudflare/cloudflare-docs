...
* [Endpoint health checks](/about/health-checks/endpoint/) determine the overall health of inter-network connections by probing an endpoint deep within your network, across active tunnels.

* [Tunnel health checks](/about/health-checks/tunnel) monitor the Generic Routing Encapsulation (GRE) tunnels that route traffic from Cloudflare to your network. Tunnel health check probes originate from within your dedicated network namespaces and typically target your GRE tunnel endpoint. Health check results influence how Cloudflare steers traffic across primary and failover tunnels so that packets transit the healthiest available routes.
