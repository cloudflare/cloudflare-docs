...
Each account has the Network-level DDoS Attack Protection Managed Ruleset enabled by default. This means that you do not need to deploy the Managed Ruleset to the `ddos_l4` phase entry point ruleset explicitly. You only have to create a rule in the phase entry point to deploy the Managed Ruleset if you need to configure overrides.

## Configure an override for the Network-level DDoS Attack Protection Managed Ruleset
...
The Network-level DDoS Attack Protection Managed Ruleset is always enabled. You cannot disable its rules using an override with `"enabled": false`.

You can only define overrides for the Network-level DDoS Attack Protection Managed Ruleset at the account level.

</Aside>

## Example

The following `PUT` example creates a new phase ruleset (or updates the existing one) for the `ddos_l4` phase at the account level. The request includes several overrides to adjust the default behavior of the Network-level DDoS Attack Protection Managed Ruleset. These overrides are the following:

* All rules of the Network-level DDoS Attack Protection Managed Ruleset will have their sensitivity set to `medium`.
...
-d '{
  "description": "Define overrides for the Network-level DDoS Attack Protection Managed Ruleset",
  "rules": [
    {
      "action": "execute",
