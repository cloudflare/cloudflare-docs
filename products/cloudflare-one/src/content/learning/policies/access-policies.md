...
* **Any Access Service Token** - the request will need to present the headers for any [service token](https://developers.cloudflare.com/access/access-service-auth/service-tokens) created for this account.
...
* **Authentication Method** - checks the [multifactor authentication](https://developers.cloudflare.com/access/learning/mfa-requirements) method used by the user, if supported by the identity provider.
