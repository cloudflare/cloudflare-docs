...
You can now use the service token when you create [service auth policies](/policies/zero-trust/). When creating these policies, using the `Service Auth` action will ensure that the identity provider login screen is not required for end users.
