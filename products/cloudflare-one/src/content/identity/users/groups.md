...
A group is a set of rules that can be configured once and then quickly applied across many applications. You can select a group as an attribute in any Policy rule, and all the criteria from the selected group will apply to that application.
...
![Access Groups](../../static/documentation/identity/users/access-groups.png)
...
Group Rules follow the same [logic](/policies/) as rules for zero-trust policies in Access.
