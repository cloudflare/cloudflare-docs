...
Start running a Cloudflare Tunnel with the given UUID or name. You may specify optional flags in the following format:
```sh
...
| `--transport-loglevel value`, `--proto-loglevel value` | Transport logging level (previously called protocol logging level) {`debug`, `info`, `warn`, `error`, `fatal`} (default: `info`) |
