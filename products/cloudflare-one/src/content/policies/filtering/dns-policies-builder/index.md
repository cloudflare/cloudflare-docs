...
You can build DNS policies using [identity-based selectors](/policies/filtering/identity-selectors). These selectors require Gateway with WARP mode to be enabled in the Cloudflare for Teams WARP client and the user to be enrolled in the organization via the WARP client. Please refer to the dedicated section
