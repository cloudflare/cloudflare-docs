...
- **Skip**: `updated` events are dropped (acked, no state/comment change) for draft, closed, or `spam`/`off topic`-labeled PRs (`shouldSkipRecommendationUpdate`). `cleared` events still process so an existing comment is removed on close.
