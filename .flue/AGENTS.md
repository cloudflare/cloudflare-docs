...
- **Event contract**: version `1`, `updated` carries a bounded projection of the recommendation (`ownershipAreas[].suggestedPeople`, `codeownersDeclared`, `fallbackOwners` [only when suggestions are empty], `totalOwners`, `satisfyingApprovers`, `satisfied`, overflow counts); `cleared` carries no payload. The coverage matrix renders CODEOWNERS declared refs per area, falls back to the expanded `fallbackOwners` roster for unsuggested areas, and reports omitted areas when `ownershipAreas.length < areaCount`. Schema + state transitions + rendering live in `lib/reviewer-recommendations.ts` (pure, unit-tested).
