...
- **Dev testing**: `POST /dev/recommendations` (gated by `DOCS_FLUE_INTERNAL_TOKEN`, `x-dev-secret` header) feeds a queue-shaped JSON payload through the same `processRecommendationEvent` path without touching the live queue. Dev builds strip the `queues.consumers` entry (`FLUE_DEV_NO_QUEUE_CONSUMER=1`, set by `flue:dev:wrangler`), so local/remote dev never attaches to the production queue.
...
- **Event contract**: version `1`, `updated` carries a bounded projection of the recommendation (`ownershipAreas[].suggestedPeople`, `codeownersDeclared`, `fallbackOwners` [only when suggestions are empty], `totalOwners`, `satisfyingApprovers`, `satisfied`, overflow counts); `cleared` carries no payload. The action table renders CODEOWNERS declared refs for uncovered areas, falls back to the expanded `fallbackOwners` roster when an uncovered area has no suggestions, and reports omitted areas when `ownershipAreas.length < areaCount`. Schema + state transitions + rendering live in `lib/reviewer-recommendations.ts` (pure, unit-tested).
- **Dedupe**: `computedAt`/`clearedAt` determine event ordering. For an `updated` event, `resultHash`, together with its timestamp, head SHA, and status, identifies an exact replay of the current event; it is not an ordering key. A same-timestamp `updated` event with a different `resultHash` is not applied. Exact replays are state no-ops when the GitHub comment is healthy. In comment mode, they still reconcile the singleton comment: a missing, edited, legacy (hashless), or stale comment is repaired, and a replayed `cleared` event removes any leftover comment. A `cleared` event supersedes prior `updated` state for the same artifact; an `updated` whose `headSha` no longer matches the current PR head is skipped. Every accepted newer `updated` event still advances and persists the latest event envelope in R2 as a heartbeat. GitHub create/PATCH is skipped only when the display-relevant recommendation projection is unchanged and the actual existing body still matches the last successfully written body hash. The display comparison excludes envelope-only or non-rendered fields such as head SHA, timestamps, roles, warnings, and overflow warning metadata.
- **Skip**: `updated` events are dropped (acked, no state change, no comment create/update) for draft, closed, and `spam`/`off topic`-labeled PRs. In comment mode, `updated` events from authors in `NO_RECOMMENDATION_COMMENT_AUTHOR_TEAMS` (live team membership via `GITHUB_ORG_TOKEN`, no static roster) are also dropped — the coverage comment is skipped entirely; in log mode those events still apply to R2 state (no comment is ever posted). For any skipped PR, an existing comment, if any, is removed in comment mode. `cleared` events still process so an existing comment is removed on close.
- **State**: R2 `diffs/pr-<n>/reviewer-recommendations.json` holds the current event, head SHA, result hash, status, latest + last-good results, comment id, the logins already @-mentioned, `commentBodyHash`, and `commentDisplayHash`. `commentDisplayHash` binds the comment to the projection it represents, which prevents stale comments after recommendation state advances in log mode. `cleared` leaves a tombstone so delayed duplicates cannot resurrect the comment. State writes use the read-time ETag for conditional writes; a concurrent update throws so the queue event is retried.
- **Comment**: one `## Review coverage` comment per PR, located by the `cloudflare-docs-flue-reviewer-recommendations` marker. It leads with outstanding approval work, lists uncovered areas in an action table, collapses covered areas into a minimal approval audit, and provides the exact matched CODEOWNERS patterns and declared owners for displayed ownership areas in a separate collapsed table. Owner refs stay inside code spans so the diagnostic table cannot notify users or teams. Only newly appearing suggestions for uncovered areas are @-mentioned, once per PR; covered-area suggestions are neither listed nor notified. Rules in `CODEOWNERS_ONLY_CONTACT_PATTERNS` (`lib/reviewer-contact-policy.ts`, currently the `*` default rule plus `/.github/CODEOWNERS` and `/public/__redirects`) suppress their suggestions from both mentions and display, showing the complete declared CODEOWNERS rule as display-only contacts instead. Area labels are the raw docs product-directory slug from the event (e.g. `workers`, `workers-ai`); areas outside the docs content roots render as `Other` with their CODEOWNERS pattern shown. The `@`-mention line and the action table's `Suggested` column are the only notification surface; no "suggestions are informational" footer is appended. An `error` result renders the last good snapshot with a generic warning; a `fallback` result renders its current bounded coverage. Raw corpus warnings/errors are never published. `cleared` deletes the comment.
