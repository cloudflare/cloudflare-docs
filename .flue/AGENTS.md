...
Each agent is a `"use agent"` module whose default-export function uses hooks (`useModel`, `useSkill`, `useInitialData`, `useTool`, `useDataWriter`, `useAgentFinish`) and sets `.agentName`. The framework generates one Durable Object class per agent, named **`Flue<PascalCase(agentName)>Agent`**. Per-run data (GitHub token, PR context) arrives as `initialData`; tools that need it are built inside the render from that input at a fixed length so the hook order is stable.
