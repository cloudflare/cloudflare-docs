...
- **Skip**: `updated` events are dropped (acked, no state/comment change) for draft, closed, `spam`/`off topic`-labeled PRs, and PRs whose author is a member of `NO_RECOMMENDATION_COMMENT_AUTHOR_TEAMS` (live team membership via `GITHUB_ORG_TOKEN`, no static roster) — the coverage comment is skipped entirely for those authors, and any existing one is removed. `cleared` events still process so an existing comment is removed on close.
