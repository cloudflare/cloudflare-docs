...
- If a ` ```toml ` fence containing Wrangler config fields (`name =`, `compatibility_date =`, `[vars]`, `[[kv_namespaces]]`, etc.) → use `<WranglerConfig>` instead of a bare fence.
- If `compatibility_date` inside `<WranglerConfig>` is a hardcoded date string → use `$today` instead — it is replaced with the current date at build time.
