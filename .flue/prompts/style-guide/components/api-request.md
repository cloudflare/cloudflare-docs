...
- If documenting a Cloudflare API endpoint with a raw `curl` example instead of `<APIRequest>` → use `<APIRequest>` for Cloudflare API endpoints to get auto-generated auth tokens and consistent formatting.
