...
- If a file has a `pcx_content_type` set and no `description:` field → `description` is required when `pcx_content_type` is set.
...
- If `pcx_content_type` value is not in the above list → use a valid value.
...
- If `sidebar.label:` contains an emoji → remove it.
