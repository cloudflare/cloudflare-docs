...
- Reusable components must be added to the `~/components` barrel export (`src/components.ts`) and imported from `~/components`. If a reusable component is imported via a deep path instead of from the barrel → report it.
- Page-specific wrapper components or one-off components may be imported via a deep path (e.g. `~/components/BaseSchemaProperties.astro`) instead of adding to the barrel. Deep imports for these components are allowed and should not be reported.
