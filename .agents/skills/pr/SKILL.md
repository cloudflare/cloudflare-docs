...
Use this skill only for pull requests in `github.com/cloudflare/cloudflare-docs`. Before using `gh` or a GitHub URL, run `git remote get-url origin` and confirm it returns `https://github.com/cloudflare/cloudflare-docs.git` or `git@github.com:cloudflare/cloudflare-docs.git`. If it returns any other value, stop using this skill immediately and create the pull request or merge request for that origin.
