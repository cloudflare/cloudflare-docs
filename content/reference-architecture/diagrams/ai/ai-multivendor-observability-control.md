...
2. **Request proxying**: Forward POST request to AI Inference provider or serve response from [cache, if enabled and available](/ai-gateway/configuration/caching). Both [analytics](/ai-gateway/observability/analytics/) and [logs](/ai-gateway/observability/analytics/#logging) are collected in the process and controls such as Rate Limiting enforced.
