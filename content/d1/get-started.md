...
- Your binding is available in your Worker at `env.<BINDING_NAME>` and the D1 [client API](/d1/manage-databases/query-databases/) is exposed on this binding.

{{<Aside type="note">}}

When you execute the `wrangler d1 create` command, the client API package (which implements the D1 API and database class) is automatically installed. For more information on the D1 Client API, refer to [D1 Client API](/d1/manage-databases/query-databases/).
...
2. Query your database using `env.DB.prepare` to issue a [prepared query](/d1/manage-databases/query-databases/) with a placeholder (the `?` in the query).
...
- Learn how to use the [D1 client API](/d1/manage-databases/query-databases/) within your Worker.
