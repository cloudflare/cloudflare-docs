...
Cloudflare follows the [SQLite convention](https://www.sqlite.org/lang_expr.html#varparam) for prepared statements parameter binding. Currently, Cloudflare only supports Ordered (?NNNN) and Anonymous (?) parameters. In the future Cloudflare will support named parameters as well.
