...
Cloudflare allows specifying the following media-ranges in the OpenAPI request body content map: `*/*`, `application/*`, and `application/json`. Further, media-ranges can also be configured to enforce a `charset` parameter. For this, we only accept the `charset` parameter with a static value of `utf-8` as part of the media-range specification and, when configured, Cloudflare will similarly require the request's content-type to carry this charset.
