...
To bind your KV namespace to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#kv-namespaces) or the dashboard:
...
While developing locally, to interact with a Durable Object namespace, run the Worker exporting the Durable object via `wrangler dev` and in another terminal, run `wrangler pages dev`.

If you set up your binding via `wrangler.toml` you can interact with your binding as demonstrated above.

You can also interact with your Durable Object namespace locally via the Wrangler CLI with `--do <BINDING_NAME>=<CLASS_NAME>@<SCRIPT_NAME>` where `CLASS_NAME` indicates the Durable Object class name and `SCRIPT_NAME` the name of your Worker. For example, if your Worker is called `do-worker` and it declares a Durable Object class called `DurableObjectExample`, access this Durable Object by running your `do-worker` via `npx wrangler dev` (in the Worker's directory) alongside `npx wrangler pages dev <OUTPUT_DIR> --do MY_DO=DurableObjectExample@do-worker` (in the Pages' directory). Interact with this binding by using `context.env` (for example, `context.env.MY_DO`).
...
To bind your R2 bucket to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#r2-buckets) or the dashboard::
...
By default, Wrangler automatically persists data to local storage. For more information read about [local development](/workers/testing/local-development/).

{{</Aside>}}

You can also interact with an R2 bucket via the Wrangler CLI by adding `--r2=<BINDING_NAME>` to your run command. If your bucket is bound to `BUCKET`, access this bucket in local dev by running `npx wrangler pages dev <OUTPUT_DIR> --r2=BUCKET`. Interact with this binding by using `context.env` (for example, `context.env.BUCKET`).
...
To bind your D1 database to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#d1-databases) or the dashboard::
...
While [developing locally](/d1/build-with-d1/local-development/#develop-locally-with-pages), you can also interact with a D1 database via the Wrangler CLI by adding `--d1 <BINDING_NAME>=<DATABASE_ID>` to your run command.
...
To bind Workers AI to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#workers-ai) or the dashboard:
...
Workers AI can be tested locally, but the implementation relies on remote resources and will count against your reported usage.
...
To add a service binding to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#service-bindings) or the dashboard::
...
To interact with a [service binding](/workers/configuration/bindings/about-service-bindings/) while developing locally, run the Worker you want to bind to via `wrangler dev` and in another terminal, run `wrangler pages dev`.

If you set up your binding via `wrangler.toml` you can interact with your binding as demonstrated above.

If you set up the binding via the dashboard, you will need to append `wrangler pages dev` with `--service <BINDING_NAME>=<SCRIPT_NAME>` where `SCRIPT_NAME` indicates the name of the Worker. For example, if your Worker is called `my-worker`, connect with this Worker by running it via `npx wrangler dev` (in the Worker's directory) alongside `npx wrangler pages dev <OUTPUT_DIR> --service MY_SERVICE=my-worker` (in the Pages' directory). Interact with this binding by using `context.env` (for example, `context.env.MY_SERVICE`).
...
To add a Queue producer binding to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#queue-producers) or the dashboard::
...
To add an Analytics Engine binding to your Pages Function you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#analytics-engine) or the dashboard::
...
To add Pages project environment variables you can use [`wrangler.toml`](/pages/functions/wrangler-configuration/bindings/#environment-variables) or the dashboard::
...
If you set up your binding via `wrangler.toml` you can interact with your environment variables as demonstrated above.
