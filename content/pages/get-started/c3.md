...
Running this command will prompt you to install the [`create-cloudflare`](https://www.npmjs.com/package/create-cloudflare) package, and lead you through a setup wizard. After you have entered the setup wizard, you will be asked which type of application you would like to create.
...
{{<tab label="npx" default="true">}}
```sh
$ npx create-cloudflare@latest [--] [<DIRECTORY>] [OPTIONS] [-- <NESTED ARGS...>]
```
{{</tab>}}
{{<tab label="yarn">}}
```sh
...
- `--type` {{<type>}}string{{</type>}} {{<prop-meta>}}optional{{</prop-meta>}}
  ...
  - The possible values for this option are:
    ...
    - `hello-world-durable-object`: A basic "Hello World" Cloudflare Worker.
    ...
    - `pre-existing`: Fetch a Worker initialized from the dashboard.
...
- `--template` {{<type>}}string{{</type>}}
  - A git repository used to create a new project. At a minimum, template folders must contain the following:
