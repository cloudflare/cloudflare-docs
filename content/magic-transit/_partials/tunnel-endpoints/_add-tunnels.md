...
11. _(Optional)_ Enable **Tunnel health checks** if you want to use this feature. If you do not enable Tunnel health checks, your tunnels will appear 100% down in your [tunnel health dashboard]($7) even when working. Cloudflare will keep sending traffic through the tunnel, without the means to detect if the tunnel goes down. You will have to set up your own system to detect down tunnels, as Cloudflare will not be able to warn you about down tunnels.
...
10. _(Optional)_ Enable **Tunnel health checks** if you want to use this feature. If you do not enable Tunnel health checks, your tunnels will appear 100% down in your [tunnel health dashboard]($7) even when working. Cloudflare will keep sending traffic through the tunnel, without the means to detect if the tunnel goes down. You will have to set up your own system to detect down tunnels, as Cloudflare will not be able to warn you about down tunnels.
...
16. (Optional) Enable **Replay protection** if you have devices that do not support disabling it. Refer to [Anti-replay protection]($5) for more information.
