...
- **Interface address**: A 31-bit (recommended) or 30-bit subnet (`/31` or `/30` in CIDR notation) supporting two hosts, one for each side of the tunnel. Select the subnet from the following private IP space:
  ...
  - `192.168.0.0/16`
  - $1
  {{<Aside type="warning">}}For AWS and other cloud service providers that automatically generate the tunnel endpoint prefixes, make sure the prefixes are within the allowed `169.254.240.0/20` range. If the prefixes fall outside this range, override them or the tunnel will not work.{{</Aside>}}
