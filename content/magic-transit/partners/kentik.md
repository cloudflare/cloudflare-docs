...
In this tutorial, the example scenario includes two mitigations, one which pulls the advertisement from the router and a second mitigation that makes an API to Cloudflare to begin advertising the prefixes from Cloudflare's edge.

## Prerequisites

You will need the email address associated with your Cloudflare account and Cloudflare Account ID to configure the connection for Magic Transit in Kentik.
...
5. Click **Edit** next to the Cloudflare branded mitigation to edit and review the information.
    ...
    In the example below, this mitigation information is used to send the API call to Cloudflare to begin advertising routes and turn on Magic Transit for the customer's network.
...
For a detailed view of actions taken and attack types, you can use the **Network Analytics** dashboard.
