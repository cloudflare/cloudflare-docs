...
Public LoRAs will have the name `cf-public-x`, and the prefix will be reserved for Cloudflare. Have more LoRAs you would like to see? Let us know on [Discord](https://discord.cloudflare.com).
...
---
curl --request POST \
  --url https://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/ai/run/@cf/mistral/mistral-7b-instruct-v0.1 \
  --header 'Authorization: Bearer ${CF_TOKEN}' \
  --data '{
    "messages": [{"role": "user", "content": "Write a python program to check if a number is even or odd."}],
