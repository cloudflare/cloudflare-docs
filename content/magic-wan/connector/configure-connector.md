...
- **UDP/53 (DNS destination IP)**: Needed to allow `1.1.1.1` DNS traffic to Cloudflare DNS servers. Cloudflare uses this port for DNS lookups of control plane API endpoints.
- **TCP/443**: The Connector will open outbound HTTPS connections over this port for control plane operations.
- **UDP/4500 (destination IP)**: Needed for Connector's initialization and discovery traffic through `162.159.64.1` outbound connections. 
- **UDP/4500 (destination IP)**: Cloudflare Anycast IPs assigned to customer for tunnel outbound connections. This traffic is tunnel traffic.
