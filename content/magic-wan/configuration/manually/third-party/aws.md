...
5. In **Inside IPv4 CIDR**, choose the IP address for internal IPsec tunnel interface. For this example, we will use `169.254.0.0/16` as the CIDR block for the IPsec tunnel: `169.254.0.1` will be the IP address for the AWS side of the tunnel, and `169.254.0.2` will be the IP for the Cloudflare side of the tunnel.
{{<Aside type="warning">}}AWS automatically generate tunnel endpoint prefixes outside the CIDR supported by Cloudflare. Make sure you override them to within the allowed `169.254.240.0/20` range, or the tunnels will not work.{{</Aside>}}
...
1. Refer to [Add tunnels](/magic-wan/configuration/manually/how-to/configure-tunnels/#add-tunnels) to learn how to add an IPsec tunnel. When creating your IPsec tunnel, make sure you define the following settings:
    ...
    - **Interface address**: The IPv4 CIDR block you have set up above.
