...
Alerts are sent via the [Cloudflare Notifications](/fundamentals/notifications/) system. They can be sent via email or webhook. When subscribed to job disablement notification, you will receive at most one alert per job per 24h. The notification email contains the job ID and destination configuration.
