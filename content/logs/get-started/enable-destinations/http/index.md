...
Note that the `ownership_challenge` parameter is not required to create a Logpush job to an HTTP endpoint. You need to make sure that the file upload to validate the destination it's accepting a gzipped `test.txt.gz` with content as {content:"tests"} compressed, otherwise will give an error like `error validating destination: error writing object: error uploading`.
{{</Aside>}}
