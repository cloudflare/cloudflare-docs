...
If you have the Cloudflare Web Application Firewall (WAF) turned on, you may see a interactive challenge when Cloudflare makes a request to Splunk HTTP Event Collector (HEC). To make sure this does not happen, you have to create a firewall rule that allows Cloudflare to bypass the HEC endpoint.
