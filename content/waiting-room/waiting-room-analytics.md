...
3. Expand the waiting room you would like to review metrics for and a preview of your waiting room analytics will be displayed. The preview gives you insights into peak traffic through your waiting room over the last 24 hours including: Maximum active users, Maximum queued users and Typical time in queue for queued users.
...
Time in queue summary values give you an insight into the user experience by indicating how long queued users spent waiting to enter your application. The time waited for the typical user, as well as for those who waited the longest are displayed for the time period you have selected. These values are an indicator of the impact your waiting room settings combined with the traffic to your waiting room had on wait times.
...
Time on origin summary values estimate how long users spent on the pages covered by your waiting room before leaving. For the time period selected, you will have access to the estimated time spent on origin for the typical user, as well as the time on origin for those who spend the most time on your site. Please keep in mind that if your session renewal is disabled and there is no active queueing, users are issued a new waiting room session every session_duration minutes.  Therefore, these users may be staying for multiple sessions.  The time on origin for these users restarts each time a session expires.
...
- Your top 5% time on origin is <70% of your configured session duration.
...
To identify and hone in on peak traffic, select a longer time period, such as 30 days. Then, drag your cursor to the left and right of any time period you would like to see with more granularity to zoom in. You can zoom in until each bar represents a one minute interval. All other metrics on the page will update automatically to reflect the data behind the time period selected.

To see more details about a particular moment in time, hover over a bar on the graph to display a tooltip which will indicate the following for the time period that bar represents:
...
<details>
<summary>Query to fetch the values for total active users and new users per minute over a certain period.</summary>
<div>
...
<details>
<summary>Query to find the average of total active users and new users per minute over a certain period and aggregate this data over a period of 15 minutes.</summary>
<div>

This query calculates the average of total active users and new users per minute. The time dimension in the query is 15 minutes, therefore the data is aggregated over fifteen minutes for the selected time period.
...
<details>
<summary>Query to find the weighted averages of time on origin (50th percentile) and total time waited (90th percentile) for a certain period and aggregate this data over one hour.</summary>
<div>
...
- Your waiting room was not receiving any traffic for the time period you are viewing.
- Your waiting room was not enabled for the time period you are viewing.
