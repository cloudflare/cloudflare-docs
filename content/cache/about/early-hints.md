...
- If you notice a influx of 504s with the `RequestSource` of `earlyHintsCache`, that is an expected behavior. These are not user requests, and they do not go to the customer origin - they are internal Early Hints subrequests. Whether they hit or miss (200 or 504, respectively) indicates only whether there are cacheable Early Hints at the request URI. These requests are already filtered out in other views such as Cache Analytics. For more information about how to filter requests by end users of your website, please refer to [Filter end users](https://developers.cloudflare.com/analytics/graphql-api/features/filtering/#filter-end-users).
