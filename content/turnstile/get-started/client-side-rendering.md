...
| `timeout-callback` | `data-timeout-callback` | A JavaScript callback invoked when the challenge presented an interactive challenge but was not solved within a given time. A callback should reset the widget to allow a visitor to solve the challenge again. |
