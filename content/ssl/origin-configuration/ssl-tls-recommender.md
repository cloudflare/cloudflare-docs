...
To adjust your SSL/TLS Recommender enrollment with the API, send a [`PATCH`](https://developers.cloudflare.com/api/operations/zone-settings-change-ssl/tls-recommender-enrollment) request with the `enabled` parameter set to your desired setting (`true` or `false`).
