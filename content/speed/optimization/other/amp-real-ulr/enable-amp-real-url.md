...
* AMP Real URL relies on signed exchanges so all [requirements for signed exchanges must be met](/speed/signed-exchanges/enable-signed-exchange/).
...
Since AMP Real URL responses are packaged in a signed exchange (SXG), the [limitations for SXGs apply](/speed/signed-exchanges/signed-exchanges-caveats/).