...
![Rate limiting example with four requests where one of the requests is being rate limited. For details, keep reading.](https://developers.cloudflare.com/waf/static/custom-rules/rate-limiting-example.png)
...
![Rate limiting example with four requests where the rate limiting rule uses a response field (the HTTP response code) in the counting expression. For details, keep reading.](https://developers.cloudflare.com/waf/static/custom-rules/rate-limiting-example-response-field.png)
