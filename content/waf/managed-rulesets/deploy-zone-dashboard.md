...
![Example Managed Rules configuration in the Managed rules tab under Security > WAF. There is a configured rule for executing the Cloudflare Managed Ruleset, and there are two Managed Rulesets available for deployment.](https://developers.cloudflare.com/waf/static/waf-managed-rules-tab.png)
...
4. Under **Ruleset configuration**, set one or more rule fields from the available values in the drop-down lists.
    ...
    ![The Configure Managed Ruleset page displaying the available options to override all the rules in the ruleset. In the displayed Managed Ruleset you can override the ruleset action.](https://developers.cloudflare.com/waf/static/waf-configure-ruleset.png)
...
4. Click **Browse rules**.
    ...
    ![The Browse rules page displaying the list of rules in the Cloudflare Managed Ruleset](https://developers.cloudflare.comhttps://developers.cloudflare.com/waf/static/waf-browse-rules.png)
...
1. Enter search terms in the available input to find the rules you want to configure. You can search for tags.
    ...
    ![Example of filtering rules by the wordpress tag in the Browse rules page](https://developers.cloudflare.com/waf/static/waf-selected-tag.png)
...
3. Update one or more fields for the selected rules using the drop-down lists displayed in the top right corner of the table.
    ...
    ![Selecting all rules in a page displays additional drop-down lists above the table to override the behavior of several rules at once.](https://developers.cloudflare.com/waf/static/waf-modify-selected-rules.png)
