---
title: 2024-02-05
type: table
pcx_content_type: changelog
weight: 839
layout: wide
---

# 2024-02-05

{{<table-wrap>}}
<table style="width: 100%">
  <thead>
    <tr>
      <th>Ruleset</th>
      <th>Rule ID</th>
      <th>Legacy Rule ID</th>
      <th>Description</th>
      <th>Previous Action</th>
      <th>New Action</th>
      <th>Comments</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}5fca26a83ed84ab9addb34dd52d6027b{{</rule-id>}}</td>
      <td>100624</td>
      <td>GoAnywhere - Auth Bypass - CVE:CVE-2024-0204</td>
      <td>N/A</td>
      <td>Block</td>
      <td>N/A</td>
    </tr>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}4d825bf2e3ae49baa2061bc7f89ab164{{</rule-id>}}</td>
      <td>100626,100626A</td>
      <td>Anomaly:Header:Content-Type - Multiple</td>
      <td>N/A</td>
      <td>Disabled</td>
      <td>New Detection</td>
    </tr>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}07e32601bfb9468abbae99ab7736c63c{{</rule-id>}}</td>
      <td>N/A</td>
      <td>AngularJS - XSS</td>
      <td>N/A</td>
      <td>Block</td>
      <td>This detection was released as ...014fc5b9 (BETA) in new WAF</td>
    </tr>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}75a0060762034a6cb663fd51a02344cb{{</rule-id>}}</td>
      <td>N/A</td>
      <td>Apache HTTP Server - Server-Side Includes</td>
      <td>N/A</td>
      <td>Disabled</td>
      <td>This detection was released as ...10cae4a8 (BETA) in new WAF</td>
    </tr>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}935a66f12be347dfa41f5d37af52d528{{</rule-id>}}</td>
      <td>N/A</td>
      <td>Command Injection - CVE:CVE-2014-6271</td>
      <td>N/A</td>
      <td>Block</td>
      <td>This detection was released as ...d2a0991c (BETA) in new WAF</td>
    </tr>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}f4a310393c564d50bd585601b090ba9a{{</rule-id>}}</td>
      <td>N/A</td>
      <td>Command Injection - Nslookup</td>
      <td>N/A</td>
      <td>Block</td>
      <td>This detection was released as ...da3d944c (BETA) in new WAF</td>
    </tr>
    <tr>
      <td>Cloudflare Specials</td>
      <td>{{<rule-id>}}2fe273498e964f0bb20ac022d5a14a5e{{</rule-id>}}</td>
      <td>N/A</td>
      <td>Microsoft ASP.NET - Code Injection</td>
      <td>N/A</td>
      <td>Disabled</td>
      <td>This detection was released as ...70f4f073 (BETA) in new WAF</td>
    </tr>
  </tbody>
</table>
{{</table-wrap>}}