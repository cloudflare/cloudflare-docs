...
Since the value for Consent Mode may change per user or session, it's recommended to dynamically set this value using `zaraz.set` in your website code. For instance, use `zaraz.set("google_consent", "G100")` on page load, and `zaraz.set("google_consent", "G111")` after the user granted consent for Marketing and Analytics. In the **Consent Mode** field, click the **+** symbol, choose **Event Property**, and type `google_consent` as the property name. Zaraz will then use the latest value of the `google_consent` Event Property as the Consent Mode string.
