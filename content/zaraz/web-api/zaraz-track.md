...
Note that name of the event (`purchase` in the above example), the names of the keys (`value` and `currency`) and the number of keys are completely up to you. You choose what and how you want to track these variables. However, picking meaningful names will help you when you configure your triggers, because the trigger configuration has to match the events your website is sending.

After using `zaraz.track()` in your website to track an action:

1. Create a trigger that matches your [`zaraz.track()` call](/zaraz/get-started/create-trigger), with `{{ client.__zarazTrack }}` in your trigger's variable name, and the `eventName` you are tracking in **Match string**. Following the above example, you would input `purchase` in **Match string**.
...
3. Each action that uses this trigger can also access the `eventProperties` you have sent. In the Action fields, you can use `{{ client.<KEY_NAME> }}` to get the the value of `<KEY_NAME>`. In the above example, Zaraz will replace `{{ client.currency }}` with `200`.
