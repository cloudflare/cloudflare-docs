...
When you set up **allowed patterns**, Cloudflare Cloud Email Security (formerly Area 1) email security exempts messages that match certain patterns from normal detection scanning.
...
5. Enter the pattern information:
    ...
    - **Allow Type**: Choose one or more of the following types:
        ...
        - **Trusted Sender**: Messages will bypass all [detections](/email-security/reference/dispositions-and-attributes/) and link following by Cloud Email Security (formerly Area 1). Typically, only applies to {{<glossary-tooltip term_id="phishing">}}phishing{{</glossary-tooltip>}} simulations from vendors such as KnowB4.
        - **Exempt Recipient**: Will exempt messages from all Cloud Email Security (formerly Area 1) [detections](/email-security/reference/dispositions-and-attributes/) intended for recipients matching this pattern (email address or regular expression only). Typically, this only applies to submission mailboxes for user reporting to security.
