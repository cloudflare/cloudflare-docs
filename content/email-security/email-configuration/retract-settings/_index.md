...
When you are using an [API setup](/email-security/deployment/api/) for Cloud Email Security, you cannot prevent mail from reaching a recipient's mailbox.
...
Setting up retraction also gives you access to metrics for this feature. After logging in to your [Cloud Email Security dashboard](https://horizon.area1security.com), search for the **Retractions** card. Metrics for retractions include information such as:

- **Total retractions**: Displays the total amount of retractions performed.
- **Success**: Shows the percentage of messages Cloud Email Security was able to find and retract successfully.
- **Fail**: Displays the percentage of messages Cloud Email Security was not successfully able to retract. Reasons for failure include:
...
- **Unread/Read**: Refers to the state of the message at the time it was retracted. For automated retractions, Cloud Email Security tries to perform retraction as quickly as possible so the user has no time to see or open the message. Manual retraction might happen at a later time, and so the messages are more likely to have already been read.
