...
Create actions for emails with specific [dispositions](/email-security/reference/dispositions-and-attributes/). For example, you can choose to defang all URLs in emails with a `MALICIOUS` disposition, with the option `URL defang`. This means that every URL in an email of the selected type will be rewritten so that they are not selectable. For example, `https://www.example.com` will become `https[:]//www[.]example[.]com`.
