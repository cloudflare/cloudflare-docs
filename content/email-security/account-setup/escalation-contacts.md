...
Whenever Cloud Email Security (formerly Area 1) finds an exceptional {{<glossary-tooltip term_id="phishing">}}phishing{{</glossary-tooltip>}} threat or Email Service irregularity behavior (compromised email servers at a partner or vendor, wire fraud tactics, and more), we try to reach out to our customers.
...
Cloud Email Security (formerly Area 1) will start by reaching out to P1-level contacts. If they do not respond, we will then try reaching out to the other contacts down the list until we receive a reply from one of these groups.
