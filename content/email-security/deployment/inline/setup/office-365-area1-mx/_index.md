...
![A schematic showing where Cloud Email Security (formerly Area 1) security is in the life cycle of an email received](/images/email-security/deployment/inline-setup/o365-area1-mx/office365-mx.png)

In this tutorial, you will learn how to configure Microsoft Office 365 with Cloud Email Security (formerly Area 1) as its MX record. This tutorial is broken down into several steps. If at any steps during this tutorial you receive a message saying that you need to run the `Enable-OrganizationCustomization` cmdlet, [refer to section 6](#6-execute-enable-organizationcustomization-if-required).
...
## 1. Add Cloud Email Security (formerly Area 1) IP addresses to Allow List
...
6. Set the following options:
    - **Name** - `Cloud Email Security (formerly Area 1) Inbound Connector`
...
9. Enter all of the egress IPs in the [Egress IPs](/email-security/deployment/inline/reference/egress-ips/) page.
    ...
    ![Enter all of Cloud Email Security (formerly Area 1)'s Egress IPs](/images/email-security/deployment/inline-setup/o365-area1-mx/step9-egress-ips.png)
...
2. Select the `Cloud Email Security (formerly Area 1) Inbound Connector` that you configured previously to edit its configuration parameters.
...
## 3. Configure Cloud Email Security (formerly Area 1) quarantine policies
...
{{<Aside type="note">}}When Cloud Email Security (formerly Area 1) is deployed as the MX record and protecting Office 365, `MALICIOUS` and `SPAM` detections will automatically be quarantined. This behavior cannot be modified.{{</Aside>}}
...
There may be scenarios where use of the Office 365 (O365) email quarantine or a combination with Cloud Email Security (formerly Area 1) is preferred. The following are the best practices for using the O365 quarantine [by disposition](/email-security/reference/dispositions-and-attributes/):
...
The Cloud Email Security (formerly Area 1) dashboard has an [Admin quarantine](/email-security/email-configuration/admin-quarantine/), and you can also use the Office 365 quarantine for when a user quarantine is needed. While there are many quarantine options, the following are the primary use cases the Office 365 [example tutorials](/email-security/deployment/inline/setup/office-365-area1-mx/use-cases/) will cover:

- **Use case 1**: Deliver emails to Office 365 junk email folder and Admin Quarantine in Cloud Email Security (formerly Area 1) (Recommended)
- **Use case 2**: Deliver emails to junk email folder and user managed quarantine (this use case requires that `MALICIOUS` emails be quarantined within the Cloud Email Security (formerly Area 1) dashboard)
...
Instructions to update your MX records will depend on the DNS provider you are using. You will need to update and replace your existing MX record with the Cloud Email Security (formerly Area 1) hosts. For example:
...
One method of DNS attacks is to search for old MX records and send {{<glossary-tooltip term_id="phishing">}}phishing{{</glossary-tooltip>}} emails directly to the mail server. To secure the email flow, you will want to enforce an email flow where inbound messages are accepted by Office 365 only when they originate from Cloud Email Security (formerly Area 1). This can be done by adding a connector to only allow email from Cloud Email Security (formerly Area 1) with TLS encryption. This step is optional but recommended.

{{<Aside type="warning" header="Important">}}
This step should not be performed until 24 hours after all domains (excluding your `<on_microsoft.com>` domain) in your Office 365 organization have been onboarded to Cloud Email Security (formerly Area 1), and Cloud Email Security (formerly Area 1) is their MX record. If a domain has not been onboarded or DNS is still propagating, you will impact production email flow for that domain.
{{</Aside>}}
