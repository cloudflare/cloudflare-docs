...
![A schematic showing where Cloud Email Security (formerly Area 1) security is in the life cycle of an email received](/images/email-security/deployment/inline-setup/cisco-cisco-mx/cisco-mx.png)

In this tutorial, you will learn how to configure Cloud Email Security (formerly Area 1) email security with Cisco as MX record. This tutorial is broken down into several steps.

## 1. Add a Sender Group for Cloud Email Security (formerly Area 1) Email Protection IPs
...
3. Configure the new Sender Group as follows:
    ...
    * **Comment**: `Cloud Email Security (formerly Area 1) Email Protection egress IP Addresses`.
...
## 2. Add {{<glossary-tooltip term_id="SMTP">}}SMTP{{</glossary-tooltip>}} route for the Cloud Email Security (formerly Area 1) Email Protection Hosts
...
3. Configure the new SMTP Route as follows:
    ...
    * In **Destination Hosts**, select **Add Row**, and add the Cloud Email Security (formerly Area 1) MX hosts. Refer to the [Geographic locations](#5-geographic-locations) table for more information on what MX hosts to use.

![Edit SMTP route](/images/email-security/deployment/inline-setup/cisco-cisco-mx/step2.png)

## 3. Create Incoming Content Filters

To manage the mail flow between Cloud Email Security (formerly Area 1) and Cisco ESA, you need two filters:
* One to direct all incoming messages to Cloud Email Security (formerly Area 1).
* One to recognize messages coming back from Cloud Email Security (formerly Area 1) to route for normal delivery.

### Incoming Content Filter - To Cloud Email Security (formerly Area 1)
...
3. Configure the new Incoming Content Filter as follows:
    * **Name**: `ESA_to_A1S`
    * **Description**: `Redirect messages to Cloud Email Security (formerly Area 1) for anti-phishing inspection`
...
### Incoming Content Filter - From Cloud Email Security (formerly Area 1)
...
3. Configure the new Incoming Content Filter as follows:
    * **Name**: `A1S_to_ESA`
    * **Description**: `Cloud Email Security (formerly Area 1) inspected messages for final delivery`
