...
![A schematic showing where Cloud Email Security (formerly Area 1) security is in the life cycle of an email received](/images/email-security/deployment/inline-setup/cisco-area1-mx/cisco-area1-mx.png)

In this tutorial, you will learn how to configure Cisco IronPort with Cloud Email Security (formerly Area 1) as MX record. This tutorial is broken down into several steps.
...
You need to configure the Incoming Relays section to tell IronPort to ignore upstream hops, since all the connections are now coming from Cloud Email Security (formerly Area 1). This step is needed so the IronPort can retrieve the original IPs to calculate IP reputation. IronPort also uses this information in the Anti-Spam (IPAS) scoring of messages.
...
Make sure you disable Sender Policy Framework (SPF) checks in IronPort. Because Cloud Email Security (formerly Area 1) is acting as the MX record, if you do not disable SPF checks, IronPort will block emails due to an SPF failure.
...
Instructions to update your MX records will depend on the DNS provider you are using. In your domain DNS zone, you need to replace your current MX records with the Cloud Email Security (formerly Area 1) hosts. This will have to be done for every domain where Cloud Email Security (formerly Area 1) will be the primary MX. For example:
