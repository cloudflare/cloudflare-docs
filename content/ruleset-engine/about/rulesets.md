...
A ruleset is an ordered set of [rules](/ruleset-engine/about/rules/) that you can apply to traffic in the Cloudflare global network. Rulesets belong to a phase and can only execute in the same phase. To deploy a ruleset to a phase, add a rule that executes the ruleset to the [phase entry point ruleset](/ruleset-engine/about/phases/#phase-entry-point-ruleset).
