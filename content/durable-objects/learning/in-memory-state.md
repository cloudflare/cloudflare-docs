...
A given instance of a Durable Object may share global memory with other instances defined in the same script. 
...
The Durable Object's storage has a built-in in-memory cache of its own. If you use `get()` to retrieve a value that was read or written recently, the result will be instantly returned from cache. Instead of writing initialization code like above, you could use `get("value")` whenever you need it, and rely on the built-in cache to make this fast. Refer to the [Counter tutorial](/durable-objects/tutorials/build-a-counter/) for an example of this approach.
