...
WebSockets do not hibernate when using local development environments such as `wrangler dev` or Miniflare. Hibernatable WebSocket events such as `webSocketMessage()` will be delivered, but your Durable Object will never be evicted from memory. Hibernation will be fully supported in local development for GA, but in the meantime, prefer `wrangler dev --remote` or deploy a test/staging namespace to production when debugging state management issues.
{{</Aside>}}
