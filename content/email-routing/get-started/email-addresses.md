...
5. In the **Action** drop-down menu, choose what this rule should do. You can choose between _Send to_ or _Drop_. Refer to [Rules actions](#rules-actions) for more information.
...
Email rules are pairs of custom addresses and destination addresses. To disable a rule:
...
1. Log in to the [Cloudflare dashboard](https://dash.cloudflare.com/) and select your account and domain.
2. Go to **Email** > **Routes**.
3. In **Custom addresses**, identify the rule you want to edit, and click **Edit**.
...
To prevent spam, Email Routing rules do not become active until after the destination address has been verified. Cloudflare sends a verification email to destination addresses specified in Custom addresses. You have to click **Verify email address** in that email to activate a destination address.
