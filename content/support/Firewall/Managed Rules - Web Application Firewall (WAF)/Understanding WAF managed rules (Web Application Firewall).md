...
Review threats blocked via the [Security Events](https://developers.cloudflare.com/waf/analytics/)' **Activity log** available in **Security** > **Events**.
...
-   Managed rules mitigate padding techniques. We recommend the following:
    ...
    2.  Create a firewall rule using the [Expression Editor](https://developers.cloudflare.com/firewall/cf-dashboard/edit-expressions/#expression-editor) depending on the need to check headers and/or body to block larger payload (> 128 KB). Make sure to test your firewall rule in _Log_ mode first as it could be prone to generating false positives.
...
To test for false positives, set WAF managed rules to **Simulate** mode, to record the response to possible attacks without challenging or blocking. Also, use the Firewall Analytics [**Activity log**](https://developers.cloudflare.com/waf/analytics/paid-plans#activity-log) to determine which managed rules caused false positives.
...
-   **Bypass WAF managed rules with a firewall rule:** Create a Firewall Rule with the **bypass** action to deactivate WAF managed rules for a specific combination of parameters. For example, [bypass managed rules](https://developers.cloudflare.com/firewall/cf-firewall-rules/actions/) for a specific URL and a specific IP address or user agent.
...
1.  **Add WAF Exception:** You can define WAF exceptions in the [Cloudflare dashboard](https://developers.cloudflare.com/waf/managed-rulesets/waf-exceptions/define-dashboard) or using the [Rulesets API](https://developers.cloudflare.com/waf/managed-rulesets/waf-exceptions/define-api).
...
-   Does an allowed country, ASN, IP range, or IP in [**IP access rules**](https://support.cloudflare.com/hc/articles/217074967) or [**firewall rules**](https://developers.cloudflare.com/firewall/cf-firewall-rules/) match the attack traffic?
...
-   _Simulate_ - the request is allowed through but is logged in the [**Activity log**](https://developers.cloudflare.com/waf/analytics/paid-plans#activity-log).

Cloudflare’s [WAF changelog](https://developers.cloudflare.com/waf/change-log/scheduled-changes/) allows customers to monitor ongoing changes to the **Cloudflare Managed Ruleset.**
...
-   [Cloudflare’s WAF changelog](https://developers.cloudflare.com/waf/change-log/scheduled-changes/)
