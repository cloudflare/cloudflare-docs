...
Any HTTP status code that is cacheable by default will also be considered to be cacheable by Cloudflare unless otherwise indicated by the method definition or explicit cache controls. Cloudflare caches HTTP responses similarly to how any request is cached. Cloudflare considers page rules (deprecated), edge TTL, and origin headers when deciding whether to cache.
