...
Cloudflare is able to generate these responses, avoiding the need to send a request to the origin server’s response through the use of Page rules (deprecated). Read how Cloudflare can help generate redirects using [Redirect Rules](/rules/url-forwarding/).
...
Cloudflare is able to generate these responses, thus avoiding the need to send a request to the origin server’s response through the use of Page rules (deprecated). Read more How Cloudflare can help generate redirects using [Redirect Rules](/rules/url-forwarding/).
