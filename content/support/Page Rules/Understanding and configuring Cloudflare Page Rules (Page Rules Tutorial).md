...
If the URL of the current request matches both a Page Rule and a [Workers custom route](/workers/platform/routes), some Pages Rules settings will not be applied. For details on using Page Rules with Workers, refer to [Workers: Page Rules](/workers/platform/workers-with-page-rules/) in the developers documentation.
