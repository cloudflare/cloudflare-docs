...
-   要注意以下 Cloudflare 功能：
    ...
    -   [DNS CAA 记录](/ssl/edge-certificates/custom-certificates/caa-records)：Shopify 使用 Let’s Encrypt 为商家域颁发 SSL/TLS 证书。如果您添加任何 DNS CAA 记录，则必须选择 Let’s Encrypt 作为证书颁发机构（CA），否则 HTTPS 连接可能会出错。
