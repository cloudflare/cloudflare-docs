...
When configuring SameSite attributes on session affinity cookies, it is recommended that you set the values. The value `Auto` is translated to `Lax` or `None` depending if **Always Use HTTPS** is enabled. When using the value `None`, the secure attribute cannot be set to `Never`.
