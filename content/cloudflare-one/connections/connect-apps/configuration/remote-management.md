...
If you created your Cloudflare Tunnel from the Zero Trust Dashboard, the tunnel [runs as a service](/cloudflare-one/connections/connect-apps/run-tunnel/as-a-service/) on your OS. You can modify the Cloudflare Tunnel service with any of the [tunnel configuration flags].

--> Remotely management tunnels cannot be configured directly from the command line — instead, 
...
2. Add the desired configuration flag to the `cloudflared tunnel run` command. The following example changes the tunnel `protocol` to QUIC:
...
2. Under the `Program Arguments` key, add the desired configuration flag. The following example changes the tunnel `protocol` to QUIC:
