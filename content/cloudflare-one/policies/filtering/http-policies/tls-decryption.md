...
If you try to perform TLS decryption, these applications may not load or may return a `526` error. You can resolve the issue by exempting unsupported applications from TLS decryption. To bypass TLS decryption, you must add a [*Do Not Inspect*](/cloudflare-one/policies/filtering/http-policies/#do-not-inspect) HTTP policy for the application or domain. Cloudflare provides a [list of trusted applications](/cloudflare-one/policies/filtering/initial-setup/http/#bypass-inspection-for-incompatible-applications) that are known to use embedded certificates.
