...
| Hostname         | `help.teams.cloudflare.com`                                  | allow     | Used by WARP client to check if Gateway is on by inspecting the certificate and checking if the certificate is properly installed on the client device. |
...
| Hostname         | `dash.teams.cloudflare.com`                                  | bypass    | Bypasses the Zero Trust dashboard to prevent users from locking themselves out.                                                                         |
