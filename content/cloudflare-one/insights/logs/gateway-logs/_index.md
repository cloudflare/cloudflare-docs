...
| **Email**              | Email address of the user who registered the WARP Client where traffic originated from. If a non identity on-ramp (Proxy Endpoint) or machine-level authentication (Service Tokens) was used, this value will display as `non_identity@<team-domain>.cloudflareaccess.com`. |
