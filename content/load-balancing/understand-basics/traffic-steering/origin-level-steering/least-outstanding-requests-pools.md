...
Least Outstanding Requests Steering is best to use if your origins can be easily overwhelmed by a spike in concurrent requests. Alike other steering policies, it supports [adaptive routing](/load-balancing/understand-basics/adaptive-routing/) and [session affinity](/load-balancing/understand-basics/session-affinity/).
...
Note that Least Outstanding Requests Steering can also be configured on a load balancer as a [steering policy](/load-balancing/understand-basics/traffic-steering/steering-policies/least-outstanding-requests/), taking into account outstanding request counts and `random_steering` weights for pools on the load balancer.
