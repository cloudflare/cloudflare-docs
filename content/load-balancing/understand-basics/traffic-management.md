...
You can [set up your Tunnels and Tunnel routes](/cloudflare-one/connections/connect-apps/private-net/tunnel-virtual-networks/) to create a `virtual_network_id` value to use in the Load Balancing API call. Enable Virtual IP support by adding the `virtual_network_id` field to your API requests. Refer to the [Cloudflare API documentation](/api/operations/account-load-balancer-pools-create-pool) for more information.
