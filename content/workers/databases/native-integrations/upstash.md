...
### Set up an integration with Upstash Redis:
...
### Set up an integration with Upstash Kafka:
...
2. Add the Upstash QStash integration to your Worker:
    ...
    - In **Account Home**, select **Workers & Pages**.
    - In **Overview**, select your Worker.
    - Select **Settings** > **Integrations** > **Upstash QStash**. 
...
3. In your Worker, install the `@upstash/qstash`, a HTTP client to connect to your database QStash endpoint:
    ...
    ```
4. Check out an [example](https://docs.upstash.com/qstash/quickstarts/cloudflare-workers#3-use-qstash-in-your-handler) published by Upstash on how to receive webhooks from QStash in your Cloudflare Worker. 