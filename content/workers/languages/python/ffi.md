...
Via Pyodide, Python Workers provide a [Foreign Function Interface (FFI)](https://en.wikipedia.org/wiki/Foreign_function_interface) to JavaScript. This allows you to:

- Use [bindings](/workers/configuration/bindings/) to resources on Cloudflare, including [Vectorize](/vectorize/), [Workers AI](/workers-ai/), [R2](/r2), [Durable Objects](/durable-objects/) and more.
- Use JavaScript globals, like [`Request`](/workers/runtime-apis/request/), [`Response`](/workers/runtime-apis/response/), [`fetch()`](/workers/runtime-apis/fetch/) and more.
...
For example, to accesss a [KV](/kb) namespace from a Python Worker, you would declare the following in your Worker's [`wrangler.toml`](/workers/wrangler/configuration/):
