...
Using Durable Objects will often add response latency, as the request must be forwarded to the data center where the object is located. Because objects are usually located near the location they were first requested, it can be bad for latency to pre-create objects from a single location such as your development workstation. It's better for latency to create objects in response to actual production traffic.
