...
Cloudflare Workers supports the `new WebSocket(url)` constructor, and so a Worker can establish a WebSocket connection to a remote server in the same manner as the client implementation described above.
