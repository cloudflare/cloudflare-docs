...
*   `dispose()` {{<type>}}() => Promise\<void>{{</type>}}
    ...
    *   Call this after the platform proxy is no longer required by the program. If you are running a long running process (such as a dev server) that can indefinitely make use of it, you don't need to call this function.
