...
- `custom_domain` {{<type>}}boolean{{</type>}} {{<prop-meta>}}optional{{</prop-meta>}}
  ...
  - Whether the Worker should be on a Custom Domain as opposed to a route. Defaults to `false`. Refer to [Custom Domains](/workers/platform/triggers/cron-triggers/).
...
This will use a Custom Domain as opposed to a route. Refer to [Custom Domains](/workers/platform/triggers/cron-triggers/).
...
Triggers allow you to define the `cron` expression to invoke your Worker's `scheduled` function. Refer to [Supported cron expressions](/workers/platform/triggers/custom-domains/#supported-cron-expressions).
