...
When you run a load test from a single client, the thousands of requests you generate per second typically all come from a single client IP address. Incoming requests to Cloudflare pass through [DDOS protection](/ddos-protection/) — mitigating DDOS attacks on your behalf. Your load test, with thousands of requests per second coming from a single client, all with the same characteristics, landing on the same server in the same location, does not represent what real-world traffic looks like.
