...
Note that checking for an empty array in `keys` is not sufficient to determine whether there are more keys to fetch, you should check `list_complete`. The reason it's possible to have an empty array in `keys` but still have more keys to fetch is because [recently deleted keys](https://en.wikipedia.org/wiki/Tombstone_%28data_store%29) must be iterated through but will not be included in the returned `keys`.
