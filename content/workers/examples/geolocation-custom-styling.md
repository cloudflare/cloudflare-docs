...
import micropip # Add micropip to requirements.txt https://developers.cloudflare.com/workers/languages/python/packages/
...
async def on_fetch(request):
    ...
    async def to_css_gradient(hour):
        css = "linear-gradient(to bottom,"
        data = grads[hour]
        lnt = len(data)
        for i in range(lnt):
            item = data[i]
            css += f' #{item["color"]} {item["position"]}%'
            if i < lnt - 1: css += ","
        return css + ")"
    ...
    # Add micropip to requirements.txt https://developers.cloudflare.com/workers/languages/python/packages/
    await micropip.install('pytz')
    import pytz

    timezone = request.cf.timezone
    localized_date = datetime.now(pytz.timezone(timezone))
    hour = localized_date.hour
    minutes = localized_date.minute

    html_content += "<h1>" + str(hour) + ":" + str(minutes) + "</h1>"
    html_content += "<p>" + timezone + "<br/></p>"
    html_style += "body{background:" + (await to_css_gradient(hour)) + ";}"
    html = f"""
    <!DOCTYPE html>
    <head>
        <title>Geolocation: Customized Design</title>
