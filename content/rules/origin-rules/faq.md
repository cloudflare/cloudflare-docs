...
## What happens if I use both an Origin Rule and a Page Rule to perform a host/resolve override?
...
* A Page Rule defines a host header override, but not an origin override. An Origin Rule defines an origin override, but not a host header override. The resulting request will have the `Host` header defined by the Page Rule and the origin host name defined by the Origin Rule.
...
No. This is currently a manual process, since your Page Rules may include additional settings that Origin Rules do not support.
