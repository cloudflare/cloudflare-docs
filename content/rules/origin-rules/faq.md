...
* A Page Rule defines a Host header override, but not an DNS record override. An Origin Rule defines an DNS record override, but not a Host header override. The resulting request will have the `Host` header defined by the Page Rule and the origin hostname defined by the Origin Rule.
