...
3.  Click **Create transform rule** > **Modify Response Header**.
    ...
    ![The HTTP Response Header Modification Rule creation page in the Cloudflare dashboard.](https://developers.cloudflare.com/rules/static/transform/create-response-header-modification-rule.png)
...
8.  To modify another HTTP response header in the same rule, click **+ Set new header**.
    ...
    ![Example configuration performing three response header modifications: set a dynamic header value, set a static header value, and remove an existing header.](https://developers.cloudflare.com/rules/static/transform/response-header-modification-example.png)
