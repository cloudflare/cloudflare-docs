...
- **Normalize incoming URLs** {{<prop-meta>}}(default: _On_){{</prop-meta>}}
  ...
  - Configures the URLs of all incoming traffic to Cloudflare:
    ...
    - When enabled, all incoming URLs are normalized before they pass to subsequent Cloudflare features that can receive a URL as input, such as Page Rules (legacy), WAF custom rules, Workers, and Access.
