...
When making requests to Workers AI, replace `https://api.cloudflare.com/client/v4/accounts/<ACCOUNT_ID>/ai/run` in the URL you’re currently using with `https://gateway.ai.cloudflare.com/v1/<ACCOUNT_ID>/<GATEWAY_NAME>/workers-ai`.
...
curl -X POST https://gateway.ai.cloudflare.com/v1/<ACCOUNT_ID>/<GATEWAY_NAME>/workers-ai/@cf/meta/llama-2-7b-chat-int8 \
...
curl -X POST https://gateway.ai.cloudflare.com/v1/<ACCOUNT_ID>/<GATEWAY_NAME>/workers-ai/@cf/huggingface/distilbert-sst-2-int8 \
  --header 'Authorization: Bearer <CLOUDFLARE_TOKEN>' \
  --header 'Content-Type: application/json' \
