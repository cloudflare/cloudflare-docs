...
Review the proposed plan before applying the changes:
...
## 3. Update the rule to ban instead of simulate

After confirming in the logs that the rule is triggering but not yet enforcing, switch from `simulate` to `ban`.
...
## 5. (Optional) Confirm the rule works as expected

This step is a good way to show that the rule works as expected. Note the final `429` response (`Too Many Requests`).
