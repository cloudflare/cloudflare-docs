...
If your domain is already live with a partial DNS setup — with Cloudflare or another DNS provider — you cannot use a TXT record for [Domain Control Validation](/ssl/edge-certificates/changing-dcv-method/#txt). That domain's TXT record needs to be reserved for forwarding traffic to Cloudflare.
