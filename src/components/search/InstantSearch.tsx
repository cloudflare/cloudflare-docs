...
function SearchBox(props: UseSearchBoxProps) {
	...
	return (
		<div className="flex items-center rounded-sm border border-cl1-gray-8 p-2 dark:border-cl1-gray-2">
			<input
				type="text"
				value={query}
		...
		<div className="space-y-4">
			{items.map((item) => {
				...
				return (
					<a
						key={item.objectID}
						href={item.url}
						className="flex flex-col rounded-sm border border-cl1-gray-8 p-6 text-black! no-underline hover:bg-cl1-gray-9 dark:border-cl1-gray-2 dark:bg-cl1-gray-0 dark:hover:bg-cl1-gray-1"
					>
						<strong>{title}</strong>
			...
			{items.length !== 0 && !isLastPage && (
				<div className="flex items-center justify-center">
					<button
						onClick={showMore}
						className="h-12 cursor-pointer rounded-sm bg-cl1-brand-orange px-6 font-medium text-cl1-black"
					>
						Load more
					</button>
				</div>
	...
	return (
		<>
			<button
				...
				{...getReferenceProps()}
				className="flex cursor-pointer items-center justify-center gap-2 rounded-sm border border-cl1-gray-8 bg-transparent p-2 dark:border-cl1-gray-2"
			>
				<span>
					{label}
			...
			{isOpen && (
				<FloatingPortal>
					<div
						...
						{...getFloatingProps()}
						className="rounded-sm border border-cl1-gray-8 bg-cl1-white p-4 shadow-md dark:border-cl1-gray-1 dark:bg-cl1-gray-0"
					>
