...
function render(
	...
	// satteri's return type is conditionally async, based on whether any
	// configured hastPlugin's visitor signature *could* be async. None of
	// ours actually are — this compiles synchronously in practice, confirmed
	// by every test below completing without awaiting.
...
describe("Nimbus Satteri HAST pipeline", () => {
	...
	test("registers order-dependent plugins in the expected sequence", () => {
		const names = hastPlugins.map((plugin) => plugin.name);
