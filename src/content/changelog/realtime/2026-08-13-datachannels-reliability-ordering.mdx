...
`maxRetransmits` and `maxPacketLifeTime` are alternative retry budgets. Set at most one for each channel. Omitting both preserves reliable delivery. You can use `ordered: false` with either retry budget.
