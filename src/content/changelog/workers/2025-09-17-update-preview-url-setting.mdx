...
Note: You can set `preview_urls = true` with any Wrangler version that supports the preview URL flag (v3.9.0+). However, we recommend updating to v4.34.0 or newer, as this version defaults `preview_urls` to false, ensuring preview URLs are always enabled by explicit choice.

## New Default Behavior Going Forward

Going forward, if a Worker's `workers.dev` subdomain is disabled, its preview URL will be disabled by default.

To activate the preview URL, you must explicitly enable it using one of the methods described above. 
