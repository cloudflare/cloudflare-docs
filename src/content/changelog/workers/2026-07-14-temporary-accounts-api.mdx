...
The response includes `account.id`, `account.apiToken`, and `claim.url`. Treat `account.apiToken` as a secret, and use it only from your platform backend with supported Workers API endpoints.
Treat `claim.url` like a bearer credential. Anyone with the URL can claim ownership of the temporary account.
Send the URL only to the intended user. Exclude it from logs and analytics.
The user must complete the claim within 60 minutes. Otherwise, Cloudflare deletes the account and its resources.
After the claim, the user can continue deployments through their authenticated Cloudflare account.
