...
Instead of running `cloudflared proxy-dns` on a machine, you can install the WARP Connector on a single Linux host within your private network. This connector will act as a gateway, securely routing all DNS and network traffic from your [entire subnet](/mesh/features/routes/) to Cloudflare for [filtering and logging](/cloudflare-one/traffic-policies/).
