---
title: "Moondream 3 now available on Workers AI"
description: Moondream 3, a fast and efficient vision language model with frontier-level visual reasoning, is now available on Workers AI.
products:
  - workers-ai
date: 2026-07-08
---

Partnering with [Moondream](https://moondream.ai/) to bring their latest model [`@cf/moondream/moondream3.1-9B-A2B`](/workers-ai/models/moondream3.1-9B-A2B/) to Workers AI. Moondream 3 is a fast vision language model built on a mixture-of-experts architecture with 9B total parameters and 2B active, delivering frontier-level visual reasoning while retaining fast, cost-efficient inference.

Moondream 3 is designed for real-world vision tasks, with a 32K token context window for handling complex queries and structured outputs.
...
Vision workloads like live camera feeds, robotics, content moderation, and interactive agents need answers in milliseconds, not seconds. Moondream 3's small active footprint (2B active parameters) pairs well with Workers AI's serverless, globally distributed inference: requests run close to your users, and streaming responses start returning tokens almost immediately.
...
Use Moondream 3 through the [Workers AI binding](/workers-ai/configuration/bindings/) (`env.AI.run()`) or the REST API at `/ai/run`. You can also use [AI Gateway](/ai-gateway/) with these endpoints.

For more information, refer to the [Moondream 3 model page](/workers-ai/models/moondream3.1-9B-A2B/) and [pricing](/workers-ai/platform/pricing/).
