...
You can now assign granular, resource-scoped roles for [Cloudflare Gateway](/cloudflare-one/traffic-controls/) firewall policies and [Zero Trust lists](/cloudflare-one/reusable-components/lists/). Administrators can delegate access to specific policy types or list management without granting account-wide or product-wide control.
