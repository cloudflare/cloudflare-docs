...
Your origin now decides which request headers matter by listing them in its `Vary` response, and you control how Cloudflare treats each one. When a response includes `Vary`, the listed request headers become part of the cache key automatically.
