...
In [AutoRAG](/ai-search/), you can now [filter](/ai-search/configuration/retrieval/metadata/) by an object's file name using the `filename` attribute, giving you more control over which files are searched for a given query.
...
Learn more in AutoRAG's [metadata filtering documentation](/ai-search/configuration/retrieval/metadata/).
