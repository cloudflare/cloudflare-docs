...
description: Four new fields expose client certificates and chain data in RFC 9440 format, making it simpler to forward mTLS certificate information to your origin.
date: 2026-03-25
---

Cloudflare now exposes four new fields in the Transform Rules phase that encode client certificate data in [RFC 9440](https://www.rfc-editor.org/rfc/rfc9440) format. Previously, forwarding client certificate information to your origin required custom parsing of PEM-encoded fields or non-standard header formats. These new fields produce output in the standardized `Client-Cert` and `Client-Cert-Chain` header format defined by RFC 9440, so your origin can consume them directly without any additional decoding logic.
...
| `cf.tls_client_auth.cert_chain_rfc9440` | String | The intermediate certificate chain in RFC 9440 format as a comma-separated list. Empty if no intermediates were sent or if the chain exceeded 16 KB. |
...
Add an HTTP Request Header Modification rule to set the `Client-Cert` and `Client-Cert-Chain` headers on requests forwarded to your origin. For example, to forward headers for verified, non-revoked certificates:
...
For more information, refer to [Mutual TLS authentication](/cloudflare-one/identity/devices/access-integrations/mutual-tls-authentication/), [Request Header Modification Rules](/rules/transform/request-header-modification/), and the [fields reference](/ruleset-engine/rules-language/fields/reference/).
