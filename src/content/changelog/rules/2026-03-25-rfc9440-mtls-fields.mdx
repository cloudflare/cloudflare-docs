...
- **If you do not enforce mTLS at the WAF:** Add a second transform rule that removes any incoming `Client-Cert` and `Client-Cert-Chain` headers from all requests (use expression `true`), ordered before the rule above. This ensures your origin server cannot receive client-supplied values for these headers.
:::
