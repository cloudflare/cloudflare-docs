...
Source code detection applies to standalone source code files in [Gateway HTTP policies](/cloudflare-one/traffic-controls/http-policies/). It does not detect source code embedded within other file types or payloads, such as `.docx` files or chat messages.
