...
This gap between actual and configured is especially useful for Durable Objects. The configured split assigns Worker versions to individual Durable Objects, not to individual requests. Because [each Durable Object is pinned to the version it started on until you create a new deployment](/workers/versions-and-deployments/gradual-deployments/with-durable-objects/) and some objects naturally receive more traffic than others, the observed split can differ from the configured one for as long as the deployment is active, not just while it settles. Seeing both numbers side by side makes it clear how closely the traffic your objects are actually seeing matches what you configured.
