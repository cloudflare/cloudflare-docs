...
description: Cleanup a Durable Object's storage with a single API call.
products:
  - durable-objects
  - workers
date: 2026-02-24
...
// Now: a single call clears both data and alarms
await this.ctx.storage.deleteAll();
```

If you need the previous behavior where alarms are preserved, call `getAlarm()` before `deleteAll()` and then restore the alarm with `setAlarm()`.
