...
- **Durable, exactly-once-on-the-happy-path completion** via a warm fast path plus a self-scheduling reconcile backbone that survives eviction and deploys. A premature give-up can never dedupe a child's real late completion away.
