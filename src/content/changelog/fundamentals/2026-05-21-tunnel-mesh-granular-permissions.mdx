...
You can now scope Cloudflare permissions to individual [Cloudflare Tunnel](/tunnel/) instances and [Cloudflare Mesh](/mesh/) nodes. Administrators can delegate access to specific Tunnels or Mesh nodes without granting account-wide control over private networking.

### What is new

When you [add a member](/fundamentals/manage-members/manage/) or create a [permission policy](/fundamentals/manage-members/policies/), the resource picker now lists [Cloudflare Tunnel](/tunnel/) instances and [Cloudflare Mesh](/mesh/) nodes as scopable resource types. You can:
...
- Configure [granular permissions for Cloudflare Tunnel](/tunnel/guides/granular-permissions/).
