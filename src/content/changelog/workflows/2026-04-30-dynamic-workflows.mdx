...
export const DynamicWorkflow = createDynamicWorkflowEntrypoint<Env>(
	async ({ env, metadata }) => {
		const stub = env.LOADER.get(metadata.tenantId as string, async () => ({
			compatibilityDate: "2026-01-01",
			mainModule: "index.js",
			modules: { "index.js": await fetchTenantCode(metadata.tenantId) },
			env: { WORKFLOWS: wrapWorkflowBinding({ tenantId: metadata.tenantId }) },
		}));
...
For a full walkthrough, refer to the [Dynamic Workflows example](/dynamic-workers/examples/dynamic-workflows/).
