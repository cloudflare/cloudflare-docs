...
Redirects defined in the `_redirects` file are not applied to requests served by {props.product === 'workers' ? <>your Worker code, even if the request URL matches a rule defined in <code>_redirects</code>. You may wish to apply redirects manually in your Worker code, or explore other options such as <a href="/rules/url-forwarding/bulk-redirects/create-dashboard/">Bulk Redirects</a></> : <><a href="/pages/functions/">Pages Functions</a>, even if the Function route matches the URL pattern. If your Pages application uses Functions, you must migrate any behaviors from the <code>_redirects</code> file to the code in the appropriate <code>/functions</code> route, or <a href="/pages/functions/routing/#create-a-_routesjson-file">exclude the route from Functions</a></>}.
