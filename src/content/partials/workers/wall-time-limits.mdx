...
The following table summarises the wall time limits for different types of Worker invocations across the developer platform:

| Invocation type | Wall time limit | Details |
| --- | --- | --- |
| Incoming HTTP request | Unlimited | No hard limit while the client remains connected. When the client disconnects, tasks are cancelled unless you call [`waitUntil()`](/workers/runtime-apis/handlers/fetch/) to extend execution by up to 30 seconds. |
