...
Generate temporary credentials to push or pull images from the Cloudflare managed registry.

```txt
wrangler containers registries credentials <DOMAIN> [OPTIONS]
```

- `DOMAIN` <Type text="string" /> <MetaInfo text="required" />
  - Domain to get credentials for. Must be `registry.cloudflare.com`.
