...
:::note[Ignoring `.env` files]

If you have `.env` files managed by other tools that should not be loaded by Wrangler, add a `# WRANGLER_IGNORE` comment anywhere in the file. For more information, refer to [Ignoring `.env` files](/workers/wrangler/system-environment-variables/#ignoring-env-files).
