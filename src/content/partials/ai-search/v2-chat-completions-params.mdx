...
- `retrieval` <Type text="object" /> <MetaInfo text="optional" />
  ...
  - `filters` <Type text="object" /> <MetaInfo text="optional" />
    - Filter search results based on metadata. Supports comparison filters (`eq`, `ne`, `gt`, `gte`, `lt`, `lte`) and compound filters (`and`, `or`). For more details, refer to [Metadata filtering](/ai-search/configuration/retrieval/metadata/).
