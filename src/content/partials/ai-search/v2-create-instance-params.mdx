...
- `web_crawler` <Type text="object" /> <MetaInfo text="optional" />
  ...
  - `parse_type` <Type text="string" /> <MetaInfo text="optional" />
    - How pages are discovered. Valid values: `sitemap` (reads XML sitemaps), `discover` (follows links from the source URL). Defaults to `sitemap`. Refer to [Parse types](/ai-search/configuration/data-source/parse-types/).
