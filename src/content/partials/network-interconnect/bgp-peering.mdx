...
You need to enable [tunnel health checks]({props.tunnelHcPath}) alongside BGP. This is essential to determine if a specific Cloudflare datacenter is reachable from a customer router or not. Tunnel health checks will modify the route's priorities for dynamically learned BGP routes, like what happens for static routes.
