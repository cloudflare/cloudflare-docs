...
1. To create a custom hostname using the API, use the [Create Custom Hostname](/api/resources/custom_hostnames/methods/create/) endpoint.
   * Only Enteprise customer can use the `certificate_authority` parameter to specify a CA, the `custom_certificate`/`custom_cert_bundle` to upload a custom certificate or `wildcard` to create a wildcard certificate. See [plans](/cloudflare-for-platforms/cloudflare-for-saas/plans/).
