...
By default, Cloudflare Tunnel expects all of the configuration to exist in the `%USERPROFILE%\.cloudflared\config.yml` <a href={props.configFileURL}>configuration file</a>. At a minimum you must specify the following arguments to run as a service:
...
1. <a href={props.downloadsURL}>Download</a> the latest `cloudflared` version.
...
9. Next, create a tunnel:
   ...
   This will generate a <a href={props.credentialsFileURL}>credentials file</a> in `.json` format.

10. <a href={props.createConfigFileURL}>Create a configuration file</a> with the
    following content:
...
14. In the Cloudflared registry entry, modify `ImagePath` to point to the `cloudflared.exe` and `config.yml` files. Make sure that there are no extra spaces or characters while you modify the registry entry, as this could cause problems with starting the service.
    ...
    Save the original `ImagePath`. To uninstall this service later with `cloudflared service uninstall`, restore the original value first.
...
You can now <a href={props.routeTrafficURL}>route traffic through your tunnel</a>. If you add IP routes or otherwise change the configuration, restart the service to load the new configuration:
