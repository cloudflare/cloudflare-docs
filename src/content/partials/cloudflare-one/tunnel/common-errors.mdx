...
This error means a service with that name is already installed. To run another remotely-managed tunnel on the same host, <a href={props.namedServicesURL}>install a service with a different name</a>. To replace an existing service, uninstall it first. Use `--service-name` to target a named service. Without that flag, `cloudflared service uninstall` targets the default service.
