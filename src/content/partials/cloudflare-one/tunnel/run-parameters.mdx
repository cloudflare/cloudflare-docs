...
This page lists the configuration flags for the `cloudflared tunnel run` command. For a remotely-managed tunnel (created via the dashboard or API), add these flags to the <a href={props.updateRunParametersURL}>tunnel service</a>. If you are using a <a href={props.locallyManagedTunnelURL}>locally-managed tunnel</a>, add these flags to your <a href={props.configurationFileURL}>configuration file</a> as key/value pairs.
