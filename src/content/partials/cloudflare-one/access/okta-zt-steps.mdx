...
14. (Optional) Create an Okta API token and enter it in Cloudflare One (the token can be read-only). This will prevent your Okta groups from failing if you have more than 100 groups.
...
15. (Optional) To configure [custom OIDC claims](/cloudflare-one/integrations/identity-providers/generic-oidc/#custom-oidc-claims):
    ...
    2. In Cloudflare One, enter the **Authorization Server ID** obtained from Okta.
