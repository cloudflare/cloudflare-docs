...
- **Non-identity failure block page**: Displays when the user fails a non-identity Access policy (such as country, IP, or device posture). Cloudflare checks non-identity attributes before prompting the user to login. Therefore, if a user fails an Access policy that contains both identity and non-identity rules, the non-identity block page would take precedence.
