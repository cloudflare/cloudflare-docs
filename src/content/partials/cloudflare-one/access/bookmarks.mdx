...
6. (Optional) Go to the **Additional settings** tab to customize the application experience:

	- **App Launcher visibility**: Turn on if you want the application to be visible in the App Launcher. The toggle does not impact the ability for users to reach the application.
	- **Custom logo**: To add a custom logo for your application, select **Custom** and enter the image URL.
...
Bookmark applications do not generate individual [Access authentication logs](/cloudflare-one/insights/logs/dashboard-logs/access-authentication-logs/#authentication-logs) when a user selects the bookmark tile. Only the authentication event to the App Launcher itself is logged.
