...
The same binding also handles outbound traffic to public Internet destinations: requests egress through [Cloudflare Gateway](/cloudflare-one/traffic-policies/), so your existing Zero Trust traffic policies are enforced and Worker traffic appears in Gateway DNS, HTTP, and Network logs alongside the rest of your traffic.
