...
:::note
Approval status does not impact a user's ability to access an application. Users are allowed or blocked according to your [Access](/cloudflare-one/access-controls/policies/) and [Gateway policies](/cloudflare-one/traffic-controls/). To filter traffic based on approval status, use the [_Application Status_](/cloudflare-one/traffic-controls/http-policies/#application-approval-status) selector.
:::
