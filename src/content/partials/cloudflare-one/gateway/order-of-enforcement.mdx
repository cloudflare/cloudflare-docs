...
For more information on creating packet filtering rules, refer to [Add policies](/cloudflare-one/traffic-controls/packet-filtering/add-policies/).
...
For proxied [HTTP/3 traffic](/cloudflare-one/traffic-controls/http-policies/http3/), Gateway applies your policies in the following order:
...
If a policy contains both pre-resolution and post-resolution selectors, Gateway will evaluate the entire policy after DNS resolution. For information on when each selector is evaluated, refer to the [list of DNS selectors](/cloudflare-one/traffic-controls/dns-policies/#selectors).
...
Gateway applies HTTP policies based on a combination of [action type](/cloudflare-one/traffic-controls/http-policies/#actions) and [order of precedence](#order-of-precedence):
...
When [resolver policies](/cloudflare-one/traffic-controls/resolver-policies/) are present, Gateway first evaluates any DNS policies with pre-resolution selectors, then routes any DNS queries according to the [order of precedence](#order-of-precedence) of your resolver policies, and lastly evaluates any DNS policies with post-resolution selectors.
...
| HTTP | Allow | HTTP and HTTPS requests are allowed. However, if you have configured a default Block action in your [HTTP policy settings](/cloudflare-one/traffic-controls/http-policies/), unmatched traffic is blocked instead. |
