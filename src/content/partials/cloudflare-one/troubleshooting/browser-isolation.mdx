...
### Audio or video stops when switching windows

Browser Isolation supports one active window at a time. When you switch to a different isolated tab or window, the previous window is suspended, meaning that any audio or video playing there will pause. This is expected behavior. For more details, refer to [Single active window](/cloudflare-one/remote-browser-isolation/known-limitations/#single-active-window).
