...
:::warning
Note that if a [DNS-only (grey cloud)](/dns/proxy-status/) CNAME record points to a proxied load balancer, the IP returned for it would be endpoint IP and a HTTP request sent to it would not be proxied.
:::
