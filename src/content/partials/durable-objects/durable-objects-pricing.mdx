...
<AnchorHeading title="Compute billing" depth={props.computeDepth} />
...
<AnchorHeading title="Storage billing" depth={props.storageDepth} />
...
- **Key-value backed Durable Objects**: [Key-value storage backend](/durable-objects/reference/durable-object-class-migrations-legacy/#create-durable-object-class-with-key-value-storage) is only available on the Workers Paid plan.

<AnchorHeading title="SQLite storage backend" depth={props.backendDepth} />
...
<AnchorHeading title="Key-value storage backend" depth={props.backendDepth} />
