...
The [key-value storage backend](/durable-objects/reference/durable-objects-migrations/#storage-backends) remains for backwards compatibility, and a migration path from KV storage backend to SQLite storage backend for existing Durable Object namespaces will be available in the future.
:::