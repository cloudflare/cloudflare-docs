...
Creating new namespaces with the [key-value storage backend](/durable-objects/reference/durable-objects-migrations/#storage-backends) is no longer supported for accounts without an existing key-value-backed namespace. The key-value storage backend remains available for existing namespaces, but you cannot migrate an existing namespace from the key-value storage backend to the SQLite storage backend.
:::
