...
CAA records are inherited. This means that, if you are using a [subdomain setup](/dns/zone-setups/subdomain-setup/) -  with `subdomain.example.com` on Cloudflare but `example.com` on a different DNS provider -, you should make sure that CAA records on the parent domain (`example.com`) are set up correctly.
:::
