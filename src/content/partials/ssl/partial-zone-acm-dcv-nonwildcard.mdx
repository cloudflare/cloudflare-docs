...
If every hostname on a non-wildcard certificate is [proxying traffic](/dns/proxy-status/) through Cloudflare and the DCV method is [HTTP](/ssl/edge-certificates/changing-dcv-method/methods/http/), Cloudflare can automatically complete DCV on your behalf.
