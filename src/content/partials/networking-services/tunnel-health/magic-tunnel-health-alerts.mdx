...
4. Select **Magic Tunnel Health Check Alert** > **Select** to add a notification. You can add alerts by tunnel or by data center.

<Details header="Alert by tunnel">

5. Enter a name and description for the notification.
6. Add webhooks or an email address for the person who should receive the notification, and select **Next**.
7. Select the **Alert Sensitivity Level** threshold from the dropdown menu. It is predefined for _Medium_, but you can choose between several options:
	- Minimum (95.0)
	- Very low (96.0)
	- Low (97.0)
	- Medium (98.0)
	...
	- Very high (99.5)
	- Maximum (99.9)
8. From the **Alert interval** drop-down menu, set the minimum amount of time that must pass before a duplicate alert is sent to you. Options range from five minutes to seven days.
9. Enable **Set as default alert for any new tunnels created in the future** if you want the alert sensitivity level you chose to be automatically applied to all new tunnels you create.
10. Select **Next**.
11. Choose the tunnels you want to receive alerts for, and select **Next**.
