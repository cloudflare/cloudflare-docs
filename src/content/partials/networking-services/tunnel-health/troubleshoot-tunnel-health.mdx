...
This guide helps you diagnose and resolve common tunnel health issues with [Magic WAN](https://developers.cloudflare.com/magic-wan/) and [Magic Transit](https://developers.cloudflare.com/magic-transit/). Tunnel health checks monitor your GRE and IPsec tunnels and steer traffic to the best available routes.
...
5. **IPsec parameters**: Confirm your crypto parameters match [Cloudflare's supported configuration](https://developers.cloudflare.com/magic-wan/reference/tunnels-and-encapsulation/#supported-configuration-parameters).
6. **Health check direction**: Magic WAN defaults to *Bidirectional*; Magic Transit (direct server return) defaults to *Unidirectional*.

---

## Tunnel health states

The Cloudflare dashboard displays three tunnel health states:
...
| Probe retry behavior When a health check probe fails, Cloudflare sends two additional probes to confirm the failure. A tunnel is only marked as unhealthy if all three probes fail. This retry behavior provides resilience against random packet loss. |
| :---- |
...
For instructions on monitoring tunnel status, refer to [Check tunnel health in the dashboard](https://developers.cloudflare.com/magic-wan/configuration/common-settings/check-tunnel-health-dashboard/).
...
1. Go to **Magic WAN** \> **Configuration** \> **Tunnels**.
2. Select **Edit** on the affected tunnel.
...
For more information, refer to [Magic Firewall rules and Magic Transit endpoint health checks](https://developers.cloudflare.com/magic-firewall/about/ruleset-logic/#magic-firewall-rules-and-magic-transit-endpoint-health-checks).
...
1. Go to **Magic WAN** \> **Configuration** \> **Tunnels**.
2. Select **Edit** on your IPsec tunnel.
...
:::caution Enabling replay protection in Cloudflare reduces the performance and resilience benefits of the anycast architecture. Only use this option when your device does not support disabling anti-replay protection. :::

For a detailed explanation of why this setting is necessary, refer to [Anti-replay protection](https://developers.cloudflare.com/magic-wan/reference/anti-replay-protection/).
...
2. **Adjust health check sensitivity**: If brief degradation during rekeys triggers alerts, consider lowering the health check rate:
   ...
   - Go to **Magic WAN** \> **Configuration** \> **Tunnels**
   - Select **Edit** on the tunnel
...
For detailed information on how bidirectional health checks work, refer to [Tunnel health checks](http:///magic-wan/reference/tunnel-health-checks/).
...
- [Tunnel health checks](https://developers.cloudflare.com/magic-wan/reference/tunnel-health-checks/) \- Technical details on health check behavior
- [Anti-replay protection](https://developers.cloudflare.com/magic-wan/reference/anti-replay-protection/) \- Why anti-replay must be disabled
- [Configure tunnel endpoints](https://developers.cloudflare.com/magic-wan/configuration/manually/how-to/configure-tunnel-endpoints/) \- Tunnel setup instructions
- [Check tunnel health in the dashboard](https://developers.cloudflare.com/magic-wan/configuration/common-settings/check-tunnel-health-dashboard/) \- Dashboard navigation guide
- [Network Analytics](https://developers.cloudflare.com/magic-wan/analytics/network-analytics/) \- Traffic analysis tools