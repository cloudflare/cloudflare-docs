...
This guide helps you diagnose and resolve common tunnel health issues with <a href={props.productLink}>{props.productName}</a> and <a href={props.otherProductLink}>{props.otherProductName}</a>. Tunnel health checks monitor your GRE and IPsec tunnels and steer traffic to the best available routes.
...
6. **Health check direction**: {props.productName} defaults to *{props.healthCheckDefault}*{props.defaultReason && `; ${props.otherProductName} ${props.defaultReason} defaults to the opposite direction`}.
...
| :---- | :---- | :---- |
| **Bidirectional** | Probe and response both traverse the tunnel | {props.healthCheckDefault === "Bidirectional" ? props.productName : props.otherProductName} |
| **Unidirectional** | Probe traverses tunnel; response returns via Internet | {props.healthCheckDefault === "Unidirectional" ? `${props.productName}${props.defaultReason ? ` ${props.defaultReason}` : ""}` : props.otherProductName} |
...
For more information, refer to <a href={props.magicFirewallRulesetLink}>Magic Firewall rules and endpoint health checks</a>.
