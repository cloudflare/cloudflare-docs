...
| All tunnels degraded or down in a specific region | Network path issue between that region and your network | Check ISP connectivity. Use traceroute or MTR from your network to Cloudflare endpoints. |
...
- **Dashboard**: Tunnel health status per data center and traffic volume per tunnel (go to **Connector health** page)
- **API**: Tunnel health status via the <a href={props.tunnelHealthChecksLink}>{props.productName} tunnel health API</a>
- **Network Analytics**: Traffic volume, packet counts, and protocol distribution through <a href={props.networkAnalyticsLink}>Network Analytics</a>
- **From your network**: Traceroute and MTR from your tunnel endpoint to Cloudflare tunnel addresses — this tests the path your health check responses take

### What you cannot check (current limitations)

- Per-data center health check probe results (only aggregate state is visible)
...
4. **IPsec parameters**: Confirm your cryptographic parameters match <a href={props.tunnelsConfigLink}>Cloudflare's supported configuration</a>.
5. **Health check direction**: {props.productName} defaults to <Markdown text={props.healthCheckDefault} />.
...
A common source of confusion is seeing "100% degraded" in the dashboard and interpreting it as 100% packet loss. It is not.
...
Focus on data centers that show traffic in the **Average ingress traffic** column. A data center showing degraded status with zero or minimal traffic is informational — it indicates a path issue between that specific Cloudflare data center and your network, but it does not affect your traffic if no traffic routes through that data center.
