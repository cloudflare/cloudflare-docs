...
{ (props.magicProduct === "Magic WAN" || props.magicProduct === "WAN Tunnels") && (
  <>
		<p>For {props.magicProduct === "Magic WAN" ? "Magic WAN Connector" : "WAN Tunnels Appliances"}, health checks are sent to IPsec tunnel endpoints.</p>
  </>
  )
}
...
{ (props.magicProduct === "Magic WAN" || props.magicProduct === "WAN Tunnels") && (
  <>
		<p>To configure health checks frequency in {props.magicProduct === "Magic WAN" ? "Magic WAN Connector" : "WAN Tunnels Appliances"}, refer to <a href={props.magicProduct === "Magic WAN" ? "#configure-connector" : "#configure-appliances"}>Configure {props.magicProduct === "Magic WAN" ? "Connector" : "Appliances"}</a></p>
  </>
  )
}
...
{ (props.magicProduct === "Magic WAN" || props.magicProduct === "WAN Tunnels") && (
  ...
  <>
		<AnchorHeading depth={2} title={props.magicProduct === "Magic WAN" ? "Configure Connector" : "Configure Appliances"} />
  	<ol>
   		<li><p>In the Cloudflare dashboard, go to the <strong>Sites</strong> page.</p>
   			<DashButton url="/?to=/:account/magic-wan/sites" />
 			</li>

   		<li>Select your site &gt; <strong>Edit</strong>.</li>
   		<li>In <strong>Network</strong> &gt; <strong>WAN configuration</strong> &gt; select your WAN &gt; <strong>Edit</strong>.</li>
   		<li>Change the <strong>Health check rate</strong> to your desire rate.</li>
