...
A tunnel health check probe comes from Cloudflare to the tunnel origin, then returns a response to Cloudflare. This response is used to determine the outcome of the probe, which is used to calculate the state of the tunnel (the following sections explain this in greater detail).
...
A tunnel health check probe has important attributes described in the following sections.
