...
| Advanced Magic Firewall features: GeoIP/Country rules, IP Lists, ASN Lists, Threat Intel Lists, IDS, Rate Limiting, SIP, Managed Rulesets | Not yet supported |
...
Cloudflare redistributes routes received from your device into the Cloudflare Virtual Network routing table, which both {props.productName} and Magic Transit use.
...
{ props.magicWord === "Magic Transit" && (
  <>
    <p>For Magic Transit customers, BGP with the {props.virtualNetworkName} is separated from the announcement of anycast prefixes at the Cloudflare edge. Anycast withdrawal must be controlled with existing methods documented in <a href="/magic-transit/how-to/advertise-prefixes/">Advertise prefixes</a>.</p>
  </>
  )
}
