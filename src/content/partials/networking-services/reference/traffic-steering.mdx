...
The Magic routing table supports routing the {props.productName} traffic through anycast tunnels using <a href={props.greIpsecReferenceURL}>GRE and Internet Protocol Security (IPsec)</a> or [CNI with Dataplane v2 (beta)](/network-interconnect/). You can add entries to the Magic routing table through static route configuration or through routes learned through BGP peering (only available over CNI with Dataplane v2 (beta)).{ props.magicWord !== "Magic Transit" && (<> Traffic can also be routed automatically according to tracked flow state.</>)}
...
When using a [CNI with Dataplane v2 (beta)](/network-interconnect/) connection as an on-ramp, you can use BGP peering between your network and your Magic routing table.
...
- Establish an eBGP session between your devices and the {props.productName} service when connected through CNI.
