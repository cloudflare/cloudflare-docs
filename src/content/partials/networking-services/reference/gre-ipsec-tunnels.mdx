...
To accommodate additional header data introduced by encapsulation, the <GlossaryTooltip term="maximum segment size (MSS)">maximum segment size (MSS)</GlossaryTooltip> must be adjusted to comply with the standard Internet routable maximum transmission unit (MTU), which is 1500 bytes.
...
Cloudflare's anycast architecture provides a conduit to your tunnel for every server in every data center on Cloudflare's global network as shown in the following image.
...
[IPsec](https://www.cloudflare.com/learning/network-layer/what-is-ipsec/) is a group of protocols that work together to set up encrypted connections between devices. It helps keep data you send over public networks secure. Organizations often use IPsec to set up VPNs, and it works by encrypting IP packets and authenticating the source where the packets come from.
...
:::note
The IKE SA and the Child SA are separate things, each with their own parameters. The Child SA is the actual IPsec tunnel where user traffic flows (that is, the dataplane). The IKE SA is used to set up and manage the Child SA.
:::
...
Choose from the following configuration parameters supported by {props.productName} based on what your appliance supports.
