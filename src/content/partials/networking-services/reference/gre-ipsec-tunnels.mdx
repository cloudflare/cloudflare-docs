...
- If your router is behind Network Address Translation (NAT) and requires NAT traversal (NAT-T), your router can initiate IKE communication on either port `500` or port `4500`. Cloudflare supports the standard NAT-T flow defined in [RFC 3947](https://datatracker.ietf.org/doc/rfc3947/) and [RFC 7296](https://datatracker.ietf.org/doc/rfc7296/), where IKE begins on port `500` and switches to port `4500` after NAT is detected. Starting on port `4500` directly is also supported.
