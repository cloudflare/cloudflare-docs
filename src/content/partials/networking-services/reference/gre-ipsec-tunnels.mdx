...
- If your router is behind Network Address Translation (NAT) and requires NAT traversal (NAT-T), your router can initiate IKE communication on either port `500` or port `4500`. Cloudflare supports the standard NAT-T flow where IKE begins on port `500` and switches to port `4500` after NAT is detected. Starting on port `4500` directly is also supported.
