...
:::note[Important to know]
CNI supports both Magic Transit (DDoS protection) and Cloudflare WAN (private networking). CNI also supports [BGP peering](/network-interconnect/get-started/) (beta) with the Cloudflare Virtual Network routing table for dynamic route exchange. When used with Magic Transit, cleaned inbound traffic always flows over CNI. Return traffic can either egress directly to the Internet (Direct Server Return, default) or route back through Cloudflare via [Magic Transit Egress](/magic-transit/reference/egress/).
:::
