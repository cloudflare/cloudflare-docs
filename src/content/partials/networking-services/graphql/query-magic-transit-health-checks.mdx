...
This example uses the GraphQL Analytics API to query {props.productName} tunnel health check results. These results are aggregated from individual health checks that Cloudflare servers perform against Generic Routing Encapsulation (GRE) tunnels you configured to work with {props.productName} during onboarding. You can query up to one week of data for dates up to three months in the past.
