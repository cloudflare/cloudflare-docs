...
{ (props.magicProduct === "Magic WAN" || props.magicProduct === "WAN Tunnels") && (
  <>
	{/* Generate dynamic anchor link: converts product name to lowercase and replaces spaces with hyphens
	    e.g., "Magic WAN" becomes "#magic-wan-site-analytics", "WAN Tunnels" becomes "#wan-tunnels-site-analytics" */}
	<Markdown
	text={`
	- Overview in [${props.productName} Site Analytics](#${props.productName.toLowerCase().replace(/ /g, '-')}-site-analytics)
	`}
	inline={false}
	/>
  </>
...
{ (props.magicProduct === "Magic WAN" || props.magicProduct === "WAN Tunnels") && (
  <>
    <AnchorHeading title={`${props.productName} Site Analytics`} depth={2} />
		<Markdown
		text={`
...
GraphQL Analytics provides customers with a GraphQL API that they can query to receive raw JSON data of their {props.productName} traffic analytics. This data can be ingested into a SIEM or other tool and analyzed further.
