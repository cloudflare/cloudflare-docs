...
If you are experiencing actual tunnel health issues (tunnels flapping, all tunnels down, or IPsec errors), refer to [Troubleshoot tunnel health]({props.tunnelHealthLink}) instead.
...
1. Go to the **Network Analytics** page.
	...
	<DashButton url="/?to=/:account/network-analytics/all-traffic" />

2. Filter traffic by your source IP addresses to isolate your traffic.
3. Under **Packets summary**, select the three-dot menu to expand the view options and select **Source data center**.
4. The data center column shows which Cloudflare data centers are handling your traffic.
5. Cross-reference these data centers with the tunnel health status on the [**Connector health** page]({props.checkHealthDashLink}). If tunnels are healthy at the data centers carrying your traffic, a degraded tunnel alert for a different data center is not the cause of your connectivity issue.
...
3. Check for incidents affecting {props.productName} specifically.
...
- [Troubleshoot tunnel health]({props.tunnelHealthLink}): Resolve common tunnel health issues (flapping, IPsec errors, stateful firewall drops).
- [Troubleshoot routing and BGP]({props.routingBgpLink}): Diagnose routing and BGP issues that affect traffic delivery.
- [Check tunnel health in the dashboard]({props.checkHealthDashLink}): Monitor tunnel status per data center.
- [Tunnel health checks]({props.tunnelHealthChecksLink}): Technical details on how health checks work.
- [Network Analytics]({props.networkAnalyticsLink}): Analyze traffic patterns over time.
