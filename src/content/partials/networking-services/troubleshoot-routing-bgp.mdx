...
${props.magicWord === "Magic Transit" ? `3. **Prefix advertisement status**: Confirm prefixes show **Advertised** in the dashboard, not **Withdrawn** or **Approved**.
...
This section covers BGP peering sessions between your network and Cloudflare, established over [CNI](/network-interconnect/) or tunnels. { props.magicWord === "Magic Transit" && (<Markdown text={` These sessions are separate from how Cloudflare advertises your prefixes to the Internet, which is covered in [Route propagation timing](#route-propagation-timing).`} />)}
