...
When you advertise or withdraw a prefix, changes propagate across Cloudflare's network within minutes.
...
{props.safelyWithdrawLink && (<Markdown text={`Refer to [Safely withdraw a BYOIP prefix](${props.safelyWithdrawLink}) for the recommended procedure.`} inline={false} />)}

## Resolve common issues

### BGP session not establishing

This section covers BGP peering sessions between your network and Cloudflare, established over [CNI](/network-interconnect/) or tunnels. These sessions are separate from how Cloudflare advertises your prefixes to the Internet, which is covered in [Route propagation timing](#route-propagation-timing).
...
{props.safelyWithdrawLink && (<Markdown text={`Refer to [Safely withdraw a BYOIP prefix](${props.safelyWithdrawLink}) for detailed instructions.`} inline={false} />)}
...
1. **Check tunnel health**: Degraded tunnels have 500,000 added to their route priority. Down tunnels have 1,000,000 added. Traffic shifts to healthier paths, which may be in different regions.

   <Markdown text={`Refer to [Troubleshoot tunnel health](${props.tunnelHealthLink}) for diagnostic steps.`} inline={false} />
...
4. **Use Network Analytics**: Review traffic patterns to identify where traffic is landing and which paths it follows.

   <Markdown text={`Refer to [Network Analytics](${props.networkAnalyticsLink}) for usage instructions.`} inline={false} />
...
<Markdown text={`Refer to [Route prioritization](${props.trafficSteeringLink}) for detailed information on how priorities work.`} inline={false} />
...
- [Traffic steering](${props.trafficSteeringLink}): Route prioritization, BGP communities, and ECMP behavior
${props.advertisePrefixesLink ? `- [Advertise prefixes](${props.advertisePrefixesLink}): BGP control methods and safe withdrawal procedures` : ""}
- [Configure routes](${props.configureRoutesLink}): Static route configuration
- [Network Interconnect](/network-interconnect/): CNI setup and BGP peering
- [Troubleshoot tunnel health](${props.tunnelHealthLink}): Tunnel-specific diagnostic steps
- [Network Analytics](${props.networkAnalyticsLink}): Traffic analysis and monitoring
