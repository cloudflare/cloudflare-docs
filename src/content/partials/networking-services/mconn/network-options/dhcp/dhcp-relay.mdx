...
<pre class="mermaid">
	{`
		...
		accTitle: An example of Connector in DHCP Relay mode
				a(${props.productName}) <--> b(Cloudflare/${props.wanName}) <--> c(DHCP server)
...
:::caution
DHCP relay will not work if your DHCP server is behind a <a href={props.cfTunnelURL}>Cloudflare Tunnel</a>. To enable DHCP relay functionality, use either a {props.cfVsCoreDashNamingMwanCni}.
:::
...
<Markdown
text={`
3. Select your ${props.productName} > **Edit**.
