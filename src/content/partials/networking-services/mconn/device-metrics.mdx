...
Cloudflare customers can inspect metrics for a specific {props.productName} in the Cloudflare dashboard. These metrics help you troubleshoot potential issues with your {props.productName}. Refer to <a href={props.troubleshootingURL}>Troubleshooting</a> for more information.
...
The metric `average CPU load` is unique and distinctly different from `CPU utilization` which is another common CPU metric. The {props.productName} uses a [Unix-style CPU load calculation](<https://en.wikipedia.org/wiki/Load_(computing)>).
