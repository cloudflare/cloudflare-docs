...
{ props.magicWord === "virtual" && (
	<>
		<p>Virtual Magic WAN Connector (Virtual Connector) is a virtual appliance alternative to the hardware based Magic WAN Connector appliance. These two versions of Connector are identical otherwise.</p> <p>Currently, you can set up Virtual Connector on VMWare ESXi and Proxmox Virtual Environment.</p>
	</>
	)
}
...
{ props.magicWord === "virtual" && (
  <>
		<AnchorHeading title="Configure Virtual Connector" depth={2} />

		<Tabs syncKey="vmwarePlusproxmox"> <TabItem label="VM Ware ESXi">
		...
		**2. Deploy the Virtual Connector on Proxmox**
		...
		1. In the terminal prompt of your Proxmox server, load the script provided by your account team. For example: \`bash YOUR_SCRIPT\`.
...
{ props.magicWord === "virtual" && (
  <>
		...
		<Markdown
		text={`
		1. Boot up the virtual machine for your Virtual Connector.
