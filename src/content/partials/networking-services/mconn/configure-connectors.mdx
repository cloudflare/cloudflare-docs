...
{ props.magicWord === "hardware" && (
  <>
		...
		<Aside type="caution">Remember that if you chose the DHCP method you have to connect {props.productName} through a route that supports DHCP for its first connection to the Internet. Otherwise, the {props.productName} will not work.</Aside>
  </>
  )
}
...
{ props.magicWord === "virtual" && (
  <>
		...
		<Aside type="caution">Remember to connect {props.productNameVirtual} through a route that supports DHCP for its first connection to the Internet. Otherwise, the Connector will not work.</Aside>
...
{ props.magicWord === "hardware" && (
  <>
    <p>After activating your {props.productName}, you can use it in a network configuration with the WAN interface set to a static IP address — that is, an Internet configuration that is not automatically set by DHCP. To use your Connector on a network configuration with a static IP, follow the steps below.</p>
		...
		4. [Activate](#activate-connector) and power on your ${props.productName}.
		5. Wait 60 seconds.
...
{ props.magicWord === "virtual" && (
  <>
		...
		2. ${props.productOriginalName === "WAN Tunnels" ? "[Create a new profile](#create-a-new-profile)" : "[Add a Connector on-ramp](#add-a-connector-on-ramp)"} in the Cloudflare dashboard.
		3. Create a [DHCP WAN](#create-a-wan).
		4. [Activate](#activate-connector) and boot your ${props.productNameVirtual}.
		5. Wait 60 seconds.
		6. Make changes to the [WAN settings](#create-a-wan) in the Cloudflare dashboard to a static IP set up.
		7. Wait 60 seconds again.
