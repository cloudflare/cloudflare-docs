...
{ props.magicWord === "virtual" && (
  <>
		...
		**1. Obtain the VMWare image**
		...
		You consume one license key for each instance created. Generate a license key for each instance from the [Connectors page](/cloudflare-wan/configuration/appliance/configure-virtual-appliance/#register-a-virtual-appliance-and-generate-a-license-key). For example, to deploy 10 ${props.productNameVirtual}s, generate 10 license keys.
		`}
		inline={false}
		/>
		...
		**1. Obtain the ${props.productNameVirtual} script**
		...
		Download the ${props.productNameVirtual} helper script for Proxmox to your Proxmox server. The script sets up and configures a Proxmox virtual machine with the appropriate settings for ${props.productNameVirtual}. For details on system requirements, refer to [Prerequisites](#prerequisites).
		...
		**2. Deploy the ${props.productNameVirtual} on Proxmox**
		...
		<Markdown
		text={`
		The script applies default settings and configures the virtual machine for ${props.productNameVirtual}. After the script finishes, adjust the VM for your environment. The new VM has all network interface cards (NICs) on the same bridge, and only the first NIC has an active link.

		4. In the **Hardware settings** for the new VM, select the NIC you want to use with ${props.productNameVirtual}.
		...
		<Markdown
		text={`
		7. Make sure the hardware settings match the minimum requirements for running ${props.productNameVirtual}. Make changes to the RAM and CPU if needed.
		...
		**1. Obtain the ${props.productNameVirtual} script**
		...
		<Markdown
		text={`
		**2. Deploy the ${props.productNameVirtual} on libvirt/KVM**
