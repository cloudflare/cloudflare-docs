...
{ props.magicWord === "virtual" && (
	<>
		<p>{props.productNameVirtual} is a virtual appliance alternative to the hardware based {props.productName}. These two versions of {props.productName} are identical otherwise.</p> <p>Currently, you can set up {props.productNameVirtual} on VMWare ESXi and Proxmox Virtual Environment. Support for Proxmox is in beta.</p>
	</>
	)
}
...
{ props.magicWord === "hardware" && (
  <>
		<p>You need to purchase <a href="https://www.cloudflare.com/magic-wan">{props.productOriginalName}</a> before you can purchase and use {props.productName}. {props.productName} can function as your primary edge device for your network, or be deployed in-line with existing network gear.</p>
...
{ props.magicWord === "virtual" && (
  <>
		<Markdown
		text={`
		Before you can install ${props.productNameVirtual}, you need an Enterprise account with ${props.productOriginalName}. Additionally, you need to have a VMware or Proxmox host with sufficient compute, memory, and storage to run the virtual machine with Virtual Connector. This includes:
...
{ props.magicWord === "hardware" && (
  <>
		...
		<p>To set up and use {props.productOriginalName}, you first need to register it with your account.</p>
		<Render
...
{ props.productOriginalName === "WAN Tunnels" && (
	<>
		...
		<ol>
			<li>Log in to the Cloudflare One dashboard, and go to <strong>Networks</strong>.</li>
...
After finishing your { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } configuration, you need to add it to a site. <Render file="sites/sites-brief" product="networking-services" />
...
{ props.productOriginalName === "WAN Tunnels" ? (
	<>
		<Markdown
	text={`
		1. Log in to the Cloudflare One dashboard, and go to **Networks**.
...
{ props.productOriginalName === "WAN Tunnels" ? (
	<>
		<Markdown
	text={`
		1. Log in to the Cloudflare One dashboard, and go to **Insights** > **Network visibility**.
	`}
	inline={false}
	/>
	</>
