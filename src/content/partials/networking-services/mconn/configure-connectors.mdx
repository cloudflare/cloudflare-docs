...
{ props.magicWord === "hardware" && (
  <>
    ...
    class a,c orange`}
		...
		<p>In the example below, the {props.productName} sits behind the WAN router in your site, and on-ramps only some of the existing LANs to Cloudflare.</p>
	...
	| \`TCP/443\` | - | The ${props.productName} will open outbound HTTPS connections over this port for control plane operations. |
	| \`UDP/4500\` | Destination IP \`162.159.64.1\` | Needed for ${props.productName}'s initialization and discovery traffic through outbound connections. |
