...
- Using `socat`, listen on the desired port for UDP traffic. Then, proxy that traffic to Magic Network Monitoring's destination and port.
...
If you do not want all the traffic in your device to be WARP-enabled, [configure split tunnels/proxy mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/route-traffic/split-tunnels/) to either only allow Magic Network Monitoring traffic towards `162.159.65.1` or exclude everything else.
