...
Configure your router to export <GlossaryTooltip term="sFlow">sFlow</GlossaryTooltip> data to Cloudflare's network for analysis in Network Flow (formerly Magic Network Monitoring). sFlow is a network monitoring protocol that samples network traffic to provide visibility into your network's performance and traffic patterns.

## Before you begin

Before configuring sFlow, verify the following:

- Your router supports sFlow export capabilities. Refer to [Supported routers](/network-flow/routers/supported-routers/) for a list of compatible routers.
- You have administrative access to your router's configuration interface.
- You have [registered your router with Cloudflare](/network-flow/get-started/#2-register-your-router-with-cloudflare) and noted the default sampling rate you configured during registration.
...
Refer to [Recommended sampling rate](/network-flow/routers/recommended-sampling-rate/) for guidance on selecting an appropriate sampling rate based on your network's traffic volume.
