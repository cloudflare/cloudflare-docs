...
{props.magicWord === "Cloudflare WAN" ? (
	<>
		<Markdown
			text={`
			1. Refer to [Add tunnels](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/#add-tunnels) to learn how to add IPsec tunnels.`}
			inline={false}
		/>
	</>
) : (
	<Markdown
		text={`1. Log in to <a href="https://dash.cloudflare.com/">Cloudflare dashboard</a>, and go to **Networks**.
...
2. Select **IPsec tunnel** > **Next**, and fill in the following settings:
	   ...
	   - **IPv4 Interface Address**: `10.252.2.28/31` or refer to the [Tunnel endpoints documentation](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/)
...
3. Configure the following settings:
   ...
   - **Tunnel IP:** `10.252.2.29/31` or refer to the [Tunnel endpoints documentation](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/).
...
{props.magicWord === "Cloudflare WAN" ? (
	<>
		<Markdown
			text={`1. Go to Cloudflare WAN's **Network Health** page.`}
			inline={false}
		/>
		<DashButton url="/?to=/:account/networking-insights/health" />
		<Markdown
			text={`2. Go to the **Connector health** tab.
3. Find the tunnel you have just created and make sure its status shows **Up**. Refer to [Check tunnel health in the dashboard](/cloudflare-wan/configuration/common-settings/check-tunnel-health-dashboard/) for more information.`}
			inline={false}
		/>
	</>
) : (
	<Markdown
		text={`1. Log in to <a href="https://dash.cloudflare.com/">Cloudflare dashboard</a>, and go to **Insights**.
...
- Use [Cloudflare Network Firewall](/cloudflare-network-firewall/) for network policies.
