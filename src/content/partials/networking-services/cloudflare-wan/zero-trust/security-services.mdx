...
[Cloudflare Network Firewall]({props.networkFirewallURL}) provides packet-level filtering at layers 3 and 4. You define allow or block rules based on IP addresses, ports, and protocols.
...
Gateway is the most comprehensive inspection layer for WAN traffic. For detailed setup instructions, refer to [Connect to Cloudflare Gateway with Cloudflare WAN]({props.gatewaySetupURL}).
...
- [Connect to Cloudflare Gateway with Cloudflare WAN]({props.gatewaySetupURL}): Detailed setup guide for Gateway integration with WAN traffic.
- [Cloudflare Network Firewall]({props.networkFirewallURL}): Configure packet-level filtering rules.
...
- [WAN transformation]({props.wanTransformationURL}): Plan your migration from traditional WAN to Cloudflare.
