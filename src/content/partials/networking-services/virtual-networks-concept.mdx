...
<Markdown
	text={`You can add entries to a virtual network routing table through static route configuration or routes learned from BGP peering (beta). Static routes are available for all connection types. BGP peering is currently available over CNI and IPsec/GRE tunnels (beta). For more information on how routes are prioritized within a virtual network, refer to [Traffic steering](${props.trafficSteeringURL}).`}
	inline={false}
/>
...
<Markdown
	text={`To create and configure virtual networks, refer to [Virtual networks (Tunnel configuration)](${props.tunnelConfigURL}).`}
	inline={false}
/>

<Markdown
	text={`To configure which routes belong to which virtual network, refer to [Add routes](${props.addRoutesURL}).`}
	inline={false}
/>

<Markdown
	text={`To add static routes or configure BGP peering within the Cloudflare Virtual Network routing table, refer to [Configure routes](${props.configureRoutesURL}).`}
	inline={false}
/>
...
<Markdown
	text={`- [Private networks](${props.privateNetworksURL}) — Connect your infrastructure to Cloudflare
- [Routes](${props.routesURL}) — Define IP and hostname routes through your connectors
- [Traffic steering](${props.trafficSteeringURL}) — Route prioritization, ECMP, and BGP within the Cloudflare Virtual Network`}
	inline={false}
/>
