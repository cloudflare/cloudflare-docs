...
import { AnchorHeading, Aside, APIRequest, CURL, DashButton, Markdown, Render, TabItem, Tabs } from "~/components";
...
{ props.magicWord !== "Magic Transit" && (
	<>
		...
		This feature requires the Unified Routing Mode. Refer to [Automatic Return Routing (Beta)](#) for more information.
		...
		<Markdown
		text={`
1. Follow the Add tunnels information to learn how to create an IPsec or GRE tunnel.
...
3. Select **Add tunnels** to save your changes.
		...
		<Markdown text={`Create a \`POST\` request to create an IPsec or GRE tunnel with ARR enabled:`} inline={false} />
...
By default, each BGP peering session will use the same Cloudflare-side ASN to represent peering with the {props.productName} routing table. This ASN is called the **CF Account ASN** and is set to `13335`. This can be configured to a private 2-byte ASN (for example, any values between `64512` and `65534`).
