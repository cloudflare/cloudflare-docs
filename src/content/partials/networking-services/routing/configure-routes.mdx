...
By default, each BGP peering session uses the same Cloudflare-side ASN to represent peering with the {props.productName} routing table. This ASN is called the **CF Account ASN** and is set to `13335`. You can configure this to a private 2-byte ASN (for example, any value between `64512` and `65534`).
...
5. Locate the Direct CNI interconnect to configure with BGP > select the **three dots** next to it > **Configure BGP**.
