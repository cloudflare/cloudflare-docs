...
If you have a Cloudflare WAN (formerly Magic WAN) client connected through <a href={props.greIpsecURL}>GRE</a>, <a href={props.greIpsecURL}>IPsec</a>, [CNI](/network-interconnect/) or <a href={props.warpURL}>WARP</a> and want to perform a `traceroute` to an endpoint behind a <a href={props.cfTunnelURL}>Cloudflare Tunnel</a>, the following settings must be applied for the command to return useful information.
