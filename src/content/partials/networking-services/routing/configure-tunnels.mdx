...
Refer to <a href={props.antiReplayPagePath}>Anti-replay protection</a> for more information on this topic, or [Add IPsec tunnels](#add-tunnels) to learn how to enable this feature.
...
<Markdown text={`${props.magicWord !== "Magic Transit" ? "19" : "18"}. (Optional) Select **Allow BGP (Border Gateway Protocol) peering**, if you want to use BGP peering with this tunnel.

	- [Cloudflare Account ASN](/magic-wan/configuration/manually/how-to/configure-routes/#choose-an-asn-for-bgp-peering) is set to 13335. You may change this to a private value if you want.
	...
	- (Optional) In **Additional Advertised prefix list**, input any additional prefixes you want to advertise alongside your existing routes. Leave this blank if you do not want to advertise extra routes. Typical prefixes to configure here include:
		- A route to <code>0.0.0.0/0</code>, the default route — to attract all Internet-bound traffic if using Magic WAN with Gateway.
		- A route to <code>100.96.0.0/12</code>, the portion of CGNAT space [used by default with WARP clients](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/warp-connector/user-to-site/#add-ip-route-to-router).
		- A route to <code>100.64.0.0/12</code>, the portion of CGNAT space [used by default for Cloudflare Source IPs](/magic-wan/configuration/manually/how-to/configure-cloudflare-source-ips/).
`} inline={false} />
...
9. _(Optional)_ **Tunnel health checks** are enabled by default. If you disable Tunnel health checks, your tunnels appear 100% down in your <a href={props.tunnelHealthDash}>tunnel health dashboard</a> even when working. Cloudflare keeps sending traffic through the tunnel without the means to detect if the tunnel goes down. You must set up your own system to detect down tunnels, as Cloudflare cannot warn you about down tunnels. Refer to <a href={props.tunnelHealthChecksPage}>Tunnel health checks</a> for more information.
...
{ props.magicWord !== "Magic Transit" && (
  <>
    ...
    <Markdown
      text={`
			- Now that you have set up your tunnel endpoints you need to configure routes to route your traffic through Cloudflare. Refer to [Configure routes](${props.configureRoutesURL}) to learn more about static routing and BGP peering (only available through CNI with Dataplane v2 (beta) connection).
			- After configuring either static routes or BGP routing, you need to [set up a site](${props.sitesURL}).
      `}
      inline={false}
    />
  </>
