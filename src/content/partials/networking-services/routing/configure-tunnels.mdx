...
4. In **Name**, give your tunnel a descriptive name. This name must be unique, and must not contain spaces or special characters. Hover the mouse over `i` in the dashboard for more information.
...
{ props.magicWord !== "Magic Transit" && (<>16. (Optional) Select **Automatic return routing** if you are setting up this tunnel for a site that only needs to send traffic to and receive responses from Cloudflare, and does not need to receive traffic from other sites in your WAN. Refer to [Configure Automatic Return Routing](#) for more information.</>)}
{ props.magicWord !== "Magic Transit" ? "17" : "16" }. To add multiple tunnels, select **Add GRE tunnel** for each new tunnel.
{ props.magicWord !== "Magic Transit" ? "18" : "17" }. After adding your tunnel information, select **Add tunnels** to save your changes.
...
{ props.magicWord !== "Magic Transit" && (<>17. (Optional) Select **Automatic return routing** if you are setting up this tunnel for a site that only needs to send traffic to and receive responses from Cloudflare, and does not need to receive traffic from other sites in your WAN. Refer to [Configure Automatic Return Routing](#) for more information.</>)}
{ props.magicWord !== "Magic Transit" ? "18" : "17" }. To add multiple tunnels, select **Add IPsec tunnel** for each new tunnel.
{ props.magicWord !== "Magic Transit" ? "19" : "18" }. After adding your tunnel information, select **Add tunnels** to save your changes.
