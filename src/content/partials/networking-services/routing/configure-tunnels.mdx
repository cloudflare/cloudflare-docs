...
Cloudflare recommends two tunnels for each ISP and network location router combination, one per Cloudflare endpoint. Shortly after your onboarding kickoff call, Cloudflare will assign you two Cloudflare endpoint addresses. Use these addresses as the tunnel destinations on your network location's routers/endpoints.
...
- **Cloudflare endpoint address**: The public IP address of the Cloudflare side of the tunnel. { props.magicWord === "Magic Transit" && (<>For details on where to find your Cloudflare IP addresses, refer to <a href="/magic-transit/cloudflare-ips/#check-your-cloudflare-ips">Check your Cloudflare IPs</a>.</>)}
...
For details on the technical requirements for GRE and IPsec tunnels used in {props.productName}, refer to <a href={props.tunnelsAndEncapsulationPagePath}>Tunnels and encapsulation</a>. In that page, you can also find the <a href={props.ciphersPagePath}>supported ciphers for IPsec</a>.
...
For details on this topic, refer to <a href={props.antiReplayPagePath}>Anti-replay protection</a>, or [Add IPsec tunnels](#add-tunnels) for instructions on enabling this feature.

### Network Interconnect (CNI)

Beyond GRE and IPsec tunnels, you can also use Network Interconnect (CNI) to onboard your traffic to {props.productName}. For details, refer to <a href={props.cniLink}>Network Interconnect (CNI)</a>.
...
8. In **Cloudflare GRE endpoint**, enter the anycast address you received from your account team. { props.magicWord === "Magic Transit" && (<>For details on where to find your Cloudflare IP addresses, refer to <a href="/magic-transit/cloudflare-ips/#check-your-cloudflare-ips">Check your Cloudflare IPs</a>.</>)}
9. Leave the default values for **TTL** and **MTU**.
10. _(Optional)_ **Tunnel health checks** are enabled by default. If you disable Tunnel health checks, your tunnels appear 100% down in your <a href={props.tunnelHealthDash}>tunnel health dashboard</a> even when working. Cloudflare keeps sending traffic through the tunnel without the means to detect if the tunnel goes down. You must set up your own system to detect down tunnels, as Cloudflare cannot warn you about down tunnels. For details, refer to <a href={props.tunnelHealthChecksPage}>Tunnel health checks</a>.
...
12. The **Health check type** defaults to _Reply_ and to creating an ICMP (Internet Control Message Protocol) reply. If your firewall drops this type of packet because it assumes the packet is an attack, change this option to _Request_ which creates an ICMP request. For details, refer to <a href={props.tunnelHealthChecksPage}>Tunnel health checks</a>.
...
8. In **Cloudflare endpoint**, enter the anycast address you received from your account team. { props.magicWord === "Magic Transit" && (<>For details on where to find your Cloudflare IP addresses, refer to <a href="/magic-transit/cloudflare-ips/#check-your-cloudflare-ips">Check your Cloudflare IPs</a>.</>)}
...
{ props.magicWord !== "Magic Transit" && (
  <>
    ...
    <Markdown
      text={`
			- Now that you have set up your tunnel endpoints you need to configure routes to route your traffic through Cloudflare. For details on static routing and BGP peering (only available through CNI with Dataplane v2 (beta) connection), refer to [Configure routes](${props.configureRoutesURL}).
