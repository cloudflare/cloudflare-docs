...
{ props.magicWord === "WAN Tunnels" && (
	...
	<ol>
		...
		<li>Go to <strong>Connectors</strong> &gt; <strong>WAN Tunnels</strong>, and select <strong>Create a tunnel</strong>.</li>
		<li>On the <strong>Add Tunnel</strong> page, choose either a <strong>GRE tunnel</strong> or <strong>IPsec tunnel</strong>.</li>
	</ol>
	</>
)
}
...
{ props.magicWord !== "WAN Tunnels" && (
	<>
	<ol>
		<li>In the Cloudflare dashboard, go to {props.productName}'s <strong>{props.targetPageName}</strong> page.
			<DashButton url={props.dashButtonPath} />
		</li>
		<li>From the <strong>Tunnels</strong> tab, select <strong>Create</strong>.</li>
		<li>On the <strong>Add tunnels</strong> page, choose either a <strong>GRE tunnel</strong> or <strong>IPsec tunnel</strong>.</li>
	</ol>
	</>
)
}
  ...
  <>
    ...
    <Markdown
      text={`
			...
			${props.sitesURL ? `- After configuring either static routes or BGP routing, you will have to [set up a site](${props.sitesURL}).` : ''}
      `}
      inline={false}
    />
  </>
