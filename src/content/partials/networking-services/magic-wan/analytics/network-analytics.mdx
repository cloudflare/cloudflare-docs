---
params:
  - productName
---

import { GlossaryTooltip, Render } from "~/components"

{props.productName} customers can view their real-time and historical network data in Network Analytics. Customers can see their network data in a time series that shows {props.productName} traffic (in <GlossaryTooltip term="data packet">packets</GlossaryTooltip> or bytes) over time, and can filter the time series data by different types of [packet](https://www.cloudflare.com/learning/network-layer/what-is-a-packet/) characteristics.

<Render file="analytics/network-analytics-data-averages" product="networking-services" />

Refer to [Network Analytics](/analytics/network-analytics/) documentation to learn more.

## Network traffic data filters

{/* Pass both magicProduct and productName to network-traffic-filters partial:
    - magicProduct: Used for conditional rendering (checks if === "Magic WAN" or "WAN Tunnels")
    - productName: Used in text content for product name display
    Both are passed to ensure the partial works correctly for all products */}
<Render
	file="analytics/network-traffic-filters"
	product="networking-services"
	params={{ 
		magicProduct: props.productName === "WAN Tunnels" ? "WAN Tunnels" : "Magic WAN",
		productName: props.productName
	}}
/>

## Access Magic Tunnel traffic analytics

<Render file="analytics/magic-tunnel-traffic-analytics" product="networking-services" />
