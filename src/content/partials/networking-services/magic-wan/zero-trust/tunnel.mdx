...
By default, <a href={props.gatewayURL}>Cloudflare Gateway</a> proxies and filters TCP, UDP, and ICMP traffic routed through Cloudflare WAN tunnels and destined to routes behind Cloudflare Tunnel.

## Route evaluation and precedence

Cloudflare evaluates <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/cloudflared/">private network routes</a> together across the Cloudflare Tunnel and Cloudflare Virtual Network routing tables. If traffic matches either a Cloudflare Tunnel route (in any virtual network) or a Cloudflare Virtual Network route, the matched route determines the next hop.
