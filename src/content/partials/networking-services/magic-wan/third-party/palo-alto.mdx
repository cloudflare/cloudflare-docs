...
Use the Cloudflare dashboard or API to [configure two IPsec Tunnels]({props.configureTwoIpsecTunnelsUrl}). The settings mentioned in [Add IPsec tunnels](#add-ipsec-tunnels) below are used for the IPsec tunnels referenced throughout the remainder of this guide.
...
:::caution
You need to [configure bidirectional health checks]({props.configureBidirectionalHealthChecksUrl}) with {props.productName}. The settings must include custom target IP addresses for each tunnel. Additionally, Cloudflare recommends that you lower the rate at which health check probes are sent.
:::
...
Create a [static route]({props.staticRouteUrl}) for each of the two IPsec tunnels configured in the previous section, with the following settings (settings not mentioned here can be left with their default settings):
...
Cloudflare {props.productName} implements [equal-cost multi-path (ECMP) routing]({props.equalcostMultipathEcmpRoutingUrl}) to steer traffic across IPsec tunnels. The default behavior is to load balance traffic equally across both tunnels.
