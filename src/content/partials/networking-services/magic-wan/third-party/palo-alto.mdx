...
:::caution
You need to <a href={props.configureBidirectionalHealthChecksUrl}>configure bidirectional health checks</a> with Magic WAN. The settings must include custom target IP addresses for each tunnel. Additionally, Cloudflare recommends that you lower the rate at which health check probes are sent.
:::
...
Establishing IPsec Tunnels to Cloudflare Magic WAN requires two tunnel interfaces - one to each of the two Cloudflare anycast IP addresses. You also have to ensure that `Allow_Ping` is bound to both tunnel adapters in **Advanced** > **Management Profile**.
...
Cloudflare Magic WAN implements <a href={props.equalcostMultipathEcmpRoutingUrl}>equal-cost multi-path (ECMP) routing</a> to steer traffic across IPsec tunnels. The default behavior is to load balance traffic equally across both tunnels.
