...
4. For the first IPsec tunnel, ensure the following settings are defined (refer to [Add tunnels](/magic-wan/configuration/manually/how-to/configure-tunnel-endpoints/#add-tunnels) for information on settings not mentioned here):
   - **Tunnel name**: `FITEL-tunnel-1`
   - **Interface address**: Enter `10.0.0.1/31` for your first tunnel.
   - **Customer endpoint**: This setting is not required unless your router is using an IKE ID of [type `ID_IPV4_ADDR`](/magic-wan/configuration/manually/how-to/configure-tunnel-endpoints/).
