...
{props.magicWord === "Magic WAN" ? (
	<>
		<Markdown
			text={`1. In the Cloudflare dashboard, go to Magic WAN's **Configuration** page.`}
			inline={false}
		/>
		<DashButton url="/?to=/:account/magic-wan/configuration" />
		<Markdown
			text={`2. Go to **Tunnels**, and select **Create**.`}
			inline={false}
		/>
	</>
) : (
...
3. Configure the following settings:
   ...
   - **Pre-shared key:** Copy this key. You need it for the Magic WAN tunnel configured in [Step 1](#1-configure-magic-wan).
...
{props.magicWord === "Magic WAN" ? (
	<>
		<DashButton url="/?to=/:account/magic-wan/configuration" />
		<Markdown
			text={`2. In **Tunnels**, find the IPsec tunnel you have just created.`}
			inline={false}
		/>
	</>
) : (
	<Markdown
		text={`2. In **WAN tunnels**, find the IPsec tunnel you have just created.`}
		inline={false}
	/>
)}
