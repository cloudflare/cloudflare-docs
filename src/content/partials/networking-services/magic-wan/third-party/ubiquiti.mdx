...
{props.magicWord === "Magic WAN" ? (
	<>
		<Markdown
			text={`
			1. Go to the [**IPsec/GRE tunnels**](https://dash.cloudflare.com/?to=/:account/magic-networks/connections/tunnels) page.
			2. From the list, find the IPsec tunnel you have just created.`}
			inline={false}
		/>
	</>
) : (
...
{props.magicWord === "Magic WAN" ? (
	<>
		<Markdown
			text={`1. Go to the [**Routes**](https://dash.cloudflare.com/?to=/:account/magic-networks/routes/route-config) page.
...
{props.magicWord === "Magic WAN" ? (
	<>
		...
		<Markdown
			text={`2. On the banner at the top of the page, select **Magic Tunnel Health Dashboard** to enable the new health checks experience.
