...
You can connect through[ Generic Routing Encapsulation (GRE) or IPsec tunnels]({props.genericRoutingEncapsulationGreOrIpsecTunnelsUrl}) to {props.productName}.
...
5. In **Gateway settings**, make sure you have the following settings:
   ...
   - **Local ID type**: Add the [IKE ID]({props.ikeIdUrl}) provided by Cloudflare.
...
You must use an interface address from the `/31` subnet required to [configure tunnel endpoints]({props.configureTunnelEndpointsUrl}) on {props.productName}.
...
3. Edit the interface to assign an address from the `/31` subnet required to [configure tunnel endpoints]({props.configureTunnelEndpointsUrl}). When you are finished, it should look similar to the following:
