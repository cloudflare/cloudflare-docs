...
You can connect through<a href={props.genericRoutingEncapsulationGreOrIpsecTunnelsUrl}> Generic Routing Encapsulation (GRE) or IPsec tunnels</a> to Magic WAN.
...
You must use an interface address from the `/31` subnet required to <a href={props.configureTunnelEndpointsUrl}>configure tunnel endpoints</a> on Magic WAN.
