...
For details on creating IPsec tunnels, refer to [API documentation for IPsec tunnels](/api/resources/magic_transit/subresources/ipsec_tunnels/methods/create/).
...
To begin filtering traffic and gathering analytics, refer to the <a href={props.magicFirewallUrl}>Magic Firewall documentation</a> for instructions on creating filters for east-west inter-branch traffic and the <a href={props.gatewayUrl}>Secure Web Gateway documentation</a> for instructions on configuring Gateway policies if you decide to send traffic from your local private subnets to the Internet through Cloudflare Gateway.
