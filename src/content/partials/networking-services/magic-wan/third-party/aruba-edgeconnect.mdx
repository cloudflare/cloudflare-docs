...
Aruba Orchestrator's Business Intent Overlays create intuitive policies which automatically identify and steer application traffic to Cloudflare. Two Business Intent Overlay (BIO) policies are created in this example.
...
2. Define static routes on the Orchestrator so Cloudflare can route traffic between sites.
   ...
   In this example, a route is created for the subnet `10.30.0.0/24` on the **west_branch** to be routed via the established GRE tunnel between the EdgeConnect appliance and Cloudflare.
...
To begin filtering traffic and gathering analytics, refer to the <a href={props.magicFirewallUrl}>Magic Firewall documentation</a> to learn how to create filters for east-west inter-branch traffic and the <a href={props.gatewayUrl}>Secure Web Gateway documentation</a> to learn how to configure Gateway policies if you decide to send traffic from your local private subnets to the Internet through Cloudflare Gateway.
