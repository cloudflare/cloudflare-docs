...
This document is intended to provide Arista VeloCloud customers with the steps to provision non SD-WAN destinations via Edge for connectivity with Cloudflare Magic WAN.
...
5. We are only using interfaces **GE3** and **GE4.** Disable all unused interfaces to ensure anyone with physical access to the edge node cannot connect any unused interfaces. Do this by selecting each interface one at a time and unchecking Interface Enabled followed by Save.
...
2. Expand **Non SD-WAN Destinations via Edge** and select **New**.
...
1. Configure the following settings based on your environment — left blank in this example.
...
5. Scroll down to **IPv4 Settings**, and configure the following options:
   ...
   3. **CIDR Prefix**: Add your subnet mask in CIDR notation
...
8. Scroll down to **IPv4 Settings**, and configure the following options:
   ...
   2. IP Address: Specify the WAN interface IP address
   3. CIDR Prefix: (Subnetmask in CIDR notation)
   4. Gateway: Default Gateway (0.0.0.0/0)
   5. All other settings are inherited from the Profile.
...
4. Confirm the **User Defined WAN Link** is displayed and you no longer see an indicator next to interface **GE3**.

#### VPN Services

1. Scroll down to **VPN Services**.
2. Expand **Non SD-WAN Destination via Edge** and select the **Override** checkbox.
3. Select **Add**.
...
6. In the **Action** column, select the **+** button, and configure the following options:
   1. **Public WAN Link**: Choose the Public WAN Link (refer to User-Defined WAN Link above)
   2. **Local Identification Type**: *FQDN*
   3. **Local Identification**: enter the FQDN specified when configuring Magic WAN IPsec Tunnels via the Custom FQDN IKE ID API endpoint.
...
1. Go to **Configure** > **Network Services**.
2. Expand the **Non SD-WAN Destinations via Edge** section.
3. Select the desired non SD-WAN destination - for example, `Magic_WAN_vc-edge-03`.
...
- Primary VPN Gateway:
  ...
  - Routed via the Primary VPN Gateway.
- Secondary VPN Gateway:
  ...
  - Routed via the Secondary VPN Gateway.
- Remote Magic WAN Site(s): CIDR blocks to route via Cloudflare Magic WAN
  ...
  - Assume two remote sites - each of which need to be defined as Site Subnets and routed via both the Primary VPN Gateway and Secondary VPN Gateway.
    - 172.16.32.0/24
    - 72.16.33.0/24
...
2. This is what the **Site Subnets** tab should look like when configured as indicated above:
...
Cloudflare Magic WAN and Secure Web Gateway (Cloudflare Gateway) are tightly integrated. Arista VeloCloud customers can easily route traffic via Magic WAN to Cloudflare Gateway. All Internet egress traffic is subject to Cloudflare Gateway policies.
...
3. Select **Add** to create a Business Policy Rule:
   ...
   4. **Action**
      ...
      3. **Network Service**: *Internet Backhaul* > *Non SD-WAN Destination via Edge / Cloud Security Service*.
      4. **Non SD-WAN Destination via Edge / Cloud Security Service**: Select the Network Service associated with the respective Edge device. For example, `MAgic_WAN_vc-edge-03`.
