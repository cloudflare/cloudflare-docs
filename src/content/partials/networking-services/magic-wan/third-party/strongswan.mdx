...
Start by configuring the [bidirectional health checks]({props.bidirectionalHealthChecksUrl}) target for {props.productName}. For this particular tutorial, we are using `172.64.240.252` as the target IP address, and `type` as the request.
