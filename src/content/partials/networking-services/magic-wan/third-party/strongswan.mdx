...
Start by configuring the <a href={props.bidirectionalHealthChecksUrl}>bidirectional health checks</a> target for Magic WAN. For this particular tutorial, we are using `172.64.240.252` as the target IP address, and `type` as the request.
