...
Use the Cloudflare dashboard or API to [configure two IPsec tunnels]({props.configureTwoIpsecTunnelsUrl}). The settings mentioned below are used for the IPsec tunnels referenced throughout the remainder of this guide.
...
:::note
If site-to-site traffic is a requirement for your use case, you need to enable replay protection. Refer to [Add tunnels]({props.addTunnelsUrl}) > IPsec tunnel to learn how to enable this feature.
:::
...
Create a [static route]({props.staticRouteUrl}) for each of the two IPsec tunnels configured in the previous section, with the following settings (settings not mentioned here can be left with their default values):
