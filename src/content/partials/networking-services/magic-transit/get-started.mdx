...
Draft a [Letter of Agency (LOA)](/byoip/concepts/loa/) that identifies the prefixes you want to advertise and authorizes Cloudflare to announce them. Cloudflare's transit providers require the LOA so they can accept the routes Cloudflare advertises on your behalf.
...
{ props.magicWord === "Learning Path" && (
  <>
	...
	<p>If you want to use <a href="/magic-transit/how-to/advertise-prefixes/#border-gateway-protocol-bgp-control-for-advertisements-optional">BGP for prefix advertisement control</a>, notify your account team of the IPs and ASN for your customer premises equipment (CPE) to use for the BGP peerings. You should allow around five working days for Cloudflare to add this to our Route Reflectors.</p>
  </>
  )
}
