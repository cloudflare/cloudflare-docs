...
description: Export traces and logs from Cloudflare Workers to any OpenTelemetry-compatible destination. Zero dependencies required.
sidebar:
  order: 5
products:
  - workers
...
Cloudflare Workers supports exporting OpenTelemetry (OTel)-compliant telemetry data to any destination with an available OTel endpoint. This is built in to the Workers platform — you do not need to import additional dependencies or use Tail Workers to export telemetry data.
