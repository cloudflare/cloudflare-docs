...
To send logs to a third party, use [OpenTelemetry export](/workers/observability/exporting-opentelemetry-data/) (recommended). This requires zero dependencies — you do not need to import additional packages or use Tail Workers. You can also use [Workers Logpush](/workers/observability/logs/logpush/) or [Tail Workers](/workers/observability/logs/tail-workers/).
