...
To send logs to a third party, use [OpenTelemetry export](/workers/observability/exporting-opentelemetry-data/) (recommended), [Workers Logpush](/workers/observability/logs/logpush/), or [Tail Workers](/workers/observability/logs/tail-workers/). OpenTelemetry export supports setting `persist: false` so that logs are sent directly to your destination without being stored in the Cloudflare dashboard.
...
| [Email](/email-service/api/route-emails/email-handler/)       | \<Email Recipient\>      |
