...
Minimum required Wrangler version: 3.77.0. Check your version by running `wrangler version`. To update Wrangler, refer to [Install/Update Wrangler](/workers/wrangler/install-and-update/).
:::

You must add the observability setting for your Worker to emit logs to Workers Logs. To add the setting to your Worker, add the following to your `wrangler.toml` file. 
...
To view Workers logs associated with any deployed Worker using the Cloudflare dashboard:
...
In addition, you can add custom logs throughout your code. Any `console.log` statements within your Worker will be visible in Workers Logs. The following example demonstrates a custom `console.log` within a Worker request handler.
