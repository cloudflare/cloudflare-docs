...
You can access the `tracing` object in two ways:

- **Import from `cloudflare:workers`** — use this when you do not have the handler context in scope (for example, in utility functions or library code).
- **`ctx.tracing`** — use this when you already have the handler context available.

Both resolve to the same underlying object.
...
{/* TODO: Add screenshot of trace waterfall showing nested spans (root fetch handler → handleOrder → kv_get, calculateTotal, fetch) */}
...
If you already use the [OpenTelemetry JS API](https://opentelemetry.io/docs/languages/js/instrumentation/) (`@opentelemetry/api`) in your application, you can bridge it to the Workers runtime with a small shim instead of rewriting your instrumentation.

For more information, refer to [OpenTelemetry JS SDK integration](/workers/observability/traces/opentelemetry-js/).
