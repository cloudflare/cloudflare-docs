...
title: Traces
description: Gain end-to-end visibility into request flows with automatic OpenTelemetry tracing. Zero dependencies required.
sidebar:
  order: 3
  badge:
    text: Beta
...
Cloudflare Workers provides tracing instrumentation automatically — no code changes, SDK imports, or additional dependencies are required. You do not need to add OpenTelemetry packages to your project or use Tail Workers. Simply enable tracing on your Worker and Cloudflare automatically captures telemetry data for:
...
[Axiom](/workers/observability/exporting-opentelemetry-data/axiom/). If your observability provider has an available OpenTelemetry endpoint, you can export traces (and logs) without adding dependencies to your project.
