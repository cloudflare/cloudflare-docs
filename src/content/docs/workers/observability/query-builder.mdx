...
When using [cohort-based deployments](/workers/configuration/versions-and-deployments/gradual-deployments/cohort-deployments/), you can filter logs by cohort using `$workers.cohort`. This lets you compare error rates, latency, or other metrics across different user segments during a rollout.

For example, to see all logs from the `employee` cohort:

1. Select **+** in the **Filter** section.
2. Select **Select key...** and input `$workers.cohort`.
3. Select the operator **Equals**.
4. Select **Select value...** and input `employee`.
