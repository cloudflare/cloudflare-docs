...
![Visualization Overview](~/assets/images/workers-observability/qb-visualization-overview.png)
...
![Invocations Overview](~/assets/images/workers-observability/qb-invocations-overview.png)
...
![Overview](~/assets/images/workers-observability/qb-events-overview.png)
...
In this example, we will construct a query to find and debug all paths that respond with 500 errors. First, we create a base query. In this base query, we want to visualize by
...
unexpected behavior. Then, we group by `$workers.event.request.path` and `$workers.event.response.status` to identify the number of requests that were
