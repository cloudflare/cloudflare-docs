...
2. Create a Worker.
3. Find your Worker's `wrangler.toml / wrangler.json(c)` file and add a KV namespace binding:
...
Review the following example environment variable configuration in the `wrangler.toml / wrangler.json(c)` file:
