...
2. Create a Worker.
3. Find your Worker's [Wrangler configuration](/workers/wrangler/configuration) file and add a KV namespace binding:
...
Review the following example environment variable configuration in [Wrangler's configuration](/workers/wrangler/configuration) file:
...
- <code>
  	event.respondWith(responseResponse|
  	<span style="margin-left:-6px">Promise</span>)
  </code> : void
