...
Builds that run `wrangler versions upload` create Worker versions and [Version URLs](/workers/versions-and-deployments/version-urls/). Version URLs use the resources configured for that Worker version. They are different from [Previews](/workers/previews/), which are branch-oriented and can use Preview-specific settings.
