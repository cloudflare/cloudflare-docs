...
- Preview builds create or update a [Preview](/workers/previews/). Preview builds run for branches that are not your production branch. The Preview URL is posted as a pull request comment and can also be found in the Previews section of your Worker.

To configure Preview settings such as variables, secrets, bindings, custom domains, or resource isolation, refer to [Previews](/workers/previews/).
...
Builds that run `wrangler versions upload` create Worker versions and [Version URLs](/workers/configuration/version-urls/). Version URLs use the resources configured for that Worker version. They are different from [Previews](/workers/previews/), which are branch-oriented and can use Preview-specific settings.
