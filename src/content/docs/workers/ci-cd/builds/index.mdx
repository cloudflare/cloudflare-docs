...
Connect your existing Worker to a GitHub or GitLab repository to set up automatic build and deployment for your Workers development workflow.
...
When a build is in progress you can monitor the build by viewing its logs. Navigate to **"View build history"** on the deployments page, and select **"View Build"**. The build can take on the status of in progress, queued, successful, failed and canceled builds.

When a commit is successfully built and deployed, you can also view build details for each new version created with git.

## Build Limits

In Beta, the following limits are applicable for Workers Builds:
