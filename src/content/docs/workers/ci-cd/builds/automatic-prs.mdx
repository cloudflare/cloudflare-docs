...
When you connect a repository that does not have a Wrangler configuration file, Workers Builds runs `wrangler deploy` which triggers [automatic project configuration](/workers/framework-guides/autoconfig/). Instead of failing, it creates a pull request with the necessary configuration for your detected framework.
...
:::note
When you merge the PR, Workers Builds will update your build and deploy commands if they don't match the detected settings, ensuring successful deployments.
:::
...
- The `name` field in your config doesn't match the connected Worker
