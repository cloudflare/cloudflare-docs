...
`wrangler deploy` can build Dockerfile images and roll out container instances. `wrangler versions upload` does not. Preview defaults therefore update Worker code only and leave the container image from the last full deploy in place. See [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/) and [Containers deployments](/containers/deployments/).
:::
...
For Workers that use [Containers](/containers/), the default preview command uploads a Worker version only. It does not build container images or roll out instances. If preview code requires a new image, see [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/#configure-preview-branches).
