...
`wrangler deploy` can build images and roll out container instances. `wrangler versions upload` does not. Refer to [Deploy Containers](/containers/deploy/#deploy-with-workers-builds).
:::
...
For Workers that use [Containers](/containers/), the default preview command uploads a Worker version only. It does not build container images or roll out instances. Refer to [Deploy Containers](/containers/deploy/#non-production-branches).
