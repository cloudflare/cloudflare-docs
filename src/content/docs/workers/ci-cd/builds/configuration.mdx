...
`wrangler deploy` can build and publish container images and roll out container instances. `wrangler versions upload` does not update container images. Preview URLs are not generated for Durable Object Workers, including Containers. Refer to [Deploy Containers](/containers/deploy/#before-production).
:::
