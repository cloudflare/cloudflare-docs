...
`wrangler deploy` can publish container images and roll out instances. `wrangler versions upload` uploads Worker code only and leaves the existing image in place. Refer to [Deploy Containers](/containers/deploy/#preview-versions-and-containers).
:::
...
For Workers that use [Containers](/containers/), the default preview command uploads a Worker version only. Preview still uses the containers already deployed for that Worker. Refer to [Deploy Containers](/containers/deploy/#preview-versions-and-containers).
