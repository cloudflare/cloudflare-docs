...
`wrangler deploy` can build Dockerfile images and roll out container instances. `wrangler versions upload` does not. Preview defaults therefore update Worker code only and leave the container image from the last full deploy in place. See [Deploy Containers](/containers/deploy/#deploy-with-workers-builds).
:::
...
| `npx wrangler deploy --containers-rollout=immediate` | For [Containers](/containers/) Workers that need Worker code and container instances to cut over together. See [Deploy Containers](/containers/deploy/).                                                                                                                        |
