...
For non-production branch builds (commits to branches other than your production branch, when enabled), the deploy command is replaced with the **non-production branch deploy command** (defaults to `npx wrangler versions upload`). That uploads a new Worker [version](/workers/versions-and-deployments/) without promoting it to the active production deployment. For Workers that support [preview URLs](/workers/versions-and-deployments/preview-urls/), that version also gets a preview URL.

:::note[Workers that use Containers or Durable Objects]
`wrangler deploy` can publish [container](/containers/) images and roll out container instances. `wrangler versions upload` does not update container images. Preview URLs are not generated for Durable Object Workers, including Containers and [Sandbox](/sandbox/). Refer to [Deploy Containers](/containers/deploy/#before-production).
:::
...
| `npx wrangler deploy --containers-rollout=immediate` | For [Containers](/containers/) Workers that need Worker code and container instances to cut over together. Refer to [Rollouts](/containers/platform-details/rollouts/).                                                                                                         |
| `npx wrangler deploy --containers-rollout=none`      | Deploy the Worker only; skip container image build/push and instance rollout.                                                                                                                                                                                                   |

### Non-production branch deploy command

The non-production branch deploy command applies only when [non-production branch builds](/workers/ci-cd/builds/build-branches/#configure-non-production-branch-builds) are enabled.

It defaults to `npx wrangler versions upload`, which creates a Worker version without making it the active production deployment. When preview URLs are available for the Worker, that version also gets a [preview URL](/workers/versions-and-deployments/preview-urls/). You can customize the command.

Preview URLs are not generated for Durable Object Workers, including [Containers](/containers/). For Containers, `versions upload` also does not publish a new container image. Refer to [Deploy Containers](/containers/deploy/#before-production).
