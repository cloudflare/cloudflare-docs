...
`wrangler deploy` can build Dockerfile images and roll out container instances. `wrangler versions upload` does not. Preview defaults therefore update Worker code only and leave the container image from the last full deploy in place. See [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/) and [Container deployments](/containers/deployments/).
:::
...
| `npx wrangler deploy --containers-rollout=immediate` | For [Containers](/containers/) Workers that need Worker code and container instances to cut over together. See [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/).                                                                                |
...
It defaults to `npx wrangler versions upload`, producing a [preview URL](/workers/versions-and-deployments/preview-urls/). Like the build and deploy commands, it can be customized to instead run anything.

For Workers that use [Containers](/containers/), the default preview command uploads a Worker version only. It does not build container images or roll out instances. If you need to validate a new image before production, use a separate Worker or Wrangler environment — do not point this production Worker's non-production command at `wrangler deploy` just to refresh images. See [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/#configure-non-production-preview-builds).
