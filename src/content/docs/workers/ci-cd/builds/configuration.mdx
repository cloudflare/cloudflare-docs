...
`wrangler deploy` can publish container images and roll out container instances. `wrangler versions upload` does not update images. Containers Workers use Durable Objects, so preview URLs are not generated for them. Refer to [Deploy Containers](/containers/deploy/#feature-branches-and-versions-upload).
:::
...
For Workers that use [Containers](/containers/), the default non-production command uploads a Worker version only and does not update container images. Preview URLs are not generated for Durable Object Workers. Refer to [Deploy Containers](/containers/deploy/#feature-branches-and-versions-upload).
