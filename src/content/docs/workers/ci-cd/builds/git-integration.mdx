...
Cloudflare supports connecting Cloudflare Workers to your GitHub and GitLab repositories to look for new changes to your project. Workers does not currently support self-hosted instances of GitHub or GitLab, but you can use `wrangler deploy` with an [external CI/CD provider](/workers/ci-cd/external-cicd/).
...
2. If the Cloudflare Workers and Pages installation is there, click **Configure**, and click **Uninstall "Cloudflare Pages"** (if there is no "Cloudflare Workers and Pages" installation, then no action is needed)
