...
If your external system supports custom headers, you can call the <a href="https://developers.cloudflare.com/api/resources/workers_builds/subresources/triggers/methods/create_build" target="_blank" rel="noopener noreferrer">manual build endpoint</a> with an API token in the `Authorization` header instead. This gives you token-based authentication and the ability to choose the branch per request. For a step-by-step walkthrough, see <a href="https://developers.cloudflare.com/workers/ci-cd/builds/api-reference/#trigger-a-manual-build" target="_blank" rel="noopener noreferrer">Trigger a manual build</a>.
