...
When a build runs `wrangler versions upload` and [preview URLs](/workers/versions-and-deployments/preview-urls/) are available for the Worker, the comment includes a preview URL so you can try that version while reviewing the pull request. Preview URLs are not generated for Workers that use [Durable Objects](/durable-objects/), including [Containers](/containers/).
