...
:::note
[Preview URLs](/workers/versions-and-deployments/preview-urls/) are not generated for Workers that implement a [Durable Object](/durable-objects/), including [Containers](/containers/) and [Sandbox](/sandbox/) Workers. Refer to [Deploy Containers](/containers/guides/deploy/#before-production) if your Worker uses containers.
:::
...
When enabled, every push event made to a non-production branch will trigger a build and execute the [build command](/workers/ci-cd/builds/configuration/#deploy-command), followed by the [non-production branch deploy command](/workers/ci-cd/builds/configuration/#non-production-branch-deploy-command).
