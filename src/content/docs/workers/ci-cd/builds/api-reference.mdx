...
Note the different `deploy_command`: production uses `wrangler deploy` while preview uses `wrangler preview` to create Worker Previews without affecting the live deployment.

:::note
Preview URLs are not generated for Durable Object Workers, including [Containers](/containers/). Refer to [Preview URL limitations](/workers/versions-and-deployments/preview-urls/#limitations).
:::
