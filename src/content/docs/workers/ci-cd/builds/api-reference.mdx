...
<Aside type="note">
	...
	the build system to deploy your Worker — you will need a build token UUID when
	[creating triggers](#step-4-get-your-build-token-uuid). The API token
...
Note the different `deploy_command`: production uses `wrangler deploy` (active deployment), while the non-production trigger uses `wrangler versions upload` (new version only, not the active deployment). When [preview URLs](/workers/versions-and-deployments/preview-urls/) are available for the Worker, that version also gets a preview URL. Preview URLs are not generated for Durable Object Workers, including [Containers](/containers/).
