...
`GET` and `HEAD` for the same URL share a single cache entry. A `HEAD` request that arrives on a cold cache is converted to a `GET` internally so the cache is populated with the full asset. See [Cache keys](/workers/cache/cache-keys/#what-goes-into-the-cache-key).
