...
`GET` and `HEAD` for the same URL share a single cache entry. A `HEAD` request that arrives on a cold cache is converted to a `GET` internally so the cache is populated with the full asset. Refer to [Cache keys](/workers/runtime-apis/cache/cache-keys/#what-goes-into-the-cache-key).
...
- **`206 Partial Content`** returned by your Worker is not stored — Workers Caching expects your Worker to return the full `200` response and does the range slicing itself. Refer to [`Range` requests](/workers/runtime-apis/cache/configuration/#range-requests) for the supported pattern.
...
- [Durable Objects](/durable-objects/) — Durable Object invocations are never cached, regardless of handler or method. To cache a Durable Object's HTTP responses, wrap it behind a Worker entrypoint with caching enabled. See [Cache Durable Object responses](/workers/runtime-apis/cache/#cache-durable-object-responses).

### Purge by host

There is no "purge by host" mode. The cache [belongs to the Worker, not to a domain](/workers/runtime-apis/cache/cache-keys/#the-cache-belongs-to-the-worker-not-to-a-domain) — the host is not part of the cache key, so purging by host would not map onto anything the cache stores. Use [purge by tag](/workers/runtime-apis/cache/purge/#purge-by-tag), [purge by path prefix](/workers/runtime-apis/cache/purge/#purge-by-path-prefix), or [`purgeEverything`](/workers/runtime-apis/cache/purge/#purge-everything) instead.
...
| [Cache key customization in Cache Rules](/cache/how-to/cache-rules/settings/#cache-key)                   | Workers Caching has its own key composition; see [Cache keys](/workers/runtime-apis/cache/cache-keys/). Shape the key by shaping the request (for example, by rewriting the URL or setting `ctx.props` in a gateway Worker).                                            |
...
To influence your Worker's cache, change your Worker. `Cache-Control` headers, `ctx.props`, service binding composition, and [`ctx.cache.purge()`](/workers/runtime-apis/cache/purge/) cover the configuration surface.
...
- It does not [collapse concurrent requests](/workers/runtime-apis/cache/#request-collapsing) for the same resource. A burst of traffic to a fresh URL invokes your Worker once per request.
...
| -------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| `cf.cacheKey`        | Supported                            | Supported — see [Custom cache keys](/workers/runtime-apis/cache/cache-keys/#custom-cache-keys)                        |
| `cf.cacheControl`    | Supported                            | Supported — see [Override `Cache-Control` from the calling Worker](/workers/runtime-apis/cache/configuration/#override-cache-control-from-the-calling-worker) |
