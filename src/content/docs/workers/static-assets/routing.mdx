...
2. If there are no assets that match the current route requested then the Worker will be invoked. If there is no Worker, [`not_found_handling`](/workers/static-assets/routing/#2-not_found_handling) will be evaluated. By default, a null-body 404-status response will be served. To invoke `not_found_handling` behavior _with_ a Worker, you must use the [asset binding](/workers/static-assets/binding/#runtime-api-reference).
...
`not_found_handling` determines how Cloudflare will construct a response if a request does not match a static asset, and there is no Worker script. If you do have a Worker script, this behavior **needs to be manually invoked** using the [assets binding](/workers/static-assets/binding/#runtime-api-reference).
