...
- Workers with assets will warn if ran on a [route or domain](/workers/configuration/routing/) with a path component. For example, `example.com/*` is an acceptable route, but `example.com/foo/*` will give a warning. Wrangler and the Cloudflare dashboard will show this warning when you try and add a route with a path component.
