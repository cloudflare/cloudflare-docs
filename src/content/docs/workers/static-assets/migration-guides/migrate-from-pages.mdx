...
1. [Enable preview builds](/workers/ci-cd/builds/build-branches/#configure-preview-builds) in Workers Builds.

Optionally, you can also [protect these Preview URLs with Cloudflare Access](/workers/previews/#access-control).

:::note

Worker Previews support different runtime settings for preview builds and production builds. Use [Preview settings](/workers/previews/configuration/) or the `previews` block in your Wrangler configuration to point preview builds at test bindings, variables, secrets, and resources.
