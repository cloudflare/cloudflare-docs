...
Take the following example that configures a Worker to return a response under all requests headed for `/api/`. It then uses the asset binding to invoke behaviour set by [`not_found_handling`](/workers/static-assets/routing/#routing-configuration) to catch requests that do not match.
...
export default {
	async fetch(request, env) {
		...
		// The asset binding invokes the behaviour set by `not_found_handling`.
