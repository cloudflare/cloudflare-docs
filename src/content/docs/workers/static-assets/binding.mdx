...
You can also specify `run_worker_first` as an array of route patterns to selectively run the Worker first only for specific routes, often paired with the [`single-page-application` setting](/workers/static-assets/routing/single-page-application/#advanced-routing-control):
