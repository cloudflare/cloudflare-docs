...
:::note
If you need to fetch assets from within an [RPC method](/workers/runtime-apis/bindings/service-bindings/rpc/#fetching-static-assets) (where there is no incoming `request`), construct a URL using any hostname — for example, `this.env.ASSETS.fetch(new Request('https://assets.local/path/to/asset'))`.
:::
