...
The Vite plugin can add Static Assets to the generated deployment configuration even when the input Wrangler configuration does not include `assets`. Workers with Static Assets cannot currently access `ctx.access` in the user Worker.
