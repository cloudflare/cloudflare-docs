...
- `tunnel` <Type text='boolean | { name?: string; autoStart?: boolean }' /> <MetaInfo text='optional' />
  ...
  See [Share a local dev server](/workers/local-development/local-dev-tunnels/) for more information.
...
- `remoteBindings` <Type text='boolean' /> <MetaInfo text='optional' />
  ...
  Whether or not [remote bindings](/workers/local-development/#remote-bindings) should be enabled. Defaults to `true`.
