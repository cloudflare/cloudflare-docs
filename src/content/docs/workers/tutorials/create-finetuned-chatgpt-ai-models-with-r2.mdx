...
To create a new R2 bucket use the [`wrangler r2 bucket create`](/workers/wrangler/commands/r2/#r2-bucket-create) command. Note that you are logged in with your Cloudflare account. If not logged in via Wrangler, use the [`wrangler login`](/workers/wrangler/commands/general/#login) command.
...
Next, upload a file using the [`wrangler r2 object put`](/workers/wrangler/commands/r2/#r2-object-put) command.
...
Before you deploy, you must set the `OPENAI_API_KEY` [secret](/workers/configuration/secrets/) for your application. Do this by running the [`wrangler secret put`](/workers/wrangler/commands/general/#secret-put) command:
...
1. Make sure you are in your Worker project's directory, then run the [`wrangler deploy`](/workers/wrangler/commands/general/#deploy) command:
