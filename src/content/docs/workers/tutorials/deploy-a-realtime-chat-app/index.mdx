...
To deploy your application to a custom domain using Wrangler, open your project's `wrangler.toml / wrangler.json(c)` file.

To configure a route in your `wrangler.toml / wrangler.json(c)` file, add the following to your environment:
...
If you have specified your zone ID in the environment of your `wrangler.toml / wrangler.json(c)` file, you will not need to write it again in object form.

To configure a subdomain in your `wrangler.toml / wrangler.json(c)` file, add the following to your environment:
