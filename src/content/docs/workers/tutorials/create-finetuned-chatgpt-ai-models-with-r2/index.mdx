...
To bind the R2 bucket to your Worker, add the following to your [Wrangler configuration](/workers/wrangler/configuration) file. Update the binding property to a valid JavaScript variable identifier. Replace `<YOUR_BUCKET_NAME>` with the name of the bucket you created in [step 2](#2-upload-a-fine-tune-document-to-r2):
...
Next, open the `src/index.ts` file and replace the default code with the below code. Replace `<MY_BUCKET>` with the binding name you set in [Wrangler configuration](/workers/wrangler/configuration) file.
...
Replace `<MY_BUCKET>` with the binding name you set in [Wrangler configuration](/workers/wrangler/configuration) file.
