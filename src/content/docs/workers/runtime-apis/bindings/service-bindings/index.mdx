...
You add a Service binding by modifying the [Wrangler configuration](/workers/wrangler/configuration) file of the caller — the Worker that you want to be able to initiate requests.

For example, if you want Worker A to be able to call Worker B — you'd add the following to the [Wrangler configuration](/workers/wrangler/configuration) file for Worker A:
