...
### Passing resources to Dynamic Workers

Dynamic Workers have their own lifetime, independent of any individual request. As a result, when passing resources to a Dynamic Worker, you must ensure the resource's lifetime is compatible with the worker's lifetime to ensure that the resource stays accessible.

**Use `env` for global resources**

Resources like [KV namespaces](/kv/), [R2 buckets](/r2/), or [D1 databases](/d1/) are globally available so you can safely pass them via the env binding at creation time.

**Use function parameters for Durable Object storage**

Durable Objects have their own lifecycle and can be shut down independently of a Dynamic Worker. This means you should not give a Dynamic Worker direct access to objects owned by the Durable Object, such as `this.storage`, at creation time since the Durable Object could be destroyed while the Dynamic Worker is still alive, causing references to its storage to become invalid.

Instead, pass storage access as an RPC method argument. This way, the Durable Object and Dynamic Worker keep their independent lifetimes, but the storage stays accessible for the duration of the call.

To give a Dynamic Worker access to Durable Object storage, wrap it in an [`RpcTarget`](/workers/runtime-apis/rpc/) and pass it as a function parameter. This allows the Dynamic Worker to call methods like `storage.put()` and `storage.get()` without direct access to your Durable Object. You can implement the storage interface using the [KV API](/durable-objects/api/sqlite-storage-api/#synchronous-kv-api), or use [SQLite](/durable-objects/api/sqlite-storage-api/#sql-api) for more complex queries, as the example below shows.

```ts
import { DurableObject, RpcTarget, WorkerEntrypoint } from "cloudflare:workers";

// Wrap Durable Object storage in an RpcTarget so it can be passed to Dynamic Workers.
...
export class MyDurableObject extends DurableObject<Env> {
	...
	async call(request: Request) {
		// Load Dynamic Worker that needs storage access
		...
		// Pass the StorageContext to the Dynamic Worker
		return await worker
			.getEntrypoint()
