...
A primary use case for Dynamic Worker Loaders is [Code Mode](/agents/tools/codemode/) in the [Agents SDK](/agents/). Code Mode converts your tools into typed TypeScript APIs and gives the LLM a single "write code" tool. The generated code runs in an isolated Worker sandbox, which lets AI agents chain multiple tool calls in one execution and reduces round-trips through the model.

Code Mode makes [AI SDK tools](/agents/tools/codemode/ai-sdk/), tools from an [MCP client connection](/agents/tools/codemode/mcp/), and [OpenAPI operations](/agents/tools/codemode/openapi/) available to model-written code.
...
When a Dynamic Worker enables the [new module registry](/workers/reference/module-registry/), compiled JavaScript and WebAssembly artifacts can be reused across isolate replicas. Module-level state remains local to each isolate.
...
With the [new module registry](/workers/reference/module-registry/), the identifier `index.js` maps to `file:///bundle/index.js`. URL specifiers resolve through the registry and do not fetch network resources.
