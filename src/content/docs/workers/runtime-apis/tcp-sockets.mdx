...
* When created from within a Durable Object, an open TCP socket keeps the Durable Object alive and causes it to incur duration charges. Each socket keeps the Durable Object alive for a maximum of 15 minutes; after 15 minutes, the socket stops preventing eviction (the socket itself continues operating) and the [standard eviction rules](/durable-objects/concepts/durable-object-lifecycle/) resume. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for more information.
