...
* When created from within a Durable Object, an open TCP socket keeps the Durable Object in memory (and incurring duration charges) for up to 15 minutes per connection. After 15 minutes, the socket stops pinning the Durable Object (the socket itself continues operating) and normal eviction rules resume. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for details.
