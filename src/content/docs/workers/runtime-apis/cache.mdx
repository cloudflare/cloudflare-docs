...
However, any Cache API operations in the Cloudflare Workers dashboard editor and [Playground](/workers/playground/) previews will have no impact. For Workers fronted by [Cloudflare Access](/workers/configuration/cloudflare-access/), the Cache API is not currently available.
