...
This way, synchronous writes without newline characters will always remain contiguous on the same line. For example, the following will behave as if calling `console.log('one')` followed by `console.log('two')`:
