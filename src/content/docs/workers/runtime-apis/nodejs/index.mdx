...
The `nodejs_compat` flag provides Node.js APIs. The separate [`new_module_registry` compatibility flag](/workers/configuration/compatibility-flags/#new-module-registry) provides Node.js-compatible module-loading behavior.

Enable both flags to use [`createRequire()`](https://nodejs.org/api/module.html#modulecreaterequirefilename) with `import.meta.url`, load ES modules through [`require()`](https://nodejs.org/api/modules.html#loading-ecmascript-modules-using-require), and receive the same built-in module instance through different loading APIs:
...
const require = createRequire(moduleMeta.url);
const requiredProcess = require("node:process");

export default {
	async fetch() {
		return Response.json({
			sameInstance: requiredProcess === process,
			resolved: moduleMeta.resolve("process"),
		});
	},
};
```
...
For resolution rules and CommonJS interoperability, refer to the [module registry reference](/workers/reference/module-registry/#requireesm).
