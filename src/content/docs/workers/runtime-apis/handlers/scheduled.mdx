...
You can switch on `controller.cron` to handle each schedule:
...
async function generateAndStoreReport(env) {
...
export default {
	async scheduled(controller, env, ctx) {
		switch (controller.cron) {
			case "0 * * * *":
				// Runs every hour — fetch latest data from an external API
				ctx.waitUntil(syncData());
				break;
			case "0 0 * * *":
				// Runs once a day at midnight — clean up expired KV entries
				ctx.waitUntil(cleanupExpiredKeys(env));
				break;
			case "0 0 1 * *":
				// Runs on the first of each month — generate a usage report
...
async function generateAndStoreReport(env: Env): Promise<void> {
	const report = await generateMonthlyReport(env);
	await env.REPORTS_BUCKET.put(
		`report-${Date.now()}.json`,
		JSON.stringify(report),
