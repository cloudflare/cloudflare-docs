...
Wrangler will bundle any Wasm module that ends in `.wasm` or `.wasm?module`, so that it is available at runtime within your Worker. This is done using a default bundling rule which can be customized in the `wrangler.toml / wrangler.json(c)` file. Refer to [Wrangler Bundling](/workers/wrangler/bundling/) for more information.
