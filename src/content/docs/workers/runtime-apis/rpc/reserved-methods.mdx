...
The `connect()` method of the `WorkerEntrypoint` class is reserved for accepting a socket-like connection forwarded from another Worker. Implement it to handle inbound TCP connections — refer to the [connect handler](/workers/runtime-apis/handlers/connect/) documentation. You can also [open a TCP socket from a Worker](/workers/runtime-apis/tcp-sockets/) or connect directly to databases over a TCP socket with [Hyperdrive](/hyperdrive/get-started/).
