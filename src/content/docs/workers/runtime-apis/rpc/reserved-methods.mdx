...
The `connect()` method of the `WorkerEntrypoint` class handles inbound TCP socket connections to your Worker, equivalent to the [connect handler](/workers/runtime-apis/handlers/connect/). You can also [open a TCP socket from a Worker](/workers/runtime-apis/tcp-sockets/) or connect directly to databases over a TCP socket with [Hyperdrive](/hyperdrive/get-started/).
