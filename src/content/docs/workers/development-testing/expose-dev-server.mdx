...
A named tunnel gives you a stable hostname on your own domain. This requires a Cloudflare account with a domain. You can create a named tunnel using the Cloudflare dashboard (recommended) or the `cloudflared` CLI.

<Tabs>
  <TabItem label="Dashboard (Recommended)">

  Create and manage the tunnel from the Cloudflare One dashboard:
