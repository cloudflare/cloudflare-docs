...
Your dev server stays on `localhost`. `cloudflared` handles TLS termination at the Cloudflare edge and encrypts the tunnel connection automatically. Hot reload continues to work because `wrangler dev` uses a stable proxy on port `8787` that persists across code changes.
...
Use the default mode or `--local` — both run your Worker code locally, which is what you want when tunneling back to your machine. Do not use `--remote`, which uploads your code to the Cloudflare network and defeats the purpose of a tunnel to your local server.
...
- Tunnel latency adds round-trip time to every request. This is acceptable for development but is not representative of production performance.
