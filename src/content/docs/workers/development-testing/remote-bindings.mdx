...
```jsonc title="wrangler.jsonc"
{
	...
	// VPC Services (Workers VPC) — no local simulation
	"vpc_services": [
		{
			"binding": "MY_VPC",
...
### [VPC Services](/workers-vpc/configuration/vpc-services/)

VPC Services allow your Worker to connect to private APIs and services in external clouds (AWS, Azure, GCP, on-premise, etc.) through Cloudflare Tunnel. There is no local simulation for VPC Services — remote connections are required.
