...
- Cloudflare's own [D1](/d1/) serverless SQL database, and [Durable Objects](/durable-objects/) with SQLite storage.
- Traditional hosted databases: relational databases such as Postgres and MySQL, using [Hyperdrive](/hyperdrive/) (recommended) to significantly speed up access, and NoSQL databases such as MongoDB.
- Serverless databases, including Supabase, PlanetScale, and Prisma.

### D1 and Durable Objects

Cloudflare provides two SQL database options native to Workers, both backed by SQLite and accessed through a [Workers binding](/workers/runtime-apis/bindings/) rather than a network connection.
...
[Durable Objects](/durable-objects/) offer a lower-level building block: a [SQLite-backed Durable Object](/durable-objects/best-practices/access-durable-objects-storage/) embeds a private SQLite database (up to 10 GB) that you query synchronously through the [SQL storage API](/durable-objects/api/sqlite-storage-api/) (`ctx.storage.sql.exec`). Because storage is colocated with compute, this suits per-user, per-tenant, or per-entity SQL state and distributed-system coordination. D1 is itself built on Durable Objects.
...
| [Durable Objects](/durable-objects/) | [SQL storage API](/durable-objects/api/sqlite-storage-api/) (`ctx.storage.sql`)                                                            | [Workers binding](/durable-objects/best-practices/access-durable-objects-storage/)                      |

### Traditional databases

Traditional databases connect over [TCP sockets](/workers/runtime-apis/tcp-sockets/) using a native driver. TCP is the de-facto standard protocol that many databases use for client connectivity, and these drivers are widely compatible with your preferred ORM libraries and query builders.

#### SQL databases

Relational databases such as PostgreSQL and MySQL connect using their native SQL drivers over TCP.
...
MongoDB is a document-oriented database that connects over TCP using its native Node.js driver. [Hyperdrive](/hyperdrive/) does not support MongoDB, so to pool connections and reduce latency, persist the client inside a [Durable Object](/durable-objects/). Refer to [Connect to MongoDB](/workers/databases/third-party-integrations/mongodb/) for setup instructions.

| Database                                                        | Integration       | Library or Driver                                                                     | Connection Method                                                  |
| --------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| [MongoDB](/workers/databases/third-party-integrations/mongodb/) | Direct connection | [mongodb](https://www.npmjs.com/package/mongodb), [mongoose](https://mongoosejs.com/) | [TCP Socket](/workers/runtime-apis/tcp-sockets/) via native driver |
...
If your database requires authentication, use Wrangler secrets to securely store your credentials. To do this, create a secret in your Cloudflare Workers project using the following [`wrangler secret`](/workers/wrangler/commands/general/#secret) command:
