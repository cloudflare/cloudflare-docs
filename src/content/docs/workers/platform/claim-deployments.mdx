...
Use `account.id` as the account identifier and `account.apiToken` as a Bearer token for supported Workers endpoints in the Cloudflare API. For example, you can upload and immediately deploy a Worker with the [Workers Script Upload API](/api/resources/workers/subresources/scripts/methods/update/):
