...
If your platform runs Wrangler for the user, pass the temporary account credentials to the Wrangler process. Set `CLOUDFLARE_ACCOUNT_ID` to `account.id` and `CLOUDFLARE_API_TOKEN` to `account.apiToken`:

```bash
CLOUDFLARE_ACCOUNT_ID="<TEMPORARY_ACCOUNT_ID>" \
CLOUDFLARE_API_TOKEN="<TEMPORARY_ACCOUNT_API_TOKEN>" \
npx wrangler deploy
```

Use the same environment variables for other supported Wrangler commands that create or deploy resources in the temporary account. Keep the temporary token in the execution environment, and do not write it to browser responses, client-side logs, or analytics.

#### Deploy with the Cloudflare API

You can also call supported Workers endpoints in the Cloudflare API directly. For example, you can upload and immediately deploy a Worker with the [Workers Script Upload API](/api/resources/workers/subresources/scripts/methods/update/):
