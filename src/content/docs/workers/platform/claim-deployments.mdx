---
title: Claim deployments for AI agents
...
Use the following flow to let an AI agent discover and use temporary deploys. The agent starts with the normal deploy command. If no Cloudflare credentials are available, the [Wrangler CLI](/workers/wrangler/) tells it to rerun with `--temporary`.
