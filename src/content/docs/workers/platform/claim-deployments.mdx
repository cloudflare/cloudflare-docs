...
| Step | Endpoint                                          | Authorization               | Purpose                                 |
| ---- | ------------------------------------------------- | --------------------------- | --------------------------------------- |
...
export function solvePreviewChallenge({ challengeToken, seed, k, g }) {
	const checkpoints = [];
	let hash = sha256(Buffer.from(seed, "base64url"));
...
Persist the response server-side only while both `account.expiresAt` and `claim.expiresAt` are in the future. Request a new challenge and account if either timestamp expires.

### Deploy and show the claim URL
...
<LinkCard
	title="Cloudflare REST API"
	href="/workers/platform/infrastructure-as-code/#cloudflare-rest-api"
	description="Review examples for deploying Workers with direct API calls."
/>
