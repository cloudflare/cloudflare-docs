...
For HTTP-triggered Workers, the total invocation wall time is 30 seconds. The response must be returned within this limit. Use [`ctx.waitUntil()`](/workers/runtime-apis/handlers/fetch/) to perform work after returning a response. Any tasks passed to `waitUntil()` must also complete within the same 30-second total wall time.
...
There is no set time limit on individual subrequests, but all subrequests must complete within the 30-second total wall time for the invocation. When the invocation ends, all tasks are canceled.
