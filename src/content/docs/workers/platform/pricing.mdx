...
<sup>4</sup> When [Workers Caching](/workers/runtime-apis/cache/) is enabled, requests served
from the Worker's cache are billed at the same per-request rate as requests that
invoke the Worker. This includes requests to [static
assets](/workers/static-assets/billing-and-limitations/) and [worker-to-worker
invocations](/workers/platform/pricing/#service-bindings).
...
The same Worker as Example 4, but with [Workers Caching](/workers/runtime-apis/cache/) enabled and an 80% cache hit rate. 80 million requests are served from cache and 20 million invoke the Worker. Cache hits count as requests but do not consume CPU time.
...
For details on what is cached and how to enable caching, refer to [Cache](/workers/runtime-apis/cache/).
...
:::note[Workflows pricing]

To learn more about Workflows billing, refer to [Workflows pricing](/workflows/reference/pricing/).
