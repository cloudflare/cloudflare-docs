...
A project that serves 15 million requests per month, with 80% (12 million) requests serving [static assets](/workers/static-assets/) and the remaining invoking dynamic Woker code. The Worker uses an average of 7 milliseconds (ms) of CPU time per request.
