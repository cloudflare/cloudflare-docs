...
**Properties:**
- `type`: `"kv_namespace"`
- `name`: The binding name to access via `env.MY_KV`
...
- `environment`: (Optional) The environment of the script if binding to an external Worker
...
- `environment`: (Optional) The environment of the target Worker
...
Bind to [Workers AI](/workers-ai/) for machine learning inference:
