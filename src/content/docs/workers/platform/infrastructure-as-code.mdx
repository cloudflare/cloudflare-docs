...
## Alchemy (TypeScript IaC)

Define, deploy, and version Cloudflare resources in TypeScript; in the same repo as the rest of your code. This improves reviewability, repeatability, and most importantly DX while keeping guardrails close to production. See [Alchemy](https://alchemy.run/) for more.

All examples on this page require an account ID and API token.
...
- **Fast path**:
  1. **Install**
     ...
     ```
  2. **Create** `alchemy.run.ts` (project root)
     ```ts
     ...
     await app.finalize();
     ```
  3. **Define resources** Fully TypeScript. Example Worker and Queue:
     ```ts
     import { Worker, Queue } from "alchemy/cloudflare";

     const hello = await Worker("hello-worker", {
       name: "hello-worker",
     ...
     console.log(`Worker URL: ${hello.url}`);

     const jobs = await Queue("jobs-queue", { name: "jobs-queue" });
     console.log(`Queue: ${jobs.name}`);
     ```
  4. **Deploy**
     ```bash
     bun alchemy.run.ts
...
- **Why use it**:
  - **TypeScript-first**: autocompletion and types for Cloudflare resources.
  - **Single source of truth**: infra and app evolve together; easier reviews and rollbacks.
  - **Fast feedback**: local runs feel like app code, not a separate toolchain.

- **Tradeoffs and ops notes**:
  - **Credentials**: provide Cloudflare account ID and API token via environment variables or your secrets manager. Never commit secrets.
  - **State**: Alchemy persists state as JSON files; commit to VCS for history. Avoid manual edits to prevent drift.
  - **Durable Objects**: migrations apply with deployments. If you bind a Durable Object class, ensure migrations are applied before expecting bindings to succeed (same constraint noted below in this doc).
  - **Monitoring**: after deploy, use Workers Analytics/Logs; for Queues, watch enqueue/dequeue/error metrics. Set alerts on failure rates and latency.
  - **Team workflow**: run bundling (for Workers code) before `up` in CI, then run the Alchemy script. Prefer PR review for infra changes.
