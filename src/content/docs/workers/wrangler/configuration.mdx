...
- `image` <Type text="string" /> <MetaInfo text="required" />
  ...
  - Not supported when `scheduling_policy` is `"durable_object"`. Use `images` instead.
- `class_name` <Type text="string" /> <MetaInfo text="required" />
...
- `scheduling_policy` <Type text="string" /> <MetaInfo text="optional" />
  - Sets how the Durable Object class runs its containers. The options are `"default"` and `"durable_object"`. The default is `"default"`.
  - With `"durable_object"`, each Durable Object starts its own container with [`start()`](/durable-objects/api/container/#start) on `this.ctx.container` and chooses the image when it starts. This policy is required for `images` and for [container snapshots](/durable-objects/api/container/#snapshotcontainer).
  - With `"durable_object"`, Wrangler supports only `name`, `class_name`, `scheduling_policy`, and `images` from this list, and rejects the other options.
  - With `"durable_object"`, `wrangler dev` runs the containers locally with Wrangler 4.136.0 or later. Earlier versions do not attach `this.ctx.container`, or reject images that do not declare a port with `EXPOSE`.
- `images` <Type text="Record<string, object>" /> <MetaInfo text="optional" />
  - Named images that a Durable Object can start when `scheduling_policy` is `"durable_object"`. With other scheduling policies, Wrangler warns and ignores `images`.
  - Each key is an image name of 1 to 128 characters, with at most 100 images. Each value sets exactly one of `dockerfile` or `image`:
    - `dockerfile` <Type text="string" />
      - The path to a `Dockerfile` that `wrangler deploy` builds and pushes.
    - `build_context` <Type text="string" /> <MetaInfo text="optional" />
      - The build context for `dockerfile`, relative to the Wrangler configuration file. Defaults to the directory of the `Dockerfile`.
    - `build_vars` <Type text="Record<string, string>" /> <MetaInfo text="optional" />
      - Variables for `dockerfile` that are available only while the image builds.
    - `image` <Type text="string" />
      - A digest-pinned image in your account's Cloudflare Registry, in the form `registry.cloudflare.com/<ACCOUNT_ID>/<REPOSITORY>@sha256:<DIGEST>`. Cannot be combined with `build_context` or `build_vars`.
  - In the Durable Object, `this.ctx.container.images.<NAME>` returns the image reference to pass to `start()`. For more information, refer to [`images`](/durable-objects/api/container/#images).
...
The following configuration sets `scheduling_policy` to `"durable_object"` and gives the class one image that Wrangler builds from a `Dockerfile`:
...
Each Durable Object then passes `this.ctx.container.images.workspace` to `start()`.
