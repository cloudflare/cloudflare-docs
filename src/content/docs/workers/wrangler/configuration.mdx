...
Only policy-level fields such as `name`, `class_name`, `scheduling_policy`, and `images` apply to a Durable Object-managed entry. Configure its image and instance size in `ctx.container.start()` instead of setting `image`, `instance_type`, or rollout fields here.
