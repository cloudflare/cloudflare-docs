...
- `rollout_active_grace_period` <Type text="number" /> <MetaInfo text="optional" />
  - During a [rollout](/containers/platform-details/rollouts/), the minimum number of seconds a Durable Object must have been connected to a container instance before that instance may be replaced. Recently connected instances are skipped until they pass this window. Defaults to `0` (no extra delay). After an instance is selected, the platform still sends `SIGTERM` and waits up to 15 minutes before `SIGKILL`. This setting still applies when you pass `--containers-rollout=immediate`.
- `rollout_step_percentage` <Type text="number | number[]" /> <MetaInfo text="optional" />
  - Configures what percentage of instances should be updated at each step of a [rollout](/containers/platform-details/rollouts/).
  ...
  - Defaults to `[10, 100]`.
  - Override for one deploy with `--containers-rollout=immediate`, which uses a single step that targets 100% of instances. That does not replace every instance at the same instant; each instance still stops and starts on its own timeline. The flag does not override `rollout_active_grace_period`.
