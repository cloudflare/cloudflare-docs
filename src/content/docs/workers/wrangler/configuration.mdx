...
- `html_handling`: <Type text="'auto-trailing-slash' | 'force-trailing-slash' | 'drop-trailing-slash' | 'none'" /> <MetaInfo text="optional" />
  ...
  - Determines redirects/rewrites of requests for HTML content. If unspecified, the default mode is `'auto-trailing-slash'`. Learn more [here](/workers/static-assets/routing/#html_handling).

- `not_found_handling`: <Type text="`'single-page-application' | '404-page' | 'none'`" /> <MetaInfo text="optional" />

  - Determines the handling of requests that do not map to an asset. If unspecified, the default mode is `'not_found_handling'`. Learn more [here](/workers/static-assets/routing/#not_found_handling).
