...
description:
  Use a [Wrangler configuration](/workers/wrangler/configuration) file to customize the
...
Wrangler optionally uses a [Wrangler configuration](/workers/wrangler/configuration) file to customize the development and deployment setup for a Worker.

:::caution

Wrangler currently supports an `--experimental-json-config` flag, which will read your configuration from a `wrangler.json` file, rather than [Wrangler configuration](/workers/wrangler/configuration) file. The format of this file is exactly the same as the [Wrangler configuration](/workers/wrangler/configuration) file, except that the syntax is `JSON` rather than `TOML`. This is experimental, and is not recommended for production use.
:::

It is best practice to treat [Wrangler configuration](/workers/wrangler/configuration) file as the [source of truth](#source-of-truth) for configuring a Worker.

## Sample [Wrangler configuration](/workers/wrangler/configuration)
...
As of March 1, 2024 the [usage model](/workers/platform/pricing/#workers) configured in your Worker's [Wrangler configuration](/workers/wrangler/configuration) file will be ignored. The [Standard](/workers/platform/pricing/#example-pricing-standard-usage-model) usage model applies.
...
Example of a [Wrangler configuration](/workers/wrangler/configuration) that includes an mTLS certificate binding:
...
- `bucket` <Type text="string" /> <MetaInfo text="required" />
  ...
  - The directory containing your static assets. It must be a path relative to your [Wrangler configuration](/workers/wrangler/configuration) file.
...
We recommend treating your [Wrangler configuration](/workers/wrangler/configuration) file as the source of truth for your Worker configuration, and to avoid making changes to your Worker via the Cloudflare dashboard if you are using Wrangler.

If you need to make changes to your Worker from the Cloudflare dashboard, the dashboard will generate a TOML snippet for you to copy into your [Wrangler configuration](/workers/wrangler/configuration) file, which will help ensure your [Wrangler configuration](/workers/wrangler/configuration) file is always up to date.

If you change your environment variables in the Cloudflare dashboard, Wrangler will override them the next time you deploy. If you want to disable this behavior, add `keep_vars = true` to your [Wrangler configuration](/workers/wrangler/configuration) file.

If you change your routes in the dashboard, Wrangler will override them in the next deploy with the routes you have set in your [Wrangler configuration](/workers/wrangler/configuration) file. To manage routes via the Cloudflare dashboard only, remove any route and routes keys from your [Wrangler configuration](/workers/wrangler/configuration) file. Then add `workers_dev = false` to your [Wrangler configuration](/workers/wrangler/configuration) file. For more information, refer to [Deprecations](/workers/wrangler/deprecations/#other-deprecated-behavior).
