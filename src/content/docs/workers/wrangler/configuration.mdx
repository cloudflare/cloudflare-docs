...
If you change your routes in the dashboard, Wrangler will override them in the next deploy with the routes you have set in your `wrangler.toml`. To manage routes via the Cloudflare dashboard only, remove any route and routes keys from your [Wrangler configuration](/workers/wrangler/configuration) file. Then add `workers_dev = false` to your [Wrangler configuration](/workers/wrangler/configuration) file. For more information, refer to [Deprecations](/workers/wrangler/deprecations/#other-deprecated-behavior).
