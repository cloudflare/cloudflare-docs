...
- `rollout_step_percentage` <Type text="number | number[]" /> <MetaInfo text="optional" />
  - Percentage of container instances to update at each [rollout](/containers/platform-details/rollouts/) step. A single number uses that step size (`5`, `10`, `20`, `25`, `50`, or `100`). An array is cumulative and must end in `100`. Defaults to `[10, 100]`. Override for one deploy with `--containers-rollout=immediate` (single 100% step; does not override grace period).
