...
- `trusted_user_ca_keys` <Type text="object[]" /> <MetaInfo text="optional" />
  - Public keys that will be used with the `TrustedUserCAKeys` sshd option
<WranglerConfig>
...
Configuration for SSH'ing into a container instance through Wrangler.

The following options are available:

- `enabled` <Type text="boolean" />
  - Whether or not SSH through Wrangler is enabled. Defaults to `false`.
...
- `public_key` <Type text="string">
  - The public key itself.
  - Currently only the `ssh-ed25519` key type is supported.
  
### Trusted User CA Keys

Trusted user CA keys are public keys used to sign another key. Any key signed by a trusted user CA key will be authorized by SSH. For more details, see https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/6/html/deployment_guide/sec-creating_ssh_ca_certificate_signing-keys.

The format of the `trusted_user_ca_keys` option is the same as `authorized_keys`.
