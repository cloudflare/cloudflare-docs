...
These are defined under `env.$NAME` keys, such as `env.staging` which you can then preview or deploy with the `-e` / `--env` flag in `wrangler` commands such as `npx wrangler deploy --env staging`.
