...
- `rollout_active_grace_period` <Type text="number" /> <MetaInfo text="optional" />
  - During a [rollout](/containers/platform-details/rollouts/), minimum seconds the Durable Object must have been connected to a container instance before that instance may be replaced. Defaults to `0`. Still applies with `--containers-rollout=immediate`.
- `rollout_step_percentage` <Type text="number | number[]" /> <MetaInfo text="optional" />
  - Percentage of instances to update at each [rollout](/containers/platform-details/rollouts/) step. A single number uses that step size (`5`, `10`, `20`, `25`, `50`, or `100`). An array is cumulative and must end in `100`. Defaults to `[10, 100]`. Override for one deploy with `--containers-rollout=immediate` (single 100% step; does not override grace period).
