...
To communicate with origins that require client authentication, a Worker can present a certificate for mTLS in subrequests. Wrangler provides the `mtls-certificate` [command](/workers/wrangler/commands/certificates/#mtls-certificate) to upload and manage these certificates.
...
- `instance_type` <Type text="string" /> <MetaInfo text="optional" />
  - The instance type of the container. This determines the amount of memory, CPU, and disk given to the container
    ...
    the see [instance types documentation](/containers/platform-details/limits/#instance-types).
