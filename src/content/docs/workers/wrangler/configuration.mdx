...
- `retention` <Type text="object" /> <MetaInfo text="optional" />
  - Configures [asset retention](/workers/static-assets/asset-retention/) for this Worker. When enabled, assets from recently deployed versions remain reachable, preventing version skew breakage for SPAs and static sites. Enabling is immediately retroactive over the last 24 hours of deployments.
  - `enabled` <Type text="boolean" /> <MetaInfo text="required" />
    - Set to `true` to enable asset retention.
