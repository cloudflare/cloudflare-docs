...
- `binding` <Type text="string" /> <MetaInfo text="required" />
  - Descriptive name for your binding. This will be used in the Workers application when [accessing your secret on the env object](/secrets-store/integrations/workers/#3-access-the-secret-on-the-env-object).
- `store_id` <Type text="string" /> <MetaInfo text="required" />
  - The corresponding Secrets Store ID where your account secret was created.
