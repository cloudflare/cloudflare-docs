...
* `exports` <Type text="object"/> <MetaInfo text="optional"/>
  - Declares the Durable Object classes this Worker exports and their lifecycle state (`created`, `deleted`, `renamed`, `transferred`, `expecting-transfer`). Refer to [Durable Object class exports](/durable-objects/reference/durable-objects-migrations/). Durable Object entries are mutually exclusive with `migrations`.
...
The `exports` field declares the Workflows this Worker defines, without a binding. Refer to [Declare Workflows in `exports`](/workflows/build/workers-api/#declare-workflows-in-exports).
...
- `limits` <Type text="object" /> <MetaInfo text="optional" />
  - `steps` <Type text="number" /> <MetaInfo text="optional" /> — The maximum number of steps a Workflow instance can run. Refer to [Workflow step limits](/workflows/build/workers-api/#workflow-step-limits).
- `schedules` <Type text="string | string[]" /> <MetaInfo text="optional" />
  - One or more cron schedules that create new instances of this Workflow automatically. Refer to [Schedule a Workflow directly](/workflows/build/trigger-workflows/#schedule-a-workflow-directly).
- `default_retention` <Type text="object" /> <MetaInfo text="optional" />
...
The Worker calls each Workflow declared in `exports` through [`ctx.exports`](/workflows/build/workers-api/#call-a-workflow-through-ctxexports), keyed by class name.
