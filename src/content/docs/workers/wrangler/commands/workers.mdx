...
- `PATH` <Type text="string" />
  ...
  - To deploy an explicit static directory without creating a Wrangler configuration file, pass a directory that contains `index.html`.
  - To deploy a static Vite app directory without creating a Wrangler configuration file, pass the directory with `--experimental-auto-config-static-assets`.
  - To configure and deploy a Dockerfile-backed Container project, pass `Dockerfile` or `Containerfile`.
...
- `--experimental-auto-config-static-assets` <Type text="boolean" /> <MetaInfo text="optional experimental" />
  - Deploy an explicit static Vite app directory without creating a Wrangler configuration file. Requires `PATH` to be a directory.
- `--site` <Type text="string" /> <MetaInfo text="optional deprecated, use `--assets`" />
  - Folder of static assets for Workers Sites.
    :::caution
    Workers Sites is deprecated. Please use [Workers Assets](/workers/static-assets/) or [Pages](/pages/).
...
To configure a Dockerfile-backed Containers project non-interactively, run `wrangler setup --yes` from a project root that contains a `Dockerfile` or `Containerfile`.
