...
Add or update a secret for one Preview.

```txt
wrangler preview secret put SECRET_NAME --name <preview-name> [OPTIONS]
```

- `SECRET_NAME` <Type text="string" />
  - The name of the secret to add or update.
- `--name` <Type text="string" /> <MetaInfo text="required" />
  - The name of the Preview to update.

---

### `preview secret delete`

Delete a secret from one Preview.

```txt
wrangler preview secret delete SECRET_NAME --name <preview-name> [OPTIONS]
```
...
List secrets configured for one Preview.

```txt
wrangler preview secret list --name <preview-name> [OPTIONS]
```

- `--name` <Type text="string" /> <MetaInfo text="required" />
  - The name of the Preview to inspect.

---

### `preview secret bulk`

Upload multiple secrets for one Preview.

```txt
wrangler preview secret bulk --name <preview-name> [OPTIONS]
```
...
### `preview base-config push`

Update the base configuration from your Wrangler `previews` block. New Previews start with this base configuration.

```txt
wrangler preview base-config push [OPTIONS]
```

- `--apply-to-existing` <Type text="boolean" /> <MetaInfo text="optional" />
  - Apply the base configuration change to Previews that already exist, instead of new Previews only.
