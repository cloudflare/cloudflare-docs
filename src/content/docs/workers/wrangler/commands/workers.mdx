...
- `--containers-rollout` <Type text="immediate | gradual | none" /> <MetaInfo text="optional" />
  - Specify the [rollout strategy](/containers/platform-details/rollouts/) for [Containers](/containers/) associated with the Worker.
  - `gradual` (default): rolling update using `rollout_step_percentage` (default 10% then 100%).
  - `immediate`: one rollout step that targets 100% of instances, overriding `rollout_step_percentage`. Instances still stop and start over time; not a simultaneous fleet restart. `rollout_active_grace_period` still applies.
  - `none`: deploy the Worker without building or updating associated Containers.
