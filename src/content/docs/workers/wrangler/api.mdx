...
The `getPlatformProxy` function uses bindings found in the Wrangler configuration file. For example, if you have an [environment variable](/workers/configuration/environment-variables/#add-environment-variables-via-wrangler) configuration set up in the Wrangler configuration file:
