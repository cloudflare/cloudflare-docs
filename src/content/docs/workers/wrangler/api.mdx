...
- `options` <Type text="object" /> <MetaInfo text="optional" />
  ...
  - Optional options object containing preferences for the bindings:
    ...
    - `populateProcessEnv` boolean
      ...
      When `true`, `process.env` is populated with environment variables and secrets from your Wrangler configuration and `.env` files. This mirrors the behavior of workerd with the [`nodejs_compat_populate_process_env`](/workers/configuration/compatibility-flags/#enable-auto-populating-processenv) compatibility flag.

      - `true`: Always populate `process.env`

      - `false`: Never populate `process.env`

      - `undefined` (default): Follows the same rules as workerd:

        - Enabled when `nodejs_compat` is set AND either `compatibility_date` is `2025-04-01` or later, OR `nodejs_compat_populate_process_env` flag is explicitly set

        - Disabled when `nodejs_compat_do_not_populate_process_env` flag is set

      JSON binding values are stringified when added to `process.env`. Original `process.env` values are restored when `dispose()` is called.
...
- `dispose()` () => `Promise<void>`
  - Terminates the underlying `workerd` process and restores any original `process.env` values that were overwritten if `populateProcessEnv` was enabled.
...
When `populateProcessEnv` is enabled (either explicitly or via compatibility settings), you can also access text and JSON bindings through `process.env`:
...
console.log(process.env.MY_VARIABLE); // prints: "test"

// Clean up and restore original process.env values
await dispose();
```
