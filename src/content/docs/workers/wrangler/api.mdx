...
- `options` <Type text="object" /> <MetaInfo text="optional" />
  ...
  - Optional options object containing preferences for the bindings:
    ...
    - `configPath` string
      ...
      If no path is specified the default behavior is to search from the current directory up the filesystem for a [Wrangler configuration](/workers/wrangler/configuration) file to use.
...
The `getPlatformProxy` function uses bindings found in [Wrangler's configuration](/workers/wrangler/configuration) file. For example, if you have an [environment variable](/workers/configuration/environment-variables/#add-environment-variables-via-wrangler) configuration set up in [Wrangler's configuration](/workers/wrangler/configuration) file:
...
All supported bindings found in your [Wrangler configuration](/workers/wrangler/configuration) file are available to you via `env`.
