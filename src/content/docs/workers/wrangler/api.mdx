...
Hyperdrive values are simple passthrough ones.   Values provided by hyperdrive bindings such as `connectionString` and `host` do not have a valid meaning outside of a `workerd` process.
