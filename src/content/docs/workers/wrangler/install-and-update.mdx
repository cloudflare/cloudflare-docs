...
Run `wrangler --version` to confirm you are on the latest release. Many issues are fixed in newer versions — upgrade first and retry your command.
...
```sh
WRANGLER_LOG=debug WRANGLER_LOG_PATH=./wrangler-debug.log wrangler deploy
```
...
1. Wrangler version (`wrangler --version`).
