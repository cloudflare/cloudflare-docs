...
* `WRANGLER_LOG_PATH` <Type text="string" /> <MetaInfo text="optional" />
  ...
  * A file or directory path where Wrangler will write its logs. When specifying a path to a file, the filename must end with `.log`. When specifying a directory or using the default location, a filename with a timestamp is used.
