...
2. You create an environment, for example, `dev`, in the Worker's `wrangler.toml / wrangler.json(c)` file.
3. In `wrangler.json(c)`, you configure the `dev` environment by [adding bindings](/workers/runtime-apis/bindings/) and/or [routes](/workers/configuration/routing/routes/).
...
[Bindings](/workers/runtime-apis/bindings/) and [environment variables](/workers/configuration/environment-variables/) must be specified per each [environment](/workers/wrangler/environments/) in your [`wrangler.toml / wrangler.json(c)` file](/workers/wrangler/configuration/).
