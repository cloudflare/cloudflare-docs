...
Use Wrangler environments for persistent Workers such as staging or development. Use [Worker Previews](/workers/previews/) when you want every branch or pull request to get a URL. Use [Version URLs](/workers/configuration/version-urls/) when you want to test one uploaded Worker version. For a side-by-side comparison, refer to [Previews, Version URLs, and Wrangler environments](/workers/configuration/compare-preview-workflows/).
