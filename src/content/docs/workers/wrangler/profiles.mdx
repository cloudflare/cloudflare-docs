...
Run [`wrangler auth create`](/workers/wrangler/commands/general/#auth) with a name. Wrangler starts the OAuth flow, where you choose which accounts the profile may reach.
...
Run [`wrangler auth activate`](/workers/wrangler/commands/general/#auth) to bind a profile to a directory. The binding applies to that directory and its subdirectories. The directory defaults to the current working directory.
...
Run [`wrangler auth list`](/workers/wrangler/commands/general/#auth) to see every profile and the directories bound to it.
...
To stop a directory from using a profile, run [`wrangler auth deactivate`](/workers/wrangler/commands/general/#auth) in that directory. The directory returns to the profile bound above it, or to the default profile.

```sh
wrangler auth deactivate ~/projects/staging
```

To remove a profile and all of its directory bindings, run [`wrangler auth delete`](/workers/wrangler/commands/general/#auth).
