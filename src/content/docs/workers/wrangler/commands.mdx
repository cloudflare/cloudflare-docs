...
- `--port` <Type text="number" /> <MetaInfo text="optional" />
  - Port to listen on. If you change the port, make sure any [Cloudflare Tunnel pointing to your dev server](/workers/development-testing/expose-dev-server/) uses the same port.
