...
Create a skeleton `wrangler.toml / wrangler.json(c)` file in an existing directory. This command can be used as an alternative to `generate` if you prefer to clone a template repository yourself or you already have a JavaScript project and would like to use Wrangler.
...
- `--preview` optional
  - Interact with a preview namespace instead of production. Pass this to use your `wrangler.toml / wrangler.json(c)` file's `kv_namespaces.preview_id` instead of `kv_namespaces.id`
...
Wrangler supports any `wrangler.toml / wrangler.json(c)` file keys passed in as environment variables. This works by passing in `CF_` + any uppercased TOML key. For example:
