...
Previous versions of Wrangler offered rudimentary support for [webpack](https://webpack.js.org/) with the `type` and `webpack_config` keys in the [`wrangler.toml / wrangler.json(c)` file](/workers/wrangler/configuration/). Starting with Wrangler v2, Wrangler no longer supports the `type` and `webpack_config` keys, but you can still use webpack with your Workers.
...
4. Remove unsupported entries from your `wrangler.toml / wrangler.json(c)` file.
