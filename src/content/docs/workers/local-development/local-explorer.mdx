...
In Local Explorer's **Observability** section, each Worker invocation appears as a trace. Select any trace to expand a waterfall showing every binding operation (D1 queries, KV reads, R2 operations, Durable Object calls, `fetch()`) with timing, status, and error details.

#### Example: Debug a failed request

This Worker makes two D1 calls. The second references a table that does not exist:
...
if (pathname === "/api/todos" && method === "POST") {
  ...
  ).bind("created", todo!.id).run(); // ← fails here: table does not exist

  return Response.json({ todo }, { status: 201 });
}
```

The trace waterfall shows which D1 call succeeded and which failed. No `console.log()` or try/catch required.

![Trace waterfall for POST /api/todos showing two D1 database spans: the first INSERT succeeded (green), the second INSERT failed with error "no such table: audit_log"](~/assets/images/workers/observability/local-trace-failed-request.png)

### Search logs

The **Logs** view captures all console output from your Worker. Filter by level (error, warn, info, log, debug) or search by text to find specific messages without scrolling through terminal output.
...
The OpenAPI spec at `/cdn-cgi/explorer/api` allows AI coding agents and other tools to discover and interact with your local development environment programmatically. An agent can fetch the spec, understand what resources are available, and make API calls to read or modify local data, query traces and logs, and debug your Worker without requiring manual setup.
