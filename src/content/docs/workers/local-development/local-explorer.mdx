...
:::note
Local observability captures traces and logs from `wrangler dev` on your machine. For production Workers, see [Workers Logs](/workers/observability/logs/workers-logs/) and [Traces](/workers/observability/traces/).
:::

### View traces

In Local Explorer's **Observability** section, each Worker invocation appears as a trace. Click any trace to expand a waterfall showing every binding operation (D1 queries, KV reads, R2 operations, Durable Object calls, `fetch()`) with timing, status, and error details.

#### Example: Debug a failed request

This Worker makes two D1 calls. The second references a table that doesn't exist:
