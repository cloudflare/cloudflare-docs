...
sidebar:
  order: 5
head: []
description: Browse and edit local binding data, and invoke email or scheduled handlers from your browser during development.
products:
  - workers
---
...
2. Open Local Explorer in your browser:
   ...
   - **Vite plugin**: go directly to `/cdn-cgi/local/explorer` using your development server route and port.
</Steps>
...
Local Explorer can send one-off invocations to the [`scheduled()` handler](/workers/runtime-apis/handlers/scheduled/) for your Worker. Configure at least one [Cron Trigger](/workers/configuration/cron-triggers/) for the active environment before using this interface.

Cron Trigger testing requires Wrangler 4.137.0 or later, or Cloudflare Vite plugin 1.58.0 or later.
...
4. Review the outcome and `noRetry()` status. To inspect logs and traces, select **Observability** > **Traces**.
...
### Dispatch a scheduled invocation

Send a `POST` request to `/local/scheduled` to invoke the `scheduled()` handler for a Worker. The `worker` query parameter must match the exact Worker name available to Local Explorer.
...
The `cron` field is required. The optional `scheduled_time` field sets `controller.scheduledTime` with epoch milliseconds. The response includes the handler outcome and `noRetry()` status.
