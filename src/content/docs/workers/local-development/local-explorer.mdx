...
Local Explorer includes an **Observability** section that automatically traces every Worker invocation.

Every binding call (D1 queries, KV reads, R2 operations, Durable Object calls, `fetch()`) is captured as a span with timing, status, and error details. Use the **Traces** view to expand a waterfall of spans and see exactly where a request failed. Use the **Logs** view to filter and search captured `console.*` output by level instead of scrolling through terminal output.

For debugging scenarios and examples, refer to [Local observability](/workers/local-development/local-observability/).
...
The OpenAPI spec at `/cdn-cgi/explorer/api` allows AI coding agents and other tools to discover and interact with your local development environment programmatically. An agent can fetch the spec, understand what resources are available, and make API calls to read or modify local data, query [traces and logs](/workers/local-development/local-observability/), and debug your Worker without requiring manual setup.
