...
Pyodide is a [CPython](https://github.com/python/cpython) (the reference implementation of Python — commonly referred to as just "Python") compiled to WebAssembly.

When you write a Python Worker, your code is interpreted directly by Pyodide, within a V8 isolate.
Refer to [How Workers works](/workers/reference/how-workers-works/) to learn more.
...
...and a [Wrangler configuration file](/workers/wrangler/configuration/) that points to this `.py` file:
...
A new version of Python is released every year in August, and a new version of Pyodide is released six (6) months later.
...
This lets us continually provide updates, without risk of breaking changes, extending the commitment we’ve made for JavaScript to Python.

Each Python release has a [five (5) year support window](https://devguide.python.org/versions/).
...
Following the "A change to the Workers Runtime must never break an application that is live in production." policy,
