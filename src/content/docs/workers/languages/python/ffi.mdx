...
We recommend using the `workers` module, which is provided by our `workers-runtime-sdk` package, to access JavaScript globals in a more Pythonic way.
However, if you need to access JavaScript globals directly for any reason, you can import them from the `js` module.
