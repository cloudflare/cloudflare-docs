...
- [Wrangler configuration](/workers/wrangler/configuration) file - Wrangler configuration, pre-populated with a custom build command to invoke `worker-build` (Refer to [Wrangler Bundling](/workers/languages/rust/#bundling-worker-build)).
...
- [`Var`](https://github.com/cloudflare/workers-rs/blob/e15f88110d814c2d7759b2368df688433f807694/worker/src/env.rs#L92) - Environment variable defined in [Wrangler's configuration](/workers/wrangler/configuration) file.
...
`worker-build` is invoked by default in the template project using a custom build command specified in [Wrangler's configuration](/workers/wrangler/configuration) file.
