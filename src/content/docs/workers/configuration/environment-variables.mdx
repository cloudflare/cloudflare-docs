...
Text and JSON values are defined via the `[vars]` configuration in your [Wrangler configuration](/workers/wrangler/configuration) file. In the following example, `API_HOST` and `API_ACCOUNT_ID` are text values and `SERVICE_X_DATA` is a JSON value.
...
Alternatively, you can specify per-environment values in [Wrangler's configuration](/workers/wrangler/configuration) file and provide an `environment` value via the `env` flag when developing locally like so `wrangler dev --env=local`.
