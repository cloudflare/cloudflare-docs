...
By default, Workers supports an all-at-once deployment model where traffic is immediately shifted from one version to the newly deployed version automatically. Alternatively, you can use [split deployments](/workers/configuration/versions-and-deployments/split-deployments/) to create a rolling deployment strategy.
