...
A Durable Object namespace (like `USER_DO`) contains many instances, one per user. Each instance has its own storage and is identified by a unique ID. When a request comes in, your Worker already knows which instance it goes to. To use cohorts, you just add the cohort at the same `.get()` call you already make.

You do not need to list out instance IDs or configure cohorts per instance. Your Worker resolves the cohort from whatever it knows about the user (their plan, their role, their email) and passes it at request time. This applies to every request automatically.
...
To assign a cohort to an instance, pass it at `.get()` time. Set the gradual deployment split to 100% stable, and only the instances you target get the new version:
