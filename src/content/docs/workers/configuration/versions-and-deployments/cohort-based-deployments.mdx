...
Roll out to your team first, verify it works, then expand to more users. If something breaks, roll back instantly. Traffic that is not in a cohort gets split randomly per your gradual deployment configuration.
...
With cohort deployments, you pass a cohort at `.get()` time to control which version an instance runs. How well this works depends on how your Durable Objects are used.

### Per-entity instances

When each user, tenant, or resource has their own Durable Object instance (for example, user profiles, shopping carts, sessions, or per-tenant state), cohorts work cleanly. Only one user's requests reach each instance, so the cohort is always consistent.
...
Because the gradual deployment split is 100% stable, every instance without a cohort stays on the stable version. Only alice's DO gets the new version. When you are confident, expand the cohort to more users or deploy to everyone.
...
When multiple users share a single DO instance (for example, chat rooms, game lobbies, or collaborative documents), different users may pass different cohorts. The first request after instantiation sets the cohort, and subsequent requests with different cohorts are ignored while the instance is in memory. This means you cannot guarantee that a specific user (for example, an enterprise customer) will not end up on the new version if they share an instance with a user from a different cohort.

For shared instances, cohorts control which **instances** get the new version, not which **users**. If you need all users of a shared instance to be on the same version, base the cohort on the instance (the room or document), not the individual user:
...
If you do not need per-instance control for shared DOs, use [gradual deployments](/workers/configuration/versions-and-deployments/gradual-deployments/#gradual-deployments-for-durable-objects) without cohorts. The hash-based split is deterministic per instance regardless of who calls it.
...
- **Optional.** If you do not pass a cohort, the Durable Object uses the gradual deployment split. If the split is 100% stable, the instance stays on stable.
...
|                                         | Gradual deployments (hash-based) | Cohort deployments                |
| --------------------------------------- | -------------------------------- | --------------------------------- |
...
| Best for                                | Shared instances                 | Per-user instances                |
...
All users in a room get the same version. The cohort is based on the room, not the user.
