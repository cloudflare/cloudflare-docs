...
The following examples keep default traffic on the stable version, while `canary` users get a 50/50 split and `employee` users are fully on the new version.
...
If you have not already uploaded two versions, make a code change and upload without deploying:

```sh
npx wrangler versions upload
```

Note the Version ID in the output. Use [`wrangler versions list`](/workers/wrangler/commands/#versions-list) to see all available versions:

```sh
npx wrangler versions list
```

#### 2. Deploy with cohorts

Use [`wrangler versions deploy`](/workers/wrangler/commands/#versions-deploy) with the `--cohort` flag. The positional version specs set the default configuration for uncohorted traffic. Each `--cohort` flag defines a named cohort with its own version split:

```sh
npx wrangler versions deploy \
  <STABLE_VERSION_ID>@100% \
  --cohort canary=<STABLE_VERSION_ID>@50%,<NEW_VERSION_ID>@50% \
  --cohort employee=<NEW_VERSION_ID>@100% \
  --message "Canary at 50%, employees on new version" -y
```

You can also run `npx wrangler versions deploy` without arguments to use the interactive prompt, which will guide you through selecting versions, setting default percentages, and adding cohorts.

#### 3. Verify the deployment

Use [`wrangler deployments status`](/workers/wrangler/commands/#deployments-status) to confirm the deployment was created with your cohort configuration:

```sh
npx wrangler deployments status
```

The output includes the default version split and each cohort's configuration:

```txt
Created:     2025-01-15T12:00:00.000Z
Author:      user@example.com
Source:      Wrangler
Message:     Canary at 50%, employees on new version
Version(s):  (100%) <STABLE_VERSION_ID>
                 Created:  2025-01-14T10:00:00.000Z
                     Tag:  -
                 Message:  -

Cohort(s):   canary:
                 (50%) <STABLE_VERSION_ID>
                 (50%) <NEW_VERSION_ID>
             employee:
                 (100%) <NEW_VERSION_ID>
```

#### 4. Progress or complete the deployment

When you are confident in the new version, update the deployment to shift more traffic. For example, to move `canary` to 100% of the new version:
...
npx wrangler versions deploy \
  ...
  --cohort employee=<NEW_VERSION_ID>@100% \
  --message "Canary fully on new version" -y
```

To complete the rollout for all traffic, deploy the new version at 100% without any cohorts:
...
Create a cohort-based deployment by sending a POST request to the deployments endpoint. The `versions` array defines the default traffic split. The `cohorts` array is optional and defines named cohort configurations:
...
Percentages within each cohort's `versions` array must sum to 100. Each cohort operates independently -- changing the `canary` split does not affect `employee` or the default. A deployment without a `cohorts` array behaves identically to a standard [gradual deployment](/workers/configuration/versions-and-deployments/gradual-deployments/).

## Assign requests to cohorts

Defining cohorts in your deployment configuration only sets up the version splits. You still need a way to assign incoming requests to the correct cohort. There are three approaches:
...
| [Service bindings](#service-bindings-and-cohort-propagation) | Yes | An upstream Worker already knows the cohort and needs to pass it to a downstream Worker |
...
```mermaid
flowchart TB
  subgraph vA ["Version A (60%)"]
    alice["alice"]
    carol["carol"]
    eve["eve"]
  end
  subgraph vB ["Version B (40%)"]
    bob["bob"]
    dave["dave"]
  end
```

Version assignment is based on `hash(DO ID)`. You cannot choose which instances get which version.
...
flowchart LR
  subgraph Deployment config
    free["cohort: free"] --> vA1["Version A (stable)"]
    paid["cohort: paid"] --> vB1["Version B (new)"]
    def["default (no cohort)"] --> vA2["Version A (stable)"]
  end
```
...
```mermaid
sequenceDiagram
    participant W1 as Request 1
    participant Platform
    participant bob as bob (DO instance)
    participant W2 as Request 2

    W1->>Platform: get(bobId, cohort: "paid")
    Platform->>bob: Start on Version B
    Note over bob: Cohort "paid" saved

    W2->>Platform: get(bobId, cohort: "free")
    Platform->>bob: Already running "paid" / Version B
    Note over bob: "free" ignored, stays on Version B
```
...
sequenceDiagram
    ...
    Platform->>bob: Start with cohort "paid" on Version B
    Note over bob: Cohort "paid" persisted

    Note over bob: Evicted (idle overnight).<br/>Stored data and cohort persist.

    Client->>Platform: New request for bob
    Platform->>Platform: Read persisted cohort: "paid"
    Platform->>bob: Reinstantiate on Version B

    Note over bob: Scheduled alarm fires.<br/>No incoming request.
    Platform->>Platform: Read persisted cohort: "paid"
    Platform->>bob: Run on Version B
```
...
The key difference from gradual deployments is the type of control you get over version assignment. With gradual deployments, you have **statistical** control -- "approximately this percentage." With cohort-based deployments, you have **semantic** control -- "these specific user segments." You can start with your lowest-risk instances, verify everything works, then move higher-value instances over deliberately.
