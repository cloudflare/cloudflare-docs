...
head: []
description: Incrementally deploy code changes to your Workers with split deployments.
---

import { Example, DashButton } from "~/components";

Split deployments give you the ability to incrementally deploy new [versions](/workers/configuration/versions-and-deployments/#versions) of Workers by splitting traffic across versions.

![Split Deployments](~/assets/images/workers/platform/versions-and-deployments/split-deployments.png)

Using split deployments, you can:

- Gradually shift traffic to a newer version of your Worker.
- Monitor error rates and exceptions across versions using [analytics and logs](/workers/configuration/versions-and-deployments/split-deployments/#observability) tooling.
...
The following section guides you through an example usage of split deployments. You will choose to use either [Wrangler](/workers/configuration/versions-and-deployments/split-deployments/#via-wrangler) or the Cloudflare dashboard to:

- Create a new Worker.
- Publish a new version of that Worker without deploying it.
- Create a split deployment between the two versions.
...
To create a new version of the Worker, edit the Worker code by changing the `Response` content to your desired text and upload the Worker by using the [`wrangler versions upload`](/workers/wrangler/commands/general/#versions-upload) command.
...
You should see 10 responses. Responses will reflect the content returned by the versions in your deployment. Responses will vary depending on the percentages configured in [step #3](/workers/configuration/versions-and-deployments/split-deployments/#3-create-a-new-deployment).
...
Run `wrangler versions deploy` again and follow the interactive prompts. Select the version uploaded in [step 2](/workers/configuration/versions-and-deployments/split-deployments/#2-create-a-new-version-of-the-worker) and set it to 100% deployment.
...
## Split deployments with static assets

When your Worker serves [static assets](/workers/static-assets/), split deployments can cause asset compatibility issues where users receive HTML from one version that references assets only available in another version, leading to 404 errors.
...
By default, the percentages configured when using split deployments operate on a per-request basis — a request has a X% probability of invoking one of two versions of the Worker in the [deployment](/workers/configuration/versions-and-deployments/#deployments).
...
You can use version overrides to send a request to a specific version of your Worker in your split deployment.
...
In the case that a request's version override is not applied, the request will be routed according to the percentages set in the split deployment configuration.
...
## Split deployments for Durable Objects

To provide [global uniqueness](/durable-objects/platform/known-issues/#global-uniqueness), only one version of each [Durable Object](/durable-objects/) can run at a time. This means that split deployments work slightly differently for Durable Objects.

When you create a new split deployment for a Worker with Durable Objects, each Durable Object is assigned a Worker version based on the percentages you configured in your [deployment](/workers/configuration/versions-and-deployments/#deployments). This version will not change until you create a new deployment.

![Split Deployments Durable Objects](~/assets/images/workers/platform/versions-and-deployments/durable-objects.png)
...
Here is how the versions of your Durable Objects might change as you progress your split deployment:
...
When using split deployments, you may want to attribute Workers invocations to a specific version in order to get visibility into the impact of deploying new versions.
