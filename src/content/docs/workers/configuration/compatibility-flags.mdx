...
This example enables the specific flag `formdata_parser_supports_files`, which is described [below](/workers/configuration/compatibility-flags/#formdata-parsing-supports-file). You should always set your `compatibility_date` to the current date. `compatibility_flags` can be used to enable upcoming features not yet included in your compatibility date, or to disable changes that became the default in the past.
