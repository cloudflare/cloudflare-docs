...
When you run `wrangler dev`, your Worker will receive this identity through `ctx.access`. Calling `ctx.access.getIdentity()` returns the identity object in the `dev` block. To test as a different user, change the identity fields. To test unauthenticated requests, remove the `dev` block — `ctx.access` will be `undefined`, just as it would be for a request that did not go through Access in production.
