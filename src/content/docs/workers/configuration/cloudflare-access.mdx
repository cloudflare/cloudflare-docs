...
Worker-level Access applications (Worker Destinations) do not currently support WebSocket connections. WebSocket upgrade requests to a Worker protected by a worker-level Access application will fail with a `403` error.
