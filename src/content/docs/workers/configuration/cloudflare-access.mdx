...
:::note
`ctx.access` is not populated during local development with `wrangler dev`. Access runs on Cloudflare's network, not your local machine, so `ctx.access` will always be `undefined` locally.
:::
