...
{
	"access": {
		// ... your Access policies, session_duration, etc.
		"dev": {
			"aud": "my-app",
...
When you run `wrangler dev`, your Worker will receive this identity through `ctx.access` — the same way it would in production. Calling `ctx.access.getIdentity()` returns the identity object you configured above.

### Switching between users

To test as a different user, update the identity in your `wrangler.jsonc`:
...
To test what happens when a request is not authenticated, remove the `dev` block (or comment it out). `ctx.access` will be `undefined`, just as it would be for an unauthenticated request in production.
...
With the `dev` config above, running `wrangler dev` and visiting `localhost:8787` would return `Hello, admin@company.com`.

### Identity fields

For the full list of fields available on the identity object, refer to [Application token — User identity](/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/application-token/).
