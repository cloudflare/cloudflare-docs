...
Use [`wrangler dev`](/workers/wrangler/commands/general/#dev) or the [Cloudflare Vite plugin](/workers/vite-plugin/) to test Cron Triggers locally. You can use Local Explorer or send an HTTP request.

Cron Trigger testing in Local Explorer requires Wrangler 4.139.0 or later, or Cloudflare Vite plugin 1.60.0 or later.

### Use Local Explorer

[Local Explorer](/workers/local-development/local-explorer/) displays the exact Cron Triggers configured for the active environment. It can also test ad hoc expressions without changing your configuration.

To test a configured expression, define at least one Cron Trigger for the active environment. You can test an ad hoc expression without configuring one.

<Steps>
1. Start your local development server and open Local Explorer.
2. Select **Cron Triggers** > **Configured Crons**.
3. Select _Now_ or add a scheduled time preset.
4. Select **Trigger** next to the expression you want to test.
5. Review the handler outcome, `noRetry()` status, dispatched expression, and scheduled time.
</Steps>
...
To test an expression that is not configured, select **Cron Triggers** > **Ad-Hoc Triggers**. Ad hoc triggers stay in your browser and do not modify deployed Cron Triggers.

### Use an HTTP request

Your local development server exposes two endpoints for scheduled invocations. If you use the Cloudflare Vite plugin, send requests to your Vite port. Vite uses port `5173` by default.

#### Use the scheduled route

Send a request to `/cdn-cgi/local/scheduled` to invoke a scheduled handler.
...
#### Use the Local Explorer API

To target an exact Worker available to Local Explorer, send a `POST` request to `/cdn-cgi/local/explorer/api/local/scheduled?worker=<WORKER_NAME>`. The `worker` query parameter must match the exact Worker name.

The JSON body requires a non-empty `cron` field. The optional `scheduled_time` field sets `controller.scheduledTime` using epoch milliseconds.
...
The response uses the Local Explorer API envelope. Its `result` includes the handler `outcome` and `noRetry` value.
