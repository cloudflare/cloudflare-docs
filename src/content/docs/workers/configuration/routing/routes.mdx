...
3. A DNS record set up for the [domain](/dns/manage-dns-records/how-to/create-zone-apex/) or [subdomain](/dns/manage-dns-records/how-to/create-subdomain/) proxied by Cloudflare (also known as [orange-clouded](/dns/proxy-status/#benefits)) you would like to route to.
...
Because `*` matches zero or more of **any character** (not just subdomains), `*example.com` will match any hostname ending in `example.com`. For example, `*example.com` matches `myexample.com` because `*` can match `my`, and it matches `not-example.com` because `*` can match `not-`. If you only want to match `example.com` and its subdomains, use two separate routes (`example.com/*` and `*.example.com/*`) instead.
...
Route patterns are evaluated within their associated [zone](/dns/zone-setups/). A route pattern configured for the `example.com` zone only matches requests for hostnames within that zone. This means that in practice, requests to `myexample.com` or `not-example.com` would not reach the `example.com` zone and would not match the route pattern.

The wildcard behavior is more relevant for subdomain patterns within a zone. For example, `*sub.example.com/*` in the `example.com` zone matches `sub.example.com`, `foo.sub.example.com`, and also unintended hostnames like `notsub.example.com`.

The following examples illustrate the difference between `*sub.example.com/*` and `*.sub.example.com/*` in the `example.com` zone:
