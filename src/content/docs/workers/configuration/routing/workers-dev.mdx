...
### Disabling `workers.dev` in the `wrangler.toml / wrangler.json(c)` file
...
If you do not specify `workers_dev = false` but add a [`routes` component](/workers/wrangler/configuration/#routes) to your `wrangler.toml / wrangler.json(c)` file, the value of `workers_dev` will be inferred as `false` on the next deploy.
