...
### Set up a Custom Domain in your [Wrangler configuration](/workers/wrangler/configuration) file

To configure a Custom Domain in your [Wrangler configuration](/workers/wrangler/configuration) file, add the `custom_domain=true` option on each pattern under `routes`. For example, to configure a Custom Domain:
...
To migrate the route `example.com/*` in your [Wrangler configuration](/workers/wrangler/configuration) file:

1. Log in to the [Cloudflare dashboard](https://dash.cloudflare.com) and select your account.
2. Go to **DNS** and delete the CNAME record for `example.com`.
3. Add the following to your [Wrangler configuration](/workers/wrangler/configuration) file:
