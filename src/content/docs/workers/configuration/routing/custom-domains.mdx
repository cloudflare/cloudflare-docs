...
To require visitors to sign in before they can access a Custom Domain, use [Cloudflare Access](/workers/configuration/cloudflare-access/#protect-a-specific-hostname-custom-domain-or-path).

You can protect a Custom Domain with hostname-based Access, or protect the Worker itself across its routes, Custom Domains, `workers.dev` hostname, and previews. For more information, refer to [Cloudflare Access](/workers/configuration/cloudflare-access/).
