...
The `getMiniflareFetchMock()` function is no longer available. To mock outbound requests, use [`@msw/cloudflare`](https://github.com/mswjs/cloudflare). Refer to [Mock outbound requests](/workers/testing/vitest-integration/mock-outbound-requests/) for setup instructions.
