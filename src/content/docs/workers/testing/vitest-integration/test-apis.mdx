...
* <code>env</code>: import("cloudflare:workers").ProvidedEnv
...
:::caution[Workflows with storage isolation]

To ensure proper test isolation in Workflows with per-file storage isolation, introspectors should be disposed at the end of each test.
...
:::note
Requires `@cloudflare/vitest-plugin` G.H.I or above.
:::

`introspectWorkflow()` and `introspectWorkflowInstance()` need a Workflow binding from `env`. They throw an error if you pass a Workflow from [`ctx.exports`](/workflows/build/workers-api/#call-a-workflow-through-ctxexports).

To introspect a Workflow declared in the [`exports`](/workers/wrangler/configuration/#workflow-exports) field of your Wrangler configuration, add a test-only binding to it in the `miniflare.workflows` option:
