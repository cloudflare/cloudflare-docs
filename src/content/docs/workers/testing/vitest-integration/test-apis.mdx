...
To ensure proper test isolation in Workflows with isolated storage, introspectors should be cleaned up at the end of each test.
This is accomplished by either:
* **Automatic**: Using an `await using` statement on the introspector.
* **Manual**: Explicitly calling the introspector `cleanUp()` method.
...
* `introspectWorkflowInstance(workflow: Workflow, instanceId: string)`: Promise\<WorkflowInstanceIntrospector>
    ...
    * The returned `WorkflowInstanceIntrospector` object has the following methods:
        ...
        * `cleanUp(): Promise<void>`: Cleans up the Workflow instance's state, which is crucial for test isolation. If this function isn't called and `await using` is not used, the instance's state will persist across subsequent tests. For example, an instance that becomes completed in one test will already be completed at the start of the next.
        * `[Symbol.asyncDispose](): Provides automatic cleanup. It's invoked by the `await using` statement, which calls `cleanUp()`.
...
* `introspectWorkflow(workflow: Workflow)`: Promise\<WorkflowIntrospector>
    ...
    * The returned `WorkflowIntrospector` object has the following methods:
        ...
        * `cleanUp()`: Cleans up and stops the introspection process for the Workflow. All `WorkflowInstanceIntrospector` from created instances will also be cleaned. This is crucial to prevent modifications and captured instances from leaking between tests. After calling this method, the `WorkflowIntrospector` should not be reused.
        * `[Symbol.asyncDispose](): Promise<void>`: Provides automatic cleanup. It's invoked by the `await using` statement, which calls `cleanUp()`. 
