...
For the platform-wide permission model, refer to the [Roles and permissions overview](/workers/authorization/).
...
API tokens need _Zone_ > _Workers Routes_ > _Write_, scoped to each affected zone.
...
| Deploy an existing Worker with Routes or Custom Domains | `wrangler deploy`                                      | `Editor` for that Worker, plus `Workers Routes Write` for each affected zone |
...
| Workers Routes Edit                   | `Editor` at the Workers product scope, plus `Workers Routes Write` for each affected zone |
...
- KV-backed Durable Objects do not support granular authorization for analytics. If access is scoped to an individual Worker, metrics and other observability data for its KV-backed Durable Objects are unavailable.
...
- [Roles and permissions overview](/workers/authorization/)
- Durable Objects roles and permissions — coming soon
- [R2 roles and permissions](/r2/api/tokens/#permissions)
