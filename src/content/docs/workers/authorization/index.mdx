...
| Deploy a Worker with Routes or Custom Domains | `wrangler deploy`                                      | `Editor` for the Worker, plus [`Workers Routes Write`](/workers/authorization/workers/#routes-and-custom-domains) for every affected zone. |
...
| Deploy a Worker with Routes or Custom Domains           | Workers product and each affected zone | Editor and [`Workers Routes Write`](/workers/authorization/workers/#routes-and-custom-domains) |
