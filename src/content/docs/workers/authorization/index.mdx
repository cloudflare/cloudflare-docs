...
| ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| [Members](/fundamentals/manage-members/manage/) and [User Groups](/fundamentals/manage-members/user-groups/) | People using the dashboard or API               | Permissions are inherited when the member signs in to the dashboard or authenticates to the API. |
...
When you assign a [role](/fundamentals/manage-members/roles/) to a [member](/fundamentals/manage-members/manage/) or [User Group](/fundamentals/manage-members/user-groups/), that permission applies whenever the user uses the Cloudflare dashboard or API.
...
If several people on the same team or project need the same access, create a [User Group](/fundamentals/manage-members/user-groups/) instead of assigning permissions to each member.
...
The [`wrangler login`](/workers/wrangler/commands/general/#login) OAuth flow does not currently support granular authorization.

:::

To use granular permissions with [Wrangler](/workers/wrangler/), authenticate with an account-owned API token.
...
| Change Routes or Custom Domains during deployment | `wrangler deploy`                                      | `Editor` for the Worker, plus [`Workers Routes Write`](/workers/authorization/workers/#routes-and-custom-domains) for every affected zone. |
...
| Change Routes or Custom Domains during deployment       | Workers product and each affected zone | Editor and [`Workers Routes Write`](/workers/authorization/workers/#routes-and-custom-domains) |
