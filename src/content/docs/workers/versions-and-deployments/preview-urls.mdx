...
When preview URLs are available for your Worker, each new [version](/workers/versions-and-deployments/#versions) gets a unique static version preview URL. These URLs use a version prefix and follow the format `<VERSION_PREFIX>-<WORKER_NAME>.<SUBDOMAIN>.workers.dev`. See [Limitations](#limitations) for Workers that do not get preview URLs.
...
The [`wrangler versions upload`](/workers/wrangler/commands/general/#versions-upload) command uploads a new [version](/workers/versions-and-deployments/#versions) of your Worker and, when preview URLs are available, returns a preview URL for that version.
...
- Preview URLs are not generated for Workers that implement a [Durable Object](/durable-objects/). That includes [Containers](/containers/) Workers and [Sandbox](/sandbox/) Workers, which use Durable Object classes. You can still upload versions with [`wrangler versions upload`](/workers/wrangler/commands/workers/#versions-upload); you do not get a preview URL for those versions. For how to test Containers changes before production, refer to [Deploy Containers](/containers/deploy/#before-production).
