...
During a [gradual deployment](/workers/versions-and-deployments/gradual-deployments/), each request has a random chance of routing to either version based on the specified percentages. This means the same user can be served content from a different version every time a request is made.
...
Version affinity is particularly important when your Worker serves [static assets](/workers/static-assets/) with fingerprinted filenames, which is the default for modern frameworks like React, Vue, Angular, and Svelte.
...
Configuring version affinity using any of the methods described below prevents this entirely by ensuring all requests from the same user are routed to the same version.
...
export default {
  async fetch(request, env) {
    ...
    // If the request doesn't already have an affinity cookie, set one
    if (!request.headers.get("Cookie")?.includes("__cf_vk")) {
      const id = crypto.randomUUID();
      response.headers.append(
        "Set-Cookie",
        `__cf_vk=${id}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=31536000`,
      );
    }
...
**Value**: `http.request.cookies["__cf_vk"][0]`
