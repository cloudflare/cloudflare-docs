...
Wrangler versions before `4.131.0` may return error code `10211` when `wrangler versions upload` encounters a pending Durable Object migration. Apply the migration with `wrangler deploy`:
