...
- A [split deployment](/workers/versions-and-deployments/gradual-deployments/) with two versions (rolling back replaces both versions with the selected version at 100% traffic).
...
If you are using a [gradual deployment](/workers/versions-and-deployments/gradual-deployments/) with two versions splitting traffic, rolling back will:
...
- A Durable Object class lifecycle change (via [`exports`](/durable-objects/reference/durable-objects-migrations/) or the legacy [`migrations`](/durable-objects/reference/durable-object-class-migrations-legacy/) array) has occurred between the version in the active deployment and the version selected to roll back to.
