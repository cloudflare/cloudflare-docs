...
Gradual Deployments give you the ability to incrementally deploy new [versions](/workers/versions-and-deployments/#versions) of Workers by splitting traffic across versions.
...
- Monitor error rates and exceptions across versions using [analytics and logs](/workers/versions-and-deployments/gradual-deployments/#observability) tooling.
- [Roll back](/workers/versions-and-deployments/rollbacks/) to a previously stable version if you notice issues when deploying a new version.

## Use gradual deployments

The following section guides you through an example usage of gradual deployments. You will choose to use either [Wrangler](/workers/versions-and-deployments/gradual-deployments/#via-wrangler) or the Cloudflare dashboard to:
...
You should see 10 responses. Responses will reflect the content returned by the versions in your deployment. Responses will vary depending on the percentages configured in [step #3](/workers/versions-and-deployments/gradual-deployments/#3-create-a-new-deployment).
...
Run `wrangler versions deploy` again and follow the interactive prompts. Select the version uploaded in [step 2](/workers/versions-and-deployments/gradual-deployments/#2-create-a-new-version-of-the-worker) and set it to 100% deployment.
...
By default, the percentages configured when using gradual deployments operate on a per-request basis — a request has a X% probability of invoking one of two versions of the Worker in the [deployment](/workers/versions-and-deployments/#deployments).
