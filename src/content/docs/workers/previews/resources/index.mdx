...
Create a separate file named `wrangler.preview-migrations.jsonc` for the database used by the Preview you are about to deploy. Declare the database under top-level `d1_databases`. Copy the `database_name` and `database_id` from the current branch's `previews.d1_databases` binding:
