...
Follow these steps to apply [D1 migrations](/d1/reference/migrations/) to databases used by Previews.

### 1. Keep the D1 binding in your Preview configuration

In your main Wrangler configuration file, bind the Preview to its database under `previews.d1_databases`:
...
Create a separate file named `wrangler.preview-migrations.jsonc` for the database used by the Preview you are about to deploy. Declare the database under top-level `d1_databases`, using the same `database_name` as the binding in `previews.d1_databases`:

```jsonc title="wrangler.preview-migrations.jsonc"
{
	"d1_databases": [
		{
			"binding": "PREVIEW_DB",
...
### 4. Apply migrations to the Preview database

Confirm that `database_name` matches the Preview binding for the current branch, then apply the migrations:
