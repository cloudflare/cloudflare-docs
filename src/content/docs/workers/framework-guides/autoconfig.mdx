...
If you don't want automatic configuration to run, ensure you have a valid Wrangler configuration file (`wrangler.toml`, `wrangler.json`, or `wrangler.jsonc`) in your project before running `wrangler deploy`.
...
If your project contains multiple frameworks, automatic configuration will fail with an error. In this case, you need to manually configure your project or specify a root directory that contains only one framework.
