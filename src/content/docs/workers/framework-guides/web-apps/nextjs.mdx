...
You can deploy an existing Next.js project to Cloudflare Workers with a single command. Run `npx wrangler deploy` in your project directory and Wrangler will automatically run `@opennextjs/cloudflare migrate` to configure your project. Refer to [Deploy an existing project](/workers/framework-guides/automatic-configuration/) for details.
:::
