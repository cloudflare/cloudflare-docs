...
You can deploy an existing Astro project to Cloudflare Workers with a single command. Run `npx wrangler deploy` in your project directory and Wrangler will automatically detect Astro, install the Cloudflare adapter, and configure your project. Refer to [Deploy an existing project](/workers/framework-guides/automatic-configuration/) for details.
:::
