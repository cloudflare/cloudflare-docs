...
[TanStack Start](https://tanstack.com/start) is a full-stack framework for building web applications. It comes with features like server-side rendering, streaming, server functions, bundling, and more. In this guide, you learn to deploy a TanStack (**React**) Start application to Cloudflare Workers. To deploy a TanStack Start application with **Solid** instead, refer to the TanStack [example](https://tanstack.com/start/latest/docs/framework/solid/examples/start-basic-cloudflare).
...
If you need to export additional declarations from your Worker file, for example class definitions for other Cloudflare primitives such as [Durable Objects](/durable-objects/get-started/#2-write-a-durable-object-class-using-sql-api), or handlers for [Queues](/queues/) or [Cron Triggers](/workers/configuration/cron-triggers/), you can specify a custom entrypoint using the `main` property in your Wrangler configuration file.
...
Export the `RequestHandler` from `@tanstack/react-start/server-entry` and wrap it as the default fetch handler.

```ts title="worker-entry.ts"
import RequestHandler from "@tanstack/react-start/server-entry";
import { DurableObject } from "cloudflare:workers";
import MyCustomQueueHandler from "foo"; // Your own code
import MyScheduledHandler from "bar"; // Your own code

export default {
	fetch: RequestHandler.fetch,
	queue: MyCustomQueueHandler,
	scheduled: MyScheduledHandler,
};
