...
If you maintain an application that uses the OpenNext adapter, refer to [OpenNext adapter](/workers/framework-guides/web-apps/nextjs/opennext/). If you maintain an existing Pages project that uses Next on Pages, refer to [Next on Pages](/pages/framework-guides/nextjs/next-on-pages/).
...
vinext is in beta. Before adopting it for an existing production application, run the compatibility check and review the [vinext compatibility dashboard](https://vinext.dev/compatibility).
...
| Incremental Static Regeneration (ISR) | Supported | Uses stale-while-revalidate caching. |
...
| Path | Use when |
| --- | --- |
| [OpenNext adapter](/workers/framework-guides/web-apps/nextjs/opennext/) | You need the mature OpenNext adapter for standard Next.js build output or your application depends on a vinext compatibility gap. |
