...
To solve this, we use [Markdown for Agents](/fundamentals/reference/markdown-for-agents/) (MFA), which converts HTML to Markdown at the Cloudflare network layer. MFA handles:
...
Taking the `Tabs` example from the previous section, MFA will give us a normal unordered list with the content properly associated with a given list item:
