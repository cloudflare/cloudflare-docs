...
For example, given our [`Tabs`](/style-guide/build-the-page/components/tabs/), the panels are hidden until the tab itself is clicked:
...
- Append `index.md` to the URL — for example, [`/docs-for-agents/index.md`](/docs-for-agents/index.md)
...
The only _special_ work we have done is adding a [`noindex` directives](https://developers.google.com/search/docs/crawling-indexing/block-indexing) to specific types of content (via a [frontmatter tag](/style-guide/build-the-page/frontmatter/custom-properties/#noindex)).
