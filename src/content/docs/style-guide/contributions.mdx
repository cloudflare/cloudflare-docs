...
On open pull requests, the Cloudflare Docs Bot posts a [Review coverage](https://github.com/cloudflare/cloudflare-docs/tree/production/.flue) comment that shows which areas still need a CODEOWNERS approval, suggests contacts for those areas, and summarizes the areas already covered. Expand the CODEOWNERS mappings section to see the exact patterns and owners for the ownership areas displayed in the comment. The bot skips draft, closed, and spam/off-topic pull requests.
