...
- [`APIRequest`](/style-guide/build-the-page/components/api-request/): For examples hitting endpoints in the Cloudflare API schema.
- [`CURL`](/style-guide/build-the-page/components/curl/): For other cURL commands.
...
* Placeholders in the request body (that is, the data included in a `POST`/`PUT`/`PATCH` request) should use [angle brackets](/style-guide/style-and-grammar/formatting/code-conventions-and-format/#angle-brackets---and--): `<RULE_ID>`
