...
Use "**`$`** "(dollar sign, space) or "**FOLDER\_NAME $** " (folder name, space, dollar sign, space).
...
Use "**PS FOLDER\_NAME>** " (the `>` is part of the prompt, and there is a space after it).
...
Use "**FOLDER\_NAME>**" (folder name, bigger than symbol, no space after).
