...
- **A tutorial.** A tutorial teaches by building something and cannot fail the reader; a how-to serves someone mid-task who already knows the goal.
...
- **A solution guide.** A solution guide starts from a problem rather than a product, then combines several products to solve it.
...
- [**Steps**](/style-guide/components/steps/) are the signature structure: the Steps component or a plain ordered list, whichever you use, must read identically in the Markdown twin. If a page has no steps, question whether it is a how-to.
- [**Tabs and code groups**](/style-guide/components/tabs/) carry variant axes such as language, platform, or CLI versus dashboard inside one canonical page. For alternative methods, pick the recommended one and link the rest; never duplicate the page.
...
- **Self-contained steps.** Name the product area, the full command, and the exact label the reader selects; never use a positional reference such as "as configured above."
