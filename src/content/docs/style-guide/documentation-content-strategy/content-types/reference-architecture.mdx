...
For a single architecture that needs little written explanation, use a [reference architecture diagram](#reference-architecture-diagrams) instead. For the full comparison, refer to [How to select a content type](/style-guide/documentation-content-strategy/content-types/select-content-type/). For live examples, refer to [Cloudflare Load Balancing Reference Architecture](/reference-architecture/architectures/load-balancing/), [Magic Transit Reference Architecture](/reference-architecture/architectures/magic-transit/), and [Evolving to a SASE architecture with Cloudflare](/reference-architecture/architectures/sase/).
...
- [**PublicStats**](/style-guide/components/public-stats/) surfaces Cloudflare network statistics where they strengthen the case for the architecture.
