...
For the full comparison, refer to [How to select a content type](/style-guide/documentation-content-strategy/content-types/select-content-type/). For live examples, see [Common Cf-Polished statuses](/images/polish/cf-polished-statuses/) and [Logpush API configuration](/logs/logpush/logpush-job/api-configuration/).
...
- [**Tables**](/style-guide/formatting/structure/tables/) are the signature component: a quick-reference table above the entries makes the common lookup zero-scroll. Keep tables simple, because merged cells and meaning-by-layout break both scanning and extraction.
...
- [**DirectoryListing**](/style-guide/components/directory-listing/) links the child pages when the reference is a section rather than a single page.
- **What does not fit:** Steps and Cards; callouts, because a fact that needs a warning usually belongs in the entry as a constraint; and tabs or accordions that hide entries, because a collapsed entry is invisible to search-and-grab readers and to extraction.
