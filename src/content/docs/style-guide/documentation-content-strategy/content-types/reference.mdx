...
For the full comparison, refer to [How to select a content type](/style-guide/documentation-content-strategy/content-types/select-content-type/). For live examples, refer to [Common Cf-Polished statuses](/images/polish/cf-polished-statuses/) and [Logpush API configuration](/logs/logpush/logpush-job/api-configuration/).
...
- [**Tables**](/style-guide/formatting/structure/tables/) are the signature component: a quick-reference table before the entries makes the common lookup zero-scroll. Keep tables simple, because merged cells and meaning-by-layout break both scanning and extraction.
