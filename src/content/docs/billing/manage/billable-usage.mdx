...
![The Billable Usage dashboard showing a stacked bar chart of daily costs broken down by product, with a tooltip displaying per-product charges for a selected day.](~/assets/images/billing/billable-usage-cost-breakdown.png)
...
![The product usage table showing columns for product name, total usage, billable usage, and usage cost, with free-tier allowances noted beneath each product name.](~/assets/images/billing/billable-usage-product-table.png)
...
- **No data export** — CSV or JSON export is not yet available.
