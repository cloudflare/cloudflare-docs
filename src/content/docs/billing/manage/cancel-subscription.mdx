...
## Transferring subscriptions between domains

Subscriptions cannot be transferred from one domain to another. To move a subscription to a different domain, cancel the subscription on the original domain and then purchase it on the new domain.

## Zone deletion and prorated credit

When a zone is deleted from your account, Cloudflare automatically cancels the associated subscriptions and applies a prorated credit to your account. This credit:

- Is valid for three years from the date it is issued.
- Can be used toward monthly recurring subscriptions only. It cannot be applied to new one-time orders or yearly domain registrar renewals.
- Is not displayed in the Cloudflare dashboard, but will appear as a line item on your next invoice.
