...
To get started, go to the top right corner of the Cloudflare dashboard and select the '+Add' menu option. From there, choose 'Existing Domain' to add your domain and set up any necessary DNS records.

Then, from your Cloudflare Account Home, go to [**Transfer Domains**](https://dash.cloudflare.com/?to=/:account/domains/transfer). Cloudflare Registrar will display the zones available for transfer.
