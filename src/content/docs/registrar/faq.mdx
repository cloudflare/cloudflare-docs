...
You cannot enter your auth code until the domain is first active on Cloudflare with a [full setup](/dns/zone-setups/full-setup/). If the transfer failed rather than just pending, work through [Registrar: troubleshoot stalled domain transfers](/registrar/troubleshooting/).
...
ICANN requires that any transfer also extends the expiration date of your domain by at least one year, that is, one year from your current expiration date, not one year from the date of transfer. For example, if your domain's current expiration date is a few months away when you transfer it, your new expiration date will be one year after that original expiration date, not one year from the transfer date.

Whenever a domain is first registered, the registrant purchases control of that domain for some number of years, up to 10 years. For example, a newly registered domain will have an expiration date on the same month and day in a future year, anywhere from one to 10 years later, depending on the number of years originally purchased.
