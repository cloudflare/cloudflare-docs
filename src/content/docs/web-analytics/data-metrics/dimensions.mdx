...
- **Navigation type**: Which method was used to load the HTML document - [see the table below](#navigation-types) for a breakdown. 
...
| Back-forward            | ❌         | The visitor used the back/forward buttons/gestures in their browser but the previously-loaded document either not found or stale in the browsers HTTP cache OR a feature was used which prevents using the cache ([see explanation](https://web.dev/articles/bfcache)), so a network request was made to load the document |
