...
Our beacon script targets modern syntax which Internet Explorer doesn't support. This will cause a non-user-visible error when the script attempts to execute. The only functional impact is that beacons are not collected from these old browsers.
...
For customers using the manual embed approach, you'll need to add `type="module"` to the `<script>` tag manually. Our dashboard has been updated to include this attribute in the installation instructions.
