...
What you see in the dashboard for the past 7 days is unsampled, when filtering beyond this point you will see data aggregated down to 10%.

* The beacon script will fire on every pageview.
* The data pipeline does not apply sampling.
* As we compact data older than 7 days, we aggregate it down so it's 10% of the original volume
