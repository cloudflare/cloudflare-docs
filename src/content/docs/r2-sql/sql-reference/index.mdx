...
Set operations combine the results of two or more `SELECT` queries. Each query must return the same number of columns with compatible types.
...
```sql
-- Distinct regions across two tables
SELECT region FROM my_namespace.sales_data
UNION
SELECT region FROM my_namespace.archived_sales

-- Customers with current orders but no archived orders
