...
Although not strictly required by the OpenAPI specification, Schema Validation strictly requires these fields Schema Validation.
...
- [`schema`](https://spec.openapis.org/oas/v3.0.3#schema-object)
   - Schema Validation does not support the content field in parameters (for more details further down). Instead, a schema is strictly required on all parameters objects.

### Validated and supported fields
