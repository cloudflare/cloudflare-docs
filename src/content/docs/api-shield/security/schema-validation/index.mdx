...
Schema validation inspects request bodies up to 128 KB. Request bodies that exceed this limit are not validated, regardless of the body inspection size configured in the [WAF](/waf/managed-rules/#maximum-body-size). Requests with bodies larger than 128 KB pass through without body validation but are still subject to path and parameter validation.
