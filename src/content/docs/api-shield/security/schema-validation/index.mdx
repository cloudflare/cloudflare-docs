...
:::note
To export a schema, refer to [Export a schema](/api-shield/management-and-monitoring/endpoint-management/schema-learning/#export-a-schema).
:::
...
If your API endpoints receive legitimate request bodies that exceed the default limit for your plan, you can request an increase up to 1 MB (1,048,576 bytes) per zone.

To request an increase, contact [Cloudflare Support](https://developers.cloudflare.com/support/contacting-cloudflare-support/) and provide the following information:

- The zone name(s) requiring the increase.
- The desired body size limit (up to 1 MB).
- A brief description of the use case (for example, large JSON payloads for data ingestion APIs).

After the change is applied, Schema validation will inspect request bodies up to the new limit on the specified zone(s).
