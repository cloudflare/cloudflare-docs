...
8. Analytics events in the dashboard (Security -> Analytics -> Events) for the `API Shield - Token Validation` service will explain violation reasons in the `Token validation violations` section of the event.
