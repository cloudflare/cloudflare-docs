...
For enhanced security, Cloudflare recommends that you validate the Issuer Subject Key Identifer (SKI) hash alongside the verified certificate field. This ensures that only requests presenting a valid client certificate with a specific issuer are allowed.
