...
When you configure [mTLS authentication](/api-shield/security/mtls/) for API hosts, Cloudflare can block requests that do not have a [client certificate](/ssl/client-certificates/) for mTLS authentication. Blocking applies only to requests that match the hostname and path conditions in your mTLS rule.
...
By default, API Shield mTLS uses client certificates issued by a Cloudflare-managed CA. If you need to use certificates issued by another CA, refer to [Bring your own CA for mTLS](/ssl/client-certificates/byo-ca/).
:::
...
Because the [action](/ruleset-engine/rules-language/actions/) for your rule is _Block_, requests that match the configured hostname and path conditions must present a valid client certificate.
