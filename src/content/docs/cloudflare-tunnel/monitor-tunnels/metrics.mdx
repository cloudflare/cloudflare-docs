...
To determine the default port being used by a `cloudflared` instance, you can check your [Tunnel logs](/cloudflare-tunnel/monitor-tunnels/logs/) around the time when the tunnel started. For example:
...
1. Run the tunnel using the [--metrics](/cloudflare-tunnel/configure-tunnels/cloudflared-parameters/run-parameters/#metrics) flag. Here is an example command for a locally-managed tunnel:

   ```sh
   cloudflared tunnel --metrics 127.0.0.1:60123 run my-tunnel
   ```

   To learn how to add the `--metrics` flag to a remotely-managed tunnel, refer to [Configure a remotely-managed tunnel](/cloudflare-tunnel/configure-tunnels/cloudflared-parameters/#update-tunnel-run-parameters).
