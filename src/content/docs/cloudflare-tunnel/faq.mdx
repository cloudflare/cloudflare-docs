...
Named Tunnels can be routed via DNS records, in which case we use CNAME records to point to `<UUID>.cfargotunnel.com`; or as Load Balancing endpoints, which also point to `<UUID>.cfargotunnel.com`.
...
No. You cannot undo a tunnel deletion. If the tunnel was locally-managed, its [configuration file](/cloudflare-tunnel/other-tunnel-types/local-management/configuration-file/) will still be present and you can create a new tunnel with the same configuration. If the tunnel was remotely-managed, both the tunnel and its configuration are permanently deleted.
...
4. If needed, set [`--loglevel`](/cloudflare-tunnel/configure-tunnels/cloudflared-parameters/run-parameters/#loglevel) to `debug`, so the Cloudflare support team can get more info from the `cloudflared.log` file.
5. Include your [Cloudflare Tunnel diagnostic logs](/cloudflare-tunnel/troubleshoot/diag-logs/) (`cloudflared-diag-YYYY-MM-DDThh-mm-ss.zip`).
