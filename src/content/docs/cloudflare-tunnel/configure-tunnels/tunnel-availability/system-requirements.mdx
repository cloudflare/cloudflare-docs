...
- Run a [`cloudflared` replica](/cloudflare-tunnel/configure-tunnels/tunnel-availability/#cloudflared-replicas) on two dedicated host machines per network location. Using two hosts enables server-side redundancy.
...
Most traffic proxied by `cloudflared` falls in one of two categories:
...
2. After a week or so, query the following [tunnel metrics](/cloudflare-tunnel/monitor-tunnels/metrics/#cloudflared-metrics):
...
4. If applicable, obtain the average **DNS requests per second** from your DNS resolver.
5. Input your values into our sizing calculator:

<TunnelCalculator />

You can use these results to determine if your tunnel is appropriately sized. To increase your tunnel capacity, add identical host machines running [`cloudflared` replicas](/cloudflare-tunnel/configure-tunnels/tunnel-availability/#cloudflared-replicas).
