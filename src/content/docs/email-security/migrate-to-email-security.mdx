...
:::note
By October 2025, Area 1 webhooks will be visible in Zero Trust Email Security, but non-configurable. Use [Logpush](/cloudflare-one/insights/logs/enable-logs/#enable-user-action-logs) to create new webhooks or configure webhooks.
:::
...
:::note
From October 2025, we will no longer receive support requests via email to any `@area1security.com` addresses. We will transition to receiving support requests from the Cloudflare dashboard.
:::
