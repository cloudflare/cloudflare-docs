...
You can also call third-party models from providers like OpenAI, Anthropic, and Google using the same `env.AI.run()` method. Use the `{provider}/{model}` format for the model ID:
...
For the full list of supported third-party models, refer to [Third-party models via Workers AI binding](/ai-gateway/integrations/aig-workers-ai-binding-third-party/).
