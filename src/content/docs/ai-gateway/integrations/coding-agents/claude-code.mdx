...
- Credentials for the provider you route to:
  - **Anthropic**: An Anthropic API key stored in AI Gateway as a [provider key (BYOK)](/ai-gateway/configuration/bring-your-own-keys/), [Unified Billing](/ai-gateway/features/unified-billing/) credits loaded on your Cloudflare account, or your own Anthropic API key passed directly in `ANTHROPIC_API_KEY` (Anthropic bills you directly).
...
:::note
The `cf-aig-authorization` header is what authenticates your request to AI Gateway. When AI Gateway supplies the Anthropic credentials — using either a [stored provider key (BYOK)](/ai-gateway/configuration/bring-your-own-keys/) or [Unified Billing](/ai-gateway/features/unified-billing/) — the `ANTHROPIC_API_KEY` value is ignored. Claude Code still requires the variable to be set, so you can set it to any value (this example reuses the gateway token). Alternatively, if you do not use BYOK or Unified Billing, set `ANTHROPIC_API_KEY` to your own Anthropic API key and Anthropic bills you directly. AI Gateway provides observability, caching, and rate limiting for the traffic either way. For details on where Claude Code reads credentials from, refer to [Anthropic's authentication documentation](https://docs.anthropic.com/en/docs/claude-code/iam#credential-management).
:::
