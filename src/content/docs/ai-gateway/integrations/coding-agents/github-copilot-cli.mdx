...
1. GitHub Copilot CLI installed. To install it, refer to [Installing GitHub Copilot CLI](https://docs.github.com/en/copilot/how-tos/set-up/install-copilot-cli).
2. A [Cloudflare API token](/fundamentals/api/get-started/create-token/) with `AI Gateway` permission.
3. [Credits loaded](/ai-gateway/features/unified-billing/#load-credits) on your account for third-party models.
4. A model that supports tool calling and streaming. For best results, use a model with a context window of at least 128k tokens.
