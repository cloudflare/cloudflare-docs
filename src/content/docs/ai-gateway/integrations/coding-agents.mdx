...
:::note
GitHub Copilot CLI authenticates the model provider with a single `Authorization` header and cannot send custom request headers. Use the REST API, which authenticates with a Cloudflare API token, rather than the gateway token and `cf-aig-authorization` header flow used for Claude Code. Because Copilot CLI cannot set the `cf-aig-gateway-id` header, requests route through your account's [default gateway](/ai-gateway/usage/rest-api/#specify-a-gateway).
:::
