...
- **Route** - A named, versioned flow (e.g., dynamic/support) that you can use as instead of the model name in your requests
- **Nodes**
  - **Start** - Entry point for the route.
  - **Conditional** - If/Else branch based on expressions that reference request body, headers, or metadata (e.g., user_plan == "paid").
...
- **Metadata** - Arbitrary key-value context attached to the request (e.g., userId, orgId, plan). You can pass this from your app so rules can reference it.
