...
Unified Billing allows users to connect to various AI providers (e.g. OpenAI, Anthropic) and receive a single Cloudflare bill. To use AI Gateway, users must purchase and load credits onto their Cloudflare account, via the Dashboard, which are then spent on AI Gateway usage.
