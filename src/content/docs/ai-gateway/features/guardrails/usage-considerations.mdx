...
Categories `S1` through `S13` are the [Llama Guard 3 ↗](https://ai.meta.com/research/publications/llama-guard-llm-based-input-output-safeguard-for-human-ai-conversations/) hazard categories, evaluated by the `@cf/meta/llama-guard-3-8b` model on Workers AI. Category `P1` is prompt injection, evaluated separately by the `@cf/meta/prompt-guard-2-86m` model.
