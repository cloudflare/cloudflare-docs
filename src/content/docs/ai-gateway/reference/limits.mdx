...
[DLP](/ai-gateway/features/dlp/) for AI Gateway uses shared [Cloudflare Zero Trust DLP profiles](/cloudflare-one/data-loss-prevention/dlp-profiles/). The following limits apply to DLP profiles and detection entries at the account level:

| Feature                                                                              | Limit          |
| ------------------------------------------------------------------------------------ | -------------- |
| [Custom detection entries](/cloudflare-one/account-limits/#data-loss-prevention-dlp) | 25 per account |
...
DLP profiles are shared Zero Trust objects that are not coupled to individual gateways. You can apply the same DLP profiles across multiple gateways without additional profile limits. There is no separate limit on the number of DLP policies per gateway.

For more information about DLP profile limits, refer to the [Zero Trust account limits](/cloudflare-one/account-limits/#data-loss-prevention-dlp).
