...
Some providers expose native tools — including server-side web search — through these endpoints. See [Web Search](/ai-gateway/usage/web-search/) for the supported models per provider and the request shape each one uses.
