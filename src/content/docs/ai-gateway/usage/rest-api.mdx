...
Set `background` to `true` and provide a `webhookUrl`. Both are top-level fields of the `/ai/run` body, alongside `model` and `input`.
...
Use the optional `webhookFormat` top-level field to control the shape of the webhook body. The default is `raw`. `webhookFormat` can only be provided when `webhookUrl` is present; otherwise the request returns a `400` error.
