...
:::note
When both `cf-aig-collect-log: false` and `cf-aig-collect-log-payload: false` are set, the entire log entry (including metadata) is skipped. Use `cf-aig-collect-log-payload: false` on its own if you only want to suppress payload storage while retaining metadata logs.
:::
