...
Uploading your public key enables Workers Logpush for the gateway, but logs will not be exported until you also create and enable a Logpush job that specifies where to send them.

To create the Logpush job, choose your destination and follow the steps in the [Enable destinations](/logs/logpush/logpush-job/enable-destinations/) documentation. For example, to export logs to Cloudflare R2, refer to [Enable Cloudflare R2](/logs/logpush/logpush-job/enable-destinations/r2/). When configuring the job, select the AI Gateway dataset.

:::note

Both steps are required. If you upload the public key in AI Gateway but do not create and enable a Logpush job in the dashboard, no logs will be exported.
