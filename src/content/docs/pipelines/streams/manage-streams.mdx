...
:::note
Events that do not match the defined schema are accepted during ingestion but will be dropped during processing. To monitor dropped events and understand why they were dropped, query the [user error metrics](/pipelines/observability/metrics/#user-error-metrics) via GraphQL. Schema modifications are not supported after stream creation.
:::
...
:::note
For details on configuring authentication tokens and making authenticated requests, refer to [Writing to streams](/pipelines/streams/writing-to-streams/).
:::
