...
To view a specific sink, run the [`pipelines sinks get`](/workers/wrangler/commands/pipelines/#pipelines-sinks-get) command with either the sink ID or sink name:

```bash
npx wrangler pipelines sinks get <SINK_NAME_OR_ID>
```

To list all sinks in your account, run the [`pipelines sinks list`](/workers/wrangler/commands/pipelines/#pipelines-sinks-list) command:
...
To delete a sink, run the [`pipelines sinks delete`](/workers/wrangler/commands/pipelines/#pipelines-sinks-delete) command:
