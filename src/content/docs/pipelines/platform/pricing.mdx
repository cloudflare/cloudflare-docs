...
SQL transforms let you filter, reshape, and compute over data before it reaches a sink. You are only charged when you go beyond a passthrough query (`SELECT * FROM stream`). Any query that filters, renames, casts, or computes columns counts as a transform.
...
[^1]: Optional. Includes stateless SQL transforms only (for example, filter, reshape, unnest, cast, compute). A passthrough `SELECT * FROM stream` is not charged.
