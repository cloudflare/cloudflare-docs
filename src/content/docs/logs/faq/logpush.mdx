...
### How can I be notified if my Logpush job fails?

Configure [Logpush health notifications](/logs/logpush/logpush-health/) to receive alerts when your Logpush job is disabled or experiencing errors. This is especially important since Logpush cannot backfill logs, so early detection of issues helps prevent data loss.


### How do sample_rate and SampleInterval relate to each other?

`sample_rate` and `SampleInterval` are independent mechanisms that operate at different stages:
...
- **`SampleInterval`**: A data field that appears in certain datasets (particularly [Network Analytics Logs](/logs/reference/log-fields/account/network_analytics_logs/)) indicating upstream sampling applied during data collection. A `SampleInterval` of 1000 means the log entry represents 1 in 1000 packets.
...
Note that when customer-configured sampling is applied, the `SampleInterval` field value in the logs is not modified. When there is no internal sampling, `SampleInterval` will always be 1 regardless of your configured `sample_rate`.


### How can I log mTLS Certificate Headers in Logpush Custom Fields?

To log mTLS certificate details (such as `cf-cert-subject-dn` or `cf-cert-issuer-dn`) in Logpush, you need to:

1. Enable the [Add TLS client auth headers](/rules/transform/managed-transforms/reference/#add-tls-client-auth-headers) Managed Transform to inject the certificate headers.
2. Configure [Logpush custom fields](/logs/logpush/logpush-job/custom-fields/) using `transformed_request_fields` (not `request_fields`) to capture these Cloudflare-injected headers.
3. Ensure your Logpush job includes the `RequestHeaders` field.

The mTLS headers are injected by Cloudflare after the client request is received, so they must be captured using `transformed_request_fields` rather than `request_fields`. Refer to [mTLS authentication](/ssl/client-certificates/enable-mtls/) for more information on configuring client certificates.


### Is it possible to log the Request Body?

No, request body content cannot be logged through Cloudflare's logging systems. While request and response headers can be logged using [Logpush custom fields](/logs/logpush/logpush-job/custom-fields/) (Enterprise), request bodies are not available in any Logpush or Logpull dataset.

Request bodies often contain sensitive data and would generate massive data volumes. If you need visibility into request body content, consider logging at your origin server or using [Cloudflare Workers](/workers/) to selectively capture specific body content to a third-party service or [R2](/r2/).

:::note

You can include the encrypted matched payload in your Logpush jobs by adding the General > [Metadata](https://developers.cloudflare.com/logs/reference/log-fields/account/firewall_events/#metadata) field from the Firewall Events dataset to your job. You can then use a Worker to decrypt the payload: [matched-data-worker](https://github.com/cloudflare/matched-data-worker).

Refer to [Store decrypted matched payloads in logs](/waf/managed-rules/payload-logging/decrypt-in-logs/) for more information.
