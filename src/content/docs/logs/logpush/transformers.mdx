...
{/* VERIFY (before opening the docs PR): the entire page. Sources: SQL Logpush Transformers Handover — Rob Taylor, 2026-07-24; docs/transformers.md in cloudflare/data/logpush@master; logpush !1422 and !1431. */}
...
- A Logpush job that uses the `ndjson` output format. Transformers are only available for NDJSON jobs.
...
- Attach a transformer to any eligible Logpush job on the account. Only NDJSON jobs matching the transformer's dataset appear as available attach targets. CSV jobs are not shown.
...
{/* VERIFY: zone-scoped Logpush support — confirmed to exist, exact entry point and attach behavior TBD for closed beta. */}

{/* VERIFY: validation error surfacing in Studio (inline in editor with line/col numbers, panel, toast?) — TBD for closed beta. */}
...
For the full API reference, refer to [Logpush Transformers API](TBD).

{/* VERIFY: replace TBD with the real API reference URL once the /accounts/{account_id}/logpush/transformers endpoints are added to the Cloudflare OpenAPI schema. Owner: Rob Taylor + docs infra. */}
...
{/* VERIFY: confirm `ActionType` enum values in audit_logs_v2 (expected: create/update/delete). Filter needs updating if v2 uses different tokens. */}

{/* VERIFY: `AccountID` chosen as v2 replacement for v1's `OwnerID`. Confirm this is the intended replacement, or drop the field from the input record if not. */}
...
{/* VERIFY: `EdgeStartTimestamp` field type in http_requests. If it arrives as an integer (unixnano) rather than an RFC3339 string, the to_timestamp wrapping is wrong. */}
