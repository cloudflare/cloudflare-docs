...
import { APIRequest, DashButton, Badge } from "~/components";
...
Each Logpush job can have either a transformer or [filters](/logs/logpush/logpush-job/filters/), but not both. When you attach a transformer to a filtered job in the Cloudflare dashboard, Transformer Studio converts the existing filter to an equivalent SQL predicate.
