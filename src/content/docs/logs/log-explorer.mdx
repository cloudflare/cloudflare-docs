...
Authentication with the API can be done via auth header or API token. Append your API call with either of the following additional parameters.
...
1. Log in to the [Cloudflare dashboard](https://dash.cloudflare.com/login) and and select your account or domain (also known as zone).
...
If you would like to enable an account-level dataset, substitute `zones/{zone_id}` with `accounts/{account_id}` in the curl command. For example:
