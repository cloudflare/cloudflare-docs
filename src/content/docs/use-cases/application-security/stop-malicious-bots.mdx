...
3. Build an expression that targets requests to sensitive paths where the `User-Agent` header is empty or matches known bad patterns. For example, to challenge requests to `/login` with an empty user agent:
...
4. Check **Events by service** to see which rules are triggering. Events are labeled by their source: Bot Fight Mode, Super Bot Fight Mode, custom rules, or rate limiting rules.
