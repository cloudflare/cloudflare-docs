...
**Likely automated** traffic receives bot scores between 1 and 29. A score of 1 means the request is definitely automated. Scores from 2 to 29 indicate likely automated behavior. This category is your primary target. It includes scrapers, credential stuffing tools, inventory hoarding bots, and spam submitters.

**Likely human** traffic receives bot scores between 30 and 99. These requests appear to come from real users. Do not challenge or block this traffic, or you risk degrading the experience for legitimate visitors.
...
3. Find **Bot Fight Mode** and turn it on.

That is the only step required. Bot Fight Mode begins challenging suspected bot traffic immediately.
...
For organizations that need machine learning-based bot scoring, behavioral analysis, and custom allow/block rules, Cloudflare Bot Management provides the most granular detection and response capabilities. Bot Management assigns a score from 1 (likely bot) to 99 (likely human) to every request, which you can reference in WAF custom rules. For more information, refer to [Bot Management](/bots/plans/bm-subscription/).
:::
...
2. Go to **Turnstile**.
3. Select **Add widget**.
4. Enter a name for the widget and add the hostnames where it will appear.
5. Choose a widget mode:
   - _Managed_ — Cloudflare decides whether to show a challenge. Most users pass without interaction.
   - _Non-interactive_ — always runs a background challenge with no user interaction.
   - _Invisible_ — runs in the background with no visible widget.
6. Select **Create**.
7. Copy the **Site Key** (public) and **Secret Key** (private). You need both in the following steps.
...
WAF custom rules and rate limiting rules let you act on specific traffic patterns. The WAF separates detection (scoring traffic) from mitigation (acting on those scores). Detections like bot score label incoming requests, but they do not block anything on their own. You write rules that reference those signals to decide what action to take.
...
4. Build an expression that targets requests to sensitive paths where the `User-Agent` header is empty or matches known bad patterns. For example, to challenge requests to `/login` with an empty user agent:
...
For step-by-step rule creation, refer to [Create a rate limiting rule in the dashboard](/waf/rate-limiting-rules/create-zone-dashboard/).
...
Set the action to **Managed Challenge**. This expression challenges requests with a bot score under 30 on the checkout path while allowing verified bots through.
...
:::note[Bot score in WAF rules requires a Pro plan or above]
...
3. Review the **Events summary** for an overview of mitigated requests over time.
4. Check **Events by service** to see which rules are triggering. Events are labeled by their source: Bot Fight Mode, Super Bot Fight Mode, WAF custom rules, or rate limiting rules.
5. Scroll to **Sampled logs** to inspect individual requests. Each log entry shows the action taken, the rule that triggered, the source IP, user agent, URI path, and country.
