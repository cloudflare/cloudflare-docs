...
<Render
	file="create-rule-nav"
	product="waf"
	params={{ ruleType: "Custom rules", oldNavPath: "Custom rules" }}
/>
...
<Render file="security-events-nav" product="waf" />

Review the **Events summary** for an overview of mitigated requests over time. 4. Check **Events by service** to see which rules are triggering. Events are labeled by their source: Bot Fight Mode, Super Bot Fight Mode, custom rules, or rate limiting rules. 5. Scroll to **Sampled logs** to inspect individual requests. Each log entry shows the action taken, the rule that triggered, the source IP, user agent, URI path, and country.
