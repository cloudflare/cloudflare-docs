...
title: "Stop malicious bots: Free, Pro, and Business plans"
...
sidebar:
  label: "Guide: Free, Pro, and Business"
---
...
- Cannot be bypassed with [custom rule](/waf/custom-rules/) Skip actions. If Bot Fight Mode challenges a request you want to allow, your only options are IP Access rules or turning off Bot Fight Mode entirely.
...
Enterprise customers with [Bot Management](/bots/plans/bm-subscription/) can reference the `cf.bot_management.score` field in custom rule expressions for granular control over bot traffic. Bot Management assigns a score from 1 (definitely a bot) to 99 (likely human) to every request. Refer to [Bot scores](/bots/concepts/bot-score/) and [Bot Management variables](/bots/reference/bot-management-variables/) for details.
:::
...
- **Bot Fight Mode** (Free) cannot be bypassed with custom rule Skip actions. Your options are IP Access rules or turning off Bot Fight Mode entirely.
...
Search engine crawlers like Googlebot or Bingbot appear in your challenged or blocked events. Create a custom rule with a Skip action that allows verified bots through:
...
- If **Super Bot Fight Mode** (Pro and above) is blocking the traffic, create a custom rule with a Skip action matching the tool IP address or user agent. For example: `(ip.src eq 192.0.2.1)` with action **Skip** > **Skip Super Bot Fight Mode**.
...
- **Add more header-based rules.** Target additional suspicious patterns, such as requests with empty `Accept-Language` headers or non-browser `User-Agent` values.
- **Lower your rate limiting thresholds.** If bots are staying under your current limits, reduce the request count or shorten the time window. Deploy changes with the **Log** action first.
- **Add path-specific rules.** Create custom rules with stricter conditions on your most targeted paths.
- **Combine signals.** Use multiple fields in a single expression for more precise targeting. For example, challenge requests to `/login` with an empty `Accept-Language` header:

  ```txt
  (http.request.uri.path eq "/login" and len(http.request.headers["accept-language"][0]) eq 0)
  ```

Custom rules execute before Super Bot Fight Mode in the evaluation order. A custom rule that takes a terminating action (Block, Managed Challenge) prevents the request from reaching Super Bot Fight Mode. Keep this in mind when troubleshooting why a rule does not appear to trigger. For details on rule execution order, refer to [Security features interoperability](/waf/feature-interoperability/).

:::note[Enterprise Bot Management: bot score-based tuning]
Enterprise customers with [Bot Management](/bots/plans/bm-subscription/) can tune rules using the `cf.bot_management.score` field for more granular control. For example, lower the bot score threshold from 30 to 50, or combine bot score with other signals in a single expression. Refer to [Bot Management variables](/bots/reference/bot-management-variables/) for available fields.
:::

### Set up alerts

:::note[Bot traffic alerts require Enterprise Bot Management]
Bot Detection Alerts and Custom Bot Detection Alerts require [Bot Management](/bots/plans/bm-subscription/) (Enterprise add-on). These alerts notify you when Cloudflare detects abnormal spikes in bot traffic. For setup instructions, refer to [Notifications](/notifications/).
:::

On Free, Pro, and Business plans, monitor bot activity by reviewing [Security Events](/waf/analytics/security-events/) regularly. You can also configure general [notification alerts](/notifications/) for security events such as firewall rule matches.
