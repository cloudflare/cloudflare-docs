...
5. Under **Increment counter when**, select **Edit expression** and enter: `http.request.uri.path eq "/login" and http.request.method eq "POST" and http.response.code in {401 403}`
...
Available periods, actions, and counting options vary by plan. Free plans support a 10-second period and Block action only. Pro plans add periods up to 1 minute and challenge actions. Business plans add periods up to 10 minutes and custom counting expressions. The example below uses a 1-minute period with Managed Challenge (Pro and above). Refer to [Rate limiting rules availability](/waf/rate-limiting-rules/#availability) for full plan details.
:::
