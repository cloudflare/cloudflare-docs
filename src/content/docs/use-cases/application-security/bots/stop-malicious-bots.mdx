...
Super Bot Fight Mode adds verified bot allowlisting, per-category actions, static resource protection, and JavaScript detections. For full details and setup instructions, refer to [Super Bot Fight Mode](/bots/get-started/super-bot-fight-mode/).

:::note
If Bot Fight Mode is currently active on your domain, disable it before enabling Super Bot Fight Mode. Go to **Security** > **Settings**, filter by **Bot traffic**, and turn **Bot fight mode** off.
:::
...
For login endpoints, a tiered rate limiting approach works well alongside Turnstile. The following example from the [Rate limiting best practices](/waf/rate-limiting-rules/best-practices/) shows rules that escalate the response based on the volume of failed attempts. Adjust the thresholds for your site's traffic patterns.
...
- **Short-window rule**: 4 `POST` requests to `/login` returning `401` or `403` within 1 minute → Managed Challenge.
- **Long-window rule**: 20 failed login attempts from one IP within 1 hour → Block for 24 hours.
...
Start with **Managed Challenge** as the action for both rules. Review the results in [Security Events](/waf/analytics/security-events/) to confirm the thresholds are not catching legitimate users. Once confirmed, change to **Block** if needed.

:::note[Log action requires an Enterprise plan]
Enterprise customers can use the **Log** action to observe which requests a rule matches without taking any action. On Free, Pro, and Business plans, start with **Managed Challenge** as a less aggressive alternative to **Block**.
:::
...
Enterprise customers can use the **Request rate analysis** tab in Security Analytics to visualize request rate distributions and the **Log** action to observe rule matches without blocking traffic. On Free, Pro, and Business plans, estimate thresholds based on your expected traffic patterns and start with **Managed Challenge** before switching to **Block**. Refer to [Find an appropriate rate limit](/waf/rate-limiting-rules/find-rate-limit/) for the full methodology.
:::
...
If bots are staying under your rate limiting thresholds, edit the rate limiting rule and reduce the request count or shorten the time window. Start with _Managed Challenge_ before switching to _Block_.
