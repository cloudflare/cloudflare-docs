...
[Turnstile](/turnstile/) and [rate limiting rules](/waf/rate-limiting-rules/) protect form endpoints in different ways and work best together.
...
[Custom rules](/waf/custom-rules/) and [rate limiting rules](/waf/rate-limiting-rules/) let you target specific traffic patterns that built-in bot protection does not catch. Cloudflare separates detection (scoring traffic) from mitigation (acting on those scores). You write rules that reference detection signals to decide what action to take.
...
After you deploy bot protection rules, use [Security Events](/waf/analytics/security-events/) to verify they are working as intended and tune thresholds based on what you see.
