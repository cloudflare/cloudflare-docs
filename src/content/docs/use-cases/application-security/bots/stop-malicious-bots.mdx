...
- If [Bot Fight Mode](/bots/get-started/bot-fight-mode/) (Free) is blocking the traffic, add the tool IP address to your [IP Access Allow list](/waf/tools/ip-access-rules/). Bot Fight Mode does not trigger when an IP Access rule matches the request.
...
You see bot activity in Security Events that your current rules do not catch. Create a new custom rule that combines multiple signals for more precise targeting. For example, to challenge requests to `/login` with an empty `Accept-Language` header:
