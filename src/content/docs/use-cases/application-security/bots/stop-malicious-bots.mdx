...
Super Bot Fight Mode adds verified bot allowlisting, per-category actions, static resource protection, and JavaScript detections. If Bot Fight Mode is currently active on your domain, disable it before enabling Super Bot Fight Mode. For full details and setup instructions, refer to [Super Bot Fight Mode](/bots/get-started/super-bot-fight-mode/).
...
Turnstile challenges suspected bots before they can submit a form, without showing visitors a CAPTCHA. It can be embedded into any website without sending traffic through Cloudflare. Use Turnstile when you need to challenge automated form submissions.
...
Replace `<YOUR-SITE-KEY>` with the site key from the previous step. The widget renders inside the `div` and produces a token when the visitor passes the challenge.
...
Replace `<YOUR-SECRET-KEY>` with your secret key and `<TURNSTILE-RESPONSE-TOKEN>` with the `cf-turnstile-response` value from the form submission. The endpoint returns a JSON object with a `success` field. Only process the form submission if `success` is `true`.
...
The built-in bot settings are part of [Super Bot Fight Mode](/bots/get-started/super-bot-fight-mode/). On Free plans, only the Bot Fight Mode toggle is available.
:::
...
After you deploy bot protection rules, use the [Security Events](/waf/analytics/security-events/) dashboard in Application Security to verify they are working as intended and tune thresholds based on what you see.
...
After reviewing Security Events, adjust your rules based on what you see.
...
- If [Super Bot Fight Mode](/bots/get-started/super-bot-fight-mode/) (Pro and above) is blocking the traffic, create a custom rule with a Skip action matching the tool IP address or user agent:
  ...
  5. Under **Then take action**, select _Skip_ > **Skip Super Bot Fight Mode**.
  6. Select **Deploy**.
...
4. Under **When incoming requests match**, select **Edit expression** and enter:
   ...
   ```txt
   (http.request.uri.path eq "/login" and len(http.request.headers["accept-language"][0]) eq 0)
   ```
