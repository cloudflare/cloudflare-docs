...
Deploy both rules with the **Log** action first. Review the results in [Security Events](/waf/analytics/security-events/) to confirm the thresholds are not catching legitimate users. Then change to the enforcing actions.
...
7. Select **Deploy**.
8. Review the results in [Security Events](/waf/analytics/security-events/). If the rule matches only bot traffic, edit the rule and change the action to _Managed Challenge_ or _Block_.
