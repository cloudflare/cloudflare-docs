...
Bot Fight Mode is included with Free plans. It provides broad, baseline protection.
...
Rate limiting caps the number of requests a client can make to an endpoint within a time window, such as the same IP hitting your login endpoint hundreds of times per minute. Use rate limiting when you need to control request volume regardless of whether the client is human.
