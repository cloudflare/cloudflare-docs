...
Review the results in [Security Events](/waf/analytics/security-events/) to confirm the thresholds are not catching legitimate users.

:::note[Log action requires an Enterprise plan]
The **Log** action records matching requests without taking action, which is useful for validating rules. Log is only available on Enterprise plans. On other plans, the examples above use **Managed Challenge**, which challenges matching requests rather than blocking them outright.
:::
