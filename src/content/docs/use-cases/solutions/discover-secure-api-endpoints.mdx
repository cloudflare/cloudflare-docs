...
2. Create a fallthrough rule based on a template that blocks requests that do not match a saved endpoint in [Endpoint Management](/api-shield/management-and-monitoring/).
...
3. If legitimate clients are being rate limited, edit the relevant rate limiting rule to increase the request threshold or widen the time period for the affected rule.

4. If abusive traffic is getting through, lower the rule threshold or narrow the time period.
