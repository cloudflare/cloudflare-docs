...
1) When there is no monitor attached to a pool then health status is not considered during steering.
2) Origins are treated as down when monitoring is enabled but health status is still unknown.
3) If there are no monitors, a fallback pool is still required but it will only be used if the default pools all have origins with FQDN addresses that cannot be resolved.
