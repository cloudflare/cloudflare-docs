...
- **Private load balancer IP**: End users connect to the application using the load balancer's IP address. This can either be a Cloudflare-assigned CGNAT IP (`100.112.0.0/16`) or a custom [RFC 1918](https://datatracker.ietf.org/doc/html/rfc1918) IP.
...
4. On the next step you can choose to associate this load balancer with either: - A Cloudflare-assigned IP from the `100.112.0.0/16` range - A custom [RFC 1918 address](https://datatracker.ietf.org/doc/html/rfc1918)
