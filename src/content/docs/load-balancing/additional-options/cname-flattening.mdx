...
If the selected endpoint has `flatten_cname: false` but any of the conditions above is not met, the load balancer flattens the CNAME and returns `A`/`AAAA` records as if the toggle were on.
