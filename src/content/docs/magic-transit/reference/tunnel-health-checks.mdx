...
Refer to [Add tunnels](/magic-transit/how-to/configure-tunnel-endpoints/#add-tunnels) to learn how to configure bidirectional or unidirectional health checks.
...
A tunnel health check probe can have two possible types: request and reply. For each type, the source and destination address depends on the direction. Refer to [Add tunnels](/magic-transit/how-to/configure-tunnel-endpoints/#add-tunnels) to learn how to change this setting.
...
Magic Transit steers traffic to tunnels based on priorities you set when you [assign tunnel route priorities during onboarding](/magic-transit/how-to/configure-routes/). Tunnel routes with lower values have priority over those with higher values.
...
The values for failure penalties are intentionally extreme so that they always exceed the priority values assigned during [routing configuration](/magic-transit/how-to/configure-routes/).
