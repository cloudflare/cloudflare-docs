...
For example, if you have onboarded `203.0.113.0/24` to Magic Transit and are advertising it from the Cloudflare edge, but have also advertised a more specific `203.0.113.128/25` via your ISP, the following applies:
 - **Internet traffic** to `203.0.113.128/25` reaches you via your ISP because the global Internet routing table uses Longest Prefix Match.
 - **Cloudflare-originated traffic** to `203.0.113.128/25` is routed through your Magic Transit tunnels and interconnects because Cloudflare keeps that traffic inside its own network when the covering /24 prefix is advertised from Cloudflare. This traffic counts toward your bandwidth usage.
