...
<AnchorHeading depth={2} title="Magic Transit Virtual Network routing table" />
...
- Magic Transit packet transport between IPsec/GRE tunnels, interconnects, [Cloudflare Load Balancer](/load-balancing/), and [Zero Trust](/cloudflare-one/) connections such as [Cloudflare One Client](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/), [Remote Browser Isolation](/cloudflare-one/remote-browser-isolation/), [Access](/cloudflare-one/access-controls/policies/), and [Gateway](/cloudflare-one/traffic-policies/).

The Magic Transit Virtual Network supports routing the Magic Transit traffic through anycast tunnels using [GRE and Internet Protocol Security (IPsec)](/magic-transit/reference/gre-ipsec-tunnels/) or [CNI with Dataplane v2](/network-interconnect/). You can add entries to the Magic Transit Virtual Network routing table through static route configuration or through routes learned through BGP peering (beta).
...
The priority value for static routes is directly configured as part of the route object in the Cloudflare [dashboard or through the API](/magic-transit/how-to/configure-routes/#create-a-static-route). For example:

| Prefix | NextHop | Priority |
| ----- | ----- | ----- |
| `203.0.113.0/24` | `TUNNEL_1_IAD` | `200` |
| `203.0.113.0/24` | `TUNNEL_2_IAD` | `200` |
| `203.0.113.0/24` | `TUNNEL_3_ATL` | `100` |
| `203.0.113.0/24` | `TUNNEL_4_ATL` | `100` |
...
| ----- | ----- | ----- | ----- |
| `203.0.113.0/24` | `TUNNEL_1_IAD` | `100` | `64` |
| `203.0.113.0/24` | `TUNNEL_2_IAD` | `100` | `128` |
| `203.0.113.0/24` | `TUNNEL_3_ATL` | `100` | `192` |
| `203.0.113.0/24` | `TUNNEL_4_ATL` | `100` | `255` |
...
| ----- | ----- | ----- | ----- |
| `203.0.113.0/24` | `TUNNEL_1_IAD` | `100` | `AFR` |
| `203.0.113.0/24` | `TUNNEL_2_IAD` | `100` | `EEUR` |
| `203.0.113.0/24` | `TUNNEL_3_ATL` | `100` | `ENAM` |
| `203.0.113.0/24` | `TUNNEL_4_ATL` | `100` | `ME` |
| `203.0.113.0/24` | `TUNNEL_5_ATL` | `100` | `WNAM` |
| `203.0.113.0/24` | `TUNNEL_4_ATL` | `100` | `ENAM` |
...
Configure scoping for your traffic in the **Region code** section when adding or editing a static route. Refer to [Create a static route](/magic-transit/how-to/configure-routes/#create-a-static-route) and [Edit a static route](/magic-transit/how-to/configure-routes/#edit-a-static-route) for more information.
...
| Prefix            | NextHop        |
| ----------------- | -------------- |
| `203.0.113.0/29` | `TUNNEL_1_IAD` |
| `203.0.113.8/29` | `TUNNEL_2_ATL` |
...
<AnchorHeading depth={3} title="BGP peering with the Magic Transit Virtual Network routing table" />

Magic Transit BGP peering is with the Magic Transit Virtual Network routing table (as opposed to peering with the Cloudflare Internet global network). BGP peers configured by following this guide will receive advertisements for all prefixes in the Magic Transit Virtual Network routing table plus any additional prefixes configured in the on-ramp [Advertised prefix list](/magic-transit/how-to/configure-routes/#set-up-bgp-peering).

If instead you are seeking to do public peering with the Cloudflare ASN 13335 at one of the Cloudflare data centers, refer to [PNI and peering setup](/network-interconnect/). It is not currently possible to share Magic Transit Virtual Network BGP peering and PNI on the same physical interconnect port.
...
For Magic Transit customers, BGP with the Magic Transit Virtual Network routing table is separated from the announcement of anycast prefixes at the Cloudflare edge. Anycast withdrawal must be controlled with existing methods documented in [Advertise prefixes](/magic-transit/how-to/advertise-prefixes/).

### Tunnel health checks

You need to enable [legacy health checks](/magic-transit/reference/tunnel-health-checks/#legacy-bidirectional-health-checks) alongside BGP. This is essential to determine if a specific Cloudflare data center is reachable from your device. [Tunnel health checks](/magic-transit/reference/tunnel-health-checks/) modify the route priorities for dynamically learned BGP routes.
