...
**Resolution:** Switch to bidirectional health checks, which send both the probe and the response through the tunnel. Bidirectional health checks require egress traffic to be enabled on your Magic Transit configuration. For more information, refer to [Tunnel health checks](/magic-transit/reference/tunnel-health-checks/).
