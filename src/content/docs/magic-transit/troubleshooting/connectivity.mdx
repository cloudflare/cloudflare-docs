...
B -- "Yes, I have<br>connectivity issues" --> C["Identify your ingress<br>data center"]
...
D -- "Yes" --> C
C --> F["Check tunnel health<br>at that data center"]
F --> G{"Are tunnels healthy<br>at your ingress<br>data center?"}
...
You can also identify which data centers handle your traffic by checking the **Average ingress traffic** column on the **Connector health** page. Data centers with traffic volume are actively routing your traffic.
...
3. Look at the **Average ingress traffic** column. Data centers with non-zero traffic values are actively routing traffic for your network.

## 2. Check tunnel health at that data center

After you identify the data center, check the health of tunnels at that specific location.
...
4. In the tunnel detail view, review the health status per data center. Focus on the data center you identified in [step 1](#1-identify-your-ingress-data-center).

If the tunnels at your ingress data center are healthy, a degraded tunnel at a different data center is not the cause of your connectivity issue.
...
2. Does this type of troubleshooting also apply to Cloudflare WAN customers? The 2025 supportability recap mentions WAN IPsec tunnel degradation as a top issue, and support runbooks point WAN tunnel troubleshooting back to the MT guide. If so, we should convert this page to a shared partial and render it from both products.
:::

## 3. Correlate with Cloudflare status
