...
import { CURL, GlossaryTooltip, TabItem, Tabs } from "~/components";

By default, Cloudflare servers send <GlossaryTooltip term="tunnel health-check" link="/magic-transit/reference/tunnel-health-checks/">health checks</GlossaryTooltip> to each <GlossaryTooltip term="GRE tunnel">GRE</GlossaryTooltip>, Cloudflare Network Interconnect (CNI), or <GlossaryTooltip term="IPsec tunnel">IPsec</GlossaryTooltip> tunnel endpoint you configure to receive traffic from Magic Transit.
...
1. To create or edit your tunnel, refer to [Add tunnels](/magic-transit/how-to/configure-tunnel-endpoints/#add-tunnels).
