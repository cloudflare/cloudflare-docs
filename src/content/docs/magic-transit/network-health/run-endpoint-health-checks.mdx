...
<Render
	file="graphql/query-magic-transit-endpoint-health-checks"
...
4. Select **Add**.
5. From the drop-down menu, select _Magic Transit_.
...
8. In the _Service Level Objective (SLO)_ drop-down menu, select the SLO threshold for your notification. The SLO defines the percentage of endpoint health checks that must pass. If the number of passing endpoint health checks falls below the SLO, Cloudflare generates an alert:
   - **High** - 99%
   - **Medium** - 98%
   - **Low** - 97%
