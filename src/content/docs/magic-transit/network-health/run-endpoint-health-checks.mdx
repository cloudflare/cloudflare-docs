...
You can only configure endpoint health checks through the Cloudflare API. You cannot configure endpoint health checks through the dashboard, and they are not shown in the dashboard. Currently, configuring health checks is a beta feature.
...
9. In the drop-down menu below SLOs, select the `id` value that matches the `id` you got through the API in step one. This `id` should match the endpoint health check you want to get notifications for.
