...
:::caution
Cloudflare allocates a `/96` IPv6 prefix for each account. The first two addresses in this range are reserved for Cloudflare internal configuration. The remaining addresses are available for customer GRE tunnels, starting from `:2`, with two IPv6 addresses assigned per tunnel.
:::
...
Your account is allocated the prefix `2001:db8:abcd:1234::/96`.

- First two addresses (`::0` and `::1`) are reserved for Cloudflare internal use.
- You can use any other addresses in the `/96` prefix to create your own `/127` subnets for tunnels.

If you decide to use the first available `/127` prefix after the reserved addresses, for example `2001:db8:abcd:1234::2/127`:
- Cloudflare IPv6 interface address would be `2001:db8:abcd:1234::2`
- Router IPv6 address would be `2001:db8:abcd:1234::3`

Tunnel 2 would be `2001:db8:abcd:1234::4/127`:
- Cloudflare IPv6 Interface address would be `2001:db8:abcd:1234::4`
