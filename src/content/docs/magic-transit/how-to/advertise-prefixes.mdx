...
- **BGP peering with route reflectors**: Control advertisements through BGP sessions to Cloudflare's globally distributed route reflectors, either over the Internet or over a Classic CNI connection. Contact your Cloudflare account team if you need this option. Refer to [BGP control with Cloudflare Route Reflectors](#bgp-control-with-cloudflare-route-reflectors).
...
- **BGP peering with Magic Transit routing table**: Automatically control prefix advertisements based on BGP routes learned through Direct CNI. Refer to [BGP control to Magic routing table](#bgp-control-to-magic-routing-table).
...
If you use Direct CNI with the 2.0 CNI dataplane, you can:
