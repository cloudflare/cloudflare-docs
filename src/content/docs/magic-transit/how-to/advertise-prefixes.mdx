...
For example, if you have a CDN zone onboarded with a Magic Transit-protected origin, the proxied CDN traffic from Cloudflare's servers will continue to be delivered to your Magic Transit network and follow its route to your configured tunnel(s), even if a more specific or shorter route is publicly available on the internet. 
:::
