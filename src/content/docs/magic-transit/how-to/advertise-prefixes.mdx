...
- **BGP peering with Magic Transit network routing table**: Automatically control prefix advertisements based on BGP routes learned through CNI with Dataplane v2 (closed beta), or GRE and IPsec tunnels (beta). This method requires [Unified Routing](/magic-transit/reference/traffic-steering/#unified-routing). Refer to [BGP control to Magic Transit Virtual Network routing table](#bgp-control-to-magic-transit-virtual-network-routing-table).
