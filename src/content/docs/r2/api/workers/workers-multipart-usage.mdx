...
```js
// List all in-progress multipart uploads, paginating through every page.
const allUploads = [];
let cursor;
let truncated = true;
while (truncated) {
	const page = await env.MY_BUCKET.listMultipartUploads({ cursor });
	for (const upload of page.uploads) {
		allUploads.push(upload);
		console.log(
			`Upload: ${upload.key}, ID: ${upload.uploadId}, started: ${upload.initiated.toISOString()}`,
		);
	}
...
// List all uploaded parts, paginating through every page.
const allParts = [];
let partNumberMarker;
let truncated = true;
while (truncated) {
	const page = await multipartUpload.listParts({ partNumberMarker });
	for (const part of page.parts) {
		allParts.push(part);
		console.log(
			`Part ${part.partNumber}: etag=${part.etag}, size=${part.size}, uploaded=${part.uploaded.toISOString()}`,
		);
	}
	truncated = page.truncated;
	partNumberMarker = page.truncated ? page.partNumberMarker : undefined;
}
```
