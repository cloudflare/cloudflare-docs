...
import { Steps, Tabs, TabItem, Render, DashButton, LinkCard } from "~/components"
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		try {
			// Upload parts — in a real application, you would split the
			// file and potentially upload parts concurrently from the client
...
const url = new URL("https://<ACCOUNT_ID>.r2.cloudflarestorage.com/my-bucket/image.png");
...
// Return signed.url to the client — they can PUT directly to R2
```
...
const fileContent: Buffer = await readFile("./image.png");
