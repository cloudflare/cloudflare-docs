...
[^5]: Concurrent writes to the same object name (key) at a higher rate return [HTTP 429 (rate limited)](/support/troubleshooting/http-status-codes/4xx-client-error/error-429/#429-too-many-requests) responses.
...
- If you exceed the rate limit (hundreds of requests/second), requests to your `r2.dev` endpoint will be temporarily throttled and you will receive a `[429 Too Many Requests](/support/troubleshooting/http-status-codes/4xx-client-error/error-429/#429-too-many-requests)` response.
