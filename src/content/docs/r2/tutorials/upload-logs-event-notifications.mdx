...
To deploy your consumer Worker, run the [`wrangler deploy`](/workers/wrangler/commands/general/#deploy) command:
...
Now that you have your consumer Worker ready to handle incoming event notification messages, you need to enable event notifications with the [`wrangler r2 bucket notification create` command](/workers/wrangler/commands/r2/#r2-bucket-notification-create) for `example-upload-bucket`:
