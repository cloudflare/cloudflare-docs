...
For this tutorial your Wrangler configuration file should be similar to the following:
...
Secrets do not belong in Wrangler configuration files. If you need to add secrets, use `.dev.vars` for local secrets and the `wranger secret put` command for deploying secrets. For more information, refer to [Secrets](/workers/configuration/secrets/).
