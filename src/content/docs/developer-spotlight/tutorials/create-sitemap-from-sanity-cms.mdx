...
For this tutorial your `wrangler.toml / wrangler.json(c)` file should be similar to the following:
...
Secrets do not belong in `wrangler.toml / wrangler.json(c)` files. If you need to add secrets, use `.dev.vars` for local secrets and the `wranger secret put` command for deploying secrets. For more information, refer to [Secrets](/workers/configuration/secrets/).
