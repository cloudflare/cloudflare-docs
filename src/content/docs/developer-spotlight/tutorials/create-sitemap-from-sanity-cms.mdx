...
## Configure [Wrangler's configuration](/workers/wrangler/configuration) file

A default [Wrangler configuration](/workers/wrangler/configuration) file was generated in the previous step.

The [Wrangler configuration](/workers/wrangler/configuration) file is a configuration file used to specify project settings and deployment configurations in a structured format.

For this tutorial your [Wrangler configuration](/workers/wrangler/configuration) file should be similar to the following:
...
Secrets do not belong in [Wrangler's configuration](/workers/wrangler/configuration) file. If you need to add secrets, use `.dev.vars` for local secrets and the `wranger secret put` command for deploying secrets. For more information, refer to [Secrets](/workers/configuration/secrets/).
 ...
 *
 * Bind resources to your worker in [Wrangler's configuration](/workers/wrangler/configuration) file. After adding bindings, a type definition for the
