...
- [**Single Redirects**](/rules/url-forwarding/single-redirects/): Allow you to create static or dynamic redirects at the zone level (a single domain or subdomain). A [wildcard-based](/ruleset-engine/rules-language/operators/#wildcard-matching) interface allows you to define source and target URL patterns without complex functions or regular expressions, efficiently covering thousands of URLs with a single rule.

- [**Bulk Redirects**](/rules/url-forwarding/bulk-redirects/): Allow you to define a large number of redirects at the account level, which can apply across domains in your account. These URL redirects are essentially static — they do not support string replacement operations or regular expressions. However, you can configure parameters that affect how source URLs are matched and how the redirect is performed.
