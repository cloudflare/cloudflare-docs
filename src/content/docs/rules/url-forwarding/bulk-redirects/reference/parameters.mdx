...
- **Include subdomains** (Boolean)
  ...
  - If true, the source URL hostname will also apply to any subdomains — the redirect will match for all subdomains to the left of the domain portion of the source URL, as well as the specified domain. The default value is `true`.
...
- <a name="preserve-query-string"></a> **Preserve query string** (Boolean)
  ...
  - If true, the redirect URL will keep the query string of the original request. The default value is `true`.
...
- **Status code** (Integer)
  ...
  - The HTTP status code returned to the client when redirecting. The default value is `301`.esc 
  - The value must be one of the following: `301` (Moved permanently), `302` (Found, also known as Moved temporarily), `307` (Temporary redirect), or `308` (Permanent redirect). tab control+shift+m to thetab
