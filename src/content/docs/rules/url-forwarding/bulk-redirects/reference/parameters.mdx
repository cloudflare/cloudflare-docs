...
API field: `subpath_matching` <Type text="Boolean" />
...
API field: `include_subdomains` <Type text="Boolean" />
...
API field: `preserve_query_string` <Type text="Boolean" />
...
API field: `preserve_path_suffix` <Type text="Boolean" />

Applicable only when [**Subpath matching**](#subpath-matching) is enabled. If `true`, defines that the redirect URL will include the remaining (non-matched) path elements of the source URL, if any. The default value is `true`.
...
API field: `status_code` <Type text="Integer" />
