...
- **Bulk Redirect Rule**: A rule powered by the [Ruleset Engine](/ruleset-engine/), which is the system Cloudflare uses to evaluate and execute rules. A Bulk Redirect Rule has an associated Bulk Redirect List.
...
For details on the CSV format for importing items to a Bulk Redirect List, refer to [CSV file format for Bulk Redirects](/rules/url-forwarding/bulk-redirects/reference/csv-file-format/).
