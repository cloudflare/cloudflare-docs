...
- An `action_parameters` object with additional configuration settings — refer to [Bulk Redirects API JSON objects](/rules/url-forwarding/bulk-redirects/reference/json-objects/#bulk-redirect-rule) for details.
...
<Render
	file="terraform-use-ref-field-with-location"
	product="rules"
/>
...
The API token used in API requests to manage Bulk Redirects objects (lists, list items, and rules) must have at least the following [permissions](/fundamentals/api/reference/permissions/):
