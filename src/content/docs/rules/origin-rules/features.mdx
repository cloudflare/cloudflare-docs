...
- When an origin rule is configured to override the `Host` header for a request destined for a [worker script](/workers/), the override is applied after the worker has executed.
