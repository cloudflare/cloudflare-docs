...
Paths used in validation procedures such as custom hostname verification ([Cloudflare for SaaS](/cloudflare-for-platforms/cloudflare-for-saas/)), [Pages domain validation](/pages/configuration/debugging-pages/), or [HTTP domain control validation (DCV)](/ssl/edge-certificates/changing-dcv-method/methods/http/) may be affected by redirects.
