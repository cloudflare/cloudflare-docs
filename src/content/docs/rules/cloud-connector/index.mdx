...
import { ProductAvailabilityText, Render } from "~/components";
...
Cloud Connector rules are evaluated last in the [request evaluation workflow](/ruleset-engine/reference/phases-list/). When a Cloud Connector rule matches and other rules have modified the same settings (such as the `Host` header), the Cloud Connector rule takes precedence.
