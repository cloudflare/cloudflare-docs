...
Use Custom Errors to replace default Cloudflare error pages with your own custom content. Custom error content is shown to visitors when an HTTP error occurs, whether the error comes from your origin server, a Cloudflare product (including [Cloudflare Workers](/workers/)), or a [security challenge](/cloudflare-challenges/).
...
3. Search for a matching custom error rule at the account level. The rule will have priority over [500 class and 1000 class](/rules/custom-errors/reference/error-page-types/) Error Pages at the account or zone level.
4. Search for a matching custom error rule at the zone level. The rule will have priority over [500 class and 1000 class](/rules/custom-errors/reference/error-page-types/) Error Pages at the account or zone level and over custom error rules at the account level.
...
When you provide a URL for a [custom error asset](#custom-error-assets), Cloudflare fetches the page and inlines all referenced resources (images, CSS, and JavaScript) into the HTML using base64 encoding. The processed page must not exceed approximately 1.5 MB.
