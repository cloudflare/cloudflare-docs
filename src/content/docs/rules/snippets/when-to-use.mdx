...
head:
  - tag: title
    content: When to use Snippets vs Workers
---

## Introduction

This guide helps you determine when to use Snippets or Workers on Cloudflare’s edge network. It provides best practices, comparisons, and real-world use cases to help you choose the right product for your workload.
...
- Persistent state management (for example, session storage, databases).
- Compute-intensive tasks (for example, image transformations, [AI inference](/workers-ai/)).
...
- Granular request matching using dozens of request attributes, including [URI](/ruleset-engine/rules-language/fields/reference/http.request.full_uri/), [user-agent](/ruleset-engine/rules-language/fields/reference/http.user_agent/), [cookies](/ruleset-engine/rules-language/fields/reference/http.request.cookies/), and more.
...
| Route traffic dynamically between [origins](/rules/snippets/examples/serve-different-origin/) | ✅ | ✅ |
...
**Use case:** Modify request and response headers dynamically.
...
**Use case:** Route traffic to a maintenance page when your origin is undergoing a planned maintenance.
...
**Use case:** Programmatic caching at the edge to reduce origin load.
...
**Use case:** Redirect users based on their geographic location.

```javascript
export default {
    async fetch(request) {
        const country = request.cf.country;
        const redirectMap = { US: "https://example.com/us", EU: "https://example.com/eu" };
        if (redirectMap[country]) return Response.redirect(redirectMap[country], 301);
...
**Use case:** If origin responded with `403 Forbidden` error code, redirect visitor to a different page.
...
**Use case:** If response to the original request is not `200 OK` or a redirect, send to another origin.
...
**Use case:** If origin responds with JSON, delete sensitive fields before returning a response to visitor.
...
**Use case:** Adjust [Cross-Origin Resource Sharing (CORS)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) headers and handle preflight requests.
...
**Use case:** Replace outdated links without making changes on your origin.
...
**Use case:** Define a delay to be used when incoming requests match your rule. Useful for suspicious requests.
...
#### Snippet: Adding a custom header
```javascript
export default {
    async fetch(request) {
        // Get the current timestamp
...
#### Worker: Reading a header and adding it to the response
```javascript
export default {
  async fetch(request) {
...
#### Worker: transforming and caching responses
```javascript
export default {
  async fetch(request) {
      const url = new URL(request.url);
...
#### Snippet: Serving cached responses or forwarding to Worker
```javascript
export default {
    async fetch(request) {
        const url = new URL(request.url);
...
## Migrating between Snippets and Workers
...
- Needs deployment automation via the CLI ([Wrangler](/workers/wrangler/)).
