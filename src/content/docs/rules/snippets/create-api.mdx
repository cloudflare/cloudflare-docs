...
```bash
curl -X PUT https://api.cloudflare.com/client/v4/zones/{zone_id}/snippets/{snippet_name} \
...
The required body parameters are `files` (the file with your JavaScript code) and `metadata` (containing the `main_module`, which must match the filename of the uploaded file).

To make this example work, you need to save your JavaScript code in a file named `example.js`, then execute `curl` command with a PUT request from the folder where `example.js` is located.
...
Once you’ve created a code snippet, you can link it to rules. This is done via the following `PUT` request to the `snippet_rules` endpoint.
