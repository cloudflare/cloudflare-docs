...
4. To create a new empty rule, select **Create rule**. To duplicate an existing rule, select the three dots next to it > **Duplicate**.
   ...
   ![The HTTP response header modification rule creation page in the Cloudflare dashboard.](~/assets/images/rules/transform/create-response-header-modification-rule.png)
...
8. For **Modify response header**, select one of the following operations:
   ...
   * *Add* — Adds an HTTP response header with a static string value. This operation will not remove any existing response headers with the same name.
