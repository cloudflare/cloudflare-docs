...
- Response header transform rule matches are determined based on the original request properties. For example, if rule #1 sets a header to a different value than the one in the original request and rule #2 checks for this new value in its filter expression, rule #2 will not match because field values are immutable within a phase.<br/>
...
- Response header transform rules that appear later in the list of rules can override changes done by previous rules. Additionally, a response header transform rule executed later will not take into account any changes already done by previous response header transform rules. For example, if multiple matching rules add or modify the same response header, only the last rule will effectively be applied, since it will override the header set by previous rules without taking into account any previous changes.<br/>
