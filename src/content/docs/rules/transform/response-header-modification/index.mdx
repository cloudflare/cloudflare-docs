...
<Render file="snippets-alternative" params={{ one: "response header modifications" }} /><br />
...
* To add a `set-cookie` header to the response, make sure you use the *Add* operation instead of *Set static*/*Set dynamic*. Using one of the *Set* operations will remove any `set-cookie` headers already in the response, including those added by other Cloudflare products such as Bot Management.

* Currently you can only use the *Add* operation with a literal string value.

<Render file="troubleshoot-rules-with-trace" params={{ one: "HTTP response header modification rules" }} />
