...
- **Dynamic rewrite**: Supports more advanced scenarios where you use a [rewrite expression](/ruleset-engine/rules-language/) (a formula based on request properties) to define the resulting path or query string.
...
When using Cloudflare Images, you can use URL Rewrite Rules to serve images from a custom path. For more information, refer to [Serve images from custom domains](/images/optimization/hosted-images/serve-from-custom-domains/).
...
- The values of request and response fields are immutable within each [phase](/ruleset-engine/about/phases/), such as the `http_request_transform` phase where URL rewrite rules are defined. This means that later URL rewrite rules will still use the original field values when evaluating their filter expressions, not the values changed by previous rules. Refer to [Field values during rule evaluation](/ruleset-engine/about/rules/#field-values-during-rule-evaluation) for more information.
