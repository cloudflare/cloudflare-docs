...
- Request header transform rule matches are determined based on the original request properties. For example, if rule #1 sets a header to a different value than the one in the original request and rule #2 checks for this new value in its filter expression, rule #2 will not match because field values are immutable within a phase.<br/>
...
- Request header transform rules that appear later in the list of rules can override changes done by previous rules. Additionally, a request header transform rule executed later will not take into account any changes already done by previous request header transform rules. For example, if multiple matching rules add or modify the same request header, only the last rule will effectively be applied, since it will override the header set by previous rules. Refer to [Transform Rules evaluation](/rules/transform/#transform-rules-evaluation) for more information.
