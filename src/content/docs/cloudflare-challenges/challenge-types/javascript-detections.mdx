...
To use JavaScript Detections, remove the `no-transform` directive from `Cache-Control` response headers on pages where you want JSD to run. For more information, refer to [Cache-Control directives](/cache/concepts/cache-control/#interaction-with-other-cloudflare-features).
