...
`<meta http-equiv="refresh" content="360">` is injected before `</head>`, and the placeholder token is replaced with the full challenge bootstrap `<script>` block. 
...
4. Do not block `/cdn-cgi/challenge-platform/` paths via CSP. The orchestrator and its sub-resources must load freely.
