...
Cloudflare replaces the `::CF_WIDGET_BOX::` placeholder [token](/rules/custom-errors/reference/error-tokens/) with the full challenge bootstrap `<script>` block.

`cTplC: 1` is set in `window._cf_chl_opt` to signal to Cloudflare that a custom template is in use.
...
3. Do not add your own `window._cf_chl_opt`. Any existing definition will cause conflicts.
4. Do not block `/cdn-cgi/challenge-platform/` paths via Content Security Policy (CSP). The orchestrator and its sub-resources must load freely.
