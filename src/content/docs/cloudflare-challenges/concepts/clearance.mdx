...
You can also use pre-clearance for sites where API endpoints are protected by WAF rules. API requests cannot render challenge pages, so when a visitor loads a page with a Turnstile widget (for example, a login form) and Turnstile issues a `cf_clearance` cookie to the visitor's browser, subsequent API requests from the browser now include the cookie to bypass WAF challenges.
