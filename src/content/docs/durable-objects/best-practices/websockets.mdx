...
:::caution[WebSocket disconnection when an instance is replaced]

Cloudflare occasionally replaces a Durable Object instance. For example, this happens when you deploy new Worker code.

All WebSockets connected to the instance close, including hibernating connections. Build clients to reconnect.

Hibernation itself does not close WebSocket connections. For reasons an instance shuts down, refer to [Shutdown behavior](/durable-objects/concepts/durable-object-lifecycle/#shutdown-behavior).
