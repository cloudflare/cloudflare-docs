...
A forced code update disconnects WebSockets attached to the running instance. This happens when the update timeout expires before hibernation or when you set the timeout to `0s`. The Web Standard WebSocket API prevents hibernation, so its connections are disconnected when the timeout expires.

Refer to [Code updates in the Durable Object lifecycle](/durable-objects/concepts/durable-object-lifecycle/#code-updates) for more information.
