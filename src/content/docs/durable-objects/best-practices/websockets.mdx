...
However, an active outbound WebSocket connection prevents eviction for up to 15 minutes. New I/O can reset or extend this period, so the invocation has no fixed 15-minute limit. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for more information.
