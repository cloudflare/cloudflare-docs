...
:::caution[WebSocket disconnection when a Durable Object restarts]

A Durable Object restarts occasionally, for example when you deploy a new version of your Worker. Restarting closes every WebSocket connected to it, including hibernating ones, so build your clients to reconnect.

Hibernation on its own does not close connections. Refer to [Shutdown behavior](/durable-objects/concepts/durable-object-lifecycle/#shutdown-behavior) for the reasons a Durable Object restarts.
