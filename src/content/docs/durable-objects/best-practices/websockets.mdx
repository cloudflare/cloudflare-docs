...
A forced code update disconnects WebSockets attached to the running instance. This happens when the deployment grace period ends before hibernation, or when you set the grace period to `0`. The Web Standard WebSocket API prevents hibernation, so its connections are disconnected when the grace period ends.

Refer to [Delay code updates until hibernation](/durable-objects/reference/durable-objects-code-updates/) for more information.
