...
However, an active outbound WebSocket connection keeps the Durable Object alive and prevents eviction for up to 15 minutes per connection. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for more information.
...
:::caution[WebSocket disconnection]

Cloudflare occasionally has to stop a running Durable Object instance. This is rare, and happens in cases such as a hardware failure, or an instance that stays active long enough that Cloudflare has to move it.

Connections accepted with the [WebSocket Hibernation API](#durable-objects-hibernation-websocket-api) are not closed by hibernation itself. Connections that use the Web Standard WebSocket API prevent hibernation, so they close whenever the instance stops.

Build clients to reconnect after any unexpected closure.
