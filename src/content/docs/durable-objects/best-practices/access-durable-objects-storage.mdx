...
[Transactional Storage API](/durable-objects/api/transactional-storage-api/#methods) methods are available on `ctx.storage` parameter passed to the Durable Object constructor.
...
[SQL API](/durable-objects/api/transactional-storage-api/#sqlexec) methods are available on `ctx.storage.sql` parameter passed to the Durable Object constructor.
