...
New beta version of Durable Objects is available where each Durable Object has a private, embedded SQLite database. SQL storage is opt-in during beta; otherwise, a Durable Object class has the standard, private key-value storage. Objects with SQL storage can use the [SQL API](/durable-objects/api/storage-api/#sqlexec). 
...
[SQL API](/durable-objects/api/storage-api/#sqlexec) methods are available on `ctx.storage.sql` parameter passed to the Durable Object constructor.
