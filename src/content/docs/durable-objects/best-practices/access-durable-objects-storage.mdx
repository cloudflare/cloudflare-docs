...
[Code updates](/durable-objects/concepts/durable-object-lifecycle/#code-updates) can wait for an active Durable Object to hibernate instead of restarting it immediately. The object still loses its in-memory state when it hibernates or an update forces a restart. Use the Storage API to persist state that must survive hibernation, eviction, or restart.
