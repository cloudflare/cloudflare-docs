...
Use [`preShutdown()`](/durable-objects/api/base/#preshutdown) to reduce how often you checkpoint buffered state, such as batched agent messages or metrics, not to replace incremental writes:
...
export class Agent extends DurableObject<Env> {
	...
	// ✅ Good: Buffer in memory, flush periodically and on shutdown
	async addMessage(message: string) {
		this.conversation.push(message);
		this.dirty = true;

		// Checkpoint on a relaxed interval, not on every message
	...
	// preShutdown() is best-effort and may not run, so the periodic
	// alarm above still owns durability. This handler only shortens
	// the gap before eviction.
	async preShutdown() {
		await this.checkpoint();
	}
	...
	private async checkpoint() {
		if (this.dirty) {
			await this.ctx.storage.put("conversation", this.conversation);
			this.dirty = false;
		}
	}

	// 🔴 Bad: Relying on preShutdown() as the only durability mechanism
	async addMessageRisky(message: string) {
		this.conversation.push(message);
		// No periodic checkpoint — data written since the last checkpoint
		// may be lost if the object shuts down unexpectedly.
	}
}
```
</TypeScriptExample>
...
For data-schema changes, run schema migrations in the constructor using `blockConcurrencyWhile()`. For class renames or deletions, change the class entry in the [`exports`](/durable-objects/reference/durable-objects-migrations/) field of your Wrangler configuration file:
...
Refer to [Durable Object class exports](/durable-objects/reference/durable-objects-migrations/) for more details on class lifecycle changes, and [Testing with Durable Objects](/durable-objects/examples/testing-with-durable-objects/) for comprehensive testing patterns including SQLite queries and alarm testing.
