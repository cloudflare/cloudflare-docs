...
### Understand how input and output gates prevent data races

While Durable Objects are single-threaded, JavaScript's `async`/`await` can allow multiple requests to interleave execution. Cloudflare's runtime uses **input gates** and **output gates** to prevent data races and ensure correctness by default.

**Input gates** block new events (incoming requests, fetch responses) while storage operations are in progress. This prevents concurrent requests from interleaving during critical storage operations:
...
**Output gates** hold outgoing network messages (responses, fetch requests) until pending storage writes complete. This ensures clients never see confirmation of data that hasn't been persisted:
...
However, input gates only protect during storage operations. Non-storage I/O like `fetch()` or writing to R2 allows other requests to interleave:
...
export class Processor extends DurableObject<Env> {
	...
	async processWithExternalCall(id: string) {
	...
	// ✅ Better: Use optimistic locking or check-and-set patterns
	async processWithCheck(id: string) {
		const item = await this.ctx.storage.get<{ status: string; version: number }>(
			`item:${id}`
		);
		...
		// Re-check version before updating
		const current = await this.ctx.storage.get<{ status: string; version: number }>(
			`item:${id}`
		);
		if (current?.version !== expectedVersion) {
			throw new Error("Item was modified by another request");
		}

		await this.ctx.storage.put(`item:${id}`, {
			status: "completed",
...
**Write coalescing:** Multiple storage writes without intervening `await` calls are automatically batched into a single atomic transaction:
...
export class Account extends DurableObject<Env> {
	...
	async transferBroken(fromId: string, toId: string, amount: number) {
		// 🔴 Bad: await breaks coalescing - each write is a separate transaction
...
**Use `blockConcurrencyWhile()` sparingly:** The [`blockConcurrencyWhile()`](/durable-objects/api/state/#blockconcurrencywhile) method guarantees that no other events are processed until the provided callback completes. This is useful for operations that must be atomic, such as schema migrations in the constructor:
