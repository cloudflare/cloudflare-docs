...
- **Strong consistency** — Operations must be serialized to avoid race conditions (counters, rate limiters, booking systems)
...
// ✅ Good use of Durable Objects: Rate limiting requires coordination
// All requests for a given API key must be counted by the same instance
export class RateLimiter extends DurableObject<Env> {
	async checkLimit(apiKey: string): Promise<{ allowed: boolean; remaining: number }> {
		const now = Date.now();
...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		// Route to a Durable Object by API key for coordinated rate limiting
		const id = env.RATE_LIMITER.idFromName(apiKey);
		const limiter = env.RATE_LIMITER.get(id);

		const { allowed, remaining } = await limiter.checkLimit(apiKey);
		...
		// ✅ Stateless work happens in the Worker
		// This part doesn't need coordination
