...
Use [`[shutdown]()`](/durable-objects/api/base/#shutdown) for a final best-effort checkpoint. Combine it with relaxed count-based or time-based checkpoints during normal requests.

The following counter accepts the loss of increments since the previous checkpoint after an unexpected shutdown. It checkpoints every 100 increments and again during clean shutdown:
...
export class Counter extends DurableObject<Env> {
	...
	constructor(ctx: DurableObjectState, env: Env) {
		...
		this.count = this.ctx.storage.sql
