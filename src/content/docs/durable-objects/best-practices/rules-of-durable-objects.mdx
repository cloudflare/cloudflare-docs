...
### Use deterministic IDs with `idFromName()` for predictable routing

Use `idFromName()` with meaningful, deterministic strings for consistent routing. The same input always produces the same Durable Object ID, ensuring requests for the same logical entity always reach the same instance.
...
SQLite storage is the recommended storage backend for new Durable Objects. It provides relational queries, indexes, transactions, and better performance than the legacy key-value storage API.
...
Durable Objects provide multiple storage layers, each with different characteristics:
...
When calling Durable Objects from a Worker, check for the `.retryable` and `.overloaded` properties on errors to determine whether to retry:
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			...
			const isOverloaded =
				error instanceof Error &&
				"overloaded" in error &&
				(error as Error & { overloaded: boolean }).overloaded;
...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		try {
			await callWithRetry(() => stub.processMessage("user-123", "Hello!"));
			return new Response("Message sent");
		} catch (error) {
...
Refer to [Error handling](/durable-objects/best-practices/error-handling/) for more details on error properties and retry strategies.
...
Each Durable Object can schedule its own future work using the Alarms API. This is more efficient than global cron jobs because only the relevant Durable Object wakes up at the scheduled time.
...
### Deleting Durable Objects

A Durable Object only ceases to exist when its storage is empty. To delete a Durable Object completely, you must call `deleteAll()` to remove all stored data. Simply deleting individual keys or dropping tables is not sufficient, as some internal metadata may remain.
...
export class ChatRoom extends DurableObject<Env> {
	async deleteRoom() {
		...
		// Delete all storage - this fully removes the Durable Object
		await this.ctx.storage.deleteAll();

		// After this, the Durable Object no longer exists
		// A new request with the same ID will create a fresh instance
	}
}
...
A single Durable Object handling all traffic becomes a bottleneck. Durable Objects execute single-threaded, so all requests to one instance are processed sequentially. Shard your workload by your coordination boundary.
...
// 🔴 Bad: Global singleton handles all counter operations
export class Counter extends DurableObject<Env> {
	async increment(counterId: string) {
		// All counters go through this one instance - bottleneck!
		const current = (await this.ctx.storage.get<number>(counterId)) ?? 0;
		await this.ctx.storage.put(counterId, current + 1);
		return current + 1;
	}
}

// 🔴 Bad: Always using the same ID
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
...
Instead, create one Durable Object per counter:
...
// ✅ Good: Each counter is its own Durable Object
export class Counter extends DurableObject<Env> {
	async increment() {
...
// ✅ Good: Different counters use different Durable Objects
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		const url = new URL(request.url);
		const counterId = url.pathname; // e.g., "/page-views" or "/api-calls"

		// Each counter ID gets its own Durable Object instance
		const stub = env.COUNTER.get(env.COUNTER.idFromName(counterId));
...
This is not a hard rule. Short external calls are fine. Consider offloading when operations consistently take more than a few hundred milliseconds.

## Testing and migrations

### Test with Vitest and plan for class migrations

Use `@cloudflare/vitest-pool-workers` for testing Durable Objects. For schema changes, run migrations in the constructor. For class renames or deletions, use Wrangler migrations.

<TypeScriptExample filename="index.test.ts">
```ts
import {
	env,
...
// Import your Durable Object class and Worker
import { ChatRoom } from "./index";

describe("ChatRoom", () => {
	it("should send and retrieve messages", async () => {
		// Get a stub to a test Durable Object instance
...
When you need to rename or delete a Durable Object class, use Wrangler migrations to ensure data is handled correctly:
...
[[migrations]]
tag = "v2"
renamed_classes = [{ from = "OldChatRoom", to = "ChatRoom" }]
...
Refer to [Durable Objects migrations](/durable-objects/reference/durable-objects-migrations/) for more details on class migrations, and [Testing](/durable-objects/examples/testing-with-durable-objects/) for testing patterns.
