...
### Store application-defined identifiers explicitly when needed

A Durable Object can access its runtime identity from `this.ctx.id`; named IDs can also expose their name. You do not need a custom `init()` method just to know the object identity. Use an explicit initializer only when your application needs to store additional metadata, such as a display name, creator, or other domain-specific identifier.
