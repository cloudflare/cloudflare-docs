...
1. Update the binding to use the SQLite storage in Durable Objects. In `wrangler.toml`, replace `new_classes=["MyDurableObject"]` with `new_sqlite_classes=["MyDurableObject"]`. Your `wrangler.toml` should look like this:
...
Replace the `MyDurableObject` class with the following code:
...
3. Add a `fetch` handler to the `MyDurableObject` class. This handler will return a text response. In [Step 5](#5-handle-websocket-connections) You will update the `fetch` handler to handle the WebSocket connection.
