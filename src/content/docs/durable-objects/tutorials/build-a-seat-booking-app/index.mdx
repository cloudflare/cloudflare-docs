...
title: Build a seat booking app with Durable Objects
products:
  - Durable Objects
languages:
  - TypeScript
...
In this tutorial, you will learn how to build a seat reservation app using Durable Objects. This app will allow users to book a seat for a flight. The app will be written in TypeScript and will use the new SQLite storage in Durable Object to store the data.

Using Durable Objects, you can write reusable code that can handle coordination and state management for multiple clients. Moreover, writing data to an SQLite database is synchronous, which means that the code will not block the main thread. You can learn more about SQLite storage in Durable Objects in the [SQLite in Durable Objects blogpost](https://blog.cloudflare.com/UPDATE_THIS/).

:::note[SQLite in Durable Objects]

SQLite in Durable Objects is currently in beta. You can learn more about the limitations of SQLite in Durable Objects in the [SQLite in Durable Objects documentation](https://blog.cloudflare.com/UPDATE_THIS/).
...
- The application will create a new Durable Object namespace for the flight number, if it does not already exist.
- If the namespace already exists, the application will retrieve the seats information from the SQLite database.
- If the namespace does not exist, the application will create a new namespace and initialize the SQLite database with the seats information. For the purpose of this tutorial, the seats information is hard-coded in the application.
...
The frontend of the application is a simple HTML page that allows users to select a seat and enter their name. The application uses [Workers Assets](/workers/platform/assets/) to serve the frontend.
	...
	<body>
		...
		<script>
			...
			function handleBooking() {
				const ws = new WebSocket(`ws://localhost:8787/ws?flightId=${flightId}`);
				...
				async function fetchSeats() {
					...
					createSeatGrid(JSON.parse(seats));
				}
...
[Workers Assets](/workers) is currently in beta. You can also use Cloudflare Pages to serve the frontend. However, you will need a separate Worker for the backend.
...
1. Update the binding to use the SQLite storage in Durable Objects. In `wrangler.toml`, update the following configuration.

```toml ins={10} del={9} title="wrangler.toml"
...
2. Add the `initializeSeats()` function to the `MyDurableObject` class. This function will be called when the Durable Object is initialized. It will check if the table exists, and if not, it will create it. It will also insert seats information in the table.
...
```ts title="src/index.ts" ins={9,12,15-38} del={2-8, 39-49}
...
3. Next, update the Worker's fetch handler to create a unique Durable Object instance for each flight.

```ts title="src/index.ts" ins={26-36} del={11-25}
export default {
	/**
...
Using the flight ID, from the query parameter, a unique Durable Object instance is created. This Durable Object instance is initialized with a table if it does not exist.

## 4. Add methods to the Durable Object

1. Add the `getSeats()` function to the `MyDurableObject` class. This function returns all the seats in the table.

```ts title="src/index.ts" ins={8-22}
export class MyDurableObject extends DurableObject {
    ...
...
2. Add the `assignSeat()` function to the `MyDurableObject` class. This function will assign a seat to a passenger. It takes the seat number and the passenger name as parameters.

```ts title="src/index.ts" ins={13-48}
export class MyDurableObject extends DurableObject {
	...
...
1. Add the `handleWebSocket()` function to the `MyDurableObject` class. This function handles the WebSocket connections.

```ts title="src/index.ts" ins={18-26}
export class MyDurableObject extends DurableObject {
	...
	...
	// Assign a seat to a passenger.
	private assignSeat(seatId: string, occupant: string) {
		...
	}
...
2. Add the `broadcastSeats()` function to the `MyDurableObject` class. This function will broadcast the updated seats to all the connected clients.

```ts title="src/index.ts" ins={22-24}
export class MyDurableObject extends DurableObject {
	...
...
3. Lastly, add the `fetch` handler to the `MyDurableObject` class. This handler will handle all the incoming requests and route them to the appropriate function.

```ts title="src/index.ts" ins={26-39}
export class MyDurableObject extends DurableObject {
	...
...
The Worker passes the request to the Durable Object as is. The `fetch` handler in the Durable Object then routes the request to the appropriate function. If the request is a `GET` request to `/seats`, the Durable Object returns the seats. If the request is a `POST` request to `/book-seat`, the Durable Object assigns the seat to the passenger. If the request is a WebSocket connection, the Durable Object handles the WebSocket connection.
...
- used the SQLite storage in Durable Objects to store the seats for a flight.
