...
- Deploying a Durable Object and a Worker that communicates with objects.
...
2. Accessed a Durable Object's attached storage, which is an private SQLite database only accesible to the object. [SQL API](/durable-objects/api/storage-api/#sqlexec) available on `ctx.storage` was called.
3. Returned an object representing the single row query result using `one()`, which checks that query result has exactly one row. 
...
To communicate with a Durable Object, the Worker fetch handler should look like the following:
...
5. Call a Durable Object by invoking a RPC method, `sayHello()`, on the Durable Object, which returns a `Hello, World!` string greeting.
