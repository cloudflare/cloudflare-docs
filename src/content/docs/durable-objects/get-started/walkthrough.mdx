...
New beta version of Durable Objects is available where each Durable Object has a private, embedded SQLite database. If you do not opt-in to the beta, then a Durable Object class has the standard key-value storage. Objects can access long-lived durable storage with the [Transactional Storage API](/durable-objects/api/transactional-storage-api/). 
