...
:::note[Pending operations prevent eviction]
Unfinished I/O operations, promises passed to `this.ctx.waitUntil()`, and open outbound connections prevent a Durable Object from hibernating or being evicted. Examples include service binding requests, Durable Object remote procedure call (RPC) calls, outbound requests, storage operations such as `this.ctx.storage.put()`, [`this.ctx.container.monitor()`](/durable-objects/api/container/#monitor), TCP sockets, and outbound WebSockets.

Each I/O operation prevents eviction until it completes or for up to 15 minutes from when it starts, whichever comes first. An operation started later can therefore extend the Durable Object's total time in memory.

Operations started together do not combine their windows. There is no overall invocation wall-time limit.

While an operation prevents eviction, the Durable Object remains in the **idle, in-memory, non-hibernateable** state and continues to [incur duration charges](/durable-objects/platform/pricing/#when-does-a-durable-object-incur-duration-charges). When no operation prevents eviction, the [standard eviction rules](#durable-object-lifecycle-state-transitions) resume.
:::
