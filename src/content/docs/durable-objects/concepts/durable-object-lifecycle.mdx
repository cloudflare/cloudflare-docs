...
A Durable Object keeps running a request after its client disconnects, for example when the browser that called your Worker closes the connection. This applies to RPC methods and to `fetch()`. The method runs to completion, including `finally` blocks, and its result is discarded. Workers Observability records the invocation with the outcome `canceled`.

To stop work when the client disconnects, turn on the [`enable_request_signal`](/workers/configuration/compatibility-flags/#enable-requestsignal-for-incoming-requests) compatibility flag and listen for `abort` on `request.signal` in `fetch()`. RPC methods do not receive a signal.

A Worker's `fetch()` handler and a `WorkerEntrypoint` RPC method stop when the client disconnects. Refer to [the RPC lifecycle](/workers/runtime-apis/rpc/lifecycle/#end-of-event-handler--execution-context).
