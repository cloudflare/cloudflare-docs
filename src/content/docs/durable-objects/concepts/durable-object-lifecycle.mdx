...
- No I/O operation or connection remains pending or active.
...
Pending or active I/O prevents a Durable Object from hibernating or being evicted. Examples include service binding requests, Durable Object remote procedure call (RPC) calls, outbound requests, [`this.ctx.container.monitor()`](/durable-objects/api/container/#monitor), TCP sockets, and outbound WebSockets.

An I/O operation prevents eviction until it completes or has run for 15 minutes, whichever comes first. Later operations can extend the Durable Object's total time in memory, but their limits do not automatically stack. There is no overall invocation wall-time limit.
