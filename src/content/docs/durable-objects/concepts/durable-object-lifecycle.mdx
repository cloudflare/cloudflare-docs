...
- No outbound request or `this.ctx.container.monitor()` promise is pending.
...
The following pending operations prevent the Durable Object from hibernating or being evicted:

- Outbound `fetch()` requests to external services
- Service binding fetches
- Requests to another Durable Object through remote procedure call (RPC) or `fetch()`
- [`this.ctx.container.monitor()`](/durable-objects/api/container/#monitor)
- Outbound TCP sockets created with `connect()`
- Outbound WebSockets

Each listed operation prevents eviction for up to 15 minutes. Starting another operation later can extend time in memory. There is no 15-minute limit for the entire invocation.
