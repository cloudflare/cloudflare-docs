...
- **WebSocket connections**: When an object restarts, all of its WebSocket connections close and clients must reconnect. This includes connections that were [hibernating](/durable-objects/best-practices/websockets/#durable-objects-hibernation-websocket-api). Evicting an idle object into hibernation does not close its connections.
- **Other invocations (email, cron)**: Other invocations are treated similarly to HTTP requests.

Closing WebSocket connections promptly allows every client to reconnect to the new instance together. If connections stayed open, some clients would remain attached to the previous instance while others connected to the new one.

Code that uses a [`DurableObjectStub`](/durable-objects/api/stub/) after a shutdown receives `Connection closed: this Durable Object instance is no longer active. Reconnect or retry the request.` A WebSocket closed by a shutdown reports `Durable Object connection closed because the object was reset.` Refer to [Troubleshooting](/durable-objects/observability/troubleshooting/) to handle both.
