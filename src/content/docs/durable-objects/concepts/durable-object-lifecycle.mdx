...
- **WebSocket connections**: Connections that use the Web Standard WebSocket API close, because they prevent the object from hibernating. Connections accepted with the [WebSocket Hibernation API](/durable-objects/best-practices/websockets/#durable-objects-hibernation-websocket-api) stay open when the object hibernates, but close if the instance stops before it hibernates.
