...
While pinned by an outbound connection, the Durable Object remains in memory in the **idle, in-memory, non-hibernateable** state and continues to [incur duration charges](/durable-objects/platform/pricing/#when-does-a-durable-object-incur-duration-charges).

Each outbound connection pins the Durable Object for a maximum of 15 minutes. After 15 minutes, the connection stops pinning the object (the connection itself continues operating), and normal eviction rules resume.
