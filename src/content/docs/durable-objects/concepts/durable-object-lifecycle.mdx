...
A Durable Object class can define a [`preShutdown()`](/durable-objects/api/base/#preshutdown) handler <Badge text="Beta" variant="caution" size="small" /> to run best-effort cleanup work, such as a final checkpoint to storage, before a shutdown. Cloudflare invokes `preShutdown()` for some, but not all, of the shutdown causes listed above:

| Shutdown cause | `preShutdown()` invoked |
| --- | --- |
| Lack of requests to an object (eviction due to inactivity) | Yes |
| New Worker deployment with a code update | Yes |
| Cloudflare updates to the Workers runtime system | No |
| Workers runtime decisions on where to host objects | No |

`preShutdown()` is not guaranteed to run, even for the causes marked "Yes" above — for example, the object may shut down unexpectedly before the handler completes. Design your Durable Object to write important state incrementally, and use `preShutdown()` only to reduce how often you need to checkpoint, not as a substitute for durable writes.
