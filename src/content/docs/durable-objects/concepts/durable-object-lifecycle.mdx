...
Durable Object instances occasionally shut down. When another request arrives, Cloudflare starts a new instance and runs your Durable Object class constructor.

An instance can shut down for several reasons:
...
When a Durable Object instance shuts down, Cloudflare routes new requests to a new instance. In-flight requests are handled as follows:
...
- **WebSocket connections**: All WebSocket connections to the instance close, including [hibernating connections](/durable-objects/best-practices/websockets/#durable-objects-hibernation-websocket-api). Clients must reconnect.
- **Other invocations (email, cron)**: Other invocations are treated similarly to HTTP requests.

Hibernation itself does not close WebSocket connections. Cloudflare does not transfer existing connections to a new instance.

For recovery guidance after an interrupted request or WebSocket connection, refer to [Troubleshooting](/durable-objects/observability/troubleshooting/).

Design services that use Durable Objects to handle instance shutdowns.
