...
- **WebSocket connections**: Standard WebSocket connections are terminated during shutdown. WebSockets accepted with the [WebSocket Hibernation API](/durable-objects/best-practices/websockets/) remain connected when an object is evicted due to inactivity.
...
A Durable Object can define a [`[shutdown]()`](/durable-objects/api/base/#shutdown) hook. Use it for best-effort clean shutdown logic and final checkpoints.
...
export class Counter extends DurableObject<Env> {
	async [shutdown](info) {
		...
		await this.ctx.storage.put("lastShutdown", Date.now());
	}
}
```
</TypeScriptExample>

`info` describes why shutdown is happening. `info.reason` is `"inactive"` for idle eviction or `"codeUpdated"` for a code update — handle unknown values, since more reasons may be added. Refer to [`[shutdown]()` parameters](/durable-objects/api/base/#shutdown) for the full reference.

The runtime invokes the hook for the shutdown causes described above:

| Shutdown cause                                              | `[shutdown]()` invoked |
| ------------------------------------------------------------ | ---------------------- |
| Lack of requests to an object (eviction due to inactivity)   | Yes                     |
| New Worker deployment with a code update                     | Yes                     |
| Workers runtime updates                                       | No                      |
| Workers runtime decisions on where to host objects            | No                      |

The hook also does not run for abnormal terminations, since these bypass the storage-healthy shutdown path required to safely run cleanup logic:

- `ctx.abort()`
- A broken input or output gate
- A CPU or memory limit termination
- Loss of connectivity to the object's host, or a crash

For runtime updates, in-flight requests retain the existing drain behavior described above, but `[shutdown]()` is not invoked.

`[shutdown]()` is not guaranteed to run. Persist important changes incrementally. SQLite storage operations such as `sql.exec()` are synchronous, while key-value storage operations are strongly consistent and complete asynchronously. Use the hook for best-effort clean shutdown work that differs from regular request handling.

### Shutdown behavior

- Once the hook starts, the shutdown is committed. The instance receives no new events. A racing request waits for teardown and any pending storage writes to complete, then reaches the new instance.
- During code updates, the hook starts immediately and may run concurrently with in-flight requests already being processed; those requests continue until the hook's returned promise resolves. Account for concurrent changes when checkpointing state.
- The hook has up to 10 seconds to run. Whether it finishes, throws an error, or times out, any storage writes it issued are still allowed to complete, and the runtime does not retry the hook.
- The hook runs before hibernatable WebSockets are hibernated. This ordering permits a final `ws.send()` call. During Beta, [Durable Object Facets](/dynamic-workers/usage/durable-object-facets/) are not supported.
- The symbol is not exposed on a Durable Object stub, so callers cannot invoke it over RPC. For the complete contract, refer to [`[shutdown]()`](/durable-objects/api/base/#shutdown).
- When [Workers tracing](/workers/observability/traces/) is enabled, the shutdown invocation produces a span with a `cloudflare.handler_type` value of `shutdown`.

For an example that combines regular checkpoints with a final shutdown checkpoint, refer to [Design for unexpected shutdowns](/durable-objects/best-practices/rules-of-durable-objects/#design-for-unexpected-shutdowns).
