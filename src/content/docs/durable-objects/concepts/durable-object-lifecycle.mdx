...
- No pending outbound I/O exists, since the Durable Object must remain available to process its completion.
...
:::note[Pending I/O keeps Durable Objects alive]
Pending outbound I/O prevents the Durable Object from hibernating or being evicted. This allows asynchronous work to finish after the caller disconnects.

This behavior covers outbound fetches, including fetches to service bindings. It also covers requests to other Durable Objects through remote procedure call (RPC) or `fetch()`, [`this.ctx.container.monitor()`](/durable-objects/api/container/#monitor), outbound TCP sockets, and outbound WebSockets.

Each pending I/O event prevents eviction for up to 15 minutes. New eligible I/O can reset or extend this window, so the invocation has no fixed 15-minute limit.

While kept alive, the Durable Object remains in the **idle, in-memory, non-hibernateable** state and continues to [incur duration charges](/durable-objects/platform/pricing/#when-does-a-durable-object-incur-duration-charges). When no I/O keeps the object alive, the [standard eviction rules](#durable-object-lifecycle-state-transitions) resume.
:::
