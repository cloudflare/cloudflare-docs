...
The runtime invokes the hook for these shutdown causes:
...
| Runtime process reload or drain                            | No                     |
| `ctx.abort()`                                              | No                     |
| Broken input or output gate                                | No                     |
| CPU or memory limit termination                            | No                     |
| Supervisor session loss or crash                           | No                     |

For runtime process reloads and drains, in-flight requests retain the existing drain behavior described above, but `[shutdown]()` is not invoked.
...
Once the hook starts, the shutdown is committed. The instance receives no new events. A racing request waits for teardown and storage flush, then reaches the successor instance.

During code updates, existing in-flight events may continue while the hook awaits. Account for concurrent changes when checkpointing state.

The runtime awaits the hook for up to 10 seconds. It drains issued storage writes after resolution, rejection, or timeout, then continues shutdown without retrying the hook.

The hook runs before hibernatable WebSockets are hibernated. This ordering permits a final `ws.send()` call. During Beta, the hook runs on root actors, but not actor facets.
