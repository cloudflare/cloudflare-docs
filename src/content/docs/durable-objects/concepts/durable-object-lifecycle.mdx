...
:::note[Profiling keeps Durable Objects from being evicted]
An active profiling session on a Durable Object prevents it from being evicted. While the session is running, the Durable Object cannot transition to the **inactive** state.

Profiling does not prevent hibernation. A Durable Object that meets the hibernation criteria can still transition to the **hibernated** state during a profiling session.
:::
