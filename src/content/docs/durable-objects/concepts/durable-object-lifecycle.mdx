...
- Worker code updates that use an immediate update or reach their hibernation timeout
...
- **WebSocket connections**: WebSocket connections attached to the running instance are terminated automatically during shutdown. This is so that the new instance can take over the connection as soon as possible.
...
If an object does not hibernate before the update timeout, the runtime shuts down the running instance and applies the update. This forced update can interrupt work and disconnect WebSockets as described in [Shutdown behavior](#shutdown-behavior).

{/* TODO: Confirm the compatibility date, timeout defaults and range, and Wrangler option before publication. */}

:::note
Delayed code updates are the default for Workers with a compatibility date of `COMPATIBILITY_DATE` or later. Immediate code updates are the default for Workers with an earlier compatibility date.
:::

By default, Cloudflare waits up to one hour for an object to hibernate. You can set a timeout from `0s` to `24h` for each deployment. Shorter timeouts reduce how long old and new code coexist, but increase the chance of interrupting active work. A timeout of `0s` applies the update immediately.
...
Keep APIs between your Workers and Durable Objects forward and backward compatible while active objects adopt the update. Refer to [Code updates](/durable-objects/platform/known-issues/#code-updates) for more information.
