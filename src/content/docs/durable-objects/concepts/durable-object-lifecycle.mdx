...
- Worker code updates that use a `0` grace period or reach the end of their grace period
...
A deployment grace period lets an active Durable Object keep serving requests, storage operations, and Hibernation API WebSockets on its current code until it hibernates, instead of shutting it down immediately. If the object is still active when the grace period ends, Cloudflare shuts it down and applies the update.

Refer to [Delay code updates until hibernation](/durable-objects/reference/durable-objects-code-updates/) to configure a grace period and understand how it interacts with gradual deployments.
