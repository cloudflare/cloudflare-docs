...
| Mode        | Behavior                                                                                                                                          |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deferred`  | Cloudflare waits for an active Durable Object to hibernate before applying the update. If the object does not hibernate within `max_delay`, Cloudflare resets it and applies the update. |
| `immediate` | Cloudflare resets an active Durable Object and applies the update right away, without waiting for it to hibernate.                                |
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| *(omitted)*                                       | Uses the default for your compatibility date: `immediate` before `COMPATIBILITY_DATE`, `deferred` with a 30-second `max_delay` on or after it. |
...
- Workers with a compatibility date before `COMPATIBILITY_DATE` default to `immediate`, unless you set `code_update_strategy` yourself.
- Workers with a compatibility date of `COMPATIBILITY_DATE` or later default to `deferred` with a 30-second `max_delay`, unless you set a different `code_update_strategy`.
...
During a [gradual deployment](/workers/versions-and-deployments/gradual-deployments/with-durable-objects/), Cloudflare assigns each Durable Object to one of the versions in the deployment. Increasing a version's traffic percentage assigns more objects to that version. Only the newly assigned objects begin waiting under a `deferred` code update strategy — for example, increasing a version's traffic percentage from 20% to 50% starts the wait only for the additional 30% of objects newly assigned to it. Objects that already run that version, and objects still assigned to an earlier version, are not affected.

```mermaid
flowchart LR
    A["Deployment progression increases<br/>version B's traffic percentage"] --> B{"Was this Durable Object<br/>already running version B?"}
    B -->|Yes| C["No change — object keeps running version B"]
    B -->|"No, newly assigned to B"| D{"Is the object<br/>currently active?"}
    D -->|Inactive| E["Adopts version B on its next request"]
    D -->|"Active, mode is deferred"| F["Starts its own max_delay timer,<br/>keeps running its previous version"]
    F --> G{"Hibernates before<br/>max_delay ends?"}
    G -->|Yes| H["Adopts version B when it wakes up"]
    G -->|"No, max_delay reached"| I["Cloudflare resets it and<br/>applies version B"]
```
