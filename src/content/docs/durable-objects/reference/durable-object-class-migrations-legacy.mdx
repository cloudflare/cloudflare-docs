...
You cannot use both `exports` and `migrations` in the same Worker configuration — they are mutually exclusive. To move an existing Worker from `migrations` to `exports`, see [Migrate from `migrations` to `exports`](/durable-objects/reference/durable-objects-migrations/#migrate-from-the-legacy-migrations-flow).
