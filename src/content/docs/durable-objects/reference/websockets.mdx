...
A Durable Object with WebSockets created via the Hibernation API will not incur billable [Duration (GB-s) charges](/durable-objects/platform/pricing/) during periods of inactivity, unlike Durable Objects using the [regular WebSockets API](/workers/runtime-apis/websockets/). Note that other events, for example [alarms](/durable-objects/api/alarms/), can prevent a Durable Object instance from being inactive and therefore prevent this cost savings.
