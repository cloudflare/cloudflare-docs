...
- **`wrangler versions upload` does not apply lifecycle changes.** Just like the legacy `migrations` array, Durable Object lifecycle changes can only be applied via `wrangler deploy`. If your Wrangler configuration contains `exports` entries, `wrangler versions upload` fails fast with an actionable error. Refer to [Versions and Deployments — Durable Object migrations](/workers/configuration/versions-and-deployments/#durable-object-migrations).
- **Gradual deployments are not supported with `exports`.** Lifecycle changes are atomic at the Cloudflare control plane and cannot be rolled out gradually. Refer to [Gradual deployments — Migrations](/workers/configuration/versions-and-deployments/gradual-deployments/#migrations).
- **Rollbacks cannot cross a lifecycle change.** You cannot roll back to a version deployed before an `exports`-driven lifecycle change. Refer to [Rollbacks — Bindings](/workers/configuration/versions-and-deployments/rollbacks/#bindings).
