...
### Enable SQL storage on new Durable Object class migration

:::note[SQLite in Durable Objects Beta]

New beta version of Durable Objects is available where each Durable Object has a private, embedded SQLite database. When deploying a new Durable Object class, users can opt-in to using SQL storage; otherwise, a Durable Object class has the standard, private key-value storage. Objects with SQL storage can use the [SQL API methods](/durable-objects/api/storage-api/#sqlexec). 

:::

To allow a new Durable Object class to use SQL storage and SQL API, use `new_sqlite_classes` on the migration in your Worker's `wrangler.toml` file:
...
You cannot enable SQL storage and SQL API on an existing, deployed Durable Object class, so setting `new_sqlite_classes` on later migrations will fail with an error. Automatic migration of deployed classes to allow SQL access will be available in the future.
