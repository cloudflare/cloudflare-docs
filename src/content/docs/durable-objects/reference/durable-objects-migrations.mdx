...
You cannot enable SQL storage and API on an existing, deployed Durable Object class. Therefore, setting `new_sqlite_classes` on later migrations will fail with an error. Automatic migration of deployed classes to allow SQL access will be available in the future.
