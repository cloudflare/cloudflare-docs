...
The `exports` field replaces the imperative [`migrations` array](/durable-objects/reference/durable-object-class-migrations-legacy/) used by older Workers. Both flows are supported, but a Worker can only use one at a time. To move an existing Worker from `migrations` to `exports`, see [Migrate from the legacy `migrations` flow](#migrate-from-the-legacy-migrations-flow).
...
The deploy succeeds when all three agree. Any disagreement (a class in code that has no entry, an entry for a class that no longer exists in code, a provisioned namespace with no matching entry) is surfaced as a structured error or, where the intent is unambiguous, an action that Cloudflare applies for you.
...
`storage` is required for live entries. New Durable Object namespaces created through `exports` always use the SQLite storage backend. The `legacy-kv` value is only accepted for namespaces that were already provisioned with key-value storage; see [Storage backends](#storage-backends).
...
After every deploy, `wrangler deploy` prints a `Durable Object exports reconciliation` block summarising the actions Cloudflare applied and any advisory information. A typical output looks like:
...
All class-level errors from a single deploy are reported together so you can fix them in one round trip. See the [error reference](#error-reference) for the full list of error scenarios.
...
[Preview deployments](/workers/configuration/previews/) and [dispatch namespaces](/cloudflare-for-platforms/workers-for-platforms/) follow the same rules. Cross-dispatch-namespace transfers are not supported — the source and target Workers in a `transferred` / `expecting-transfer` pair must both live in the same dispatch-namespace context (or both be outside any dispatch namespace).
...
- `type` <Type text="string" /> <MetaInfo text="required" />
  - The kind of export. Currently always `"durable-object"`.
...
Allowed and forbidden fields for each `state`:
...
| ------------------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `provisioned_class_missing_from_config`           | A namespace exists for a class but `exports` has no entry for it.                        | Add a live entry (`state: "created"`) to keep the namespace, or add a tombstone (`deleted`, `renamed`, or `transferred`) to retire it.       |
...
| `tombstone_pending_transfer_not_found`            | A `transferred` tombstone has no matching `expecting-transfer` entry on the target.      | Deploy the target Worker first with an `expecting-transfer` entry naming this Worker.                                                        |
| `tombstone_transfer_target_missing`               | The target Worker named by `transferred_to` no longer exists.                            | Update `transferred_to` to a valid target, or remove the tombstone.                                                                          |
...
| `phase_one_transfer_namespace_missing`            | The source Worker has no namespace for the class.                                        | Make sure the source Worker has the class deployed before the target declares `expecting-transfer`.                                          |
...
- **`wrangler versions upload` does not apply lifecycle changes.** Just like the legacy `migrations` array, Durable Object lifecycle changes can only be applied via `wrangler deploy`. If your Wrangler configuration contains `exports` entries, `wrangler versions upload` fails fast with an actionable error. See [Versions and Deployments — Durable Object migrations](/workers/configuration/versions-and-deployments/#durable-object-migrations).
- **Gradual deployments are not supported with `exports`.** Lifecycle changes are atomic at the Cloudflare control plane and cannot be rolled out gradually. See [Gradual deployments — Migrations](/workers/configuration/versions-and-deployments/gradual-deployments/#migrations).
- **Rollbacks cannot cross a lifecycle change.** You cannot roll back to a version deployed before an `exports`-driven lifecycle change. See [Rollbacks — Bindings](/workers/configuration/versions-and-deployments/rollbacks/#bindings).
