...
You cannot enable SQL storage and API on an existing, deployed Durable Object class, so setting `new_sqlite_classes` on later migrations will fail with an error.
