...
- An active outbound connection (via [`connect()`](/workers/runtime-apis/tcp-sockets/) or an outbound WebSocket) keeps a Durable Object in memory — and therefore incurring duration charges — even with no incoming requests. Each connection pins the Durable Object for up to 15 minutes; after 15 minutes, the connection stops pinning the object (the connection itself continues operating) and normal eviction rules resume. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for details.
