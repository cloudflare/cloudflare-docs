...
- Pending outbound `fetch()` requests to external services, service binding fetches, requests to another Durable Object through [RPC](/workers/runtime-apis/rpc/) or `fetch()`, `this.ctx.container.monitor()`, outbound TCP sockets, and outbound WebSockets can keep an object in memory. Each operation can keep the object in memory for up to 15 minutes. The object incurs duration charges while an operation prevents eviction. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for more information.
