...
With a [deployment grace period](/durable-objects/reference/durable-objects-code-updates/), an active Durable Object can continue running a previous version until it hibernates or the grace period ends. Gradual deployments can extend this period while your deployment uses more than one version. If you deploy multiple versions before an object hibernates, it might skip intermediate versions and use the latest version when it wakes.
