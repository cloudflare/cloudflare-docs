...
New beta version of Durable Objects is available where each Durable Object has a private, embedded SQLite database. When creating a Durabe Object class, users can [opt-in to using SQL storage](/durable-objects/best-practices/access-durable-objects-storage/#sql-storage) in order to access Storage [SQL API](/durable-objects/api/storage-api/#sqlexec).
...
Learn how Durable Objects provide transactional, strongly consistent, and serializable storage, either key-value storage or (beta) SQL storage.
