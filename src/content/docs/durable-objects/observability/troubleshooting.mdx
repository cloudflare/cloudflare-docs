...
This error means the connection between your Worker and the Durable Object was interrupted by the platform. It is unrelated to your own Worker code or deployments, and it has its `.retryable` property set to `true`.
...
This error means the Durable Object's connection was interrupted by the platform while a WebSocket was open. The WebSocket closes, and the client must reconnect.
...
Keep any state that must survive a reconnect in [storage](/durable-objects/best-practices/access-durable-objects-storage/) rather than in memory.
