...
The Durable Object instance that received your request was shut down. Subsequent requests are routed to a new instance automatically. Refer to [Shutdown behavior](/durable-objects/concepts/durable-object-lifecycle/#shutdown-behavior) for the reasons an instance shuts down.
...
To recover, create a new [`DurableObjectStub`](/durable-objects/api/stub/) and send the request again, if the request is idempotent. Reusing the stub that threw this error fails immediately. Refer to [Error handling](/durable-objects/best-practices/error-handling/) for guidance on handling exceptions and applying backoff.

### Durable Object connection closed because the object was reset.

The Durable Object instance a WebSocket was connected to was shut down, so the connection closed. This includes connections that were [hibernating](/durable-objects/best-practices/websockets/#durable-objects-hibernation-websocket-api). Refer to [Shutdown behavior](/durable-objects/concepts/durable-object-lifecycle/#shutdown-behavior) for the reasons an instance shuts down.

Build WebSocket clients to reconnect when a connection closes. Reconnecting routes the client to the new instance. Keep any state that must survive a reconnect in [storage](/durable-objects/best-practices/access-durable-objects-storage/) rather than in memory.
