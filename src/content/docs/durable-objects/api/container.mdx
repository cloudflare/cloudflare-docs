...
`interceptOutboundTcp` routes outbound TCP connections matching a host:port, IP address, or IP:port through a `WorkerEntrypoint`. The Worker's `connect()` handler receives the raw TCP stream. Can be called before or after starting the container. Open connections pick up the new handler without being dropped.
...
// Match an IP:port
this.ctx.container.interceptOutboundTcp("15.0.0.1:80", worker);
```

#### Parameters

- `target` (string): A host:port, IP address, or IP:port to match.
- `worker` (WorkerEntrypoint): A `WorkerEntrypoint` instance whose `connect()` handler will process matching TCP streams.
