...
`interceptOutboundTcp` routes outbound TCP connections matching a host:port, IP address, or IP:port through a `WorkerEntrypoint`. The Worker's `connect()` handler receives the raw TCP stream. Can be called before or after starting the container. Any existing open connections will pick up the new handler without being dropped.
...
The following example routes a container's database connection through [Hyperdrive](/hyperdrive/), which handles connection pooling and query caching:
...
`target` (string) is a host:port, IP address, or IP:port to match.
`worker` (WorkerEntrypoint) is a `WorkerEntrypoint` instance whose `connect()` handler will process matching TCP streams.
