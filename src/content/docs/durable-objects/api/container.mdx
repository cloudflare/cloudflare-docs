...
While `monitor()` remains pending, it prevents eviction for up to 15 minutes. Starting another operation that prevents eviction can extend the object's [time in memory](/durable-objects/concepts/durable-object-lifecycle/).
