...
`setInactivityTimeout` sets how long a running container can stay idle before Cloudflare stops it. A container is idle when it has no open connections or other activity. Cloudflare does not stop a container for inactivity while a connection to it is open.

Without an inactivity timeout, Cloudflare decides when to stop a container that has no open connections. With a timeout, the container can keep running after the Durable Object that started it becomes inactive. This lets a long-running server, such as a development preview, handle later requests.

Call `setInactivityTimeout` after each call to `start()`.
...
// Stop the container after 5 minutes without activity.
...
- `durationMs` (`number | bigint`): idle time in milliseconds before Cloudflare stops the container. The value must be greater than `0`.
