...
To use the WebGPU API in local development, enable the `experimental` and `webgpu` [compatibility flags](/workers/configuration/compatibility-dates/#compatibility-flags) in the [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/) of your Durable Object.
