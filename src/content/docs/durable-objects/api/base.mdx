...
- <code>
  ...
  with a binding to that object is the client.
...
- <code>
  ...
  when a scheduled alarm time is reached.
...
- <code>
  ...
  : <Type text="void" /> | <Type text="Promise<void>" />- Called by the system
  when an accepted WebSocket receives a message.
...
- <code>
  ...
  : <Type text="void" /> | <Type text="Promise<void>" />- Called by the system
  when a non-disconnection error occurs on a WebSocket connection.
  - This method can be `async`.
