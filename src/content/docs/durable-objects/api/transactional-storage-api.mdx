...
The Transactional Storage API allows Durable Objects to access consistent key-value storage or (beta) [SQL storage](/durable-objects/api/transactional-storage-api/#sqlexec). A Durable Object's attached storage is private to its unique instance and cannot be accessed by other objects. 

Durable Objects gain access to a persistent Transactional Storage API via `ctx.storage`, on the `ctx` parameter passed to the Durable Object constructor.
...
New beta version of Durable Objects is available where each Durable Object has a private, embedded SQLite database. SQL storage is [opt-in during beta](/durable-objects/reference/durable-objects-migrations/#enable-sql-storage-on-create-durable-object-class-migration) when creating a Durable Object class; otherwise, a Durable Object class has the standard, private key-value storage. Objects with SQL storage can use the [SQL API](/durable-objects/api/transactional-storage-api/#sqlexec). 

All Transactional Storage API methods work with SQL storage. Key-value methods like `get()`, `put()`, `delete()`, or `list()` store data in a hidden SQLite table. 
...
#### Returns 
A cursor (`SqlStorageCursor`) to iterate over query row results as objects with the following supported methods:
  
* `raw()`: Iterator
  * Returns cursor of query rows as arrays of row values.
* `columnNames()`: string[]
  * Returns an array of column name strings. Often used with `raw()` to find index of specific column(s).
*  `rowsRead()`: number
  * Returns the number of rows read so far as part of this SQL query. This may increase as you iterate the cursor. The final value is used for [SQL billing](TODO link).
*  `rowsWritten()`: number
  * Returns the number of rows written so far as part of this SQL query. This may increase as you iterate the cursor. The final value is used for [SQL billing](TODO link).
...
<code>ctx.storage.sql.prepare(query: string)</code> : SqlStorageStatement

#### Parameters
* `query`: string 
  * The prepared SQL query string to be executed. `query` can contain `?` placeholders for parameter bindings.

#### Returns 
A cursor to iterate over query row results as objects with the [same supported methods as `sql.exec`](/durable-objects/api/transactional-storage-api/#returns)

#### Examples

```ts
  let stmt = this.sql.prepare("SELECT * FROM artist WHERE artistid = ?;");
  let result = stmt(123);
  console.log([...result.raw()]); // prints [[123, 'Alice' ]]
```
...
For [Durable Objects classes with SQL storage](/durable-objects/reference/durable-objects-migrations/#enable-sql-storage-on-create-durable-object-class-migration), the following Storage API methods are available to restore a Durable Object's embedded SQLite database to any point in time in the past 30 days.
...
```ts
  let now = new Date();
  // restore to 2 days from current date and time
