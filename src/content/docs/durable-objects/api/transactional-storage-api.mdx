...
*  `rowsRead()`: number
  * Returns number of rows read by the executed SQL `query` based on cursor consumption. Metric is the same one used for [SQL billing](TODO link).
*  `rowsWritten()`: number
  * Returns number of rows written by the executed SQL `query` based on curosr consumption. Metric is the same one used for [SQL billing](TODO link).
