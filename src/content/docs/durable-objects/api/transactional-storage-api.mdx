...
The Transactional Storage API allows you to access consistent key-value storage or (beta) SQL storage.

Durable Objects gain access to a persistent Transactional Storage API via `ctx.storage`, the first parameter passed to the Durable Object constructor.
...
SQL API methods are only allowed on [Durable Object classes with SQL storage](/durable-objects/reference/durable-objects-migrations/#enable-sql-storage-on-create-durable-object-class-migration).
...
*  `rowsRead()`: number
  * Returns the number of rows read by the executed SQL `query`. Metric is the same one used for [SQL billing](TODO link).
*  `rowsWritten()`: number
  * Returns the number of rows written by the executed SQL `query`. Metric is the same one used for [SQL billing](TODO link).

#### Examples

SQL examples below use the following SQL schema:
...
```ts
  const cursor = this.sql.exec("SELECT * FROM artist;");
...
```ts
  ...
  const resultsArray = [...this.sql.exec("SELECT * FROM artist;")];
  // OR
  const resultsArray2 = Array.from(this.sql.exec("SELECT * FROM artist;"));
  // OR
  const resultsArray3 = this.sql.exec("SELECT * FROM artist;").toArray();
```

Convert query results to an array of row values arrays:
```ts
  // Returns [[123,"Alice"],[456,"Bob"],[789,"Charlie"]]
  const resultsArray = [...this.sql.exec("SELECT * FROM artist;").raw()];	

  // Returns ["artistid","artistname"]
  const columnNameArray = Array.from(this.sql.exec("SELECT * FROM artist;").columnNames);
```
...
```ts
  ...
  // returns { artistid: 123, artistname: 'Alice' }
  const oneRow = this.sql.exec("SELECT * FROM artist WHERE artistname = ?;", "Alice").one()
```

Cursor behavior:
```ts
  const cursor = this.sql.exec("SELECT * FROM artist ORDER BY artistname ASC;");
  let { value: firstRow } = cursor[Symbol.iterator]().next()
  console.log(firstRow); // prints { artistid: 123, artistname: 'Alice' }

  const remainingRows = Array.from(cursor); // TODO: will be cursor.toArray
  console.log(remainingRows); // prints [{ artistid: 456, artistname: 'Bob' },{ artistid: 789, artistname: 'Charlie' }]
```

```ts
  const cursor = this.sql.exec("SELECT * FROM artist;");
  const { value: firstRow } = cursor[Symbol.iterator]().next()
