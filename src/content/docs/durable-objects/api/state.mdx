...
`onNextSessionRestore` only schedules the restore, it does not perform it. The following call to `abort` always throws, including when the restore succeeds, because `abort` immediately terminates the current session. Catch the specific message passed to `abort` to tell a successful restore apart from an unrelated failure.
