...
:::note[`waitUntil` can prevent eviction]

A promise passed to `waitUntil` can prevent a Durable Object from being evicted for up to 15 minutes while the promise remains incomplete. It does not affect when a request or RPC completes.
:::
