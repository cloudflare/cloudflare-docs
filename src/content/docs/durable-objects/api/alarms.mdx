...
How are alarms different from Cron Triggers?. Alarms are more fine grained than [Cron Triggers](/workers/configuration/cron-triggers/#cron-triggers). A Worker can have up to three Cron Triggers configured at once, but it can have an unlimited amount of Durable Objects, each of which can have an alarm set.
...
* <code>getAlarm()</code> : number | null
...
* <code>setAlarm(scheduledTimeMsnumber)</code> : void
