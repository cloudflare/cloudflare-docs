...
<Tabs>
	<TabItem label="npm">
		```sh npm i -D vitest@^4.1.0 @cloudflare/vitest-pool-workers ```
	</TabItem>
	<TabItem label="pnpm">
		```sh pnpm add -D vitest@^4.1.0 @cloudflare/vitest-pool-workers ```
	</TabItem>
	<TabItem label="yarn">
		```sh yarn add -D vitest@^4.1.0 @cloudflare/vitest-pool-workers ```
	</TabItem>
</Tabs>
...
Use the `exports` binding to test your Worker's HTTP handler, which routes requests to Durable Objects:
