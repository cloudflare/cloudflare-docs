...
By default, Durable Objects delay applying code updates so in-flight requests, WebSocket connections, and storage operations can finish. Instead of resetting an active Durable Object immediately, Cloudflare applies the update once the object hibernates.

Set a Durable Object's code update strategy to `immediate` to reset it and apply the update right away instead of waiting. Refer to [Lifecycle of a Durable Object](/durable-objects/concepts/durable-object-lifecycle/) for more on when an object hibernates.
...
3. If a Durable Object is active — performing work or holding open connections — with a `deferred` code update strategy, it keeps running its existing version, handling requests, events, storage operations, and Hibernation API WebSocket messages, until it hibernates. Once it hibernates before `max_delay` is reached, Cloudflare applies the new version the next time it "wakes up." Hibernated WebSocket connections stay connected throughout.
...
flowchart TD
    ...
    D -->|immediate| E["Cloudflare resets it and<br/>applies the update"]
    D -->|deferred| F["Starts its max_delay timer,<br/>keeps running its current version"]
    F --> G{"Hibernates before<br/>max_delay ends?"}
    G -->|Yes| H["Adopts the new version<br/>on next request"]
