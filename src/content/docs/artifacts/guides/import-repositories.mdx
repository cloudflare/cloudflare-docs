...
Use a gateway JWT that your Artifacts gateway accepts for `/v1/api/...` requests.
...
The response includes the new Artifacts repo metadata, including `result.remote`, `result.token`, and `result.token_expires_at`.
