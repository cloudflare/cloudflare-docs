...
Start by setting up your namespace. For more information, refer to [Namespaces](/artifacts/guides/namespaces/). This guide uses `default` in the examples.
...
- A gateway JWT that your Artifacts gateway accepts for `/v1/api/...` requests.
- A local `git` client.
...
Artifacts uses Bearer authentication for `/v1/api/...` requests:
