...
- Wrangler installed and already authenticated. For local OAuth authentication or CI setup, refer to [`wrangler login`](/workers/wrangler/commands/general/#login) and [Running Wrangler in CI/CD](/workers/ci-cd/).
...
Use the exact `remote` value from the response. In that value, `{account_id}` is your Cloudflare account ID.
