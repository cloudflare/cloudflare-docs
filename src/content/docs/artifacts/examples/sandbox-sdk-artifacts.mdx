...
```ts
const repoResult = await env.ARTIFACTS.get(sandboxId);
...
if (repoResult.status === "ready") {
	const info = await repoResult.repo.info();
	if (!info) {
	...
	remote = info.remote;
	token = (await repoResult.repo.createToken("write", 3600)).plaintext;
} else if (repoResult.status === "not_found") {
	...
	throw new Error(
		`Repo is ${repoResult.status}. Retry after ${repoResult.retryAfter}s.`,
	);
}
```
