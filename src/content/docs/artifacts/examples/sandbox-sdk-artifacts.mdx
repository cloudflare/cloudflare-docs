...
try {
	...
	try {
		...
		throw new Error(`Repo ${sandboxId} is not ready yet. Retry shortly.`);
	}
}
```
...
The template already knows the repo name, so start with `get(sandboxId)` instead of scanning `list()` pages. If `get()` or `create()` fails because the repo is still provisioning, retry after a short delay.
