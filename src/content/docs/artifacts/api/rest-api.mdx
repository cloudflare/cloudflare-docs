...
Set up your namespace first. For more information, refer to [Namespaces](/artifacts/guides/namespaces/).
...
For edge-mediated routing, use:

```txt
https://artifacts.cloudflare.net/edge/v1/api/namespaces/$ARTIFACTS_NAMESPACE
```

Requests to `/v1/api/...` use a gateway JWT in the `Authorization` header:

```txt
Authorization: Bearer $ARTIFACTS_JWT
```

Requests to `/edge/v1/api/...` use edge-api gateway auth.

All routes below are relative to the `/v1/api` base URL.
...
```json
{
	"result": {
		...
		"description": "Repository for automation experiments",
		"default_branch": "main",
		"remote": "https://<ACCOUNT>.artifacts.cloudflare.net/git/default/starter-repo.git",
...
```json
{
	"result": {
		...
		"description": "Repository for automation experiments",
		"default_branch": "main",
		"remote": "https://<ACCOUNT>.artifacts.cloudflare.net/git/default/starter-repo-copy.git",
...
```json
{
	"result": {
		...
		"description": null,
		"default_branch": "main",
		"remote": "https://<ACCOUNT>.artifacts.cloudflare.net/git/default/react-mirror.git",
