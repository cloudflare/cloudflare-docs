...
export interface Env {
	ARTIFACTS: Service<ArtifactsBinding>;
}
```
...
export interface ArtifactsRepoHandle {
	info(): Promise<RemoteRepoInfo | null>;
	createToken(scope?: Scope, ttl?: number): Promise<CreateTokenResponse>;
	validateToken(token: ArtifactToken): Promise<ValidateTokenResponse>;
	...
	fork(target: {
		...
		readOnly?: boolean;
	}): Promise<CreateRepoResponse & { repo: ArtifactsRepoHandle }>;
}
...
export interface ArtifactsBinding {
	...
	get(name: RepoName): Promise<ArtifactsRepoHandle | null>;

	list(opts?: { limit?: number; cursor?: Cursor }): Promise<ListReposResponse>;
...
async function createRepo(artifacts: Service<ArtifactsBinding>) {
...
async function getRepoHandle(artifacts: Service<ArtifactsBinding>) {
...
async function listRepos(artifacts: Service<ArtifactsBinding>) {
...
async function deleteRepo(artifacts: Service<ArtifactsBinding>) {
...
async function importRepo(artifacts: Service<ArtifactsBinding>) {
...
async function getRemoteUrl(artifacts: Service<ArtifactsBinding>) {
	const repo = await artifacts.get("starter-repo");
	if (!repo) {
		return null;
	}
...
async function mintReadToken(artifacts: Service<ArtifactsBinding>) {
	const repo = await artifacts.get("starter-repo");
	if (!repo) {
...
async function listRepoTokens(artifacts: Service<ArtifactsBinding>) {
	const repo = await artifacts.get("starter-repo");
	if (!repo) {
		throw new Error("Repo not found");
	}
...
async function forkRepo(artifacts: Service<ArtifactsBinding>) {
	const repo = await artifacts.get("starter-repo");
	if (!repo) {
		throw new Error("Repo not found");
	}
...
<LinkCard
	...
	description="Use repo remotes and tokens with external git-compatible clients."
/>
