...
The example follows the flow below. It only exercises the Origin, Client, and Issuer — the Attester is stubbed, since no real attestation runs, so it is left out of the diagram:
