...
1. The issuance protocol begins with the Client completing a deployment-specific **attestation process**. This step is intentionally open-ended so different use cases can define their own attestation, which is where most use cases are defined.
2. Once the Attester verifies the Client, the Client sends a **blinded token request** to the Issuer, along with proof of attestation.
