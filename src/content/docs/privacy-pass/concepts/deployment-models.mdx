...
:::note
Privacy Pass provides unlinkability between a token issuance and redemption. This unlinkability relies only partially on the separation of roles. The anonymity set, which can be impacted by network-level identifiers (visible IP address, TLS fingerprint, HTTP headers) and partitionable token-bound information (excessive public key usage or token metadata), is also key in protecting a Client's unlinkability. Customers are trusted to follow Privacy Pass [guidelines](https://datatracker.ietf.org/doc/html/rfc9576#name-privacy-considerations) to mitigate such threats.
:::
...
This deployment uses a **joint Attester-Issuer model**, where Cloudflare operates both the Issuer and Attester. If the customer is acting as the Origin, it is essential that they do not operate the Attester, as the Origin and Attester should generally not be operated by the same entity. Having Cloudflare operate it jointly with the Issuer prevents having to introduce an entirely new third party, although is not necessary. The joint Attester-Issuer model is also useful if the roles are reversed, such that the customer is an identity authority (e.g., an enterprise IdP/SSO provider) who wants to offer private verification for their Origins.
...
- [cloudflare/privacypass-issuer](https://github.com/cloudflare/privacypass-issuer) — TypeScript issuer implementation.
