...
1. **You are running the latest version of `cloudflared`**. Many issues are resolved by upgrading. Refer to [Update cloudflared](/tunnel/downloads/update-cloudflared/) for instructions.
2. **Required ports and IPs are allowed**. Your firewall must permit egress to the [Cloudflare Tunnel required destinations](/tunnel/configuration/#firewall-rules) on port `7844` (TCP/UDP). Port `443` destinations (including `api.cloudflare.com`) are optional and only needed for features such as software update checks and Access JWT validation.
...
Attach logs when opening a support ticket or investigating an issue.

- **Locally-managed tunnels**: Run `cloudflared` with the `--loglevel debug` flag to capture verbose output:
...
When [contacting support](/support/contacting-cloudflare-support/), include the following:
...
- Debug log output or `cloudflared.log` file
