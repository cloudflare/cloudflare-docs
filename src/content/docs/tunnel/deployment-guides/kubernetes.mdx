...
As shown in the diagram, we recommend setting up `cloudflared` as an adjacent [deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) to the application deployments. Having a separate Kubernetes deployment for `cloudflared` allows you to scale `cloudflared` independently of the application. In the `cloudflared` deployment, you can spin up <a href="/tunnel/configuration/#replicas-and-high-availability">multiple replicas</a> running the same Cloudflare Tunnel — there is no need to build a dedicated tunnel for each `cloudflared` pod. Each `cloudflared` replica / pod can reach all Kubernetes services in the cluster.
...
You should see two `cloudflared` pods and two `httpbin` pods with a `Running` status. If your `cloudflared` pods keep restarting, check the `command` syntax in `tunnel.yaml` and make sure that the <a href="/tunnel/configuration/#run-parameters">tunnel run parameters</a> are in the correct order.
...
You can optionally add <a href="/tunnel/integrations/#cloudflare-access">Cloudflare Access</a> to control who can access the service.
