...
The UI Kit includes a full chat panel and lower-level chat components. Use the full panel when you want RealtimeKit to manage chat behavior for you.

Use lower-level components when you need to control the layout around message lists, pinned messages, or the composer.

<RTKCodeSnippet id="web-web-components">

Use `rtk-chat` to render the full chat panel. It includes public chat, private chat, paginated messages, pinned messages, file uploads, image uploads, edit actions, delete actions, and emoji input.

```html
<rtk-ui-provider id="rtk-ui-provider">
	<rtk-chat-toggle></rtk-chat-toggle>
	<rtk-chat style="height: 480px;"></rtk-chat>
</rtk-ui-provider>
```

To set chat UI overrides, assign the `overrides` property on `rtk-ui-provider` or `rtk-meeting`.

```js
const provider = document.querySelector("rtk-ui-provider");
...
The `disableEmojiPicker` override hides the emoji picker button. The `disablePrivateChat` override hides private chat even when the preset allows it.

To control private chat for all participants, prefer preset permissions.

</RTKCodeSnippet>

<RTKCodeSnippet id="web-react">

Use `RtkChat` to render the full chat panel. It includes public chat, private chat, paginated messages, pinned messages, file uploads, image uploads, edit actions, delete actions, and emoji input.
...
```html
<rtk-ui-provider [meeting]="meeting" [overrides]="chatOverrides">
	<rtk-chat-toggle></rtk-chat-toggle>
	<rtk-chat style="height: 480px;"></rtk-chat>
</rtk-ui-provider>
```
...
If you need more layout control, compose the lower-level chat components. `RtkChatMessagesUiPaginated` loads public messages when `privateChatRecipient` is empty and private messages when you pass a participant.
...
function CustomChat({ meeting }) {
	...
	const canSendPrivateText =
		canPrivateMessage && meeting.self.permissions.chatPrivate?.text;
	const canSendPrivateFiles =
		canPrivateMessage && meeting.self.permissions.chatPrivate?.files;

	return (
		<div style={{ height: "480px" }}>
			<RtkChatSelector
				meeting={meeting}
				onRtkChatSelectorChange={(event) => {
					setPrivateChatRecipient(event.detail.selectedUser ?? null);
				}}
			/>
			<RtkPinnedMessageSelector meeting={meeting} />
			<RtkChatMessagesUiPaginated
				meeting={meeting}
			...
			<RtkChatComposerView
				canSendTextMessage={
					privateChatRecipient ? canSendPrivateText : canSendPublicText
				}
				canSendFiles={
					privateChatRecipient ? canSendPrivateFiles : canSendPublicFiles
				}
				inputTextPlaceholder="Enter a message"
				onNewMessage={(event) =>
					meeting.chat.sendMessage(
						event.detail,
						privateChatRecipient ? [privateChatRecipient.id] : [],
					)
				}
			/>
		</div>
...
Refer to the [`RtkChat`](/realtime/realtimekit/ui-kit/api-reference/react/RtkChat/), [`RtkChatSelector`](/realtime/realtimekit/ui-kit/api-reference/react/RtkChatSelector/), [`RtkPinnedMessageSelector`](/realtime/realtimekit/ui-kit/api-reference/react/RtkPinnedMessageSelector/), [`RtkChatMessagesUiPaginated`](/realtime/realtimekit/ui-kit/api-reference/react/RtkChatMessagesUiPaginated/), and [`RtkChatComposerView`](/realtime/realtimekit/ui-kit/api-reference/react/RtkChatComposerView/) API references for component props.
