...
There are four types of messages that can be sent in chat:
...
Use `fetchPublicMessages()`, `fetchPrivateMessages()`, or `fetchPinnedMessages()` to load paginated chat messages.

```js
const messages = await meeting.chat.fetchPublicMessages({
	timestamp,
	limit: size,
	direction: "before",
});
...
The `meeting.chat.messages` array contains recent messages and live updates. This array is deprecated for loading chat history. Use `fetchPublicMessages()`, `fetchPrivateMessages()`, or `fetchPinnedMessages()` instead.
...
Use `meeting.chat.sendCustomMessage()` to send structured chat content. Custom messages can include text, HTML, files, images, or videos.

```js
await meeting.chat.sendCustomMessage({
	type: "custom",
	message: "Agenda for today",
	html: "<strong>Agenda for today</strong>",
	files: [{ link: "https://example.com/agenda.pdf" }],
});
```
...
Pass peer IDs as the second argument to send a private message. The message is visible only to the selected peers and the sender.

```js
const peerIds = ["peer-id-1", "peer-id-2"];

await meeting.chat.sendTextMessage("Hello", peerIds);
```
...
Here, `message` is of type `Message`, as defined in the introduction. `messages` contains the current in-memory chat messages.
...
Use `fetchPublicMessages()` to load public chat history with pagination. Pass a timestamp in milliseconds, a limit, and a direction.
...
Use `fetchPrivateMessages()` to load messages between the current user and another participant. Pass the participant user ID with pagination options.

```js
const messages = await meeting.chat.fetchPrivateMessages({
...
Use `fetchPinnedMessages()` to load pinned chat messages with pagination.

```js
const pinnedMessages = await meeting.chat.fetchPinnedMessages({
	timestamp: Date.now(),
	limit: 50,
	direction: "before",
});
...
You can pin any text, image, file, or custom message. The message object includes `pinned: true` after the pin state changes.
...
To retrieve pinned messages, use `fetchPinnedMessages()`.
