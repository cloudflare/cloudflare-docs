...
RealtimeKit API requests count toward the global Cloudflare API limit, which
applies across all `api.cloudflare.com` API requests. Recording APIs are also
...
Each participant has a separate limit for SDK operations during an active session.
Limits are not shared across participants or applied to your app as a whole. For
example, in a two-participant meeting, each participant can send up to 180 text
messages per minute.
