...
import { Details, RealtimeKitMeetingBlueprintDiagram } from "~/components";

A **Meeting** is a persistent, reusable virtual room record. It has a stable ID, title, status, and meeting-level configuration.

A Meeting has no scheduled date or time. The Meeting itself does not start or end.

You can create a Meeting ahead of time or just in time. Reuse it until you set its status to `INACTIVE`.

The diagram shows the Meeting abstraction:

<RealtimeKitMeetingBlueprintDiagram />
...
A Session uses the Meeting title and meeting-level configuration, while each connected participant uses the Preset assigned to their participant record.
