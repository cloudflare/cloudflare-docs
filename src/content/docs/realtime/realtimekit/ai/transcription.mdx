...
RealtimeKit accepts the following language codes for real-time transcription. These are a subset of the languages supported by [Deepgram Nova-3](/workers-ai/models/deepgram-nova-3/) — only the codes listed below can be passed to `ai_config.transcription.language`.
...
Use `multi` for automatic language detection across English, Spanish, French, German, Hindi, Russian, Portuguese, Japanese, Italian, and Dutch. If no language is specified, the default is `en-US`.
