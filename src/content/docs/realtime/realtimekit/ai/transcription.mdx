...
| ---------------- | -------------------------------------------------------------------- | ---------------------- | ----------------------------- |
| **Real-time**    | [Deepgram Nova-3](/workers-ai/models/nova-3/)                        | During the meeting     | Live captions for attendees   |
| **Post-meeting** | [Whisper Large v3 Turbo](/workers-ai/models/whisper-large-v3-turbo/) | After the meeting ends | Transcript files and webhooks |
...
During the meeting, RealtimeKit streams transcript updates to the client SDK. To access existing transcripts from `meeting.ai.transcripts` or listen for new transcript events with `meeting.ai.on("transcript", ...)`, refer to [Consume transcripts](#consume-transcripts).
...
Post-meeting transcription generates a transcript after the meeting ends using [Whisper Large v3 Turbo](/workers-ai/models/whisper-large-v3-turbo/) on Workers AI and delivers it through a webhook or REST API. To identify speakers, RealtimeKit processes each participant's audio separately before creating the final transcript.

### Enable post-meeting transcription

You can enable post-meeting transcription when you create a meeting. Set `transcribe_on_end: true` to generate a transcript after the meeting ends.
...
### Fetch post-meeting transcripts
...
Configure the `meeting.transcript` event in [webhooks](/api/resources/realtime_kit/subresources/webhooks/):
...
RealtimeKit's out-of-the-box transcription records each participant's audio track and processes it with Workers AI. Workers AI usage is billed to your Cloudflare account using [audio model pricing](/workers-ai/platform/pricing/#audio-model-pricing), which scales by participant audio minutes, not meeting duration.
...
For real-time transcription, RealtimeKit streams enabled participant audio to Workers AI and sends transcript updates to meeting participants during the meeting.
