...
RealtimeKit supports the following language codes for real-time transcription, powered by [Deepgram Nova-3](/workers-ai/models/deepgram-nova-3/) on Workers AI.
...
Use `multi` for automatic language detection across all of the languages listed above. If no language is specified, the default is `en-US`.
