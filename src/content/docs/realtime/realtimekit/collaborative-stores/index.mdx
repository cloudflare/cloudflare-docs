...
```
```ts
const store = meeting.stores.stores.get('myStore');
...
You can attach event listenerds on a store's key, which fire when the value changes.
...
You can delete a store using `storeName`.
