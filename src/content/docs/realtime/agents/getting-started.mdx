...
2. Click on the `Join` icon next to the meeting and generate the RealtimeKit link. This contains the `meetingId` (`bbb...`) and the `authToken` to be passed in the final step:
