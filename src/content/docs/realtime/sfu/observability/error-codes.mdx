...
A top-level error applies to the request. Batch operations can also return errors inside individual `tracks` or `dataChannels` results. Check every item and retain successful allocations, even when another item fails.
