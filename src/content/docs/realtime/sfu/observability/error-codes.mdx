...
| Code                          | Meaning and next action                                                                                                                                                                                                                                                                                                             |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `session_error`               | The session is not ready, is disconnected, or has closed. With top-level HTTP `425`, finish setup and wait for connection before later operations. With `410`, check the session ID and endpoint state; replace a closed connection and rebuild its publications and subscriptions. Retrying the old session does not reconnect it. |
...
```json
{
  ...
  "tracks": [
    ...
    { "mid": "2", "errorCode": "internal_error" }
  ]
}
```

Track `0` closed; `1` is absent or already closed. Keep `2` and any unreported mids pending. For DataChannels, match results by `id`.
