...
For an existing connection, refer to [Add media](#add-media-to-an-existing-connection), [Add DataChannels](#add-datachannels-to-a-media-connection), or [Close media tracks](#close-media-tracks). To run an application before implementing these exchanges, follow the [quickstart](/realtime/sfu/get-started/).
...
1. **Prepare the media offer.** In the browser, capture tracks with `navigator.mediaDevices.getUserMedia()`. Create an `RTCPeerConnection`, called `pc` here, and add each track with `pc.addTransceiver(track, { direction: "sendonly" })`. Create an offer with `await pc.createOffer()`, then apply it with `await pc.setLocalDescription(offer)`.
   ...
   Wait for [candidate gathering](#network-connectivity), then read each transceiver's `mid`. This identifier associates a media section in this connection's SDP with the track you publish.
...
Start with a connected PeerConnection and keep its SFU session. Serialize additions with [other operations on that session](/realtime/sfu/concepts/negotiation/#serialize-mutations-per-session).

- **Publish more media:** Add transceivers for the new tracks, then create and set a fresh local offer. Follow the [publication recipe](#publish-audio-or-video) from `tracks/new`, sending the new local entries with the gathered `pc.localDescription`.
- **Receive more media:** Call `tracks/new` on the same session with additional remote entries in the [receive request format](#receive-a-published-track). [Complete any returned SFU offer](#complete-an-sfu-offer).
