...
Choose the task for your endpoint. Each recipe starts with a new browser connection and matching SFU session. Your application can combine these endpoints into rooms, broadcasts, and device interfaces.
...
For an existing connection, refer to [Add DataChannels to a media connection](#add-datachannels-to-a-media-connection). To run an application before implementing these exchanges, follow the [quickstart](/realtime/sfu/get-started/).
...
Stop accepting new operations for the connection. Close its media tracks through `PUT /sessions/{sessionId}/tracks/close` using that session's track `mid` values. Close application DataChannels through `PUT /sessions/{sessionId}/datachannels/close` with `dataChannels: [{ id: <ALLOCATED_ID> }]`. Use the [OpenAPI schemas](/realtime/static/realtime-api-2024-05-21.yaml) for complete request bodies.

Complete each close operation's required SDP exchange before starting the next close on that session. Retain failed cleanup for retry. Close the endpoint's DataChannels and PeerConnection, and stop captured camera/microphone tracks when they are no longer needed. Follow [teardown ordering](/realtime/sfu/concepts/negotiation/#teardown) when requests remain in flight.
