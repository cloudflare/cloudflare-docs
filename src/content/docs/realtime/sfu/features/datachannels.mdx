...
```mermaid
flowchart LR
  P[Publisher] -->|Messages| SFU[Realtime SFU]
  SFU -->|Messages| V[Subscribers]
  SFU -->|Messages| C[Selected subscriber]
  C -->|Authorized replies| SFU
  SFU -->|Replies| P
```
...
1. On your backend, call `POST /apps/{appId}/sessions/{publisherSessionId}/datachannels/new`. In the request's `dataChannels` array, add an entry with `location: "local"` and the chosen `dataChannelName`.
2. On the subscriber's session, call `POST /apps/{appId}/sessions/{subscriberSessionId}/datachannels/new`. Its `dataChannels` entry uses `location: "remote"`, the publisher's `sessionId`, and the same `dataChannelName`.
3. Check every API result and retain successful allocations. Return each channel `id` to its own endpoint. Create its browser channel with `pc.createDataChannel(name, { negotiated: true, id: channelId })`. Publisher and subscriber IDs can differ. This configuration uses reliable, ordered delivery by default.
...
Match results by `id`. A `close_track_error` item means absent or already closed; retain other failed or unreported IDs for retry. Refer to [close-result handling](/realtime/sfu/observability/error-codes/#interpret-close-results).
