...
Close application channels through `PUT /apps/{appId}/sessions/{sessionId}/datachannels/close` and check individual results. Preserve unsuccessful cleanup work for retry. Close the endpoint's DataChannels and PeerConnection when that connection is no longer needed.
