...
```mermaid
flowchart LR
  Text[Publisher text] --> TTS[TTS Durable Object]
  TTS <--> Speech[Workers AI speech generation]
  TTS -->|PCM over WebSocket| SFU[Realtime SFU]
  SFU -->|WebRTC audio| Listeners[Browser listeners]
  Mic[Browser microphone] -->|WebRTC audio| SFU
  SFU -->|PCM over WebSocket| STT[STT Durable Object]
  STT <--> Transcription[Workers AI transcription]
  STT -->|Transcript WebSocket| UI[Browser transcript]
```
