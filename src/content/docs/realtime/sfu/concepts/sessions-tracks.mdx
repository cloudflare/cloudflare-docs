...
Close tracks and DataChannels when the application no longer needs them. Remove their discovery entries after cleanup succeeds, and preserve enough state to retry failed cleanup.

Inactivity timeouts are described in [limits](/realtime/sfu/platform/limits/). They do not replace application membership, reconnect decisions, or explicit cleanup.
