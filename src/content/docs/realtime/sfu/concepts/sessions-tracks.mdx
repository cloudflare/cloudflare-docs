...
Follow [connection patterns](/realtime/sfu/get-started/connection-patterns/) to publish media, receive a track, exchange messages, or combine media with controls. Follow [per-session mutation ordering](/realtime/sfu/concepts/negotiation/#serialize-mutations-per-session) when updating those connections.
