...
- Cloudflare cannot send webhooks to `localhost` or local IP addresses. A publicly accessible URL is required. For local testing, use a [Quick Tunnel](/tunnel/get-started/quick-tunnels/) to expose your local server to the Internet. For a step-by-step walkthrough, refer to [Test webhooks locally](/stream/examples/test-webhooks-locally/).
