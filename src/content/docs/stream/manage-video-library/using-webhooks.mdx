...
* Cloudflare cannot send webhooks to `localhost` or local IP addresses. A publicly accessible URL is required. To test webhooks during local development, use [Cloudflare Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/) to expose your local server. Third-party tunneling tools (such as ngrok) are often blocked by Cloudflare's security features. Refer to [Test webhooks locally](/stream/examples/test-webhooks-locally/) for a complete walkthrough.
