...
title: Receive events and webhooks
description: Receive notifications when Cloudflare Stream videos or live inputs are ready, enter an error state, or changes status.
sidebar:
  order: 7
products:
  - stream
...
import { Details, DashButton, Render } from "~/components";

Cloudflare Stream can notify your service when videos are ready to play, are edited, have updated captions/downloads, or error; or when a live input connects, disconnects, or errors. You have two options for receiving these notifications:

**Event subscriptions _(recommended)_** publish structured events to a [Cloudflare Queue](/queues/), which you can consume from a Worker or [HTTP pull consumer](/queues/configuration/pull-consumers/). Use event subscriptions when you want to build pipelines on Cloudflare without managing webhook endpoints, signature verification, or retries. Event subscriptions offer a greater set of events and properties.

**Webhooks** send an HTTP `POST` request to a URL you control. Use webhooks when you need to deliver notifications to a service outside of Cloudflare.

## Event subscriptions

<Render file="event-subscriptions/intro" product="queues" />

Stream publishes events for on-demand uploads and live broadcasts. For the full list of events and example payloads, refer to [Events & schemas](/queues/event-subscriptions/events-schemas/#stream).
...
## Webhooks for uploaded videos

Webhooks notify your service when videos successfully are ready to stream or if your video enters an error state.

:::note

Webhooks work differently for live broadcasting. For more information, refer to [Receive live events and webhooks](/stream/stream-live/webhooks/).
...
- Cloudflare cannot send webhooks to `localhost` or local IP addresses. A publicly accessible URL is required. For local testing, use a [Quick Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/) to expose your local server to the Internet. For a step-by-step walkthrough, refer to [Test webhooks locally](/stream/examples/test-webhooks-locally/).
