...
* Cloudflare cannot send webhooks to `localhost` or local IP addresses. A publicly accessible URL is required. Popular third-party tunneling tools (like [Ngrok](https://ngrok.com)) are sometimes blocked by Cloudflare's security features. For an alternative that does not require administrator access or DNS control, use [Cloudflare Tunnel](/pages/how-to/preview-with-cloudflare-tunnel/).
