...
Cloudflare Stream can notify your service when videos finish processing, enter an error state, or when a live input connects, disconnects, or errors. You have two options for receiving these notifications:

- **Event subscriptions** publish structured events to a [Cloudflare Queue](/queues/), which you can consume from a Worker or [HTTP pull consumer](/queues/configuration/pull-consumers/). Use event subscriptions when you want to build pipelines on Cloudflare without managing webhook endpoints, signature verification, or retries.
