...
Media Transforamtions and Image Transformations use the same subscriptions and usage metrics.
...
- While the binding is in beta, operations via the binding will not be billed.
- The costs are the same, but all operations are billed individually, not based on uniqueness. Cache or store results for cost optimization and performance.
