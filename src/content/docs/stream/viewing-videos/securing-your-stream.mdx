...
The Allowed Origins feature allows you to specify which origins are allowed for playback. This feature works even if you are using your own video player. When using your own video player, Allowed Origins restrics which domain the HLS/DASH manfifest and the video segments can be requested from. 
