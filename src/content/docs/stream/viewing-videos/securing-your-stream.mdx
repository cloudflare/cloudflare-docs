...
If you have enabled downloads by setting the downloadable property to `true`, you can download the MP4 file by replacing the video ID in the download URL with the `token` value:

- `https://customer-<CODE>.cloudflarestream.com/<TOKEN>/downloads/default.mp4`
...
To download the video, make sure your token includes `downloadable: true` and replace the video id in the download url with the `token` value:
