...
If your broadcast software shows a connection error or the stream fails to start, verify that the live input is enabled. A live input that is _disabled_ will reject all incoming connections.
...
```bash title="Check if a live input is enabled"
curl -X GET \
...
```bash title="Turn on a live input"
curl --request PUT \
https://api.cloudflare.com/client/v4/accounts/{account_id}/stream/live_inputs/{input_id} \
--header "Authorization: Bearer <API_TOKEN>" \
--data '{"enabled": true}'
