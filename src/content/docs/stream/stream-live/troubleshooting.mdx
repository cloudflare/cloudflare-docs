...
- If the connection fails, check whether the live input is [disabled](/stream/stream-live/start-stream-live/#enable-or-disable-a-live-input) or its [broadcast keys have been rotated](/stream/stream-live/start-stream-live/#rotate-broadcast-keys).
