...
- **Event subscriptions** publish structured events to a [Cloudflare Queue](/queues/), which you can consume from a Worker or [HTTP pull consumer](/queues/configuration/pull-consumers/). Use event subscriptions when you want to build pipelines on Cloudflare without managing webhook endpoints or notification destinations.
- **Webhooks** send an HTTP `POST` request to a URL you control, configured through the Cloudflare dashboard's **Notifications** page. Use webhooks when you need to deliver notifications to a service outside of Cloudflare.
