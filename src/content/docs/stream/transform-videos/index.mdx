...
When using Media Transformations from a Cloudflare Worker, we recommend using the [bindings](/stream/transform-videos/bindings/). Otherwise, if the Worker is calling Media Transformations on the same zone used as its trigger, apply the `global_fetch_strictly_public` compatibility flag to avoid 404 errors on `/cdn-cgi/media` paths.
