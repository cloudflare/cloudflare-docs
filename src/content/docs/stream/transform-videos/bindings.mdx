...
The Media Transformations binding is similar to the [Images binding](/images/transform-images/bindings/), except the method chain order is fixed and the result of an `input()` cannot be reused across multiple transformations.
...
Defines how the video input should be transformed by resizing or cropping.
...
- Return value should be used with `.output()`.

### `.output()`

Defines what to extract from the transformed video and how the output will be formatted.
