...
The Media Transformations binding is similar to the [Images binding](/images/optimization/transformations/bindings/), except the method chain order is fixed and the result of an `input()` cannot be reused across multiple transformations.
...
export default {
	async fetch(request, env) {
		...
		const response = await env.AI.run("@cf/unum/uform-gen2-qwen-500m", payload);
...
export default {
	async fetch(request, env) {
		...
		const response = await env.AI.run("@cf/openai/whisper", payload);
