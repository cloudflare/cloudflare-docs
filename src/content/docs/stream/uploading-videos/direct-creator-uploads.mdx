...
The examples below show how to provision a one-time upload URL for your end users.
...
For tus protocol uploads, your backend must pass the `Tus-Resumable`, `Upload-Length`, and `Upload-Metadata` headers. The one-time upload URL is returned in the `Location` header of the response, not in the response body.
...
The Workers Binding API creates a basic POST direct upload URL. For tus protocol uploads that require custom headers, use the REST API approach.

:::

See the full [Workers Stream binding API reference](/kv/api/read-key-value-pairs/).
