...
- If your video is a [basic upload](/stream/uploading-videos/direct-creator-uploads/#basic-uploads) under 200MB and users do not need resumable uploads, generate a URL that accepts an HTTP post request.

- If your video is over 200MB or if you need to allow users to [resume interrupted uploads](/stream/uploading-videos/direct-creator-uploads/#resumable-uploads), generate a URL using the tus protocol.
...
Use this option if your users upload videos under 200MB, and you do not need to allow resumable uploads.
...
2. With the `uploadURL` from the previous step, users can upload video files that are limited to 200MB in size. Refer to the example request below.
...
The example below shows how to build a Worker to get a URL you can use to upload your video. Note that the one-time upload URL is returned in the `Location` header of the response, not in the response body.
...
- [Search for a video](/stream/manage-video-library/searching/) with the UID
   to check the status.
