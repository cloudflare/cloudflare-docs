...
The Workers Binding API creates a basic POST direct upload URL. For tus protocol uploads that require custom headers like `Tus-Resumable`, `Upload-Length`, and `Upload-Metadata`, use the REST API approach shown above.
