...
### Purge when using URL rewrite rules

If you use [URL rewrite rules](/images/optimization/transformations/rewrite-rules/) to send image requests through `/cdn-cgi/image/`, rewrite rules are also evaluated on purge requests. This can cause a single-file purge to return a success response without clearing the cached image.

To resolve this, exclude purge requests from the rewrite rule by adding the following to the rule expression:

```txt
and (not (http.request.method eq "PURGE"))
```


If you cannot modify your rewrite rules, [purge by prefix](/cache/how-to/purge-cache/purge_by_prefix/) and [purge by cache tag](/cache/how-to/purge-cache/purge-by-tags/) are alternatives.
