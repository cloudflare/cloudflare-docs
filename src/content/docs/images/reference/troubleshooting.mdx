...
- 9401 — The required arguments in `{cf:image{…}}` options are missing or are invalid. Try again. Refer to [Fetch options](/images/optimization/features/#parameters) for supported arguments.
- 9402 — The image was too large or the connection was interrupted. Refer to [Supported formats and limitations](/images/get-started/limits/) for more information.
- 9403 — A [request loop](/images/optimization/transformations/transform-via-workers/#prevent-request-loops) occurred because the image was already resized or the Worker fetched its own URL. Verify your Worker path and image path on the server do not overlap.
...
- 9524 — The `/cdn-cgi/image/` resizing service could not perform resizing. This may happen when an image URL is intercepted by a Worker. As an alternative you can [resize within the Worker](/images/optimization/transformations/transform-via-workers/). This can also happen when using a `pages.dev` URL of a [Cloudflare Pages](/pages/) project. In that case, you can use a [Custom Domain](/pages/configuration/custom-domains/) instead.
- 9520 — The image format is not supported. Refer to [Supported formats and limitations](/images/get-started/limits/) to learn about supported input and output formats.
...
## Warnings from resizing

Some conditions do not fail a transformation, but stop Cloudflare from optimizing an image as well as it otherwise would. The image is still returned with a `200` status code, and a `Warning` response header explains why. A response may carry more than one. Warnings are advisory and never change the status code.

### cache-control is too restrictive

Optimizing an image costs CPU time, and that cost is only worthwhile if the result can be reused from cache. Before transforming a remote image, Cloudflare inspects the cache directives on the source image response. If the optimized image is unlikely to stay in cache for at least an hour, Cloudflare returns:

```txt
Warning: cf-images 299 "cache-control is too restrictive"
```

The source image is treated as poorly cacheable when any of the following is true:

- The directives include `private` or `no-store`.
- The directives include `no-cache` or `must-revalidate`, and the response has neither an `ETag` nor a `Last-Modified` header.
- `max-age` is less than `3600` seconds. If `s-maxage` is present, it is used instead of `max-age`.

Cloudflare then optimizes the image with reduced effort.

To restore full-effort optimization, make the source image cacheable for at least an hour:

- Return `Cache-Control: public, max-age=3600` or longer from your origin. Images often suit much longer lifetimes, for example `max-age=604800`.
- Without an origin change, a Cache Response Rule can set the max-age directive on origin responses before they reach our cache: https://developers.cloudflare.com/cache/how-to/cache-response-rules/

After changing the headers, purge the source image URL so the new directives take effect. Refer to [Caching and purging](#caching-and-purging).
...
- [Bring Your Own IP (BYOIP)](/byoip/) is not compatible with Images when optimizing remote images (transformations).
- When [Polish](/images/polish/) can't optimize an image the Response Header `Warning: cf-images 299 "original is smaller"` is returned.
...
However, in cases where customers agree to store such images in public cache, Cloudflare supports resizing images through Workers [on authenticated origins](/images/optimization/transformations/transform-via-workers/).
