...
We strongly recommend enabling [Workers Cache](/workers/runtime-apis/cache/) so that repeat requests are served from cache without re-running your Worker or re-transforming the image. Add the following to your Wrangler configuration file:
...
Creates an optimization handle for an image. All operations begin with this method. Accepts image bytes up to 20 MB from any source, including [Images](/images/storage/upload-images/methods/), [R2](/r2/), a `fetch()` response, or a request body.
