...
Applies [optimization parameters](/images/optimization/features/#parameters) to the image, such as `width`, `height`, or `blur`. You can chain multiple `.transform()` calls to control the order that parameters will be applied.

The example below shows how you can optimize [images that are stored in Images](/images/storage/binding/) by getting the image bytes:
...
Accepts `opacity`, `repeat`, and a side (`left`, `right`, `top`, `bottom`).

#### `opacity`

Sets the opacity of the overlay image. Accepts a decimal value between `0.0` (fully transparent) and `1.0` (fully opaque).
...
- `true` — The overlay image will be tiled to cover the entire area. This is useful for stock photos.
- `x` — The overlay image will be tiled horizontally.
- `y` — The overlay image will be tiled vertically.

#### `left`, `right`, `top`, `bottom`

Sets the position of the overlay image as an offset, in pixels, to the specified edge. If this option is not set, then the image will be centered.

`0` aligns the overlay flush to the edge. For example, `{ bottom: 0, right: 10 }` places the overlay at the bottom-right corner, 10 pixels inward from the right edge.

The example below shows how you can overlay a resized watermark on an image from external URLs:
...
export default {
  async fetch(request,env) {
    ...
    const overlayURL = "https://example.com/watermark.png";
      ...
      await env.IMAGES
        ...
        .draw(
          env.IMAGES.input(overlay.body)
            .transform({width: 32, height: 32}), // Resize the overlay image
            {bottom: 0, right: 0} // Position the overlay
        )
