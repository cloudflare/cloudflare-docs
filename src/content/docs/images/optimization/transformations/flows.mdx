...
Each flow pairs a set of conditional triggers (e.g. image is a JPEG or PNG) with optimization parameters (e.g. transcode to AVIF).
...
- A custom flow triggers only on requests for [supported image types](/images/get-started/limits/). HTML pages, CSS files, and other non-images are never affected.
