...
You can set image optimization parameters on a `fetch()` subrequest in a Worker using the `cf.image` property. This is the same underlying mechanism as the [URL interface](/images/optimization/transformations/overview/#url-interface), but gives you programmatic control over every image request.

To work with image bytes directly instead of URLs, use the [Images binding](/images/optimization/binding/).
...
The resizing feature is accessed via the [options](/workers/runtime-apis/request/#the-cf-property-requestinitcfproperties) of a `fetch()` [subrequest inside a Worker](/workers/runtime-apis/fetch/). The `fetch()` function accepts parameters in the second argument inside the `{cf: {image: {…}}}` object.
