...
- `filter` <Type text="ImageListFilter" /> — Filter results by image properties. Accepts a `metadata` field to filter by custom metadata.
...
- `in` <Type text="Array&lt;string&gt; | Array&lt;number&gt;" /> — Matches a field against any value in an array. An array accepts up to 10 values, and a string value cannot contain the pipe character (`|`).
