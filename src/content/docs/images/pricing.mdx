...
You are billed on the number of unique transformations per calendar month. Repeat requests for the same transformation within the same month are only charged once for that month.
