...
Qwen 3.8 supports `"low"`, `"medium"`, and `"xhigh"`. It defaults to `"xhigh"`. Other effort names, including `"high"`, `"max"`, and `"none"`, are not aliases. To turn off Qwen 3.8 reasoning, use `chat_template_kwargs: { enable_thinking: false }`.

<Aside type="note" title="Nemotron uses model-specific reasoning controls">
