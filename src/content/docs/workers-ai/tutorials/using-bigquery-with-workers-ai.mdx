...
You can skip this tutorial and check its [resulting code](#final-result).
...
## Step 2 - Import GCP Service key into the Worker as Secrets
...
Don't include any double quotes in the Secret that you store, as it will be already interpreted as a string.
...
## Step 3 - Install library to handle JWT operations
...
## Step 7 - Feeding data into Workers AI
...
export default {
	async fetch(request, env, ctx) {
    	...
    	let ticketInfo
    	try {
        	ticketInfo = await queryBQ(bqJWT, `/bigquery/v2/projects/${env.BQ_PROJECT_ID}/queries`)
    	} catch (error) {
        	console.log(error)
        	return new Response('An error has occurred while querying BQ', { status: 500 })
    	}
