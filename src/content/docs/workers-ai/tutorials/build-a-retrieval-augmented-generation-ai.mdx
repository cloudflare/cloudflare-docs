...
To begin using Cloudflare's AI products, you can add the `ai` block to the `wrangler.toml / wrangler.json(c)` file. This will set up a binding to Cloudflare's AI models in your code that you can use to interact with the available AI models on the platform.
...
Then, add the configuration details for your new Vectorize index to the `wrangler.toml / wrangler.json(c)` file:
...
Then, paste the configuration details output from the previous command into the `wrangler.toml / wrangler.json(c)` file:
