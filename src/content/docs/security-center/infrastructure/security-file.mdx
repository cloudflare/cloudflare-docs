...
- **(Required) Contact**: You can enter one of the following to contact you about security issues:
  - An email address: The email address must start with `mailto:` (e.g. `mailto:help@example.com`).
  - A phone number: The phone number must start with `tel:` (e.g. `tel:+1 1234567890`).
  - A URL link: The URL link must start with `https://` (e.g. `https://example.com`. 
