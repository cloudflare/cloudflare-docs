...
description: Receive curated intelligence feeds from Cyber Defense Collaboration groups.
products:
  - security-center
tags:
  - REST API
...
With Custom Indicator Feeds, Cloudflare provides an intelligence feed based on data received from various Cyber Defense Collaboration groups. The security filtering capabilities are available to eligible public and private sector organizations.
...
For more information on creating Gateway policies, refer to [DNS policies](/cloudflare-one/traffic-policies/dns-policies/).
