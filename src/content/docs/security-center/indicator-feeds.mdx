...
Once an account is granted access to a feed, it will be available to match traffic as a [selector in Gateway DNS policies](/cloudflare-one/traffic-policies/policy-types/dns-policies/selectors/#indicator-feeds).
...
For more information on creating Gateway policies, refer to [DNS policies](/cloudflare-one/traffic-policies/policy-types/dns-policies/).
