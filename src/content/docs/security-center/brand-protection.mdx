...
2. In **Strings**, enter a brand phrase, then select **Add string**. Repeat this step to add multiple phrases.

3. Select **Save query**. In the **Save brand protection query** dialog, enter a **Query name**, then select **Save query** again.

   :::note[Matching behavior]

   Character distance is calculated automatically and cannot be configured in the dashboard. Hyphens and dots are treated as separators. To treat a compound phrase as a single string, enter it without separators. For example, enter `examplebrand` to monitor variants of `example-brand`.
   :::

4. To export all matches from a saved query, select your **Query name** > select the three dots > **Export matches**.
