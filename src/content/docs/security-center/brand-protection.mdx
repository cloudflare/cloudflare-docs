...
4. Set the threshold: Set a match threshold (the minimun is 75%). A higher score ensures high-precision matches, while a lower score catches remixed or slightly altered versions of your logo.
5. Select **Save logo**. The system will now scan newly detected logos for visual matches.
