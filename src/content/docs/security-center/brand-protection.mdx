...
2. Create a string query, provide a name for the query, and enter one or more brand phrases.

3. Select **Save query** to monitor matching domains. Character distance is calculated automatically for each brand phrase and cannot be configured in the dashboard.

   :::note[Compound phrases]

   Hyphens and dots are treated as separators. To treat a compound phrase as a single string, enter it without separators. For example, enter `examplebrand` to monitor variants of `example-brand`.
   :::
