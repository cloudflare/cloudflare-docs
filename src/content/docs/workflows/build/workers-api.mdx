...
If `retention` is not set, instance state is retained for the [`default_retention`](/workflows/build/workers-api/#default-instance-retention) configured on the Workflow binding. If the binding does not set a default either, instance state is retained for 3 days on the Workers Free plan or 7 days on the Workers Paid plan. Refer to the [retention limit](/workflows/reference/limits/) for more information.
