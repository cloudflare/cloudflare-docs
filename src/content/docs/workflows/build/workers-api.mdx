...
- <code>createBatch(options: WorkflowBatchCreateOptions): Promise&lt;WorkflowBatchCreateResult&gt;</code>
  - `options` - either a `count` of instances to create with shared options, or a list of `instances`, each with its own options.

:::note
Local development support for this form of `createBatch` requires Wrangler X.Y.Z or later.
:::
...
To give each instance its own ID or options, pass `instances`. Each entry accepts the same [`WorkflowInstanceCreateOptions`](/workflows/build/workers-api/#workflowinstancecreateoptions) as `create`:
...
Unlike [`create`](/workflows/build/workers-api/#create), `createBatch` does not throw when an ID is already in use. Instead, it reports the entry in `errors` with one of the following codes:
