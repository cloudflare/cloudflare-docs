...
You can declare the Workflows a Worker defines in the `exports` field of your Wrangler configuration. Use this when the Worker that defines a Workflow does not need a binding to it, for example:

- The Workflow runs on a [schedule](/workflows/build/trigger-workflows/#schedule-a-workflow-directly).
- Another Worker calls the Workflow through a [cross-script binding](#cross-script-calls).
