...
Workflows currently requires you to bind to a Workflow via the `wrangler.toml / wrangler.json(c)` file, and does not yet support bindings via the Workers dashboard.
...
For example, if your Workflow is defined in a Worker script named `billing-worker`, but you are calling it from your `web-api-worker` script, your `wrangler.toml / wrangler.json(c)` file would resemble the following:
