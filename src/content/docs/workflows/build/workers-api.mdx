...
- <code>throw new NonRetryableError(message: <Type text="string" />, name <Type text="string" /> <MetaInfo text="optional" />)</code>: <Type text="NonRetryableError" />
  - When thrown inside [`step.do()`](/workflows/build/workers-api/#step), this error stops step retries, propagating the failure to the top level (the [run](/workflows/build/workers-api/#run) function) and causing the Workflow instance to fail.
