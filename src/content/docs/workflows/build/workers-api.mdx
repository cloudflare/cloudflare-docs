...
- Values larger than your account's retention limit are capped at that limit.

An instance that sets its own [`retention`](/workflows/build/workers-api/#workflowinstancecreateoptions) when it is created uses that value instead of the binding default.
