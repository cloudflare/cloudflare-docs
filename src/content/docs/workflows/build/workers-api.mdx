...
By default, the state of a finished Workflow instance is retained for the maximum [retention period](/workflows/reference/limits/) available on your account. To retain instances for a shorter period, configure `default_retention` on the Workflow binding in your Wrangler configuration:
...
Instance-level retention settings take precedence over Workflow binding settings, which take precedence over the account default. The retention period cannot exceed your account's [retention limit](/workflows/reference/limits/).
...
A `workflow` export accepts the same settings as a `workflows` binding: `limits`, `schedules`, and `default_retention`. When you run `wrangler deploy`, Wrangler creates or updates the Workflow with these settings. Refer to [Workflow exports](/workers/wrangler/configuration/#workflow-exports) for the full list of fields.
...
- Wrangler A.B.C or above
- `@cloudflare/vite-plugin` D.E.F or above
- `@cloudflare/vitest-plugin` G.H.I or above

:::

Each Workflow declared in `exports` is available on [`ctx.exports`](/workers/runtime-apis/context/#exports), keyed by class name. It has the same API as a [Workflow binding](#workflow), so the Worker does not need a `workflows` binding to call it.
...
If you use TypeScript, run [`wrangler types`](/workers/wrangler/commands/workers/#types) to type `ctx.exports.MyWorkflow` as `Workflow<Params>`.
...
To test a Workflow declared in `exports` with the Vitest integration, refer to [Introspect Workflows declared in `exports`](/workers/testing/vitest-integration/test-apis/#introspect-workflows-declared-in-exports).
...
If `retention` is not set, instance state is retained for the [`default_retention`](/workflows/build/workers-api/#default-instance-retention) configured on the Workflow binding. If the binding does not set a default either, instance state is retained for the maximum retention period available on your account (3 days on the Workers Free plan, 30 days on the Workers Paid plan). Refer to the [retention limit](/workflows/reference/limits/) for more information.
...
Delete a Workflow instance and its stored state with <code>delete(): Promise&lt;void&gt;</code>.
