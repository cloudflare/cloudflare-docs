...
- The workflow retrieves 500 chunks at a time, processes the chunks, and updates their prefix
- Then, if there are remaining objects with the original prefix, the Workflow instance restarts

```TypeScriptExample
export class MyWorkflow extends WorkflowEntrypoint<Env, Params> {
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    // Step 1: List first 500 files from R2 with prefix "chunk/"
    ...
    await step.do("restart workflow", async () => {
      if (listResult.truncated) {
        const counterKey = `stats/${event.instanceId}/processed-count`;
        const existing = await this.env.CHUNKS_BUCKET.get(counterKey);
        const currentCount = existing
          ? (await existing.json<{ count: number }>()).count
          : 0;
        const newCount = currentCount + listResult.keys.length;
