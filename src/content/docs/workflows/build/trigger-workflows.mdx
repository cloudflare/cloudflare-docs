...
Restarting an instance will immediately cancel any in-progress steps, erase any intermediate state, and treat the Workflow as if it was run for the first time. However, the restarted instance will maintain the concurrency slot of the previous instance. 

:::caution[Exercise caution when using the `restart()` method to avoid concurrency slot deprivation. If you call the `restart()` method recursively without a conditional, that instance will run indefinitely, effectively eliminating a concurrency slot from your queue.] 


The following example shows how you can use the `restart` method to maintain a document processing pipeline:
  ...
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    const listResult = await step.do("list files from R2", async () => {
     ...
     // So we will need a cursor to keep track of progress
      ...
      // Get the first 100 chunks to process
