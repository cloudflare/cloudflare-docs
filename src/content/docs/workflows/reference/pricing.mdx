...
- If you do not need to retain state for errored or completed Workflows, you can set a shorter retention period for every instance of a Workflow with [`default_retention`](/workflows/build/workers-api/#default-instance-retention) in your Wrangler configuration, or per instance with the [`retention` option in `WorkflowInstanceCreateOptions`](/workflows/build/workers-api/#workflowinstancecreateoptions).
