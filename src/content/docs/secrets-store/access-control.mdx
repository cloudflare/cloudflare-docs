...
Each secret has a list of **scopes** that determine which Cloudflare services are allowed to consume it. You set scopes when you create a secret, and can update them later by editing the secret.

The currently supported scopes are:

| Scope        | Service                             | Use                                                                       | Integration documentation                                                                                                        |
| ------------ | ----------------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `workers`    | Workers                             | Bind the secret to a Worker.                                              | [Workers integration](/secrets-store/integrations/workers/)                                                                      |
| `ai_gateway` | AI Gateway                          | Associate the secret with an AI Gateway.                                  | [BYOK (Store Keys)](/ai-gateway/configuration/bring-your-own-keys/)                                                              |
| `dex`        | Digital Experience Monitoring (DEX) | Allow DEX to use the secret.                                              | None                                                                                                                             |
| `access`     | Cloudflare Access                   | Allow Cloudflare Access to use the secret.                                | None                                                                                                                             |
| `containers` | Containers                          | Configure private registry credentials or reference existing credentials. | [Configure private registry credentials](/containers/platform-details/image-management/#configure-private-registry-credentials/) |

When you create or edit a secret, use one of the supported scope values. A request to consume a secret is rejected if the service is not in the scope list, even if the caller has the correct role or API token permission. Deploying a Worker with a Secrets Store binding therefore requires both:
...
You can set scopes when [creating a secret](/secrets-store/manage-secrets/) using the dashboard, the API (`scopes` field), or Wrangler (`--scopes` flag). Use the exact scope values shown in the table.
