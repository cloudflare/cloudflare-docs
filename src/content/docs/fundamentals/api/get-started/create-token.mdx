...
## API tokens for AI coding agents

When configuring AI coding agents (such as OpenCode, Claude Code, or Cursor) to deploy Workers and manage Cloudflare resources, create a scoped API token with the permissions your agent needs.

For Workers development, use these permissions:
...
Set the token as an environment variable for Wrangler:
...
For detailed setup instructions including skill installation and MCP server configuration, refer to [AI agent setup](/workers/get-started/agent-setup/).
