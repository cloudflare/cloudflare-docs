...
R2 does not charge for egress bandwidth when accessed via the S3 API or Workers, making it a lower-cost option for serving large files. You can connect an R2 bucket to a [custom domain](/r2/buckets/public-buckets/#custom-domains) and use [Cache Rules](/cache/how-to/cache-rules/) to control caching behavior.

:::note
Serving R2 objects through a custom domain routes traffic through the Cloudflare CDN proxy. For Enterprise customers, this may have billing implications depending on your plan terms.
:::
