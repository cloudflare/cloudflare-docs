...
Because R2 does not charge for egress bandwidth, it can be a lower-cost option for serving large files through Cloudflare's CDN cache. You can connect an R2 bucket to a [custom domain](/r2/buckets/public-buckets/#custom-domains) and use [Cache Rules](/cache/how-to/cache-rules/) to control caching behavior.
