...
By default, error responses are HTML. Clients that request a structured format — such as `application/json`, `application/problem+json`, or `text/markdown` — receive a machine-readable response instead, covering all 70 [1xxx error codes](/support/troubleshooting/http-status-codes/cloudflare-1xxx-errors/) (which return HTTP 4xx or 5xx status codes depending on the error) and Cloudflare-generated [5xx errors](/support/troubleshooting/http-status-codes/cloudflare-5xx-errors/) (500, 502, 504, 520-526). Origin-generated 5xx responses that Cloudflare passes through are not affected.

:::note
This page covers the format of Cloudflare-generated error responses. For converting origin page content to Markdown, see [Markdown for Agents](/fundamentals/reference/markdown-for-agents/). For replacing default error pages with your own content, see [Custom Errors](/rules/custom-errors/).
:::
...
Structured error responses are available on all plans, including Free. [Custom Error Rules](/rules/custom-errors/#custom-error-rules) for overriding these responses require a paid plan.

---

## Interaction with Custom Errors

Structured error responses are the default for zones with no custom error configuration. Zones that use [Custom Errors](/rules/custom-errors/) retain full control over what clients see. What a client receives depends on which custom error features your zone has configured.
...
The zone has an Error Page uploaded via the Cloudflare dashboard. No Custom Error Rules are configured. The Error Page is served to all clients regardless of `Accept` header — Error Pages do not content-negotiate.
...
If you want agents to receive structured responses while keeping your custom HTML for browsers, add a Custom Error Rule that matches on the `Accept` header (see below).
...
<Details header="Example: serving custom JSON to API clients on a 522 error">
...
This rule takes priority over both the default structured JSON response and any configured Error Page. Clients that don't match the rule (for example, browsers requesting HTML) fall through to the Error Page or the default Cloudflare response.

</Details>

<Details header="Example: letting agents receive structured responses while keeping a custom HTML page for browsers">
...
If you need to keep the Error Page for browsers but want to serve custom structured content to agents, create Custom Error Rules that match on the `Accept` header and serve your own JSON or Markdown content. Browsers that don't match either rule continue to receive your custom HTML Error Page.
...
For the full priority order including account-level vs zone-level rules, WAF custom block responses, and security challenge pages, see the [Custom Errors documentation](/rules/custom-errors/).
...
| `title`    | string  | Short summary, e.g. `"Error 522: Connection timed out"`.                 |
...
| Field                   | Type            | Description                                                                                                                  |
| ----------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `error_code`            | integer         | Cloudflare error code (e.g. `522`, `1015`).                                                                                  |
| `error_name`            | string          | Machine-readable name in `snake_case` (e.g. `connection_timeout`, `rate_limited`). Stable — suitable for programmatic matching. |
| `error_category`        | string          | Fault classification. See [Error categories](#error-categories). Stable — suitable for programmatic matching.                 |
...
| Code | `retry_after` (seconds) | Error name                  |
| ---- | ----------------------- | --------------------------- |
...
curl -s --compressed -H "Accept: application/json" \
  -A "TestAgent/1.0" -H "Accept-Encoding: gzip, deflate" \
...
```bash
curl -s --compressed -H "Accept: text/markdown" \
  -A "TestAgent/1.0" -H "Accept-Encoding: gzip, deflate" \
...
curl -s --compressed -D - -o /dev/null \
  -H "Accept: application/json" -A "TestAgent/1.0" \
...
- [Markdown for Agents](/fundamentals/reference/markdown-for-agents/) (content conversion, not error responses)

### Related changelogs

- [Markdown responses for Cloudflare 1xxx errors](/changelog/post/2026-02-26-markdown-responses-for-1xxx-errors/) (Feb 26, 2026)
- [JSON responses and RFC 9457 support for Cloudflare 1xxx errors](/changelog/post/2026-03-11-json-rfc9457-responses-for-1xxx-errors/) (Mar 11, 2026)
- [Retry-After HTTP header for retryable 1xxx errors](/changelog/post/2026-03-12-retry-after-header-for-1xxx-errors/) (Mar 12, 2026)
- [Structured error responses for Cloudflare 5xx errors](/changelog/post/2026-04-27-structured-responses-for-5xx-errors/) (Apr 27, 2026)
