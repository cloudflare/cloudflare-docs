...
When you enable a zone hold, no one else can [add your zone](/fundamentals/manage-domains/add-site/) to their Cloudflare account. If they attempt to, they will receive the following message:
...
:::note
A zone hold also blocks operations such as adding a [Pages custom domain](/pages/configuration/custom-domains/) or custom hostname for a hostname under the hold, even within the same Cloudflare account. To configure a custom domain on a held zone or subdomain, temporarily [release the zone hold](#release-zone-holds) during setup.
:::

## Release zone holds

You may want to temporarily release a zone hold to allow another team to [register a subdomain](/dns/zone-setups/subdomain-setup/) in a separate Cloudflare account (such as `docs.example.com`), or to add a [Pages custom domain](/pages/configuration/custom-domains/) under a held domain.
