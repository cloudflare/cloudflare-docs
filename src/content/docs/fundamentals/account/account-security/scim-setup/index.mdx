...
- You must be a [Super Administrator](/fundamentals/manage-members/roles/) of the account.
...
1. [Create an API token](/fundamentals/api/get-started/create-token/) with the following permissions:
   ...
   Account Owned API Tokens are recommended for SCIM Provisioning. Using user-specific API tokens, while supported, will lead to a broken SCIM connection in the event that the user's policies are revoked from the account with the SCIM integration, or the [API access](/fundamentals/api/how-to/control-api-access/) is unexpected disabled. Learn more about [account owned tokens](/fundamentals/api/get-started/account-owned-tokens/).
   :::
