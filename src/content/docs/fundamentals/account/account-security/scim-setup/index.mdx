...
1. A Cloudflare account can receive user group provisioning from the identity provider.
2. Members of each user group can be assigned one or more [policies](/fundamentals/manage-members/policies/). Each policy defines one or more [roles](https://developers.cloudflare.com/fundamentals/manage-members/roles/) applied to all group members thereof.
3. Members can belong to multiple user groups, each group can also be configured with different policies.
4. Policies provisioned via SCIM can coexist with policies configured via the [traditional setup](/fundamentals/manage-members/manage/#edit-member-permissions).
