...
* A Cloudflare account has already been created with your Google Account's email: At this time, this option cannot be used, but we are working on the capability to link and de-link Social Login providers to your Cloudflare User

* Clicking "Sign in with Google" with an email that doesn't already have a Cloudflare account associated will create a new Cloudflare account, and allow you to sign in with the "Sign in with Google" button moving forward
