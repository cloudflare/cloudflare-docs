...
Organization audit logs capture Organization-level activity such as member invitations, account assignments, policy sharing changes, and Organization setting updates.

1. Go to **Organizations** > **Manage Organization**.
2. Select **Audit Logs**.
3. Review the log entries. Each entry includes the action taken, the user who performed it, and a timestamp.

Organization audit logs are separate from [account-level audit logs](/fundamentals/setup/account/account-audit-logs/). Account-level audit logs capture activity within individual accounts, while Organization audit logs capture activity at the Organization level.
