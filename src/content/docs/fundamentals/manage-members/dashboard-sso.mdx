...
Once you have configured an IdP in Zero Trust, head over to the account members settings page to manage SSO connectors.

<DashButton url="/?to=/:account/members/settings" />

If step 1 was successful, a button to add a new SSO domain will be present. Click the button to begin the process of adding a new SSO domain.

![Screenshot of the SSO connector create modal](~/assets/images/fundamentals/members/sso/create_modal.png)

Enter your email domain and click **Create** to move to the verification step.

:::note
Some top level domains, such as `.edu`, are prohibeted from being used as SSO domains.
:::
...
If the verification process fails due to timeout, you may manually reinitiate the polling by clicking **Begin verification** in the actions menu for that connector in the list.
...
Enable the connector by clicking **Enable** in the actions menu for that connector in the list.
...
Navigate to the account members settings page.

<DashButton url="/?to=/:account/members/settings" />

Click the actions menu for the SSO connector in the list and select **Disable**. Type the domain of the connector and click confirm to complete the disable action.
...
1. Navigate to the account members settings page.
