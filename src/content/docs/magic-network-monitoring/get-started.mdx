...
- [VPC flow log guide](#vpc-flow-log-guide)
...
Verify that your AWS account is capable of exporting AWS VPC flow logs via AWS Firehose.

:::note
Magic Network Monitoring only supports VPC flow log ingestion for AWS.
:::
...
```sh
cloudflared access curl https://fbm-config.cfdata.org/accounts/<account?id>/vpc-flows/token -X POST
```
