...
Magic Network Monitoring is generally available to everyone with a Cloudflare account by default. You can login to your Cloudflare dashboard, select your account, then go to **Analytics & Logs** > **Magic Monitoring** to get started.

Everyone can use the [free version](/magic-network-monitoring/free-version) of Magic Network Monitoring in a home network, network lab, or business to get end to end visibility across their network traffic. Potential enterprise customers are encouraged to use the free version to run a proof of concept.
