...
description: >-
  Cloudflare Magic Network Monitoring encrypts network flow data. Customers can encrypt the network flowData sent from their router to Cloudflare by routing their network traffic through a device running the WARP client. Then, encrypted network flow traffic can be forwarded from the device to Cloud Flare.
---
