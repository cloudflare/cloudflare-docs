...
| **Prefix Match** | The field `prefix_match` determines how IP matches are handled. <br /> **Recommended**: Subnet — Automatically advertise if the attacked IPs are within a subnet of a public IP prefix that can be advertised by Magic Transit. <br /> **Other prefix match options**: Exact — Automatically advertise if the attacked IPs are an exact match with a public IP prefix that can be advertised by Magic Transit. |
