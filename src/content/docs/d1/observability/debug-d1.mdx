...
| `too many columns on <table>: SQLITE_ERROR`                                 | A `CREATE TABLE` statement specified more columns than D1's [per-table column limit](/d1/platform/limits/).                                                               | App action: Reduce the number of columns to within the [per-table column limit](/d1/platform/limits/), or split the schema across multiple related tables. |
| `too many SQL variables at offset N: SQLITE_ERROR`                          | A prepared statement bound more parameters than D1's [per-query bound-parameter limit](/d1/platform/limits/).                                                             | App action: Reduce the bound parameters to within the [per-query bound-parameter limit](/d1/platform/limits/); for bulk inserts, split into multiple `db.batch()` statements. |
...
* Use [`wrangler dev`](/workers/wrangler/commands/general/#dev) to run your Worker and D1 locally and [debug issues before deploying](/workers/development-testing/).
