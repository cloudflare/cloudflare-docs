...
`wrangler d1 insights` is an experimental Wrangler command, and its options and output may change.

Run `wrangler d1 insights --help` to view the current options.
