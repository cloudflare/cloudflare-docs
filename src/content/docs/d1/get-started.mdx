...
To bind your D1 database to your Worker, copy the following lines from your terminal obtained from step 2, then add the following to the end of your `wrangler.toml` file:
...
- Your binding is available in your Worker at `env.binding` [^1] and the D1 [client API](/d1/build-with-d1/d1-client-api/) is exposed on this binding.

[^1]: Note that `binding` refers to your specific binding name. For example, `env.DB`.
...
You can also bind your D1 database to a [Pages Function](/pages/functions/index). For more information, refer to [Functions Bindings for D1](/pages/functions/bindings/#d1-databases).
...
To deploy your Worker to production, you must first repeat the [database bootstrapping](/d1/get-started/#configure-your-d1-database) steps after replacing the `--local` flag with the `--remote` flag to give your Worker data to read. This will create the database tables and import the data into the production version of your database, running on Cloudflare's global network.
