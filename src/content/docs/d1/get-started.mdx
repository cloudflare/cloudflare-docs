...
1. Create a new project named `d1-tutorial` by running:
    ...
    - A [`wrangler.toml / wrangler.json(c)` file](/workers/wrangler/configuration/). This file is how your `d1-tutorial` Worker accesses your D1 database.
...
After correctly preparing your `wrangler.toml / wrangler.json(c)` file, set up your database. Use the example `schema.sql` file below to initialize your database.
...
3. Paste the following code snippet into your `index.ts` file:
    ...
    1. Define a binding to your D1 database in your TypeScript code. This binding matches the `binding` value you set in the `wrangler.toml / wrangler.json(c)` file under `[[d1_databases]]`.
