...
import { Render, PackageManagers, Steps, FileTree, Tabs, TabItem } from "~/components";
...
<Steps>
1. Log in to your [Cloudflare dashboard](https://dash.cloudflare.com/sign-up/workers-and-pages).
2. From the menu, go to your account > **Workers & Pages** > **Overview**.
3. Select **Create**.
4. Select **Create Worker**.
5. Name for your Worker. For this tutorial, name your Worker "d1-tutorial".
6. Select **Deploy**.
</Steps>
</TabItem> </Tabs>
...
4. (Optional) Provide a location hint. Location hint is an optional parameter you can provide to indicate your desired geographical location for your database. See [Provide a location hint](/d1/configuration/data-location/#provide-a-location-hint).
5. Select **Create**.
...
<Steps>
1. From the menu, go to your account > **Workers & Pages** > **Overview**.
...
5. From the **Add a resource binding** panel, select **D1 database**.
6. Name your binding in the **Variable name** field, then select the "prod-d1-tutorial" D1 database you created in [step 2](/d1/get-started/#2-create-a-database) from the dropdown menu. For this tutorial, name your binding "DB".
...
4. Name your table. For this tutorial, create a table called "Customers".
5. Create three columns (add more columns by selecting **+ Add Column**):
	1. Column Name: `CustomerId`, Type: `integer`, Default Value: `NULL` (leave blank). Set this column as a primary key.
	2. Column Name: `CompanyName`, Type: `text`, Default Value: `NULL` (leave blank).
	3. Column Name: `ContactName`, Type: `text`, Default Value: `NULL` (leave blank).
6. Select **Create**.
7. Select the **Customers** table.
8. Select **Add data** to populate the columns of the table. Select **Save** to add the row to the table. Add the following rows:
	1. CustomerId: `1`, CompanyName: `Alfreds Futterkiste`, ContactName: `Maria Anders`.
	2. CustomerId: `4`, CompanyName: `Around the Horn`, ContactName: `Thomas Hardy`.
	3. CustomerId: `11`, CompanyName: `Bs Beverages`, ContactName: `Victoria Ashworth`.
	4. CustomerId: `13`, CompanyName: `Bs Beverages`, ContactName: `Random Name`.
</Steps>
</TabItem>
</Tabs>
...
<Steps>
1. Go to your account > **Workers & Pages** > **Overview**.
...
To deploy your Worker to production using Wrangler, you must first repeat the [database bootstrapping](/d1/get-started/#configure-your-d1-database) steps after replacing the `--local` flag with the `--remote` flag to give your Worker data to read. This creates the database tables and imports the data into the production version of your database.
...
<Steps>
1. Go to your account > **Storages & Databases** > **D1**.
