...
In this function, you accept a `slug` URL query parameter and set up a new SQL statement where you select all comments with a matching `post_slug` value to your query parameter. The code also includes optional pagination support with `page` and `limit` query parameters. You can then return it as a JSON response.
...
app.post("/api/posts/:slug/comments", async (c) => {
	try {
		...
		if (body.length > 5000) {
			return c.json(
				{ error: "Comment body too long (max 5000 characters)" },
				400,
			);
		}
		...
		const { success, meta } = await c.env.DB.prepare(
			`INSERT INTO comments (author, body, post_slug, created_at) 
			 VALUES (?, ?, ?, ?)`,
		)
			.bind(author.trim(), body.trim(), slug, Date.now())
			.run();
...
For production applications, consider using a validation library like [zod](https://zod.dev/) for more robust input validation:
...
// In your handler:
const validation = CommentSchema.safeParse(await c.req.json());
