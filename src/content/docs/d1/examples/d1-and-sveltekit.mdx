...
export const GET: RequestHandler = async ({ platform }) => {
	// Verify the platform binding exists
	if (!platform?.env?.DB) {
	...
	try {
		// Destructure the success flag from D1 response
		const {
			results,
			success,
			error: dbError,
		...
		// Use SvelteKit's json helper for proper response formatting
		return json({ results });
	} catch (e) {
		console.error("Error querying D1:", e);

		// Re-throw SvelteKit errors, wrap others
