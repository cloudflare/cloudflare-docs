...
app.get("/query/users/:id", async (c) => {
	...
	// Validate input: user IDs should be numeric to prevent SQL injection attempts
	// Even though we use prepared statements with bind(), validating input is a best practice
	if (!userId || isNaN(Number(userId))) {
		return c.json({ error: "Invalid user ID" }, 400);
	}

	try {
		// Destructure all relevant properties from the D1 response
		// The success flag tells us if the query executed without errors
		...
		// Always check the success flag before returning results
		// This ensures we don't return undefined or partial data
		if (!success) {
		...
		// Log the full error for debugging but return a generic message to clients
		// This prevents leaking implementation details that could be security risks
...
// Add TypeScript types for Pages Functions bindings
// This ensures type safety when accessing environment bindings
type Bindings = {
	DB: D1Database;
};
