...
- Ensure you have configured [[Wrangler configuration](/workers/wrangler/configuration) file](/d1/get-started/#3-bind-your-worker-to-your-d1-database) to bind your D1 database to your Worker.
...
2. Pass the `--d1 BINDING_NAME=DATABASE_ID` flag to `wrangler dev` when developing locally. `BINDING_NAME` should match what call in your code, and `DATABASE_ID` should match the `database_id` defined in your [Wrangler configuration](/workers/wrangler/configuration) file: for example, `--d1 DB=xxxx-xxxx-xxxx-xxxx-xxxx`.
