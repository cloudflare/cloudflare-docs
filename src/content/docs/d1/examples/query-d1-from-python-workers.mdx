...
class Default(WorkerEntrypoint):
    async def fetch(self, request):
        ...
        try:
            ...
            # The run() method returns a dict with 'success', 'results', and 'error' keys
            result = await self.env.DB.prepare("PRAGMA table_list").run()

            # Always check if the query was successful before processing results
            ...
            if not result.get("success"):
                ...
                # Status 500 indicates a server error that the client can't fix
                return Response.json(
                    {"error": "Database query failed"},
                    status=500
                )

            # Return a JSON response with the successful results
            # Wrap results in an object for better API structure and future extensibility
            return Response.json({"results": result.get("results")})

        except Exception as e:
            # Catch any unexpected errors (network issues, binding problems, etc.)
            # Log the error for debugging but return a generic message to clients
