...
This example assumes you have an existing D1 database. To allow your Python Worker to query your database, you first need to create a [binding](/workers/runtime-apis/bindings/) between your Worker and your D1 database and define this in your Wrangler configuration file.
...
- Make sure you have configured your Wrangler configuration file with the `database_id` and `database_name` of a valid D1 database.
- Ensure `compatibility_flags = ["python_workers"]` is set in your Wrangler configuration file, which is required for Python.
