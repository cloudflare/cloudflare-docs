...
D1 [bills by the number of rows read and rows written](/d1/platform/pricing/), not by the number of rows your query returns. A query that scans an entire table to return a single row is billed for every row it scans. Adding an index so that D1 can jump straight to the rows it needs is therefore one of the most effective ways to reduce both latency **and** cost.

Most unexpectedly large D1 bills come from a small number of frequently run queries that each read (or write) far more rows than they return. The table below lists common patterns that touch too many rows, along with how to fix them. Use [`EXPLAIN QUERY PLAN`](#test-an-index) to check whether a query does a full `SCAN` (reads every row) or a `SEARCH ... USING INDEX` (jumps to matching rows).
...
| Re-running `CREATE INDEX` (or other schema changes) on every request                    | Building an index writes a row for every row it indexes, and writes are billed at a higher rate than reads. Doing this per request repeats that cost.                    | Run schema changes once with [D1 migrations](/d1/reference/migrations/), not on your application's hot path.                                                                                                                                                                                                                                                                                                                                                                                      |
...
- Indexes cannot be updated. To add or remove a column from an index, [remove](#remove-indexes) the index and then [create a new index](#create-an-index) with the new columns. Apply these schema changes once through a versioned system such as [D1 migrations](/d1/reference/migrations/).
