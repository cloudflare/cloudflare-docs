...
// Get bookmark for sequential consistency in future requests
...
// Get bookmark to ensure future reads see at least this version
const bookmark = session.getBookmark();
```
...
const previousBookmark = request.headers.get("x-d1-bookmark") ?? "first-unconstrained";

const session = env.DB.withSession(previousBookmark);
const result = await session
	.prepare(`SELECT * FROM Customers WHERE CompanyName = 'Bs Beverages'`)
	.run();

// Store updated bookmark for the next request
...
#### Complete example with bookmark persistence

Here's a practical example showing how to use bookmarks across multiple requests:
...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		try {
			// Example: List products (use primary for latest data)
			if (url.pathname === "/api/products") {
				const session = env.DB.withSession("first-primary");

				const { results } = await session
					.prepare("SELECT * FROM products WHERE active = 1 ORDER BY created_at DESC LIMIT 20")
					.run();
			...
			// Example: Get product details (use bookmark from previous request)
