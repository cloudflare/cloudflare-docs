...
The `exec()` method does NOT support parameter binding and should only be used for:

- Schema migrations
- One-time administrative tasks
- Queries without user input

**Never use `exec()` with user-provided data.** This creates SQL injection vulnerabilities. Always use `prepare()` + `bind()` for queries involving user input.
:::

<Tabs syncKey="workersExamples"> <TabItem label="JavaScript" icon="seti:javascript">
```js
// ✅ Acceptable use case: Schema migration
const returnValue = await env.DB.exec(`
  CREATE INDEX IF NOT EXISTS idx_customers_name 
  ON Customers(CompanyName)
`);

// ❌ NEVER DO THIS with user input
// const userInput = request.query.get('company');
// await env.DB.exec(`SELECT * FROM Customers WHERE CompanyName = "${userInput}"`); // SQL INJECTION RISK!

// ✅ For user input, use prepare() + bind() instead
// const stmt = env.DB.prepare("SELECT \* FROM Customers WHERE CompanyName = ?");
// const result = await stmt.bind(userInput).run();

````
</TabItem> <TabItem label="Python" icon="seti:python">
```py
# ✅ Acceptable use case: Schema migration
return_value = await self.env.DB.exec('''
  CREATE INDEX IF NOT EXISTS idx_customers_name
  ON Customers(CompanyName)
''')

# ❌ NEVER DO THIS with user input
# user_input = request.query.get('company')
# await self.env.DB.exec(f'SELECT * FROM Customers WHERE CompanyName = "{user_input}"')  # SQL INJECTION RISK!

# ✅ For user input, use prepare() + bind() instead
# stmt = self.env.DB.prepare("SELECT * FROM Customers WHERE CompanyName = ?")
# result = await stmt.bind(user_input).run()
````
...
- **Important**: `exec()` does not support parameter binding. Use `prepare()` with `bind()` for any queries that include dynamic values or user input.
