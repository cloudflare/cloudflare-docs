...
- For TCP services, include the port in `--hostname` using `host:port` syntax (for example, `--hostname db.internal:5432`) instead of specifying `--tcp-port` separately
...
If you encounter permission errors, refer to [Required roles](/workers-vpc/configuration/vpc-services/#required-roles).
...
Your Worker is now deployed and can access your private network resources securely through the Cloudflare Tunnel. If you encounter permission errors, refer to [Required roles](/workers-vpc/configuration/vpc-services/#required-roles).
