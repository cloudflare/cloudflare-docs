...
Returns a `Promise<Socket>` that resolves to a [`Socket`](/workers/runtime-apis/tcp-sockets/#socket) with `readable` and `writable` streams. If the connection cannot be established, `connect()` throws an exception.
