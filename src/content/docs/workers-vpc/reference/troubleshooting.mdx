...
Errors are grouped into three categories based on the likely cause:

- **Service Unreachable** — Your tunnel or private service is not reachable. Check tunnel health, service availability, and network/TLS configuration.
- **Configuration Error** — Your VPC Service configuration or Worker code caused the failure. Check your target hostname and Worker request behavior.
- **Internal Error** — A Cloudflare infrastructure issue. Contact Cloudflare support if this persists.
...
| `dns_error` | DNS resolution failed (for example, SERVFAIL). | Check that the hostname configured for your VPC Service is resolvable from within your private network. Verify your DNS resolver is working correctly. See [Tunnel errors](#tunnel-errors) for common DNS causes. |
...
These errors indicate an issue within Cloudflare's infrastructure that is not caused by your configuration.
