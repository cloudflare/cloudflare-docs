...
### Connect Containers to private services

Run existing containerized applications on Cloudflare and connect them to private APIs, databases, and services in your VPC — without exposing public endpoints. Use outbound Workers to intercept container HTTP requests and route them through Workers VPC. Refer to the [Connect from Containers](/workers-vpc/examples/connect-from-containers/) example.
...
Run containerized applications on Cloudflare. Combine with Workers VPC to connect containers to private services in your network.
