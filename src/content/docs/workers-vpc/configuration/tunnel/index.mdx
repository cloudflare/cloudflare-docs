...
When you create a VPC Service or VPC Network, you specify a tunnel ID. Workers VPC then routes requests from your Worker to the specified tunnel, which establishes a connection to your private network such that your services receive the request and return a response back to your Worker.

To allow members to create VPC Services or VPC Networks, you must assign them the **Connectivity Directory Admin** role. Members must possess **Connectivity Directory Bind** role to bind to existing VPC Services or VPC Networks from Workers.
...
Workers VPC also requires Cloudflare Tunnel to connect using the [QUIC transport protocol](/cloudflare-one/networks/connectors/cloudflare-tunnel/configure-tunnels/run-parameters/#protocol) using `auto` or `quic`. Ensure outbound UDP traffic on port 7844 is allowed through your firewall for QUIC connections.
:::
...
- Configure [VPC Services](/workers-vpc/configuration/vpc-services/) or [VPC Networks](/workers-vpc/configuration/vpc-networks/) to connect your tunnels to Workers
