...
This example demonstrates how to use a VPC Network binding with [Cloudflare Mesh](/mesh/) (formerly WARP Connector) to connect to any private service in your account from a Worker — without pre-registering individual hosts or specifying a Cloudflare Tunnel UUID.

When you bind to [Cloudflare Mesh](/mesh/) using `network_id: "cf1:network"`, your Worker can reach any Mesh node, client device, subnet or hostname route announced through Cloudflare Tunnel or Cloudflare Mesh, or destination connected through a [Cloudflare WAN](/cloudflare-wan/) on-ramp (GRE, IPsec, or CNI).

## Prerequisites

- A [Cloudflare Mesh](/mesh/) node connected to your private network
...
- [Set up Cloudflare Mesh](/mesh/get-started/) for your account
