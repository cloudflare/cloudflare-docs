...
Queues supports three types of consumer:
...
3. A [notification consumer](/queues/configuration/notification-consumers/), which sends messages to email, PagerDuty, or webhook destinations through Cloudflare Notifications.

A queue can only have one active consumer configured.
...
A queue can send messages to email, PagerDuty, or webhook destinations through Cloudflare Notifications. Review [Notification consumers](/queues/configuration/notification-consumers/) to learn how to configure notification delivery.
