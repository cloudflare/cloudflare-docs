...
Choose a consumer based on how and where you process messages.

- Start with a [consumer Worker](/queues/reference/how-queues-works/#consumers) to automatically process and scale as messages arrive.
...
- Use a [notification consumer](/queues/configuration/notification-consumers/) to send messages to email, PagerDuty, or webhook destinations.

You can remove and attach a new consumer at any time.
...
You can enable HTTP pull or change a queue's consumer via the `wrangler` CLI or the [Cloudflare dashboard](https://dash.cloudflare.com/). Enabling HTTP pull from a [Wrangler configuration file](/workers/wrangler/configuration/) is no longer supported.
...
If you have an existing consumer, you will need to remove it first. `wrangler` will return an error if you attempt to call `consumer http add` on a queue with an existing consumer configuration. For example, remove a consumer Worker with:
