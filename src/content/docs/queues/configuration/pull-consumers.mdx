...
You can enable HTTP pull or change a queue from push-based to pull-based via the Wrangler configuration file, the `wrangler` CLI, or via the [Cloudflare dashboard](https://dash.cloudflare.com/).

### Wrangler configuration file

A HTTP consumer can be configured in the Wrangler configuration file by setting `type = "http_pull"` in the consumer configuration:
...
If you remove the Worker consumer with `wrangler` but do not delete the `[[queues.consumer]]` configuration from Wrangler configuration file, subsequent deployments of your Worker will fail when they attempt to add a conflicting consumer configuration.
