...
A queue can send messages to email, PagerDuty, or webhook destinations through Cloudflare Notifications. Notification consumers can be configured with the dashboard or Wrangler, but not with a Wrangler configuration file. Refer to [Notification consumers](/queues/configuration/notification-consumers/) for setup instructions.
