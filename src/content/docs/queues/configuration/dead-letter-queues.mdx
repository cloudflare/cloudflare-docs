...
The DLQ must use the same [jurisdiction](/queues/configuration/data-location/) as its source queue. If you specify a queue name that does not exist as the DLQ, Cloudflare automatically creates that queue in the same jurisdiction as the source queue.
