...
Delays can also be configured in the [`wrangler.toml / wrangler.json(c)` file](/workers/wrangler/configuration/#queues) with the `delivery_delay` setting for producers (when sending) and/or the `retry_delay` (when retrying) per-consumer:
...
If you use both the `wrangler` CLI and the `wrangler.toml / wrangler.json(c)` file to change the settings associated with a queue or a queue consumer, the most recent configuration change will take effect.
