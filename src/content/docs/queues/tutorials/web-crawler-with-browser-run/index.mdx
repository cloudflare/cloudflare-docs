...
summary: Example of how to use Queues and Browser Run to power a web crawler.
pcx_content_type: tutorial
products: [workers, queues, browser-run]
tags:
  - TypeScript
sidebar:
  order: 1002
head:
  - tag: title
    content: Cloudflare Queues - Queues & Browser Run
description: Example of how to use Queues and Browser Run to power a web crawler.
---

import { Render, PackageManagers, WranglerConfig } from "~/components";

This tutorial explains how to build and deploy a web crawler with Queues, [Browser Run](/browser-run/), and [Puppeteer](/browser-run/puppeteer/).
...
In your current directory, install Cloudflare's [fork of Puppeteer](/browser-run/puppeteer/) and also [robots-parser](https://www.npmjs.com/package/robots-parser):
...
Adding the `max_batch_timeout` of 60 seconds to the consumer queue is important because it allows the Queue to collect messages into a batch over a longer period. This helps manage Browser Run [rate limits](/browser-run/limits/) and can improve efficiency by processing multiple URLs in a single batch with one browser instance.
