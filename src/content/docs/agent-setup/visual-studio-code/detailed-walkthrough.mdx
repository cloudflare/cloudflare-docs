...
For the condensed version, see the [Visual Studio Code quick start](/agent-setup/visual-studio-code/).
...
7. **Choose an access template**
	...
	Two things to know before you authorize:

	- **Full access** lets the agent create and delete real resources, which is why this belongs on a demo account.
	- You can revoke the grant at any time from the dashboard under **My Profile** > **Access Management** > **Connected Applications**. Visit that page if a re-authorization behaves unexpectedly.
...
12. **Create a DNS record**
	...
	Ask the agent to create a DNS AAAA record and enable the Cloudflare proxy. This example points `test.cloudflaredemo.net` at `100::`, an IPv6 blackhole route from the [documentation-only prefix](https://www.rfc-editor.org/rfc/rfc9780.html#section-7.1-1). Approve the tool call (**Always Allow**), and the agent reports the record created.
