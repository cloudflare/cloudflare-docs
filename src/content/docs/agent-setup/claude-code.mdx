...
- Claude Code works best from the root of your Workers project where `wrangler.jsonc` lives — that's what the agent uses to understand your bindings.
...
<FAQList>
	...
	<FAQItem question="How do I give Claude Code access to my Cloudflare account?">
		The first time Claude calls a Cloudflare tool, you'll be redirected to
...
<TroubleshootingList>
	...
	<TroubleshootingItem issue="Claude does not understand Workers APIs">
		Connect the docs MCP server: `claude mcp add cloudflare-docs -- npx
		mcp-remote https://docs.mcp.cloudflare.com/mcp`. This gives Claude access to
		up-to-date Cloudflare documentation.
	</TroubleshootingItem>
	<TroubleshootingItem issue="Deployment fails with permission error">
		Verify your Cloudflare OAuth permissions include Workers write access.
		Re-authorize via the MCP server if needed, or check your API token scopes.
	</TroubleshootingItem>
</TroubleshootingList>
