...
Cloudflare sends all isolated traffic through our Secure Web Gateway inspection engine, which allows you to apply [Gateway HTTP policies](/cloudflare-one/traffic-policies/policy-types/http-policies/) such as:

- Restrict specific actions and HTTP request methods.
- Inspect the request body to match against [Data Loss Prevention](/cloudflare-one/data-loss-prevention/) (DLP) profiles with as much specificity and control as if the user had deployed an endpoint agent.
...
|        | Require   | [Device Posture - Serial Number List](/cloudflare-one/reusable-components/posture-checks/client-checks/corp-device/) | `Corporate serial numbers` |
...
| [DLP Profile](/cloudflare-one/data-loss-prevention/dlp-profiles/) | in       | _Financial Information_    | And   |        |
