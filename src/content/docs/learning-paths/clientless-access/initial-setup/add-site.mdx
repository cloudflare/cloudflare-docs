...
In clientless access deployments, users connect to internal applications via public hostnames. You will need to own a domain, add it to Cloudflare, and configure Cloudflare as the [authoritative DNS provider](/dns/zone-setups/full-setup/setup/#34-update-your-registrar) for that domain. Enterprise customers who cannot change their authoritative DNS provider have the option to configure a [CNAME setup](/dns/zone-setups/partial-setup/).
...
6. Before your domain can begin using Cloudflare for DNS resolution, you need to [add these nameservers](/dns/nameservers/update-nameservers/) at your registrar. Make sure [DNSSEC](/dns/dnssec/) is turned off before proceeding.
