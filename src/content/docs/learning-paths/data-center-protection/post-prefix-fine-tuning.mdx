...
Refer to Cloudflare Network Firewall's [best practices](/cloudflare-network-firewall/best-practices/) for configuration guidance and suggestions.
...
- Refer to [Set up tunnel health alerts](/learning-paths/data-center-protection/enable-notifications/#set-up-tunnel-health-alerts) and [DDoS alerts](/ddos-protection/reference/alerts/) for more details.

## Optional

- Enable [Logpush](/logs/logpush/) to your Security Information and Event Management (SIEM).
- Enable Cloudflare Network Firewall's [Intrusion Detection System (IDS)](/cloudflare-network-firewall/about/ids/). Requires Logpush and is only available for accounts with [Cloudflare Advanced Network Firewall](/cloudflare-network-firewall/plans/#advanced-features).
- Use [Network Flow](/network-flow/) (formerly Magic Network Monitoring) for visibility into traffic on your non-Magic Transit prefixes, using NetFlow or sFlow from your CPEs.
