...
### Devices within the Magic Transit prefix are unable to reach any remote IPs on the Internet after the prefix was cut over to Cloudflare, regardless of traffic type (UDP, TCP, or ICMP)
...
### Devices on the Internet cannot access a TCP service on your Magic Transit prefix — for example, browsing to a server which is hosted on the Magic Transit prefix
...
### Users with traffic passing via IPsec or GRE tunnels between the Magic Transit prefix and third parties are reporting issues
...
### You suspect that Cloudflare mitigations are dropping legitimate traffic to your Magic Transit prefix
