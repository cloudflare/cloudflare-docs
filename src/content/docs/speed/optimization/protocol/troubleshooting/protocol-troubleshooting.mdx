...
If the issue reproduces only in Chrome over HTTP/3 and disappears when HTTP/3 is disabled, the problem may be related to a browser-side QUIC handling issue rather than your origin server. This is a known Chrome issue (Chromium #41161335) — Cloudflare's QUIC implementation is not the cause.
...
**Test immediately:** Disable QUIC in Chrome via `chrome://flags` → search "QUIC" → set to Disabled → Relaunch.
...
2. Set the matching expression to your hostname: `(http.host eq "example.com")`.
3. Set the action to **Remove** the `alt-svc` header.
