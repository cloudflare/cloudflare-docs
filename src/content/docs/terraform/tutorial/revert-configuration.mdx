...
Push your revert commit:
```bash
git push
```

### Alternative: Reset to a specific commit
If you need to go back multiple commits, you can reset to a specific point:
```bash
# Go back to before load balancing was added
git reset --hard a7b8c9d

# Force push (use with caution in team environments)
git push --force-with-lease
```
**Warning**: git reset --hard and force pushing rewrites history. Use git revert for shared repositories where others may have pulled your changes.
