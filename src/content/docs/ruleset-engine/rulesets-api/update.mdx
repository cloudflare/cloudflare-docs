...
To deploy a ruleset, add a rule with `"action": "execute"` to the list of rules of an [entry point ruleset](/ruleset-engine/about/rulesets/#entry-point-ruleset). Refer to [Deploy a ruleset](#example---deploy-a-ruleset) for an example.

:::caution[Risk of replacing all rules]
...
To add a single rule without replacing the entire list of rules in the ruleset, use one of the `POST` requests described in [Add a rule to a ruleset](/ruleset-engine/rulesets-api/add-rule/) instead.
