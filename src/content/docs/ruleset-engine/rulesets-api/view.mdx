...
:::note
You can only use the [Get a zone ruleset](/api/resources/rulesets/methods/get/) operation for zone-level phase entry point rulesets (entry points where `kind` is set to `zone`).
:::
