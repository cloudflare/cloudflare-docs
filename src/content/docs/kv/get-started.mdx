...
<Steps>
1. Go to the KV namespace you created, then select **View**.
2. Select **KV Pairs**.
3. Enter a `<KEY>` of your choice.
4. Enter a `<VALUE>` of your choice.
5. Select **Add entry**.
</Steps>
...
2. Go to the KV namespace you created, then select **View**.
3. Select **KV Pairs**.
</Steps>
</TabItem></Tabs>
...
6. Name your binding in **Variable name**, then select the KV namespace you created in [step 2](/kv/get-started/#2-create-a-kv-namespace) from the dropdown menu.
...
1. Use the `put()` method on `YOUR_KV_NAMESPACE` to create a new key-value pair, or to update the value for a particular key:
