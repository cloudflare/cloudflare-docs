...
1. Create a new project named `kv-tutorial` by running:
    ...
    - A [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/) configuration file. [Wrangler configuration](/workers/wrangler/configuration) file is how your `kv-tutorial` Worker accesses your kv database.
...
1. Open your terminal and run the following command:
    ...
    The `npx wrangler kv namespace create <BINDING_NAME>` subcommand takes a new binding name as its argument. A KV namespace is created using a concatenation of your Worker’s name (from your [Wrangler configuration](/workers/wrangler/configuration) file) and the binding name you provide. A `BINDING_ID` is randomly generated for you.
...
A binding is how your Worker interacts with external resources such as [KV namespaces](/kv/concepts/kv-namespaces/). A binding is a runtime variable that the Workers runtime provides to your code. You can declare a variable name in your [Wrangler configuration](/workers/wrangler/configuration) file that binds to these resources at runtime, and interact with them through this variable. Every binding's variable name and behavior is determined by you when deploying the Worker.
...
To have `wrangler dev` connect to your Workers KV namespace running on Cloudflare's global network, call `wrangler dev --remote` instead. This uses the `preview_id` of the KV binding configuration in the [Wrangler configuration](/workers/wrangler/configuration) file. See the [KV binding docs](/kv/concepts/kv-bindings/#using-the-kv-binding-when-developing-locally) for more information.
