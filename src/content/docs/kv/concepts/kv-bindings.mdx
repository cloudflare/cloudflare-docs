...
In the following example, the binding is called `TODO`. In the `kv_namespaces` portion of your [Wrangler configuration](/workers/wrangler/configuration) file, add:
...
To have `wrangler dev` connect to your Workers KV namespace running on Cloudflare's global network, call `wrangler dev --remote` instead. This will use the `preview_id` of the KV binding configuration in the [Wrangler configuration](/workers/wrangler/configuration) file. This is how a [Wrangler configuration](/workers/wrangler/configuration) file looks with the `preview_id` specified.
