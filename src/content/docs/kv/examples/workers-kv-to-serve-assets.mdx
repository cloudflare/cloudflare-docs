...
2. In your [Wrangler configuration](/workers/wrangler/configuration) file, add the following with the values generated in the terminal:
   ...
   We'll also create a preview KV namespace. It is recommended to create a separate KV namespace when developing locally to avoid making changes to the production namespace. When developing locally against remote resources, the Wrangler CLI will only use the namespace specified by `preview_id` in the KV namespace configuration of the [Wrangler configuration](/workers/wrangler/configuration) file.
...
4. In your [Wrangler configuration](/workers/wrangler/configuration) file, add the additional preview_id below kv_namespaces with the values generated in the terminal:
...
This will create a KV pair with the filename as key and the file content as value, within the our production and preview namespaces specified by your binding in your [Wrangler configuration](/workers/wrangler/configuration) file.
...
Wrangler will automatically set your KV binding to use the production KV namespace set in our [Wrangler configuration](/workers/wrangler/configuration) file with the KV namespace id. Throughout this example, we uploaded our assets to both the preview and the production KV namespaces.
