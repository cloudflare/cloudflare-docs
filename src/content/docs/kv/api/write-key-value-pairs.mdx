...
To associate some <GlossaryTooltip term="metadata">metadata</GlossaryTooltip> with a key-value pair, set `metadata` to any arbitrary object (must serialize to JSON) in the `put()` options object on a `put()` call.
