...
If disabled, Zaraz will not automatically trigger the `Pageview` action immediately upon user's visit. You can then run `zaraz.track("Pageview")` manually somewhere in the browserside scripts in order to register this event. This can be useful when integrating custom consent solutions – see [Consent API](/zaraz/consent-management/api/).
