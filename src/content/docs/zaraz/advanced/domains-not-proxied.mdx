...
You can load Zaraz on domains that are not proxied through Cloudflare. However, you will need to create a separate domain, or subdomain, proxied by Cloudflare (also known as [orange-clouded](/dns/proxy-status/#benefits) domains), and load the script from it:
