...
- **Per-domain charges**: Argo is billed separately for each domain that has the feature turned on. If you have multiple domains with Argo, each domain has its own usage and charges.
- **Mitigated traffic excluded**: If Cloudflare mitigates attacks on your site — whether through DDoS protection, the WAF, or other mechanisms — that traffic is not included in Argo charges.
- **Cache hits included**: Argo billing includes charges for cache hits in addition to requests that reach your origin through the Cloudflare network.
- **Usage-based billing cycle**: Each invoice reflects the **prior month's** usage. For example, your September invoice includes charges for Argo usage in August.
- **Monitor your usage**: To avoid unexpected charges, set up [usage-based billing notifications](/fundamentals/notifications/notification-available/#usage-based-billing). When turned on, you receive a notification at the billing email address on file when traffic exceeds your configured threshold.
