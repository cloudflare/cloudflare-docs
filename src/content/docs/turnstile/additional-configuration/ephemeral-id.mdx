...
Ephemeral IDs use advanced analysis techniques to create unique identifiers that help identify patterns of behavior across multiple interactions. These identifiers are generated dynamically for each visitor interaction with Turnstile widgets.

- **No persistent storage**: No cookies or local storage required
- **Scoped to Cloudflare account**: Ephemeral IDs cannot be shared across accounts 
- **Short lifespan**: IDs expire within a few days
- **Non-reversible**: Cannot be used to identify individual users
