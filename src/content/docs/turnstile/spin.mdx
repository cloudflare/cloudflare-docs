...
Turnstile Spin is a setup flow for Cloudflare Turnstile. It creates the widget for you, then hands your AI coding agent the materials (sitekey, secret, and a curated prompt) to embed the widget on the right forms and wire canonical server-side siteverify into your existing backend. Spin runs two ways:
...
Both paths produce the same widget. The only difference is where the create call runs. Neither path deploys infrastructure on your behalf — Spin uses Turnstile's canonical siteverify endpoint, called from the backend you already have.
...
Whichever setup path you use, Spin gives you a sitekey and a secret. The dashboard hands you a prompt for your AI coding agent. The command-line setup edits files directly via the agent.
...
Equivalent calls in other backend languages (Ruby, Python, Go, PHP) are in the per-framework references shipped with the skill. The canonical fetch idiom works inside a Cloudflare Worker the same way it does in Node — no special binding required. The `data-action="turnstile-spin-v2"` attribute is the telemetry marker; refer to [Telemetry marker](#telemetry-marker) for the marker convention.
...
The sitekey does not change. Your existing widget keeps working throughout. The dashboard's `Deployment` column flips from `Manual` to `Spin` once the first request carrying `data-action="turnstile-spin-v2"` lands.
...
| ------------------- | ------------------------------------------------------------------------------------- |
| `turnstile-spin-v2` | Current Spin flow (dashboard **Set up with Spin** + command-line skill, all variants) |
| `turnstile-spin-v1` | Legacy V1 agent flow. Preserved if encountered on an existing widget; not used for new setups. |

Spin applies the marker automatically. If you customize the widget snippet by hand, you do not need to add the marker yourself.
