...
:::note
Responses from test secret keys always include `metadata.result_with_testing_key` set to `true`. This field is not present when using production secret keys. Your application should not rely on this field for any logic beyond debugging.
:::
