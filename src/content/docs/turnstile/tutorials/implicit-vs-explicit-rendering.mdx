...
Cloudflare offers a seamless and user-friendly alternative to traditional CAPTCHAs that verifies users without the need for annoying puzzles or challenges. In this tutorial, we’ll explore the two primary methods of implementing Turnstile in your website - **implicit vs explicit rendering**. This tutorial will provide detailed explanation, step by step implementation guide and examples to help you protect your web application security while maintaining a good user experience.

## Prerequisites

Before you begin, ensure you have the following:

- A Cloudflare account - sign up for an account if you haven't already.
- Site Key and Secret Key - obtain these from the Cloudflare dashboard after adding your site to Turnstile.
- Basic Knowledge of HTML and JavaScript - familiarity with web development will help you follow along.
...
To set up implicit rendering you can follow these steps:
...
To set up explicit rendering you can follow these steps:
...
After rendering the Turnstile widget explicitly, you may need to interact with it based on your application's requirements. Here's how you can manage the widget's state:

### Reset a Widget

To reset the widget if for example the given widget timed out or expired you can use the function:
...
## Choosing Between Implicit and Explicit Rendering

Choosing the appropriate rendering method for Turnstile is important and helps you to effectively add it to your website while ensuring optimal performance and user experience. Both implicit and explicit rendering have their own advantages and are suited to different types of web applications. Here's a comparison table that highlights the ideal use case for each of these rendering methods:
...
- [Server-side validation](/turnstile/get-started/server-side-validation/): A guide on how to implement server-side verification to ensure that only valid, human-generated responses are accepted by your application.
