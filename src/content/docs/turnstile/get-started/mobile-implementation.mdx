...
For Turnstile to function properly in any WebView, the following requirements must be met.
...
The exact configuration method varies by platform, but the principle is to ensure these domains are whitelisted for network access.
