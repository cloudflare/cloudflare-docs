...
:::note
Account takeover (ATO) detections are not available for accounts using the [Data Localization Suite](/data-localization/) with [Regional Services](/data-localization/regional-services/) in the EU region. ATO detections rely on zone logs to identify suspicious activity, and Cloudflare does not ingest logs for these accounts to comply with General Data Protection (GDPR).
:::
