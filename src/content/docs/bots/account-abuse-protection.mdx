...
:::note
Account takeover (ATO) detections are not available for accounts using the [Data Localization Suite](/data-localization/) with [Customer Metadata Boundary](/data-localization/metadata-boundary/) set to the EU region. These detections rely on analyzing zone logs in Cloudflare's central detection pipeline, which is not available when Customer Metadata Boundary keeps log processing within the EU region.
:::
