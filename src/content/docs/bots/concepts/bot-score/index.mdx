...
Bot scores are not computed for requests to paths that are handled by Cloudflare and will never be blocked or forwarded to the origin. Note that some features occurring before Bot Management, such as Redirect Rules, may result in requests not being scored.
:::
