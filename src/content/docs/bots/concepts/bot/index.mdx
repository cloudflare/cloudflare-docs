...
| **User** | Automated activity acting in real time on a person's behalf, such as browser-use agents and chat fetch bots. | ChatGPT-User, Claude-User, browser-use agents |
...
For new domains, **Training** and **User** are blocked by default **on pages that display ads**, while **Search** is allowed by default. An ad signals that a page is meant for a person to land on and see, so on those pages Cloudflare keeps off the automated traffic that does not send readers back.
