...
A bot that does not give a website owner any way to tell whether it is crawling to train a model or to build a search index does not get the benefit of the doubt — it is blocked by default on pages that display ads.
