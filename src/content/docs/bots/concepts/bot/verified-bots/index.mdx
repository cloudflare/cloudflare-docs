...
Previously, *all* verified bots were allowed by default. Starting July 1, 2026, the verified label no longer guarantees access on its own. Bots without the verified label are still blocked by default, but which verified bots are allowed by default is now governed by [behavior-based presets](/bots/concepts/bot/#ai-bots) (Training, Search, and User) rather than a single one-size-fits-all label. A verified bot that misbehaves can lose its verified status, and is then removed from the groups you allow.
:::
