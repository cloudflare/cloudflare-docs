...
Previously, *all* verified bots were allowed by default. Starting July 1, 2026, Cloudflare no longer treats the verified label as "allow everything." Bots without the verified label are still blocked by default, and a verified bot is now *allowable through its behavior category* (for example, allowing **Search**) rather than through a single one-size-fits-all label. The default allow and block baselines for these categories [take effect on September 15, 2026](/bots/concepts/bot/#ai-bots). A verified bot that misbehaves can lose its verified status, and is then removed from the groups you allow.
:::
...
| **Intermediary** | Signed agent | An agentic service that a wide range of end users can operate. The operator runs the software, but each action is initiated by a different end user. | Anchor Browser, Browserbase, Cloudflare [`/crawl`](/browser-run/quick-actions/crawl-endpoint/) |
