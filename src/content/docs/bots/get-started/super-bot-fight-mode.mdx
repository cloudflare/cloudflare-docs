...
This order has a critical impact on traffic processing. If a WAF custom rule performs a terminating action (such as _Block_, _Non-Interactive Challenge_, _Managed Challenge_, or _Interactive Challenge_), the request will not be processed further, and the Super Bot Fight Mode configuration will not be evaluated.
