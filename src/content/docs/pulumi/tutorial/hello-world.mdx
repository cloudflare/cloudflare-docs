...
With the Cloudflare package installed, you can now define any [supported Cloudflare resource](https://www.pulumi.com/registry/packages/cloudflare/) in your Pulumi program. You'll define a Worker, a Route, and a DNS Record next.
...
You will now add a DNS [Record resource](https://www.pulumi.com/registry/packages/cloudflare/api-docs/record/) so the previously configured Route can be resolved. You'll also output the Route URL so it can be easily accessed in the next step.
...
Because you're now exporting a value, notice the updated `return` statement. Ensure to also include `using System.Collections.Generic;` in your imports.
