...
```sh output
Creating environment my-dev-env for stack dev...
```
...
Notice the code also outputs a number of properties from the Zone resource, such as the `zoneId`, `nameservers`, and `status` so that these can easily be accessed in subsequent steps.

### b. Add a DNS Record

You will now add a DNS [Record resource](https://www.pulumi.com/registry/packages/cloudflare/api-docs/record/) so the previously configured Zone can be tested.
...
Now that you have defined your resources, you can deploy the changes so these are reflected in your Cloudflare account using the Pulumi CLI.
...
:::note
If you use `example.com` as your site, skip ahead to [Test your site](#5-test-your-site).
:::
...
:::note
If you use `example.com` as your site, you will not receive a valid response.
:::
