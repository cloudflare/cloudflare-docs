...
Any additions you make are preserved when you subsequently update crawler actions in AI Crawl Control. If the expression has been modified in a way AI Crawl Control cannot parse, a warning banner will appear on the **Security** page. Select **View rule in WAF** in the banner to inspect or correct the rule.

:::caution
Changes made directly to the WAF rule are not reflected back in the AI Crawl Control dashboard. Use the **Security** tab as your primary interface, and reserve direct WAF edits for cases where the dashboard does not cover your requirements.
:::
