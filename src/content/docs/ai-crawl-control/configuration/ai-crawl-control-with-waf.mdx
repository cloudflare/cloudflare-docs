...
When you block AI crawlers via AI Crawl Control (either all or some), AI Crawl Control creates **one** WAF custom rule to block those AI crawlers. This rule is automatically added at the end of your existing WAF custom rules, meaning it executes after all other custom rules.

**Important considerations:**

- The AI Crawl Control WAF rule **only includes bots set to Block**. Bots set to Allow are not included in the rule.
...
<Steps>
	...
	select your account and domain. 2. Go to **Security** > **WAF** > **Custom
	rules** tab. 3. Identify your WAF custom rule and the AI Crawl Control rule.
	4. Drag the rule you wish to prioritize to the top, or modify your WAF custom
	rule to ensure it does not conflict with your AI Crawl Control configurations.
</Steps>
