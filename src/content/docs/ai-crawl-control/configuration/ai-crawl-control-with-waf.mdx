...
- **Path-based exceptions** — allow a blocked crawler to access specific sections of your site by adding an `AND` clause that excludes certain paths
- **Extra user agents or detection IDs** — match crawlers that are not listed in AI Crawl Control
- **Additional expression clauses** — restrict blocking to specific hostnames or other request properties
