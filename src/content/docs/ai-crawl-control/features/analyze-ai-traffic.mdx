...
You can find meaningful information across the **Overview**, **Security**, and **Metrics** tabs.
...
| **Crawlers grouped by operators** | Explore crawlers organized by company (OpenAI, Microsoft, Google, ByteDance, Anthropic, Meta) with allowed requests, referrals (paid plans only), and activity change percentages. Select a crawler or operator to drill down in the **Security** tab. |
...
The **Security** tab provides detailed information about individual AI crawlers and allows you to set **Block** or **Allow** controls:
...
The **Security** tab includes an actions menu for each crawler with the following options:
