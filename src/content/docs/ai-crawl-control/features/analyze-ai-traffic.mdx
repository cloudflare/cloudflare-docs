...
You can group data by different dimensions:

| Dimension    | Description                                                                      |
| ------------ | -------------------------------------------------------------------------------- |
| **Crawler**  | Track activity from individual AI crawlers (like GPTBot, ClaudeBot, Bytespider). |
| **Category** | Analyze crawlers by their purpose or type.                                       |
| **Operator** | Discover which companies (OpenAI, Anthropic, ByteDance) are crawling your site.  |
| **Host**     | Break down activity across multiple subdomains.                                  |

<Aside type="note">
	The Data transfer view shows `edgeResponseBytes`. Some crawlers may show
	higher data transfer per request than others depending on the content they
	request.
</Aside>
