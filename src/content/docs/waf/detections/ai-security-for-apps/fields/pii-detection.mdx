...
AI Security for Apps can detect personally identifiable information (PII) in incoming LLM prompts. There are two approaches to PII detection, and you can use them together for layered protection:
...
- [`cf.llm.prompt.pii_detected`](/ruleset-engine/rules-language/fields/reference/cf.llm.prompt.pii_detected/) — `true` if any PII was found.
- [`cf.llm.prompt.pii_categories`](/ruleset-engine/rules-language/fields/reference/cf.llm.prompt.pii_categories/) — an array of the specific PII types found.
...
The blanket `cf.llm.prompt.pii_detected` field returns `true` when _any_ PII category is detected — including broad categories like `PERSON`, `DATE_TIME`, and `LOCATION` that frequently appear in normal conversation. Blocking on this field alone will produce a high false-positive rate for most applications.
...
- **When incoming requests match**:
  ...
  In the dashboard rule builder, select **LLM PII Detected** from the field dropdown. If you use the Expression Editor, enter the following expression:<br/>
...
- **When incoming requests match**:
  ...
  In the dashboard rule builder, select **LLM PII Categories** from the field dropdown. If you use the Expression Editor, enter the following expression:<br/>
...
1. A rule with action _Block_:<br/>
   `(any(cf.llm.prompt.pii_categories[*] in {"CREDIT_CARD" "US_SSN"}))`

2. A rule with action _Log_:<br/>
   `(any(cf.llm.prompt.pii_categories[*] in {"EMAIL_ADDRESS"}))`

## Exact PII detection (regex)

If you need to detect **custom PII formats** specific to your organization — such as internal employee IDs, patient record numbers, or proprietary account identifiers — you can create a WAF [custom rule](/waf/custom-rules/) using a regex match on [`http.request.body.raw`](/ruleset-engine/rules-language/fields/reference/http.request.body.raw/).

This approach complements fuzzy detection by covering formats the AI model does not natively recognize.

### Example: detect a custom employee ID

Suppose your organization uses employee IDs in the format `EMP-` followed by exactly six digits (for example, `EMP-482910`).
...
- **Plan requirement.** Regex operators (`matches` and `~`) require a Business or Enterprise plan.
...
- **Performance.** Complex regex patterns with excessive backtracking can impact rule evaluation time. Keep patterns as specific as possible.

## Combine both approaches

Use fuzzy and exact detection together for layered protection:
