...
| LLM Token count <br/> [`cf.llm.prompt.token_count`][7] <br/> <Type text="Number"/>                                | An estimated token count for the LLM prompt in the request. Refer to [Token counting](/waf/detections/ai-security-for-apps/fields/token-counting/) for details.                                                                                         |
| LLM Custom topic categories <br/> [`cf.llm.prompt.custom_topic_categories`][8] <br/> <Type text="Map<Number>"/>   | A map of custom topic labels to relevance scores (1–99). Lower scores indicate the prompt is more relevant to that topic. Only populated when [custom topics](/waf/detections/ai-security-for-apps/fields/unsafe-topics/#custom-topics) are configured. |
