...
- When validating that [custom topic](/waf/detections/ai-security-for-apps/fields/unsafe-topics/#custom-topics) definitions are working as expected.
...
Enable the managed ruleset using a `PUT` request:
...
To set individual rule actions to **Log**, override the rules within the managed ruleset using `action_parameters.overrides`.
...
1. **Start in log mode.** Enable the AI Security Log Mode Ruleset with the action set to **Log**. Configure [payload logging](/waf/managed-rules/payload-logging/) so you can view prompts alongside detection results.
