...
On Free plans, the leaked credentials detection is enabled by default, and no action is required. You can configure whether to add or not the `Exposed-Credential-Check` header via [managed transform](/rules/transform/managed-transforms/reference/#add-leaked-credentials-header) to incoming requests when leaked credentials are detected. This option is enabled by default.

On paid plans, you can turn on the detection in the Cloudflare dashboard or via API.
...
:::note
To achieve optimal latency performance, Cloudflare recommends that you turn off [Exposed Credentials Checks](/waf/managed-rules/reference/exposed-credentials-check/), which refers to a previous implementation.
:::
...
If you are on a Free plan, deploy the suggested [rate limiting rule](/waf/rate-limiting-rules/) template available in **WAF** > **Rate limiting**. When you deploy a rule using this template, you get instant protection against IPs attempting to access your application with a leaked password more than 5 times per 10 seconds. This rule can delay attacks by blocking them for a period of time.

Paid plans have access to more granular controls when creating a WAF rule. If you are on a paid plan, create a [custom rule](/waf/custom-rules/) that logs requests containing leaked credentials.

For example, create a custom rule that logs requests containing leaked credentials:
