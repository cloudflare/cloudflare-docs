...
Firewall for AI capabilities vary by Cloudflare plan:
...
| **Rules based on detections** — create custom rules or rate limiting rules using Firewall for AI fields | No | No | No | Yes |
...
Firewall for AI is built into the Cloudflare [Web Application Firewall (WAF)](/waf/) — the WAF must be enabled on your zone before detection fields can be populated and used in rule expressions.
