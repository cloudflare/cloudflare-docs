...
Firewall for AI can detect personally identifiable information (PII) in incoming LLM prompts. There are two approaches to PII detection, and you can use them together for layered protection:

- **Fuzzy detection (AI-powered)** — Firewall for AI uses an AI model to identify common PII types in the prompt content. This approach catches PII even when it appears in natural language or unexpected formats.
...
When Firewall for AI is enabled and a request arrives at a `cf-llm` labeled endpoint, it scans the prompt for PII and populates two fields:
...
The detection is based on [Presidio](https://microsoft.github.io/presidio/supported_entities/), a data protection and de-identification SDK. It recognizes the following categories:
...
- **Body size limit.** The `http.request.body.raw` field inspects up to the first 128 KB of the request body.
