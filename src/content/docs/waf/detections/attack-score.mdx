...
In [Logpush](/logs/logpush/) you may also get the special score `0`, which means that the request did not reach the stage where attack scores are calculated (for example, when a request has already been mitigated by another protection system). The value `0` is not shown in the Cloudflare dashboard.
