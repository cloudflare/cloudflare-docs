...
In [Logpush](/logs/logpush/) you may also get the special score `0`, which means that the request did not reach the attack score classification system. The value `0` is not available in the Cloudflare dashboard.
