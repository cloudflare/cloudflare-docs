...
head:
  - tag: title
    content: Deploy a managed ruleset in the dashboard for an account
...
![Example WAF Managed Rules configuration in the Managed rulesets tab under Application Security > WAF.](~/assets/images/waf/account/managed-rulesets-dashboard.png)
...
To skip one or more rules — or even entire WAF managed rulesets — for specific incoming requests, [add an exception](/waf/managed-rules/waf-exceptions/). Exceptions, also called skip rules, are shown as *Skip* rules in the **Deployed managed rulesets** list.
