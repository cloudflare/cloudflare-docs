...
<Steps>
	...
	5. (Optional) To execute the Cloudflare Managed Ruleset for a subset of incoming requests, select **Edit scope** and [configure the expression](/ruleset-engine/rules-language/expressions/edit-expressions/) that will determine the scope of the current rule deploying the managed ruleset.
...
<Render
	file="managed-rules-browse-zone-new-nav-cloudflare-managed"
	product="waf"
/>
...
<Render file="managed-rules-browse-zone-new-nav-cloudflare-managed" product="waf" />
...
<Render
	file="managed-rulesets/api-next-steps-customizable"
	product="waf"
	params={{
