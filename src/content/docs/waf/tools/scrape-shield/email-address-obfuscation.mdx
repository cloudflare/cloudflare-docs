...
- A configuration rule to override the zone-level email obfuscation setting will not be applied if the request is served by a worker which makes a fetch request to a URL not proxied through Cloudflare
:::
