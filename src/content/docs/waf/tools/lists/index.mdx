...
For IP lists, Cloudflare returns IP addresses or ranges that start with your search query. For example, searching `192.0.2` matches `192.0.2.1` and `192.0.2.0/24`, but searching for `192.0.2.100` does not match a [CIDR range](/fundamentals/concepts/glossary/#cidr) like `192.0.2.0/24` that contains that address.
