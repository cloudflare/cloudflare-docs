...
:::note
For [Spectrum](/spectrum/) applications configured with the TCP or UDP application type, you can use IP Access rules to filter traffic by IP address, IP range, country, or ASN. To use them, enable IP Access rules in your [Spectrum app configuration](/spectrum/reference/configuration-options/#ip-access-rules). IP Access rules do not apply to HTTP/HTTPS type Spectrum applications — use [custom rules](/waf/custom-rules/) for those instead.
:::
