...
When upgrading to custom rules, consider replacing the _Allow_ action supported by IP Access rules with the [_Skip_ action](/waf/custom-rules/skip/). However, the _Skip_ action does not bypass all of Cloudflare's app security features.
