...
import { GlossaryTooltip, FeatureTable } from "~/components";
...
When upgrading to custom rules, consider replacing the _Allow_ action supported by IP Access rules with the [_Skip_ action](/waf/custom-rules/skip/). Note that the _Skip_ action does not bypass all of Cloudflare's app security features.
...
<FeatureTable id="security.x_ip_access_rules" />
...
Block by country is only available on Enterprise plans. Other customers may perform country blocking using [WAF custom rules](/waf/custom-rules/).
...
To learn more about protection options provided by Cloudflare to protect your website against malicious traffic and bad actors, refer to [Account security](/learning-paths/application-security/account-security/).
