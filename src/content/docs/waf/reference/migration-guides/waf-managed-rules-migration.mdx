...
The concept of <GlossaryTooltip term="paranoia level">paranoia level</GlossaryTooltip> did not exist in the OWASP version (2.x) used in WAF managed rules. Based on the OWASP guide recommendations, the WAF migration process will set the paranoia level of the Cloudflare OWASP Core Ruleset to *PL2*.
