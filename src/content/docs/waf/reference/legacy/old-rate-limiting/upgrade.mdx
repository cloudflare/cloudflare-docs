...
- **Counter scope:** The previous version relied on a global counter model. The new version of rate limiting uses counters scoped per data center, with `cf.colo.id` always included as a characteristic. This means thresholds can behave differently for traffic distributed across multiple Cloudflare locations. Note that data centers in the same geographic location may share counters. For more information, refer to [How Cloudflare determines the request rate](/waf/rate-limiting-rules/request-rate/).
