...
To work with WAF Managed Rules you must use the [Rulesets API](/ruleset-engine/managed-rulesets/). For more information on deploying WAF Managed Rules via API, refer to [Deploy a WAF managed ruleset via API (zone)](/waf/managed-rules/deploy-api/).
...
2. A) If you are using the old dashboard:
   ...
   B) If you are using the [new security dashboard](/security/):
...
### Length of rule expression exceeds 4 KB

The migration process converts your existing firewall rules and Page Rules into skip rule expressions for the new WAF. If a zone has many firewall rules or Page Rules that interact with WAF managed rules, the resulting expression may exceed the 4 KB limit.

To resolve this issue:

1. Review your existing Page Rules and firewall rules that reference WAF managed rules.
2. Delete or consolidate rules that are no longer needed to reduce the total expression length.
3. Retry the migration after reducing the number of rules.

If the expression is still too long after consolidation, contact [Cloudflare Support](/support/contacting-cloudflare-support/) for assistance.

### Number of firewall rules exceeds 200

The new WAF supports up to 200 exceptions per zone. If your zone has more than 200 firewall rules that need to be migrated, the automatic migration will fail.

To resolve this issue, reduce the number of firewall rules below 200 by removing or consolidating rules, then retry the migration. Contact Cloudflare Support if you need help.

### "WAF is deprecated for this zone" error after migration

After a zone completes the migration, the Cloudflare dashboard may briefly call the legacy WAF API endpoint (`/firewall/waf/packages`), which returns error code `1040` with the message `WAF is deprecated for this zone, Managed Rulesets should be used instead`.

This error is expected and harmless. It confirms that your zone has been successfully migrated to the new WAF Managed Rules. The dashboard uses this response to detect that the zone is on the new system.

If you see toast notifications about this error, you can safely ignore them. Your managed rules are active and protecting your zone.

### OWASP event counts differ after migration

After migrating, you may notice a difference in the number of security events generated by the OWASP managed ruleset compared to the previous version. This is expected because the old and new OWASP implementations use different detection mechanisms:

- The previous version used OWASP ModSecurity Core Rule Set version 2.x with a sensitivity setting (High, Medium, Low).
- The new version uses OWASP Core Rule Set version 3.x with [paranoia levels](/waf/managed-rules/reference/owasp-core-ruleset/concepts/#paranoia-level) and [score thresholds](/waf/managed-rules/reference/owasp-core-ruleset/concepts/#score-threshold).

Refer to the [OWASP parameter upgrade information](#configurations-that-will-be-lost-in-the-upgrade-process) for details on how your previous settings were converted. To fine-tune the new OWASP configuration, deploy the ruleset in _Log_ mode first, review events in [Security Events](/waf/analytics/security-events/), and adjust the paranoia level or score threshold as needed. For more information, refer to [Configure the OWASP managed ruleset in the dashboard](/waf/managed-rules/reference/owasp-core-ruleset/configure-dashboard/).
