...
If you are setting a CSP using Transform Rules for your entire website, you should [exclude URI paths starting with `/cdn-cgi/challenge-platform/`](#interaction-between-cloudflare-challenges-and-rules-features) in the rule expression to avoid issues with challenges.
...
If you are issuing a challenge for a given URI path that has one or more [Rules](/rules/) features enabled, you should exclude URI paths starting with `/cdn-cgi/challenge-platform/` in your rule expressions to avoid challenge loops.
