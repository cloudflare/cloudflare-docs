...
To deploy a rate limiting ruleset, do the following:
...
2. Deploy the ruleset to the `http_ratelimit` [phase entry point ruleset](/ruleset-engine/about/rulesets/#entry-point-ruleset).
...
For more information on the different rate limiting parameters (`ratelimit` object), refer to [Rate limiting parameters](/waf/rate-limiting-rules/parameters/).
...
1. Invoke the [Get an account entry point ruleset](/api/operations/getAccountEntrypointRuleset) method to obtain the definition of the entry point ruleset for the `http_ratelimit` phase. You will need the [account ID](/fundamentals/setup/find-account-and-zone-ids/) for this operation.

2. If the entry point ruleset already exists (that is, if you receive a `200 OK` status code and the ruleset definition), take note of the ruleset ID in the response. Then, invoke the [Create an account ruleset rule](/api/operations/createAccountRulesetRule) operation to add an `execute` rule to the existing ruleset. By default, the rule will be added at the end of the list of rules already in the ruleset.

   The following example `POST` request adds a new `execute` rule to the phase entry point ruleset deploying the rate limiting ruleset with ID `<RATE_LIMITING_RULESET_ID>` to all Enterprise zones in the account:
   ...
   If the entry point ruleset does not exist (that is, if you receive a `404 Not Found` status code), invoke the [Create an account ruleset](/api/operations/createAccountRuleset) operation to create the entry point ruleset with the new rule executing the rate limiting ruleset. Refer to [Create ruleset](/ruleset-engine/rulesets-api/create/#example---create-an-account-level-phase-entry-point-ruleset) for an example.
