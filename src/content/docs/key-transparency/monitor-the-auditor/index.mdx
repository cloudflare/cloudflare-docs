...
Cloudflare Key Transparency Auditor validates Log audit proofs and provides a signature for them. This allows Log to distribute these signatures to its end-users, and provides users with confidence that the Log is not tricking them.

Cloudflare provides [Plexi](https://github.com/cloudflare/plexi), a cli companion to verify an Auditor claims locally. This leverages the public [API](./api/index.mdx), and allows users to verify Cloudflare Auditor claims.
...
Key Transparency Auditor vouches for Log validity by ensuring epoch uniqueness. and verifying the associated proof
