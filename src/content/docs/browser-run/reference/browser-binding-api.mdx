...
| `outboundByHost`         | `Record<string, Fetcher>` | Routes requests for each hostname through a Worker Fetcher. Refer to [Route requests through an outbound Worker](/browser-run/features/outbound-workers/). |
