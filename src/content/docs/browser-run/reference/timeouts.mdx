...
| `PDFOptions.timeout`    | Same as `actionTimeout`, but only applies to the [/pdf endpoint](/browser-run/quick-actions/pdf-endpoint/).           | 30 s               | 5 min |
...
- If you receive a `422` error, it may be the action itself (ex: taking a screenshot, extracting the html content) that takes a long time. Try increasing the actionTimeout instead.
