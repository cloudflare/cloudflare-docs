---
pcx_content_type: how-to
title: Human in the Loop
description: Temporarily hand off browser control to a human operator for authentication, sensitive actions, or tasks that are difficult to fully automate.
sidebar:
  order: 2
  badge: Beta
products:
  - browser-run
---

Some browser automation workflows require manual intervention. A login page may need multi-factor authentication, a form may require sensitive credentials you do not want to pass to an automation script, or a task may be too complex to fully automate. Human in the Loop lets a human step into a live browser session through [Live View](/browser-run/features/live-view/) to handle what automation cannot, then hand control back to the script.

## How it works

Human in the Loop works with any [Browser Session](/browser-run/#integration-methods) and provides two approaches for human intervention:

### Structured handoff (recommended)

Using the Chrome DevTools Protocol (CDP) extensions, your script can formally request human intervention and wait for completion:

1. Your automation script encounters a scenario requiring human input.
2. The script calls `Cloudflare.handoff` with instructions for the human operator.
3. The script automatically enters a waiting state and receives a handoff ID.
4. A human operator opens the [Live View](/browser-run/features/live-view/) URL to complete the required actions.
5. The script receives a `Cloudflare.handoffComplete` event when the human finishes or the handoff times out.
6. The automation resumes with knowledge of whether the intervention succeeded.

### Manual detection

For simpler use cases, you can manually manage the handoff:

1. Your automation script navigates to a page that needs human input.
2. The script retrieves the [Live View](/browser-run/features/live-view/) URL from the session's target list and shares it with a human operator.
3. The human operator opens the Live View URL and completes the required action.
4. The automation script detects completion by polling for page elements or waiting for navigation events.

## Example: structured handoff with CDP

This example demonstrates the structured handoff flow using the Cloudflare extensions to CDP. The script formally requests human intervention and is notified when complete:

```js
import puppeteer from "puppeteer-core";

const ACCOUNT_ID = "<your-account-id>";
const API_TOKEN = "<your-api-token>";

// Connect to Browser Run
const browser = await puppeteer.connect({
	browserWSEndpoint: `wss://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/browser-run/devtools/browser`,
	headers: { Authorization: `Bearer ${API_TOKEN}` },
});
// Remember that if using a Cloudflare worker, you can simply do await puppeteer.launch(ENV.BROWSER)

const page = await browser.newPage();
await page.goto("https://github.com/login");

// Create a CDP session to send Browser Run commands
const cdp = await page.createCDPSession();

// Get the Live View URL for the human operator
const { devtoolsFrontendUrl } = await cdp.send("Cloudflare.getLiveView", {
	mode: "tab", // Options: "devtools", "tab", or "full"
	expiresInMs: 300000, // optional, default is 5 minutes
});

// Share the Live View URL with the human operator (for example, send it via Slack, email, or display it in a UI)
console.log(`Human input needed. Open this URL: ${devtoolsFrontendUrl}`);

// Set up completion listener before initiating handoff
const handoffCompletePromise = new Promise((resolve) => {
	cdp.once("Cloudflare.handoffComplete", resolve);
});

// Request human intervention with specific instructions
const { handoffId } = await cdp.send("Cloudflare.handoff", {
	instructions: "Please log in with your GitHub credentials",
	timeout: 600000, // 10 minute timeout
});

console.log(`Human intervention requested (ID: ${handoffId})`);
console.log(`Live View URL: ${devtoolsFrontendUrl}`);

// Wait for human to complete the task
const result = await handoffCompletePromise;

if (result.success) {
	console.log("Login successful, continuing automation...");

	// Continue with your automation...
	await page.goto("https://github.com/settings/profile");
} else {
	console.error(`Human intervention failed: ${result.reason}`);
}

await browser.close();
```

## Example: manual detection approach

This example uses the traditional approach of sharing a Live View URL and polling for completion:

```js
import puppeteer from "puppeteer-core";

const ACCOUNT_ID = "<your-account-id>";
const API_TOKEN = "<your-api-token>";

// Connect to Browser Run
const browser = await puppeteer.connect({
	browserWSEndpoint: `wss://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/browser-run/devtools/browser`,
	headers: { Authorization: `Bearer ${API_TOKEN}` },
});

const page = await browser.newPage();
await page.goto("https://example.com/login");

// Create CDP session and get Live View URL
const cdp = await page.createCDPSession();
const { devtoolsFrontendUrl } = await cdp.send("Cloudflare.getLiveView", {
	//targetId: page.target()._targetId, // optional, defaults to current page
	expiresInMs: 300000, // 5 minutes
});

// Share the Live View URL with the human operator (for example, send it via Slack, email, or display it in a UI)
console.log(`Human input needed. Open this URL: ${devtoolsFrontendUrl}`);

// Wait for a selector that only appears after login (5 minute timeout)
await page.waitForSelector("[data-login]", { timeout: 300000 });

// Login complete, continue automation
console.log("Login complete. Continuing automation...");

// Verify we're logged in and get username
const username = await page.$eval("[data-login]", (el) =>
	el.getAttribute("data-login"),
);
console.log(`Logged in as: ${username}`);

await page.goto("https://example.com/dashboard");

await browser.close();
```

The Live View URL is valid for five minutes by default but can be extended for up to 1 hour using the `expiresInMs` parameter. If the URL expires before the human operator opens it, send the CDP `Cloudflare.getLiveView` command again to get a fresh URL.

## CDP commands for structured handoff

Browser Run extends the Chrome DevTools Protocol with Cloudflare-specific commands for managing human intervention. For full parameter and return type details, refer to the [Browser Rendering protocol reference](/api/resources/browser_rendering/subresources/devtools/subresources/browser/methods/protocol/).

### `Cloudflare.handoff`

Requests human intervention for a specific page target. Returns a `handoffId` to track the session. You can optionally specify a `targetId` to target a specific page. If omitted, the target is automatically resolved from the CDP session.

```js
const { handoffId } = await cdp.send("Cloudflare.handoff", {
	targetId: page.target()._targetId, // optional, defaults to current page
	instructions: "Please log in",
	timeout: 1800000, // optional, 30 minutes
});
```

### `Cloudflare.handoffComplete` event

Emitted when human intervention completes or times out. Listen for this event to know when to resume automation.

```js
cdp.once("Cloudflare.handoffComplete", (result) => {
	if (result.success) {
		console.log("Handoff completed successfully");
	} else {
		console.log(`Handoff failed: ${result.reason}`);
	}
});
```

### `Cloudflare.getHandoffState`

Checks if a handoff is currently active for a target.

```js
const state = await cdp.send("Cloudflare.getHandoffState", {
	targetId, // optional, defaults to current page
});
if (state.active) {
	console.log(
		`Handoff ${state.handoffId} has been active for ${state.durationMs}ms`,
	);
}
```

### `Cloudflare.getLiveView`

Gets a Live View URL for the browser session. Used in conjunction with handoff to provide human access.

```js
const { devtoolsFrontendUrl } = await cdp.send("Cloudflare.getLiveView", {
	targetId, // optional, defaults to current page
	mode: "tab",
	expiresInMs: 600000, // 10 minutes
});
console.log(`Live View URL: ${devtoolsFrontendUrl}`);
```

## Use cases

- **Authentication flows**: Login pages with MFA, SSO, or CAPTCHA that cannot be bypassed programmatically
- **Sensitive data entry**: Forms requiring credentials or personal information you do not want to pass to an automation script
- **Complex interactions**: One-off tasks that are too difficult or not worth fully automating, such as configuring a dashboard or approving a workflow
- **Verification steps**: Confirming an order, reviewing generated content, or approving an action before the script proceeds

## Best practices

### Provide clear instructions

Give human operators specific, actionable guidance:

```js
// Good - specific and actionable
await cdp.send("Cloudflare.handoff", {
	instructions:
		"Review the items in the cart and if approved for checkout, click 'Done'. Otherwise, click 'Failed' and provide a reason.",
});

// Less helpful - too vague
await cdp.send("Cloudflare.handoff", {
	instructions: "Log in to the site",
});
```

### Set appropriate timeouts

Match timeout duration to task complexity:

```js
// Quick tasks - 2-3 minutes
await cdp.send("Cloudflare.handoff", {
	instructions: "Click the 'I agree' checkbox and submit",
	timeout: 120000,
});

// Complex tasks - 10-15 minutes
await cdp.send("Cloudflare.handoff", {
	instructions: "Complete the multi-page application form with test data",
	timeout: 900000,
});
```

### Handle failures gracefully

Always prepare for handoff failures or timeouts:

```js
const handoffComplete = new Promise((resolve, reject) => {
	cdp.once("Cloudflare.handoffComplete", resolve);

	// Safety timeout slightly longer than handoff timeout
	setTimeout(() => {
		reject(new Error("Handoff listener timeout"));
	}, 320000);
});

try {
	await cdp.send("Cloudflare.handoff", {
		instructions: "Complete the required task",
		timeout: 300000,
	});

	const result = await handoffComplete;

	if (!result.success) {
		// Handle failed handoff
		console.error(`Handoff failed: ${result.reason}`);
		// Implement retry logic or alternative flow
	}
} catch (error) {
	// Handle timeout or other errors
	console.error("Handoff error:", error);
}
```

### Monitor handoff state

Check handoff status when needed:

```js
// Check if a handoff is already active before requesting a new one
const currentState = await cdp.send("Cloudflare.getHandoffState");
if (currentState.active) {
	console.log(`Handoff already active: ${currentState.handoffId}`);
	// Wait for current handoff or handle appropriately
} else {
	// Safe to start new handoff
	await cdp.send("Cloudflare.handoff", {
		/* ... */
	});
}
```

:::note[Bot detection]
Browser Run requests are [always identified as bot traffic](/browser-run/faq/#will-browser-run-be-detected-by-bot-management). Even with a human controlling the session, some third-party services may still block the request.
:::
