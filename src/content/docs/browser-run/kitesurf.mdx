...
The best way to know whether a specific site is compatible with Kitesurf is to try it — either through the [CDP endpoint](/browser-run/cdp/) and [Quick Actions](/browser-run/quick-actions/) or in the [public playground](https://kitesurf.dev/).
...
To start exploring Kitesurf without writing any code, use the [public playground](https://kitesurf.dev/). Type in any URL to see how Kitesurf renders the page and interact with it.
...
:::note
These numbers are a snapshot and improve over time. WPT measures conformance to web standards, not a browser's ability to render and interact with every real-world website. The best way to confirm a specific site works is to try it in the [playground](https://kitesurf.dev/).
:::
...
The table below shows the medians of five Browser Run [Quick Action](/browser-run/quick-actions/) runs across a [14-URL corpus](https://kitesurf.dev/corpus.txt), comparing Chromium (warm pool) with Kitesurf:
