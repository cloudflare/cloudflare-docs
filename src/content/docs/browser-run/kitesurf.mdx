...
Kitesurf is Cloudflare's stateless, highly scalable browser that runs entirely on top of [Workers](/workers/) and was built specifically for AI agents. Instead of shipping a full desktop browser engine like Chromium, Kitesurf focuses on what matters to an agent — token count, context windows, scalability, performance, and cost — while trading away features that only humans need, such as tabs, themes, extensions, and pixel-perfect rendering.
...
:::note
Learn more about Kitesurf and how it was built in the [blog announcement](https://blog.cloudflare.com/kitesurf).
:::

:::note[Pricing]
Kitesurf is in beta and available for free, behind per-account [limits](/browser-run/limits/). To use it, add the `browser=kitesurf` parameter to any Browser Run [CDP](/browser-run/cdp/) or [Quick Action](/browser-run/quick-actions/) endpoint.
:::
...
The best way to know whether a specific site is compatible with Kitesurf is to try it — either through the APIs below or in the [public playground](https://kitesurf.cloudflare.app/).

## Use Kitesurf with the CDP endpoint

The [Browser Run CDP endpoint](/browser-run/cdp/) supports Kitesurf as an option, so your existing [Puppeteer](/browser-run/puppeteer/), [Playwright](/browser-run/playwright/), [chrome-remote-interface](https://www.npmjs.com/package/chrome-remote-interface), or any AI agent that speaks MCP and CDP already works. To opt in, add the `browser=kitesurf` parameter to the endpoint.

For example, to use Kitesurf from an [MCP client](/browser-run/cdp/mcp-clients/), add `browser=kitesurf` to the `--wsEndpoint` URL:
