...
This guide shows you how to deploy a [Worker](/workers/) that uses Stagehand, Browser Run, and [Workers AI](/workers-ai/) to automate a web task.

:::note
Browser Run currently supports `@browserbasehq/stagehand` `v2.5.x` only.
...
<Details header="See a video of this example">

![Stagehand video](/images/browser-run/speedystagehand.gif)

Output:

![Stagehand example result](~/assets/images/browser-run/stagehand-example.png)
...
Update your Wrangler configuration file to include the bindings for Browser Run and [Workers AI](/workers-ai/):
