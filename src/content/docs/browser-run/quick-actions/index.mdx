...
- **Browser binding** — Call Quick Actions directly from a Cloudflare Worker using `env.BROWSER.quickAction()`
...
To use Quick Actions from a Worker, configure a [browser binding](/browser-run/reference/wrangler/#bindings) in your `wrangler.json`:
...
No API token is needed when using the browser binding.
