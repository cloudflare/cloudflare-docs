...
```json output collapse={12-80}
{
	...
	"result": [
		...
		"https://developers.cloudflare.com/cloudflare-one/traffic-controls/",
...
Visit the [Browser Run API reference](/api/resources/browser_rendering/subresources/links/methods/create/) for all available parameters, such as setting HTTP credentials using `authenticate`, setting `cookies`, and customizing load behavior using `gotoOptions`.
:::
