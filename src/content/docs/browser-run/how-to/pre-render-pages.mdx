...
- A Cloudflare account with [Browser Run enabled](/browser-run/get-started/#quick-actions)
...
const renderHtml = async (env: Env, targetUrl: URL) => {
	// The /content Quick Action loads the page in Browser Run and returns
	// the page HTML after the configured loading condition is met.
...
The `waitUntil: "networkidle2"` option waits until the page has no more than two network connections for at least 500 ms. This is often enough for client-rendered pages. If your page needs a more specific readiness signal, use [`waitForSelector`](/browser-run/reference/timeouts/#notes-and-recommendations) to wait for an element that only appears after your content has loaded.
