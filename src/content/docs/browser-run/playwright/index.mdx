...
The Workers team forked a [version of Playwright](https://github.com/cloudflare/playwright) that was modified to be compatible with [Cloudflare Workers](/workers/) and [Browser Run](/browser-run/).
...
Make sure you have the [browser binding](/browser-run/reference/wrangler/#bindings) configured in your Wrangler configuration file:
...
If users omit the `browser.close()` statement, the browser instance will stay open, ready to be connected to again and [re-used](/browser-run/features/reuse-sessions/) but it will, by default, close automatically after 1 minute of inactivity. Users can optionally extend this idle time up to 10 minutes, by using the `keep_alive` option, set in milliseconds:
...
Session `2be00a21-9fb6-4bb2-9861-8cd48e40e771` was closed explicitly with `browser.close()` by the client, while session `478f4d7d-e943-40f6-a414-837d3736a1dc` was closed due to reaching the maximum idle time (check [limits](/browser-run/limits/)).
...
- `allowedBrowserAcquisitions` specifies if a new browser session can be opened according to the rate [limits](/browser-run/limits/) in place.
