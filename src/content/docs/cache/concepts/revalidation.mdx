...
For stale (expired TTL) content, Cloudflare will send a revalidation request to the origin, if the stale content is still valid a new TTL will be set, if the content is expired then the origin will provide new fresh content replacing the old.
