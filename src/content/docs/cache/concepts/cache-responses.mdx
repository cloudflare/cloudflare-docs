...
* The origin returned `no-store` or bare `private` in a `Cloudflare-CDN-Cache-Control` or `CDN-Cache-Control` header. Cloudflare evaluates these headers ahead of `Cache-Control`, in the precedence `Cloudflare-CDN-Cache-Control` > `CDN-Cache-Control` > `Cache-Control` — so an origin returning `Cache-Control: public, max-age=3600` together with `CDN-Cache-Control: no-store` produces `BYPASS`. A [Cache Rule](/cache/how-to/cache-rules/) with an [Edge Cache TTL](/cache/how-to/cache-rules/settings/#edge-ttl) setting that ignores origin cache-control overrides these directives, same as it does for `Cache-Control: no-store`. `no-cache`, `max-age=0`, or `s-maxage=0` in these headers do not produce `BYPASS`; they produce `MISS` on the first request, then [`REVALIDATED`](#revalidated) or [`EXPIRED`](#expired). Refer to [CDN-Cache-Control](/cache/concepts/cdn-cache-control/) for the precedence rules.
