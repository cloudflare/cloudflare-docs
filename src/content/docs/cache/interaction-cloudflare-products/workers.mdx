...
:::note
This page describes how Workers interact with a zone's Cloudflare Cache. You can also enable **[Workers Cache](/workers/cache/)** for your Worker — a cache that sits in front of the Worker itself, so that Cloudflare returns a cached response without running the Worker. Use Workers Caching to cache the output of a Worker's logic directly, independent of any zone configuration.
:::
