...
* You may see an influx of `504` responses with `ClientRequestSource: earlyHintsCache` in Cloudflare Logs when Early Hints is enabled, which is expected and benign. Requests with this source are internal subrequests for cached Early Hints; they are neither end user requests, nor do they reach your origin. Their response status only indicates whether there are cached Early Hints for the request URI (`200` on cache HIT, `504` on cache MISS). These requests are already filtered out in other views, such as Cache Analytics. To filter them out of Log Explorer, use `ClientRequestSource != "earlyHintsCache"`. To filter using the GraphQL API, refer to [Filter end users](/analytics/graphql-api/features/filtering/#filter-end-users).
