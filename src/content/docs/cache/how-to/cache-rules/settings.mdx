...
[Single-file purge](/cache/how-to/purge-cache/purge-by-single-file/) may not work as expected if your Cache Rule sets a [custom cache key](/cache/how-to/cache-keys/) (because the dashboard cannot send the headers or cookies that are part of the key), or if your rule expression matches only `GET` requests (because purge requests use a different HTTP method internally).

For more information and alternatives, refer to [Purge by single-file limitations](/cache/how-to/purge-cache/purge-by-single-file/#limitations-and-alternatives).
...
:::note
When using Custom Cache Rules with a Bypass setting, the response header may return [DYNAMIC](/cache/concepts/cache-responses/#dynamic) rather than explicitly indicating a bypass. This occurs because either the asset is originally ineligible for caching or the rule makes it ineligible for caching, even if the origin response is otherwise cacheable.
:::
...
This setting does not make a request or response cacheable. Cloudflare can use origin range fetching for an eligible `GET` even if the response is not stored. File-size limits apply to the complete object, not the requested range. [Cache Reserve](/cache/advanced-configuration/cache-reserve/#limits) does not support Origin Range Requests.

Cloudflare asks the origin for unencoded content. Origins should return compatible range responses throughout the request. If an origin ignores `Range` and returns `200 OK`, Cloudflare can use the response but must download the complete file. For all origin and client response requirements, refer to [Range request behavior](/cache/reference/range-requests/).

<Details header="API information">

API configuration object name: `"origin_range_requests"`.

The `mode` property accepts these values:

| Value | Behavior |
| --- | --- |
| `on` | Allows Cloudflare to generate origin range requests for matching, eligible `GET` requests. |
| `off` | Does not generate Origin Range Requests. Cloudflare can still serve ranges from a complete cached file. |
| `default` | Applies no rule override. Cloudflare uses its platform default. No file extensions are currently turned on by default. |

The `mode` property is required. The `on` and `off` modes override the default behavior for matching requests.

The `off` mode controls only range requests generated by Origin Range Requests. On a cacheable miss, Cloudflare may remove the client's `Range` header and fetch the complete file. If caching is bypassed or the request is not eligible for caching, Cloudflare may forward the client's original `Range` header.
...
To override Cloudflare's default behavior, use `off` in a rule that matches the affected traffic.

```json title="Turn off default Origin Range Requests"
"action_parameters": {
  "origin_range_requests": {
    "mode": "off"
  }
...
Refer to [Create a cache rule via API](/cache/how-to/cache-rules/create-api/#example-turn-off-default-origin-range-requests) for a complete opt-out example.
...
If the `vary` object is omitted, this Cache Rules Vary setting is turned off. Other Vary behavior, such as `Vary: *`, [Vary for images](/cache/advanced-configuration/vary-for-images/), and compression handling, is unaffected. If the `vary` object is present, `default` is required. An empty `vary` object is invalid.

Each header configuration object, and the `default` object, must include an `action` key set to one of `normalize`, `passthrough`, or `bypass`. Refer to [Actions](/cache/concepts/vary/#actions) for guidance on when to use each.
...
Refer to [Create a cache rule via API](/cache/how-to/cache-rules/create-api/#example-requests) for complete API examples, or to the [Terraform example](/cache/how-to/cache-rules/terraform-example/).
