...
<Details header="Example: Turn on Origin Range Requests for large media files">

<APIRequest
  path="/zones/{zone_id}/rulesets/phases/{ruleset_phase}/entrypoint"
  method="PUT"
  parameters={{ ruleset_phase: "http_request_cache_settings" }}
  json={{
    rules: [
      {
        expression:
          '(http.request.uri.path.extension in {"mp4" "mov" "m4v"})',
...
<Details
  header="Example: Turn off default Origin Range Requests"
  id="example-turn-off-default-origin-range-requests"
>
...
<Details header="Example: Cache expected Vary responses">
