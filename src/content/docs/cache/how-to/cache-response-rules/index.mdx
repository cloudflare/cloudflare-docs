...
Cache Response Rules apply to both cached and non-cached ([DYNAMIC](/cache/concepts/cache-responses/#dynamic)) responses from the origin. For example, you can strip Set-Cookie headers from responses that are not eligible for caching.
