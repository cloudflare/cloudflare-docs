...
The Cache Key is visible in the result of a [Cloudflare Trace](/rules/trace-request/), under `Cache Parameters` > `Cache Key`.
In the pop-up you can see the full cache key as `key_string`.
