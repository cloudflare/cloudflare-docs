...
To mark content as stale instead of removing it, [invalidate cached content](/cache/guides/invalidate-cache/). Cloudflare revalidates invalidated content on the next request and reuses content that has not changed.
...
Invalidation requests count toward the same limits as purge requests. For details, refer to [Invalidation limits](/cache/guides/invalidate-cache/#limits).
