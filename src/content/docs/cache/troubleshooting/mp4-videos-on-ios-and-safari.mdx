...
The first rule preserves strong ETags for MP4 files, which satisfies Safari's requirements for range request handling. The second rule bypasses Cloudflare's cache, allowing Safari to communicate directly with the origin server for correct range request negotiation.
