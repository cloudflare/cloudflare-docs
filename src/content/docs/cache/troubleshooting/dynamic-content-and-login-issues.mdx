...
If you see either of the following, the page is probably cached when it should not be:
...
4. Verify the fixed response now returns `CF-Cache-Status: DYNAMIC` or `BYPASS`, and preserves `Set-Cookie`.
