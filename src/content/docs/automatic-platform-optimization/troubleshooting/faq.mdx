...
Many testing tools send no `Accept` header, which is why they can return `MISS` or `DYNAMIC`. To reliably reproduce a browser-like HTML request, include `-H 'accept: text/html'` in your cURL command.
