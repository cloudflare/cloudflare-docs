...
sidebar:
  order: 9
description: >-
  When APO is working, three headers are present:  CF-Cache-Status, cf-apo-via,cf-edge-cache. APO works correctly when the headers exactly match the headers below.
---
...
In a terminal, use the following cURL. Include the `'accept: text/html'` header so the request is treated as a browser-like HTML request. This makes the test result deterministic. APO can also cache requests that omit the header, depending on the URL path. For more information, refer to [FAQ on `cf-cache-status` results](/automatic-platform-optimization/troubleshooting/faq/).
