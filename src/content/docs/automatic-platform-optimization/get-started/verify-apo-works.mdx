...
In a terminal, use the following cURL. Include the `'accept: text/html'` header so the request is treated as a browser-like HTML request. This makes the test result deterministic. APO can also cache requests that omit the header, depending on the URL path — for details, see the [FAQ on `cf-cache-status` results](/automatic-platform-optimization/troubleshooting/faq/).
