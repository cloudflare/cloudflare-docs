...
### Generate a placeholder connection string

Use this helper to generate a syntactically valid PostgreSQL placeholder connection string. Each value is randomized in your browser and uses the reserved `.example` domain, so the result never points at a real database or contains real credentials.

<PlaceholderConnectionString />

## Pricing

Hyperdrive with Dynamic Workers adds a separate usage charge for active Hyperdrive configurations. This charge applies only to Hyperdrive configurations created at runtime with `HYPERDRIVE.get()`.
