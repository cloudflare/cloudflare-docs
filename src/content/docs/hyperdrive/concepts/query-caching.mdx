...
Hyperdrive does not invalidate cached read query results when you write to your database. If your application needs read-after-write consistency for a specific path, use a cache-disabled Hyperdrive configuration for the read that must return fresh data. You still get Hyperdrive's connection pooling and fast connection setup on cache-disabled configurations.
...
The default caching behavior for Hyperdrive is defined as below:
...
- **Use a cache-disabled Hyperdrive configuration for reads that must be fresh.** Good examples include authentication, sessions, permissions, billing state, admin settings, and reads immediately after a write.
...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		const clientFresh = postgres(env.HYPERDRIVE_FRESH.connectionString);
		// ...
	},
...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		// Use the cache-disabled binding for auth, permissions, and reads after writes.
		const connectionFresh = await createConnection({
			host: env.HYPERDRIVE_FRESH.host,
			user: env.HYPERDRIVE_FRESH.user,
			password: env.HYPERDRIVE_FRESH.password,
			database: env.HYPERDRIVE_FRESH.database,
			port: env.HYPERDRIVE_FRESH.port,
		});
		// ...
	},
} satisfies ExportedHandler<Env>;
...
```jsonc
{
	"hyperdrive": [
		...
		},
		{
			...
			"id": "<YOUR_HYPERDRIVE_FRESH_CONFIGURATION_ID>",
		},
	],
}
```
