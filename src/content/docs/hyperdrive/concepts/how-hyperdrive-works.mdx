...
Hyperdrive does not invalidate cached read query results when your application writes to your database. If a read must return fresh data after a write, use a cache-disabled Hyperdrive configuration for that read.
