...
3. Have a publicly accessible PostgreSQL or MySQL (or compatible) database that is directly reachable from the Internet. _If your database is in a private network (like a VPC)_, refer to [Connect to a private database](/hyperdrive/configuration/connect-to-private-database/) for instructions on using Cloudflare Tunnel with Hyperdrive.
...
The `localConnectionString` field works with both local and remote databases and allows you to connect directly to your database from your Worker project running locally.
