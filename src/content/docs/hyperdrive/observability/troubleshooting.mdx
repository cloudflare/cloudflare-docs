...
### Diagnose connection contention

Hyperdrive exposes pool size metrics that allow you to identify when your connection pool is being exhausted. You can view these metrics in the [Cloudflare dashboard](/hyperdrive/observability/metrics/#view-metrics-in-the-dashboard) or query them via the [GraphQL Analytics API](/hyperdrive/observability/metrics/#query-via-the-graphql-api).

Connection contention appears in the **Pool connections** chart as one or both of the following patterns:

- A spike in **waiting connections** — the number of client requests waiting for an available connection.
- **Open connections** consistently approaching the **pool size maximum** (your configured [origin connection limit](/hyperdrive/configuration/tune-connection-pool/)).

If you observe these patterns, you can:

1. [Tune your connection pool size](/hyperdrive/configuration/tune-connection-pool/) if you have not already set it to the maximum allowed for your plan.
2. [Request a limit increase](/hyperdrive/platform/limits/#request-a-limit-increase) if your application requires more origin connections.
3. Review your application for long-running queries or transactions that hold connections open unnecessarily. Where possible, avoid wrapping queries in transactions to allow connections to be shared more aggressively.
