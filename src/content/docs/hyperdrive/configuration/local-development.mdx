...
- **`wrangler dev --remote`**: Runs your Worker on Cloudflare's edge using your deployed Hyperdrive configuration. This is useful for testing with Hyperdrive's connection pooling and query caching enabled.
...
Similar to using an environment variable we will allow for setting the same variable format
`CLOUDFLARE_HYPERDRIVE_LOCAL_CONNECTION_STRING_<BINDING_NAME>`
within a environment variable file such as `.env` or `.dev.vars` or `.dev.vars.<env>`.

For example, to set the connection string for a local database: <br/>
