...
| `verify-full` | `VERIFY_IDENTITY` | Identical to `verify-ca`/`VERIFY_CA`, but Hyperdrive also requires the database hostname to match a Subject Alternative Name (SAN) or Common Name (CN) present on the certificate. |
