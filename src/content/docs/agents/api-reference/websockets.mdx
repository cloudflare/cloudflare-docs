...
When you close an `AgentClient` connection using the `close()` method, any pending RPC calls are immediately rejected with a `Connection closed` error. This provides immediate feedback to your application when intentionally disconnecting, rather than waiting for the WebSocket close handshake to complete.
...
// Start an RPC call
const resultPromise = connection.call("analyzeData", [data]);

// Close the connection - pending RPC calls will be rejected immediately
connection.close();
...
try {
  ...
  console.error("Call was interrupted:", error); // "Connection closed"
}
```

</TypeScriptExample>

This behavior ensures your application receives timely error handling when closing connections, preventing long waits for timeout periods.
