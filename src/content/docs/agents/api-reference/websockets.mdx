...
| `onError(connection, error)`                  | When a WebSocket connection error occurs                  |
| `onError(error)`                              | When a server-level (non-connection) error occurs         |
| `shouldSendProtocolMessages(connection, ctx)` | Return `false` to suppress protocol frames for a connection. Refer to [Protocol messages](/agents/api-reference/protocol-messages/) |
...
| `server`                | `string` | Agent instance name this connection belongs to |
...
Every agent instance exposes the following built-in properties:

| Property    | Type                   | Description                                            |
| ----------- | ---------------------- | ------------------------------------------------------ |
| `this.name` | `string`               | The unique name of this agent instance                 |
| `this.state`| `State`                | The agent's persisted state object                     |
| `this.env`  | `Env`                  | The Worker environment bindings                        |
| `this.ctx`  | `DurableObjectState`   | The underlying Durable Object state and storage APIs   |
...
| `isConnectionProtocolEnabled`  | `(connection: Connection) => boolean`           | Returns `true` if protocol messages are enabled for this connection |
