...
When connecting to an Agent, you can pass authentication credentials or other parameters using the `query` option. The `query` parameter supports both static objects and async functions for dynamic authentication:
...
function AuthenticatedAgentInterface() {
  // Static query parameters
  const connection = useAgent({
    agent: "secure-agent",
    name: "user-session",
    query: { token: "static-auth-token" }
  });

  // Async query function for dynamic authentication
  const connectionWithAsync = useAgent({
    agent: "secure-agent",
    name: "user-session",
    query: async () => {
      // Fetch a fresh auth token from your authentication service
      const token = await fetchAuthToken();
      return { token };
    }
  });

  return <div>Connected to secure agent</div>;
}
```

</TypeScriptExample>

When using an async query function, the result is cached to avoid redundant calls during component re-renders. However, the cache is automatically invalidated when the WebSocket connection closes, ensuring that a fresh authentication token is fetched when the connection is re-established. This prevents authentication failures caused by expired or stale tokens on reconnection.

You can customize the cache time-to-live (TTL) using the `cacheTtl` option (in milliseconds). The cache will be invalidated when the TTL expires or when the connection closes, whichever comes first:
...
const connection = useAgent({
  ...
  // Cache auth tokens for 5 minutes
  cacheTtl: 5 * 60 * 1000
});
```
