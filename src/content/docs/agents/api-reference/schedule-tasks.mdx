...
description: Run agent callbacks later, at a fixed time, or on recurring intervals.
...
`scheduleEvery()` is idempotent for the combination of callback name, interval, and payload. Calling it again with the same arguments returns the existing schedule instead of creating a duplicate row. This makes it safe to call from `onStart()`.
...
class MyAgent extends Agent {
	async onStart() {
		// Safe on every Durable Object wake
...
Calling `scheduleEvery()` with a different interval or payload creates a separate schedule.
...
If you use OpenAI models with `generateObject()`, pass `providerOptions: { openai: { strictJsonSchema: false } }` because this discriminated union schema is not compatible with OpenAI strict structured outputs.
...
- `options.idempotent` - Deduplicate repeated schedules with the same callback and payload. Cron schedules are idempotent by default; delayed and `Date` schedules opt in with `true`.

**Returns:** A `Schedule` object with the task details

For recurring cron work, calling `schedule()` with the same cron expression, callback, and payload returns the existing schedule unless you explicitly set `idempotent: false`.

If you call delayed or `Date`-based `schedule()` inside `onStart()`, pass `{ idempotent: true }` to avoid duplicate rows across Durable Object restarts.
...
- Repeating the same `callback`, `intervalSeconds`, and `payload` returns the existing schedule
...
Prevent the Durable Object from being evicted due to inactivity by creating a 30-second heartbeat alarm. Returns a disposer function that cancels the heartbeat when called. The disposer is idempotent — calling it multiple times is safe.
...
`keepAlive()` prevents this by keeping a 30-second heartbeat alarm active while work is in flight. It uses an in-memory reference count and the Durable Object alarm system directly. Because of that:

- The heartbeat does not conflict with your own schedules because everything still shares the same Durable Object alarm slot
- The heartbeat does **not** create rows in `cf_agents_schedules`
- The heartbeat does **not** appear in `getSchedules()` or scheduling diagnostics events
- Multiple concurrent `keepAlive()` calls increment the same internal reference count
...
// Ref count is 2
dispose1(); // Ref count becomes 1
// Agent is still kept alive

dispose2(); // Ref count reaches 0; the agent can go idle
```
