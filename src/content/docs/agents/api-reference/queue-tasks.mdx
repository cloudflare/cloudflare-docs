...
5. **Automatic retries**: If a task fails and retry options are configured, the SDK schedules the next attempt with exponential backoff.
6. **Automatic dequeue**: Successfully executed tasks are automatically removed from the queue.
...
Use the built-in `retry` option on `queue()` to automatically retry failed tasks with exponential backoff. You do not need to manually re-queue tasks on failure.
