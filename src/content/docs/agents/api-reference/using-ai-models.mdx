...
- And many more through AI SDK providers
...
The AI SDK supports over 15 AI providers out-of-the-box, including Amazon Bedrock, Azure, Cerebras, Cohere, Deepgram, Deepseek, ElevenLabs, Fireworks, Google Vertex, Groq, Mistral, OpenRouter, Perplexity, and xAI. Refer to the [AI SDK providers documentation](https://sdk.vercel.ai/providers/ai-sdk-providers) for the full list.
...
### AI Gateway with external providers {/* ai-gateway-external-providers */}

AI Gateway can route requests to external AI providers (OpenAI, Anthropic, Google, and more) using unified billing. Instead of managing API keys for each provider separately, you can use Cloudflare credits to pay for API calls across all providers.

To use external providers with AI Gateway:

1. Install the necessary packages:

<PackageManagers pkg="ai ai-gateway-provider @ai-sdk/openai @ai-sdk/anthropic @ai-sdk/google" />

2. Use the `ai-gateway-provider` package to create a gateway client:
...
export class MyAgent extends Agent<Env> {
	async onRequest(request: Request): Promise<Response> {
		...
		// Create provider without an API key (unified billing)
		const openai = createOpenAI();

		// Wrap the model with the gateway
		const model = gateway(openai.chat("gpt-5-mini"));
...
You can use any supported provider with unified billing:
...
const claudeModel = gateway(anthropic.chat("claude-sonnet-4-5-20250929"));

// Google
const google = createGoogleGenerativeAI();
const geminiModel = gateway(google.chat("gemini-3-pro-preview"));
```
...
When using unified billing with AI Gateway, you do not need to provide API keys for individual providers. All requests are authenticated using your Cloudflare API key, and charges are deducted from your Cloudflare credits. Load credits in your [Cloudflare dashboard](https://dash.cloudflare.com/?to=/:account/ai/ai-gateway).

:::

Alternatively, you can use provider API keys directly (bring your own key):
...
export class MyAgent extends Agent<Env> {
	async onRequest(request: Request): Promise<Response> {
		// Use provider SDK directly with your API key
		const openai = createOpenAI({
			apiKey: this.env.OPENAI_API_KEY,
		});
