---
title: Client SDK
description: Connect to agents from React, browsers, or server runtimes with WebSockets or HTTP.
...
Connect to agents from any JavaScript runtime — browsers, Node.js, Deno, Bun, or edge functions — using WebSockets or HTTP. The SDK provides real-time state synchronization, readable client state, RPC method calls, and streaming responses.
...
WebSocket clients (`useAgent` and `AgentClient`) provide:

- **Bidirectional state sync** - Push and receive state updates in real-time
- **Readable state** - Access the current agent state through `agent.state` or `client.state`
...
`agentFetch()` is the HTTP-only option for one-off requests when you do not need state sync or a persistent connection.
...
const client = new AgentClient<ChatAgent>({
	agent: "ChatAgent",
	name: "room-123",
...
Agents can maintain state that syncs bidirectionally with all connected clients. Both `useAgent` and `AgentClient` expose a `state` property with the latest known agent state.
...
`agent.state` starts as `undefined` and is populated when the server sends state on connect or when the client calls `setState()`. Use optional chaining for safe reads.
...
4. `agent.state` updates automatically, and `onStateUpdate` fires with `source: "client"`
...
For full type safety, pass your Agent class as a type parameter to `useAgent` or `AgentClient`:
...
Vanilla JavaScript clients support the same pattern:
...
const result = await client.stub.processData({ input: "test" });
```
...
const agent = useAgent({
	...
	onMcpUpdate: (mcpServers) => {
		for (const [serverId, server] of Object.entries(mcpServers.servers)) {
			console.log(`${serverId}: ${server.state}`);
		}

		console.log(`Tools: ${mcpServers.tools.map((tool) => tool.name).join(", ")}`);
	},
});
```
...
### 1. Read `state` directly for UI

For most UI code, read `agent.state` directly instead of copying it into separate local state. Use `onStateUpdate` when you need side effects.
...
type UseAgentOptions<State> = {
	...
	basePath?: string; // Full base path instead of /agents/:agent/:name
...
| ------------------------------- | --------- | -------------------------- |
| `state`                         | `State \| undefined` | Current agent state         |
...
| `agent`                         | `string`  | Kebab-case agent name      |
| `name`                          | `string`  | Instance name              |
| `ready`                         | `Promise<void>` | Resolves after identity is received |
| `setState(state)`               | `void`    | Push state to agent        |
| `call(method, args?, options?)` | `Promise` | Call agent method          |
| `send(data)`                    | `void`    | Send raw WebSocket message |
| `close()`                       | `void`    | Close connection           |
