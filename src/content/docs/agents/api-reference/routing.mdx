...
:::note
For `McpAgent`, props are automatically stored and accessible via `this.props`. See [MCP servers](/agents/model-context-protocol/mcp-agent-api/) for details.
:::
...
These hooks are useful for authentication and validation. See [Cross-domain authentication](/agents/guides/cross-domain-authentication/) for detailed examples.
...
For options like `locationHint`, `jurisdiction`, and `props`, see [Routing options](#routing-options).
