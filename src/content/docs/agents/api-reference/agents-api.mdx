...
> & {
	...
	// Async functions are cached and invalidated on reconnect to ensure fresh tokens
...
function useAgent<State = unknown>(
	...
	name: string;
	// Whether the agent identity has been received
	identified: boolean;
	...
	// Stub object for calling Agent methods directly
	stub: Record<string, (...args: unknown[]) => Promise<unknown>>;
};
```

#### Authentication with async query functions

The `query` parameter in `useAgent` can be an async function, which is useful for fetching authentication tokens or other dynamic query parameters. The hook automatically caches the result of async query functions to avoid redundant calls.

When the WebSocket connection closes (due to network issues, server restarts, or explicit disconnection), the cache is automatically invalidated. This ensures that when the connection reconnects, the async query function is re-executed to fetch fresh authentication tokens.
...
function MyComponent() {
  const agent = useAgent({
    agent: "my-agent",
    name: "my-instance",
    // Async function to fetch auth token
    query: async () => {
      const token = await fetchAuthToken();
      return { token };
    },
    // Re-run query when user changes
    queryDeps: [userId],
    // Cache tokens for 5 minutes
    cacheTtl: 5 * 60 * 1000,
  });

  return <div>Connected to agent</div>;
}
```
...
- **Automatic reconnection refresh**: When the WebSocket disconnects and reconnects, the async query function is automatically called again to ensure fresh tokens are used.
- **Dependency tracking**: Use `queryDeps` to specify when the cache should be invalidated and the query re-executed (similar to React's `useEffect` dependencies).
- **Cache TTL**: Set `cacheTtl` to control how long tokens are cached. After the TTL expires, the query function will be called again on the next connection attempt.

This pattern is particularly useful for:
- Fetching short-lived authentication tokens
- Implementing token refresh logic
- Ensuring secure reconnections after network interruptions
