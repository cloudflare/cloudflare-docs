...
Available jurisdictions include `"eu"` (European Union) and `"fedramp"` (FedRAMP compliant locations). Refer to [Durable Objects data location](/durable-objects/how-to/configure-data-location/) for more options.

## Hibernation support

`McpAgent` instances automatically support [WebSockets Hibernation](/durable-objects/how-to/use-websockets/#websocket-hibernation-api), allowing stateful MCP servers to sleep during inactive periods while preserving their state. This means your agents only consume compute resources when actively processing requests, optimizing costs while maintaining the full context and conversation history.
