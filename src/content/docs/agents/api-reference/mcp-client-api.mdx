...
Loopback addresses (`localhost`, `127.0.0.1`, `::1`) are **allowed** to support local development against local MCP servers.

If you need to connect to an internal MCP server in production, use the [RPC transport](/agents/model-context-protocol/transport/) with a Durable Object binding instead of HTTP.
