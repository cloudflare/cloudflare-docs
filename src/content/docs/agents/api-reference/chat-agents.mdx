...
`saveMessages()` waits for any active chat turn to finish before starting a new one, so scheduled or programmatic messages do not overlap an in-flight stream.

### Turn coordination helpers

`AIChatAgent` serializes chat turns — WebSocket requests, tool continuations, and `saveMessages()` calls all run one at a time. The SDK handles queuing automatically for most subclasses.

Coordination becomes relevant when your subclass runs code outside the normal `onChatMessage()` flow — for example, a `schedule()` callback that injects a message, a workflow-switching method, or a custom clear handler. In these situations, your subclass needs to know whether the conversation is mid-stream or waiting on user input before it can safely act on `this.messages`.
...
The most common pattern is a `schedule()` callback that needs to inject a synthetic message. Call `waitUntilStable()` before reading `this.messages` or calling `saveMessages()`. Always pass a `timeout` — without one, `waitUntilStable()` waits indefinitely. If a tool is pending user approval and the user closes their browser, the promise never resolves.
...
Call `resetTurnState()` when the user switches context and the current stream and any queued continuations are no longer relevant. It increments the internal epoch (so queued continuations skip themselves), fires the abort signal on the active stream, and clears pending interaction state.
...
The SDK's built-in `CF_AGENT_CHAT_CLEAR` handler calls `resetTurnState()` automatically. If your `onMessage` override intercepts `CF_AGENT_CHAT_CLEAR` and returns early — for example, to scope the delete to a specific workflow — the built-in handler never runs, so the active stream continues and queued continuations persist into the newly-cleared conversation. Call `this.resetTurnState()` before performing your scoped delete:
...
| `setMessages`             | `(messages \| updater) => Promise<void>` | Set messages directly (syncs to server)           |
...
Override this method to transform or redact messages before they are written to SQLite. It runs after the built-in sanitization (OpenAI metadata stripping, Anthropic provider-executed tool payload truncation, empty reasoning part removal), so built-in cleanup is never bypassed.

The default implementation returns the message unchanged. Return `null` to skip persisting the message entirely.
