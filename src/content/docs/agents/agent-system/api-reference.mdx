...
This reference covers the TypeScript/Worker API and the HTTP surface for the Agent System.
...
- `handlerOptions` – Passed into `createHandler` (refer to [Worker handler](#worker-handler) below).
...
- Registers middleware globally with tags equal to the union of `tags` and `middleware.tags`.
...
- `tags` – Used to select middleware and tools
...
Behavior is described in the [Architecture documentation](/agents/agent-system/architecture/).
...
- `GET /agency/:agencyId/blueprints` – Static and dynamic blueprints
