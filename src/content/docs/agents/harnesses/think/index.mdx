...
Think instruments model turns and tool calls automatically, just like [Flue](https://flueframework.com/). To send `invoke_agent`, `chat`, `execute_tool`, and `tool_approval` spans to Workers Observability, turn on Workers traces:
...
Traces appear in the Agents view in the Cloudflare Dashboard, where you can replay sessions and inspect trace waterfalls. To turn tracing off, set `observability.traces.enabled` to `false`. You do not need to change the Think agent class.
