...
[^2]: You can deploy up to [500 scripts per account](/workers/platform/limits/), but each script (project) can define multiple Agents. Each deployed script can be up to [64 MiB before compression](/workers/platform/limits/#worker-size) on all Workers plans.
