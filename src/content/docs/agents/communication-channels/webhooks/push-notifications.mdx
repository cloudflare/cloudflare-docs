...
If the push service returns a [5xx error](/support/troubleshooting/http-status-codes/cloudflare-5xx-errors/) (temporary failure), you can retry using `this.schedule()` with a short delay:
