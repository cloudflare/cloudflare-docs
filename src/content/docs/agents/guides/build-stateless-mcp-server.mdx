...
**Stateless MCP Server (Worker):**
```
Client → Worker → createMcpHandler → MCP Server → Tools
```

**Authenticated Stateless MCP Server:**
```
Client → OAuth Flow → Token → Worker → OAuthProvider → createMcpHandler → MCP Server → Tools
                                                                              ↓
                                                                     getMcpAuthContext()
```

**Durable Object MCP Server (MCPAgent):**
```
Client → Worker → Durable Object (MCPAgent) → MCP Server → Tools
                        ↓
                   Persistent State
```

Stateless servers are ideal when you don't need persistent state between requests. For servers that need to maintain conversation history, state, or long-running sessions, consider using the [Durable Object-based MCPAgent approach](/agents/model-context-protocol/).

Learn more about connecting MCP clients in the [Test a Remote MCP Server](/agents/guides/test-remote-mcp-server/) guide.
