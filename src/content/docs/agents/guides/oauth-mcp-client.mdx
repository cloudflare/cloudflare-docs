...
## OAuth persistence across reconnections

After a user successfully completes the OAuth flow, the Agent automatically clears the OAuth requirement and stores the authentication tokens. If the connection is lost and the Agent reconnects (for example, after a deployment or network interruption), the Agent will re-establish the connection without requiring the user to authenticate again.

This automatic behavior means:

- **First connection**: User authenticates via OAuth if the MCP server requires it
- **Reconnections**: Agent reuses the existing authorization automatically
- **No re-authentication needed**: Users only authenticate once per MCP server

The Agent stores the OAuth tokens securely in [SQL storage](/agents/api-reference/store-and-sync-state/) and uses them for all future connections to that MCP server. This includes reconnections after Agent restarts, Durable Object hibernation, network interruptions, and temporary connection failures.

If the OAuth tokens expire or become invalid, the server state will return to `authenticating` and present a new `auth_url` for re-authorization. You do not need to implement any special logic to handle reconnections - the SDK manages this automatically.
