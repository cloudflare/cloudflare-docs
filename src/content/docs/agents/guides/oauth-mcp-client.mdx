...
After a user successfully completes the OAuth flow, the Agent automatically clears the OAuth requirement. If the connection is lost and the Agent reconnects (for example, after a deployment or network interruption), the Agent will re-establish the connection without requiring the user to authenticate again.
...
The Agent stores the OAuth tokens securely in SQL storage and uses them for all future connections to that MCP server. You do not need to implement any special logic to handle reconnections - the SDK manages this automatically.
