...
Enter the URL of your remote MCP server under the **MCP Server** field section of the playground and select **Connect**. You can use an MCP server running on your local machine on localhost, or you can use a remote MCP server running on Cloudflare.
