...
You can start by deploying a [public MCP server](https://github.com/cloudflare/agents/tree/main/examples/mcp-worker) without authentication, then add user authentication and scoped authorization later. If you already know your server will require authentication, you can skip ahead to the [next section](/agents/guides/remote-mcp-server/#add-authentication).

The button below will guide you through everything you need to do to deploy this [example MCP server](https://github.com/cloudflare/agents/tree/main/examples/mcp-worker) to your Cloudflare account:
...
Once deployed, this server will be live at your workers.dev subdomain (e.g. `remote-mcp-server.your-account.workers.dev/mcp`). You can connect to it immediately using the [AI Playground](https://playground.ai.cloudflare.com/) (a remote MCP client), [MCP inspector](https://github.com/modelcontextprotocol/inspector) or [other MCP clients](/agents/guides/remote-mcp-server/#connect-your-remote-mcp-server-to-claude-and-other-mcp-clients-via-a-local-proxy). Then, once you're ready, you can customize the MCP server and add your own [tools](/agents/model-context-protocol/tools/).
...
<PackageManagers
	...
	args={"my-mcp-server --template=cloudflare/agents/examples/mcp-worker"}
/>
...
<PackageManagers
	...
	pkg="cloudflare@latest"
	args={
		"my-mcp-server-auth --template=cloudflare/agents/examples/mcp-worker-authenticated"
	}
/>
...
You'll notice that in the example MCP server, if you open `src/index.ts`, the server uses `createMcpHandler` with the `OAuthProvider`:
...
This will ensure that your users are redirected to authenticate before accessing your MCP server. To get this working, you need to create OAuth client apps in the steps below.
...
In the inspector, set **Transport Type** to `Streamable HTTP` and enter the URL of your MCP server, `http://localhost:8788/mcp`
