...
- **[State management](/agents/api-reference/store-and-sync-state/)** — Deep dive into `setState()`, `initialState`, and `onStateUpdate()`
- **[Client SDK](/agents/api-reference/client-sdk/)** — Full `useAgent` and `AgentClient` API reference
- **[Callable methods](/agents/api-reference/callable-methods/)** — Expose methods to clients with `@callable()`
- **[Scheduling](/agents/api-reference/schedule-tasks/)** — Run tasks on a delay, schedule, or cron
