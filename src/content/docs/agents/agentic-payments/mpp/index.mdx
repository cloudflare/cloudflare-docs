...
[MPP (Machine Payments Protocol)](https://mpp.dev) is an open protocol for machine-to-machine payments, co-authored by [Tempo Labs](https://tempo.xyz) and [Stripe](https://stripe.com). It standardizes the HTTP `402 Payment Required` status code with a formal authentication scheme proposed to the [IETF](https://paymentauth.org), giving agents, apps, and humans a single interface to pay for any service in the same HTTP request.
...
- [x402](/agents/agentic-payments/x402/) — Coinbase's open payment standard, compatible with MPP
