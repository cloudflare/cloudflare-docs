...
The built-in `AgentSearchProvider` uses Durable Object SQLite with FTS5 as a simple default:
...
const session = Session.create(this)
  .withContext("knowledge", {
    ...
    provider: {
      ...
      search: async (query) => {
        // Use Vectorize, Algolia, Elasticsearch — whatever you need
...
The agent gets a `search_context` tool for querying and a `set_context` tool for indexing new entries. It decides what to search for — you decide how the search works.
