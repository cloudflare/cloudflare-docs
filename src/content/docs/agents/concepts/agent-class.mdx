...
class MyAgent extends Agent {
  async onEmail(email: AgentEmail) {
    ...
    // Reply to the email with secure signature for reply routing
    await this.replyToEmail(email, {
      fromName: "My Agent",
      subject: "Re: " + email.headers.get("subject"),
      body: "Thanks for your email!",
      contentType: "text/plain",
      secret: this.env.EMAIL_SECRET // Enable secure reply routing
    });
  }
}
```
...
When replying to emails, you can include a `secret` parameter to enable HMAC-signed headers. This allows incoming replies to be securely routed back to the same agent instance, preventing header forgery and replay attacks:
...
The outbound email will include signed headers (`X-Agent-Name`, `X-Agent-ID`, `X-Agent-Sig`, `X-Agent-Sig-Ts`). To verify these signatures on incoming emails, combine the secure reply resolver with address-based routing:

```ts
import { createSecureReplyEmailResolver, createAddressBasedEmailResolver } from "agents";
...
For a complete example with security tests, refer to the [Email Agent guide](/agents/guides/email-agent/).
