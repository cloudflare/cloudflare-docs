...
- Follow the [Human-in-the-loop patterns guide](/agents/guides/human-in-the-loop/) for implementation examples
- Learn about [Workflows](/agents/api-reference/run-workflows/) for the complete approval API
- Explore [MCP server elicitation](/agents/model-context-protocol/mcp-agent-api/#elicitation-human-in-the-loop) for interactive input
