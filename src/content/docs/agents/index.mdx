...
description: Build AI agents that persist state, call tools, and communicate with users in real time.
...
import { RelatedProduct, TypeScriptExample, LinkCard } from "~/components";
...
- **Think with any model** — Call [any AI model](/agents/api-reference/using-ai-models/) — Workers AI, OpenAI, Anthropic, Gemini — and stream responses over [WebSockets](/agents/api-reference/websockets/) or [Server-Sent Events](/agents/api-reference/http-sse/). Long-running chat responses work out of the box with [`AIChatAgent`](/agents/api-reference/chat-agents/), and custom Agents can use [`keepAlive()`](/agents/api-reference/schedule-tasks/#keeping-the-agent-alive) for other long-lived work.
- **Talk in real time** — Build [voice agents](/agents/api-reference/voice-agents/) with speech-to-text, text-to-speech, and interruption handling over the same durable Agent connection.
...
- **Work on durable files** — Give an agent a persistent [Workspace](/agents/api-reference/workspace/) so it can read, write, diff, and organize files across sessions.
...
- **Reach users offline** — Send [push notifications](/agents/guides/push-notifications/) by combining durable state, scheduling, and the browser Push API.
