...
Ship interactive, real-time agents that can connect to the latest AI models, stream responses, and don't time out while waiting for the latest chain-of-thought models — including `o1` or `deepseek-r1` — to respond.
  ...
  async webSocketMessage(ws: WebSocket, message: ArrayBuffer | string) {
    try {
      ...
      const stream = await openai.chat.completions.create({
        ...
        stream: true, // Stream the response to immediately start sending chunks to the client,
