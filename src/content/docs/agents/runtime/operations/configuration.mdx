...
| `state`   | Lifecycle state (optional). Defaults to `"created"`. Set to `"deleted"`, `"renamed"`, or `"transferred"` to retire an Agent. |

For details on renaming, deleting, or transferring Agent classes, see [Durable Object class exports](/durable-objects/reference/durable-objects-migrations/). Existing Workers using the legacy `migrations` array continue to work — see [Durable Object class migrations (legacy)](/durable-objects/reference/durable-object-class-migrations-legacy/).
...
Each Agent maps to a Durable Object class. You manage the lifecycle of those classes (create, rename, delete, transfer) through the [`exports`](/durable-objects/reference/durable-objects-migrations/) field of your Wrangler configuration file.

### Adding a new agent

Add a live entry to `exports` for the new class:
...
{
	"exports": {
		"ExistingAgent": { "type": "durable-object", "storage": "sqlite" },
...
For zero-downtime renames, see the [three-deploy rename pattern](/durable-objects/reference/durable-objects-migrations/#avoid-downtime-during-a-rename).
...
Existing Workers using the legacy `migrations` array continue to work — see [Durable Object class migrations (legacy)](/durable-objects/reference/durable-object-class-migrations-legacy/) for the legacy reference, or [Migrate from the legacy `migrations` flow](/durable-objects/reference/durable-objects-migrations/#migrate-from-the-legacy-migrations-flow) to move to `exports`.

## Troubleshooting

### No such Durable Object class

The class is missing from `exports`. Add a live entry for it:
...
If your Worker uses the legacy [`migrations`](/durable-objects/reference/durable-object-class-migrations-legacy/) array, each entry must have a unique `tag`:
...
Consider converting to the declarative [`exports`](/durable-objects/reference/durable-objects-migrations/) field.
...
<LinkCard
	title="Routing"
	href="/agents/runtime/communication/routing/"
