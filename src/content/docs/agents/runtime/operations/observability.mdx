...
### Instrument AI SDK v7

AI SDK v7 provides a telemetry lifecycle. Register the Agents SDK adapter once to instrument every supported generation call:
...
To instrument one call, pass the adapter through `experimental_telemetry.integrations` instead:
...
For AI SDK v6, pass storage options to `wrapAISDK`:
...
For AI SDK v7, pass the same options to `createAISDKTelemetry`. For Think, override the corresponding properties on the agent:
