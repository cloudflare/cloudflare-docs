...
The returned handler is callable when composing MCP routing inside another Worker handler:

```ts
const response = await createMcpHandler(createServer)(request, env, ctx);
```

Do not export the callable directly as a Worker's default export. Wrangler treats function default exports as `WorkerEntrypoint` classes. Invoke it from an object handler's `fetch()` method, as shown above.
