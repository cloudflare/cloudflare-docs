...
Elicitation requires MCP client support. Not all MCP clients implement the elicitation capability. Check the client documentation for compatibility. Agents acting as MCP clients can respond to elicitation (including url mode) by overriding [`onElicitRequest`](/agents/model-context-protocol/apis/client-api/#elicitation).
:::
