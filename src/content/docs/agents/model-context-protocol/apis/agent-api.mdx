...
`McpAgent` remains available for existing servers that need Durable Object state, RPC, protocol sessions, or pushed server-to-client requests. It is deprecated and feature-frozen. New stateless servers should use [`createMcpHandler`](/agents/model-context-protocol/apis/handler-api/).

Keep importing its `McpServer` from `@modelcontextprotocol/sdk`. An SDK v2 server from `@modelcontextprotocol/server` cannot run inside `McpAgent`.
