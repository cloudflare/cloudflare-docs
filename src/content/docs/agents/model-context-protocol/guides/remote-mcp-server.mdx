...
Once deployed, this server will be live at your `workers.dev` subdomain (for example, `remote-mcp-server-authless.your-account.workers.dev/mcp`). You can connect to it immediately using the [AI Playground](https://playground.ai.cloudflare.com/) (a remote MCP client), [MCP inspector](https://github.com/modelcontextprotocol/inspector) or [other MCP clients](/agents/model-context-protocol/guides/remote-mcp-server/#connect-from-an-mcp-client).
...
1. Open a terminal and run the following command:
   ...
   Cloudflare APIs?_, choose `No`. - For _Do you want to use git for version control?_, choose `No`. - For _Do you want to deploy your application?_, choose `No` (we will be testing the server before deploying).
...
Connect an MCP client with Streamable HTTP support directly to your deployed endpoint. For example, add the server to Claude Code:

```sh
claude mcp add --transport http math https://remote-mcp-server-authless.your-account.workers.dev/mcp
```

Run `claude mcp list` to verify the connection. Then ask Claude to use one of your tools. For example:

```txt
Could you use the math tool to add 23 and 19?
```

Claude should invoke the tool and show the server response.

For other clients, refer to [Test a Remote MCP Server](/agents/model-context-protocol/guides/test-remote-mcp-server/).
...
2. For the OAuth app you just created, add the client ID of the OAuth app as `GITHUB_CLIENT_ID` and generate a client secret, adding it as `GITHUB_CLIENT_SECRET` to a `.env` file in the root of your project, which [will be used to set secrets in local development](/workers/configuration/secrets/).
