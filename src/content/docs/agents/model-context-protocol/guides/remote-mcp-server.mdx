...
| [`createLegacyMcpHandler()`](/agents/model-context-protocol/apis/handler-api/#createlegacymcphandler) | Optional             | legacy                              | Existing `WorkerTransport` servers      |
| [`McpAgent`](/agents/model-context-protocol/apis/agent-api/)                                          | Yes                  | legacy                              | Existing Durable Object and RPC servers |
...
The quick-deploy templates in this section currently use the legacy `McpAgent` path. You can still use it for sessionful deployments. For a new stateless server, start with the [`mcp-worker` example](https://github.com/cloudflare/agents/tree/main/examples/mcp-worker).
:::
