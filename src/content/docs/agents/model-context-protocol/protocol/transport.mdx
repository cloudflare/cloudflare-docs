...
Test your MCP server with a client that supports Streamable HTTP connections.

Follow [Test a Remote MCP Server](/agents/model-context-protocol/guides/test-remote-mcp-server/) for instructions for Claude Code, Cursor, Windsurf, and other MCP clients.
