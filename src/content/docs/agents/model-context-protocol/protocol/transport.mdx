...
Move stateless tools to a server factory from `@modelcontextprotocol/server`, then pass that factory to `createMcpHandler`.

Keep `McpAgent` temporarily when an endpoint needs Durable Object state, RPC, or published 2025 sessions. It remains available but is deprecated and feature-frozen.
