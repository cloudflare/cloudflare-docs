...
Use `@modelcontextprotocol/server` for a stateless `createMcpHandler` server. Existing legacy `McpAgent` servers must keep using `@modelcontextprotocol/sdk`.
...
For legacy stateful MCP servers, define tools in the `init()` method of an [`McpAgent`](/agents/model-context-protocol/apis/agent-api/). Tools have access to the agent instance via `this`, which means they can read and write state.
