...
Use `@modelcontextprotocol/server` for a modern `createMcpHandler` server. Existing `McpAgent` servers must keep using `@modelcontextprotocol/sdk`.
...
Use `server.registerTool()` to register a tool on a modern `McpServer` instance. Each tool has a name, a description, an input schema defined with [Zod](https://zod.dev), and a handler function.
