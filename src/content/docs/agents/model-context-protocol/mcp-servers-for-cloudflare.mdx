...
:::note
When using a User API Token, the token must include the **User → User Details → Read** permission. The MCP server calls the `/user` API to verify your identity — without this permission, authentication will fail with `"Failed to verify token: no user or account information"`. For an Account API Token, an equivalent **Account Read** permission is required.
:::
