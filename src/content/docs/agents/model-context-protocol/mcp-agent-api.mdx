...
Available jurisdictions include `"eu"` (European Union) and `"fedramp"` (FedRAMP compliant locations). See [Durable Objects data location](/durable-objects/reference/data-location/) for more options.
...
For more human-in-the-loop patterns including workflow-based approval, see [Human-in-the-loop patterns](/agents/guides/human-in-the-loop/).

## Next steps

- Follow the [Build a Remote MCP server](/agents/guides/remote-mcp-server/) guide to get started
- Learn about [Tools](/agents/model-context-protocol/tools/) and how to design them
- Set up [Authorization](/agents/model-context-protocol/authorization/) with OAuth
- Review [Securing MCP servers](/agents/guides/securing-mcp-server/) for production deployments
- Explore [createMcpHandler](/agents/model-context-protocol/mcp-handler-api/) for stateless MCP servers
