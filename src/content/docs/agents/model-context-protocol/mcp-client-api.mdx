...
```ts
// Preferred signature (options object)
async addMcpServer(
  serverName: string,
  url: string,
  options?: {
...
export class MyAgent extends Agent<Env, never> {
	async onRequest(request: Request): Promise<Response> {
		// New preferred format: options object as third parameter
		const result = await this.addMcpServer(
			"Weather API",
			"https://weather-mcp.example.com/mcp",
			{
...
The new options-based API is recommended as it avoids positional undefined parameters when you want to pass transport or client options without specifying callback configuration.
...
```ts title="src/index.ts"
// New format: clean options object
await this.addMcpServer("github", "https://mcp.github.com/mcp", {
	transport: { type: "sse", headers: { Authorization: "Bearer token" } }
});

// Legacy format: requires undefined placeholders
await this.addMcpServer("github", "https://mcp.github.com/mcp", undefined, undefined, {
	transport: { type: "sse", headers: { Authorization: "Bearer token" } }
});
```
...
The legacy signature with positional parameters is still supported for backward compatibility.
