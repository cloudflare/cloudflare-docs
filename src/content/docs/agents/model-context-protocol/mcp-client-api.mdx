...
When using a custom `callbackPath`, the URL becomes:

```txt
https://{host}/{callbackPath}
```

For example: `https://my-worker.workers.dev/mcp-oauth-callback`

OAuth tokens are securely stored in SQLite and persist across agent restarts.

### Protecting instance names in OAuth callbacks

When using `sendIdentityOnConnect: false` to hide sensitive instance names (like session IDs or user IDs), the default OAuth callback URL would expose the instance name in the path. This creates a security vulnerability where the sensitive instance name is:

- Sent to remote OAuth providers as the redirect URI
- Displayed in the browser address bar
- Saved in browser history

To prevent this, you **must** provide a custom `callbackPath` when `sendIdentityOnConnect` is `false`. If you attempt to call `addMcpServer()` without `callbackPath` in this configuration, it will throw an error.
...
export class SecureAgent extends Agent {
	...
	async onRequest(request: Request) {
		...
		// Without it, this call will throw an error
		const result = await this.addMcpServer(
			"github",
			"https://mcp.github.com/mcp",
			{
...
// Route the custom callback path to the agent instance
export default {
	async fetch(request: Request, env: Env) {
		...
		if (url.pathname.startsWith("/mcp-oauth-callback")) {
			// Extract the instance name from your session/auth mechanism
...
OAuth callbacks are matched by the `state` query parameter (format: `{serverId}:{stateValue}`), not by URL path substring matching. The agent verifies both:

1. The `state` parameter contains a valid server ID
2. The request origin and pathname match the registered callback URL

This means your custom `callbackPath` can be any path you choose (it does not need to contain `/callback`), as long as requests to that path are routed to the correct agent instance.
...
- `options` (object, optional) — Connection configuration:
  ...
  - `callbackPath` — Custom callback URL path that bypasses the default `/agents/{class}/{name}/callback` construction. **Required when `sendIdentityOnConnect` is `false`** to prevent leaking the instance name in the OAuth callback URL. When set, the callback URL becomes `{callbackHost}/{callbackPath}`. Should be a plain path (for example, `/mcp-callback`) without query strings or fragments. You must route this path to the agent instance via `getAgentByName`. Optional when `sendIdentityOnConnect` is `true` (the default)
