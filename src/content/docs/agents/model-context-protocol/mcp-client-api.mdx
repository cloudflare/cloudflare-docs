...
When using `sendIdentityOnConnect: false` to hide sensitive instance names (like session IDs or user IDs), the default OAuth callback URL would expose the instance name in the URL path. To prevent this security issue, you **must** provide a custom `callbackPath`. Without it, `addMcpServer()` will throw an error.
...
export class SecureAgent extends Agent {
	...
	async onRequest(request: Request) {
		...
		// Without it, addMcpServer() will throw an error
		const result = await this.addMcpServer(
			"github",
			"https://mcp.github.com/mcp",
			{
...
OAuth callbacks are matched by the `state` query parameter (format: `{serverId}:{stateValue}`), not by URL path. The agent also verifies that the request's origin and pathname match the registered callback URL for defense-in-depth. This means your custom `callbackPath` can be any path you choose, as long as requests to that path are routed to the correct agent instance.
