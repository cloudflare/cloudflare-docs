...
If the MCP server requires OAuth authentication, `authUrl` will be returned for user authentication. After the user successfully completes OAuth, the Agent clears this `authUrl` and stores the authentication tokens. Connections persist across requests, and the Agent will automatically reconnect using stored tokens if the connection is lost - without requiring users to repeat the OAuth flow.
...
You can filter the tools list to show only tools from a specific MCP server. This is useful for building UI that shows which capabilities each connected server provides:
...
export class MyAgent extends Agent<Env, never> {
	async onRequest(request: Request): Promise<Response> {
		...
		if (url.pathname === "/get-tools" && request.method === "POST") {
			const { serverId } = (await request.json()) as { serverId: string };

			// Get all tools and filter by server ID
			const allTools = this.mcp.listTools();
			const serverTools = allTools.filter((tool) => tool.serverId === serverId);

			return new Response(JSON.stringify({ tools: serverTools }), {
				headers: { "Content-Type": "application/json" },
			});
		}
	}
...
- **`serverId`** — ID of the MCP server providing this tool

This allows you to show users exactly what capabilities each connected MCP server provides, helping them understand which tools are available from which sources.
