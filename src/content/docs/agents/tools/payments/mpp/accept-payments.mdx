...
Use Cloudflare Workers to accept Machine Payments Protocol (MPP) payments. Protect an origin with a proxy, or add `mppx` to a Worker route or Model Context Protocol (MCP) tool.

<CardGrid>
	<LinkCard
		title="Protect an existing origin"
		description="Deploy an MPP proxy without changing origin code"
		href="/agents/tools/payments/mpp/protect-an-origin/"
	/>
	<LinkCard
		title="Charge for a Worker route"
		description="Add payment middleware to a Worker application"
		href="/agents/tools/payments/mpp/accept-payments/#charge-for-a-worker-route"
	/>
	<LinkCard
		title="Charge for an MCP tool"
		description="Require payment before an MCP tool returns its result"
		href="/agents/tools/payments/mpp/accept-payments/#charge-for-an-mcp-tool"
	/>
</CardGrid>
