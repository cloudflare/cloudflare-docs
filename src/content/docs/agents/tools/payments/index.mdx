...
No pre-created service account or pre-shared API key is required. Agents handle the payment exchange programmatically. A protocol may still use durable sessions or subscriptions after the first payment.
...
[Machine Payments Protocol (MPP)](https://mpp.dev) is an open protocol for machine-to-machine payments. It extends the HTTP `402` pattern with a formal `WWW-Authenticate: Payment` / `Authorization: Payment` header scheme and is on the IETF standards track.

MPP supports multiple payment methods beyond blockchain — including cards through Stripe, Bitcoin Lightning, and stablecoins — and provides **charges**, **sessions**, and **subscriptions** for fixed-price, usage-based, and recurring billing. With an EVM charge method configured, `mppx` can also handle x402 `exact` payment Challenges.
