...
Agents can use the [Sandbox SDK](/sandbox/sdk/) to run code in isolated container environments. Use Sandbox when an agent needs a real filesystem, shell commands, language runtimes, package installation, or long-lived project state that should not run inside the agent's own Worker isolate.
...
<LinkCard
	title="Sandboxes"
	href="/sandbox/"
	description="Choose between Dynamic Workers and Containers to run untrusted or generated code."
/>

<LinkCard
	title="Sandbox SDK"
	href="/sandbox/sdk/"
	description="Full Sandbox SDK documentation for commands, files, sessions, and deployment."
/>
