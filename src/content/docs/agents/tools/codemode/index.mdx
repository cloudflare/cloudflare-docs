...
Code Mode does not replace direct tool calls. Direct calls remain the simpler choice for a single, predictable operation.

Use each pattern based on the work the model must perform:

| Pattern           | Best suited for                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------- |
| Direct tool calls | One operation, small tool sets, or simple model-directed sequences                                |
| Code Mode         | Dependent calls, loops, filtering, branching, result shaping, or large discoverable tool catalogs |
...
<CardGrid>
	<LinkCard
		title="Agents SDK"
		description="Use Code Mode runtimes and connectors in an Agents SDK application."
		href="/agents/tools/codemode/agents-sdk/"
	/>
	<LinkCard
		title="AI SDK"
	...
	<LinkCard
		...
		description="Use MCP tools in Code Mode or expose Code Mode to MCP clients."
