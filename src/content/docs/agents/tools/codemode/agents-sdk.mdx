---
title: Integrate Code Mode with the Agents SDK
description: Add a durable Code Mode runtime, connectors, approvals, rollback, and reusable snippets to an Agents SDK application.
...
3. Add the Agents and Code Mode plugins to `vite.config.ts`:
   ...
   The Code Mode plugin discovers `*.codemode.ts` files. It also exports connector classes and `CodemodeRuntime` from the Worker entry module.

   Set `module` to `esnext` in `tsconfig.json` to use connector import attributes:
...
4. Create a connector. This example stores notes in the Agent's Durable Object storage:

   <TypeScriptExample filename="src/notes.codemode.ts">
   ...
   Use `McpConnector` for MCP tools or `OpenApiConnector` for OpenAPI operations. For MCP-specific setup, refer to [Use MCP tools with Code Mode](/agents/tools/codemode/mcp/use-mcp-tools/).
...
5. Import the connector and create a runtime in your Agent:
   ...
   import { NotesConnector } from "./notes.codemode" with { type: "connectors" };
