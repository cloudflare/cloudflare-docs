---
title: Wrap an MCP server with Code Mode
...
If you provide a custom `description`, include `{{types}}` for generated declarations. Include `{{example}}` to insert an example call.
...
To publish an OpenAPI service with separate `search` and `execute` tools, refer to [Publish an OpenAPI service through MCP](/agents/tools/codemode/mcp/build-from-openapi/).
