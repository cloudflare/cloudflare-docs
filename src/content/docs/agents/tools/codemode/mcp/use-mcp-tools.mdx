...
Use `McpConnector` to expose tools from an existing Model Context Protocol (MCP) connection inside the Code Mode sandbox. The connector works with the durable runtime, including discovery, approvals, and execution history.

## Prerequisites

You need a Workers project with a Code Mode runtime and an existing Agents SDK MCP connection. To create and authorize the connection, refer to the [McpClient API](/agents/model-context-protocol/apis/client-api/).
