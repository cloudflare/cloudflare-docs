...
You need a project with the [durable Code Mode runtime](/agents/tools/codemode/durable-runtime/) configured. The runtime setup provides the Worker Loader binding, Code Mode Vite plugin, and connector import convention used in this guide.
...
2. Create a `*.codemode.ts` connector. Implement `spec()` to return the document and `request()` to make authenticated host-side requests:

   <TypeScriptExample filename="src/orders.codemode.ts">
...
3. Import the connector and add it to the runtime:
   ...
   import { OrdersConnector } from "./orders.codemode" with {
   	type: "connectors",
   };
