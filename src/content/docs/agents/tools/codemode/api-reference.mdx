...
| `DEFAULT_PAUSED_TTL_MS`   | `86,400,000` | Default stale execution age in milliseconds  |
| `MAX_DURABLE_VALUE_BYTES` | `1,000,000`  | Serialized JavaScript string-length limit for one durable value |
...
The default budget is `6,000` estimated tokens at four characters per token. `maxChars` overrides the derived character budget.
...
A custom description can contain `{{types}}` and `{{example}}`. The wrapper replaces both tokens. Returned MCP values are unwrapped in this order: compatibility `toolResult`, MCP errors, `structuredContent`, all-text content, then the original mixed-content result.
...
| Behavior            | Details                                                                                                                               |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Connector discovery | Recursively finds `*.codemode.ts`, `*.codemode.tsx`, `*.codemode.js`, and `*.codemode.jsx` files. It skips `node_modules` and `dist`. |
| Import attributes   | Resolves a file or directory imported with `with { type: "connectors" }` through a generated virtual module.                          |
| Connector exports   | Re-exports discovered connector classes from `src/server`, `src/index`, or `src/worker` entry modules.                                |
| Runtime export      | Adds `export { CodemodeRuntime } from "@cloudflare/codemode"` to the Worker entry module.                                             |
| Watch mode          | Watches connector files and imported connector directories.                                                                           |
