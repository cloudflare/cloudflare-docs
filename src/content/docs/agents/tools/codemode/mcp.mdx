...
- A project with the [durable Code Mode runtime](/agents/tools/codemode/durable-runtime/) configured. That setup provides the Worker Loader binding, the Code Mode Vite plugin, the `CodemodeRuntime` export, and the `tsconfig` `esnext` setting that the `with { type: "connectors" }` import requires.
...
2. **Create an MCP connector**
   ...
   Create `src/github.codemode.ts`. The `*.codemode.ts` suffix lets the Code Mode Vite plugin discover and export the connector.

   <TypeScriptExample filename="src/github.codemode.ts">
...
3. **Add the connector to the runtime**
   ...
   import { GithubConnector } from "./github.codemode" with {
   	type: "connectors",
   };
