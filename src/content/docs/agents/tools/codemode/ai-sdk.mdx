...
const codemode = createCodeTool({
	tools: [aiTools(weatherTools), stateTools(workspace)],
...
The example exports `CodemodeRuntime` manually. If you configure the Code Mode Vite plugin as described in [Integrate Code Mode with the Agents SDK](/agents/tools/codemode/agents-sdk/), remove that manual export because the plugin adds it.
