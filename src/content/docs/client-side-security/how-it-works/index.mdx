...
When you turn on resource monitoring, Cloudflare automatically adds a `content-security-policy-report-only` HTTP header with `'none'` directives to a sample of approximately 20% of HTML responses. For details on the header format, refer to [CSP HTTP header format](/client-side-security/reference/csp-header/).
