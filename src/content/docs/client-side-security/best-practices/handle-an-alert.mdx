...
If you receive a [client-side resource alert](/client-side-security/alerts/alert-types/), sometimes you need to perform some manual investigation to confirm the nature of the script. Use the guidance provided in this page as a starting point for your investigation.

## 1. Understand what triggered the alert

Start by identifying the [detection system](/client-side-security/how-it-works/malicious-script-detection/) that triggered the alert. A link is provided in the alert that will send you directly to the Cloudflare dashboard to the relevant resource that needs reviewing. Alternatively, do the following:
...
3. Select **Details** next to the resource you identified. The example screenshot below shows a malicious script resource.
   ...
   ![Dialog box showing the details of a script considered malicious.](~/assets/images/client-side-security/handle-alert-malicious-script-example.png)
