...
:::note
Policy violation reports are only available to customers with Client-Side Security Advanced.
:::

Shortly after you configure policies (or content security rules), the Cloudflare dashboard will start displaying any violations of those policies. This information will be available for policies with any [action](/client-side-security/policies/#policy-actions) (_Allow_ and _Log_).
