...
- **Scoped alert**: An alert scoped to one or more zones. You must configure [content security rules](/client-side-security/rules/) for the zones you select to receive any notifications. Scoped alerts are triggered immediately. Policy violations will not trigger an alert. For more information, refer to [Scoped alerts](#scoped-alerts).
...
- No configured content security rules in the zone
- Content security rule is not enabled
