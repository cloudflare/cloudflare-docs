...
Once you [activate Client-Side Security's resource monitoring](/client-side-security/get-started/), the main client-side resources dashboard will show which resources (scripts and connections) are running on your domain, as well as the cookies recently detected in HTTP traffic.

If you notice unexpected scripts or connections on the dashboard, check them for signs of malicious activity. Customers with Client-Side Security Advanced will have their [connections and scripts classified as potentially malicious](/client-side-security/how-it-works/malicious-script-detection/) based on threat feeds. You should also check for any new or unexpected cookies.
...
2. Review the list of scripts, connections, and cookies for your domain, depending on your Cloudflare plan. To apply a filter, select **Add filter** and use one or more of the available options.
   ...
   - **Status**: Filter scripts or connections by [status](/client-side-security/reference/script-statuses/).
   - **Script URL**: Filter scripts by their URL.
   - **Connection URL**: Filter connections by their target URL. Depending on your [configuration](/client-side-security/reference/settings/#connection-target-details), it may search only by target hostname.
...
:::note
Data export is only available to customers with Client-Side Security Advanced.
:::
