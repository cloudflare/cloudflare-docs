...
| [List client-side security policies][9]    | `GET zones/{zone_id}/page_shield/policies`                    | Fetch a list of all configured CSP policies.                |
| [Get a client-side security policy][10]    | `GET zones/{zone_id}/page_shield/policies/{policy_id}`        | Fetch the details of a CSP policy.                          |
| [Create a client-side security policy][11] | `POST zones/{zone_id}/page_shield/policies`                   | Creates a CSP policy with the provided configuration.       |
| [Update a client-side security policy][12] | `PUT zones/{zone_id}/page_shield/policies/{policy_id}`        | Updates an existing CSP policy.                             |
| [Delete a client-side security policy][13] | `DELETE zones/{zone_id}/page_shield/policies/{policy_id}`     | Deletes an existing CSP policy.                             |
...
This example enables client-side security in the specified zone.
...
This `POST` request creates a client-side security policy (or content security rule) with _Log_ action, defining the following scripts as allowed based on where they are hosted:
...
:::note
For a list of CSP directives and keywords supported by policies, refer to [Supported CSP directives](/client-side-security/policies/csp-directives/).
:::
