...
A free Universal SSL certificate is available for all new Cloudflare domains added via a hosting partner using both [primary (full)](/dns/zone-setups/full-setup/) and <GlossaryTooltip term="partial setup" link="/dns/zone-setups/partial-setup/">CNAME (partial)</GlossaryTooltip> setups.
