...
DCV tokens are generated by the certificate authority (CA), not by Cloudflare. Each CA defines how long its tokens remain valid:
...
After this period, tokens expire as required by [CA/B Baseline Requirements](https://cabforum.org/baseline-requirements-documents/), and Cloudflare generates new ones automatically. Tokens can also become invalid when a validation attempt fails — refer to [DCV tokens](/ssl/edge-certificates/changing-dcv-method/dcv-flow/#dcv-tokens) for details on when this occurs.
...
If HTTP DCV fails continuously, Cloudflare falls back to TXT-based DCV on a schedule based on the certificate validity period:

| Certificate validity | Fallback after |
| --- | --- |
| 90 days | 15 days of continuous HTTP failure |
| 30 days | 7 days of continuous HTTP failure |
| 14 days | 3 days of continuous HTTP failure |

Once the fallback occurs, TXT tokens appear in the [SSL Verification API](/api/resources/ssl/subresources/verification/methods/list/) response. You must place the TXT record at your authoritative DNS provider to complete validation.

### Renewal behaviour

At certificate renewal, Cloudflare always attempts HTTP DCV first, regardless of the method used during initial issuance. Automatic HTTP renewal succeeds only when all of the following conditions are met:

- All hostnames on the certificate are proxied through Cloudflare.
- All hostnames resolve to the IPs assigned to the zone.
- The certificate does not contain wildcard entries.

If any condition is not met, renewal falls back to your configured DCV method and you must complete the DCV process manually. Cloudflare generates renewal tokens 30 days before the certificate expiration date.
