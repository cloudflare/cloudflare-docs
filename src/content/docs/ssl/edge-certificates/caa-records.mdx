...
- Your domain is on a [full setup](/dns/zone-setups/full-setup/) (not a [CNAME setup](/dns/zone-setups/partial-setup)).
...
If you have CAA records on your domain, they must permit the certificate authority (CA) that Cloudflare uses.

| Cloudflare CA | Required CAA `issue` value |
|---|---|
...
**Example:** If Cloudflare is using Google Trust Services, your CAA record must include:
