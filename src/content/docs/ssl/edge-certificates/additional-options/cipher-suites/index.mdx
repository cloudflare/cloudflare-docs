...
The settings on this page apply to cipher suites used in connections between visitors and the Cloudflare network. Cipher suites used between Cloudflare and your origin server are configured separately — refer to [Origin server > Cipher suites](/ssl/origin-configuration/cipher-suites/).
...
Cloudflare's default cipher suites ([Legacy](/ssl/edge-certificates/additional-options/cipher-suites/recommendations/)) prioritize broad compatibility, which means they include older algorithms that security testing tools may flag.
