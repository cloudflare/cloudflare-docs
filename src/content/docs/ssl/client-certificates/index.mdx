...
Cloudflare validates client certificates against CAs set at the account level. Because validation is account-level, the same certificates work across multiple zones and domains under your account (for example, `host.example.com`, `name.example.net`, `secure.anotherdomain.test`) as long as mTLS is enabled for each hostname.
