...
### How mTLS works on Cloudflare

When a client connects to a hostname configured for mTLS, Cloudflare does the following:

1. Requests a client certificate during the TLS handshake.
2. Validates the certificate against the Client CA configured on your account (Cloudflare-managed by default, or a custom CA on Enterprise accounts).
3. Records the validation result in [`cf.tls_client_auth.cert_verified`](/ruleset-engine/rules-language/fields/reference/?field-category=mTLS&field-category=SSL/TLS).

You can enforce client certificate validation using [WAF custom rules](/waf/custom-rules/), [API Shield](/api-shield/security/mtls/configure/), Cloudflare Workers (by inspecting [`request.cf.tlsClientAuth`](/workers/runtime-apis/request/)), or by [forwarding a client certificate](/ssl/client-certificates/forward-a-client-certificate/) to your origin.
