...
- [BYOCA](/ssl/client-certificates/byo-ca/) certificates: This is an API-only option, available on Enterprise accounts. Certificates and hostname associations are **not** listed on your dashboard.

Cloudflare then records the validation result in [`cf.tls_client_auth.cert_verified`](/ruleset-engine/rules-language/fields/reference/?field-category=mTLS&field-category=SSL/TLS).
