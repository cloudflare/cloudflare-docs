...
title: Entrust distrust by popular browsers
sidebar:
  order: 1
...
New Entrust certificates issued on **November 1, 2024 or after** will not be trusted on Chrome by default. And new Entrust certificates issued or on **December 1, 2024 or after** will not be trusted on Mozilla by default.
...
Since Cloudflare also [partners with SSL.com](/ssl/reference/certificate-authorities/), switching from uploading custom certificates by Entrust to using Cloudflare's managed certificates brings several advantages:
