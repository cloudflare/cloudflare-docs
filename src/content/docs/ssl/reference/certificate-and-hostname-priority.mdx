...
* If Customer1 already has a proxied record for `www.example.com` in a legacy custom hostname setup and Customer2 creates and verifies a new wildcard custom hostname for `*.example.com`, traffic is routed to Customer1's due to exact hostname match taking precedence.
