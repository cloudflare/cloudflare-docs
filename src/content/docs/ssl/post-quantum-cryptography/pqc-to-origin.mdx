...
Cloudflare uses [automatic key exchange](/ssl/origin-configuration/automatic-key-exchange/) to learn which key agreement the origin servers for a zone prefer. Cloudflare applies one preference across the zone. When the selected preference is [X25519MLKEM768](/ssl/post-quantum-cryptography/#hybrid-key-agreement), Cloudflare sends that key share in the first `ClientHello`.
...
Automatic key exchange is the default for eligible zones. You can use the [API](/api/resources/origin_post_quantum_encryption/methods/update/) to set a manual post-quantum preference.
