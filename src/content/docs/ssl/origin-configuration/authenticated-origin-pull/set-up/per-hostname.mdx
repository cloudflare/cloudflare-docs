...
2. Select the **Authenticated Origin Pulls** tab.
3. In the **Per-hostname** section, switch the toggle to **On** for the hostname you want to enable.

If you had set up logging on your origin during step 2, test and confirm that Authenticated Origin Pulls is working.

</TabItem> <TabItem label="API">

Send a [`PUT`](/api/resources/origin_tls_client_auth/subresources/hostnames/methods/update/) request to enable Authenticated Origin Pulls for specific hostnames.
