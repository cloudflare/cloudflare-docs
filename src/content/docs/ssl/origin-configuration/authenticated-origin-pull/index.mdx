...
There are three independent AOP configurations. Each has its own certificate and enablement setting. All of them require you also set up your origin server - refer to each of the specific guides to learn more.
...
- **[Zone-level](/ssl/origin-configuration/authenticated-origin-pull/set-up/zone-level/)**: Uses a certificate that you upload. Applies to all proxied traffic on the zone. Provides stricter security because the certificate is exclusive to your account.
...
If you need to guarantee that requests come from your specific Cloudflare account (not just from the Cloudflare network), upload your own certificate when setting up [zone-level](/ssl/origin-configuration/authenticated-origin-pull/set-up/zone-level/) or [per-hostname](/ssl/origin-configuration/authenticated-origin-pull/set-up/per-hostname/) AOP. You should also consider [additional security measures for your origin](/fundamentals/security/protect-your-origin-server/).
