...
1. Work on a branch or staging deployment. Finish the code changes below, then cut production over in one deploy.
...
3. Inventory call sites in the Worker **and** any self-deployed bridge clients:
...
Finish the code changes above on a branch first. Production cutover is one deploy of the preview Worker package and the matching container image.

Stable Sandbox and `@next` use different control protocols. A mixed pair does not work in either direction:

- New Worker code against an old container image fails.
- Old Worker code against a new container image fails.
...
If you also run a self-deployed bridge, redeploy that Worker from the preview template in the same cutover window.
...
9. If you run a self-deployed bridge: confirm the preview template is live and clients no longer use `/exec`, sessions, `/pty`, or `timeout_ms`.
