...
Background on container rollouts: [Containers rollouts](/containers/platform-details/rollouts/). For day-to-day Sandbox deploys (not only this migration), see [Deploy a Sandbox application](/sandbox/guides/deploy/) and [Containers deployments](/containers/deployments/).
...
Install [Cloudflare Skills](https://github.com/cloudflare/skills) for your agent ([Agent setup](/agent-setup/)). The **`sandbox-migrate-to-next`** skill performs this migration. For new apps on `@next`, use **`sandbox-next`**. For day-to-day work on the current stable package, use **`sandbox-stable`**. Deprecated-API cleanup while staying on stable is in the [2026 deprecation guide](/sandbox/guides/2026-deprecation/) (and **`sandbox-stable`**) before or instead of this guide.
...
- [Deploy a Sandbox application](/sandbox/guides/deploy/)
