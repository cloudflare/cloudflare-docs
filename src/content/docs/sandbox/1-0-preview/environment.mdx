...
This page documents environment variables on `@cloudflare/sandbox@next`, the preview of Sandbox SDK 1.0. For today's stable package, refer to [Environment variables](/sandbox/configuration/environment-variables/).
:::
...
2. Names from **`setEnvVars`**, when you use `exec()` (see below).
...
| Value | Effect |
| ----- | ------ |
| string | Store the name for later `exec()` launches |
| `undefined` or `null` | Remove a stored name |
...
Inside an interactive shell, `export` applies for the life of that terminal. It does not apply to later `exec()` calls. See [Terminals](/sandbox/1-0-preview/terminals/).
...
See [Handle outbound traffic](/sandbox/guides/outbound-traffic/), including securely injecting credentials. For Workers bindings (KV, R2, and similar) reached by hostname from the sandbox, see [Connect to Workers bindings](/sandbox/guides/workers-connections/).
