...
Start a terminal from an argument vector (usually a shell). Resolves when the terminal resource is created.
...
```ts
const terminal = await sandbox.getTerminal(id: string): Promise<Terminal | null>;
```
...
```ts
const terminals = await sandbox.listTerminals(): Promise<Terminal[]>;
```
...
```ts
await terminal.write(data: Uint8Array): Promise<void>;
```
...
```ts
await terminal.resize(cols: number, rows: number): Promise<void>;
```

### `output()`

```ts
const stream = await terminal.output(options?: TerminalOutputOptions);
// ReadableStream<TerminalOutputEvent>
```
...
```ts
const exit = await terminal.waitForExit(options?: {
	timeout?: number;
	signal?: AbortSignal;
}): Promise<ProcessExit>;
```
...
```ts
await terminal.interrupt(): Promise<void>;
await terminal.terminate(): Promise<void>;
```
...
```ts
const response = await terminal.connect(
	request: Request,
	options?: {
		cursor?: string;
		cols?: number;
...
| --------- | --------------- |
| Unknown terminal ID in the current container | Terminal not found |
| `getTerminal` / `listTerminals` while no container is running | `null` / `[]` (does not start a container; not an error) |
| Handle or terminal ID from a previous container for this sandbox | Stale terminal handle |
| Invalid working directory at create | Invalid terminal cwd |
| Invalid output cursor | Invalid terminal cursor |
| Control operation failed | Terminal control error |

Exact class names include `TerminalNotFoundError`, `StaleTerminalHandleError`, `InvalidTerminalCwdError`, `InvalidTerminalCursorError`, and `TerminalControlError` from `@cloudflare/sandbox`.

Terminals follow the same lifetime rules as processes: [How long a process lives](/sandbox/1-0-preview/processes/#how-long-a-process-lives).
