...
Process IDs are not durable across container stop or replace. Refer to [How long a process lives](/sandbox/1-0-preview/processes/#how-long-a-process-lives).
...
Signals delivered only to descendants do not rewrite this outcome. Refer to [Process execution](/sandbox/1-0-preview/processes/).
...
`getProcess` and `listProcesses` do not throw for missing work. They return `null` or `[]` when no container is up, the ID is unknown in the current container, or the process belonged to a previous container. The error classes below apply to operations on a process handle (and to launch), not to those lookups.
