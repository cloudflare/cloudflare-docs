...
try {
	...
	if (error instanceof OperationInterruptedError) {
		// error.reason, error.retryable, error.context.admitted
	}
	if (error instanceof RPCTransportError) {
		// error.kind, error.originalMessage
	}
}
```
...
| `toJSON()` | Serializes the fields above for logs |
...
| Class | Code | Key context | Details | Recommended fix |
| ----- | ---- | ----------- | ------- | ---------------- |
| `RuntimeControlProtocolError` | `INTERNAL_ERROR` | `reason` | Worker and container could not complete setup together (metadata or protocol mismatch). | Deploy the Worker package and container image from the same release line. Fix configuration if needed. |
...
Related permanent problems (not always a typed `RuntimeControlProtocolError`, but same response):
...
| Account or location capacity limits | See [Production capacity limits](#production-capacity-limits) |
...
See [Platform limits](/sandbox/platform/limits/).
