...
This page documents sandbox lifecycle on `@cloudflare/sandbox@next`, the preview of Sandbox SDK 1.0. For the current stable package, also refer to [Sandbox lifecycle](/sandbox/concepts/sandboxes/). Platform placement and shutdown details live in [Lifecycle of a Container](/containers/platform-details/architecture/).
:::
...
`keepAlive` and `sleepAfter` change idle behavior. They do not keep one container instance forever. Refer to the stable [Lifecycle API](/sandbox/api/lifecycle/) and [Sandbox options](/sandbox/configuration/sandbox-options/) (ignore transport and default-session options on `@next`).
