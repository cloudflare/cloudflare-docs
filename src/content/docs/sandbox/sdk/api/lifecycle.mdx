---
title: Lifecycle
description: Create, configure, and manage Sandbox SDK 0.x container instances and their resources.
...
head:
  - tag: title
    content: "Lifecycle (Sandbox SDK 0.x)"
---
...
- `options` (optional) - See [SandboxOptions](/sandbox/sdk/configuration/sandbox-options/) for all available options:
...
:::note
The container starts lazily on first operation. Calling `getSandbox()` returns immediately—the container only spins up when you execute a command, write a file, or perform other operations. See [Sandbox lifecycle](/sandbox/sdk/concepts/sandboxes/) for details.
:::
...
- [Sandbox lifecycle concept](/sandbox/sdk/concepts/sandboxes/) - Understanding container lifecycle and state
- [Sandbox options configuration](/sandbox/sdk/configuration/sandbox-options/) - Configure `keepAlive` and other options
- [Sessions API](/sandbox/sdk/api/sessions/) - Create execution contexts within a sandbox
