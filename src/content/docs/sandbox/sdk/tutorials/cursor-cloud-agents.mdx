...
sidebar:
  order: 5
description: Deploy Cursor self-hosted machines that run each assigned session in a Sandbox SDK 0.x container.
products:
  - sandbox
  - containers
head:
  - tag: title
    content: "Run Cursor Cloud Agents on Cloudflare via self-hosted machines (Sandbox SDK 0.x)"
---
