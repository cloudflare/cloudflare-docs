...
sidebar:
  order: 9
description: Connect browser-based terminals to Sandbox SDK 0.x shells using xterm.js or raw WebSockets.
products:
  - sandbox
head:
  - tag: title
    content: "Browser terminals (Sandbox SDK 0.x)"
---

import { TypeScriptExample, PackageManagers, Render } from "~/components";
...
For the full addon API, refer to the [Terminal API reference](/sandbox/sdk/api/terminal/).
...
For the full protocol specification, refer to the [WebSocket protocol section](/sandbox/sdk/api/terminal/#websocket-protocol) in the API reference.
...
- [Terminal API reference](/sandbox/sdk/api/terminal/) — Method signatures, addon API, and WebSocket protocol
- [Terminal connections](/sandbox/sdk/concepts/terminal/) — How terminal connections work
