...
sidebar:
  order: 6
description: Clone repositories, manage branches, and automate Git operations with the Sandbox SDK 0.x.
products:
  - sandbox
head:
  - tag: title
    content: "Work with Git (Sandbox SDK 0.x)"
---

import { TypeScriptExample, Render } from "~/components";

<Render file="sdk-0x-note" product="sandbox" params={{ href: "/sandbox/commands/run-tests-from-a-git-repository/", title: "Run tests from a Git repository" }} />
...
:::note[More secure alternative]
Embedding a token in the URL passes the credential directly into the sandbox. For better access control, use an outbound handler that injects the real token at request time — the sandbox never holds the credential. Refer to [Handle outbound traffic](/sandbox/sdk/guides/outbound-traffic/).
:::
...
- [Files API reference](/sandbox/sdk/api/files/) - File operations after cloning
- [Execute commands guide](/sandbox/sdk/guides/execute-commands/) - Run git commands
- [Manage files guide](/sandbox/sdk/guides/manage-files/) - Work with cloned files
