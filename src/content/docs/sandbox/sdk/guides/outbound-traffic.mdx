...
sidebar:
  order: 16
description: Intercept and handle outbound HTTP from Sandbox SDK 0.x sandboxes using Workers.
products:
  - sandbox
head:
  - tag: title
    content: "Handle outbound traffic (Sandbox SDK 0.x)"
---

import { TypeScriptExample, Render } from "~/components";

<Render file="sdk-0x-note" product="sandbox" params={{ href: "/sandbox/network/call-an-authenticated-api/", title: "Call an authenticated API from a sandbox" }} />
...
- [Connect to Workers bindings](/sandbox/sdk/guides/workers-connections/) like KV, R2, and Durable Objects
...
- [Connect to Workers bindings](/sandbox/sdk/guides/workers-connections/) — Access KV, R2, Durable Objects, and other bindings from a sandbox
