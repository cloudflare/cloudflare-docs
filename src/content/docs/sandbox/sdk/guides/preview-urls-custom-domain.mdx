...
sidebar:
  order: 10
description: Set up wildcard DNS, routes, and TLS so Sandbox SDK 0.x exposePort preview URLs work on your domain.
products:
  - sandbox
head:
  - tag: title
    content: "Configure preview URLs on a custom domain (Sandbox SDK 0.x)"
---

import { WranglerConfig, Render } from "~/components";

<Render file="sdk-0x-note" product="sandbox" params={{ href: "/sandbox/previews/serve-previews-on-their-own-hostnames/", title: "Serve previews on their own hostnames" }} />

Set up wildcard DNS, routes, and TLS so `exposePort()` preview URLs work on your domain. To deploy the Worker and sandbox image, refer to [Deploy a Sandbox application](/sandbox/sdk/guides/deploy/).
...
For public URLs without custom-domain setup, [`sandbox.tunnels`](/sandbox/sdk/api/tunnels/) is an alternative: quick tunnels for development, named tunnels for stable hostnames in production.
:::
...
- Sandbox app already [deployable](/sandbox/sdk/guides/deploy/) (Worker + image)
...
If this deploy also changes your sandbox image or package, follow [Deploy a Sandbox application](/sandbox/sdk/guides/deploy/) for rollout and package/image pairing. For route-only changes you can still use a normal deploy. Use `--containers-rollout=none` only when you intentionally skip container image and instance updates.
...
- [Deploy a Sandbox application](/sandbox/sdk/guides/deploy/) - Deploy Worker and image
- [Preview URLs](/sandbox/sdk/concepts/preview-urls/) - How preview URLs work
- [Expose services](/sandbox/sdk/guides/expose-services/) - Patterns for exposing ports
- [Tunnels API](/sandbox/sdk/api/tunnels/) - Zero-config `*.trycloudflare.com` URLs for development
