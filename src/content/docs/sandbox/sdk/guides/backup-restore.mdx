...
sidebar:
  order: 11
description: Snapshot a sandbox directory to R2 and restore it later with the Sandbox SDK 0.x.
products:
  - sandbox
head:
  - tag: title
    content: "Backup and restore (Sandbox SDK 0.x)"
---
...
For why production restore uses an overlay, refer to [Directory backups](/sandbox/sdk/concepts/backup-restore/).
...
This failure does not occur in `wrangler dev`, which extracts the archive. For overlay restore, refer to [Directory backups](/sandbox/sdk/concepts/backup-restore/).
...
- [Directory backups](/sandbox/sdk/concepts/backup-restore/) - Overlay restore, local extract, and `EXDEV`
- [Backups API](/sandbox/sdk/api/backups/) - Methods, options, and types
