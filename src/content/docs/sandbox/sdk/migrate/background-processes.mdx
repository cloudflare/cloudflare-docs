...
Remove the `timeout` option. 0.12 did not stop a background process when the time passed. To limit how long a process runs, start it with `timeout`, such as `["timeout", "300", "npm", "test"]`, which exits with code `124` when the time passes. For the `sessionId` option, refer to [Replace sessions](/sandbox/sdk/migrate/commands/#replace-sessions).
