...
7. Replace each class field and `getSandbox()` option that your code sets:
   ...
   | `defaultPort`, `requiredPorts` | `this.container.getTcpPort(port)`. Refer to [Move preview URLs and tunnels](/sandbox/sdk/migrate/previews-and-tunnels/). |
   ...
   | `containerTimeouts` | An `AbortSignal` on the calls that need a deadline. Refer to [Replace timeouts](/sandbox/sdk/migrate/commands-and-files/#replace-timeouts). |
   | `enableDefaultSession` | Nothing. Each `exec()` call is its own process. Refer to [Replace sessions](/sandbox/sdk/migrate/commands-and-files/#replace-sessions). |
...
- To change `exec()`, file calls, and sessions, refer to [Change commands and file calls](/sandbox/sdk/migrate/commands-and-files/).
