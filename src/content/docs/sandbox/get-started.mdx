...
**Architecture note:** The Sandbox SDK base images (`docker.io/cloudflare/sandbox:*`) are built for `linux/amd64` to match production. If you're on an ARM-based system (Apple Silicon, ARM Windows/Linux), Docker will use emulation automatically. See [Troubleshooting](/sandbox/troubleshooting/) if you encounter `exec format error`.
