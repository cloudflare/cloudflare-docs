...
description: Compare Dynamic Workers and Containers for isolated code execution, scoped APIs, and workloads that require Linux.
...
Your application may need to run code it did not create. A small task may need only APIs you provide, while a test runner, build tool, or server may expect Linux. Cloudflare supports both kinds of work, and your [Worker](/workers/) chooses the environment for each job.
...
Both environments can process text or bytes. Pass the data into a Dynamic Worker when code only needs to parse or transform it. Use a Container when the tool needs a Linux command, runtime, or filesystem behavior.
