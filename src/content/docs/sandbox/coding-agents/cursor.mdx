...
Run Cursor Cloud Agents on Cloudflare through [self-hosted machines](https://cursor.com/docs/cloud-agent/self-hosted). Each Cursor session assigned to the deployment runs in its own Linux sandbox: a [Container](/containers/) that one Durable Object starts for that session.
...
<VendorAgentConnections vendor="cursor" client:visible>
...
A cron trigger runs the Worker every five minutes. Each run lists the sessions that wait for `CURSOR_POOL`, then keeps Cursor's server-sent events stream open until shortly before the next run, so the Worker also receives sessions that arrive between runs.

The Worker claims each session with a unique worker ID. Cursor assigns a claimed session only to that Worker, which prevents duplicate processing. A Durable Object for the session then starts one container with the session environment and repository information.
...
The container opens an outbound connection to Cursor, so it does not require an inbound port or public IP address. The Worker only exposes its health and optional snapshot routes. Cursor remains responsible for the agent loop and session orchestration.

## Security

Each session runs in its own container, so sessions do not share files or processes. The container receives the `CURSOR_API_KEY` service-account key, because the Cursor worker needs it. It also receives `GIT_USERNAME`, `GIT_TOKEN`, and `SNAPSHOT_AUTH_TOKEN` if you set them. Code that runs in a session can read these values. Give `GIT_TOKEN` access only to the repositories that the agents use.
