...
- **Network access:** The example enables outbound Internet access so `codex exec-server` can reach OpenAI. Use [Container outbound traffic controls](/containers/guides/outbound-traffic/) to restrict destinations or inject credentials for other services.
...
For more information, refer to [Containers architecture](/containers/concepts/architecture/).
