...
1. Replace the `Dockerfile`. Only the highlighted lines differ from the tutorial's:

   ```dockerfile title="Dockerfile" ins={7-8}
...
4. Replace the `agentCommand()` and `agentEnv()` methods:
   ...
   `opencode run` runs one task without an interactive terminal:
...
5. Replace the `outcome()` method:
   ...
   OpenCode exits with a nonzero code on any model or session error, so the exit code decides the outcome. A successful task returns the last `text` event. A failed task returns the message from the last `error` event, such as the gateway's message for a rejected token or missing Unified Billing credits.
...
- [OpenCode example](https://github.com/cloudflare/sandbox-sdk/tree/main/examples/coding-agents/opencode): a separate deployable Worker that runs OpenCode.
