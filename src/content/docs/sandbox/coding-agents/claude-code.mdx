...
1. Create a Worker project:
   ...
   <PackageManagers
   	...
   	args={"sandbox-claude-code --category=hello-world --type=hello-world --lang=ts --no-deploy --no-git --no-agents"}
   />

2. Change into the project directory:

   ```sh
   cd sandbox-claude-code
   ```
...
5. Replace `wrangler.jsonc`. Replace `<ACCOUNT_ID>` with your account ID, and `default` with your gateway ID if it is different:
   ...
   ```jsonc
   {
   	...
   	"migrations": [
   		{
   			"new_sqlite_classes": ["ClaudeCodeSandbox"],
   			"tag": "v1",
   		},
   	],
   }
...
Create `src/sandbox.ts`. The `ClaudeCodeSandbox` Durable Object clones a repository, runs one Claude Code task at a time, and reports the task's state and the repository diff:
...
export class ClaudeCodeSandbox extends DurableObject<Env> {
	...
	startTask(prompt: string): Promise<"started" | "busy"> {
		return this.ctx.blockConcurrencyWhile(async () => {
			...
			const task = await this.container.exec(
				["/bin/sh", "-c", taskScript, "agent", ...this.claudeCommand(prompt)],
				{
					cwd: repositoryDirectory,
					env: { ...trustEnv, ...this.claudeEnv() },
					stdout: "ignore",
					stderr: "ignore",
				},
			);
	...
	private claudeCommand(prompt: string): string[] {
		return [
			"claude",
			"--print",
			"--output-format",
...
export { ClaudeCodeSandbox } from "./sandbox";
...
4. Save the `workers.dev` URL that Wrangler printed in a shell variable:

   ```sh
   WORKER_URL=https://sandbox-claude-code.<YOUR_SUBDOMAIN>.workers.dev
   ```
...
- Send Claude Code's events to a client while the task runs. Refer to [Stream command output](/sandbox/how-to/stream-command-output/).
