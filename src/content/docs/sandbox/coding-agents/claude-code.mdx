...
Claude Code running in the container does not keep the container running. `startTask()` schedules an [alarm](/durable-objects/api/alarms/) that checks the task every minute while it runs, and stops rescheduling when the task ends. The constructor sets the 30-minute inactivity timeout again whenever it finds the container running. For more information, refer to [Sandbox lifetime](/sandbox/lifetime/#activity-keeps-an-instance-running).
