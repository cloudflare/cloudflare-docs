        	...
        	export default {
        		async fetch(request: Request, env: Env): Promise<Response> {
        			...
        			if (state.watch.changed || state.watch.overflowed) {
        				// Reconcile from the current filesystem state before checkpointing
        			...
        			return Response.json({
        				...
        				changed: state.watch.changed
        			});
        		}
        	};
        	```

    	Use `watch()` for live SSE streams, or `ensureWatch()` when a background consumer needs watch state across reconnects.
    </TabItem>
...
Monitor files and directories with live SSE streams or persistent watch state. Build hot reloading development servers, background indexers, and automation that reconnects while the sandbox stays alive.

</Feature>

<Feature header="Proxy external API requests securely" href="/sandbox/guides/proxy-requests/" cta="Learn about request proxying">
