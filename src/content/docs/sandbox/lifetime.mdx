...
A running instance keeps the image, entrypoint, and environment variables that it started with. `start()` throws while an instance runs, so a new image or new startup options apply only to instances that start after the deploy. To move a sandbox to the new image, call `destroy()` and start the instance again. Its files and processes end with the old instance.
