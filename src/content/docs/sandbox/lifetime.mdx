...
Code that runs inside the instance does not count as activity. A build, a test suite, or an agent that works longer than the inactivity timeout stops with the instance when no requests arrive. To keep long work running, schedule a Durable Object [alarm](/durable-objects/api/alarms/) that runs more often than the timeout and checks the work. [Run background processes](/sandbox/commands/run-background-processes/) uses an alarm that runs every minute.

## Set the timeout on every request
...
Call `setInactivityTimeout()` on every request that uses the instance, including alarms. A timeout set only after `start()` lasts until the first restart.
...
The coding agent's repository, dependencies, and edits return from a snapshot. The development server does not, so the agent starts it again. To save and restore a workspace, refer to [Save and restore a sandbox with snapshots](/sandbox/files/save-and-restore-a-workspace/).
