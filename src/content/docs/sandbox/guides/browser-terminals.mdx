...
This page documents browser terminals on today's stable `@cloudflare/sandbox` package.

**Sandbox SDK 1.0** (preview on `@next`) keeps terminals separate from process handles. Confirm preview terminal method names in your installed `@next` package and read [processes versus terminals](/sandbox/1-0-preview/processes/#processes-versus-terminals).
:::
