...
// Access files directly under the prefix
await sandbox.exec('ls', { args: ['/images'] });
// Shows files from my-bucket:/uploads/images/ subdirectory
...
Mount specific subdirectories within a bucket using the `prefix` option. Only the contents under the prefix will be visible at the mount point.

<TypeScriptExample>
```typescript
// Mount only the /user-data/uploads/ subdirectory
await sandbox.mountBucket('my-bucket', '/uploads', {
	endpoint: 'https://YOUR_ACCOUNT_ID.r2.cloudflarestorage.com',
	prefix: '/user-data/uploads/'
});

// Files appear at mount point without the prefix
// Bucket: my-bucket/user-data/uploads/file.txt
// Mounted path: /uploads/file.txt
await sandbox.exec('ls', { args: ['/uploads'] });
...
:::note[Prefix format]
The `prefix` must start and end with `/` (for example, `/data/uploads/`). This follows s3fs format requirements.
:::
