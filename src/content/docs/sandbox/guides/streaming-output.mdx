...
In the **1.0 preview** (`@next`), stream process output with `process.logs()` (and related waits) on the handle returned by `exec()`. Refer to [Process execution](/sandbox/1-0-preview/processes/#logs-and-large-output).
:::
