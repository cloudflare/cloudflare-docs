...
In the **1.0 preview** (`@next`), stream with process handle methods such as `logs()` after `exec(argv)`. Refer to [Process execution](/sandbox/1-0-preview/processes/) or the [Process API](/sandbox/1-0-preview/process-api/).
:::
