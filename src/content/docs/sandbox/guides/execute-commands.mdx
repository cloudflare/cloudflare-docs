...
This page documents today's stable `@cloudflare/sandbox` package (`exec`, `execStream`, `startProcess`).

**Sandbox SDK 1.0** (preview on `@next`) consolidates those APIs into argv `exec()` plus process handles. Prefer the preview for new work. Refer to [Process execution](/sandbox/1-0-preview/processes/) or [migrate command execution](/sandbox/1-0-preview/migrate/#migrate-command-execution).
:::
...
// Run a script file
await sandbox.writeFile('/workspace/analyze.py', `import sys
