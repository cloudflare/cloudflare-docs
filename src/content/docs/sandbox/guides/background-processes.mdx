...
This page documents `startProcess()` on today's stable `@cloudflare/sandbox` package.

In the **1.0 preview** (`@next`), long-running work uses the same `exec()` process handle as other commands (`waitForPort`, `logs`, `kill`). Refer to [migrate command execution](/sandbox/1-0-preview/migrate/#background-processes-and-streaming).
:::
