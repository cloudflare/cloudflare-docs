...
In the **1.0 preview** (`@next`), the interpreter is an opt-in extension at `@cloudflare/sandbox/interpreter`. Refer to [Extensions](/sandbox/1-0-preview/extensions/#code-interpreter).
:::
...
// Execute code
const result = await sandbox.runCode(`print("Hello from Code Interpreter!")
result = 2 + 2
print(f"2 + 2 = {result}")`, { context: context.id });
...
const result = await sandbox.runCode(`mean = np.mean(data)
print(f"Mean: {mean}")`, { context: context.id });
