...
Update to the latest Sandbox SDK release before changing transport or session configuration.

- RPC transport: if your project uses a version earlier than `0.9.1`, deploy a newer `@cloudflare/sandbox` package and container image before switching to RPC.
- Session isolation with `enableDefaultSession: false`: requires Sandbox SDK `0.10.3` or newer. On `0.9.1`–`0.10.2`, upgrade first, then set the flag.
...
An agent skill is available to assist with the migration: [deprecation skill](/sandbox/guides/2026-deprecation/SKILL/).
