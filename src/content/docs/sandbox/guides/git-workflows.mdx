...
In `@cloudflare/sandbox@next`, Git support moves from the core SDK into a plugin. The public interface remains available through a plugin namespace. For more information, refer to [Migrate to Sandbox SDK 1.0](/sandbox/guides/migrating-to-1-0/).
:::
...
Embedding a token in the URL passes the credential directly into the sandbox. For better access control, use a Worker proxy that validates a short-lived JWT and injects the real token at request time — the sandbox never holds the credential. Refer to [Proxy requests to external APIs](/sandbox/guides/proxy-requests/).
:::
