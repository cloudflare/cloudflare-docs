...
This guide configures **preview URLs** for `exposePort()` on a custom domain. It does **not** deploy or roll out your sandbox container image. For Worker + image deploys, see [Deploy a Sandbox application](/sandbox/guides/deploy/).
...
Command examples that use `startProcess` are stable-only. On **`@next`**, use argv `exec` process handles — [Process execution](/sandbox/1-0-preview/processes/). Package and image still must match on the same release line.
:::

Preview URLs need wildcard DNS because each exposed port gets a unique subdomain: `https://8080-abc123.yourdomain.com`.

The `.workers.dev` domain does not support wildcard subdomains, so preview URLs in production need a custom domain.
...
- [Deploy a Sandbox application](/sandbox/guides/deploy/) — Worker + image deploy and rollouts
