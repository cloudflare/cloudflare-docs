...
```
import { parseSSEStream } from "@cloudflare/sandbox/sse-parser";
...
for await (const event of parseSSEStream<FileWatchSSEEvent>(stream)) {
  if (event.type === "event" && event.eventType === "modify" && !buildInProgress) {
...
async function watchFiles(
  ...
  handler: (eventType: string, filePath: string, isDirectory: boolean) => Promise<void>,
) {
...
The stream ends naturally when the container sleeps or shuts down. To stop a watch early, cancel the stream reader or use an `AbortController` on the underlying fetch (if applicable). Breaking out of the `for await` loop also cancels the stream:
...
Filter with `include` and `exclude` patterns rather than filtering events in JavaScript. Server-side filtering happens at the inotify level, which reduces the number of events sent over the network:
...
for await (const event of parseSSEStream<FileWatchSSEEvent>(stream)) {
  if (event.type === "event") {
    try {
      ...
      console.error(`Failed to handle ${event.eventType} for ${event.path}:`, error);
