...
Add the `BACKUP_BUCKET` R2 binding and presigned URL credentials to your Wrangler configuration:
...
Then set your R2 API credentials as secrets:
...
Set a custom time-to-live for backups. The default TTL is 24 hours (86400 seconds). The `ttl` value must be a positive number of seconds:
...
The TTL is enforced at **restore time**, not at creation time. When you call `restoreBackup()`, the SDK reads the backup metadata from R2 and compares the creation timestamp plus TTL against the current time. If the TTL has elapsed, the restore is rejected with a `BACKUP_EXPIRED` error.
