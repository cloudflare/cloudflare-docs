...
The example manages the lifecycle of each Devin session:
...
- **Suspend:** When a session suspends, the container archives `/root`, `/workspace`, and `/opt/devin-persistent` to R2. A new container restores the checkpoint when the session resumes.
...
- [Devin Outpost example](https://github.com/cloudflare/sandbox-sdk/tree/main/examples/devin-outpost)
