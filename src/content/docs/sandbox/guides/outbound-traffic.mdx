...
- **Per-host and per-instance rules.** Combine `outboundByHost` with `ctx.containerId` to scope credentials or permissions to a specific sandbox instance, or use [per-instance handlers](#per-instance-handlers) to pass credentials directly into a specific instance's handler.
...
Set outbound handlers on a specific sandbox instance using `setOutbound()` or `setOutboundByHost()`. Both accept an optional final argument that populates `ctx.params` inside the handler, letting you pass per-instance configuration such as credentials or feature flags without sharing them across every instance.
...
export default {
	async fetch(request: Request, env: Env) {
		const instanceA = getSandbox(env.Sandbox, "instance-a");
		const instanceB = getSandbox(env.Sandbox, "instance-b");
...
Use this pattern to pass per-instance proxy credentials, tenant identifiers, or any other information scoped to a single sandbox instance.
