...
You can filter outbound traffic with the `allowHosts` and `denyHosts` properties on the Sandbox class.

By default, a Sandbox allows internet access, and you can set `denyHosts` to disallow specific hosts or IPs:
...
The `outboundByHost` configuration takes precedence over the `allowHosts` and `denyHosts` lists as well as catch-all `outbound` handlers.
