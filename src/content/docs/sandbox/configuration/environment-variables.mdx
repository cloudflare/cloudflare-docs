...
On the **1.0 preview** (`@next`), `setEnvVars` and per-launch `env` remain. There is no default session that keeps shell `export` across `exec` calls. Remove `SANDBOX_TRANSPORT`. Refer to [Migrate](/sandbox/1-0-preview/migrate/) for the changed execution model.
:::
...
For a complete guide including valid transport modes, performance considerations, and migration instructions, refer to [Transport modes](/sandbox/configuration/transport/).
...
Set `SANDBOX_TRANSPORT` in your Worker's `vars` to switch between HTTP, WebSocket, and RPC transport. For details on when and how to configure each transport, refer to [Transport modes](/sandbox/configuration/transport/).
...
- [Transport modes](/sandbox/configuration/transport/) - Configure HTTP, WebSocket, and RPC transport
...
- [Handle outbound traffic](/sandbox/guides/outbound-traffic/) - Keep credentials out of the sandbox entirely using outbound handlers
