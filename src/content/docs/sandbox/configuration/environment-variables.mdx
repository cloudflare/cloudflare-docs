...
See [Transport modes](/sandbox/configuration/transport/) for a complete guide including valid transport modes, performance considerations, and migration instructions.
...
Set `SANDBOX_TRANSPORT` in your Worker's `vars` to switch between HTTP, WebSocket, and RPC transport. See [Transport modes](/sandbox/configuration/transport/) for details on when and how to configure each transport.
