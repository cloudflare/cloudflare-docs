...
To register an Any repo pool with the Cursor Agent CLI, set `CURSOR_API_KEY` in your shell. Then, run the controller before you submit any agent requests:

```sh
CURSOR_API_KEY="$CURSOR_API_KEY" agent worker controller \
  --pool cloudflare-pool \
  --spawn /usr/bin/true
```

To restrict the pool to one repository, add `--repository` with its URL:
...
The controller registers a non-default pool when it starts. After the pool appears in Cursor, stop the controller with `Ctrl+C` and run `unset CURSOR_API_KEY`. The Cloudflare deployment starts workers for the pool on demand.
...
| The image build returns `curl: (60)`, or the container reports that the Cursor CLI is missing | Cloudflare WARP or another TLS-inspecting proxy prevents Docker from verifying the certificate chain for `cursor.com` | Disconnect WARP while you run `npx wrangler deploy`, or configure Docker to trust your organization's root certificate. Reconnect WARP after the deployment finishes. |
