...
For projects that require browser testing (like E2E tests with Playwright or Puppeteer), you can extend your pipeline to support headless browser automation:

```typescript
// Enhanced test runner with browser support
async function runTests(sandbox: Sandbox): Promise<TestResult> {
  // Install browser dependencies for E2E testing
  const installBrowsers = await sandbox.exec('npx playwright install chromium');
  
  if (installBrowsers.exitCode !== 0) {
    return {
      success: false,
      exitCode: installBrowsers.exitCode,
      message: 'Failed to install browser dependencies'
    };
  }
  
  // Run tests with headless browser
  const testResult = await sandbox.exec('CI=true PLAYWRIGHT_BROWSERS_PATH=/tmp/ms-playwright npm test -- --headless');
  
  return {
    success: testResult.exitCode === 0,
    exitCode: testResult.exitCode,
    projectType: 'browser-testing',
    message: testResult.exitCode === 0 ? 'All browser tests passed' : testResult.stderr
  };
}
```

Browser testing capabilities include:
- **Playwright/Puppeteer support** - Run headless Chrome, Firefox, Safari
- **Visual regression testing** - Compare screenshots across test runs  
- **Interactive testing** - Use terminal integration for debugging test failures
- **Parallel execution** - Run multiple browser instances using sessions
...
- [Terminal Integration](/sandbox/guides/terminal-integration/) - Debug test failures interactively
