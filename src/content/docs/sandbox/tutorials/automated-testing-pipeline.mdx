...
export default {
	async fetch(
		...
		const proxyResponse = await proxyToSandbox(request, ctx);
		if (proxyResponse) return proxyResponse;

		if (request.method !== "POST") {
			return new Response(
				'POST { "repoUrl": "https://github.com/owner/repo", "branch": "main" }',
			);
		}
		...
		try {
			...
			const sandbox = getSandbox(ctx.exports.Sandbox, `test-${Date.now()}`);
			...
			try {
				...
				if (installCmd) {
					...
					const installStream = await sandbox.execStream(
				...
				const testStream = await sandbox.execStream(
