...
- `s3fs` caches file metadata for 900 seconds by default, and it remembers for the same time that a file does not exist. A file that another client adds to the bucket or changes can take that long to appear in the container. To shorten the wait, set a lower `stat_cache_expire` in `s3fsOptions`, or set `disable_noobj_cache: true`.
