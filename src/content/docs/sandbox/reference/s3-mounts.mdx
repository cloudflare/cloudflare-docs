...
- `s3fsOptions` (`object`, optional) — extra `s3fs` options. A `true` value adds the option as a flag, `false` omits it, and a string or number adds `name=value`. Options that set the endpoint, credentials, access mode, proxy, or filesystem identity throw `TypeError`.
...
- `s3fs` caches file metadata. A change made directly in the bucket can take time to appear in the container.
