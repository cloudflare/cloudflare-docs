...
Each new mount adds an outbound intercept to the container. Reusing a mount does not. A container has a limited number of outbound intercepts, and `unmount()` does not remove one. Repeated mounts with new settings in one container eventually fail.
