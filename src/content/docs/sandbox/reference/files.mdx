...
- `cwd` is also the working directory of the helper process. The directory must exist, even when the path is absolute. A `cwd` that cannot be entered rejects with `SandboxFileError`, such as `ENOENT` when it does not exist.
