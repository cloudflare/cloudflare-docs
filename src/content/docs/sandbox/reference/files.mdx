...
`rename()` replaces an existing destination in one step, so a file written to a temporary path and then renamed is never visible partially written:
...
A recursive listing is a walk over `readDirectory()`. This walk does not follow symbolic links to directories:
