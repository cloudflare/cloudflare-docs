...
- The Worker has the [`nodejs_compat`](/workers/runtime-apis/nodejs/) compatibility flag. `Files` reads Linux error names through `node:os`.
...
| `signal` | `AbortSignal` | Cancels the operation. Passed to `exec()`. An aborted signal rejects the call with `AbortError`. Cancellation can leave the partial effects described for each method. |
