...
In this tutorial, you will build a Worker that runs a coding agent on a GitHub repository in a Linux sandbox. You send a prompt, the agent edits the repository inside the sandbox, and you read its changes as a diff. The runner uses [Claude Code](https://code.claude.com/docs/en/overview). The pages in [Coding agents](/sandbox/coding-agents/) switch it to other agents.
...
3. Install the [`@cloudflare/sandbox`](/sandbox/reference/) package and [Zod](https://zod.dev/):
   ...
   <PackageManagers pkg="@cloudflare/sandbox@rc zod" />
...
The tutorial adds or replaces these files. You create `src/outbound.ts` and `src/sandbox.ts`, and replace `src/index.ts`, in the next three sections:
...
Create `src/outbound.ts`. The sandbox sends every HTTP request on port `80` and HTTPS request on port `443` through this entrypoint in your Worker. Without Internet access, it cannot reach other ports:
...
Both are HTTPS only. The Worker fetches with the scheme the container used, so a plain HTTP request to the gateway would send the gateway token unencrypted. Every other request gets a `403` response. Claude Code therefore cannot install packages from npm or fetch web pages while it works. To allow another destination, add it to this entrypoint. Each destination you add is another way for data to leave the sandbox. For more information, refer to [Sandbox security](/sandbox/concepts/security/#every-opening-is-also-a-way-out).
...
export class AgentSandbox extends DurableObject<Env> {
	...
	private async taskStatus(): Promise<TaskStatus> {
		...
		// The task can finish after the first read. Its wrapper writes the
		// exit code before it exits, so read it again.
...
A running process does not keep the container running, so `startTask()` schedules an [alarm](/durable-objects/api/alarms/) that checks the task every minute until it ends. Each check keeps the container running. For more information, refer to [Sandbox lifetime](/sandbox/concepts/lifetime/).
...
For more information, refer to [Sandbox security](/sandbox/concepts/security/#the-sandbox-name-decides-what-a-request-reaches).
:::
...
You built a Worker that runs a coding agent on a repository in a sandbox. The sandbox holds the repository and the agent, and your Worker holds the gateway token and decides where the sandbox can connect.

## Next steps

- Run another agent in the same Worker. Refer to [Codex](/sandbox/coding-agents/codex/), [OpenCode](/sandbox/coding-agents/opencode/), or [Pi](/sandbox/coding-agents/pi/).
- Deploy a finished runner for each agent. Refer to the [coding agents example](https://github.com/cloudflare/sandbox-sdk/tree/main/examples/coding-agents), which deploys one Worker for each agent instead of switching one Worker.
- Keep a sandbox's files after its container stops. Refer to [Save and restore a sandbox with snapshots](/sandbox/files/save-and-restore-a-workspace/).
- Clone private repositories with a token that stays in your Worker. Refer to [Clone a private repository](/sandbox/network/clone-a-private-repository/).
- Decide what one sandbox should hold. Refer to [Sandbox security](/sandbox/concepts/security/).
