...
4. Add methods that return a process's output so far, and follow it while the process runs:
   ...
   `follow()` sends the output so far, then new output until the process exits, when `tail --pid` stops. Each `data` event holds a JSON-encoded chunk of output, which can be part of a line or several lines, so a page can read the stream with `EventSource`, as in [Stream command output](/sandbox/commands/stream-command-output/#read-the-stream-in-a-browser). A disconnected client is noticed only when a write fails, so the comment every five seconds stops `tail` even when the log is quiet. Without it, `tail` keeps running until the process exits.
...
- [Preview a web application](/sandbox/previews/): a web server as the main process.
