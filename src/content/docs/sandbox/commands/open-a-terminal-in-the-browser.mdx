...
3. Add a `fetch()` handler to your Durable Object that attaches a WebSocket to a tmux session:
   ...
   `enableInternet: false` keeps commands in the terminal off the Internet. The shell runs as `root`. To run it as another Linux user, pass numeric IDs in the [`user` option of `exec()`](/durable-objects/api/container/#exec), such as `user: "1000:1000"`.
