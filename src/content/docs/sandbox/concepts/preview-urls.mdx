...
We are working on production deployments, custom hostnames, and authentication for Cloudflare Tunnel support. In the meantime, we recommend using `exposePort()` and `proxyToSandbox()`. Refer to [Production usage, stable URLs and custom domains](#production-usage-stable-urls--custom-domains) for details.
:::

Refer to the [tunnels API reference](/sandbox/api/tunnels/) for the full API and feature set.
