...
```typescript
const sandbox = getSandbox(ctx.exports.Sandbox, "user-123");
...
```typescript
const sandbox = getSandbox(ctx.exports.Sandbox, `user-${userId}`);
```
...
const sandbox = getSandbox(ctx.exports.Sandbox, sessionId);
// Later:
await sandbox.destroy();
```
...
```typescript
const sandbox = getSandbox(ctx.exports.Sandbox, `build-${repoName}-${commit}`);
```
