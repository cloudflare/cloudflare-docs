...
import { SandboxLifetime } from "~/components/react/sandbox/SandboxLifetime";
...
<SandboxLifetime client:visible>

The coding agent's sandbox, from top to bottom: the name, the Durable Object, whose storage keeps a snapshot ID, the Linux instance, which runs the development server and holds the repository and edits on its disk, and a mounted R2 bucket. When the instance stops, its processes and disk files end. The name, the Durable Object, and the objects in the bucket remain. A new instance started from the snapshot has the repository and edits on its disk, and no development server runs until the agent starts it again.
