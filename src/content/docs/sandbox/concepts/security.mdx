...
const userSandbox = getSandbox(ctx.exports.Sandbox, `user-${userId}`);

// Bad - Users sharing one sandbox
const shared = getSandbox(ctx.exports.Sandbox, "shared");
...
export default {
	async fetch(
		...
		const sandbox = getSandbox(ctx.exports.Sandbox, userId);
