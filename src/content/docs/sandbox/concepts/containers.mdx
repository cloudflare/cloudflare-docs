...
Examples that use `startProcess` apply to the stable package. On **`@next`**, long-running work uses `exec(argv)` process handles — [Process execution](/sandbox/1-0-preview/processes/).
:::
...
- [Deploy a Sandbox application](/sandbox/guides/deploy/) - Worker package and container image deploys
- [Containers deployments](/containers/deployments/) - What changes on `wrangler deploy`
