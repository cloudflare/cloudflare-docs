...
**Sandbox SDK 1.0** (preview on `@next`) removes core session execution APIs. Use per-launch `cwd` and `env`, an explicit shell command, or a [terminal](/sandbox/1-0-preview/processes/#processes-versus-terminals). Refer to [Process execution](/sandbox/1-0-preview/processes/) or [migrate session workflows](/sandbox/1-0-preview/migrate/#drop-session-apis).
:::
