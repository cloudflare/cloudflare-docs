...
**Sandbox SDK 1.0** (preview on `@next`) does not use sessions as the primary execution model. Each `exec()` is a fresh process; interactive shell state lives on terminals. Refer to [Process execution](/sandbox/1-0-preview/processes/).
:::
