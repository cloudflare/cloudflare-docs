...
**Sandbox SDK 1.0** (preview on `@next`) removes core session execution APIs. Pass `cwd` and `env` on each `exec`, or use one explicit shell argv script. Refer to [Process execution](/sandbox/1-0-preview/processes/) or [migrate](/sandbox/1-0-preview/migrate/#drop-session-apis).
:::
