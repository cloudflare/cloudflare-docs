...
1. **Client SDK** validates parameters and sends requests over a persistent connection
2. **Durable Object** maintains the connection and multiplexes concurrent requests
...
The persistent connection is established on first SDK call and reused for all subsequent operations, reducing overhead for high-frequency operations.
