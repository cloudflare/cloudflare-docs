...
Multiplexes all SDK calls over a single persistent WebSocket connection. Avoids sub-request limits when making many concurrent operations.
...
- High-frequency SDK calls from a single Worker or Durable Object
- Complex workflows with numerous file operations and command executions
- Applications approaching Workers' sub-request limits

The transport layer is transparent to your application code - all SDK methods work identically regardless of transport. See [Environment variables configuration](/sandbox/configuration/environment-variables/) for details.
