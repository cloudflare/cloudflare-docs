...
`sandbox.tunnels` requires RPC transport — calling it on HTTP/Websocket transports will throw `"RPC transport required"`. Refer to [Transport configuration](/sandbox/configuration/transport/). It also requires a glibc image variant: the default, `python`, `opencode`, and `desktop` images ship `cloudflared`, but the `musl`/Alpine variant does not — there is no upstream `cloudflared` build for musl at this time.
...
**Returns**: `Promise<TunnelInfo>` — the tunnel record. Refer to [`TunnelInfo`](#tunnelinfo).
...
- **No persistent hostname.** Every restart picks a new `<random-words>.trycloudflare.com` hostname. If you need a stable URL, use [`exposePort()`](/sandbox/api/ports/#exposeport) with a custom token.
