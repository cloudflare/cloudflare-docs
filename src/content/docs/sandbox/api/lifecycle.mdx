...
In the **1.0 preview** (`@next`), remove transport options on `getSandbox()` and do not rely on `enableDefaultSession`. Process and terminal handles are runtime-local after sleep or replacement. Refer to [Migrate to the 1.0 preview](/sandbox/1-0-preview/migrate/).
:::
