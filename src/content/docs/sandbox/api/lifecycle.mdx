...
- `namespace` - The Durable Object namespace, typically accessed via `ctx.exports.Sandbox` using [automatic loopback bindings](/changelog/2025-09-26-ctx-exports/)
...
<TypeScriptExample>
```
async function executeCode(code: string, ctx: ExecutionContext): Promise<string> {
  const sandbox = getSandbox(ctx.exports.Sandbox, `temp-${Date.now()}`);
