...
Wait for a process to listen on a port. Uses Server-Sent Events (SSE) streaming internally for efficient port readiness detection without repeated HTTP requests.
...
- `options` (optional):
  ...
  - `interval` - Server-side polling interval in milliseconds (default: `500`)
