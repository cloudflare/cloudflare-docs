...
Create a WebSocket connection to an interactive terminal session (PTY).

```ts
const response = await session.terminal(request: Request): Promise<Response>
```

**Parameters**:
- `request` - WebSocket upgrade request from client

**Returns**: `Promise<Response>` with WebSocket upgrade response
...
The terminal connection provides:
- **Interactive shell** - Real bash session with full TTY support
- **Bidirectional streaming** - User input and command output via WebSocket
- **Session isolation** - Each session has independent shell state
- **Automatic cleanup** - Terminal processes cleaned up when session ends

:::note
Use the [XTerm addon](/sandbox/api/xterm/) to connect browser terminals to the WebSocket endpoint.
:::
...
| **Terminal** | [`terminal()`](/sandbox/api/sessions/#terminal) |
...
| **Terminals** | [`terminal()`](/sandbox/api/terminals/#worker-integration) |

## Related resources

- [Session management concept](/sandbox/concepts/sessions/) - How sessions work
- [Commands API](/sandbox/api/commands/) - Execute commands
- [Terminals API](/sandbox/api/terminals/) - Browser-based terminal integration
