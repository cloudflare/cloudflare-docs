...
This page documents the HTTP API of the bridge template on sandbox-sdk **`main`**. For the preview template on **`next`** (processes, terminals, no sessions), refer to [Bridge HTTP API (`@next`)](/sandbox/1-0-preview/bridge/http-api/).
:::

This page documents every route exposed by the [sandbox bridge](/sandbox/bridge/) on the stable template.
