...
| Plan | WebSocket idle timeout |
|------|----------------------|
| Free, Pro, Business | 100 seconds |
| Enterprise | Configurable |

A WebSocket connection is considered idle when no data is transmitted in either direction. When the idle timeout is reached, Cloudflare closes the connection.

**Recommendation:** Implement a client-side heartbeat (ping/pong) mechanism to keep connections alive during periods of inactivity.

### Session Affinity for load-balanced WebSocket origins

If your WebSocket origin is behind a Cloudflare Load Balancer, enable **Session Affinity** to ensure all requests from the same client are routed to the same origin server. Without session affinity, a WebSocket reconnect may land on a different origin that does not have the session state.
