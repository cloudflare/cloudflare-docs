...
Gateway supports using [Cloudflare Organizations](/fundamentals/organizations/) to share configurations between and apply specific policies to accounts within an Organization. Tiered Gateway policies with Organizations support [DNS](/cloudflare-one/traffic-controls/dns-policies/), [network](/cloudflare-one/traffic-controls/network-policies/), [HTTP](/cloudflare-one/traffic-controls/http-policies/), and [resolver](/cloudflare-one/traffic-controls/resolver-policies/) policies.

For a DNS-only deployment using the Tenant API, refer to [Tenant API](/cloudflare-one/traffic-controls/tiered-policies/tenant-api/).

## Get started

To set up Cloudflare Organizations, refer to [Create an Organization](/fundamentals/organizations/#create-an-organization). Once you have provisioned and configured your Organization's accounts, you can create [Gateway policies](/cloudflare-one/traffic-controls/).
...
- [Egress policies](/cloudflare-one/traffic-controls/egress-policies/) cannot be shared between accounts.
- Source accounts cannot share policies that use [device posture](/cloudflare-one/reusable-components/posture-checks/) selectors, the [Detected protocol](/cloudflare-one/traffic-controls/network-policies/#detected-protocol) selector, or the [Quarantine](/cloudflare-one/traffic-controls/http-policies/#quarantine) action. Source and recipient accounts can still create and apply policies with these selectors and actions separately from the Organization share.
...
You can share certain Gateway settings - the Gateway block page and extended email address matching - from your source account to recipient accounts in your Cloudflare Organization. Other Gateway settings configured in a source account, such as [AV scanning](/cloudflare-one/traffic-controls/http-policies/antivirus-scanning/) and [file sandboxing](/cloudflare-one/traffic-controls/http-policies/file-sandboxing/), will not affect recipient account configurations.
...
To share your [extended email address matching](/cloudflare-one/traffic-controls/identity-selectors/#extended-email-addresses) settings from a source account to a recipient account:
