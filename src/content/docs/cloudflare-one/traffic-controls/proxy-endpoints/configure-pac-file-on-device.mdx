...
After you [create a proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/) and [create a PAC file](/cloudflare-one/traffic-controls/proxy-endpoints/#2-create-a-pac-file), configure your devices to use the PAC file URL. You can configure system-level proxy settings (which apply to most browsers) or configure individual browsers separately.
...
- A [Cloudflare Gateway proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/#1-create-a-proxy-endpoint)
- A PAC file URL (either [hosted by Cloudflare](/cloudflare-one/traffic-controls/proxy-endpoints/#create-a-hosted-pac-file) or [self-hosted](/cloudflare-one/traffic-controls/proxy-endpoints/#self-hosting-pac-files))
...
:::note
Safari does not support the HTTPS proxy type required by Cloudflare proxy endpoints. Use a Chromium-based browser (Chrome, Edge, Brave) or [Firefox](/cloudflare-one/traffic-controls/proxy-endpoints/configure-pac-file-on-device/#configure-firefox-separately) on macOS instead.
:::
...
2. Create an [HTTP policy](/cloudflare-one/traffic-controls/http-policies/) to block a test domain (for example, `example.com`).
3. Visit the blocked domain in your browser.
4. Verify that the Gateway block page appears.

If the block page does not appear, refer to the [PAC file troubleshooting section](/cloudflare-one/traffic-controls/proxy-endpoints/best-practices/#troubleshoot-configurations) for debugging steps.

## Next steps

- [Create HTTP policies](/cloudflare-one/traffic-controls/http-policies/) to filter proxy endpoint traffic.
- Review [PAC file best practices](/cloudflare-one/traffic-controls/proxy-endpoints/best-practices/) for formatting, performance optimization, and bypass rules.
- Use the [Proxy Endpoint selector](/cloudflare-one/traffic-controls/http-policies/#proxy-endpoint) in HTTP and network policies to apply rules to proxy traffic.
