...
Dedicated egress IPs are now turned on for all network and HTTP traffic proxied by Gateway. To selectively turn on dedicated egress IPs for a subset of your traffic, refer to [egress policies](/cloudflare-one/traffic-controls/egress-policies/).
...
After you onboard your IP addresses, they appear as options when you create an [egress policy](/cloudflare-one/traffic-controls/egress-policies/) and choose **Use dedicated egress IPs (Cloudflare or BYOIP)** as the [egress method](/cloudflare-one/traffic-controls/egress-policies/#egress-methods). BYOIP dedicated egress IPs do not support [IP geolocation](#ip-geolocation).
...
By default, DNS queries that Gateway sends to custom resolvers use shared Cloudflare source IP addresses. You can [configure a resolver policy to send these queries from dedicated egress IPs](/cloudflare-one/traffic-controls/resolver-policies/#send-dns-queries-sourced-from-dedicated-egress-ips).
...
### Can I use dedicated egress IPs with traffic proxied via [PAC files](/cloudflare-one/traffic-controls/proxy-endpoints/)?
