...
The following policies are commonly used to secure HTTP traffic. HTTP policies are evaluated in order from top to bottom, and the first matching policy applies — except for [Do Not Inspect](/cloudflare-one/traffic-controls/http-policies/#do-not-inspect) policies, which are always evaluated first.
...
Refer to the [HTTP policies page](/cloudflare-one/traffic-controls/http-policies/) for a comprehensive list of other selectors, operators, and actions.
...
Gateway [evaluates Do Not Inspect policies first](/cloudflare-one/traffic-controls/order-of-enforcement/#http-policies), regardless of their position in the policy list. Cloudflare recommends moving your Do Not Inspect policies to the top of the list to reduce confusion.
...
For more information on supported file types, refer to [Download and Upload File Types](/cloudflare-one/traffic-controls/http-policies/#download-and-upload-file-types).
