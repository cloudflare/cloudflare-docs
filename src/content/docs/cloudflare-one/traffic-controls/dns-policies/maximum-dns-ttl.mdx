...
Each [DNS location](/cloudflare-one/traffic-controls/resolver-endpoints/) can override the account-level setting. The per-location setting supports three modes:
...
1. In [Zero Trust](https://dash.cloudflare.com/one), go to **Traffic controls** > **Resolver endpoints**.
