...
When a user makes a DNS request, [Gateway](/cloudflare-one/traffic-controls/) matches the request against the DNS policies you have set up for your organization. If the domain does not belong to any blocked categories, or if it matches an Allow or Override policy, the user's client receives an address based on DNS resolution from Cloudflare's public DNS resolver (1.1.1.1). You can also use a [resolver policy](/cloudflare-one/traffic-controls/resolver-policies/) to redirect DNS requests to a custom server.
