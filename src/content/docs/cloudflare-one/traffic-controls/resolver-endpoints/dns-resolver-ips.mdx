...
1. In [Cloudflare One](https://one.dash.cloudflare.com/), go to **Traffic controls** > **Resolver endpoints**.
...
2. Add a [Source IP](/cloudflare-one/traffic-controls/dns-policies/#source-ip) condition to your DNS policies.
...
DNS queries made from IP addresses that are not in your IP list will not be filtered or populate your organization's [Gateway activity logs](/cloudflare-one/insights/logs/dashboard-logs/gateway-logs/).
