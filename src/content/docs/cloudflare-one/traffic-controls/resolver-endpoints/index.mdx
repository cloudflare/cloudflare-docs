...
description: Configure resolver endpoints in Cloudflare One.
products:
  - cloudflare-one
...
You can now apply [DNS policies](/cloudflare-one/traffic-controls/dns-policies/) to your location using the [Location selector](/cloudflare-one/traffic-controls/dns-policies/#location).

## DNS endpoints

### IPv4 and IPv6 DNS

Cloudflare will prefill the [**Source IPv4 Address**](/cloudflare-one/traffic-controls/resolver-endpoints/dns-resolver-ips/#source-ip) based on the network you are on. Additionally, Enterprise users can use [dedicated DNS resolver IP addresses](/cloudflare-one/traffic-controls/resolver-endpoints/dns-resolver-ips/#dns-resolver-ip) assigned to their account or [resolver IP addresses they provide (BYOIP)](/cloudflare-one/traffic-controls/resolver-endpoints/dns-resolver-ips/#bring-your-own-dns-resolver-ip).
...
For more information, refer to [DNS over TLS](/cloudflare-one/traffic-controls/resolver-endpoints/dns-over-tls/).

### DNS over HTTPS (DoH)

<GlossaryDefinition term="DNS over HTTPS" />

Gateway requires a DoH endpoint for default DNS locations. For more information, refer to [DNS over HTTPS](/cloudflare-one/traffic-controls/resolver-endpoints/dns-over-https/).
...
- Your IPv4 and IPv6 endpoints use your [BYOIP addresses](/cloudflare-one/traffic-controls/resolver-endpoints/dns-resolver-ips/#bring-your-own-dns-resolver-ip) (if any).
- [Source network filtering](/cloudflare-one/traffic-controls/network-policies/) is configured for your IPv4, IPv6, and DoT endpoints.
