...
These rules are also available as [managed rules](/cloudflare-one/traffic-controls/packet-filtering/enable-managed-rulesets/) that you can enable without manual configuration. The rules above are provided for reference and customization.
...
- SMCP
- NTP
  - Common vector for reflection attacks. Consider using [Cloudflare One traffic policies](/cloudflare-one/traffic-controls/), [1.1.1.1's DNS over HTTPS (DoH)](/1.1.1.1/), or an internal DNS service if possible. Consider restricting your firewall rules to only allow the source and destination of DNS traffic.
- MS-SQL
