...
**[Packet filtering](/cloudflare-one/traffic-controls/packet-filtering/network-firewall-overview/)** inspects raw network packets and blocks traffic based on properties like source IP address or protocol. It does not need to know who the user is or what session they belong to.
...
**[DNS policies](/cloudflare-one/traffic-controls/dns-policies/)** check every DNS query your users make. When a query matches a policy rule, Gateway can block the domain from resolving — the site never loads because the domain name is never translated to an IP address.
...
**[Network policies](/cloudflare-one/traffic-controls/network-policies/)** inspect individual TCP, UDP, and Generic Routing Encapsulation (GRE) packets. They can match on IP addresses, ports, protocols, and the server name sent at the start of an encrypted connection (Server Name Indication, or SNI).
...
**[HTTP policies](/cloudflare-one/traffic-controls/http-policies/)** inspect the full content of web requests — including URLs, headers, and uploaded or downloaded files. Gateway decrypts HTTPS traffic so it can examine what DNS and network policies cannot see. This requires installing a [Cloudflare root certificate](/cloudflare-one/team-and-resources/devices/user-side-certificates/) on user devices.

Use HTTP policies to block specific URLs, scan file uploads for sensitive data, block malware in downloads, [quarantine suspicious files](/cloudflare-one/traffic-controls/http-policies/file-sandboxing/) for sandbox analysis, and control which accounts users can sign in to. For example, allow your company Google Workspace account but block personal Gmail.

</Details>

<Details header="Egress policies">

**[Egress policies](/cloudflare-one/traffic-controls/egress-policies/)** control how traffic leaves your network by assigning fixed IP addresses that belong to your organization. Third-party services can recognize these IPs as yours.
...
**[Resolver policies](/cloudflare-one/traffic-controls/resolver-policies/)** send DNS queries to specific DNS servers instead of the default Cloudflare resolver.
...
For details on building policies with identity selectors, refer to [Identity-based policies](/cloudflare-one/traffic-controls/identity-selectors/).
...
3. Set up recommended security policies — for example, block all [security threat categories](/cloudflare-one/traffic-controls/domain-categories/#security-categories) with a DNS policy.
...
For step-by-step setup guides, refer to [DNS](/cloudflare-one/traffic-controls/get-started/dns/), [Network](/cloudflare-one/traffic-controls/get-started/network/), and [HTTP](/cloudflare-one/traffic-controls/get-started/http/) policies.
...
| [DNS resolver](/cloudflare-one/traffic-controls/resolver-endpoints/) configuration | Yes | No | No | Unmanaged devices, entire networks, or initial rollout |
| [Proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/) (PAC file) | No | No | Yes (browser only) | Browser-level HTTP filtering without a device agent |
...
4. Gateway evaluates the request against your configured policies in [order of enforcement](/cloudflare-one/traffic-controls/order-of-enforcement/): DNS policies first, then network policies, then HTTP policies.
...
For details on how Gateway proxies traffic and establishes connections, refer to [Proxy](/cloudflare-one/traffic-controls/proxy/).

## Troubleshoot Cloudflare Gateway policies

For help resolving common issues with Cloudflare Gateway policies, refer to [Troubleshooting](/cloudflare-one/traffic-controls/troubleshooting/).
