...
- **DNS location** — If you are using a DNS-only deployment (without the Cloudflare One Client), verify that your network's DNS resolver points to your [Gateway DNS location's](/cloudflare-one/networks/resolvers-and-proxies/dns/locations/) IP address.
...
If you are blocking a [security category](/cloudflare-one/traffic-policies/policy-types/dns-policies/selectors/#security-categories) or a [content category](/cloudflare-one/traffic-policies/policy-types/dns-policies/selectors/#content-categories), you can test that the policy is working by using the [test domain](#common-test-domains) associated with each category.
