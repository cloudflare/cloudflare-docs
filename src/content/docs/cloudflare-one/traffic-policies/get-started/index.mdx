...
- Deploy the [Cloudflare One Client](/cloudflare-one/connections/connect-devices/warp/) and enable the [Gateway proxy](/cloudflare-one/traffic-policies/proxy/) for TCP.
...
- Block risky file types, enable [anti-virus scanning](/cloudflare-one/traffic-policies/http-policies/antivirus-scanning/), and configure [DLP profiles](/cloudflare-one/policies/data-loss-prevention/) to detect sensitive data.
...
- Connect branch offices and data centers with [network tunnels](/cloudflare-one/connections/connect-networks/) (IPsec/GRE via Magic WAN).
...
- Monitor SaaS application usage with [CASB](/cloudflare-one/applications/casb/).
