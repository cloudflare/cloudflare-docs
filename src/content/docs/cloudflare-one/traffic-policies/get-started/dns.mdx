...
1. [Install the WARP client](/cloudflare-one/team-and-resources/devices/warp/download-warp/) on your device. WARP is a lightweight agent that routes the device's DNS queries through Cloudflare so Gateway can inspect and filter them.
2. [Enroll the WARP client](/cloudflare-one/team-and-resources/devices/warp/deployment/manual-deployment/) in your organization's <GlossaryTooltip term="team name">Zero Trust instance</GlossaryTooltip> [^1]. This tells WARP which Gateway policies to enforce.
