...
- Turn on [TLS decryption](/cloudflare-one/traffic-policies/policy-types/http-policies/tls-decryption/).
- Turn on the [Gateway proxy](/cloudflare-one/traffic-policies/proxy/#turn-on-the-gateway-proxy).
- (Optional) If an application uses HTTP/3, turn on the [Gateway proxy for UDP traffic](/cloudflare-one/traffic-policies/policy-types/http-policies/http3/#enable-http3-inspection).
