...
In addition to [anti-virus (AV) scanning](/cloudflare-one/traffic-policies/policy-types/http-policies/antivirus-scanning/), Gateway can quarantine previously unseen files downloaded by your users into a sandbox and scan them for malware.
...
Gateway will log any file sandbox decisions in your [HTTP logs](/cloudflare-one/insights/logs/dashboard-logs/gateway-logs/#http-logs).
...
You can now create [Quarantine HTTP policies](/cloudflare-one/traffic-policies/policy-types/http-policies/actions/#quarantine) to determine what files to scan in the sandbox.
