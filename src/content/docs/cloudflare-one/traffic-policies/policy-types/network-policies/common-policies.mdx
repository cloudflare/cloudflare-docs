...
Refer to the [network policies page](/cloudflare-one/traffic-policies/policy-types/network-policies/) for a comprehensive list of other selectors, operators, and actions.
...
If your organization blocks traffic by default with a Network policy and you want to [inspect HTTP traffic on all ports](/cloudflare-one/traffic-policies/policy-types/network-policies/protocol-detection/#inspect-on-all-ports), you need to explicitly allow HTTP and TLS traffic to filter it.
