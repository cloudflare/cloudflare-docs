...
You can build [Gateway network policies](/cloudflare-one/traffic-policies/policy-types/network-policies/) to filter HTTPS traffic to your public hostname on port `443`. For example, to restrict `app.bank.com` so that only certain users or groups can access it through your AWS egress IP, create two policies: one to allow authorized users, and one to block everyone else.
...
Gateway does not support hostname-based filtering for traffic on non-`443` ports. To block traffic to `app.bank.com` on all ports, use the [Destination IP](/cloudflare-one/traffic-policies/policy-types/network-policies/selectors/#destination-ip) selector and specify the public IP range of `app.bank.com`.
