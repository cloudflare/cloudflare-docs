...
:::caution
The Tenant API platform only supports [DNS policies](/cloudflare-one/traffic-policies/dns-policies/). To apply HTTP, network, egress, and resolver policies, use [Cloudflare Organizations](/cloudflare-one/traffic-policies/tiered-policies/organizations/) instead.
:::
...
For a comparison between Organizations and the Tenant API, refer to [Tiered policies](/cloudflare-one/traffic-policies/tiered-policies/).
