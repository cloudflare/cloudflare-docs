...
:::caution
The Tenant API platform only supports [DNS policies](/cloudflare-one/traffic-policies/dns-policies/). To apply HTTP, network, egress, and resolver policies, use [Tiered policies with Cloudflare Organizations](/cloudflare-one/traffic-policies/tiered-policies/) instead.
:::
