...
Tiered policies do not support [egress policies](/cloudflare-one/traffic-policies/egress-policies/). Source accounts cannot share policies with selectors that target [device posture checks](/cloudflare-one/reusable-components/posture-checks/), [Access private apps](/cloudflare-one/access-controls/applications/non-http/self-hosted-private-app/), or [virtual networks](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/). Source and recipient accounts can still create and apply policies with these selectors separately from the Organization share.
