...
Managed service providers (MSPs) that are Cloudflare Partners can use tiered or siloed Gateway accounts with the Tenant API. For more information, refer to [Managed service providers (MSPs)](/cloudflare-one/traffic-policies/tiered-policies/managed-service-providers/).
...
To share your [extended email address matching](/cloudflare-one/identity/users/#extended-email-addresses) settings from a source account to a recipient account:
