...
Gateway allows you to create DNS, Network, and HTTP policies based on applications and app types. Instead of writing separate rules for every domain an application uses, you can select the application by name. You can select individual applications or groups of app types to filter specific traffic on your network.
...
An application like Google Drive uses its own domains (for example, `drive.google.com`) and shared resources (for example, `accounts.google.com` for login). Gateway separates these into [hostnames](#hostnames) and [support hostnames](#support-hostnames) so that blocking one application does not accidentally break another.
