...
- SMCP
- NTP
  - Common vector for reflection attacks. Consider using [Cloudflare Gateway](/cloudflare-one/policies/gateway/), [1.1.1.1's DNS over HTTPS (DoH)](/1.1.1.1/), or an internal DNS service if possible. Consider restricting your firewall rules to only allow the source and destination of DNS traffic.
- MS-SQL
