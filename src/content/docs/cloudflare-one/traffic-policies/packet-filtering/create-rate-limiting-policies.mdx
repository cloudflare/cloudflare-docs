...
Rate limiting policies (beta) let you set a maximum traffic rate (measured in packets per second or bits per second) for traffic arriving at specific Cloudflare data center locations. When traffic exceeds the threshold you define, the policy takes action.
