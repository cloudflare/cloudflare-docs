...
By default, Cloudflare Network Firewall policies allow all traffic until explicitly blocked by a rule. If no policy is configured, all traffic is permitted after Cloudflare's DDoS mitigations have been applied.
...
Traffic is matched in order of the configured rules. As soon as traffic is matched by an enabled rule, it is no longer validated against the later rules, and traffic will pass through disabled rules. In the dashboard under **Traffic policies** > **Firewall policies**, rule order begins from the top and flows down your list of rules.
...
Cloudflare Network Firewall processes traffic in two phases: in the first phase, Cloudflare Network Firewall policies match packets against rules in the Custom phase. In the second phase, Cloudflare Network Firewall match packets against rules in the Managed phase.
