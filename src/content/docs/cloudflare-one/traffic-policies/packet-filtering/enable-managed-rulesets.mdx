...
With [managed rulesets](/ruleset-engine/managed-rulesets/), you can quickly deploy pre-built firewall rules maintained by Cloudflare rather than writing rules from scratch. You use Cloudflare Network Firewall to control which managed rules are enabled.
