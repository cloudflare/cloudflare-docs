...
The Gateway proxy is required for filtering HTTP and network traffic via the WARP client in Traffic and DNS mode. To proxy HTTP traffic without deploying the WARP client, you can configure [PAC files](/cloudflare-one/networks/resolvers-and-proxies/proxy-endpoints/) on your devices.
