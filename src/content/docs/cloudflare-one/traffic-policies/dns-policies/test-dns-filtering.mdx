...
Before you start, make sure your device is sending DNS queries through a [DNS location](/cloudflare-one/networks/resolvers-and-proxies/dns/locations/) where the policy is active. If your device uses WARP, DNS queries route through Gateway automatically. If you are using a DNS-only deployment (without WARP), verify that your network's DNS resolver points to your Gateway DNS location's IP address.
...
If you are blocking a [security category](/cloudflare-one/traffic-policies/dns-policies/#security-categories) or a [content category](/cloudflare-one/traffic-policies/dns-policies/#content-categories), you can test that the policy is working by using the [test domain](#common-test-domains) associated with each category.
...
EDNS client subnet is a DNS extension that sends a portion of the user's IP address (a `/24` subnet) to the upstream DNS resolver, allowing it to return geographically optimal answers. If you [enabled EDNS client subnet](/cloudflare-one/networks/resolvers-and-proxies/dns/locations/) for your DNS location, you can validate it as follows:
