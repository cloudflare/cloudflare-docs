...
- **WARP** — If your device runs the [WARP client](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/), DNS queries route through Gateway automatically.
- **DNS location** — If you are using a DNS-only deployment (without WARP), verify that your network's DNS resolver points to your [Gateway DNS location's](/cloudflare-one/networks/resolvers-and-proxies/dns/locations/) IP address.
