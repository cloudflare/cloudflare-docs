...
For a more comprehensive list of recommended policies, refer to the [Recommended DNS Policies page](/learning-paths/secure-internet-traffic/build-dns-policies/recommended-dns-policies/).
...
Enterprise users can pair these policies with an [egress policy](/cloudflare-one/traffic-policies/egress-policies/) to control which IP version is used to egress to the origin server.
