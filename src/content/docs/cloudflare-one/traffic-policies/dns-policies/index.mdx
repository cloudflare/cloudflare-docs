...
Some apps, such as WhatsApp and Android Studio, have hard-coded fallback DNS, which can cause unexpected behavior.  For example, if you have a Block DNS policy, the requests from those apps will initially be blocked by Gateway, but can resolve correctly from the fallback DNS.
