...
:::note
Package protection requires [TLS decryption](/cloudflare-one/traffic-policies/http-policies/tls-decryption/) to be turned on for the traffic you want to inspect.
:::

Gateway can detect software package downloads across seven major package ecosystems and give you policy control over supply chain traffic. When a developer or CI/CD pipeline downloads a package through Gateway, the proxy identifies the registry protocol from the request URL and extracts the package ecosystem, name, version, and namespace. You can then write HTTP policies using `pkg.*` selectors to allow, block, or log package downloads at whatever granularity you need, from blocking an entire ecosystem to restricting a single package version.
...
| `pkg.purl` | Package URL | `pkg.purl == "pkg:npm/lodash@4.17.21"` | The [Package URL (PURL)](https://github.com/package-url/purl-spec) derived from the detected coordinates. |
