...
For a DNS-only deployment using the Tenant API, refer to [Tenant API](/cloudflare-one/traffic-policies/reference/tiered-policies/tenant-api/).
...
- Source accounts cannot share policies that use [device posture](/cloudflare-one/reusable-components/posture-checks/) selectors, the [Detected protocol](/cloudflare-one/traffic-policies/policy-types/network-policies/#detected-protocol) selector, or the [Quarantine](/cloudflare-one/traffic-policies/policy-types/http-policies/#quarantine) action. Source and recipient accounts can still create and apply policies with these selectors and actions separately from the Organization share.
