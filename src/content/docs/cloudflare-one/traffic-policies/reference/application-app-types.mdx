...
When you use the [_Application_ selector](/cloudflare-one/traffic-policies/policy-types/http-policies/#granular-controls) in an HTTP policy with the _is_ operator, you can choose specific actions and operations to match application traffic. Supported applications and operations include:
...
Gateway automatically groups applications incompatible with TLS decryption into the _Do Not Inspect_ app type. As Cloudflare identifies incompatible applications, Gateway will periodically update this app type to add new applications. To ensure Gateway does not intercept any current or future incompatible traffic, you can [create a Do Not Inspect HTTP policy](/cloudflare-one/traffic-policies/policy-types/http-policies/#do-not-inspect) with the entire _Do Not Inspect_ app type selected.
...
Applications can be incompatible with [TLS decryption](/cloudflare-one/traffic-policies/tls-decryption/tls-decryption/) for various reasons:
