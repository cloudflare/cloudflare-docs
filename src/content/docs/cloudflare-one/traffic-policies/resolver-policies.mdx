...
Use [Local Domain Fallback](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/route-traffic/local-domains/) when your DNS server is only reachable from the user's local network and cannot be reached by Gateway through a Cloudflare Tunnel, IPsec/GRE tunnel, or other public Internet connection. Local Domain Fallback resolves DNS directly on the device — the query never reaches Gateway and is not logged or filtered.

If both Local Domain Fallback and resolver policies are configured for the same domain on the same device, Local Domain Fallback takes priority. If you onboard DNS queries to Gateway with the Cloudflare One Client and route them with resolver policies, the source IP of the queries will be the IP address assigned by the Cloudflare One Client.

| Criteria                        | Resolver policies                   | Local Domain Fallback                |
| ------------------------------- | ----------------------------------- | ------------------------------------ |
| DNS server reachable by Gateway | Yes (via tunnel or public Internet) | No (local network only)              |
| Queries logged by Gateway       | Yes                                 | No                                   |
| Queries filtered by Gateway     | Yes                                 | No                                   |
| Configured in                   | Zero Trust dashboard                | Cloudflare One Client device profile |
