...
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DNS resolution to carrier-grade NAT (CGNAT) | When an egress policy uses a _Domain_ or _Host_ selector, Gateway resolves the domain to an IP address before applying the policy. For Cloudflare-proxied sites, the resolved IP is often a CGNAT address in the `100.64.0.0/10` range. CGNAT addresses are internal to Cloudflare's network and never leave it, so egress policies — which apply to traffic exiting the network — do not match. To fix this, change the selector in your egress policy from _Domain_ or _Host_ to _Destination IP_ and use the public IP addresses of the service you are trying to reach. |
