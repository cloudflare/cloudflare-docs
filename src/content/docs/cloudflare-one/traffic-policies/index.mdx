...
When users connect through the [Cloudflare One Client](/cloudflare-one/connections/connect-devices/warp/), Gateway can evaluate:

- **User identity** – Email address, group membership, and authentication method from your [identity provider](/cloudflare-one/identity/idp-integration/) (for example, Okta, Microsoft Entra ID, or Google Workspace).
- **Device posture** – Signals such as operating system version, disk encryption status, firewall state, and whether the device serial number matches a managed device list. For the full list of available checks, refer to [Device posture](/cloudflare-one/identity/devices/).
...
| --- | --- | --- | --- | --- |
| [Cloudflare One Client](/cloudflare-one/connections/connect-devices/warp/) (WARP) | Yes | Yes | Yes | Roaming users on managed devices (laptops, phones) |
| [DNS resolver](/cloudflare-one/connections/connect-devices/agentless/dns/) configuration | Yes | No | No | Unmanaged devices, entire networks, or initial rollout |
| [Proxy endpoint](/cloudflare-one/connections/connect-devices/agentless/pac-files/) (PAC file) | No | No | Yes (browser only) | Browser-level HTTP filtering without a device agent |
| [Network tunnel](/cloudflare-one/connections/connect-networks/) (IPsec/GRE via Magic WAN) | Yes | Yes | Yes | Branch offices, data centers, and site-level connectivity |
