...
In the rare event your primary egress IPs location goes down, and there is no secondary back up IP configured in the egress rule, your traffic will be impacted. Cloudflare strongly recommends ensuring there is always a fallback egress IP configured on every egress policy.
...
### Can I use dedicated egress IPs with traffic proxied via [PAC files](/cloudflare-one/team-and-resources/devices/agentless/pac-files/)?
