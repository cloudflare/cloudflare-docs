...
4. **Pair with HTTP policies** to add deeper controls — for example, block file uploads to personal accounts, enforce DLP profiles to prevent sensitive data from leaving the organization, or require [device posture checks](/cloudflare-one/identity/devices/) before allowing access.
