...
5. Select **Block with Gateway HTTP policy**. A new browser tab will open with a pre-filled HTTP policy.
   ...
   :::
6. (Optional) [Configure the HTTP policy](/cloudflare-one/traffic-policies/policy-types/http-policies/). For example, if the policy blocks an unsanctioned third-party app, you can apply the policy to some or all users, or only block uploads or downloads.
7. Select **Save**.
