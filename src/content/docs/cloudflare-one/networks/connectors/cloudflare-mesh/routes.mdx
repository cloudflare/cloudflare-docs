...
1. **Point your router's DNS to the Gateway resolver IPs:**
   - `172.64.36.1`
   - `172.64.36.2`

2. **Add an IP route** on your router that routes these Gateway resolver IPs to the Mesh node's local IP, so DNS traffic reaches Cloudflare through the node.

3. **Ensure the following IPs route through the Mesh node** in your [Split Tunnels](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/route-traffic/split-tunnels/) configuration:
