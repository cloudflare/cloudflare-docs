...
Hostname routes replace [virtual networks](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/tunnel-virtual-networks/) as the way to reach resources: because a hostname is globally unique, **overlapping hostnames are not supported** and a hostname can only be routed to one node or tunnel at a time.
...
In the common case, the same Mesh node carries both the DNS resolution and the application traffic, so the node resolves the hostname itself. Unlike a Cloudflare Tunnel, a Mesh node does **not** require you to run a DNS server — add an entry to the node's hosts file (for example, `/etc/hosts` on Linux) mapping the hostname to its private IP:

```txt title="/etc/hosts"
10.0.0.50 wiki.internal.local
```

Alternatively, the node's system resolver can resolve the hostname if it already points at a DNS server the node can reach.
...
A **public** hostname does not need internal DNS resolution — Gateway resolves it publicly and the node egresses the traffic using its public IP.
