...
If you installed the node, it should appear as **Online** on the Mesh overview page along with its assigned **Mesh IP**. You can verify the connection from the server by running `warp-cli status` — the output shows `Status update: Connected` when the node is successfully connected.
