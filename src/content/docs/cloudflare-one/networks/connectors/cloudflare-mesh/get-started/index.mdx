...
| Connect an unattended Windows, macOS, or Linux device | [Headless client device](/cloudflare-one/networks/connectors/cloudflare-mesh/guides/connect-client-devices/#headless-windows-macos-and-linux-devices) | Service token and managed deployment parameters | No |
...
- **Client device cannot reach Mesh IPs** — Verify that your Split Tunnel configuration routes the Mesh IP range (`100.96.0.0/12`) through Cloudflare. For details, refer to [Connect client devices](/cloudflare-one/networks/connectors/cloudflare-mesh/guides/connect-client-devices/).
...
- [**Connect client devices**](/cloudflare-one/networks/connectors/cloudflare-mesh/guides/connect-client-devices/) — Platform-specific installation details, Split Tunnel configuration, and firewall considerations.
- [**Run in Docker / Kubernetes**](/cloudflare-one/networks/connectors/cloudflare-mesh/guides/run-mesh-in-containers/) — Deploy a Mesh node as a Docker container for Docker Compose, Kubernetes, and CI/CD pipelines.
- [**Add routes**](/cloudflare-one/networks/connectors/cloudflare-mesh/features/routes/) — Make an entire subnet behind your node reachable (databases, printers, other servers).
...
- [**Tips and best practices**](/cloudflare-one/networks/connectors/cloudflare-mesh/best-practices/) — Cloud VPC configuration, updating the client, running alongside cloudflared.
