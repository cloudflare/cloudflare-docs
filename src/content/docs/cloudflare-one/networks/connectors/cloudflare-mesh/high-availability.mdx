...
- Failover is handled at the Cloudflare edge.
...
When you create a Mesh node through the [dashboard setup wizard](/cloudflare-one/networks/connectors/cloudflare-mesh/get-started/), high availability is enabled by default.

1. Go to **Networking** > **Mesh**.
2. Select **Add a node**.
3. Follow the setup wizard. The node is created with HA enabled automatically.
4. Copy the install commands and run them on your first Linux host.
...
1. In the [Cloudflare dashboard](https://dash.cloudflare.com/), go to **Networking** > **Mesh**.
...
First, retrieve the node's token:
...
The response contains the token string. Then install the client and register on a new Linux host:
...
List all replicas and their HA status using the connections endpoint:
...
- **Inbound traffic** (from Mesh clients to the subnet) fails over automatically at the Cloudflare edge — Cloudflare routes traffic to the newly promoted active replica.
...
- **MASQUE protocol required** — High availability requires the MASQUE transport protocol. WireGuard does not support HA. Mesh nodes use MASQUE by default.
