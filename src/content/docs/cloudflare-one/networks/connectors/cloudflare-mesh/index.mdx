...
import { DashButton, Render, Details } from "~/components";
...
Every participant is assigned a private IP from the `100.96.0.0/12` range. In other parts of the Cloudflare One documentation, these are called [device IPs](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/device-ips/) — they are the same thing.
...
<Details header="Coming from another mesh product?">
...
| Admin console          | [Cloudflare dashboard](https://dash.cloudflare.com/?to=/:account/mesh) — **Networking** > **Mesh**                                                                                                                  |

Key differences:

- Configuration is managed entirely through the Cloudflare dashboard or API — no command-line administration required.
...
Cloudflare Mesh is in the [Cloudflare dashboard](https://dash.cloudflare.com/) at **Networking** > **Mesh**.
