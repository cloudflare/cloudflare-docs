...
For production deployments, you can run multiple replicas of a Mesh node in active-passive mode. All replicas share the same node identity and advertise the same [routes](/cloudflare-one/networks/connectors/cloudflare-mesh/features/routes/). If the active replica goes down, Cloudflare automatically promotes a standby replica.
