...
For production site-to-site deployments, consider enabling [high availability](/cloudflare-one/networks/connectors/cloudflare-mesh/features/high-availability/) on each node. HA provides failover for the CIDR routes advertised by a node — if the active replica goes down, Cloudflare promotes a standby so traffic to the subnet continues to flow.
