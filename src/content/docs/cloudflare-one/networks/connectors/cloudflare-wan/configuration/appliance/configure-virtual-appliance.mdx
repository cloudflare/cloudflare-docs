...
import { Aside, Tabs, TabItem, Card, Code, GlossaryTooltip } from "~/components";
...
5. Select the network interface card (NIC) you want to use with Virtual Appliance.
...
9. The script will apply your settings and configure the virtual machine template for Virtual Appliance.
10. In the **Hardware settings** for the new VM, make sure the hardware settings match the minimum requirements for running Virtual Appliance. Make changes to the RAM and CPU if needed.
...
<Aside type="caution">Remember to connect Virtual Appliance through a route that supports DHCP for its first connection to the Internet. Otherwise, Virtual Appliance will not work.</Aside>
...
<Aside type="caution">Make sure you complete the setup workflow and activate your Virtual Appliance before changing the WAN settings to a static IP.</Aside>
