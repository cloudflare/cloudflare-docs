...
![Table of branch subnet information](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/branch-subnets.png)

_Note: Labels in this image may reflect a previous product name._

The following example shows the **east_branch** deployment on the Orchestrator.

![GCP East deployment configuration](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/east-branch-deployment.png)
...
![IPsec tunnel values for east and west branches](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/central-west-branch-ipsec.png)

_Note: Labels in this image may reflect a previous product name._

The following example shows the **central_branch** deployment on the Orchestrator.

![Values for central branch configuration within Orchestrator](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/orchestrator-ipsec.png)
...
Cloudflare's [tunnel health checks](/cloudflare-one/networks/connectors/cloudflare-wan/reference/tunnel-health-checks/) are ping reply packets encapsulated in GRE packets. The source IP is the EdgeConnect WAN interface used to establish a tunnel, and the destination IP is Cloudflare servers. These packets need to be sent directly from the WAN interface and not through the established tunnels.
...
![Application definition screen with IP values](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/app-definition.png)
...
![Business Intent Overlay screen with breakout and CF overlays](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/biz-intent-overlay.png)
...
![Business Intent Overlay screen with breakout and CF overlays for IPsec](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/biz-intent-overlay-ipsec.png)
...
![Diagram of GCP, Aruba Orchestratror, and Cloudflare products](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/gcp-edgeconnect-diagram.png)
...
![Modify Passthrough Tunnel screen](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/modify-passthrough.png)

![Edit Virtual Tunnel Interface screen](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/edit-vti.png)
...
![GRE tunnels information for each branch](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/gre-tunnels-edgeconnect.png)

</Details>

<Details header="IPsec tunnel configuration">

![Diagram of GCP, Aruba Orchestratror, and Cloudflare products for IPsec tunnels](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/gcp-edgeconnect-diagram-ipsec.png)
...
![Modify Passthrough Tunnel dialog with General values](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/general-modify-passthrough.png)

![Modify Passthrough Tunnel dialog with IKE values](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/ike-modify-passthrough.png)

![Modify Passthrough Tunnel dialog with IPsec values](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/ipsec-modify-passthrough.png)

**Create a Virtual Tunnel Interface (VTI) on the EdgeConnect appliance**

![Values for Edit VTI Interface](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/vti-interface-ipsec.png)
...
1. Define static routes on the Cloudflare dashboard for the LAN subnet(s) attached to the EdgeConnect appliance. Use the private IP pair for the EdgeConnect tunnel endpoint.
   ...
   In this example, the traffic to subnet `10.3.0.0/16` attached to the **east_branch** EdgeConnect appliance has a next hop of `10.40.8.10`.

![Static route information for each branch](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/static-routes-cf.png)

2. Define static routes on the Orchestrator so Cloudflare can route traffic between sites.

   This example creates a route for the subnet `10.30.0.0/24` on the **west_branch** to route via the established GRE tunnel between the EdgeConnect appliance and Cloudflare.

![Static route information for each branch](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/static-routes-edgeconnect.png)

</Details>

<Details header="IPsec tunnel configuration">

![Static route values from Cloudflare dashboard](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/static-routes-ipsec.png)

**Static routes for central branch on EdgeConnect**

![Static route values from EdgeConnect for central branch](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/static-routes-central-ipsec.png)

**Static routes for west branch on EdgeConnect**

![Static route values from EdgeConnect for west branch](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/static-routes-west-ipsec.png)
...
![Curl example for validating Secure Web Gateway](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/validate-swg-curl.png)
...
![Dashboard example for validating Secure Web Gateway](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/dash-validate-swg.png)

**Validate east-west traffic**

To validate east-west traffic flow, perform a traceroute as shown in the example.

![Traceroute example for verifying east-west traffic](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/validate-traceroute.png)
...
![cURL example for validating traffic](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/static-routes-west-ipsec.png)
...
![Dashboard example for validating Secure Web Gateway](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/dash-validation-ipsec.png)

**Validate east-west traffic**

To validate east-west traffic flow, perform a traceroute as shown in the example.

![Traceroute example for IPsec validation](~/assets/images/cloudflare-wan/third-party/aruba-edge-connect/traceroute-ipsec.png)
...
To begin filtering traffic and gathering analytics, refer to the [Cloudflare Network Firewall documentation](/cloudflare-network-firewall/) to learn how to create filters for east-west inter-branch traffic and the [Secure Web Gateway documentation](/cloudflare-one/traffic-policies/) to learn how to configure Gateway policies if you decide to send traffic from your local private subnets to the Internet through Cloudflare Gateway.
