...
![Traffic flow diagram for GRE](~/assets/images/cloudflare-wan/third-party/viptela/viptela-flow-diagram-gre.png)
...
![Update template fields for GRE tunnel](~/assets/images/cloudflare-wan/third-party/viptela/viptela-update-device-template-gre.png)

## 3. Create tunnels in Cloudflare

Refer to [Configure tunnel endpoints](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/) for more information on creating a GRE tunnel.

![Established GRE tunnel in Cloudflare dashboard](~/assets/images/cloudflare-wan/third-party/viptela/viptela-gre-tunnel.png)

## 4. Define static routes

Refer to [Configure static routes](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-routes/#create-a-static-route) for more information on configuring your static routes.

![Established GRE static routes in Cloudflare dashboard](~/assets/images/cloudflare-wan/third-party/viptela/viptela-gre-static-routes.png)
...
![cURL example for a request to neverssl.com](~/assets/images/cloudflare-wan/third-party/viptela/viptela-curl-traffic-flow.png)

A matching blocked log line is visible from the Cloudflare logs.

![A blocked log from Gateway Activity Log in the Cloudflare dashboard](~/assets/images/cloudflare-wan/third-party/viptela/viptela-gre-swg-traffic.png)

## Add new tunnels using IPsec

IPsec tunnels to Cloudflare can only be created on Cisco 8000v in the router mode today. Refer to the [Cisco IOS XE](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/third-party/cisco-ios-xe/) for more information.
