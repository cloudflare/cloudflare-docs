...
You can connect through [Generic Routing Encapsulation (GRE) or IPsec tunnels](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/) to Cloudflare WAN.
...
5. In **Gateway settings**, make sure you have the following settings:
   ...
   - **Local ID type**: Add the [IKE ID](/cloudflare-one/networks/connectors/cloudflare-wan/reference/gre-ipsec-tunnels/#supported-ike-id-formats) provided by Cloudflare.

![Configure an IPsec tunnel.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/2-ipsec-tunnel.png)

_Note: Labels in this image may reflect a previous product name._

After setting up your IPsec tunnel, it will show up on the IPsec connections list with an **Active** status.

![The IPsec tunnel should show up on the IPsec connections list.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/2b-ipsec-tunnel.png)

_Note: Labels in this image may reflect a previous product name._

### 3. Assign the XFRM interface address

You must use an interface address from the `/31` subnet required to [configure tunnel endpoints](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/) on Cloudflare WAN.
...
3. Edit the interface to assign an address from the `/31` subnet required to [configure tunnel endpoints](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/). When you are finished, it should look similar to the following:

![Configure a XFRM interface.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/3-xfrm-interface.png)
...
![Create a firewall rule with the criteria and security policies from your company](~/assets/images/cloudflare-wan/third-party/sophos-firewall/4-firewall-rule.png)
...
2. Enter **4** to choose **Device console**, and enter the following command:
   ...
   ![Access the CLI to disable anti-replay](~/assets/images/cloudflare-wan/third-party/sophos-firewall/5-sfos-19.png)
...
2. Enter **4** to choose **Device console**, and enter the following command:
   ...
   ![Access the CLI to configure a GRE tunnel](~/assets/images/cloudflare-wan/third-party/sophos-firewall/1-gre-connection.png)
...
![Go to static routes to add an XFRM interface-based route](~/assets/images/cloudflare-wan/third-party/sophos-firewall/static-route.png)
...
![Go to Gateways to add an XFRM interface-based route](~/assets/images/cloudflare-wan/third-party/sophos-firewall/1-sd-wan-gateway.png)
...
![Go to SD-WAN to add the desired networks and services in the route.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/2-sd-wan-routes.png)
...
![Add the route on the CLI.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/gre-route-cli.png)
...
![Add a custom gateway on GRE.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/sd-wan-1-gre.png)

2. Add an SD-WAN route with the desired networks and services in the route to redirect traffic to Cloudflare.

![Add an SD-WAN route.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/2-sd-wan-routes.png)

## Verify tunnel status on Cloudflare dashboard

The Cloudflare dashboard monitors the health of all anycast tunnels on your account that route traffic from Cloudflare to your origin network. Refer to [Check tunnel health in the dashboard](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/common-settings/check-tunnel-health-dashboard/) for more information.
...
![Add the IP address provided by Cloudflare to prevent the probe from being dropped by the firewall.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/2-icmp-probe-firewall.png)
...
![Configure an SD-WAN route so the ICMP reply goes back to Cloudflare via the same tunnel.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/3-icmp-probe-reply.png)
...
![If the tunnel is working, it will show up with a green status.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/2b-ipsec-tunnel.png)

_Note: Labels in this image may reflect a previous product name._

The corresponding XFRM interface will also show a **Connected** status.

![The XFRM interface will also show a connected status.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/1-sd-wan-gateway.png)
...
![The GRE tunnel will show a status of Enabled when working.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/gre-status-enabled.png)

![The GRE tunnel will show a status of Enabled when working.](~/assets/images/cloudflare-wan/third-party/sophos-firewall/gre-status-enabled-b.png)
