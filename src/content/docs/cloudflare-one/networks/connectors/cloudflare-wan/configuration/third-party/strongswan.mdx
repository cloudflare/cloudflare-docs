...
Configure the [bidirectional health checks](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/#add-tunnels) target for Cloudflare WAN. For this tutorial, use `172.64.240.252` as the target IP address, and `type` as the request.
...
![tcpdump shows the IPsec encrypted packets from Cloudflare's health probes](~/assets/images/cloudflare-wan/third-party/strongswan/ipsec.png)
...
![If you run tcpdump on vti0 you can check for decrypted packets](~/assets/images/cloudflare-wan/third-party/strongswan/tcpdump.png)
