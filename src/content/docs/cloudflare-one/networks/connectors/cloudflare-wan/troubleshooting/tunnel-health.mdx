...
- **API**: Tunnel health status via the [Cloudflare WAN tunnel health API](/cloudflare-one/networks/connectors/cloudflare-wan/reference/tunnel-health-checks/)
- **Network Analytics**: Traffic volume, packet counts, and protocol distribution through [Network Analytics](/cloudflare-one/networks/connectors/cloudflare-wan/analytics/network-analytics/)
- **From your network**: Traceroute and MTR from your tunnel endpoint toward Cloudflare. Since Cloudflare endpoints use anycast, this tests the path to the nearest data center only. To test specific regions, use the [Cloudflare Traceroute API](/api/resources/diagnostics/subresources/traceroutes/methods/create/) to run traceroutes from specific Cloudflare locations to your network.
...
4. **IPsec parameters**: Confirm your cryptographic parameters match [Cloudflare's supported configuration](/cloudflare-one/networks/connectors/cloudflare-wan/reference/gre-ipsec-tunnels/#supported-configuration-parameters).
...
:::note[100% degraded is a state, not a packet loss percentage]
...
For instructions on monitoring tunnel status, refer to [Check tunnel health in the dashboard](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/common-settings/check-tunnel-health-dashboard/).
...
For more information, refer to [Cloudflare Network Firewall rules and endpoint health checks](/cloudflare-network-firewall/about/ruleset-logic/#cloudflare-network-firewall-rules-and-magic-transit-endpoint-health-checks).
...
For a detailed explanation of why this setting is necessary, refer to [Anti-replay protection](/cloudflare-one/networks/connectors/cloudflare-wan/reference/anti-replay-protection/).
...
For detailed information on how bidirectional health checks work, refer to [Tunnel health checks](/cloudflare-one/networks/connectors/cloudflare-wan/reference/tunnel-health-checks/).
...
For the complete list of supported parameters, refer to [Supported configuration parameters](/cloudflare-one/networks/connectors/cloudflare-wan/reference/gre-ipsec-tunnels/#supported-configuration-parameters).
...
- [Tunnel health checks](/cloudflare-one/networks/connectors/cloudflare-wan/reference/tunnel-health-checks/): Technical details on health check behavior
- [Anti-replay protection](/cloudflare-one/networks/connectors/cloudflare-wan/reference/anti-replay-protection/): Why anti-replay must be disabled
- [Configure tunnel endpoints](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/): Tunnel setup instructions
- [Check tunnel health in the dashboard](/cloudflare-one/networks/connectors/cloudflare-wan/configuration/common-settings/check-tunnel-health-dashboard/): Dashboard navigation guide
- [Network Analytics](/cloudflare-one/networks/connectors/cloudflare-wan/analytics/network-analytics/): Traffic analysis tools
