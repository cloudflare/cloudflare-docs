...
Verify that your local firewall allows traffic from the <GlossaryTooltip term="WARP CGNAT IP">WARP CGNAT IPs</GlossaryTooltip> (or your [custom device IP subnet](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/device-ips/)). For example, Windows Firewall blocks inbound traffic from `100.96.0.0/12` by default. On Windows devices, you will need to add a firewall rule that allows incoming requests from `100.96.0.0/12` for the desired protocols and/or ports.