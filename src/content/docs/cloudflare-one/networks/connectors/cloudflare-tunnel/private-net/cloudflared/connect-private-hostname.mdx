...
| [Cloudflare Mesh](/cloudflare-one/networks/connectors/cloudflare-mesh/routes/#hostname-routes) | ✅             | 2026.6.822.0 (Linux) |
...
To attract a hostname's traffic to a Mesh node instead of a `cloudflared` tunnel, add a [hostname route](/cloudflare-one/networks/connectors/cloudflare-mesh/routes/#hostname-routes) to the node. The <GlossaryTooltip term="initial resolved IP">initial resolved IP</GlossaryTooltip> listed above must route through Cloudflare on both the Mesh node and client device profiles, and — for a private hostname — the node must be able to resolve the hostname (via its local hosts file or a Gateway resolver policy). Refer to [Hostname routes](/cloudflare-one/networks/connectors/cloudflare-mesh/routes/#hostname-routes).
...
If you prefer to secure the application using a traditional firewall model, you can build Gateway network policies using the [SNI](/cloudflare-one/traffic-policies/policy-types/network-policies/selectors/#sni) or [SNI Domain](/cloudflare-one/traffic-policies/policy-types/network-policies/selectors/#sni-domain) selector. For an additional layer of protection, add a Gateway DNS policy to allow or block the [Host](/cloudflare-one/traffic-policies/policy-types/dns-policies/selectors/#host) or [Domain](/cloudflare-one/traffic-policies/policy-types/dns-policies/selectors/#domain) from resolving.
...
Additionally, SNI selectors will only apply to Cloudflare One Client traffic. If your users will be connecting from other [on-ramps](#device-connectivity), you can allow or block network traffic using the [Destination IP](/cloudflare-one/traffic-policies/policy-types/network-policies/selectors/#destination-ip) selector instead of SNI.
:::
