...
If you prefer to secure the application using a traditional firewall model, you can build Gateway network policies using the [SNI](/cloudflare-one/traffic-policies/policy-types/network-policies/#sni) or [SNI Domain](/cloudflare-one/traffic-policies/policy-types/network-policies/#sni-domain) selector. For an additional layer of protection, add a Gateway DNS policy to allow or block the [Host](/cloudflare-one/traffic-policies/policy-types/dns-policies/#host) or [Domain](/cloudflare-one/traffic-policies/policy-types/dns-policies/#domain) from resolving.
