...
A replica is an additional instance of `cloudflared` running the same tunnel. For remotely-managed tunnels, you can [run multiple named services](/tunnel/guides/named-services/) on one host. To keep the tunnel available if a host goes offline, deploy replicas on different hosts. DNS records and Cloudflare Load Balancers continue to point to the tunnel (`UUID.cfargotunnel.com`). Cloudflare routes traffic to the geographically closest replica.
