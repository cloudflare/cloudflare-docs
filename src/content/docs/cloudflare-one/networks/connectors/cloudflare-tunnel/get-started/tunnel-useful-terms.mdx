...
A tunnel is a secure, [outbound-only]() pathway you can establish between your infrastructure and Cloudflare's global network. Each tunnel you create will be assigned a [name](#tunnel-name) and a [UUID](#tunnel-uuid).
...
`cloudflared` is a software daemon that establishes connectivity from your infrastructure to the Cloudflare global network, and a command-line interface (CLI) tool that allows you to manage your Tunnel lifecycle. `cloudflared` creates [outbound-only connections](/cloudflare-one/networks/connectors/cloudflare-tunnel/#outbound-only-connection) to Cloudflare's global network and supports high availability by running multiple [replicas](), which provide redundancy and failover if a connector instance goes offline.
