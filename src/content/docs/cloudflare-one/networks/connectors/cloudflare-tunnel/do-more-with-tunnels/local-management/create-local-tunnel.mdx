...
1. Download `cloudflared` on your machine. Visit the <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/">downloads</a> page to find the right package for your OS.
...
Alternatively, you can <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/">download the latest Darwin amd64 release</a> directly.
...
Alternatively you can download the `cloudflared` binary or the linux packages to your machine and install manually. Visit the <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/">downloads</a> page to find the right package for your OS.
...
- Generate an account certificate, the <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/local-management/local-tunnel-terms/#certpem">cert.pem file</a>, in the <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/local-management/local-tunnel-terms/#default-cloudflared-directory">default `cloudflared` directory</a>.
...
1.  In your `.cloudflared` directory, create a <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/local-management/configuration-file/">`config.yml` file</a> using any text editor. This file will configure the tunnel to route traffic from a given origin to the hostname of your choice.

2.  Add the following fields to the file:

    If you are connecting a <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/routing-to-tunnel/">published application</a>:
    ...
    If you are connecting a <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/">private network</a>:

    <Code code={`tunnel: <Tunnel-UUID>\ncredentials-file: /root/.cloudflared/<Tunnel-UUID>.json`} lang="yml" />
...
1. To route a <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/routing-to-tunnel/">published application</a> through the tunnel:
...
Cloudflare Tunnel can install itself as a system service on Linux and Windows and as a launch agent on macOS. For more information, refer to <a href="/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/local-management/as-a-service/">run as a service</a>.
