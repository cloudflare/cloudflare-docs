...
- `cloudflared` installed on the server hosting your SSH service
...
End users must have the WARP client installed and configured to route traffic to Cloudflare.

### Prerequisites for WARP clients

To connect to private hostnames, WARP clients must forward the following traffic to Cloudflare:
