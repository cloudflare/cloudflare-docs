...
- A [Cloudflare Zero Trust organization](/cloudflare-one/setup/#2-create-a-zero-trust-organization)
...
Hostname routes allow you to SSH directly to `ssh.internal.local` without managing static IP routes.  Hostname routes are especially useful when your SSH server has an unknown or ephemeral IP address, such as dynamic infrastructure provisioned by cloud providers.
   ...
   <Details header="How hostname routing works">
	...
	2. Gateway resolves the hostname to an <GlossaryTooltip term="initial resolved IP">initial resolved IP</GlossaryTooltip>.
...
In [Local Domain Fallback](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/route-traffic/local-domains/), delete the top-level domain for your private hostname. This configures WARP to send the DNS query to Cloudflare Gateway for resolution.
