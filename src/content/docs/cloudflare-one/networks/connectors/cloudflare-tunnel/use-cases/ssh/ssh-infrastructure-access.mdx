...
2. [Enable the Gateway proxy for TCP](/cloudflare-one/traffic-controls/proxy/#turn-on-the-gateway-proxy).
...
Traffic from the Cloudflare One Client to your infrastructure targets is filtered by both [Gateway network policies](/cloudflare-one/traffic-controls/get-started/network/) and the application-specific Access policies.
