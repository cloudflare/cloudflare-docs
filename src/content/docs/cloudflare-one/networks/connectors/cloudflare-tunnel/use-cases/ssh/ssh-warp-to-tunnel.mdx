...
By default, `cloudflared` uses the private DNS resolver configured on its host machine (for example, in `/etc/resolv.conf` on Linux). To check if `cloudflared` successfully resolves `sh.internal.local`, run the following command from the `cloudflared` host machine:
...
If `ssh.internal.local` resolves to the correct private IP (the **Internal IP** of the GCP VM), no further configuration is required. You can skip to [Step 3.3](#33-configure-warp-clients).

<Details header="Troubleshoot private DNS resolver">
- Make sure that your private DNS resolver has a record that points `ssh.internal.local` to the SSH server's private IP.
- If using Google Cloud, you may need to [add a private zone to Cloud DNS](https://docs.cloud.google.com/dns/docs/zones#create-private-zone) so that `internal.local` resolves using your private DNS resolver.
</Details>
