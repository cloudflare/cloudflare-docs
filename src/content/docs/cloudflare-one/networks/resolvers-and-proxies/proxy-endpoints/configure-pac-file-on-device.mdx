...
2. Create an [HTTP policy](/cloudflare-one/traffic-policies/policy-types/http-policies/) to block a test domain (for example, `example.com`).
...
- [Create HTTP policies](/cloudflare-one/traffic-policies/policy-types/http-policies/) to filter proxy endpoint traffic.
...
- Use the [Proxy Endpoint selector](/cloudflare-one/traffic-policies/policy-types/http-policies/selectors/#proxy-endpoint) in HTTP and network policies to apply rules to proxy traffic.
