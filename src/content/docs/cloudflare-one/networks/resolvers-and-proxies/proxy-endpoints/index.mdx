...
#### Referrer header traffic

Traffic with a referrer HTTP header matching the domain of a recently logged in user from the same source IP will be allowed through and logged with a non-identity email address.
