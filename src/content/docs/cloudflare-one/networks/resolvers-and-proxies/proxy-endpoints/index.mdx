...
3. Use [Add An Access Application](/api/resources/zero_trust/subresources/access/subresources/applications/methods/create/) to associate the proxy endpoint with Access policies:
   ...
   Replace `<SUBDOMAIN>` with the subdomain from step 2 and `<ACCESS_POLICY_ID>` with the ID of an existing [reusable Access policy](/cloudflare-one/access-controls/policies/reusable-policies/).
