...
With Cloudflare Gateway, you can filter DNS over HTTPS (DoH) requests by [DNS location](/cloudflare-one/networks/resolvers-and-proxies/locations/) or by user without needing to install the WARP client on your devices.
...
Your DNS queries will now be sent to Gateway for filtering. To filter these requests, build a DNS policy using the [**DNS Location**](/cloudflare-one/networks/resolvers-and-proxies/locations/) selector.
