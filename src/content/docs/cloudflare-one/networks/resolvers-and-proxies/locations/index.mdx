...
Cloudflare will prefill the [**Source IPv4 Address**](/cloudflare-one/networks/resolvers-and-proxies/locations/dns-resolver-ips/#source-ip) based on the network you are on. Additionally, Enterprise users can use [dedicated DNS resolver IP addresses](/cloudflare-one/networks/resolvers-and-proxies/locations/dns-resolver-ips/#dns-resolver-ip) assigned to their account or [resolver IP addresses they provide (BYOIP)](/cloudflare-one/networks/resolvers-and-proxies/locations/dns-resolver-ips/#bring-your-own-dns-resolver-ip).
...
For more information, refer to [DNS over TLS](/cloudflare-one/networks/resolvers-and-proxies/dns-over-tls/).

### DNS over HTTPS (DoH)

<GlossaryDefinition term="DNS over HTTPS" />

Gateway requires a DoH endpoint for default DNS locations. For more information, refer to [DNS over HTTPS](/cloudflare-one/networks/resolvers-and-proxies/dns-over-https/).
...
- Your IPv4 and IPv6 endpoints use your [BYOIP addresses](/cloudflare-one/networks/resolvers-and-proxies/locations/dns-resolver-ips/#bring-your-own-dns-resolver-ip) (if any).
