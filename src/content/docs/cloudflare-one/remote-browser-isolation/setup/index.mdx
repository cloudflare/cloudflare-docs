...
Browser Isolation is enabled through [Secure Web Gateway HTTP policies](/cloudflare-one/traffic-controls/http-policies/). By default, no traffic is isolated until you have added an Isolate policy to your HTTP policies.
...
| --------------------------------------------------------------------------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------- |
| [Traffic and DNS mode](/cloudflare-one/traffic-controls/get-started/http/)                                | In-line      | Apply identity-based HTTP policies to traffic proxied through the Cloudflare One Client.                                       |
...
| [Gateway proxy endpoint (source IP)](/cloudflare-one/remote-browser-isolation/setup/non-identity/)        | In-line      | Apply non-identity HTTP policies to traffic forwarded to a [source IP proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/#source-ip-endpoint).              |
| [Gateway proxy endpoint (authorization)](/cloudflare-one/traffic-controls/proxy-endpoints/#authorization-endpoint) | In-line      | Apply identity-based HTTP policies to traffic forwarded to an [authorization proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/#authorization-endpoint).    |
...
3. Use the HTTP policy [selectors](/cloudflare-one/traffic-controls/http-policies/#selectors) and [operators](/cloudflare-one/traffic-controls/http-policies/#comparison-operators) to specify the websites or content you want to isolate.
...
5. (Optional) Configure [settings](/cloudflare-one/remote-browser-isolation/isolation-policies/#policy-settings) for an Isolate policy.
...
Cloudflare One Client users can temporarily disable remote browsing by [disconnecting the Cloudflare One Client](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/settings/#lock-device-client-switch).
