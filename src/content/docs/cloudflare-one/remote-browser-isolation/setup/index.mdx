...
Browser Isolation is enabled through [Secure Web Gateway HTTP policies](/cloudflare-one/traffic-policies/policy-types/http-policies/). By default, no traffic is isolated until you have added an Isolate policy to your HTTP policies.
...
| [Gateway proxy endpoint (source IP)](/cloudflare-one/remote-browser-isolation/setup/non-identity/)        | In-line      | Apply non-identity HTTP policies to traffic forwarded to a [source IP proxy endpoint](/cloudflare-one/networks/resolvers-and-proxies/proxy-endpoints/#source-ip-endpoint).              |
| [Gateway proxy endpoint (authorization)](/cloudflare-one/networks/resolvers-and-proxies/proxy-endpoints/#authorization-endpoint) | In-line      | Apply identity-based HTTP policies to traffic forwarded to an [authorization proxy endpoint](/cloudflare-one/networks/resolvers-and-proxies/proxy-endpoints/#authorization-endpoint).    |
...
3. Use the HTTP policy [selectors](/cloudflare-one/traffic-policies/policy-types/http-policies/#selectors) and [operators](/cloudflare-one/traffic-policies/policy-types/http-policies/#comparison-operators) to specify the websites or content you want to isolate.
