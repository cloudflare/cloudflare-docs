...
Browser Isolation supports one active window at a time. All tabs and windows in the same local browser (for example, all Chrome tabs) share a single isolated session. Within that session, only the tab or window currently in focus is actively rendered and processed by the remote browser. Background tabs and windows are suspended until you switch to them.
