...
import { Render, Badge, Tabs, TabItem, APIRequest , MermaidDiagram } from "~/components";
...
:::note
This option does not prevent files from being downloaded into the remote browser. To prevent files being downloaded into the remote browser, use HTTP Policies to block by [Download Mime Type](/cloudflare-one/traffic-policies/http-policies/#download-and-upload-mime-type).
:::
