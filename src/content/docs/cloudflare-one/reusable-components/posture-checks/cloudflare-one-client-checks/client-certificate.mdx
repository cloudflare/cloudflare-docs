...
<sup>1</sup> Client certificate checks that ran on an earlier client version will continue to work. To configure a new certificate check, update the Cloudflare One Client to the versions listed above.
</Details>

## Prerequisites

- A CA that issues client certificates for your devices. The Cloudflare One Client (formerly WARP Client) does not evaluate the certificate trust chain; this needs to be the issuing certificate.
...
5. You will be prompted for the following information:
   ...
   7. **Check for private key**: (Recommended) When enabled, the Cloudflare One Client checks that the device has a private key associated with the client certificate.
