...
5. You will be prompted for the following information:
   ...
   8. **Subject Alternative Name**: (Optional) To check for a Subject Alternative Name (SAN) on the client certificate, enter a string with optional `${serial_number}` and `${hostname}` variables (for example, `${serial_number}_mycompany`). The Cloudflare One Client matches supported SAN types exactly, ignoring case. The client certificate posture check does not match `otherName` SAN entries. You can add multiple SAN values to the posture check. One value with a supported SAN type must match.
