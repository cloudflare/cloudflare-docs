...
The legacy Tanium integration cannot be used in [Gateway network policies](/cloudflare-one/traffic-policies/policy-types/network-policies/selectors/#device-posture). Only [Access policies](/cloudflare-one/access-controls/policies/) are supported.
:::
...
You can now build [Access policies](/cloudflare-one/access-controls/policies/) that check [device posture signals](#tanium-endpoint-signals) from the Tanium endpoint.
