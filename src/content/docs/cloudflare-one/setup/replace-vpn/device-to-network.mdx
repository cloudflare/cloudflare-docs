...
<GlossaryTooltip term="Cloudflare Tunnel">
	[Cloudflare Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/)
</GlossaryTooltip>
is a network connector that creates an outbound-only connection between your
private network and Cloudflare. No open inbound ports or firewall changes are
required.
