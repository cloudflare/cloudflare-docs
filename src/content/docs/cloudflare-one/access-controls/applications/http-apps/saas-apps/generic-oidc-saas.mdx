...
10. Copy the following values to input into your SaaS application. Different SaaS applications may require different sets of input values.
    ...
    | Key endpoint | Returns the current public keys used to [verify the Access JWT](/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/validating-json/) <br/> `https://<your-team-name>.cloudflareaccess.com/cdn-cgi/access/sso/oidc/<client-id>/jwks` |
...
13. (Optional) Go to the **Additional settings** tab to customize the application experience:
...
<Render
	...
	params={{ session: "OIDC Access tokens", token: "Access token" }}
/>
:::
