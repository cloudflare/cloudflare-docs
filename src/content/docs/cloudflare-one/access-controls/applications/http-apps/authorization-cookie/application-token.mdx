...
Access includes identity provider groups in the application token on a best-effort basis. If the token approaches the 4 KB cookie size limit, Access drops the group claims to keep the cookie under the limit. Because group membership varies per user, this means a user who belongs to many groups may receive a token without group claims, while other users authenticating to the same application receive them.
