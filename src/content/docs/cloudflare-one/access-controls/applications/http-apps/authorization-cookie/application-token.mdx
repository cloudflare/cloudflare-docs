...
:::caution[Group claims are not guaranteed in the JWT]
When your identity provider sends groups, Access includes them in the token's `custom` claim on a best-effort basis. To keep the token within browser cookie size limits, Access trims custom SAML attributes and OIDC claims when they exceed an internal size budget, dropping configured attributes from the end of the list first. A user who belongs to many groups can therefore receive a token without group claims, while other users on the same application keep them.

Do not rely on group claims in the JWT for authorization decisions. If your application needs a user's groups, call the `/cdn-cgi/access/get-identity` endpoint instead, which always returns the full set of groups.
:::
