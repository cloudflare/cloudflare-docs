...
The binding cookie protects against a stolen `CF_Authorization` cookie being used without the associated binding cookie. If a request arrives at Cloudflare's network with a valid `CF_Authorization` cookie but without the expected binding cookie, Cloudflare rejects the request.
...
- You have turned on [WARP authentication identity](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/client-sessions/) for the application.
