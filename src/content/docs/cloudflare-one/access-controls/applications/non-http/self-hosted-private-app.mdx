...
12. (Optional) Turn on **Enable clientless access in App Launcher** to allow users who match the Access policies to access this private destination through the App Launcher without the Cloudflare One client. When enabled, the application tile in the App Launcher opens the application in [Clientless Browser Isolation](/cloudflare-one/remote-browser-isolation/setup/clientless-browser-isolation/), which routes traffic through a remote browser. This is useful for users on unmanaged devices or contractors who cannot install the device client.

	:::note
	This setting requires Clientless Browser Isolation with policies that allow users of this application.
	:::
