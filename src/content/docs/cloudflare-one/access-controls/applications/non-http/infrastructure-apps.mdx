...
<Render file="access/add-target-generic" product="cloudflare-one" />
...
Infrastructure Access supports granular permissions through [Cloudflare's role-based access control](/cloudflare-one/access-controls/granular-permissions/). Administrators can assign roles scoped to specific targets instead of granting account-wide access. When a user with a scoped role calls the targets list API, the response is automatically filtered to include only the targets they have permission to view.
