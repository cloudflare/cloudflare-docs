...
Once connected to a portal, users can manage their upstream MCP server sessions without leaving their MCP client.

### Reauthenticate a server

When an upstream MCP server token expires or needs a refresh, the portal prompts you to reauthenticate directly from within your MCP client. The portal opens an authorization prompt in your browser. After you complete the login, the updated session returns to the client automatically.

If your MCP client does not support in-session reauthentication, you can force all MCP servers to reauthenticate by clearing cached credentials:
...
You can toggle individual upstream MCP servers on or off during an active session. The portal uses [MCP elicitations](https://modelcontextprotocol.io/specification/2025-03-26/server/elicitation) to present a server selection prompt within your MCP client. You can also ask your AI agent to enable or disable a specific server by name.

Disabling a server removes its tools from the active session. This is useful for reducing context window usage when you do not need all available servers.
...
### Clear a server session

To revoke your authorization for an individual upstream MCP server, ask your AI agent to disconnect or log out from that server. The portal clears the session for that server while keeping your other server sessions active.
