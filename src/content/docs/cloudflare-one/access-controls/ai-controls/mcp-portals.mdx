...
1. After authenticating to the Portal, my user gets the error "No allowed servers available, check your Zero Trust Policies"
	1. A Portal and server must both have an attached policy. Ensure that any assigned servers to the Portal have their own associated policy.
	2. The server's admin authentication may be expired. Check that the server's status in the AI Controls dashboard is "Ready"
2. The portal URL will not prompt for authentication when it's added to an MCP client
	1. Verify that the Portal has an assigned policy
