...
When Code Mode is active, the portal advertises `portal_codemode_search` and `portal_codemode_execute` to connected MCP clients. The AI agent searches the available tool definitions, then composes one or more tool calls in the Dynamic Worker environment.
