...
- An [active domain on Cloudflare](/fundamentals/manage-domains/add-site/) for the portal URL that uses either a [full setup](/dns/zone-setups/full-setup/) or a [partial (`CNAME`) setup](/dns/zone-setups/partial-setup/)
...
Before you add the server, connect its network to Cloudflare. Use [Cloudflare Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/) or another [Cloudflare One connector](/cloudflare-one/networks/connectors/). Configure a [private hostname route](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/cloudflared/connect-private-hostname/) or [CIDR route](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/cloudflared/connect-cidr/) for the server.
...
4. Scroll down to the **Tools** section, then turn off the toggle next to any tool or prompt that you want to hide from users.
   ...
   ![The tools panel for an MCP server shows a list of tools with a toggle next to each one.](~/assets/images/cloudflare-one/applications/mcp-portal-manage-tools.png)
...
Refer to the [Code Mode SDK reference](/agents/tools/codemode/api-reference/) for details on writing code for these tools.
...
You can manage MCP servers and portals using the Cloudflare Terraform provider. Use the [`cloudflare_zero_trust_access_ai_controls_mcp_server`](https://registry.terraform.io/providers/cloudflare/cloudflare/latest/docs/resources/zero_trust_access_ai_controls_mcp_server) and [`cloudflare_zero_trust_access_ai_controls_mcp_portal`](https://registry.terraform.io/providers/cloudflare/cloudflare/latest/docs/resources/zero_trust_access_ai_controls_mcp_portal) resources.
...
The following example creates an unauthenticated upstream MCP server, a portal, and the Access resources required for a service token to use the server through the portal:

```hcl title="MCP server portal with service token access"
resource "cloudflare_zero_trust_access_service_token" "mcp_bot" {
  account_id = var.cloudflare_account_id
  name       = "MCP bot"
  duration   = "8760h"
}

resource "cloudflare_zero_trust_access_policy" "mcp_bot" {
  account_id = var.cloudflare_account_id
  name       = "Allow MCP bot"
  ...
  include = [{
    service_token = {
      token_id = cloudflare_zero_trust_access_service_token.mcp_bot.id
    }
  }]
}

resource "cloudflare_zero_trust_access_ai_controls_mcp_server" "docs" {
  account_id = var.cloudflare_account_id
  id         = "cloudflare-docs"
...
resource "cloudflare_zero_trust_access_application" "docs" {
  ...
  destinations = [{
    ...
    mcp_server_id = cloudflare_zero_trust_access_ai_controls_mcp_server.docs.id
  }]

  policies = [{
    id         = cloudflare_zero_trust_access_policy.mcp_bot.id
    precedence = 1
  }]
}

resource "cloudflare_zero_trust_access_ai_controls_mcp_portal" "engineering" {
  account_id = var.cloudflare_account_id
  id         = "engineering"
  name       = "Engineering Portal"
  hostname   = "mcp.example.com"
  ...
  servers = [{
    server_id = cloudflare_zero_trust_access_ai_controls_mcp_server.docs.id
    on_behalf = false
  }]
}

resource "cloudflare_zero_trust_access_application" "engineering" {
  account_id = var.cloudflare_account_id
  name       = "Engineering Portal"
  type       = "mcp_portal"
  domain     = cloudflare_zero_trust_access_ai_controls_mcp_portal.engineering.hostname

  destinations = [{
    type = "public"
    uri  = cloudflare_zero_trust_access_ai_controls_mcp_portal.engineering.hostname
  }]
...
The provider marks the service token's `client_secret` as sensitive, but Terraform still stores it in state. Store Terraform state securely and pass the credentials to your MCP client through a secret manager or environment variables.

## Code Mode

[Code Mode](/agents/tools/codemode/) reduces context window usage by replacing upstream tool definitions with two tools for search and code execution. The connected AI agent writes JavaScript that calls typed `codemode.*` methods. The generated code runs in an isolated [Dynamic Worker](/workers/runtime-apis/bindings/worker-loader/) environment. Authentication credentials and environment variables remain outside the model context.
...
3. Send the complete body in a `PUT` request to the [Update a MCP Portal](/api/resources/zero_trust/subresources/access/subresources/ai_controls/subresources/mcp/subresources/portals/methods/update/) endpoint. Including the complete body prevents other portal settings from being overwritten.
</Steps>
...
For more information on building with Code Mode, refer to the [Code Mode SDK reference](/agents/tools/codemode/api-reference/).
...
To configure this flow with the API, set `servers[].on_behalf` to `false` in the [MCP portal request](/api/resources/zero_trust/subresources/access/subresources/ai_controls/subresources/mcp/subresources/portals/methods/create/). Create a `non_identity` Access policy with a `service_token` include rule. Attach the policy to the portal and each linked server Access application. For a complete Terraform example, refer to [Configure via Terraform](#configure-via-terraform).

To verify the connection, send an MCP `initialize` request with the service token headers:
...
curl https://<subdomain>.<domain>/mcp \
  ...
  --data '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-11-25","capabilities":{},"clientInfo":{"name":"service-token-test","version":"1.0.0"}}}'
```

For `mcp-remote`, pass the credentials through environment variables instead of storing them in the MCP client configuration:
...
For more information on the Code Mode pattern behind `search_and_execute`, refer to [Code Mode](/agents/tools/codemode/).
...
[Release the zone hold](/fundamentals/account/account-security/zone-holds/#release-zone-holds) for the selected zone and each parent zone. Then create the portal again. A parent zone can belong to a different Cloudflare account.
