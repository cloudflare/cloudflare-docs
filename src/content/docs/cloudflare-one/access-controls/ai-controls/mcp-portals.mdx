...
1. In the [Cloudflare dashboard](https://dash.cloudflare.com/), go to **Zero Trust** > **Access controls** > **AI controls**.
2. Find the portal you want to connect, then select the three dots > **Edit**.
3. In **Advanced settings**, add `https://claude.ai/api/mcp/auth_callback` to **Allowed redirect URIs**, then select **Save**.
4. In Claude Desktop, go to **Customize** > **Connectors**, select **+**, and then select **Add custom connector**. Team and Enterprise users need an owner to add the connector from **Organization settings** > **Connectors**.
5. Enter `https://<subdomain>.<domain>/mcp` as the remote MCP server URL, then finish adding the connector.

</Steps>

For plan-specific instructions, refer to [Claude's custom connector guide](https://support.claude.com/en/articles/11175166-getting-started-with-custom-connectors-using-remote-mcp).

:::note
Claude connects to remote MCP servers from Anthropic's cloud infrastructure, including when you use Claude Desktop. The connection does not pass through the Cloudflare One Client on the user's device. Do not add a Cloudflare root certificate to the Claude connector.
:::

### Test in Workers AI Playground
...
For clients that do not support remote MCP servers directly, use `mcp-remote` as a local bridge:

```json title="MCP client configuration using mcp-remote"
{
	"mcpServers": {
		"example-mcp-server": {
			"command": "npx",
...
Do not use the `serverURL` parameter with MCP portals. It may cause issues with portal session creation and management.
