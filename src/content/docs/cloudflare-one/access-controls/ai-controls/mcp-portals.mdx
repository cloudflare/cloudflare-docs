...
The portal responds with an authorization URL via elicitation. Open this URL in your web browser to access the server selection page:
...
You can also enable or disable a specific server directly from your MCP client without visiting the server selection page. Ask your AI agent by name, for example:
...
When an upstream MCP server token expires or needs a refresh, the portal prompts you to reauthenticate directly from within your MCP client. The portal provides an authorization URL via elicitation that you open in your browser. After you complete the login, the updated session returns to the client automatically.

If your MCP client does not support elicitations, you can force all MCP servers to reauthenticate by clearing cached credentials:
