...
MCP server portals can connect to an MCP server available only on your private network. The MCP server URL can use a private hostname or IP address.

:::note
OAuth-protected private MCP servers are supported. The MCP server URL and its [OAuth protected resource metadata](https://www.rfc-editor.org/rfc/rfc9728.html) can remain private. The OAuth authorization and token endpoints must be accessible on the public Internet. If Cloudflare automatically registers the OAuth client through Dynamic Client Registration (DCR), the registration endpoint must also be accessible on the public Internet.
:::
...
1. Follow steps 1 through 7 in [Add an MCP server](#add-an-mcp-server), and enter the private URL in **HTTP URL**.
...
:::caution
If you manage MCP servers with Terraform, changing `hostname` or `auth_type` replaces the MCP server. The replacement is not automatically added to portals that referenced the original server. After you apply either change, add the replacement server to each portal.
:::
