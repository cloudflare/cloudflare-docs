...
11. Go to **Advanced settings** and turn on **Managed OAuth**. This allows MCP clients to authenticate using a standard OAuth 2.0 flow. Only enable Managed OAuth for an MCP server that validates the Access JWT sent by Cloudflare. The example Worker in this guide validates this JWT. For more information, refer to [Managed OAuth](/cloudflare-one/access-controls/applications/http-apps/managed-oauth/).
...
4. Under **Variables and Secrets**, update each variable with the corresponding value:
   ...
   | `POLICY_AUD`     | The AUD tag copied from your [Access application](#2-create-an-mcp-server-access-application) |
