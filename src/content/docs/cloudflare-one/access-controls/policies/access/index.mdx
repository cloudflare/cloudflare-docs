...
Bypass does not enforce any Access security controls and requests are not logged. Bypass policies should be tested before deploying to production. Consider using [Service Auth](/cloudflare-one/access-controls/policies/access/#service-auth) if you would like to enforce policies and maintain logging without requiring user authentication.

As Bypass does not enforce Access security controls, Bypass policies do not support identity-based [rule types](/cloudflare-one/access-controls/policies/access/#rule-types). When making Bypass policies, you will not be able to apply certain identity-based [selectors](/cloudflare-one/access-controls/policies/access/#selectors) (such as email).
...
1.  [Create a rule group](/cloudflare-one/access-controls/policies/access/groups/) that includes users in Portugal OR in the United States:
...
| External Evaluation      | Allows or denies access based on [custom logic](/cloudflare-one/access-controls/policies/access/external-evaluation/) in an external API.                                                                                                                 | ✅               | ❌                               | ✅                       |
...
| Authentication Method    | Checks the [multifactor authentication](/cloudflare-one/access-controls/policies/access/mfa-requirements/) method used by the user, if supported by the identity provider.                                                                                | ✅               | ❌                               | ✅                       |
