...
A rule group is a collection of Access rules that can be configured once and then quickly applied across many Access policies. Rule groups use the same [rule types](/cloudflare-one/access-controls/policies/access/#rule-types) and [selectors](/cloudflare-one/access-controls/policies/access/#selectors) shown in the Access policy builder.
