...
For example, your organization may wish to deploy an MCP server that helps employees interact with internal applications. You can configure [Access policies](/cloudflare-one/access-controls/policies/access/#selectors) to ensure that only authorized users can access those applications, either directly or by using an <GlossaryTooltip term="MCP client">MCP client</GlossaryTooltip>.
...
1. Create the following Access policy, replacing the `app_uid` value with the `id` of your SaaS application:
	...
	:::note
	The `linked_app_token` rule type only works with [`non_identity` decisions](/cloudflare-one/access-controls/policies/access/#service-auth), similar to service token rules.
	:::
