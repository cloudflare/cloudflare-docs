...
6.  Add the private IP and/or private hostname that represents the application. You can use [wildcards](/cloudflare-one/access-controls/policies/access/app-paths/) with private hostnames to protect multiple parts of an application that share a root path.
