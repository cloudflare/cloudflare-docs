...
| **Email**             | Email address of the user who registered the Cloudflare One Client where traffic originated from. If a non-identity on-ramp (such as a [proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/)) or machine-level authentication (such as a [service token](/cloudflare-one/access-controls/service-credentials/service-tokens/)) was used, this value will be `non_identity@<team-domain>.cloudflareaccess.com`. |
| **Action**            | The [Action](/cloudflare-one/traffic-controls/dns-policies/#actions) Gateway applied to the query (such as Allow or Block).                                                                                                                                                                                                                                                                                                      |
...
| Field                  | Description                                                                                                                  |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **DNS location**       | [User-configured location](/cloudflare-one/traffic-controls/resolver-endpoints/) from where the DNS query was made. |
...
| **Initial query domain categories**        | [Content categories](/cloudflare-one/traffic-controls/domain-categories/) that the domain belongs to. |
...
| **Action**             | The Gateway [Action](/cloudflare-one/traffic-controls/dns-policies/#actions) taken based on the first rule that matched (such as Allow or Block). |
...
| **Detected Protocol**        | The detected [network protocol](/cloudflare-one/traffic-controls/network-policies/protocol-detection/).                                                    |
...
| **Proxy endpoint**           | [PAC file proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/) Gateway forwarded traffic to, if applicable.                    |
...
| **Application ID**           | ID of the application that matched the domain.                                                                                                             |
| **Application name**         | Name of the application that matched the domain.                                                                                                           |
| **Categories**               | [Content categories](/cloudflare-one/traffic-controls/domain-categories/) that the domain belongs to.                                                      |
| **Proxy endpoint**           | [PAC file proxy endpoint](/cloudflare-one/traffic-controls/proxy-endpoints/) Gateway forwarded traffic to, if applicable.                    |
| **Virtual Network**          | [Virtual network](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/cloudflared/tunnel-virtual-networks/) that the client is connected to. |
| **Sandbox scanned**          | Status of the [file quarantine](/cloudflare-one/traffic-controls/http-policies/file-sandboxing/).                                                          |
