...
Gateway activity logs are not available if you turn on the [Customer Metadata Boundary](/data-localization/metadata-boundary/) within Cloudflare Data Localization Suite (DLS). For more information, refer to [DLS product compatibility](/data-localization/compatibility/#zero-trust).
