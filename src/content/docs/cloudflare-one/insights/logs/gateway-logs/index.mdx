...
***Note***: **Gateway will only log TCP traffic with completed connections, in the event a connection is not complete (such as a TCP SYN with no SYN ACK) this traffic will not show in Network logs.**
