...
The log data for all [DEX device state events](/logs/logpush/logpush-job/datasets/account/dex_device_state_events/) can be exported to R2, a cloud bucket, or a SIEM via [Logpush](/cloudflare-one/insights/logs/logpush/). 