...
Monitor performance and network status for your organization's [fleet](/cloudflare-one/insights/dex/device-monitoring/#device-overview) or individual [user devices](/cloudflare-one/insights/dex/device-monitoring/#device-monitoring).
...
To view analytics on a per-device level, go to [Device monitoring](/cloudflare-one/insights/dex/device-monitoring/#device-monitoring).
...
- **Connectivity status**: Percentage of devices in a given WARP client state.
  ...
  | Paused       | A user or administrator has taken an explicit action to temporarily turn off WARP, for example by entering an [admin override code](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#allow-admin-override-codes). Paused clients will [auto-connect](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#auto-connect) after a timeout period. |
