...
You can enable payload logging for any Allow or Block HTTP policy that uses the [_DLP Profile_](/cloudflare-one/traffic-controls/http-policies/#dlp-profile) selector — the filter condition that matches traffic against your DLP detection profiles.
...
You can enable AI prompt content logging for any Allow or Block HTTP policy that uses the [_Application_](/cloudflare-one/traffic-controls/http-policies/#application) selector with a supported [Application Granular Controls](/cloudflare-one/traffic-controls/http-policies/#granular-controls) application. This means your policy must target a specific AI application (such as ChatGPT) that Gateway can inspect at a granular level.
