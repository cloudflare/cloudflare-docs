...
3. Confirm that the expected detection entry is referenced by at least one DLP profile in the account. The entry can be enabled or disabled.
...
5. Check whether a [Do Not Inspect policy](/cloudflare-one/traffic-policies/http-policies/#do-not-inspect) or another routing configuration excludes the traffic from inspection.
</Steps>
