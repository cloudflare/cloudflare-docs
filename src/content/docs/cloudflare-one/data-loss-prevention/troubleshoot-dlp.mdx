...
[Passive Detection](/cloudflare-one/data-loss-prevention/passive-detection/) shows detections from sampled Gateway traffic. If you expect results but the dashboard is empty:
...
4. Confirm that traffic meets the [Gateway inspection prerequisites](/cloudflare-one/data-loss-prevention/passive-detection/#prerequisites), including HTTP filtering and TLS decryption for HTTPS.
5. Check whether a [Do Not Inspect policy](/cloudflare-one/traffic-controls/http-policies/#do-not-inspect) or another routing configuration excludes the traffic from inspection.
</Steps>
...
You can also create policies that match trusted applications using the [**Do Not Scan** action](/cloudflare-one/traffic-controls/http-policies/#do-not-scan).
