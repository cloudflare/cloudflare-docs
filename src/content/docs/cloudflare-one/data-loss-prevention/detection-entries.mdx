...
Detection entries are the data patterns that DLP looks for when scanning your web traffic and SaaS applications. You add detection entries to [DLP profiles](/cloudflare-one/data-loss-prevention/dlp-profiles/), which define what DLP should detect. There are three types:
...
- [AI prompt topics](#ai-prompt-topics) — categories of sensitive prompts submitted to generative AI tools

For datasets containing sensitive data, you can configure values to be hashed before reaching Cloudflare and redacted from matches in [payload logs](/cloudflare-one/data-loss-prevention/dlp-policies/logging-options/#log-the-payload-of-matched-rules).
