...
### Relationship to devices and access

Seat management controls billing, not device or network access.

[Removing a user](/cloudflare-one/team-and-resources/users/seat-management/#remove-a-user) determines whether that user consumes a billable seat, but it does not directly manage device registrations or [prevent users from accessing resources](/cloudflare-one/team-and-resources/devices/device-registration/#remove-user-access) behind Cloudflare Access.

For more information about managing device registrations, refer to [Device registration](/cloudflare-one/team-and-resources/devices/device-registration/).
...
Revoked users can still log in if your policies allow them. To prevent a user from authenticating, you must remove them from your [device enrollment permissions](/cloudflare-one/team-and-resources/devices/warp/deployment/device-enrollment/) or from your Identity Provider (IdP).
...
The user will now show as **Inactive** and will no longer occupy a seat. If a user is removed but authenticates later, they will consume a seat again. To prevent a user from authenticating, you must remove them from your [device enrollment permissions](/cloudflare-one/team-and-resources/devices/warp/deployment/device-enrollment/) or from your Identity Provider (IdP).
