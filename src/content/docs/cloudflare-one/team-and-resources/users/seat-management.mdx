...
To automate the removal of users who have not logged in or triggered a device enrollment in a specific amount of time, turn on [seat expiration](#enable-seat-expiration) or utilize [SCIM](/scim/) to remove users when they are deactivated in your Identity Provider.
