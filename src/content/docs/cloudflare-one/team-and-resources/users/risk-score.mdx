...
While the Okta integration is turned on, Cloudflare One will send any user risk score updates to Okta, including score increases and resets. Score update events will appear in your [Access authentication logs](/cloudflare-one/insights/logs/audit-logs/).
...
1. In [Cloudflare One](https://one.dash.cloudflare.com/), go to **Access** > **Applications**.
2. Select an application and go to **Policies**.
3. Create or edit a policy.
4. Add a rule with the **User Risk Score** selector.
5. Select the risk level threshold (Low, Medium, or High).
6. Save the policy.

### Block high-risk users

To prevent users with elevated risk scores from accessing sensitive applications:

1. Create an Allow policy that includes your user group.
2. Add an Exclude rule with **User Risk Score** set to **High**.
