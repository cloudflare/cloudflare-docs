...
This guide covers how to deploy the Cloudflare One Client (formerly WARP Client) using Jamf.
...
1. [Download](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#macos) the `Cloudflare_WARP.pkg` file.
...
:::note[Repeat this step to update Cloudflare One Client when a new release is available]
Every time the Cloudflare One Client releases a new version, you must repeat this process and upload a new `.pkg` file for the new version.
:::
...
3. For **Triggers**, choose the events that will trigger the deployment. We recommend selecting **Startup**, **Login**, **Enrollment Complete**, and **Recurring Check-in**.
...
5. To configure the **Property List**:
	1. [Create a `plist` file](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/#plist-file) with your desired deployment parameters.
...
Jamf will now deploy the Cloudflare One Client to targeted macOS devices. <Render file="warp/connectivity-status-check" product="cloudflare-one" />
...
Create an [XML file](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/#ios) with your custom deployment preferences.
