...
title: Register Cloudflare One Client with minimal user interaction
sidebar:
  order: 4
  label: Automated registration
---

import { Render, GlossaryTooltip, Tabs, TabItem } from "~/components";

Administrators can automate Cloudflare One Client (formerly WARP Client) registration on managed devices and minimize the number of clicks required from an end user.
...
![Browser popup requesting permission to open Cloudflare One Client](~/assets/images/cloudflare-one/connections/warp-protocol-handler.png)
...
Instead of using the Registry Editor, the registry key can also be created using a Group Policy Object (GPO), PowerShell script, or with an MDM such as [Intune](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/partners/intune/#update-mdm-parameters).

</TabItem>
<TabItem label="macOS">

On macOS, you can configure `AutoLaunchProtocolsFromOrigins` by deploying a property list (plist) file for the browser. The exact instructions will vary depending on your [MDM](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/partners/). The general procedure is as follows:
