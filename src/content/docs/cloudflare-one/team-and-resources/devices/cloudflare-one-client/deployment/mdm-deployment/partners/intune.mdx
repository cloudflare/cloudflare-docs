...
This guide covers how to deploy the Cloudflare One Client (formerly WARP Client) using Microsoft Intune.
...
1. [Download the `Cloudflare_WARP_<VERSION>.msi` installer](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#windows).
...
:::tip[Best practice]
Deploy configuration profiles (steps 1, 2, and 3) before the Cloudflare One Client application (step 4) itself. This order ensures that when the client installs, it already has the required permissions and certificates, avoiding failed installations.
:::
...
This step allows the Cloudflare One Client to install without user interaction. By completing this step, you allow the client to install and manage its required system extensions without end-user prompts.
...
1. In [Cloudflare One](https://one.dash.cloudflare.com/), [download the Cloudflare One Client](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#macos) in `.pkg` format.

   :::note[Repeat this step to update the Cloudflare One Client when a new release is available]
   Every time a new version is released, you must repeat this process and get a new `.pkg` file for the new version.
   :::
...
6. In **Requirements**, refer to the OS versions listed in [stable releases for macOS](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#macos) and find what matches for you.
7. In **Detection rules**, note that the package will have filled in the App bundle ID and App version.
...
4. Select **Search the App Store** and search for the [Cloudflare One Agent](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#ios). After you have found the Cloudflare One Agent, select it and select **Select** to continue.

	:::caution[Add the right app]
	
	Make sure to add the [Cloudflare One Agent](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#ios) application. Do not add the 1.1.1.1 app.
...
Before proceeding with per-app VPN configuration, you must make sure [Auto connect](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#auto-connect) is disabled in Zero Trust. To disable Auto connect:
