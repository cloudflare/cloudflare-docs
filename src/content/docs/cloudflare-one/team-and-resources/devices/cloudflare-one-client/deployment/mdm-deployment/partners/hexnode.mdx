...
1. Create a script file with `.bat`, `.cmd`, and `.ps1` file formats to download, install and configure the Cloudflare One Client (formerly WARP Client) Windows application on the device. Listed below is a sample script with all of the configurable parameters:
...
1. [Download](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#macos) the Cloudflare One Client for macOS.
