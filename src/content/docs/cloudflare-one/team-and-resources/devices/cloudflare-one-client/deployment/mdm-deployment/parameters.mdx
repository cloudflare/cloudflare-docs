...
Most of the parameters listed below are also configurable in Cloudflare One under **Team & Resources** > **Devices**. In the event of conflicting settings, the Cloudflare One Client (formerly WARP Client) will always give precedence to settings on the local device (for example, in your `mdm.xml` or `com.cloudflare.warp.plist` files).
...
- `true` — Enables PMTUD on the tunnel interface.
...
- `true` — Enables post-quantum key agreement for all traffic through the tunnel.

### `external_emergency_signal_fingerprint`

The SHA-256 fingerprint that the Cloudflare One Client will use to validate the [`external_emergency_signal_url`](#external_emergency_signal_url) HTTPS endpoint. Refer to [External Emergency Disconnect](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/external-disconnect) for details on how to extract this fingerprint.
...
How often the Cloudflare One Client will poll [`external_emergency_signal_url`](#external_emergency_signal_url) for an [External Emergency Disconnect](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/external-disconnect/) signal.
...
The HTTPS endpoint that the Cloudflare One Client will poll for an [External Emergency Disconnect](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/external-disconnect/) signal.
...
This functionality is intended for use with a Cloudflare China local network partner or any other third-party network partner that can maintain the integrity of network traffic. Most IT admins should not set this setting as it will redirect all client traffic to a new IP.

**Value Type:** `string`

**Value:** `203.0.113.0:500` — Redirect all client traffic to `203.0.113.0` on port `500`.
...
- `warp` — (default) [Gateway with WARP](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#gateway-with-warp-default).
- `1dot1` — [Gateway with DoH](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#gateway-with-doh).
- `proxy` — [Proxy mode](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#proxy-mode). Use the `proxy_port` parameter to specify the localhost SOCKS proxy port (between `0`-`66535`). For example,
  ```xml
  ...
  ```
- `postureonly` — [Device Information Only](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#device-information-only).
- `tunnelonly` - [Secure Web Gateway without DNS filtering](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#secure-web-gateway-without-dns-filtering).
...
Allows the user to turn off the Cloudflare One Client switch and disconnect the client.
...
Assigns a unique identifier to the device for the [device UUID posture check](/cloudflare-one/reusable-components/posture-checks/cloudflare-one-client-checks/device-uuid).
...
Configures the protocol used to route IP traffic from the device to Cloudflare Gateway. For more information, refer to [Device tunnel protocol](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#device-tunnel-protocol).
...
- `false` — (default) Only one Cloudflare One Client registration is stored per device. After a user logs in to the client, their settings and identity will apply to all traffic from the device.
...
Allows the Cloudflare One Client to connect with a service token before a user completes the initial Windows login. For more information, refer to [Connect Cloudflare One Client before Windows login](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/windows-prelogin/).
