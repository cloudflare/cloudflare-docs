...
import { Badge, Details } from "~/components";

## Availability <Badge text="Beta" variant="caution" size="small" />
...
The lockdown re-engages automatically any time the device transitions back to an unauthenticated state — for example, if the registration is deleted/revoked, another OS user that has never authenticated with the Cloudflare One Client logs in while in [multi-user mode](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/windows-multiuser/), or the user switches into a new organization.
