...
3. Select the **Windows** tab, then select **(+)**.
   ...
   ![Configuring Cloudflare One Client in the JumpCloud Windows tab](~/assets/images/cloudflare-one/connections/jumpcloud.png)
...
3. Select the **Apple** tab, then select **(+)**.
   ...
   ![Configuring Cloudflare One Client in the JumpCloud Apple tab](~/assets/images/cloudflare-one/connections/jumpcloud-mac.png)

4. In the **Software Description** field, enter a unique display name.

5. In the **Software Package URL**, enter the URL location of the `Cloudflare_WARP_<VERSION>.pkg` file. If you do not already have the installer package, [download it here](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/#macos).
