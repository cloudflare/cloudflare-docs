...
| `*-traceroute-*.txt`                                                                                | One-shot traceroute to WARP ingress IPs, edge API endpoint hostname and IPs, and DoH resolver hostname and IPs; started when [extra debug logging](#extra-debug-logging) is first enabled.                                                                                                                                                             |
...
- Pcaps on all interfaces
- QLOGs for MASQUE tunnel connections
...
- One-shot connectivity checks: These contain the same information as `connectivity.txt`, but are collected when extra debug logging is started, rather than when a warp-diag is collected.
