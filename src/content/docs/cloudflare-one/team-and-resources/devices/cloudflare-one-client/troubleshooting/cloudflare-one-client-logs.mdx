...
The Cloudflare One Client (formerly WARP Client) provides diagnostic logs that you can use to troubleshoot connectivity issues on a device.

<Stream
	id="c29964ab3dcf7c3432ebb2b4e93c3aca"
	title="Understand Cloudflare One Client diagnostic logs"
...
You can collect Cloudflare One Client diagnostic logs remotely from the Zero Trust dashboard by using Digital Experience Monitoring (DEX) [remote captures](/cloudflare-one/insights/dex/remote-captures/).
...
#### Cloudflare One Client diagnostics analyzer (beta)
...
| `alternate-networks-check.txt`                                                                      | Connectivity status for each [managed network](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/managed-networks/).                                                                                                                                                                                                          |
| `boringtun.log`                                                                                     | Log for the tunnel that serves traffic from the device to Cloudflare's global network.                                                                                                                                                                                                                                                 |
| `bound-dns-ports.txt`                                                                               | Active processes on port `53`.                                                                                                                                                                                                                                                                                                              |
| `captive-portal-hotspot-detect.txt`                                                                 | HTTP response of `captive.apple.com`                                                                                                                                                                                                                                                                                                        |
| `connectivity.txt`                                                                                  | DNS resolution and HTTP trace requests to [validate a successful connection](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/firewall/#connectivity-checks). Can be used to determine whether traffic is routing through the tunnel.                                                                                       |
...
| `dns-check.txt`                                                                                     | Verifies that the Cloudflare One Client DNS servers are set as system default. For [operating modes](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/) where DNS filtering is enabled, this file contains the IPs of the local DNS proxy (`127.0.2.2:0`, `127.0.2.3:0`, `[fd01:db8:1111::2]:0`, and `[fd01:db8:1111::3]:0`). |
...
| `firewall-rules.txt`                                                                                | The [system firewall rules](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/cloudflare-one-client-architecture/#system-firewall) configured by the Cloudflare One Client.                                                                                                                                                                           |
...
| `stats.log`                                                                                         | Uptime and throughput stats for the tunnel, generated every two minutes.                                                                                                                                                                                                                                                               |
...
| `traceroute.txt`                                                                                    | Traceroute to the [ingress IPs](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/firewall/#warp-ingress-ip) showing the path from the device to Cloudflare's global network.                                                                                                                                                |
...
| `version.txt`                                                                                       | [Cloudflare One Client version](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/) installed on the device.                                                                                                                                                                                                                             |
...
| `warp-device-posture.txt`                                                                           | Current [device posture](/cloudflare-one/reusable-components/posture-checks/cloudflare-one-client-checks/) status.                                                                                                                                                                                                      |
| `warp-dex-data.txt`                                                                                 | Currently configured [DEX tests](/cloudflare-one/insights/dex/tests/) and their most recent statuses.                                                                                                                                                                                                                                       |
| `warp-dns-fallbacks.txt`                                                                            | List of default DNS fallbacks used by the Cloudflare One Client DNS proxy.                                                                                                                                                                                                                                                   |
...
| `warp-settings.txt`                                                                                 | [Cloudflare One Client settings](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/) applied to the device.                                                                                                                                                                                                               |
| `warp-stats.txt`                                                                                    | Uptime and throughput of the tunnel since `stats.log` was generated.                                                                                                                                                                                                                                   |
| `warp-status.txt`                                                                                   | Status of the Cloudflare One Client switch (`Connected` or `Disconnected`).                                                                                                                                                                                                                                                      |
...
Each Cloudflare One Client log file (such as `connection_stats.log`, `dns_stats.log`, `daemon.log`, `boringtun.log`, `daemon_dns.log`, `dex.log` and `captive-portal.log`) is limited by size and age:
...
On Linux, the Cloudflare One Client daemon logs are written to the system logs by the `warp-svc` service.

If you need to adjust the maximum log level or filter what gets logged, you can configure the `systemd` unit file using the [`LogLevelMax=` option](https://www.freedesktop.org/software/systemd/man/latest/systemd.exec.html#LogLevelMax=).
...
To view Cloudflare One Client logs on mobile devices:
