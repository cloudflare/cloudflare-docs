...
This guide helps you diagnose and resolve common issues with the Cloudflare One Client (formerly WARP Client). It covers how to troubleshoot the Cloudflare One Client on desktop operating systems, including Windows, macOS, and Linux.
...
[**Diagnostics Analyzer**](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/cloudflare-one-client-logs/#diagnostics-analyzer-beta) - Uses AI to parse a device's diagnostic log and summarizes key events, likely causes, and recommended next steps in a concise summary. This analyzer is available for logs collected via the dashboard.
...
1. Open the Cloudflare One Client GUI on the desktop.
2. Select the gear icon.
3. Select **About**.
4. Compare your device's version with the [latest version](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/download-cloudflare-one-client/).
...
To troubleshoot effectively, Cloudflare recommends reproducing the issue and noting your timestamps immediately before collecting logs. Though recreating the issue may not be possible in all cases, reproducing the issue right before diagnostic collection or during the window that a packet capture (PCAP) is running will help you troubleshoot with greater visibility.

Refer to [log retention window](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/cloudflare-one-client-logs/#log-retention-window) to learn more.
:::
...
The [Diagnostics Analyzer](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/cloudflare-one-client-logs/#diagnostics-analyzer-beta) uses AI to parse a device's diagnostic log and summarizes key events, likely causes, and recommended next steps in a concise summary.

After you run a [DEX remote capture](#option-a-collect-logs-via-the-cloudflare-dashboard) for diagnostics:

1. Go to **Insights** > **Digital experience** and select the **Diagnotics** tab.
2. Find your capture in the list of captures.
3. Select the three-dot icon next to **Status** > select **View Diag** to generate an AI summary.
...
Collect diagnostic logs on your desktop using the `warp-diag` CLI.
...
Diagnostic logs capture the final Cloudflare One Client configuration and status on a device after all MDM policies and other software settings have been applied. Reviewing these logs can help you identify misconfigurations or unexpected behavior.

<Stream
        id="c29964ab3dcf7c3432ebb2b4e93c3aca"
        title="Cloudflare One Client diagnostic logs"
...
Open the `warp-status.txt` file to review the status of the client connection when the `warp-diag` was collected. A connected Cloudflare One Client will appear as:

```
Ok(Connected)
```

If the Cloudflare One Client is experiencing issues, the error will display in the client GUI on the device. Use the [Client errors](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/client-errors/) documentation to identify your error, its cause, and the solution.

### Check client settings

After you have checked client status, review the client's settings on the device to check if the expected configuration has been applied. Open the `warp-settings.txt` file to review the Cloudflare One Client settings. You will check the device's applied device profile and split tunnel configuration.

#### Example `warp-settings.txt` file

Find the diagnostic logs on your desktop, and open the `warp-settings.txt` file. Review the following example `warp-settings.txt` file and the descriptions of its content below.

```txt
Merged configuration:
(derived)   Always On: true
(network policy)    Switch Locked: false # If false, does not allow the user to turn off the client toggle and disconnect the Cloudflare One Client
(network policy)    Mode: WarpWithDnsOverHttps # The device's client mode, this mode is Gateway with WARP mode
...
Refers to the current state of the client toggle in the GUI. In the example file, the client toggle is switched on.
...
Refers to the [Lock Client Switch](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#lock-cloudflare-one-client-switch) which allows the user to turn off the client switch and disconnect the client. In the example file, the value is `false` meaning the user is able to turn the client switch on or off at their discretion.

```txt
Switch Locked: false
```

When the Lock Client switch is enabled (`true`), users will need an [admin override code](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#allow-admin-override-codes) to temporarily turn off the Cloudflare One Client on their device.

##### Mode

Refers to the [client mode](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/) the device is using. In the example file, the client mode is `WarpWithDnsOverHttps` which is Gateway with WARP mode. Refer to the [client modes comparison matrix](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/) to match your `warp-settings.txt` file's value with the mode name.
...
Refers to your [split tunnel](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/split-tunnels/) settings. In the example file, the client is running in Exclude mode, meaning all traffic except for the traffic destined for these hosts and IPs will be sent through the tunnel. The host `cname.user.net` and the IP `1xx.1xx.1xx.1xx/25 ` are both excluded from the tunnel.
...
:::note[Exclude mode versus Include mode]
	`Exclude mode` means all traffic will be sent through the tunnel except for the IPs and domains you specify.

	`Include mode` means only traffic destined to the IPs or domains you specify will be sent through the tunnel.
:::

##### Fallback domains

Refers to your [Local Domain Fallback](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/local-domains/) settings. In the example file, the client lists `intranet` as a domain that will not be sent to Gateway for processing and will instead be sent directly to the configured fallback servers.
...
Refers to the [Mode switch](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#mode-switch) setting. In the example file, the mode switch is enabled (`true`) which means the user has the option to switch between [Gateway with WARP](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#gateway-with-warp-default) mode and [Gateway with DNS-over-HTTPS (DoH)](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#gateway-with-doh) mode.
...
Refers to the [Allow users to enable local network exclusion](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#allow-users-to-enable-local-network-exclusion) setting. When enabled, it allows users to temporarily access local devices (like printers) by excluding the detected local subnet from the tunnel. This example indicates access is allowed until the next client reconnection, and only for subnets up to `/24`.
...
Refers to the [Device profile](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/) a device is using. In this example, the ID is `000000x1-00x1-1xx0-1xx1-11101x1axx11`.
...
A profile ID is a unique identifier assigned to each [device profile](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/) in the Cloudflare One dashboard, used to determine which configuration settings apply to a device.
...
:::caution
Avoid [reordering profiles](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/#order-of-precedence) unless you are confident it will not affect other users.
:::

#### Review your managed network settings

A [managed network](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/managed-networks/) is a network location that you define with a TLS endpoint, like a physical office. The Cloudflare One Client checks for this TLS endpoint to determine its location and apply the corresponding device profile.
...
1. Verify the endpoint is reachable.
	...
	The Cloudflare One Client connects to the TLS endpoint to identify the network. If the endpoint is down or unreachable, the client will fail to detect the network and apply the wrong profile.
...
A misconfigured [split tunnel](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/split-tunnels/) can cause connectivity issues.

For example, if you set your mode to Exclude IPs and domains and accidentally exclude an IP address needed by an application, that application may not work correctly. Similarly, in Include IPs and domains mode, forgetting to include a necessary IP or domain will cause traffic to bypass the tunnel, and you will lose access to your Zero Trust security features.
...
If your dashboard split tunnel configuration does not match your `warp-settings.txt` file configuration, you may need to force the Cloudflare One Client to [update its settings](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/troubleshooting-guide/#update-the-clients-settings).

#### 2. Update the client's settings
...
This can be done by instructing the end user to [toggle the client off and on](#option-a-toggle-the-client-off-and-back-on), or [reset their encryption keys](#option-b-reset-the-encryption-keys).

Both methods update the client with the latest configuration.

##### Option A: Toggle the client off and back on

On the end user device, open the Cloudflare One Client GUI and toggle the client on and off.

:::note[What if the end user cannot turn off the client?]
If the end user's [client switch](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#lock-cloudflare-one-client-switch) is locked, they will need an [admin override code](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#allow-admin-override-codes) to be able to toggle the client switch.

[Resetting the encryption keys](#option-b-reset-the-encryption-keys) may be a faster solution.
:::

After you toggle the client back on, the Cloudflare One Client will fetch new settings when it reconnects.

##### Option B: Reset the encryption keys

To reset the encryption keys on an end user's desktop:

1. Open the Cloudflare One Client GUI.
2. Select the gear icon.
3. Select **Preferences** > **Connection** > **Reset encryption keys**.

Resetting the encryption keys forces the Cloudflare One Client to reestablish its tunnel and retrieve the latest configuration.
