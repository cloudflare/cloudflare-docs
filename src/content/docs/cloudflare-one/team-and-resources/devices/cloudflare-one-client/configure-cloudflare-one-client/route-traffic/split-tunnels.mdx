...
Use Split Tunnels when you need to bypass Gateway entirely for a site or allow traffic through the [firewall that the Cloudflare One Client creates](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/cloudflare-one-client-architecture/#system-firewall). Common scenarios include:

- Connect to a third-party application which requires the actual IP address of the end-user device (for example, [Microsoft 365](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#directly-route-microsoft-365-traffic)).
...
Many Cloudflare Zero Trust services rely on traffic going through the Cloudflare One Client, such as [device posture checks](/cloudflare-one/reusable-components/posture-checks/) and [client session durations](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-sessions/). If you are using Split Tunnels in Include mode, you will need to manually add Cloudflare Zero Trust domains and IPs in order for these features to function.
...
In [Secure Web Gateway without DNS filtering](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-modes/#secure-web-gateway-without-dns-filtering) mode, you cannot [add domains](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/split-tunnels/#cloudflare-zero-trust-domains) to Split Tunnels. If you are using Split Tunnels in Include mode, you must include the IPs that resolve to `<your-team-name>.cloudflareaccess.com` instead:
...
- Routes excluded or included from the Cloudflare One Client and Gateway visibility may change day to day, and may be different for each user depending on where they are.
...
2. Under **Profiles**, locate the [device profile](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/) you would like to modify and select **Edit**.
