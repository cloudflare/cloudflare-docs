...
The External Emergency Disconnect feature allows organizations to remotely disconnect and reconnect their fleet of Cloudflare One Clients (formerly WARP Clients) using their own infrastructure, independent of Cloudflare's infrastructure. For example, in the event of a [Cloudflare network outage](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/external-disconnect/#use-cases) you ensure that you can still manage your devices even if Cloudflare’s systems are down or unreachable.
...
You can use External Emergency Disconnect in combination with the dashboard-initiated [Disconnect Cloudflare One Client on all devices](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#disconnect-cloudflare-one-client-on-all-devices) setting. A disconnect signal retrieved from the external endpoint will [take precedence](#client-settings-precedence).
...
- **Security Incident Response**: Provides the ability to quickly terminate all Cloudflare One Client tunnels across the entire fleet.
...
- **Disaster Recovery**: If devices running the Cloudflare One Client cannot reach Cloudflare's API (due to a network outage, routing issue, or client-side misconfiguration), administrators retain the ability to force-disconnect the fleet via the customer-hosted endpoint.
...
The Cloudflare One Client will automatically exclude the external endpoint IP address from the tunnel. If a device profile uses [Split Tunnels](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/split-tunnels/) in **Include** mode, ensure that your Split Tunnel entries do not contain the external endpoint IP; otherwise the client will exclude the entire Split Tunnel entry from the tunnel.
:::
...
The Cloudflare One Client will automatically disconnect within the configured polling interval, and the client GUI will display [`Admin directed disconnect`](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/client-errors/#admin-directed-disconnect). To reconnect all devices, change `emergency_disconnect` back to `false`.
...
The current status is also available in [Cloudflare One Client diagnostic logs](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/cloudflare-one-client-logs/#macoswindowslinux) in `warp-settings.txt`.
...
Alternatively, users can switch to a [different MDM configuration](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/mdm-deployment/switch-organizations/) that does not have the feature configured.
...
This command will clear the client registration, clear the local policy, and discard the cached emergency state. To reconnect, you will need to [turn off External Emergency Disconnect](#clear-external-emergency-disconnect-state) and then [re-enroll the Cloudflare One Client](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/deployment/manual-deployment/) with your Zero Trust organization.

## Client settings precedence

Learn how global disconnect settings interact and how they impact other client profile settings.

### Global disconnection settings

The client will honor disconnect signals from both the Cloudflare dashboard (via [Disconnect Cloudflare One Client on all devices](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#disconnect-cloudflare-one-client-on-all-devices)) and the external endpoint. A global disconnect is enforced if either source triggers it.

| | [Disconnect Cloudflare One Client on all devices](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#disconnect-cloudflare-one-client-on-all-devices) is `On` | [Disconnect Cloudflare One Client on all devices](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#disconnect-cloudflare-one-client-on-all-devices) is `Off` |
| --- | --- | --- |
...
[Auto connect](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#auto-connect) does not apply while a global disconnect is in effect.

### Lock client switch

[Lock client switch](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/#lock-cloudflare-one-client-switch) does not apply while a global disconnect is in effect. Users will be unable to turn on the Cloudflare One Client unless they have an [admin override code](#admin-override).
