...
The Cloudflare One Client (formerly WARP Client) allows you to selectively apply specific [device profiles](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/) and client settings when a device connects to a secure network location, such as an office. The client identifies these managed networks by detecting a TLS endpoint you set up on the network.
...
If the TLS endpoint times out after 5 seconds, the Cloudflare One Client will determine that the device is not on a managed network and will apply the default device profile. The client only retries detection if a non-timeout error occurs. A timeout triggers fallback to the default device profile without further retries.
...
The Cloudflare One Client will automatically exclude the TLS endpoint from all device profiles if it is specified as a private IP address. This exclusion prevents remote users from accessing the endpoint through the tunnel on any port. If the TLS endpoint is specified as a hostname instead of a private IP, the client will not automatically exclude it.

:::note[Split Tunnels in Include mode]
If a device profile uses [Split Tunnels](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/split-tunnels/) in **Include** mode, ensure that your Split Tunnel entries do not contain the TLS endpoint IP address; otherwise the client will exclude the entire Split Tunnel entry from the tunnel. For example, if you are currently including `10.0.0.0/8` but your TLS endpoint is on `10.0.0.1`, use our [IP subtraction calculator](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/route-traffic/split-tunnels/#add-a-route) to remove `10.0.0.1` from `10.0.0.0/8`.
:::
...
2. Under **Profiles**, create a new [profile](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/) or edit an existing profile.
...
1. Turn on the Cloudflare One Client.
2. Disconnect and reconnect to the network.
3. Open a terminal and run `warp-cli debug alternate-network`.

## Related resources

- [Device profiles](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/device-profiles/) - How to create and manage the device profiles you apply via managed networks.
- [Client settings](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure-cloudflare-one-client/cloudflare-one-client-settings/) - Defines how the Cloudflare One Client behaves and what users can do.
- [Cloudflare One Client troubleshooting guide](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/troubleshooting/troubleshooting-guide/) - Troubleshoot common Cloudflare One Client issues.
