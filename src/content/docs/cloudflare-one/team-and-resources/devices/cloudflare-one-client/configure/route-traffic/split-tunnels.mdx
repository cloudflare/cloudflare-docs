...
Cloudflare One Client versions `2026.8.1350.1` and above attempt to maintain name-based Split Tunnel rule results across tunnel connections. Earlier versions did not maintain these results, so access to these domains was not maintained while the client was disconnected or reconnecting. Before the first tunnel connection, however, the client firewall blocks these domains. This is because the client has not yet queried which IP addresses are associated with each domain and therefore defaults to blocking this traffic to avoid unintended leaks.
