...
:::note
Some applications require the use of a publicly trusted certificate — they do not trust the system certificate, nor do they have a configurable private store. For these applications to function, you must add a [Do Not Inspect policy](/cloudflare-one/traffic-controls/http-policies/#do-not-inspect) for the domains or IPs that the application relies on.
:::
