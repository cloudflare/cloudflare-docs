...
If your users need to connect to self-signed origin servers, create an HTTP Allow policy for the origin server with the [untrusted certificate action](/cloudflare-one/traffic-policies/http-policies/#untrusted-certificates) set to _Pass through_.
:::
...
1. In [Cloudflare One](https://dash.cloudflare.com/one/), go to **Traffic policies** > **Traffic settings** > **Certificates**.
