...
If your users need to connect to self-signed origin servers, create an HTTP Allow policy for the origin server with the [untrusted certificate action](/cloudflare-one/traffic-policies/policy-types/http-policies/actions/#untrusted-certificates) set to _Pass through_.
:::
