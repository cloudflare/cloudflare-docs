...
5. In the [Microsoft Intune admin center](https://intune.microsoft.com), go to **Devices** > **iOS/iPadOS**.

6. Under **Manage devices**, select **Configuration**.

7. Select **Create** > **New policy**.

8. For **Profile type**, select *Templates* > **Trusted certificate** > **Create**.

9. In **Basics**, fill in the required fields and give your policy a name, such as `Cloudflare Gateway TLS certificate` > **Next**.

11. Upload the `.crt` file you downloaded (convert to `.cer` if Intune requests it) > **Next**.

12. Under **Assignments**, select the appropriate device or user groups > **Next**.

13. Review the configuration and select **Create**.

By completing this step, you distribute the Cloudflare Gateway root certificate to managed iOS devices so that TLS-decrypted traffic is trusted.
