...
You can use Intune to update [MDM parameters](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/parameters/) for the Cloudflare One Client. On Windows, these parameters are stored on the local device in `C:\ProgramData\Cloudflare\mdm.xml`.
...
1. In [Cloudflare One](https://one.dash.cloudflare.com/), [download the Cloudflare One Client](/cloudflare-one/team-and-resources/devices/warp/download-warp/#macos) in `.pkg` format.
...
By completing this step, you preconfigure the Cloudflare One Agent with your [Zero Trust organization](/cloudflare-one/setup/#create-a-zero-trust-organization) and connection settings so that enrolled iOS devices automatically apply a consistent Cloudflare One Client configuration when the app installs.
