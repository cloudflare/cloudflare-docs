...
:::note
The service token must have _Service Auth_ [device enrollment permissions](/cloudflare-one/team-and-resources/devices/warp/deployment/device-enrollment/#check-for-service-token). Allow permissions will not work for service tokens.
:::
...
Identifies a Zero Trust organization in the Cloudflare One Client GUI when the client is deployed with [multiple organizations](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/switch-organizations/). Required if the `organization` parameter is specified within a [`configs` array](#configs).
...
[Path MTU Discovery (PMTUD)](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/path-mtu-discovery/) allows the Cloudflare One Client to discover the largest packet size that can be sent over the current network and optimize connection performance.
...
- `fedramp_high` — The Cloudflare One Client connects to FedRAMP-specific API endpoints, IPs, and domains (like `<ACCOUNT_ID>.fed.cloudflare-gateway.com`). Traffic is forwarded to FedRAMP High compliant data centers for processing. To configure the FedRAMP High environment, you must allow the [FedRAMP-specific endpoints, IPs, and domains](/cloudflare-one/team-and-resources/devices/warp/deployment/firewall/) through your firewall.

When using [multiple configurations](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/switch-organizations/) for the same organization, all configurations must specify the same `environment` value. A single organization cannot operate in both the normal and FedRAMP High environments. For example, if your FedRAMP High organization has multiple MDM configurations (such as production and staging), each configuration for that organization must include `environment` set to `fedramp_high`:
...
The SHA-256 fingerprint that the Cloudflare One Client will use to validate the [`external_emergency_signal_url`](#external_emergency_signal_url) HTTPS endpoint. Refer to [External Emergency Disconnect](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/external-disconnect) for details on how to extract this fingerprint.
...
How often the Cloudflare One Client will poll [`external_emergency_signal_url`](#external_emergency_signal_url) for an [External Emergency Disconnect](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/external-disconnect/) signal.
...
The HTTPS endpoint that the Cloudflare One Client will poll for an [External Emergency Disconnect](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/external-disconnect/) signal.
...
Overrides the [IP address](/cloudflare-one/team-and-resources/devices/warp/deployment/firewall/#doh-ip) used by the Cloudflare One Client to resolve DNS queries via DNS over HTTPS (DoH). If you set this parameter, be sure to update your organization's firewall to ensure the new IP is allowed through.
...
- `warp` — (default) [Traffic and DNS mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/#traffic-and-dns-mode-default).
- `1dot1` — [DNS only mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/#dns-only-mode).
- `proxy` — [Local proxy mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/#local-proxy-mode). Use the `proxy_port` parameter to specify the localhost SOCKS proxy port (between `0`-`66535`). For example,
  ```xml
  ...
  ```
- `postureonly` — [Posture only mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/#posture-only-mode).
- `tunnelonly` - [Traffic only mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/#traffic-only-mode).
...
- `true` — Each Windows user has their own Cloudflare One Client registration. For more information, refer to [Multiple users on a Windows device](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/windows-multiuser/).

### `pre_login`

Allows the Cloudflare One Client to connect with a service token before a user completes the initial Windows login. For more information, refer to [Connect the Cloudflare One Client before Windows login](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/windows-prelogin/).
