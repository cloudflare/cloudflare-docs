...
|--------|---------------|----------------------|-------------|
| Routing all traffic | Full tunnel | Exclude IPs and domains | WARP asserts precedence over the default route. All traffic goes through WARP tunnel except specified exclusions. |
| Routing specific traffic | Split tunnel | Include IPs and domains | WARP does not assert precedence over the default route. Only traffic explicitly defined in the Include list enters the WARP tunnel. |

Second draft:

| Concept | Cisco AnyConnect term | Cloudflare WARP term | WARP behavior |
|--------|----------------------|----------------------|---------------|
| Route all traffic | Tunnel All Networks | Not applicable | WARP does not provide a full tunnel configuration option like Cisco. Instead, users configure WARP to Exclude IPs and domains to route all traffic through the WARP tunnel. |
| Route all traffic except specific networks | Exclude Network List Below | Exclude IPs and domains | WARP asserts precedence over the system default route. All traffic is sent through the WARP tunnel except destinations explicitly excluded via split tunnel exclusions. |
| Route only specific networks | Tunnel Network List Below | Include IPs and domains | WARP does not assert precedence over the default route. Only traffic matching the Include list is sent through the WARP tunnel; all other traffic follows the system default route. |
...
More importantly, `tunnelall` mode cannot operate alongside Cloudflare WARP. WARP relies on explicit Include or Exclude routing rules to determine how traffic is handled, and an unconditional full-tunnel configuration would conflict with WARP's routing model.

As a result, environments using `tunnelall` must transition to explicit Include or Exclude routing when deploying Cloudflare WARP.
:::
...
First draft (to be deleted): 

| Cloudflare WARP | Cisco AnyConnect | Compatibility |
|------------------------------|-------------------------|---------------|
| Include IPs and domains | Exclude Network List Below <br /> `excludespecified` | ✅ Supported |
| Include IPs and domains | Tunnel Network List Below <br /> `tunnelspecified` | ✅ Supported |
| Include IPs and domains | Tunnel All Networks <br /> `tunnelall` | ❌ Not supported |
| Exclude IPs and domains | Exclude Network List Below <br /> `excludespecified` | ❌ Not supported |
| Exclude IPs and domains | Tunnel Network List Below <br /> `tunnelspecified` | ✅ Supported |
| Exclude IPs and domains | Tunnel All Networks <br /> `tunnelall` | ❌ Not supported |
...
Running WARP in `Exclude IPs and domains` (essentially, a full tunnel) and Cisco AnyConnect as a full tunnel at the same time will lead to a tunnel-in-a-tunnel conflict.

A tunnel-in-a-tunnel conflict occurs when the destination of one tunnel is configured in the other tunnel. For example, Cisco AnyConnect is configured for a full tunnel and is not configured to exclude the [WARP ingress IPs](/cloudflare-one/team-and-resources/devices/warp/deployment/firewall/#warp-ingress-ip). Similarly, WARP is configured to `Exclude IPs and domains` but is not configured to exclude the public IP address of the Cisco AnyConnect firewall. 
...
flowchart TD
    ...
    subgraph ST[" "]
        ...
        J["Set WARP tunnel to **Include IPs and domains** (split tunnel)"]
        K["Set WARP tunnel to **Exclude IPs and domains** (full tunnel)"]
        L["Set WARP tunnel to **Include IPs and domains** (split tunnel)"]
    end
