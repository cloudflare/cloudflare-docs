...
This page lists the error codes that can appear in the Cloudflare One Client (formerly WARP) GUI. If you do not see your error below, refer to [common issues](/cloudflare-one/team-and-resources/devices/warp/troubleshooting/common-issues/) or [contact Cloudflare Support](/support/contacting-cloudflare-support/).
...
2. If this does not resolve the issue, allow users to manually [disconnect](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#lock-warp-switch). We recommend setting an [auto connect](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#auto-connect) value so that the client turns itself back on after a few minutes.
...
The initial [connectivity check](/cloudflare-one/team-and-resources/devices/warp/deployment/firewall/#connectivity-checks) failed for an unknown reason. Refer to [Unable to connect the Cloudflare One Client](/cloudflare-one/team-and-resources/devices/warp/troubleshooting/common-issues/#unable-to-connect-warp) for the most common reasons why this error occurs.

### Resolution

1. Retrieve [client diagnostic logs](/cloudflare-one/team-and-resources/devices/warp/troubleshooting/warp-logs/) for the device.
2. Follow the troubleshooting steps in [Unable to connect the Cloudflare One Client](/cloudflare-one/team-and-resources/devices/warp/troubleshooting/common-issues/#unable-to-connect-warp).
...
The Cloudflare One Client was unable to resolve hostnames via its [local DNS proxy](/cloudflare-one/team-and-resources/devices/warp/configure-warp/route-traffic/warp-architecture/#dns-traffic).
...
- Unable to connect the Cloudflare One Client in a [mode that enables DNS filtering](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/).

### Cause

A third-party process (usually a third-party DNS software) is bound to port `53`, which is used by the Cloudflare One Client's [local DNS proxy](/cloudflare-one/team-and-resources/devices/warp/configure-warp/route-traffic/warp-architecture/#dns-traffic) to perform DNS resolution. The name of third-party process will appear in the GUI error message.
...
2. Alternatively, switch the Cloudflare One Client to [Traffic only mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/#traffic-only-mode) mode.
...
The device failed to present a [valid mTLS certificate](/cloudflare-one/team-and-resources/devices/warp/deployment/device-enrollment/#check-for-mtls-certificate) during device enrollment.
...
The [connectivity check](/cloudflare-one/team-and-resources/devices/warp/deployment/firewall/#connectivity-checks) inside of the WARP tunnel has failed.
...
2. In the third-party software, bypass inspection for all IP traffic going through the Cloudflare One Client. To find out what traffic routes through the WARP tunnel, refer to [Split Tunnels](/cloudflare-one/team-and-resources/devices/warp/configure-warp/route-traffic/split-tunnels/).
...
1. Ensure that your device meets the [HD space requirements](/cloudflare-one/team-and-resources/devices/warp/download-warp/) for the Cloudflare One Client.
...
1. Ensure that your device meets the [minimum memory requirements](/cloudflare-one/team-and-resources/devices/warp/download-warp/) for the Cloudflare One Client.
...
1. Review the [managed deployment guide](/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/) for your operating system.
...
5.  If this does not resolve the error, select **Logout** and then [re-enroll your device](/cloudflare-one/team-and-resources/devices/warp/deployment/manual-deployment/). Logging out is only possible if [Allow device to leave organization](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#allow-device-to-leave-organization) is enabled for your device.
...
A third-party security product on the device or network is performing TLS decryption on HTTPS traffic. For more information, refer to the [Troubleshooting guide](/cloudflare-one/team-and-resources/devices/warp/troubleshooting/common-issues/#a-third-party-security-product-is-interfering-with-gateway).

### Resolution

In the third-party security product, disable HTTPS inspection and TLS decryption for the [WARP IP addresses](/cloudflare-one/team-and-resources/devices/warp/deployment/firewall/).
...
The account administrator must turn off both of the following features: - [Disconnect WARP on all devices](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#disconnect-warp-on-all-devices) - [Manage device connection using an external signal](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-settings/#manage-device-connection-using-an-external-signal)
