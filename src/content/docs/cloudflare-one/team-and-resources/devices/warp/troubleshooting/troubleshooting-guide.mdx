...
A profile ID is a unique identifier assigned to each [device profile](/cloudflare-one/team-and-resources/devices/warp/configure-warp/device-profiles/) in the Zero Trust dashboard, used to determine which configuration settings apply to a device.
