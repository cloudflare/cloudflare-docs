...
Refers to the [WARP mode](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/) the device is using. In the example file, the WARP mode is `WarpWithDnsOverHttps` which is Traffic and DNS mode mode. Refer to the [WARP modes comparison matrix](/cloudflare-one/team-and-resources/devices/warp/configure-warp/warp-modes/) to match your `warp-settings.txt` file's value with the mode name.
