...
For Dell devices running firmware version `1.35.0` (released 7/7/2025), regardless of operating system, Cloudflare has confirmed a bug that prevents the WARP service from starting. Cloudflare recommends users experiencing these issues upgrade their Dell device firmware to version `1.36.0` or later.
