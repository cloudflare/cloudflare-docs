...
- **Unified security policies everywhere**: With the WARP client deployed in the Gateway with WARP mode, [Gateway policies](/cloudflare-one/traffic-policies/) are not location-dependent — they can be enforced anywhere.
...
- Explore [Cloudflare Gateway](/cloudflare-one/traffic-policies/) to enforce advanced DNS, network, HTTP, and egress policies with WARP.
