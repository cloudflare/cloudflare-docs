...
- Use HTTPS
- Use an IPv4 or IPv6 address as the host. A public IP is recommended to ensure that devices can fetch the latest state regardless of their network location.
...
2. Ensure that **Manage device connection using an external signal** is [turned on](#3-turn-on-external-emergency-disconnect).
