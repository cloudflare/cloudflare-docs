...
If your organization already uses the default IPv4 range for internal networking, or if you require predictable IP assignments for firewall policy management, you can configure custom WARP IPv4 subnets. You can assign different IPv4 subnets to devices based on the user's identity.
...
4. Create rules to define the users or devices that will receive these IPs. Learn more about the available [Selectors](#selectors), [Operators](/cloudflare-one/traffic-policies/network-policies/#comparison-operators), and [Values](/cloudflare-one/traffic-policies/network-policies/#value).
...
not in profile settings. must check network interface via ifconfig, etc..

## View subnet usage

You can monitor the consumption of your IPv4 subnets to ensure you have enough addresses for new device registrations.
...
The Usage column displays the total number of IPs currently assigned to active device registrations versus the total capacity of the CIDR block.

### Via the API

use API to see which devices are assigned to the subnet, including revoked devices.

For a granular list of which specific IPs are "taken" (including those held by revoked or inactive devices), use the Cloudflare API.

This is particularly useful for identifying "ghost" registrations that may be consuming IP space despite the device no longer being in use.
