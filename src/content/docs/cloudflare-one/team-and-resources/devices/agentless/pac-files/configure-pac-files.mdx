...
Authorization endpoints support being associated with [Cloudflare Access](/cloudflare-one/access-controls/policies/) to provide full authorization capabilities. Authorization endpoints give you a comprehendive control of access policies to authorize traffic before it is sent for processing.

Authorization endpoints offer a more flexible way to authorize traffic by leveraging Cloudflare Access. Authorization endpoint support associating a user identity with the connection.
