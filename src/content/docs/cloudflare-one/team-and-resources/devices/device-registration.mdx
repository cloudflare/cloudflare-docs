...
## Checklist: offboard users and devices

```mermaid
flowchart TD
    Q{"What do you want to do?"}

    Q --> O1["Offboard a user (free a billable seat and prevent access)"]
    Q --> O2["Delete a device from Cloudflare"]

    %% Offboard user path
    O1 --> Q1{"Did you prevent the user from authenticating via device enrollment policies or IdP?"}
    Q1 -- No --> A1["Remove the user from device enrollment policies or from the Identity Provider (IdP)"]
    Q1 -- Yes --> Q2

    A1 --> Q2{"Did you delete the user's device registrations?"}
    Q2 -- No --> A2["Teams & Resources > Devices: delete the user's device registrations"]
    Q2 -- Yes --> Q3

    A2 --> Q3{"Did you remove the user from the Zero Trust organization to stop seat consumption?"}
    Q3 -- No --> A3["Teams & Resources > Users: remove the user"]
    Q3 -- Yes --> END1

    A3 --> END1(["User successfully offboarded"])

    %% Delete device path
    O2 --> Q4{"Did you delete the device or all associated device registrations?"}
    Q4 -- No --> A4["Teams & Resources > Devices: select device and delete it or delete all registrations"]
    Q4 -- Yes --> Q5

    A4 --> Q5{"Can the user or service token still authenticate?"}
    Q5 -- Yes --> A5["Remove user access or service token access from enrollment policies or IdP"]
    Q5 -- No --> END2

    A5 --> END2(["Device successfully removed"])
```