...
:::note
SAML assertion encryption is separate from [signed AuthN requests](/cloudflare-one/integrations/identity-providers/signed_authn/). Signing verifies the authenticity of SAML messages, while encryption adds an additional layer of security by protecting assertion contents. You can use both features together.
:::
...
| Key transport | RSA-OAEP, RSA-1.5 |

#### Rotate encryption certificates

You can rotate your SAML encryption certificate at any time without downtime. When you rotate, the current certificate becomes the previous certificate and a new certificate is generated. The previous certificate remains valid until its expiration date, giving you time to update your IdP configuration.
...
Your IdP can also retrieve the encryption certificate automatically from the [SAML metadata endpoint](#optional-upload-saml-metadata).
...
SAML attributes are added to the [Access JWT](/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/). These attributes can then be consumed by self-hosted or SaaS applications connected to Access. Any SAML attribute configured in the SAML integration must also be sent from the IdP.
