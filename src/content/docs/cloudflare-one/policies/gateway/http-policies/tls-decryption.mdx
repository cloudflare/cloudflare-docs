...
TLS traffic on-ramped to Gateway via WARP will be FedRAMP compliant because Gateway's TLS decryption forces traffic to egress from a Cloudflare data center within Cloudflare's FedRAMP boundary.
...
flowchart LR
 ...
 subgraph s3["Private internal network"]
    ...
    n1(["User"]) -- Gateway TLS connection wrapped with WARP TLS --> n2
    n2 --> n3
    n3 --> n4(["HTTPS server"])
    n5 --> n3 & n6
