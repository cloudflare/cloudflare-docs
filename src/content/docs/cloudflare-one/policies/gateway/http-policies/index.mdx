...
:::note
To use HTTP policies, install the <a href="/cloudflare-one/connections/connect-devices/warp/user-side-certificates/">Cloudflare root certificate</a>.
:::
...
The Quarantine action sends files in matching requests to a file sandbox to scan for malware. Gateway will only send files not previously seen to the sandbox. For more information on this action, refer to [File sandboxing](/cloudflare-one/policies/gateway/http-policies/file-sandboxing/).

#### Sandbox file types

Quarantine policies require you to select at least one file type to sandbox after download.
