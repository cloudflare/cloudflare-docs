...
Gateway will not apply egress policies with these selectors to DNS queries sent outside of the supported on-ramps. Unsupported traffic will be resolved with your default settings. Gateway will only overwrite the DNS response when the query matches a condition in the egress policy. call out we won't return this DNS location on-ramps
