...
Gateway allows you to send copies of entire HTTP requests matched in HTTP Allow and Block policies to storage destinations configured in [Logpush](/logs/about/), including third-party destinations. Forensic copies include un-altered payloads and headers which may include sensitive data. Logpush logs are encrypted in transit only (e.g., via TLS) and further encryption depends on your storage destination's policies. 
