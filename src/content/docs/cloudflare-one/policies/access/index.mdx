...
| Identity provider group  | Checks the user groups configured with your identity provider (IdP). This selector only displays if you use Microsoft Entra ID, GitHub, Google, or Okta as your IdP, or if you provision the IdP groups using [SCIM](/cloudflare-one/identity/users/scim/).      | ✅               | ❌                               |
