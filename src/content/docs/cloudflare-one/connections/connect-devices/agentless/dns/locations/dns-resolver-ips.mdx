...
Enterprise users can request a dedicated DNS resolver IPv4 address to be provisioned for a DNS location instead of the default Anycast addresses. Like IPv6, queries forwarded to that address will be identified using the dedicated DNS resolver IPv4 address.
