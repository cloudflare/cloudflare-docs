...
Captive portal logs are used by Cloudflare Support to troubleshoot WARP captive portal issues. When an end user reports an issue with a captive portal, administrators can ask the user to collect captive portal logs and then attach the logs to a Cloudflare Support ticket.
...
Once the diagnostic finishes running, WARP will place a `warp-captive-portal-diag-<date>-<time>.zip` file on the user's Desktop. The user can now share this file with their administrator.
