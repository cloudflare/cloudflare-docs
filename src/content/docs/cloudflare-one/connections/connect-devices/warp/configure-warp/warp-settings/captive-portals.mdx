...
Captive portal logs make it easier for Cloudflare Support to troubleshoot WARP captive portal issues. When an end user reports an issue with a captive portal, you can ask the user to collect captive portal logs and then attach the logs to your support ticket.
...
4. The WARP client will ask if your device is connected (or attempting to connect) to the Wi-Fi network that is causing issues. Select **Yes** to confirm.
5. WARP will place a `warp-captive-portal-diag-<date>-<time>.zip` file on your Desktop. Send this file to your administrator.
