...
:::caution[User PII]
WARP's DNS logging feature stores all DNS queries on the device onto disk. If a user enables **Log DNS queries** in the GUI or runs `warp-cli dns log enable`, any user on the device will be able to examine the DNS queries of another user.
:::
