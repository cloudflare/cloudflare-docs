...
`cloudflared` connects to Cloudflare's global network on port `7844`. To use Cloudflare Tunnel, your firewall must [allow outbound connections](/cloudflare-one/connections/connect-networks/configure-tunnels/tunnel-with-firewall/#configure-your-firewall) to the following destinations on port `7844` (via UDP if using the `quic` protocol or TCP if using the `http2` protocol).
...
When using the [US region](/cloudflare-one/connections/connect-networks/configure-tunnels/cloudflared-parameters/run-parameters/#region), ensure your firewall [allows outbound connections](/cloudflare-one/connections/connect-networks/configure-tunnels/tunnel-with-firewall/#configure-your-firewall) to these US-region destinations on port `7844` (TCP/UDP) for tunnel operation.
