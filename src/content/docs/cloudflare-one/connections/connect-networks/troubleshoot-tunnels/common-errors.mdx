...
- One or more [Workers Routes](/workers/configuration/routing/routes/) are overlapping with the tunnel hostname, and the Workers do not properly handle the traffic. To resolve, check if the Routes or the code can be adjusted accordingly.
