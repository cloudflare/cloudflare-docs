...
In this example, we will connect a gRPC server to Cloudflare using the `cloudflared` <GlossaryTooltip term="daemon">daemon</GlossaryTooltip>, secure the server with Gateway policies, and open a gRPC channel to the server using the Cloudflare WARP client.
