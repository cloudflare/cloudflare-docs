...
title: Connect to SSH with Access for Infrastructure
sidebar:
  order: 3
  label: Connect with Access for Infrastructure
---

import { Render } from "~/components";

[Access for Infrastructure](/cloudflare-one/applications/non-http/infrastructure-apps/) uses the same deployment model as [WARP-to-Tunnel](/cloudflare-one/connections/connect-networks/use-cases/ssh/ssh-warp-to-tunnel/) but unlocks more granular policy options, such as defining the Unix usernames that users can SSH with.
...
If your organization has configured a private DNS resolver, then this command may look like:
