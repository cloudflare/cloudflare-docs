...
Refer to [our IdP setup instructions](/cloudflare-one/identity/idp-integration/entra-id/#set-up-azure-ad-as-an-identity-provider) for Entra ID.
...
3. Modify `wrangler.toml` to include the following values:
   ...
   - `<TENANT_ID>`: your Entra ID **Directory (tenant) ID**, obtained when [setting up Entra ID as an identity provider](#1-set-up-azure-ad-as-an-identity-provider).
   - `<CLIENT_ID>`: your Entra ID **Application (client) ID**, obtained when [setting up Entra ID as an identity provider](#1-set-up-azure-ad-as-an-identity-provider).
