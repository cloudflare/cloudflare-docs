...
An IT adminstrator, you want to identify shadow MCP traffic to can prevent unauthorized data exfiltration while still supporting governed use cases. In this tutorial, you will use the Cloudflare GraphQL Analytics API to scan Gateway HTTP logs for MCP traffic patterns, create DLP profiles that detect MCP JSON-RPC methods, and classify traffic to differentiate between authorized traffic sent to MCP server portals and straffic sent to "shadow" remote MCP servers.

## Prerequisites

- A Cloudflare account with a [Zero Trust organization](/cloudflare-one/setup/)
- [Gateway](/cloudflare-one/policies/gateway/) with HTTP filtering enabled and actively proxying user traffic
...
:::note
Sample code for the complete GraphQL query and result processing is available in the [MCP Shield query processing source](https://github.com/janguiano301184/helix-mcp-shield/blob/main/src/services/cloudflare-api.ts). The example includes error handling, retry logic, and pagination for accounts with more than 10,000 results.
:::
...
const response = await fetch(
	`https://api.cloudflare.com/client/v4/accounts/${accountId}/dlp/profiles/custom`,
	{
		method: "POST",
		headers: {
			Authorization: `Bearer ${apiToken}`,
		...
		body: JSON.stringify(dlpProfile),
	},
);
...
:::note
Sample code for all 10 DLP regex patterns and DLP profile creation is available in the [MCP Shield DLP patterns source](https://github.com/janguiano301184/helix-mcp-shield/blob/main/src/data/mcp-patterns.ts). The example includes a known MCP server registry with more than 20 servers.
:::
...
- [DLP profiles](/cloudflare-one/policies/data-loss-prevention/)
