...
The registration and status commands verify enrollment and the connection to Cloudflare. To verify end-to-end traffic, connect to an included destination using the protocol you intend to use. For Cloudflare Mesh, follow [Verify connectivity](/cloudflare-one/networks/connectors/cloudflare-mesh/guides/connect-client-devices/#2-verify-connectivity).
