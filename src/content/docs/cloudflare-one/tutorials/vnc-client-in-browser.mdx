...
3. Name the application and set the domain to which you would like to expose the VNC server.
   ...
   ![Example domain name input for VNC application](~/assets/images/cloudflare-one/zero-trust-security/vnc-client-in-browser/vnc-domain-application.png)

4. Add an Allow or Block policy. In this example, we are only allowing users with emails ending in `@example.com`.

   ![Example Zero Trust policy input for VNC application](~/assets/images/cloudflare-one/zero-trust-security/vnc-client-in-browser/vnc-policy.png)
...
5. In **Additional settings**, set **Browser rendering** to *VNC*.
