...
1. Follow [these instructions](/cloudflare-one/networks/connectors/cloudflare-tunnel/downloads/) to install `cloudflared`.
...
6. [Route your Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/routing-to-tunnel/dns/) to your website.
...
8. Follow [this guide](/cloudflare-one/networks/connectors/cloudflare-tunnel/configure-tunnels/tunnel-with-firewall/) to open outbound connections for Cloudflare Tunnel if you have a firewall enabled.
...
6. In **Access policies**, add an Allow or Block policy. For example policies, refer to the [Access policies documentation](/cloudflare-one/access-controls/policies/access/#allow).
