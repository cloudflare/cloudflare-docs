...
5. **Attachments**: If an email has an attachment, the dashboard will display the filename, and the disposition assigned. You can open attachments in [Browser Isolation](/cloudflare-one/policies/browser-isolation/). Only PDF files are currently supported.
