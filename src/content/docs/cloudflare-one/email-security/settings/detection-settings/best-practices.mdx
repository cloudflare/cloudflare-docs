...
For impersonation events that are caused by systems, we recommend that you configure an [allow policy](/cloudflare-one/email-security/settings/detection-settings/allow-policies/) to mitigate delivery disruptions.

To maintain a higher security posture, allow policies should be defined with the narrowest possible scope. Start with specific expressions or email addresses that will target the actual sender or system. If the system is sending from a variety of addresses, then, create an expression that is wider while keeping the expression specific.  In some situations,  it is better to have multiple specific entries  than creating a more generic policy that allows a whole domain.
