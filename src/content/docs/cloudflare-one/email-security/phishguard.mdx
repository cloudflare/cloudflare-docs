...
PhishGuard only works on a post-delivery environment (only emails that have already landed in your email inbox are reviewed). As a result, PhishGuard analysts may [submit a message for review](/cloudflare-one/email-security/submissions/#submit-messages-for-review) or [auto-move](/cloudflare-one/email-security/settings/auto-moves/) based on their findings.
...
- Review every [user](/cloudflare-one/email-security/investigation/search-email/#user-submissions) and [team](/cloudflare-one/email-security/investigation/search-email/#team-submissions) submission so your security team can focus on more critical activities.
...
1. In the [Cloudflare dashboard](https://dash.cloudflare.com/), go to **Zero Trust** > **Email security** > **PhishGuard**.

   <DashButton url="/?to=/:account/email-security/phishguard" zeroTrust />
