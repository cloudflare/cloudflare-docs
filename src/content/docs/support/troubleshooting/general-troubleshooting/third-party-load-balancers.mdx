...
Instead of modifying your F5 configuration, it is possible to do load balancing completely with a [Cloudflare load balancer](/load-balancing/) with [session affinity](/load-balancing/understand-basics/session-affinity/) to handle session persistence at the Cloudflare global network:
