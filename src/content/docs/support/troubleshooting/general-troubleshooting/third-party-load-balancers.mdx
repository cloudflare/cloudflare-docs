...
When using Cloudflare as a reverse proxy ([orange-clouded](/dns/proxy-status/#benefits)) in front of F5 BIG-IP load balancers, you may encounter session affinity issues due to how Cloudflare maintains persistent connections.
