...
An `OriginResponseStatus` value of **`0`** means Cloudflare received **no HTTP response** from the origin. The origin either:

- Dropped the TCP connection before sending response headers
- Timed out before sending headers
- Sent a malformed response that Cloudflare could not parse

This is different from the origin returning a 500 status — when `OriginResponseStatus = 0`, no HTTP response was completed. Cloudflare generates the error page itself (typically a 520 or 500).

:::caution

Do not conclude your origin returned a 5xx error if `OriginResponseStatus = 0`. The origin may have silently closed the TCP connection without sending any HTTP response.
