...
- Before a connection is established, the origin web server does not return a SYN+ACK to Cloudflare within 19 seconds of Cloudflare sending a SYN. (the SYN retry intervals are 1 + 1 + 1 + 1 + 1 + 2 + 4 + 8)
