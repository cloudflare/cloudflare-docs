...
Error `524` indicates that Cloudflare successfully connected to the origin web server, but the origin did not provide an HTTP response before the default 125 seconds [Proxy Read Timeout](/fundamentals/reference/connection-limits/).
...
The error `524` occurs if the origin web server acknowledges (ACK) the resource request after the connection has been established, but does not send a timely response (within the [Proxy Read Timeout](/fundamentals/reference/connection-limits/) delay, 125 seconds by default).
...
- If you regularly run HTTP requests that take over 125 seconds to complete (for example, large data exports), move those processes behind a [subdomain not proxied (DNS-only, grey clouded)](/dns/proxy-status/#dns-only-records) in the Cloudflare **DNS** app.
...
As a workaround, you can simply set the timeout to one second more (126 seconds instead of 125 seconds, for example).
:::
