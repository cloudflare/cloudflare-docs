...
- Enterprise customers can increase the `524` timeout up to 6,000 seconds:
  - If your content can be cached, you can create a [Cache Rule](/cache/how-to/cache-rules/settings/#proxy-read-timeout-enterprise-only) with the `Proxy Read Timeout` setting. (The content needs to be cacheable for the rule to be triggered, but doesn't need to be cached)  
