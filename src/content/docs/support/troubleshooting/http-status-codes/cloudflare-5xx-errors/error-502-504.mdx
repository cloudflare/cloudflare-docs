...
- Using [Gateway](/cloudflare-one/traffic-controls/) can lead to an HTTP Error `502` if the origin only partially supports HTTP/2. Refer to [Troubleshooting](/cloudflare-one/traffic-controls/troubleshooting/#error-502-bad-gateway) for more details and resolution.
