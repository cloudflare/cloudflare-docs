...
- [Custom rules](/waf/custom-rules/) can return any response code in the range 400-499 in your HTML page, if the site owner has created a rule with _Block_ action and configured a custom response code. Refer to [custom response](/waf/reference/response-for-blocked-requests/#custom-response) for more details.
