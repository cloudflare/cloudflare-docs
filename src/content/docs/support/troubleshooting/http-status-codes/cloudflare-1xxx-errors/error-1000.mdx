...
- If your domain points to a SaaS provider that uses Cloudflare (for example, Shopify), contact the SaaS provider to verify that a custom hostname is properly configured for your domain. The error originates from the provider's Cloudflare account, not yours. You may also need to remove and re-add your domain in the provider's settings to trigger the custom hostname provisioning.