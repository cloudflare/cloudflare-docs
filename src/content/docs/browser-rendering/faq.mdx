...
Most often, this error is caused by a timeout. You can review the different timers and their limits in the [Quick Actions timeouts reference](/browser-rendering/reference/timeouts/).
...
For a complete reference, see [Quick Actions timeouts](/browser-rendering/reference/timeouts/).
...
If the page you are rendering requires authentication, you can pass credentials using one of the following methods. These parameters work with all [Quick Actions](/browser-rendering/quick-actions/) endpoints.
...
For complete working examples of all three methods, refer to [Capture a screenshot of an authenticated page](/browser-rendering/quick-actions/screenshot-endpoint/#capture-a-screenshot-of-an-authenticated-page).
...
Yes. If your webpage or PDF requires a font that is not pre-installed, you can load custom fonts at render time using `addStyleTag`. This works with [Quick Actions](/browser-rendering/quick-actions/), [Puppeteer](/browser-rendering/puppeteer/), and [Playwright](/browser-rendering/playwright/). For instructions and examples, refer to [Custom fonts](/browser-rendering/features/custom-fonts/).
...
This applies to all integration methods, including [Quick Actions](/browser-rendering/quick-actions/), [Puppeteer](/browser-rendering/puppeteer/), [Playwright](/browser-rendering/playwright/), and [CDP](/browser-rendering/cdp/).

### Is there any temporary caching of submitted content?

For [Quick Actions](/browser-rendering/quick-actions/), generated content is cached by default for five seconds (configurable up to one day via the `cacheTTL` parameter, or set to `0` to disable caching). This cache protects against repeated requests for the same URL by the same account. Customer-submitted HTML content itself is not cached.

For [Puppeteer](/browser-rendering/puppeteer/), [Playwright](/browser-rendering/playwright/), and [CDP](/browser-rendering/cdp/), no caching is used. Content exists only in memory for the duration of the rendering operation and is discarded immediately after the response is returned.
