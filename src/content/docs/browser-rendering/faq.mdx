...
4. Under **When incoming requests match**, use the **Field** drop-down list to choose _Bot Detection ID_. For **Operator**, select _equals_. For **Value**, enter `128292352`.
...
- Use the `history()` method ([Puppeteer](/browser-rendering/puppeteer/#list-recent-sessions) or [Playwright](/browser-rendering/playwright/#list-recent-sessions)) to review recent sessions and identify any that closed due to `BrowserIdle` instead of `NormalClosure`. Sessions that close due to idle timeout indicate the browser was not closed explicitly.
