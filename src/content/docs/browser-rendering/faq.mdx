...
For the [REST API](/browser-rendering/rest-api/), generated content is cached by default for 5 seconds (configurable up to 1 day via the `cacheTTL` parameter, or set to `0` to disable caching). This cache protects against repeated requests for the same URL by the same account. Customer-submitted HTML content itself is not cached.
