...
## 5. Configure [Wrangler's configuration](/workers/wrangler/configuration) file

Configure your `browser-worker` project's configuration file by adding a browser [binding](/workers/runtime-apis/bindings/) and a [Node.js compatibility flag](/workers/configuration/compatibility-dates/#nodejs-compatibility-flag). Browser bindings allow for communication between a Worker and a headless browser which allows you to do actions such as taking a screenshot, generating a PDF and more.

Update your [Wrangler configuration](/workers/wrangler/configuration) file with the Browser Rendering API binding, the R2 bucket you created and a Durable Object:
