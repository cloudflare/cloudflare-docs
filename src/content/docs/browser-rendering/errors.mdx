...
| `422` | - | `Unprocessable Entity` | Browser Rendering could not complete the action because of an issue with the target site. | Review timeout settings and target site requirements. Refer to [422 Unprocessable Entity](/browser-rendering/faq/#error-422-unprocessable-entity). |
| `429` | - | `Browser connection lost. Please try again in a few minutes.` | Rate limit exceeded. | Refer to [429 Too many requests](/browser-rendering/limits/#error-429-too-many-requests). |
...
| `5001` | `net::ERR_ADDRESS_UNREACHABLE` | The address is unreachable. | Check if the site is blocked by firewalls or network policies. |
| `5002` | `net::ERR_ABORTED` | The network connection was aborted. | Review and adjust timeout settings. Refer to [REST API timeouts](/browser-rendering/reference/timeouts/). |
...
| `5006` | `net::ERR_CONNECTION_CLOSED`, `net::ERR_CONNECTION_RESET`, `net::ERR_TUNNEL_CONNECTION_FAILED` | The network connection was closed. Can also happen due to DNS resolution failure. | Verify the target site is operational and network connectivity is stable. Check DNS records if the issue persists. |
...
| `No such module 'cloudflare:browser'` | Browser Rendering is not enabled on your account. | [Enable Browser Rendering](/browser-rendering/get-started/) by creating a browser binding in your Worker configuration. |
