...
Refer to [optional parameters](/browser-rendering/quick-actions/crawl-endpoint/#optional-parameters) for additional customization options.
...
1. [Initiate the crawl job](/browser-rendering/quick-actions/crawl-endpoint/#initiate-the-crawl-job) — A `POST` request where you initiate the crawl and receive a response with a job `id`.
2. [Request results of the crawl job](/browser-rendering/quick-actions/crawl-endpoint/#request-results-of-the-crawl-job) — A `GET` request where you request the status or results of the crawl.
...
Send a `POST` request with a `url` to start a crawl job. The API responds immediately with a job `id` you will use to retrieve results. Refer to [optional parameters](/browser-rendering/quick-actions/crawl-endpoint/#optional-parameters) for additional customization options.
...
This information is only available in the crawl results (step 2) — the [initiation response](/browser-rendering/quick-actions/crawl-endpoint/#initiate-the-crawl-job) only returns the job `id`. Because crawl jobs run asynchronously, the crawler does not fetch page content at initiation time.
...
| `formats` | Array of strings | Response format (default is HTML, other options are Markdown and JSON). The JSON format leverages [Workers AI](/workers-ai/) by default for data extraction, which incurs usage on Workers AI. Refer to the [`/json` endpoint](/browser-rendering/quick-actions/json-endpoint/) to learn more, including how to use a custom model and fallbacks. |
...
| `jsonOptions` | Object | Only required if `formats` includes `json`. Contains `prompt`, `response_format`, and `custom_ai` properties (same types as the [`/json` endpoint](/browser-rendering/quick-actions/json-endpoint/)). |
...
Extract structured product data using the `json` format. This leverages [Workers AI](/workers-ai/) by default. Refer to the [`/json` endpoint](/browser-rendering/quick-actions/json-endpoint/) to learn more.
...
The `/crawl` endpoint uses `CloudflareBrowserRenderingCrawler/1.0` as its User-Agent, which is different from other [Quick Actions](/browser-rendering/quick-actions/) endpoints. This User-Agent is not customizable. Unlike other Quick Actions endpoints, the `userAgent` parameter is not supported on the `/crawl` endpoint.
...
- **Use a custom model** — If the default [Workers AI](/workers-ai/) model does not produce the desired results, use the `custom_ai` parameter to specify a different model. Refer to [Using a custom model (BYO API Key)](/browser-rendering/quick-actions/json-endpoint/#using-a-custom-model-byo-api-key) for details.
