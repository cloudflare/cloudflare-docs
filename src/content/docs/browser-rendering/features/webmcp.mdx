...
WebMCP is currently available in Chrome beta, so it requires a Lab session. Lab sessions run in a separate pool from the standard Browser Rendering pool, using Chrome beta with experimental features enabled. Your production workloads on the [standard pool](/browser-rendering/#key-features) remain on a stable version of Chrome.

Use the REST API to acquire a lab browser session (replace `$ACCOUNT_ID` with your Cloudflare account ID and `$CLOUDFLARE_API_TOKEN` with your API token):

<CURL
	method="POST"
	url="https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/browser-rendering/devtools/browser?lab=true&targets=true&keep_alive=300000"
...
1. Navigate to some of the sites listed in the [WebMCP documentation](https://github.com/GoogleChromeLabs/webmcp-tools/?tab=readme-ov-file#demos). The following instructions are based on the [L'Atelier Hotel Chain](https://github.com/GoogleChromeLabs/webmcp-tools/tree/main/demos/hotel-chain) demo.
...
The list of tools will change depending on the website you are visiting or even the page or actions you perform until that moment.
...
You will get a list of filtered results, where you can pick the best option for your needs. Once you click on a hotel, you can use the `start_booking` tool:
...
After you click the **Confirm Reservation** button, you will get a confirmation message and the booking will be complete.

### Using an AI Agent

#### 1. Configure Chrome DevTools MCP

[Chrome DevTools MCP](https://github.com/anthropics/anthropic-quickstarts/tree/main/mcp-devtools) allows AI agents to control a browser via CDP. Configure your MCP client (such as Claude Desktop, OpenCode, and Cursor) with the following settings. For more details on MCP client configuration, refer to [Using with MCP clients](/browser-rendering/cdp/mcp-clients/).
...
Then, use the session ID from the previous response to get the DevTools URL (replace `$SESSION_ID` in the URL):

<CURL
	method="GET"
	url="https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/browser-rendering/devtools/browser/$SESSION_ID/json"
	headers={{
		Authorization: `Bearer $CLOUDFLARE_API_TOKEN`,
	}}
/>
