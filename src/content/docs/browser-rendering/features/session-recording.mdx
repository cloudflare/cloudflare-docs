...
const browserWSEndpoint = `wss://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/browser-rendering/devtools/browser?recording=true&keep_alive=600000`;
...
After a session closes, its recording is available in the Cloudflare dashboard under **Browser Rendering** > **Runs**. Select a session to open the recording viewer, where you can scrub the timeline and inspect DOM state, console output, and browser interactions at each point in time.
...
- Session recording is available with Workers Bindings via `launch()` and the [CDP endpoint](/browser-rendering/cdp/). It is not available with the REST API.
