...
Session reuse applies to Browser Sessions ([Puppeteer](/browser-rendering/puppeteer/), [Playwright](/browser-rendering/playwright/), and [CDP](/browser-rendering/cdp/)). [Quick Actions](/browser-rendering/quick-actions/) handle session lifecycle automatically.

The best way to improve the performance of your browser rendering Worker is to reuse sessions. One way to do that is via [Durable Objects](/browser-rendering/how-to/browser-rendering-with-do/), which allows you to keep a long running connection from a Worker to a browser. Another way is to keep the browser open after you've finished with it, and connect to that session each time you have a new request.
