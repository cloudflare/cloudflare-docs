...
- A Cloudflare account with [Browser Rendering enabled](/browser-rendering/get-started/#rest-api)
...
Create a Astro route that renders your OG image design. This page serves as the source of truth for your image layout.
...
// Configuration
const BASE_URL = "https://your-site.pages.dev"; // Your deployed site URL
...
Browser Rendering has [rate limits](/browser-rendering/limits/) that vary by plan:
- **Free plan**: 6 requests per minute
- **Paid plan**: 180 requests per minute

The script includes a 200ms delay between requests to help stay within these limits. For large sites on the free plan, you may need to run the script in batches.
