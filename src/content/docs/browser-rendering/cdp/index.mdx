...
Connect to the WebSocket endpoint `/devtools/browser` to acquire a session and send [CDP commands](https://chromedevtools.github.io/devtools-protocol/) directly over the connection. This is the standard way to use CDP and works with any CDP client, including [Puppeteer](/browser-rendering/cdp/puppeteer/), Playwright, and [MCP clients](/browser-rendering/cdp/mcp-clients/).
