...
title: /devtools - Browser sessions management
sidebar:
  order: 12
---
...
Unlike the [Workers binding](/browser-rendering/workers-bindings/), the Browser Sessions endpoints can be accessed from any environment that supports WebSocket connections — including local development machines, external servers, and CI/CD pipelines. This means you can connect to Browser Rendering from Node.js scripts, Puppeteer, Playwright, or any CDP-compatible client.
...
Connect to the WebSocket endpoint `/devtools/browser` to acquire a session and send [CDP commands](https://chromedevtools.github.io/devtools-protocol/) directly over the connection. This is the standard way to use CDP and works with any CDP client, including [Puppeteer](/browser-rendering/rest-api/devtools/puppeteer/), Playwright, and [MCP clients](/browser-rendering/rest-api/devtools/mcp-clients/).

### HTTP API

We also provide HTTP endpoints to manage the browser lifecycle without using WebSockets. These follow the standard [CDP HTTP endpoints](https://chromedevtools.github.io/devtools-protocol/#endpoints):
