...
The crawler currently uses this fixed discovery order. A `source` parameter to customize this behavior (such as using only sitemaps or RSS feeds) will be added in a future release.
...
URLs that are discovered but skipped due to these rules will be visible in the dashboard logs in a future release. This will help you understand which items were intentionally omitted and debug cases where exclude and include rules accidentally overlap.
:::
