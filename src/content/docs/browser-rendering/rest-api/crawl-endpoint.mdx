...
If a crawled page returns an HTTP error (such as `402`, `403`, or `500`), the record for that URL will have `"status": "errored"`. The HTTP status code and the response body are captured in the `metadata` field.
...
The `metadata.status` field contains the HTTP status code returned by the origin server, and `metadata.html` contains the response body. This is useful for understanding site owners' intent when they block crawlers — for example, sites using [AI Crawl Control](https://blog.cloudflare.com/ai-crawl-control) may return a custom status code and message.
...
The `/crawl` endpoint respects [Content Signals](https://contentsignals.org/) directives found in a target site's `robots.txt` file. Content Signals are part of an [emerging IETF standard](https://ietf-wg-aipref.github.io/drafts/draft-ietf-aipref-vocab.html) that allows site owners to express preferences about how their content can be used by automated systems. For more background, refer to [Giving users choice with Cloudflare's new Content Signals Policy](https://blog.cloudflare.com/content-signals-policy/).
...
:::note
Content Signals are trust-based. By setting `crawlPurposes`, you are declaring to the site owner how you intend to use the crawled content. Misrepresenting your intended use may violate the site owner's expressed preferences and could have legal implications depending on jurisdiction.
:::
