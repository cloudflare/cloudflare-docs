...
However, if the starting URL belongs to a domain that is onboarded to your Cloudflare account, the crawler will bypass `robots.txt` restrictions. This allows you to crawl your own sites without `robots.txt` blocking the crawler.

:::note
To bypass `robots.txt` restrictions, the domain must be onboarded to the same Cloudflare account you are using to make the API request.

Refer to [Add a site to Cloudflare](/fundamentals/setup/manage-domains/add-site/) for more information on adding a domain to your Cloudflare account.
:::
...
- `status` — Filter by URL status (`queued`, `completed`, `disallowed`, `errored`, or `cancelled`).
