...
Unlike the automatic headers listed below, the User-Agent can be customized using the `userAgent` parameter in your API request. Because the User-Agent is configurable, destination servers should use the automatic headers below to identify Browser Rendering requests rather than relying on the User-Agent string.
