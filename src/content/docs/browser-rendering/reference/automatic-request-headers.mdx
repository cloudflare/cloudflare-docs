...
:::note
The Chrome version in the User-Agent string may change as Browser Rendering updates its underlying browser engine. Do not rely on a specific version number.
:::

Unlike the headers listed below, the User-Agent can be customized using the [`userAgent` parameter](/browser-rendering/rest-api/screenshot-endpoint/#set-a-custom-user-agent) in your API request. Because the User-Agent is configurable, destination servers should use the headers below to identify Browser Rendering requests rather than relying on the User-Agent string.
