...
- **API familiarity** -- The API is the preferred interface for managing tags. You can also use the dashboard under **Manage Account** > **Resource Tagging**, but you should be comfortable making authenticated HTTP requests for automation workflows.
- **API token** -- An API token with the required permissions. [Account Owned Tokens](/fundamentals/api/get-started/account-owned-tokens/) are recommended for automation.
