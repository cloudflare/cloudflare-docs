...
When you enable multi-provider DNS on a primary zone (full):
...
* Cloudflare will activate a primary zone (full) even if its [nameservers listed at the registrar](/dns/nameservers/update-nameservers/) include nameservers from other DNS providers.
