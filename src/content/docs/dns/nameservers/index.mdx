...
<GlossaryTooltip term="nameserver">Nameservers</GlossaryTooltip> are DNS servers
that answer queries about the domains they are responsible for. When a visitor
types your domain into a browser, the [DNS resolution
process](https://www.cloudflare.com/learning/dns/what-is-dns/) passes through
several server types and eventually reaches the authoritative nameservers for
the final answer.
...
You provide fully qualified domain names — complete domain names like `ns1.mydomain.com` — for your nameservers, and Cloudflare assigns one IPv4 and one IPv6 address to each.
