...
- Transfer your domain to a registrar that allows nameserver management, such as [Cloudflare Registrar](/registrar/).
- Use a [CNAME setup (partial)](/dns/zone-setups/partial-setup/) instead, which does not require nameserver changes. This option is available on Business and Enterprise plans.

### Platforms that already use Cloudflare

Platforms like Shopify and GoHighLevel pre-configure domains with their own Cloudflare nameserver pair through partner programs such as [Cloudflare for SaaS](/cloudflare-for-platforms/cloudflare-for-saas/). Adding the same domain to your own Cloudflare account assigns a different nameserver pair, which creates a conflict.

Use a [CNAME setup (partial)](/dns/zone-setups/partial-setup/) to proxy traffic through your own account without changing the nameservers at the registrar. For provider-specific guidance, refer to [Vendor-specific DNS records](/dns/manage-dns-records/reference/vendor-specific-records/).
