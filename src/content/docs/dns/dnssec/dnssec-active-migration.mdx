...
:::note[Cloudflare manages DNSSEC keys]
Cloudflare does not support uploading private DNSSEC keys. When you enable DNSSEC on Cloudflare, Cloudflare generates and manages all keys for your zone. If you are migrating from another provider with DNSSEC enabled, you must either disable DNSSEC before migration or use the multi-signer approach described in this tutorial.
:::
...
- Using the multi-signer DNSSEC approach described below to coordinate keys between providers
...
1. [Add your zone to Cloudflare](/fundamentals/manage-domains/add-site/).
