...
## What happens when multi-signer is enabled

When you enable multi-signer DNSSEC on Cloudflare (by setting `dnssec_multi_signer` to `true` [via API](/api/resources/dns/subresources/dnssec/methods/edit/)), the following changes occur:
