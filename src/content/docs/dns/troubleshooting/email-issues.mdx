...
NS records are used to delegate the management of a hostname to another DNS provider (refer to [Delegate a subdomain (outgoing)](/dns/manage-dns-records/how-to/subdomains-outside-cloudflare/#delegate-a-subdomain-outgoing) for further context). If you have NS records configured in **DNS** > **Records** on the Cloudflare dashboard, confirm that these are expected and not generating conflicts.
...
Also, if `CNAME` records are not returned by the queried nameserver (sometimes nameservers will return TXT records), this may result in nothing being returned when **Flatten all CNAMEs** is turned on. Changing to **Flatten at the root** should fix any issues with your CNAME records not being returned.
...
- [Exposed IP addresses](/dns/manage-dns-records/troubleshooting/exposed-ip-address/)
- [NS records already exist](/dns/manage-dns-records/troubleshooting/existing-ns-record/)
- [Verify a domain with CNAME](/dns/manage-dns-records/troubleshooting/cname-domain-verification/)
