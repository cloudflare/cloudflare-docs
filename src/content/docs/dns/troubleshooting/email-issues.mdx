...
If [Email Routing](/email-service/) is turned on, Cloudflare manages your MX records and may create additional DNS records automatically.

<DashButton url="/?to=/:account/:zone/email/routing" />

If Email Routing is turned on but you use a different mail provider, the Email Routing MX records may conflict with your provider's records. You can [turn off Email Routing](/email-service/configuration/domains/#remove-a-domain-from-email-routing) to remove the managed records and configure your own.
