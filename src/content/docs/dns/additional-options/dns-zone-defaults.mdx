...
In addition to zone defaults, the account DNS settings API also supports the [enforce DNS-only](/dns/proxy-status/enforce-dns-only/) setting — an emergency break-glass mechanism that bypasses Cloudflare's reverse proxy for all zones in your account. This setting is API-only and is not available in the dashboard.
