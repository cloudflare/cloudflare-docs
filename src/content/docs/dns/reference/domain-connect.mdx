...
- **REDIR301** and **REDIR302**: Supported. When applied, these records are converted to zone-specific [bulk redirect](/rules/url-forwarding/bulk-redirects/) rules. If a zone has existing bulk redirects before applying the template, they will be replaced.
- **APEXCNAME**: Not supported, and will cause the onboarding to fail. You can use a standard CNAME record instead, as Cloudflare automatically applies [CNAME flattening](/dns/cname-flattening/) at the zone apex.
