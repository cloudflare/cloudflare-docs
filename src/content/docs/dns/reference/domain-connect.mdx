...
Since September 2024 Cloudflare is using automatic template updates.
The automation works by comparing template version number in Cloudflare with authoritative source of the template in the Internet.
The template in the Internet needs to be in raw json format for automation to work correctly, for example:

https://raw.githubusercontent.com/Domain-Connect/Templates/master/exampleservice.domainconnect.org.template1.json

Update automation is trying to find new versions multiple times per day.
Cloudflare cannot promise when exactly update happens, but one should never need to wait longer than eight hours.

If upstream template is unavailable, or technically invalid, Cloudflare systems will keep old template in use until the upstream is fixed.

A service provider can ask Cloudflare to stop automatic updates.
Updates after automation is disabled can be requested one-by-one from `domain-connect@cloudflare.com`.

## Troubleshooting

Please get in touch with `domain-connect@cloudflare.com` with following information in the email.

1. Describe in the email what is wrong.
   - Mention the record(s) issue is related with.
   - Tell what the template did.
   - Tell what you expected template to do.
2. Collect a
   [HAR file](/support/troubleshooting/general-troubleshooting/gathering-information-for-troubleshooting-sites/#generate-a-har-file)
   attachment that contains problematic update.
