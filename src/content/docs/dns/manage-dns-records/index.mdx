---
pcx_content_type: navigation
title: DNS records
sidebar:
  order: 5

---

import { DirectoryListing, FeatureTable, GlossaryTooltip } from "~/components"

DNS records communicate information about your domain to visitors and other web services.

For background about different types of DNS records, refer to the [Learning Center](https://www.cloudflare.com/learning/dns/dns-records/).

## DNS records quota

There is a limit to the number of records you can create on a single <GlossaryTooltip term="DNS zone">zone</GlossaryTooltip>.

- Free zones created before `2024-09-01 00:00:00 UTC`: 1,000
- Free zones created on or after `2024-09-01 00:00:00 UTC`: 200
- Pro: 3,500
- Business: 3,500
- Enterprise: 3,500

:::note

For more DNS records. If you are an Enterprise customer and require more DNS records, contact your account team. Cloudflare can support millions of DNS records on a single zone.

:::

## Resources

<DirectoryListing />