...
A DNS zone file can be constructed using Directives in addition to Resource Records (RRs). Directives start with `$` and are standardized - `$ORIGIN` and `$INCLUDE` (defined in [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035#section-5.1)) and `$TTL` (defined in [RFC 2308](https://www.rfc-editor.org/rfc/rfc2308)). BIND additionally provides the non-standard `$GENERATE` directive. Cloudflare support `$ORIGIN`, `$TTL` and `$GENERATE` directives as expected, however `$INCLUDE` does not work in the context of a file uploaded to Cloudflare.
