...
Does the user client receive a network range of address (in this case 192.0.2.0/x) when a request is made to an unproxied dns server (typically, only one IP address should respond to the client's request for connection)?
Ideally, the expected behavior should be as follows:
"When a request to one IP address fails, the unproxied dns server (if load balancing or failover feature is available) should automatically retry the request to other IP addresses associated with the same hostname. This behavior prevents end users from experiencing downtime."
