...
Cloudflare allows you to apply several changes to your zone records in just one action. You can [use the dashboard](#use-the-dashboard) to delete or update the <GlossaryTooltip term="proxy status">proxy status</GlossaryTooltip> of DNS records in bulk, or perform further batched operations [via API](#use-the-api).
