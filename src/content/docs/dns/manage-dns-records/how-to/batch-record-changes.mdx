...
Although Cloudflare will execute the batched operations in a single [database transaction](https://en.wikipedia.org/wiki/Database_transaction), Cloudflare's distributed KV store must treat each record change as a single key-value pair. This means that the changes propagation is not guaranteed to be atomic. Refer to our [blog post](https://blog.cloudflare.com/batched-dns-changes/) for details.
:::
