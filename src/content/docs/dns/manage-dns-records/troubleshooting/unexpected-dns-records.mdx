...
You might notice TXT records like `_acme-challenge.<hostname>` are returned by your domain but cannot be found in **DNS** > **Records** on the Cloudflare dashboard.
...
As these records are tied to the certificates, they cannot be deleted from **DNS** > **Records** on the Cloudflare dashboard.
...
If the issue is not related to caching, refer to [Stale response for upstream DNS resolution](/dns/manage-dns-records/troubleshooting/stale-response/).
