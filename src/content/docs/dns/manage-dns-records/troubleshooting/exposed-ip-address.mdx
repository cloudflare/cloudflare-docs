...
For records that support proxying:

`This record exposes the IP address used in the <RECORD_TYPE> record on <ORIGIN>. Enable the proxy status to protect your origin server.`

For records that do not support proxying (for example, `MX` records):

`This record exposes the IP address used in the <RECORD_TYPE> record on <ORIGIN>, which you have proxied through Cloudflare.`

A zone-level summary may also appear:
