...
- The CNAME record is correctly set to DNS only (not proxied) but, in your [zone settings](https://dash.cloudflare.com/?to=/:account/:zone/dns/settings), [**CNAME flattening for all CNAME records**](/dns/cname-flattening/set-up-cname-flattening/#for-all-cname-records) is on.
- The CNAME record is correctly set to DNS only (not proxied) but CNAME flattening is set [for that record specifically](/dns/cname-flattening/set-up-cname-flattening/#per-record).
...
- In your zone DNS settings: [**CNAME flattening for all CNAME records**](/dns/cname-flattening/) is turned off.
