...
- **Authoritative zone transfer ([AXFR](https://datatracker.ietf.org/doc/html/rfc5936))**: Copies the entire zone from the primary to the secondary provider, even if only one record changes.
- **Incremental zone transfer ([IXFR](https://datatracker.ietf.org/doc/html/rfc1995))**: Transfers only the changes since the last transfer, rather than the entire zone.
