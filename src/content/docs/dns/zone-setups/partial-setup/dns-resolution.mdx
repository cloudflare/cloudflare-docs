...
When you have a partial zone (<GlossaryTooltip term="partial setup" link="/dns/zone-setups/partial-setup/">CNAME setup</GlossaryTooltip>), Cloudflare handles DNS records a bit differently from primary zones (full setup) in order to internally resolve the origin server where proxied HTTP requests are sent to.
...
You could also [create a CNAME record](/dns/manage-dns-records/how-to/create-dns-records/#create-dns-records) in a zone (partial (CNAME setup) or full (primary setup)) that points to a record in another partial zone within your account.
