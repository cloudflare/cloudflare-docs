...
When you have a <GlossaryTooltip term="partial setup" link="/dns/zone-setups/partial-setup/">CNAME zone (partial)</GlossaryTooltip>, Cloudflare handles DNS records a bit differently from primary zones (full) in order to internally resolve the origin server where proxied HTTP requests are sent to.

## Records within the same zone

When you [create a new DNS record](/dns/manage-dns-records/how-to/create-dns-records/#create-dns-records) in a CNAME zone (partial), Cloudflare automatically checks whether any of your CNAME records point to existing A, AAAA, or CNAME records within the same zone.

For example, Cloudflare would show a warning if you had the following records in your CNAME zone (partial):
...
```mermaid
    flowchart TD
      accTitle: DNS resolution flow with CNAME target in same CNAME zone (partial)
...
When you avoid this situation - meaning you do not have the **target** of the CNAME record within your CNAME zone (partial) - this DNS resolution would happen differently.

```mermaid
    flowchart TD
      accTitle: DNS resolution flow with CNAME target not in CNAME zone (partial)
      A[Request to <code>sub1.partialzone.com</code>] --> B[<code>CNAME</code> record for <code>sub1.partialzone.com</code> to <code>sub2.partialzone.com</code>]
...
You could also [create a CNAME record](/dns/manage-dns-records/how-to/create-dns-records/#create-dns-records) in a zone (CNAME setup (partial) or primary setup (full)) that points to a record in another CNAME zone (partial) within your account.
...
- The target zone (Cloudflare zone 2 in this example) is a CNAME zone (partial) and the DNS record on the CNAME zone (partial) is proxied.
