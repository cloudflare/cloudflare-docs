...
* [Primary setup (Full)](/dns/zone-setups/full-setup/) (most common): Use Cloudflare as your primary DNS provider and manage your DNS records on Cloudflare.
* [CNAME setup (Partial)](/dns/zone-setups/partial-setup/): Keep your primary DNS provider and only use Cloudflare's reverse proxy for individual subdomains.
...
* [Subdomain setup](/dns/zone-setups/subdomain-setup/): With your apex domain (`example.com`) on a CNAME setup (partial) or primary setup (full), independently manage the settings for a delegated subdomain (`blog.example.com`) within a separate zone and, potentially, a separate account.

   <Render file="subdomain-setup-matrix" product="dns" />

   For details, refer to [setup](/dns/zone-setups/subdomain-setup/setup/).
...
For details on each status and how a zone can transition from one status to the other, consider the [Reference page](/dns/zone-setups/reference/domain-status/).

:::caution[Do not use pending zones in production]
If you have a paid plan, make sure not to use pending zones for production traffic. Cloudflare responds to DNS queries for pending zones on the assigned Cloudflare nameserver IPs but there are associated risks, especially if you do not use [zone holds](/fundamentals/account/account-security/zone-holds/).
:::
...
If you are on a Free or Pro plan, [primary setup (full)](/dns/zone-setups/full-setup/) is the only one available. This is the recommended and most common option.
...
If you are on a Business or Enterprise plan, you can use [CNAME setup (partial)](/dns/zone-setups/partial-setup/) to keep your primary DNS provider and only proxy individual subdomains through Cloudflare.
