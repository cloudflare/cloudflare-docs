...
:::caution
When you re-add a previously deleted domain, Cloudflare assigns a new nameserver pair as a security measure. The nameservers from the previous setup will not work. You must [update your registrar](/dns/nameservers/update-nameservers/) with the new nameservers, or the domain will remain in a **Pending Nameserver Update** status.
:::
