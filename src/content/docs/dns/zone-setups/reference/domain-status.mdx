...
When you re-add a previously deleted domain, Cloudflare assigns a **new nameserver pair**. This is a security measure to prevent domain hijacking. The nameservers from the previous setup will not work.

After re-adding the domain, check the assigned nameservers in the Cloudflare dashboard and [update your registrar](/dns/nameservers/update-nameservers/) accordingly. If the old nameservers remain at your registrar, the domain will show as **Pending Nameserver Update** and will not activate.
:::
