...
If you initially configured a [primary setup (full)](/dns/zone-setups/full-setup/), you can later convert your zone to use a CNAME setup (partial) (also known as CNAME setup).

A CNAME setup (partial) allows you to use [Cloudflare's reverse proxy](/fundamentals/concepts/how-cloudflare-works/) on individual subdomains while using a different authoritative DNS provider.
