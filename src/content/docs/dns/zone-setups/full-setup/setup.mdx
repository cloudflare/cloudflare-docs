...
- **Check whether your domain uses DNSSEC** — [DNSSEC](/dns/concepts/#dnssec) is a security feature that signs DNS records to prevent spoofing. If it is active at your registrar (the provider where you purchased your domain), you will need to turn it off before changing nameservers. Refer to [Change your nameservers](#3-change-your-nameservers) for details.
...
3. Enter your apex domain (for example, `example.com`) and choose how you would like to add your DNS records. The default option scans your existing DNS provider and imports records automatically.
4. Select **Continue** and choose a [plan](https://www.cloudflare.com/plans/#compare-features).

For a detailed walkthrough with additional context for each step, refer to [Onboard a domain](/fundamentals/manage-domains/add-site/).
