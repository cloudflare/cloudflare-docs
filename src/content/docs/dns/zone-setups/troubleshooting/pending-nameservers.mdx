...
You can request an earlier check from the **Overview** page or by [triggering one via API](/api/resources/zones/subresources/activation_check/methods/trigger/). This endpoint is rate-limited and may return an error if you have requested a check recently. A successful request does not activate the zone immediately — it places your zone in a prioritized queue, and the check itself can still take a few minutes to a few hours to complete depending on DNS propagation.
