...
If you are on Windows or prefer `nslookup`, query the parent zone the same way. Add the `-debug` flag so the output includes the authoritative response, not just the cached answer from your local resolver:
