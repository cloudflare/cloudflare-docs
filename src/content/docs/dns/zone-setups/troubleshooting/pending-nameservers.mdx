...
`dig +trace` follows the delegation from the root zone down, so its final output shows the nameservers that the parent zone is actually publishing for your domain. In a terminal, run:

```sh
dig +trace example.com NS
```

The last section of the output should return **only** the Cloudflare nameservers assigned to your zone (or, if you use [multi-provider DNS](/dns/nameservers/nameserver-options/#multi-provider-dns), it should include them alongside your other provider's nameservers).
