...
This page lists common scenarios where DNS records should be <GlossaryTooltip term="proxy status">proxied</GlossaryTooltip> or set to **DNS Only**, and describes expected behaviors when traffic is proxied. For background on how proxy status works, refer to [Proxy status](/dns/proxy-status/).
...
## Expected behaviors when proxying

When traffic is proxied through Cloudflare, the following behaviors apply. These are not limitations — they are how Cloudflare's reverse proxy works by design. Your origin configuration may need to account for them.
