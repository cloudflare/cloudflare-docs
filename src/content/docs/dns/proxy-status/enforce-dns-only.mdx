...
- **Spectrum applications** — These are already DNS-only CNAME records pointing to Spectrum endpoints.
- **Cloudflare for SaaS fallback origin records** — Default origin records for custom hostnames.
- **Cloudflare for SaaS records in eyeball zones** — Records pointing to SaaS providers. Bypassing the proxy on these records does not help if the SaaS provider's target remains proxied.
- **Workers custom domains** — Proxied A records pointing to `100::`.
- **R2 custom domains** — Proxied CNAME records pointing to `public.r2.dev`.
- **Web3 gateways** — Ethereum and IPFS DNSLink gateways (CNAMEs to `ethereum.cloudflare.com` / `ipfs.cloudflare.com`).
- **Cloudflare Tunnel records** — Records required for tunnel routing. Setting these to DNS-only may break tunnel connectivity.
