...
- Proxied [Load Balancing](/load-balancing/): An **LB** record is visible on the DNS records table but proxy status is managed through the [Load Balancing configurations](/load-balancing/load-balancers/create-load-balancer/).
- [Workers](/workers/) custom domains: records added to the DNS records table when you set up Workers [custom domains](/workers/configuration/routing/custom-domains/).
...
- [Web3 gateways](/web3/): CNAMEs to `ethereum.cloudflare.com`, `ipfs.cloudflare.com`, or `universal-gateway.ipfs.cloudflare.com`. These are read-only proxied records managed by the [gateway configuration](/web3/reference/gateway-dns-records/).
- [Cloudflare Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/routing-to-tunnel/dns/): CNAME records pointing to a tunnel subdomain (**Target** ending in `.cfargotunnel.com`).
