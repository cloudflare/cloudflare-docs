...
- Proxied [Load Balancing](/load-balancing/): An **LB** record is visible on the DNS records table but proxy status is managed through the [Load Balancing configurations](/load-balancing/how-to/create-load-balancer/).
...
- [Cloudflare Tunnel](/cloudflare-one/connections/connect-networks/): CNAME records pointing to a tunnel subdomain (**Target** ending in `.cfargotunnel.com`).
