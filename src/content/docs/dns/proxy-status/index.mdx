...
Proxied [CNAME records](/dns/manage-dns-records/reference/dns-record-types/#cname) are flattened by default. With CNAME flattening, Cloudflare finds the IP address that a CNAME points to, helping DNS queries resolve up to 30% faster. Refer to [CNAME flattening](/dns/cname-flattening/) for details.
