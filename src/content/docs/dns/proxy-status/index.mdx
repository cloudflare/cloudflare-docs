...
### Enforce DNS-only for all zones

In emergency situations, you can use the account-level [enforce DNS-only](/dns/proxy-status/enforce-dns-only/) setting to bypass Cloudflare's reverse proxy for all zones in your account at once, without modifying individual DNS records. This API-only break-glass mechanism is intended for scenarios where you need to quickly route all traffic directly to your origins.
