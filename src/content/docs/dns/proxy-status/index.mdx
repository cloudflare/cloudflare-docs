...
Every <GlossaryTooltip term="DNS record">DNS record</GlossaryTooltip> in Cloudflare has a proxy status that controls whether HTTP/HTTPS traffic for that record routes through Cloudflare's network or goes directly to your origin server.
...
Cloudflare recommends proxying all A, AAAA, and CNAME records that serve web traffic. Records used for other purposes, such as CNAME records for third-party domain verification, should not be proxied.
