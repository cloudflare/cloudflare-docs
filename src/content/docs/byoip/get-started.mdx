...
1. Use the [Add Prefix endpoint](/api/resources/addressing/subresources/prefixes/methods/create/) to create a prefix in the Cloudflare account that should own the BYOIP prefix.
    ...
    When you use your own ASN, `result` also includes its ownership validation state:
    ...
    The field is omitted when ASN ownership validation does not apply, such as when you use AS13335. Its initial state can be `valid` if the account previously verified the ASN.
...
When all applicable validations pass (RPKI, IRR, prefix ownership, and ASN ownership), the `approved` field in your prefix will return `"V"`. This means you can proceed to create IP address service bindings[^1].
