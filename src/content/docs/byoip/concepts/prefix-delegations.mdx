...
Prefix delegations allow a prefix owner (Account A) to grant another Cloudflare account (Account B) permission to use all or part of their BYOIP prefix. This is useful when you use [Cloudflare for SaaS](/cloudflare-for-platforms/cloudflare-for-saas/) but manage different configurations in different accounts — all accounts can use the IPs through a delegation.

The original prefix remains managed by Account A, but Account B can use the delegated IPs with CDN services (including Cloudflare for SaaS) or Spectrum. Refer to [service bindings](/byoip/service-bindings/) for more information on the services an IP can be bound to.
