...
Service bindings map traffic destined for IP addresses within a BYOIP prefix to the Cloudflare service it should be routed through, such as Magic Transit, CDN, or Spectrum. A default binding covering the entire prefix is required when you first [onboard](/byoip/get-started/#2-create-service-bindings), and additional bindings can be created at any time to route specific IP addresses or CIDR ranges to a different service.
