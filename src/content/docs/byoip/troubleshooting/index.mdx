...
|---|---|---|
| `prefix_not_valid_and_approved` | The prefix has not passed all applicable validation checks or manual approval. | Use the Prefix Details endpoint and follow [Prefix validation checks](/byoip/troubleshooting/prefix-validation/). ASN ownership and Autonomous System Provider Authorization (ASPA) checks apply only when you use your own ASN. If all applicable checks pass but you still see this error, contact support for manual approval. |
