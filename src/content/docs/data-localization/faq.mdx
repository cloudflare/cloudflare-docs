...
The Cloudflare API (even through the dashboard) uses dynamic steering based on health check probes. This can lead to users being steered temporarily to another core data center (AMS or PDX) for some period of time. [Out of region access](/data-localization/metadata-boundary/out-of-region-access/) allows requests arriving at PDX to pull analytics from AMS and vice-versa. The analytics are still exclusively stored in the CMB configured region.
