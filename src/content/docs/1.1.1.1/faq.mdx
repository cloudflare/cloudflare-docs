...
- **`SERVFAIL` or other transient failure versus a valid response:** Transient failures are treated as upstream errors rather than authoritative answers. 1.1.1.1 retries against another authoritative nameserver and returns the first valid response it receives. Only if all authoritative nameservers fail does the resolver return `SERVFAIL` to the client.
