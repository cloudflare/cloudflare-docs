...
## Intra - an alternative option, for Android

Intra is a recent Android client app developed by contributors under license by Jigsaw Operations LLC, that allows to use DNS-over-HTTPS services,
with a feature to choose Android apps to exclude from re-routing through its app service. It is open-source, [on GitHub](https://github.com/Jigsaw-Code/Intra), supports Cloudflare 1.1.1.1 (and other DoH servers) and to download it, refer to the Google Play Store [app listing](https://play.google.com/store/apps/details?id=app.intra).
