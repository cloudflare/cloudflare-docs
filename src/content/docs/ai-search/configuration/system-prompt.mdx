...
When you make a `/ai-search` request using the [Workers Binding](/ai-search/api/search/workers-binding/) or [REST API](/ai-search/api/search/rest-api/), you can set the system prompt programmatically.
...
const response = await instance.chatCompletions({
	messages: [
		...
		{ role: "user", content: "How do I train a llama to deliver coffee?" },
	],
