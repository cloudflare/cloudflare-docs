...
For instances connected to a [website](/ai-search/configuration/data-source/website/) or [R2 bucket](/ai-search/configuration/data-source/r2/), AI Search creates jobs to sync your data source. Jobs run automatically on a schedule, every 6 hours by default, and process new, modified, or deleted files to keep your search index up to date.
...
```sh
npx wrangler ai-search jobs create <INSTANCE_ID>
```

Or call the [Create job REST API](/ai-search/api/instances/rest-api/#jobs) from a CMS webhook or a Worker. Sync jobs can be triggered at most once every 30 seconds.
