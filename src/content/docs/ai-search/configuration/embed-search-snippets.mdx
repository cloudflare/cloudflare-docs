...
You can add AI Search easily into your website using the [Cloudflare AI Search UI snippet library](https://search.ai.cloudflare.com/). The library provides production-ready, customizable web components.
...
1. Add the CDN script tag to your HTML file (for example, `index.html`):

   ```html
   <script
   	type="module"
   	src="https://unpkg.com/@cloudflare/ai-search-snippet"
   ></script>
   ```
   ...
   <script
   	type="module"
   	src="https://cdn.jsdelivr.net/npm/@cloudflare/ai-search-snippet"
   ></script>
   ```

2. Add a component with your `api-url`. Replace `<INSTANCE_ID>` with your AI Search instance's public endpoint ID, which you can find in your AI Search instance's **Settings** > **Public Endpoint**.
