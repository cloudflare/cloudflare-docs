...
Choose `discover` when a sitemap is missing, incomplete, or stale, or when the source URL is on a domain that is not onboarded onto your Cloudflare account, which `sitemap` requires.
...
`sitemap` requires a domain that you have onboarded onto the same Cloudflare account. Refer to [Onboard a domain](/fundamentals/manage-domains/add-site/) if your domain is not on Cloudflare yet.
...
Because `discover` does not depend on a sitemap, it reaches pages that a sitemap omits. It also accepts a source URL on a domain that is not onboarded onto your Cloudflare account.
