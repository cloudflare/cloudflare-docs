...
- **Rendered sites**: Loads pages with a headless browser and downloads the fully rendered version, including dynamic JavaScript content. For instances with [built-in storage](/ai-search/configuration/data-source/built-in-storage/), Browser Rendering is included. For older instances, [Browser Rendering](/browser-rendering/pricing/) limits and billing apply.
...
- **Browser Rendering**: Content selectors apply to the HTML that AI Search receives. For sites that render content with JavaScript, turn on [Browser Run](#rendering-mode) so that selectors can target the fully rendered DOM.
...
For instances with [built-in storage](/ai-search/configuration/data-source/built-in-storage/), Browser Rendering and storage are included. For older instances, [R2](/r2/pricing/), [Vectorize](/vectorize/platform/pricing/), and [Browser Rendering](/browser-rendering/pricing/) are billed separately.
