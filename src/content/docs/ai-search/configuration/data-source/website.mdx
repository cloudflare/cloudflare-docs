...
The [parse type](/ai-search/configuration/data-source/parse-types/) determines how the crawler finds pages. This section describes the default `sitemap` parse type. To follow links instead of reading sitemaps, use the [`discover` parse type](/ai-search/configuration/data-source/parse-types/#discover).
...
**Parse type** selects how the crawler finds pages. **Sitemap** reads your XML sitemaps, and **Discover** follows links from the source URL. Refer to [Parse types](/ai-search/configuration/data-source/parse-types/) for the available options and their defaults.
...
Browser Run and storage are included for AI Search crawling. For full pricing details, refer to [Limits and pricing](/ai-search/platform/limits-pricing/).
