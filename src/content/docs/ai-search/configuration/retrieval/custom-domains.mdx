...
Custom domains are also the foundation for [restricting access with Cloudflare Access](/ai-search/configuration/retrieval/cloudflare-access/), which lets users authenticate with your identity provider before they can query your indexed content.
...
The same field is available on namespaces. Refer to [Namespace public endpoints](/ai-search/configuration/retrieval/namespace-public-endpoint/).
...
Set the record to **Proxied**, not **DNS only**. A proxied record routes traffic through your own zone before it reaches AI Search, so your zone settings apply first. This is what makes [Cloudflare Access](/ai-search/configuration/retrieval/cloudflare-access/), [WAF custom rules](/waf/custom-rules/), and [Bot Management](/bots/) usable in front of your public endpoint.
...
Turn this off whenever you put security controls in front of the custom domain. Those controls run in your own zone, so any traffic that reaches the default hostname skips them. Refer to [Cloudflare Access](/ai-search/configuration/retrieval/cloudflare-access/).
