...
A [namespace](/ai-search/concepts/namespaces/) can expose its own public endpoint. A single URL then searches across several instances in that namespace and merges the results.

This is the multi-instance equivalent of an [instance public endpoint](/ai-search/configuration/retrieval/public-endpoint/). The two are independent. A namespace endpoint and the endpoints of the instances inside it can be enabled or disabled separately.

Use a namespace endpoint when one search experience covers content that lives in several instances, such as documentation, a blog, and a support portal.

## Available endpoints

| Endpoint            | Description                                            |
| ------------------- | ------------------------------------------------------ |
| `/search`           | Returns relevant chunks from across the instances      |
| `/chat/completions` | Generates a response from across the instances         |
| `/mcp`              | Model Context Protocol endpoint for AI agents          |

No other paths are supported. A request to any other path returns a `404` with error `60006`.

## Public URL format

A namespace public endpoint identifier is prefixed with `ns-`:

```txt
https://ns-<NAMESPACE_ENDPOINT_ID>.search.ai.cloudflare.com/search
```

The identifier is generated the first time you enable the endpoint and is never rotated. Clearing the configuration keeps the identifier, so re-enabling the endpoint reuses the same URL.

Namespace endpoints support [custom domains](/ai-search/configuration/retrieval/custom-domains/) and [Cloudflare Access](/ai-search/configuration/retrieval/cloudflare-access/) in the same way instance endpoints do.
...
The response returns the generated `public_endpoint_id`.
...
Namespace endpoints accept the same configuration as instance endpoints, including `rate_limit`, `authorized_hosts`, `mcp`, `search_endpoint`, `chat_completions_endpoint`, `custom_domains`, and `default_domain_enabled`. Refer to [Public endpoint settings](/ai-search/configuration/retrieval/public-endpoint/).
...
| `60006` | `endpoint not found`                               | 404         | The path is not `/search`, `/chat/completions`, or `/mcp`.            |
