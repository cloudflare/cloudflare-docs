...
curl https://api.cloudflare.com/client/v4/accounts/{ACCOUNT_ID}/ai-search/instances/{NAME}/search \
  ...
  -d '{
    ...
    "filters": {
      "folder": "customer-a/contracts/"
    }
  }'
```

To filter across multiple folders, use the `$in` operator. For date-based filtering, use range operators. Refer to [Metadata filtering](/ai-search/configuration/metadata/) for more examples.
