---
title: Dashboard
pcx_content_type: get-started
sidebar:
  order: 1
description: Create and configure AI Search using the Cloudflare dashboard.
---

import { LinkCard } from "@astrojs/starlight/components";
import { DashButton, Steps } from "~/components";

This guide walks you through creating an AI Search instance using the Cloudflare dashboard.

## Create an AI Search instance

<DashButton url="/?to=/:account/ai/ai-search" buttonName="AI Search" />

<Steps>

1. Go to **Compute & AI** > **AI Search**.
2. Select **Create Instance**.
3. Name your AI Search instance.
4. Optionally connect a [data source](/ai-search/configuration/data-source/) such as a website you own or an R2 bucket. You can skip this step and upload files later.
5. Review your configurations.
6. Select **Create**.

</Steps>

## Upload content

If you did not connect a data source, you can upload files directly from the dashboard.

<Steps>

1. Go to **Compute & AI** > **AI Search**.
2. Select your instance.
3. Select the **Items** tab.
4. Upload your files. AI Search indexes them automatically.

</Steps>

## Try it out

Once your content is indexed, you can run your first query.

<Steps>

1. Go to your AI Search instance.
2. Select the **Playground** tab.
3. Select **Chat** or **Search**.
4. Enter a query to test the response.

</Steps>

## Add to your application

There are multiple ways you can connect AI Search to your application:

<LinkCard
	title="Workers Binding"
	description="Query AI Search directly from your Workers code."
	href="/ai-search/api/search/workers-binding/"
/>
<LinkCard
	title="REST API"
	description="Query AI Search using HTTP requests."
	href="/ai-search/api/search/rest-api/"
/>
