...
:::note
You only need to register a service token once per account. As long as the token exists, the `token_id` field in step 5 becomes optional for future instances.
:::
