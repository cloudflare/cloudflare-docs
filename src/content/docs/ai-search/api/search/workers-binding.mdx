...
[Workers](/workers/) provides a serverless execution environment that allows you to create new applications or augment existing ones. Use a Workers binding to search and chat with your AI Search instances from a Cloudflare Worker.
...
const response = await instance.chatCompletions({
	messages: [
		...
		{ role: "user", content: "Explain the rules of hooks" },
	],
...
const stream = await instance.chatCompletions({
	messages: [{ role: "user", content: "How does Suspense work?" }],
	stream: true,
});
