...
[Workers](/workers/) provides a serverless execution environment that allows you to create new applications or augment existing ones. Use a Workers binding to create, list, update, and delete AI Search instances from a Cloudflare Worker. You can also check instance configuration and monitor indexing progress.
...
const results = await instance.search({
	messages: [{ role: "user", content: "What is Workers KV?" }],
});
```
...
console.log(`Queued: ${stats.queued}, Completed: ${stats.completed}`);
```
...
After creating an instance with a data source, indexing runs automatically. For empty instances, use the [Items API](/ai-search/api/items/workers-binding/) to upload content.
