...
This example uses `@cf/zai-org/glm-5.2`: streaming agents need a model that emits structured tool calls, or the tool never runs and the agent cannot search. Choose a Workers AI model that supports [function calling](/workers-ai/features/function-calling/).
