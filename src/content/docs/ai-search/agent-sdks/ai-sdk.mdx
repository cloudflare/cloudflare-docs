...
async function ensureInstance(env: Env) {
	...
	try {
		await env.AI_SEARCH.create({ id: INSTANCE_ID, ...hybrid });
	} catch {
		await env.AI_SEARCH.get(INSTANCE_ID).update(hybrid);
	}
}
...
export default {
	async fetch(request, env): Promise<Response> {
		...
		if (url.pathname === "/setup") {
			...
			const instance = aiSearch.get(INSTANCE_ID);
...
const { text, sources } = await generateText({
	model: aiSearch.get(INSTANCE_ID).chat({
		ai_search_options: {
