...
5. **Vector storage:** The resulting vectors, along with metadata like file name, are stored in a [Vectorize](/vectorize/) database created on your Cloudflare account.
6. **Keyword indexing (optional):** If [hybrid search](/ai-search/configuration/hybrid-search/) is enabled, each chunk is also tokenized and added to a BM25 keyword index. This index runs alongside the vector index and is used for exact term matching during retrieval.

After the initial data set is indexed, AI Search will regularly check for updates in your data source (for example, additions, updates, or deletes) and index changes to ensure your vector database is up to date.
...
4. **Retrieval:** The query vector is [queried](/vectorize/best-practices/query-vectors/) against stored vectors in the associated Vectorize database. If [hybrid search](/ai-search/configuration/hybrid-search/) is enabled, a BM25 keyword search also runs in parallel against the keyword index, and the results from both retrievers are merged using the configured fusion method.
5. **Content retrieval:** The most relevant chunks are returned with their original content from R2. If you are using the Search endpoint, the content is returned at this point.
