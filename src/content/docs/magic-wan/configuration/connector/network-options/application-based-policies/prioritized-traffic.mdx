...
1. Create a [`GET` request](/api/operations/magic-account-apps-list-apps) to list the applications associated with an account.
...
2. Create a [`POST` request](/api/operations/magic-site-app-configs-add-app-config) to add new apps the priority traffic policy.
...
2. Create a `DELETE` request [using the API](/api/operations/magic-site-app-configs-delete-app-config) to delete an application from the breakout traffic policy.
   ...
   If successful, you will receive a confirmation message like the following:
