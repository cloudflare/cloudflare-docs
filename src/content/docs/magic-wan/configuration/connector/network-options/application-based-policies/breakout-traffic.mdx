...
1.  Send a [`GET` request](/api/operations/magic-account-apps-list-apps) to list the applications associated with an account.
...
2.  Send a [`POST` request](/api/operations/magic-site-app-configs-add-app-config) to add new apps the breakout traffic policy.
...
1. Send a [`GET` request](/api/operations/magic-site-app-configs-list-app-configs) to list the applications associated with a site.
...
2. Send a [`DELETE` request](/api/operations/magic-site-app-configs-delete-app-config) to delete an application from the breakout traffic policy.
...
## Avoid double-encapsulating WARP traffic

<Render
	file="connector/app-aware-policies/warp-traffic"
	params={{ one: "If you have the Cloudflare WARP client deployed in your premises, you should avoid double-encapsulating traffic, first by the WARP client and then by Magic WAN Connector. This is done by bypassing Magic WAN's IPsec tunnels and sending WARP traffic directly to the Internet.", two: "[Add an application](#add-an-application)" }}
/>