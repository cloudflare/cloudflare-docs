...
7. From the drop-down menu **Origin**, select your origin LAN.
8. (Optional) Specify a subnet for your first LAN in **Subnets**.
9. (Optional) In **Ports** specify the TCP/UDP ports you want to use. Valid ports are between `1` and `65000`. Zero (`0`) is not a valid port number. Add a comma to separate each of the ports or add a port range. For example, `2,5,6,9-14`.
10. In **Destination**, select the destination LAN and repeat the above process to configure it.
