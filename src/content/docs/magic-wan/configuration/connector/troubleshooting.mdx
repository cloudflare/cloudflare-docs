...
To access the performance analytics:
...
The dashboard shows this information about all the active ports in your Connector. To access port analytics:
...
- `Init`: Initialized `mcon-agent` process. This process managed Connector.
- `Leave`: Stopped `mcon-agent` process.
- `StartAttestation`: Started attestation to verify the integrity of the Connector before allowing the device to connect to your account.
...
- `StartRotatePki`: Started public kei infrastructure (PKI) rotation.
