...
This is where you add and configure your WAN connections to Connector. Each configured WAN will create one IPsec tunnel.
...
For high-capacity use cases, multiple tunnels can be established with equal priority. Outgoing traffic is then distributed across all available connections using an ECMP routing algorithm, which balances the load based. Refer to [Equal-cost multi-path routing](/magic-wan/reference/traffic-steering/#equal-cost-multi-path-routing) for more information.
