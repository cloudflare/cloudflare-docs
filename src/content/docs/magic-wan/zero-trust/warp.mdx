...
## Double-encapsulating and Magic WAN Connector

<Render
	file="connector/app-aware-policies/warp-traffic"
	params={{ one: "If you have Magic WAN Connector and the Cloudflare WARP client deployed in your premises, you should also avoid double-encapsulating traffic. This is done by setting up WARP as a breakout app in the Connector dashboard. This bypasses Magic WAN's IPsec tunnels and sends WARP traffic directly to the Internet", two:"**Breakout traffic** > [Add an application](/magic-wan/configuration/connector/network-options/application-based-policies/breakout-traffic/#add-an-application)" }}
/>
