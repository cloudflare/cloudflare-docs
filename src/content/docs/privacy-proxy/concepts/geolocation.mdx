...
Privacy Proxy uses reduced-precision geohashes (typically 4-5 characters) to locate users to a city or region without pinpointing their exact location.
...
A simple way to verify geolocation accuracy: search for "pizza near me" through the proxy. If results show pizza places in the user's actual city rather than a distant data center, geolocation is working correctly.
