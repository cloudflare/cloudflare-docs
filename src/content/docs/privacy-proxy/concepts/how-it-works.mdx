...
1. **Connect and authenticate**: The client establishes an HTTP/2 or HTTP/3 connection to Privacy Proxy and presents credentials (PSK or Privacy Pass token) in the `Proxy-Authorization` header.
2. **CONNECT request**: The client sends a CONNECT request specifying the destination hostname and port.
3. **Tunnel established**: The proxy responds with `200 OK` to confirm the tunnel is ready.
4. **Egress connection**: The proxy opens a connection to the destination using an egress IP address selected based on the client's geolocation.
5. **Data transfer**: The client sends encrypted data through the tunnel. The proxy forwards bytes without inspection.
...
For a technical deep dive into how these protocols work, refer to [A Primer on Proxies](https://blog.cloudflare.com/a-primer-on-proxies/).
