...
On deploy, Wrangler uploads your Worker, builds and pushes the container image with Docker, and sets up container instances on Cloudflare's network. The first build and push usually take the longest; later deploys [reuse cached image layers](https://docs.docker.com/build/cache/). Worker code and container instances do not always update at the same moment — see [Container deployments](/containers/deployments/) when you need the full model.
...
- Requests to `/lb` load-balance across 3 containers chosen at random.
...
- Read [Container deployments](/containers/deployments/) for what changes on deploy, version skew, and how to confirm updates
- Use [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/) when you connect the Worker to Git
- Review [Rollouts](/containers/platform-details/rollouts/) when you need gradual or immediate instance updates
- Browse [examples](/containers/examples/) for more patterns
- Check the [Frequently Asked Questions](/containers/faq/) for platform behavior and limitations
