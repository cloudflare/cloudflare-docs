...
In CI, pass the `Dockerfile` or `Containerfile` target explicitly, or run `wrangler setup --yes` before deploying. After Wrangler configures the project, use `wrangler deploy` for future deploys. Continue with the starter template to create a new Containers project.
