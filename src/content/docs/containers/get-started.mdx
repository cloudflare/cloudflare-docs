...
- The `enable_ctx_exports` compatibility flag enables [automatic loopback bindings](/changelog/2025-09-26-ctx-exports/), so you do not need to configure a Durable Object binding explicitly.
...
- Making requests to `/container/` passes requests to a new container for
  ...
  if (pathname.startsWith("/container")) {
  	const container = ctx.exports.MyContainer.getByName(pathname);
...
- Making requests to `/lb` will load balance requests across several containers.
  ...
  if (pathname.startsWith("/lb")) {
  	const container = await getRandom(ctx.exports.MyContainer, 3);
  	return await container.fetch(request);
  }
  ```
