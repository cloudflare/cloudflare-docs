...
You can manage this lifecycle through the [Durable Object Container API](/containers/api/durable-object-container/) or the higher-level [`Container` class](/containers/api/container-class/). To compare both options, refer to [Containers APIs](/containers/api/).
...
- [`onError()`](/containers/api/container-class/#onerror) — Runs when the container exits with an error.
