...
Each container instance runs in a Firecracker microVM. That VM has its own kernel and network, so the instance does not share a kernel with other workloads on Cloudflare's network. Your image is a Linux container inside the VM.

Containers should be built for the `linux/amd64` architecture, and should stay within
[size limits](/containers/platform/limits/).
