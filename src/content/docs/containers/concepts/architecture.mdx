...
Worker code goes live on deploy. Container instances update with a [rollout](/containers/reference/rollouts/). Refer to [Deploy Containers](/containers/guides/deploy/).
...
Handle `SIGTERM` in your image if you need cleanup before exit. The same sequence runs when a [rollout](/containers/reference/rollouts/) replaces a container instance with a new image.
