...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		const containerInstance = getRandom(ctx.exports.Backend, INSTANCE_COUNT)
...
```javascript
export class MyBackend extends Container {
	autoscale = true;
	defaultPort = 8080;
}
```
...
export default {
	async fetch(request, env, ctx) {
		return getContainer(ctx.exports.MyBackend).fetch(request);
	},
};
```
