...
2. **Signal stop.** The platform sends `SIGTERM` to the main process so it can stop accepting new work and finish in-flight work. Handle `SIGTERM` in your image if that process needs cleanup before exit.
...
5. **After exit.** The Container class [`onStop`](/containers/container-class/#onstop) hook can run in the Worker once the container process has exited.
