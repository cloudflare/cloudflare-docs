...
At that point, the container will be sent a `SIGTERM` signal and still has 15 minutes to shut down gracefully.
...
Because Worker code updates immediately while container instances roll out gradually, keep
changes backwards compatible across both versions until the rollout completes.
...
## Skip Container updates

To deploy Worker code changes without building or updating Containers, deploy with:
