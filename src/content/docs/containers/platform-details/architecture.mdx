...
Worker code and container instances update on different schedules. The Worker version goes live on deploy; container instances follow a [rollout](/containers/platform-details/rollouts/). For the deploy timeline, artifacts, and version skew, see [Container deployments](/containers/deployments/).
