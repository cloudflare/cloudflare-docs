...
process in the container, then `SIGKILL` after 15 minutes if it has not exited.
Handle `SIGTERM` if you need cleanup. Image updates use the same stop sequence
during a [rollout](/containers/platform-details/rollouts/).
