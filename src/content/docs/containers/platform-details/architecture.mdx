...
When a container instance is going to be shut down (including during a
[rollout](/containers/platform-details/rollouts/)), the platform sends `SIGTERM`
to the main process in the container, then `SIGKILL` after 15 minutes if it has
not exited. Handle `SIGTERM` if you need to finish in-flight work. While a
replacement instance starts, requests that need it may wait until the container
is ready, or fail if a timeout is shorter than startup.
