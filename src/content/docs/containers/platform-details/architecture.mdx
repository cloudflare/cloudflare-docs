...
Worker code goes live on deploy. Container instances update with a [rollout](/containers/platform-details/rollouts/). Refer to [Deploy Containers](/containers/deploy/).
...
The Container class sets [`sleepAfter`](/containers/container-class/#sleepafter) to 10 minutes by default. Its default [`onActivityExpired()`](/containers/container-class/#onactivityexpired) implementation signals the container to stop after that period without activity. You can change the duration or override the hook.

You can stop a container instance yourself with [`stop()`](/containers/container-class/#stop) or [`destroy()`](/containers/container-class/#destroy).
...
Handle `SIGTERM` in your image if you need cleanup before exit. The same sequence runs when a [rollout](/containers/platform-details/rollouts/) replaces a container instance with a new image.
