...
- **Per-host and per-instance rules.** Combine `outboundByHost` with `ctx.containerId` to scope credentials or permissions to a specific container instance, or use [per-instance handlers](#per-instance-handlers) to pass credentials directly into a specific instance's handler.
...
Set outbound handlers on a specific container instance using `setOutbound()` or `setOutboundByHost()`. Both accept an optional final argument that populates `ctx.params` inside the handler, letting you pass per-instance configuration such as credentials or feature flags without sharing them across every instance.

Instance-level handlers take precedence over class-level `outbound` and `outboundByHost` handlers (see [Handler precedence](#handler-precedence)).

Define a named handler on the class that reads `ctx.params`:
...
MyContainer.outboundHandlers = {
	authenticatedApi: async (request, env, ctx) => {
		const requestWithAuth = new Request(request);
		requestWithAuth.headers.set("x-auth-token", ctx.params.key);
		return fetch(requestWithAuth);
	},
};
```

Use `setOutboundByHost()` to apply the handler to a specific host on each instance. Each instance gets its own credential via `ctx.params`:

```js
export default {
	async fetch(request, env) {
		const instanceA = await env.MY_CONTAINER.getByName("instance-a");
		const instanceB = await env.MY_CONTAINER.getByName("instance-b");

		await instanceA.setOutboundByHost("api.example.com", "authenticatedApi", {
			key: "key-for-instance-a",
		});
		await instanceB.setOutboundByHost("api.example.com", "authenticatedApi", {
			key: "key-for-instance-b",
		});
...
Or use `setOutbound()` to apply the handler as a catch-all on each instance:
...
export default {
	async fetch(request, env) {
		...
		await instanceA.setOutbound("authenticatedApi", {
			key: "key-for-instance-a",
		});
		await instanceB.setOutbound("authenticatedApi", {
			key: "key-for-instance-b",
		});
...
Use this pattern to pass per-instance proxy credentials, tenant identifiers, or any other information scoped to a single container instance.
