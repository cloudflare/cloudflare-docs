...
You can filter outbound traffic with the `allowHosts` and `denyHosts` properties on the Container class.

By default, a Container will allow internet access, and you can set `denyHosts` to
...
export class MyContainer extends Container {
	...
	interceptHttps = true;
	denyHosts = ["some-nefarious-website.com", "141.101.64.0/18"];
}
```
...
export class MyContainer extends Container {
	...
	// default internet access to off unless overridden by 'allowHosts' or outbound proxy
	enableInternet = false;

	// overrides enableInternet = false
	allowHosts = ["allowed.com"];
}
```
...
The `outboundByHost` configuration takes precedence over the `allowHosts` and `denyHosts` lists
as well as catch-all `outbound` handlers.
