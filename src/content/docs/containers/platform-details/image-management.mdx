...
On subsequent deploys, Wrangler will only push image layers that have changed, which saves space and time. Image build and push run after the Worker upload on a full deploy. For that timeline and for commands that skip image publish, see [Deploy Containers](/containers/deploy/).
