...
When a container instance shuts down, the platform sends `SIGTERM` to the main
process, then `SIGKILL` after 15 minutes if needed. Handle `SIGTERM` if you need
cleanup. The instance starts again elsewhere shortly after. See also
