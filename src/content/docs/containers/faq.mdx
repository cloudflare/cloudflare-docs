...
On `wrangler deploy`, the Worker goes live first. Container instances update with a gradual rollout by default. Refer to [Deploy Containers](/containers/deploy/) and [Rollouts](/containers/platform-details/rollouts/).
...
When a container instance is going to be shut down, the platform sends `SIGTERM`
to the main process in the container, then `SIGKILL` after 15 minutes if it has
not exited. Handle `SIGTERM` if you need cleanup. The instance is started again
elsewhere shortly after. The same stop sequence is used during image
