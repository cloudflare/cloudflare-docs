...
Cloudflare does not stop a container instance after a fixed maximum runtime. If you do not set `sleepAfter` on your Container class, and you do not stop the instance yourself, it keeps running until something else stops it. One of those cases is a host server restart. That happens on an irregular cadence. Cloudflare does not guarantee that any container instance will run for any set period of time.
