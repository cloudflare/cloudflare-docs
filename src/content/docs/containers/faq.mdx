...
On `wrangler deploy`, the Worker goes live first. Container instances update with a gradual rollout by default. Refer to [Rollouts](/containers/configuration/rollouts/) for steps, grace periods, and modes. Refer to [Deploy Containers](/containers/guides/deploy/) to run a deploy.

## How do Workers Builds work with Containers?

On the production branch, Workers Builds should run `wrangler deploy` so images and container instances can update. Non-production Workers Builds defaults to `wrangler versions upload`, which does not update images. Containers Workers implement Durable Objects, so preview URLs are not generated for them. Refer to [Deploy Containers](/containers/guides/deploy/#before-production).
...
Refer to [scaling and routing](/containers/configuration/scaling-and-routing/) for details.
...
Refer to [image management](/containers/guides/image-management/#use-pre-built-container-images).
...
To keep the disk, save it with [`snapshotContainer()`](/durable-objects/api/container/#snapshotcontainer)
and start a later instance from the snapshot. Snapshots are in public beta and
require `scheduling_policy: "durable_object"`. For an example, refer to
...
Image updates during a deploy use the same stop sequence. Refer to [Rollouts](/containers/configuration/rollouts/).
...
Yes. Use the `docker:dind` image, and start the Docker daemon with iptables and IP forwarding disabled:

```dockerfile title="Dockerfile"
FROM docker:dind

# Start dockerd with iptables and IP forwarding disabled, then run your app
ENTRYPOINT ["sh", "-c", "dockerd-entrypoint.sh dockerd --iptables=false --ip6tables=false --ip-forward=false & exec /path/to/your-app"]
```

Containers do not support iptables manipulation. Containers with `scheduling_policy: "durable_object"` also cannot turn on IP forwarding, so without `--ip-forward=false` the Docker daemon exits on them. Rootless Docker, which the `docker:dind-rootless` image runs unless its Dockerfile switches to `USER root`, does not start in Containers, because it cannot set up its runtime directory or its network.
...
For a complete working example, see the [Docker-in-Docker Containers example](https://github.com/th0m/containers-dind). It runs the Docker daemon as `root` from `docker:dind-rootless`. On containers with `scheduling_policy: "durable_object"`, add `--ip-forward=false` to its `dockerd` flags.
