...
## How do I connect a container to private services in my VPC?

You can use outbound Workers combined with [Workers VPC](/workers-vpc/) to connect containers to private endpoints in AWS, Azure, GCP, or on-premise networks that are not exposed to the public internet.

Define `static outboundByHost` on your `Container` class to intercept outbound HTTP requests by hostname and route them through a Workers VPC Service binding:
...
class MyContainer extends Container {
	...
	static outboundByHost = {
		"my-private-api": async (request, env, ctx) => {
...
Inside the container, your application makes a standard HTTP request to `http://my-private-api/data` — no special SDK required.

:::note
Outbound interception currently works with HTTP only. HTTPS interception from the container is not yet supported.
:::

For a complete walkthrough, refer to the [Connect from Containers to private services](/workers-vpc/examples/connect-from-containers/) example.
