...
ENTRYPOINT ["sh", "-c", "dockerd-entrypoint.sh dockerd --iptables=false --ip6tables=false & exec /your/app"]
```

Wait for dockerd to become ready before your application starts using Docker:
...
For a complete working example, see the [containers-dind repository](https://github.com/th0m/containers-dind).
