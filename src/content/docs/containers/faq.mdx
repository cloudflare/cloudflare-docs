...
Production Builds should run `wrangler deploy` so images and container instances can update. Non-production builds default to `wrangler versions upload`, which does not update images. Containers Workers use Durable Objects, so preview URLs are not generated for them. Refer to [Deploy Containers](/containers/deploy/#before-production).
