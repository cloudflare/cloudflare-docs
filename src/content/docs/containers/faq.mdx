...
On `wrangler deploy`, the Worker goes live first. Container instances update with a gradual rollout by default. Refer to [Rollouts](/containers/platform-details/rollouts/) for steps, grace periods, and modes. Refer to [Deploy Containers](/containers/deploy/) to run a deploy.

## How do Workers Builds work with Containers?

On the production branch, Workers Builds should run `wrangler deploy` so images and container instances can update. Non-production Workers Builds defaults to `wrangler versions upload`, which does not update images. Containers Workers implement Durable Objects, so preview URLs are not generated for them. Refer to [Deploy Containers](/containers/deploy/#before-production).
...
Refer to [image management](/containers/platform-details/image-management/#use-pre-built-container-images).
...
Image updates during a deploy use the same stop sequence. Refer to [Rollouts](/containers/platform-details/rollouts/).
