...
Workers Builds runs your deploy command. Production defaults to `wrangler deploy`. Non-production defaults to `wrangler versions upload`, which does not update container images. Refer to [Deploy Containers](/containers/deploy/#deploy-with-workers-builds).
