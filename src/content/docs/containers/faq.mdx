...
Production Builds that run `wrangler deploy` can publish images and roll out instances. Preview builds default to `wrangler versions upload`, which changes Worker code only and keeps the last deployed image. Refer to [Deploy Containers](/containers/deploy/#preview-versions-and-containers).
