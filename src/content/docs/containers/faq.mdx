...
Production Builds that run `wrangler deploy` can publish images and roll out container instances. Non-production builds default to `wrangler versions upload`, which does not update images. Containers Workers use Durable Objects, so preview URLs are not generated for them. Refer to [Deploy Containers](/containers/deploy/#feature-branches-and-versions-upload).
