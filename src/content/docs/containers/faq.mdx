...
On `wrangler deploy`, the Worker goes live first. Container instances update with a gradual rollout by default. Refer to [Rollouts](/containers/reference/rollouts/) for steps, grace periods, and modes. Refer to [Deploy Containers](/containers/guides/deploy/) to run a deploy.
...
Refer to [scaling and routing](/containers/reference/scaling-and-routing/) for details.
...
Image updates during a deploy use the same stop sequence. Refer to [Rollouts](/containers/reference/rollouts/).
...
Refer to [Handle outbound traffic](/containers/concepts/outbound-traffic/) for how to control outbound traffic and internet access.
