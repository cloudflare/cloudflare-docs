...
`wrangler deploy` updates Worker code when the new version becomes active, then builds or resolves images and rolls out container instances. Instances default to a gradual rollout, so Worker code and images can temporarily disagree. See [Container deployments](/containers/deployments/) for the model and [Rollouts](/containers/platform-details/rollouts/) for steps, grace periods, and flags.

## How do Workers Builds work with Containers?

Workers Builds runs your deploy command. Production defaults to `wrangler deploy` (full container path). Non-production defaults to `wrangler versions upload` (Worker version only; no image build or instance rollout). See [Deploy Containers with Workers Builds](/containers/deploy-with-workers-builds/).
...
See [image management](/containers/platform-details/image-management/#use-pre-built-container-images). For when the image updates relative to Worker code on deploy, see [Container deployments](/containers/deployments/).
