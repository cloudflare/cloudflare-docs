...
This guide uses the higher-level `Container` class. You can also manage containers directly through `ctx.container`. To compare both options, refer to [Containers APIs](/containers/api/).
...
environment variable](/containers/configuration/environment-variables/) `CLOUDFLARE_DEPLOYMENT_ID.`
...
- Refer to [Deploy Containers](/containers/guides/deploy/) for Workers Builds and rollout behavior
