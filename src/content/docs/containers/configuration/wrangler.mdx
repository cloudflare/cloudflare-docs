...
The `class_name` in `containers` and `durable_objects.bindings`, and the class listed in `migrations.new_sqlite_classes`, must match the exported Durable Object class in your Worker.
