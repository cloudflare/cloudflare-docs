...
If your application uses the `Container` class or Sandbox SDK, refer to [Migrate to the Durable Object Container API](/containers/guides/migrate-to-durable-object-container-api/).
...
- `options` (optional): An object with the existing startup options:
  - `env`: Environment variables to pass to the container.
  - `entrypoint`: The command and arguments to run in the container.
  - `enableInternet`: Whether to allow outbound Internet access.
