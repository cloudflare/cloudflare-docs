...
Each [container](/containers/) is managed by a Durable Object. The Durable Object manages routing and persistent state. The container process runs your image inside a Linux VM.
...
You can instead use the [`Container` class](/containers/api/container-class/) from `@cloudflare/containers`. The class adds routing, readiness checks, lifecycle hooks, activity tracking, and scheduling. To compare both APIs, refer to [Containers APIs](/containers/api/).
