...
If you already have a `Dockerfile` or `Containerfile`, Wrangler can generate a singleton Containers Worker for you:
...
In CI, pass the `Dockerfile` or `Containerfile` target explicitly with `--experimental-auto-config-containers`, or use `wrangler setup --yes --experimental-auto-config-containers` before deploying.
