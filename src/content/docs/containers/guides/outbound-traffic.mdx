...
- [Connect to Workers bindings](/containers/configuration/workers-connections/) like KV, R2, and Durable Objects
...
When `enableInternet` is `false`, only traffic you explicitly allow later on this page through `allowedHosts` or outbound handlers can leave the container. Only ports `80` and `443` are available. DNS answers `A` and `AAAA` lookups for intercepted hostnames with a placeholder address, which routes the request to your Worker. Other lookups time out.
...
:::caution
`deniedHosts` applies to HTTP on port `80` and, with `interceptHttps = true`, HTTPS on port `443`. While `enableInternet` is `true`, connections to other ports skip it. To block a destination on every port, set `enableInternet = false` and allow only what the container needs.
:::
...
If you set `enableInternet = false`, that traffic is denied. DNS is answered inside the container network instead of by a public resolver. `A` and `AAAA` lookups for intercepted hostnames get a placeholder address, and other lookups time out. Queries never leave the container network, so DNS cannot carry data out.
...
or `interceptAllOutboundHttp` for all HTTP on port `80`. Both accept a `WorkerEntrypoint`.
...
- [Call an authenticated API from a sandbox](/sandbox/network/call-an-authenticated-api/) — Add a credential to a sandbox's requests in an outbound handler
