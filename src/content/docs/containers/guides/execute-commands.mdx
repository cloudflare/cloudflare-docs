...
Use `cwd`, `env`, and `user` to set the process context. The process receives only the variables in `env`, plus the image's `PATH`. It does not inherit `envVars` or other variables set in the image. To give a command a value from `envVars`, pass it in `env` as well.
...
The `user` option sets the numeric Linux user and group IDs for the process, as `uid:gid`. When omitted, the process runs as the image user. For current limitations, refer to [`exec()`](/durable-objects/api/container/#exec).
...
The signal reaches only the process that `exec()` started. When that process starts others, such as the commands in a `bash -c` script, they keep running after `exitCode` resolves, and `output()` waits until they exit. To stop a command and every process it started, run it under GNU coreutils `timeout`. The image must include `timeout`.
...
// The exit code is 124 when the command runs longer than 30 seconds.
console.log(output.exitCode);
```

`timeout` sends `SIGTERM` to the command and every process it started. With `--kill-after=5`, it sends `SIGKILL` five seconds later if they are still running.
