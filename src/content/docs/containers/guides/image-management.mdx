...
:::note
The `image` examples on this page apply to Container applications that use the [`default` scheduling policy](/containers/configuration/scheduling-policy/). For the `durable_object` policy, configure a named `images` map and select an image from `ctx.container.images` at runtime. A named `image` entry must be a digest-pinned reference in the Cloudflare managed registry; a named `dockerfile` entry can build from an external base image.
:::
