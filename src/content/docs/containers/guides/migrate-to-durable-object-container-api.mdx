...
Use the Durable Object Container API when you want direct control over a container's lifecycle. The API is available as `this.ctx.container` inside a Durable Object with a container binding. It does not provide all the helpers of the `Container` class.
