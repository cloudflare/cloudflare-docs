...
You can pass environment variables when the Durable Object Container API starts a Container, or through the `envVars` field on the [`Container`](/containers/api/container-class/) class.
...
Copy the store ID returned by this command. The following commands and the Wrangler configuration require the ID, not the store name.

<PackageManagers
	type="exec"
	pkg="wrangler"
	args="secrets-store secret create <STORE_ID> --name SECRET_STORE_SECRET --scopes workers --remote"
/>
...
<PackageManagers
	type="exec"
	pkg="wrangler"
	args="kv key put --namespace-id <KV_NAMESPACE_ID> --remote KV_VALUE 'Hello from KV!'"
/>

Replace `<KV_NAMESPACE_ID>` with the ID returned by `kv namespace create`. The `--namespace-id` flag works before you add the binding to your Wrangler configuration; `--remote` stores the value in the namespace used by the deployed Worker.
...
Pass synchronous Worker variables and secrets when the Container starts.
...
export class MyContainer extends DurableObject {
	launch(env: Record<string, string>): void {
		...
		this.ctx.container!.start({ enableInternet: true, env });
	}
}
...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		await Promise.all([
			env.MY_CONTAINER.getByName("foo").launch({
				ENV_VAR: `${env.ENV_VAR}foo`,
				WORKER_SECRET: env.WORKER_SECRET,
				SECRET_STORE_SECRET: secretStoreSecret,
				KV_VALUE: kvValue,
			}),
			env.MY_CONTAINER.getByName("bar").launch({
