...
export default {
	async fetch(request, env, ctx) {
		if (new URL(request.url).pathname === "/launch-instances") {
			let instanceOne = ctx.exports.MyContainer.getByName("foo");
			let instanceTwo = ctx.exports.MyContainer.getByName("bar");
...
export default {
	async fetch(request, env, ctx) {
		if (new URL(request.url).pathname === "/configure-container") {
			...
			let container = ctx.exports.MyContainer.getByName("configured");
...
export default {
	async fetch(request, env, ctx) {
		if (new URL(request.url).pathname === "/launch-with-features") {
			...
			let container = ctx.exports.MyContainer.getByName("features");
