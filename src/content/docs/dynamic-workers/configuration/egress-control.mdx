...
Both `fetch()` and `connect()` throw exceptions inside the dynamic Worker.

Start here if the dynamic Worker does not need network access. You can always open access later by switching to a service binding.
