...
Now we can load a Dynamic Worker and give it a Chat Room. To create the chat room RPC stub, we use [`ctx.exports`](http://localhost:1111/workers/runtime-apis/context#exports), then we simply pass it into the Dynamic Worker Loader in the `env` object:
