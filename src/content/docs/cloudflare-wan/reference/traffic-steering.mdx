...
When using Cloudflare WAN and Cloudflare Tunnel together, consider the IP ranges utilized in the static routes of Cloudflare Tunnel when selecting static routes for Cloudflare WAN. For more information, refer to [Cloudflare Tunnel](/cloudflare-wan/zero-trust/cloudflare-tunnel/).
...
The priority value for static routes is directly configured as part of the route object in the Cloudflare [dashboard or through the API](/cloudflare-wan/configuration/how-to/configure-routes/#create-a-static-route). For example:
...
| [Cloudflare source IPs](/cloudflare-wan/configuration/how-to/configure-cloudflare-source-ips/)                                                            | Generally available.                                                                               |
...
Configure scoping for your traffic in the **Region code** section when adding or editing a static route. Refer to [Create a static route](/cloudflare-wan/configuration/how-to/configure-routes/#create-a-static-route) and [Edit a static route](/cloudflare-wan/configuration/how-to/configure-routes/#edit-a-static-route) for more information.
...
You need to enable [legacy health checks](/cloudflare-wan/reference/tunnel-health-checks/#legacy-bidirectional-health-checks) alongside BGP. This is essential to determine if a specific Cloudflare data center is reachable from your device. [Tunnel health checks](/cloudflare-wan/reference/tunnel-health-checks/) modify the route priorities for dynamically learned BGP routes.
