...
Cloudflare Mesh connects your services and devices with post-quantum encrypted networking. Every enrolled device and mesh node receives a private [Mesh IP](/cloudflare-one/networks/connectors/cloudflare-mesh/concepts/#mesh-ips) and can communicate with any other participant over TCP, UDP, or ICMP — including device-to-device without any infrastructure.

Mesh nodes run the Cloudflare One Client (`warp-cli`) in headless mode on Linux servers. They can advertise [CIDR routes](/cloudflare-one/networks/connectors/cloudflare-mesh/features/routes/) to make subnets behind them reachable, enabling connectivity to devices that cannot run the client (IoT, printers, legacy servers). All traffic preserves source IP addresses end-to-end.
...
:::note
Cloudflare Mesh supports [high availability](/cloudflare-one/networks/connectors/cloudflare-mesh/features/high-availability/) with active-passive replicas for nodes with CIDR routes.
:::
...
| **High availability** | [Active-passive replicas](/cloudflare-one/networks/connectors/cloudflare-mesh/features/high-availability/) for nodes with routes | Supported through multiple connectors per site |
