...
2. Make sure you have the following settings:
   1. **Interface address**: Add the upper IP address within the first `/31` subnet selected in step 4 of the Start Azure Configuration section. Refer to [Tunnel endpoints](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/) for more details.
...
Choose the following settings. These settings have been tested by Cloudflare. However, when setting up your VPN connection note that there are other configuration parameters are also technically feasible, as documented in the [Azure documentation](https://learn.microsoft.com/en-us/azure/virtual-wan/virtual-wan-ipsec) and in the [Cloudflare documentation](/cloudflare-wan/reference/gre-ipsec-tunnels/#supported-configuration-parameters).
