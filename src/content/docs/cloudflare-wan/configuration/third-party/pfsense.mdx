...
Use the Cloudflare dashboard or API to [configure two IPsec tunnels](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/#add-tunnels). This guide uses the settings mentioned below for the IPsec tunnels throughout the remainder.

### Add IPsec tunnels

1. Follow the [Add tunnels](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/#add-tunnels) instructions to create the required IPsec tunnels with the following options:
...
:::note
If site-to-site traffic is a requirement, enable replay protection. Refer to [Add tunnels](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/#add-tunnels) > IPsec tunnel to learn how to enable this feature.
:::
...
Create a [static route](/cloudflare-wan/configuration/how-to/configure-routes/#create-a-static-route) for each of the two IPsec tunnels configured in the previous section, with the following settings (settings not mentioned here can be left with their default values):
