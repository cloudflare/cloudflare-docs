...
- **Anti-Replay Protection**: Cloudflare recommends [disabling Anti-Replay Protection](/cloudflare-wan/reference/anti-replay-protection/) for optimal performance with Cloudflare WAN. The Cisco Meraki MX platform does not permit administrators to disable this feature.
...
- **MSS Clamping**: Cloudflare recommends specific Maximum Segment Size (MSS) [clamping](/cloudflare-wan/reference/mtu-mss/#mss-clamping) values to account for IPsec overhead and prevent fragmentation.
...
- This configuration requires the [Unified Routing](/cloudflare-wan/reference/traffic-steering/#unified-routing-mode-beta) dataplane to support [Automatic Return Routing](/cloudflare-wan/reference/traffic-steering/#automatic-return-routing-beta).
...
- You understand the importance of [MSS clamping](/cloudflare-wan/reference/mtu-mss/#mss-clamping) and adjusting it based on the traffic flows traversing the Cloudflare WAN IPsec tunnels.
...
As `192.0.2.3/32` falls outside the traditional [RFC 1918](https://datatracker.ietf.org/doc/html/rfc1918) address space, you must add it to the [Unified Routing](/cloudflare-wan/reference/traffic-steering/#unified-routing-mode-beta) dataplane associated with your Cloudflare account.
...
Return traffic from Cloudflare WAN is steered based on the Cloudflare virtual network routing table (refer to [Traffic steering](/cloudflare-wan/reference/traffic-steering/) for details). Routes are specified based on the MX LAN prefix and corresponding IPsec tunnels.
...
- Using unsupported Phase 1 or Phase 2 encryption or integrity settings — look for messages indicating `No proposal chosen`.
  ...
  - Refer to [Supported configuration parameters](/cloudflare-wan/reference/gre-ipsec-tunnels/#supported-configuration-parameters).
- IKE/IPsec identity: local or remote identity not defined or with incorrect values.
  - Refer to the [Palo Alto third-party integration guide](/cloudflare-wan/configuration/third-party/palo-alto/) for an example of FQDN-based local identification.
- Authentication failures: wrong pre-shared key.

Refer to [Configure tunnel endpoints](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/#add-tunnels) for more details.
