...
You can connect through [Generic Routing Encapsulation (GRE) or IPsec tunnels](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/) to Cloudflare WAN.
...
5. In **Gateway settings**, make sure you have the following settings:
   ...
   - **Local ID type**: Add the [IKE ID](/cloudflare-wan/reference/gre-ipsec-tunnels/#supported-ike-id-formats) provided by Cloudflare.
...
You must use an interface address from the `/31` subnet required to [configure tunnel endpoints](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/) on Cloudflare WAN.
...
3. Edit the interface to assign an address from the `/31` subnet required to [configure tunnel endpoints](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/). When you are finished, it should look similar to the following:
...
The Cloudflare dashboard monitors the health of all anycast tunnels on your account that route traffic from Cloudflare to your origin network. Refer to [Check tunnel health in the dashboard](/cloudflare-wan/configuration/common-settings/check-tunnel-health-dashboard/) for more information.
