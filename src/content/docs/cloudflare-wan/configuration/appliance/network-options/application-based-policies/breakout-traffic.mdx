...
- **Custom applications** — applications you define by **hostname**, **IP subnet**, and/or **source subnet**. Custom applications can be created, edited, and deleted directly from the dashboard, or through the [`managed_app_id`](/api/resources/magic_transit/subresources/apps/methods/create/) endpoint.
...
| Source CIDR / IP range | All traffic with a source IP in the specified range is broken out. Accepts a single IP, a range, or a CIDR block.   | Dashboard (**Source subnets** field on a [custom application](#create-edit-or-delete-a-custom-application)), API, and Terraform.     |
