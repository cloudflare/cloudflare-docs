...
:::note
To access more than one hour of tunnel health data, you should use the [GraphQL API](/cloudflare-wan/analytics/query-tunnel-health/).
:::
...
4. Find the tunnel you want to inspect, select the three dots next to it, and select:
	- **Create alert**: Opens the [notifications wizard](/cloudflare-wan/configuration/common-settings/configure-tunnel-health-alerts/) so you can create specific alerts for that tunnel when specific conditions are met.
...
- Network Analytics: Leverage real-time insights into [network analytics](/cloudflare-wan/analytics/network-analytics/).
...
Refer to [Heartbeat](/cloudflare-wan/configuration/appliance/maintenance/heartbeat/) to learn more.

## Troubleshooting

If you received a tunnel health alert but are unsure whether it affects your traffic, refer to [Troubleshoot connectivity](/cloudflare-wan/troubleshooting/connectivity/) to determine whether the alert is relevant.
