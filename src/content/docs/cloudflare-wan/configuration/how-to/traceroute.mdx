...
If you have a Cloudflare WAN (formerly Magic WAN) client connected through [GRE](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/), [IPsec](/cloudflare-wan/configuration/how-to/configure-tunnel-endpoints/), [CNI](/network-interconnect/) or [WARP](/cloudflare-wan/zero-trust/cloudflare-one-client/) and want to perform a `traceroute` to an endpoint behind a [Cloudflare Tunnel](/cloudflare-wan/zero-trust/cloudflare-tunnel/), the following settings must be applied for the command to return useful information.
