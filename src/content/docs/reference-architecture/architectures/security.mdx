...
2.  [Protecting public resources](#protecting-public-resources)
    ...
    2.  [Cloudflare application security products](#cloudflare-application-security-products)
        ...
        7. [Client-Side Security](/client-side-security/) (formerly known as Page Shield)
        8. [SSL/TLS](#ssltls)
...
Client-side security [Content Security policies](/client-side-security/policies/) can be created and applied to add an additional level of security that helps detect and mitigate certain types of attacks, including:
...
| [WARP Connector](/cloudflare-one/networks/connectors/cloudflare-tunnel/private-net/warp-connector/)           | Software agent deployed on servers for creating a tunnel for incoming and outgoing connections to private applications or networks.                          | Similar to cloudflared, but supports East to West traffic and is often used in place of Cloudflare WAN when there is no ability to create an IPsec tunnel from existing devices.    |
