...
1. Client request is sent to the closest Cloudflare Data Center via anycast ensuring low latency. Via LLM Discovery, Cloudflare detects LLM or AI Specific traffic by looking at LLM specific heuristics. Discovered LLM endpoints are automatically labeled with the `cf-llm` label.
...
AI Security for Apps detects attempts to manipulate, misuse, or elicit unintended outputs. An Injection score signifying the likeliness of a prompt injection or jailbreak attempt is given to every request that is routed to an LLM endpoint. A score of less than 20 signifies a prompt injection attack.
...
AI is powerful and organizations continue to adopt AI at a rapid pace, but without protections in place, it's risky. Cloudflare provides a layered security approach incorporating AI Security to secure your AI-powered applications.
