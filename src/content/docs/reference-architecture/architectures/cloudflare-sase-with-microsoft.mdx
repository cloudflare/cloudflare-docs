...
- Using Cloudflare's [Secure Web Gateway](/cloudflare-one/traffic-controls/) to control access to Microsoft SaaS applications such as Outlook, OneDrive and Teams
...
- With a device connected using our [agent](/cloudflare-one/team-and-resources/devices/cloudflare-one-client/), Cloudflare's ZTNA service can leverage the enhanced telemetry and context provided by Intune regarding a user's device posture and compliance state.
...
- [Set up Microsoft Entra ID (formerly Azure Active Directory) as an identity provider](/cloudflare-one/integrations/identity-providers/entra-id/#set-up-entra-id-as-an-identity-provider)
