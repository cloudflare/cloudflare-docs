...
reviewed: 2024-11-14
description: >-
  Magic WAN Connector is a virtual (deployed as a VM on a hypervisor) device. It automatically on-ramps traffic for a local network to Cloudflare, and replaces existing, difficult to manage edge hardware. The first decision for a Magic WAN. Connector deployment is its location in the network, and this relates to. whether the organization wants to keep the existing Customer Premises Equipment (CPE, edge. router or firewall at a site)
---
