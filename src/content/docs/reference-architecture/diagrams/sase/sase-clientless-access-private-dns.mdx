...
3. Cloudflare looks up the internal hostname using [resolver policies](/cloudflare-one/traffic-policies/policy-types/resolver-policies/), and gets the private IP address from the internal DNS server. This DNS resolution takes place within the Cloudflare network and requires no DNS client changes on the user's device.
