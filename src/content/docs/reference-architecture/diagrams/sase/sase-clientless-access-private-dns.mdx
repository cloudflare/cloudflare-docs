...
reviewed: 2024-04-03
description: >-
  Cloudflare Reference Architecture docs: Access to private apps without having to deploy client agents. Using Cloudflare to access private resources - such as applications, servers, and networks that are not exposed directly to the internet - usually involves deploying an (agent) to devices. This document describes an alternative approach which removes the need to deploy software to the user's device.
---
