...
2. **Request proxying**: Forward `POST` request to AI Inference provider or serve response from [cache, if enabled and available](/ai-gateway/features/caching). During this process, both [analytics](/ai-gateway/observability/analytics/) and [logs](/ai-gateway/observability/logging/) are collected. Additionally, controls such as Rate Limiting are enforced.
...
- [AI Gateway: Supported Providers](/ai-gateway/usage/providers/)
