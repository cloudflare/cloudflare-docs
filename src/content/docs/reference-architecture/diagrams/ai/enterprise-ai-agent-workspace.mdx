...
The architecture uses [Cloudflare Workers](/workers/) and the [Agents SDK](/agents/) for orchestration, [Durable Objects](/durable-objects/) for state, [AI Gateway](/ai-gateway/) for model governance, and [MCP server portals](/cloudflare-one/access-controls/ai-controls/mcp-portals/) for enterprise tools. [Dynamic Workers](/dynamic-workers/), [Sandbox SDK](/sandbox/sdk/) containers, and [Browser Run](/browser-run/) handle work that needs more than model inference.
