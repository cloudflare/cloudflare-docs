...
reviewed: 2024-12-19
description: >-
  This document focuses on the reference architecture of using Cloudflare WAN, Cloudflare Network Firewall, and Cloudflare Gateway services.
---
...
3. Outbound Internet traffic is subsequently sent to [Cloudflare Gateway](/cloudflare-one/traffic-controls/), our secure web gateway service where various [policies](/cloudflare-one/traffic-controls/) enforce a comprehensive set of security and control measures on the outbound traffic, ensuring the utmost protection for your networks. For example, Gateway DNS and HTTP policies can both be configured to prevent your servers from connecting to questionable Internet sites and from downloading malware or other malicious content.
4. Once traffic clears inspection, Gateway proxies the outbound traffic to their destinations on the Internet. The source IP addresses of the outbound traffic are the Cloudflare owned IP addresses associated with the Gateway service, which if you want you can purchase and set your [own egress IP](/cloudflare-one/traffic-controls/egress-policies/dedicated-egress-ips/)
...
3. After clearing Cloudflare Network Firewall, traffic is subsequently routed to [Cloudflare Gateway](/cloudflare-one/traffic-controls/), our secure web gateway service.
4. Cloudflare Gateway subjects traffic to any configured L3-7 [policies](/cloudflare-one/traffic-controls/) that enforce a comprehensive set of security and control measures, ensuring the utmost protection for your networks. Once traffic clears inspection, Gateway proxies the traffic to its destination private network. The source IP addresses of the proxied traffic are the Cloudflare owned IP addresses associated with the Gateway service.
...
2. Upon entering the Cloudflare network, outbound Internet traffic is first routed through Cloudflare Network Firewall where it is subject to any configured network firewall policies.
3. Traffic that clears Cloudflare Network Firewall is subsequently sent to [Cloudflare Gateway](/cloudflare-one/traffic-controls/), our secure web gateway service where any configured L3-7 [policies](/cloudflare-one/traffic-controls/) enforce a comprehensive set of security and control measures on the outbound traffic, ensuring the utmost protection for your networks.
...
- [Cloudflare Cloudflare Network Firewall](/cloudflare-network-firewall/)
- [Cloudflare WAN](/cloudflare-wan/)
- [Cloudflare Gateway](/cloudflare-one/traffic-controls/)
- [Integration of Cloudflare Magic services and Cloudflare Gateway](/cloudflare-wan/zero-trust/cloudflare-gateway/)
