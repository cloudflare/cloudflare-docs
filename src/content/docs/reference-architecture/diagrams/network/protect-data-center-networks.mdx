...
3. Outbound Internet traffic is subsequently sent to [Cloudflare Gateway](/cloudflare-one/traffic-policies/gateway/), our secure web gateway service where various [policies](/cloudflare-one/traffic-policies/gateway/) enforce a comprehensive set of security and control measures on the outbound traffic, ensuring the utmost protection for your networks. For example, Gateway DNS and HTTP policies can both be configured to prevent your servers from connecting to questionable Internet sites and from downloading malware or other malicious content.
4. Once traffic clears inspection, Gateway proxies the outbound traffic to their destinations on the Internet. The source IP addresses of the outbound traffic are the Cloudflare owned IP addresses associated with the Gateway service, which if you want you can purchase and set your [own egress IP](/cloudflare-one/traffic-policies/gateway/egress-policies/dedicated-egress-ips/)
...
3. After clearing Magic Firewall, traffic is subsequently routed to [Cloudflare Gateway](/cloudflare-one/traffic-policies/gateway/), our secure web gateway service.
4. Cloudflare Gateway subjects traffic to any configured L3-7 [policies](/cloudflare-one/traffic-policies/gateway/) that enforce a comprehensive set of security and control measures, ensuring the utmost protection for your networks. Once traffic clears inspection, Gateway proxies the traffic to its destination private network. The source IP addresses of the proxied traffic are the Cloudflare owned IP addresses associated with the Gateway service.
...
2. Upon entering the Cloudflare network, outbound Internet traffic is first routed through Magic Firewall where it is subject to any configured network firewall policies.
3. Traffic that clears Magic Firewall is subsequently sent to [Cloudflare Gateway](/cloudflare-one/traffic-policies/gateway/), our secure web gateway service where any configured L3-7 [policies](/cloudflare-one/traffic-policies/gateway/) enforce a comprehensive set of security and control measures on the outbound traffic, ensuring the utmost protection for your networks.
...
- [Cloudflare Gateway](/cloudflare-one/traffic-policies/gateway/)
