...
reviewed: 2024-12-19
description: >-
  This document focuses on the reference architecture of using Cloudflare's Magic WAN, Cloudflare Network Firewall, and Cloudflare Gateway services.
---
...
4. Traffic that passes DDoS mitigation is subjected to additional network firewall filtering using [Cloudflare Network Firewall](/cloudflare-network-firewall/).
...
3. Outbound Internet traffic is subsequently sent to [Cloudflare Gateway](/cloudflare-one/traffic-policies/), our secure web gateway service where various [policies](/cloudflare-one/traffic-policies/) enforce a comprehensive set of security and control measures on the outbound traffic, ensuring the utmost protection for your networks. For example, Gateway DNS and HTTP policies can both be configured to prevent your servers from connecting to questionable Internet sites and from downloading malware or other malicious content.
...
The reference architecture diagrams below illustrate how Cloudflare services — Magic WAN, Cloudflare Network Firewall and Cloudflare Gateway — can be used to protect site-to-site, inter-data center traffic between your private networks.
...
2. Upon entering the Cloudflare network, traffic is routed through Network Firewall.
3. Network Firewall subjects traffic to any configured network firewall policies.
4. Traffic that clears the Network Firewall rules and is not intended to be further proxied by Cloudflare Gateway service, is routed back to the destination network via the corresponding CNI.
...
3. After clearing Cloudflare Network Firewall, traffic is subsequently routed to [Cloudflare Gateway](/cloudflare-one/traffic-policies/), our secure web gateway service.
...
2. Upon entering the Cloudflare network, outbound Internet traffic is first routed through Cloudflare Network Firewall where it is subject to any configured network firewall policies.
3. Traffic that clears Cloudflare Network Firewall is subsequently sent to [Cloudflare Gateway](/cloudflare-one/traffic-policies/), our secure web gateway service where any configured L3-7 [policies](/cloudflare-one/traffic-policies/) enforce a comprehensive set of security and control measures on the outbound traffic, ensuring the utmost protection for your networks.
...
- [Cloudflare Network Firewall](/cloudflare-network-firewall/)
- [Cloudflare Magic WAN](/magic-wan/)
- [Cloudflare Gateway](/cloudflare-one/traffic-policies/)
