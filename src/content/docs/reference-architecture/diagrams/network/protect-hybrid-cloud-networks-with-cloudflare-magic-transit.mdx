...
3. All traffic is scrubbed, i.e. DDoS attack traffic removed and mitigated, in-line at every Cloudflare data center using advanced and automated [DDoS mitigation](/ddos-protection/) technologies.
...
*Note*: Alternatively, customers can choose to also route the on-premise networks’ server return traffic through Cloudflare via policy-based routing and Magic Transit Egress functionality, adding an additional layer of security and control for the egress traffic with Magic Firewall filtering, e.g. blocking traffic destined to questionable IP addresses and sites, prohibited destinations or countries, etc.
