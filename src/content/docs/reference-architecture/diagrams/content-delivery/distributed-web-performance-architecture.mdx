...
While this guide focuses on maximizing speed and User Experience (UX), performance cannot come at the expense of security. Architects must balance latency reduction against the necessary processing overhead of rigorous security controls (such as DDoS protection, WAF and Bot Management). In high-risk scenarios, security must take precedence, where the *"latency budget"* gained from these performance optimizations is strategically reinvested to power essential protections, ensuring the application remains both fast enough to convert users and secure enough to protect the business.

| Key Business Metrics | Why it matters |
| :---- | :---- |
| **User Experience (UX)**	  | A fast-loading website is fundamental to a positive user experience. Users today expect instant access to information. Research highlights this, showing that a significant portion of users will abandon a website if it [takes too long to load](https://support.google.com/adsense/answer/7450973?hl=en). |
| **Bounce Rate**	 | Bounce rate is a web traffic metric that measures the percentage of visitors who land on a single page of a website and then leave without navigating to any other pages or interacting further with the site, slow loading can significantly increase a bounce rate, as users are likely to leave if a page takes too long to load. |
| **Conversion Rate** | Web performance directly impacts a website's conversion rate, which is the percentage of visitors who complete a desired action, such as making a purchase or signing up for a newsletter. A faster site leads to higher conversion rates. One [study](https://www.cloudflare.com/en-gb/learning/performance/more/website-performance-conversion-rates/) found that even a 100-millisecond reduction in homepage load time resulted in a 1.11% increase in conversions. |
| **SEO**	 | Search Engine Optimization (SEO) is how Search engines like Google use page speed as a ranking factor. Faster-loading websites tend to rank higher in search results, which leads to more organic traffic. Google's Core Web Vitals (CWVs) are a set of metrics that measure a page's loading speed, interactivity, and visual stability, all of which are directly tied to performance. Improving these metrics can significantly boost a site's search engine ranking. |
...
* **Dedicated long-term Cache:** [**Cache Reserve**](https://developers.cloudflare.com/cache/advanced-configuration/cache-reserve/) extends the life of large, infrequently accessed assets (e.g., images, archived video, software updates, or static AI models)assets by moving infrequently accessed content to persistent object storage backend (powered by R2). This prevents eviction due to [Least Recently Used (LRU)](https://developers.cloudflare.com/cache/concepts/retention-vs-freshness/) algorithms and avoids latency-inducing origin fetches, while simultaneously supporting storage redundancy and resilience requirements.
...
* **Security:** [**Dedicated Egress IPs (Aegis)**](https://developers.cloudflare.com/smart-shield/configuration/dedicated-egress-ips/) – part of the [Smart Shield](https://developers.cloudflare.com/smart-shield/) offering – allow the origin to allowlist Cloudflare traffic, ensuring only [proxied](https://developers.cloudflare.com/dns/proxy-status/) and protected requests reach the infrastructure. Note that there are additional [options to protect the origin server](https://developers.cloudflare.com/fundamentals/security/protect-your-origin-server/).
