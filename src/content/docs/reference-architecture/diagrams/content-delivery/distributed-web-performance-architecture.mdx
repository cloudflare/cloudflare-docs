...
- **Content Optimization:** Text assets are compressed using [Compression Rules](/rules/compression-rules/) (Brotli/Gzip). Images are processed on-the-fly via [Image Transformations](/images/optimization/transformations/overview/) or [Polish](/images/polish/) to ensure they are served in the optimal format (AVIF/WebP) and size for the device, significantly improving LCP and CLS.
