...
Detailed documents that provide [step-by-step instructions](/reference-architecture/implementation-guides/) and practical guidance for how to effectively deploy and configure specific solutions or services and are typically focused on a specific implementation goal. A design guide provides the overall best practices for designing a solution, while the implementation guide details the actual steps to deploy. These documents are:

- Focused on a specific implementation outcome, that is, connecting a remote office using the Magic WAN Connector.
- Information the exact commands and configuration steps to take.
