...
products: [cloudflare-for-saas, tunnel, load-balancing, data-localization]
sidebar:
  order: 1
...
We provide several key features such as [hostname routing](/cloudflare-for-platforms/workers-for-platforms/inbound-control/hostname-routing/), which gives you flexibility in deciding which customer application to route to. For example, you can decode an HTTP header telling you the user's ID, then load the appropriate serverless application for this user's request. [Inbound control](/cloudflare-for-platforms/workers-for-platforms/inbound-control/) lets your platform run policy before user code runs, and [outbound control](/cloudflare-for-platforms/workers-for-platforms/outbound-control/) gives you visibility and control into what Internet resources your customer's applications can access.
...
3. Just set up a [Cloudflare for SaaS](/cloudflare-for-platforms/cloudflare-for-saas/start/getting-started/) zone and [order a custom hostname](/cloudflare-for-platforms/cloudflare-for-saas/domain-support/create-custom-hostnames/) for each client hostname. The system will take you through an easy flow to point each client's traffic to your platform, and order their certificate.
   ...
   2. Origin traffic routing is also handled through the SSL for SaaS process. Our default configuration is secure for most needs.
      - For highly secure use cases, you can use [Authenticated Origin Pulls](/ssl/origin-configuration/authenticated-origin-pull/), [Dedicated CDN Egress IPs](/smart-shield/configuration/dedicated-egress-ips/), or an advanced design with [Tunnels](/cloudflare-one/networks/connectors/cloudflare-tunnel/).
