...
- Our recommended approach is to use [software agents](/cloudflare-one/networks/connectors/cloudflare-tunnel/) such as [cloudflared](/cloudflare-one/networks/connectors/cloudflare-tunnel/get-started/) or [Cloudflare Mesh](/mesh/). (Note, only cloudflared currently supports proxying of public hostnames to private applications.)
