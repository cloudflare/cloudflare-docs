...
The Botnet Statistics dataset provides a ranked list of Autonomous Systems based on their presence in the [Cloudflare Botnet Threat Feed](/ddos-protection/botnet-threat-feed/). Rankings can be sorted by two metrics:

- **Offense count**: Total number of mitigated HTTP requests attributed to IPs within the AS. Each offense represents a single mitigated HTTP request from a specific IP address.
- **Originating IP address count**: Number of distinct IPs observed originating botnet activity within the AS.

Data is generated daily from information collected the previous day.
