...
In your [Wrangler configuration](/workers/wrangler/configuration) file, change the default checking frequency (once per hour) to what you like. Here is an example
...
In this example, we will also need to use Cloudflare KV to save the latest checked event IDs which allows us to know what events are new. Once you have created a KV, you can head back to the [Wrangler configuration](/workers/wrangler/configuration) file and add the following sections:
...
For this alert to work, you will need to configure the proper email bindings in the [[Wrangler configuration](/workers/wrangler/configuration) file][wrangler-send-email] file.
