...
Use an app ID, account ID, and an API token when running outside of a Worker (for example, in Node.js). Generate an [API token](/flagship/api-tokens/) from your Cloudflare account with Flagship Evaluate or Flagship App Evaluate permission.
...
- Create an [app-scoped API token](/flagship/api-tokens/) to evaluate flags from a browser or other client-side environment.
