...
:::caution
The client provider embeds an API token in browser code, where anyone can inspect it. Create an [app-scoped token](/flagship/api-tokens/) with **Flagship App Evaluate** for a single app. A leaked token can only evaluate flags in that app — it cannot read flag configuration, change flags, or access other apps. Do not use an account-wide Flagship token in client-side code.
:::
...
| `authToken`     | `string`      | Yes      | A Cloudflare [app-scoped API token](/flagship/api-tokens/) with **Flagship App Evaluate** for this app.                              |
