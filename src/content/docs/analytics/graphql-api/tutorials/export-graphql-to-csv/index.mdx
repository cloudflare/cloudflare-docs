...
```python title="Set up script and authentication"
#!/usr/bin/env python3
...
```python title="Calculates the datetime num_days ago and returns it in ISO format"
...
```python title="Query the Network Analytics GraphQL API"
...
```python title="Convert the data to CSV"
...
```python title="Get Cloudflare Network Analytics via GraphQL API in CSV format"
#!/usr/bin/env python3
