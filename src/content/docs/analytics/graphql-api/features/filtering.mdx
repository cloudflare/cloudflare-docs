...
The availability of filters depends on the queried node. For example, some nodes may support only a zoneTag filter, others only an accountTag filter, and some may support both.
