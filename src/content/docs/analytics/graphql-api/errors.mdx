...
* `query consumed excessive resources, please try running smaller queries which consume fewer resources`
...
A `403` response means the token does not have the required permissions for the requested account or zone. Verify the token has the **Analytics: Read** permission for the relevant resources.
