...
This endpoint and its SQL dialect are separate from the [Analytics SQL API](/analytics/sql-api/). To query Analytics Engine data through that API, use `events.analyticsEngine.<DATASET_NAME>`. Double-quote dataset names that are not ordinary SQL identifiers, for example `events.analyticsEngine."example-dataset"`.
