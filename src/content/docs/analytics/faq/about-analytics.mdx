...
* All metrics reflect traffic proxied through the Cloudflare network (also known as [orange-clouded](/dns/proxy-status/#benefits)), as configured via DNS records in the Cloudflare DNS app. Note that for a [CNAME setup](/dns/zone-setups/partial-setup/), Cloudflare is unable to offer DNS metrics.
