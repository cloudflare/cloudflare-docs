...
The deploying user must have **Microsoft Sentinel Contributor** (or `Contributor` / `Owner`) on the Microsoft Sentinel workspace and **Owner** or **EventGrid Contributor** on the storage account, so that the deployment can create the Event Grid system topic and subscription.
