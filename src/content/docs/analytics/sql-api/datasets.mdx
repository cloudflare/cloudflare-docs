...
The response contains dataset names and descriptions, sorted by name. Columns are omitted by default.
...
| ----------------- | ------- | -------- | ----------------------------------------------------------------------------------------- |
| `account_tag`     | string  | Yes      | Account identifier used to authorize the request.                                         |
...
Introspection returns catalog metadata and does not query the underlying data store. A dataset or column appearing in the response does not guarantee that your plan and permissions allow you to query it. The SQL API applies dataset and field authorization when you submit a query.
