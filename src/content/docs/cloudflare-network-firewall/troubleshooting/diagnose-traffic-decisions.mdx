...
2. Configure the filter:
    ...
    - Select **Apply**.
3. Filter the time range to when the issue occurred.
4. Add additional filters if you know the affected traffic characteristics (such as Source IP, Destination IP, and more).
5. To identify the blocking system:
...
4. To resolve: [Adjust the DDoS managed rule](/ddos-protection/managed-rulesets/adjust-rules/) sensitivity or [create an override](/ddos-protection/managed-rulesets/network/network-overrides/) for the affected traffic pattern.
...
**To resolve**, create an [Advanced TCP Protection allowlist](/ddos-protection/tcp-protection/how-to/add-prefix-allowlist/) or [filter](/ddos-protection/tcp-protection/how-to/create-filter/) to bypass protection for the affected traffic.
...
2. Make sure to include any relevant filters to identify the traffic and to narrow down the time range to the relevant issue timing.
3. In the **Packets summary** graph, select the three dots, then choose **Rule**. The dashboard will show you the rules that were acting on your traffic.
4. Review your [Network Firewall policies](/network-firewall/how-to/add-policies/) and adjust the rule order or expressions as needed.
...
3. Go to **Diagnostics**, and configure a packet capture filter matching the affected traffic. Try to be as specific as possible to avoid generating too many packet capture files.
...
- [Network Firewall rule configuration](/network-firewall/how-to/add-policies/)
- [Packet captures](/network-firewall/packet-captures/)