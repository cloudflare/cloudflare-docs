...
Additionally, the reverse proxy will timeout incomplete HTTP requests after a series of [keepalive probes](/fundamentals/reference/tcp-connections/#tcp-connections-and-keep-alives).
...
DDoS Protection does not inspect or mitigate threats delivered over email protocols such as SMTP, IMAP, or POP3. To protect against email-borne threats such as phishing, business email compromise (BEC), spoofing, and malware delivered via email, use [Cloudflare Email Security](/email-security/).
