...
After Programmable Flow Protection has been enabled to your account, navigate to the Networking > L3/4 DDoS Protection > Advanced Protection page in the dashboard. Within the Programmable Flow Protection tab:

<Steps>
1. Upload your eBPF program written in C.   
  
    The program is validated by the system and stored in your account. The API compiles the program, then runs a verifier against the compiled program to enforce memory checks and verify program termination. If the program fails compilation or verification, the dashboard will return a detailed error message.
...
The steps below write a sample program that drops all User Datagram Protocol (UDP) traffic with an IPv6 header. It also drops traffic destined to port 66. Finally, it drops traffic that does not have some custom specific application header value in the UDP payload.
...
5. Fill variables by calling the helper function.
   ...
   For a full definition of helper functions and structures, see [Supported BPF helper functions and structures](/ddos-protection/advanced-ddos-systems/overview#supported-bpf-helper-functions-and-structures).
...
6. Write your custom logic.
   ...
   It will then check some application header value in the UDP payload and verify its last byte is some fixed value `0xCF`. 
...
For reference, this is the "basic program" example in its entirety:
```c
#define CF_EBPF_HELPER_V0
...
1. **New source IP**: When a packet arrives from an unknown source IP, the program generates a challenge packet containing a random nonce and marks the source IP as "challenged" in the state table. The original packet is dropped.

2. **Challenged source IP**: If a packet arrives from a source IP that has already been challenged, the program checks if the packet contains the correct challenge response (the nonce XORed with a secret value). If the response is correct, the source IP is marked as "verified". If incorrect, the source IP is immediately blocklisted.

3. **Verified source IP**: Packets from verified source IPs are passed through without further checks.
...
6. Handle verified source IPs. 
   ...
   The Programmable Flow Protection platform will drop packets from blocklisted IPs before the program is invoked. Therefore there's no need to explicitly handle the blocklisted case.
...
For reference, this is the "complex program" example in its entirety:
...
To do this, click the button with three dots next to your program. Then select the option to "Overwrite". This will prompt you to select a file to upload as your `C` source code. 

:::note
It is possible to update and overwrite a program that is currently in use by one or more rules. 
When doing so, you will be warned that the program is currently active and will be overwritten. 
However, if an active program is being updated with a program that either does not compile or can not be verified, the update will fail and the old program will continue to be in use.
:::
...
To delete a program, click the button with three dots next to the program you'd like to delete. Then select the option to "Delete". 
...
To create a rule, navigate to Networking > L3/4 DDoS protection > Advanced Protection in the Cloudflare dashboard. Then select the tab titled Programmable Flow Protection.

Scroll to the section entitled **Rules**. Click the button "Create rule". Then fill out the corresponding fields of your new rule.
