...
| `hash_blake2b512` | `int hash_blake2b512(uint8_t *src, size_t src_len, uint8_t *dest, size_t dest_len)` | Computes BLAKE2B-512 hash of the source and stores result in destination buffer. |
| `hmac_blake2b512` | `int hmac_blake2b512(uint8_t *src, size_t src_len, uint8_t *dest, size_t dest_len)` | Computes BLAKE2B-512 HMAC of the message and stores result in the destination buffer. |
...
| `get_src_ip_data` | `int get_src_ip_data(uint64_t *data)` | Retrieves custom data associated with the source IP address from the state table. |
| `set_src_ip_data` | `int set_src_ip_data(uint64_t data)` | Stores custom data associated with the source IP address in the state table. |
| `get_flow_data` | `int get_flow_data(uint64_t *data)` | Retrieves custom data associated with the current flow from the state table. |
| `set_flow_data` | `int set_flow_data(uint64_t data)` | Stores custom data associated with the current flow in the state table. |
