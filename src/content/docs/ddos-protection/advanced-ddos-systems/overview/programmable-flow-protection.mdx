...
4. When you are ready, [update the existing rule](#update-a-rule) to set the mode value to `enabled`.
...
You can modify the mode and scope of an existing rule. The example below modifies an existing rule to make it run in `disabled` mode.
