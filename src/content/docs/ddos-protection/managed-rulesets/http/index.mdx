...
For zones on the Pro, Business, or Enterprise plan the rule is adaptive. An additional check is performed for better detection accuracy: the errors-per-second rate must also be at least five times the normal traffic levels.
