...
You can only configure the behavior of the managed ruleset to set a stronger or weaker mitigation action (depending on the default action of a specific rule, e.g. if the default action is `DDoS Dynamic` or `Log` and you change it to `Block`.), or a lower sensitivity as the default sensitivity for all rules is `High`. Refer to [Managed ruleset parameters](/ddos-protection/managed-rulesets/network/override-parameters/) for more information.
