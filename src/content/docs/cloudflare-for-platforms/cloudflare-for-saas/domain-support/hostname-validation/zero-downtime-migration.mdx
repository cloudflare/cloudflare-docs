...
3. (Optional) Pre-issue the TLS certificate using Delegated DCV.
...
1. Copy the `http_url` and `http_body` from the `ownership_verification_http` object in the response above.
...
1. Copy the `name` and `value` from the `ownership_verification` object in the response above.
...
## Step 3: Pre-issue the TLS certificate (optional)

By default, the TLS certificate issues and deploys after the end customer's CNAME points to your fallback origin. To avoid any TLS errors during cutover, pre-issue the certificate before updating DNS using one of these methods:

- [**Delegated DCV**](/cloudflare-for-platforms/cloudflare-for-saas/security/certificate-management/issue-and-validate/validate-certificates/delegated-dcv/) - Place a one-time CNAME record at your authoritative DNS that allows Cloudflare to validate and auto-renew certificates without any further customer action. Best when you control a shared DNS zone.
...
Once both statuses are `active`, have the end customer update their CNAME to point to your fallback origin:
