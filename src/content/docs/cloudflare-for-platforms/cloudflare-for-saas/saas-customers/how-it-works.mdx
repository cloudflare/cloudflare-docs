...
:::note
O2O only applies when the 2 zones are part of different Cloudflare accounts.
:::
...
:::note
As O2O is based on CNAME, it does not apply when an A record is used to point to the SaaS provider's zone because of [Apex Proxying](/cloudflare-for-platforms/cloudflare-for-saas/start/advanced-settings/apex-proxying/).
:::
