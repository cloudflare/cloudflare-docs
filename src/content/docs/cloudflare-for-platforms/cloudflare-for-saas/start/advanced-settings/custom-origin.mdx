...
* In the [O2O context](/cloudflare-for-platforms/cloudflare-for-saas/saas-customers/how-it-works/) (when requests are originating from a proxied hostname on zone also on Cloudflare), altering the SNI value to use host header is currently not supported.
