...
:::warning
If you need to use an A record to point to the SaaS target, you'll need to get [Apex Proxying](/cloudflare-for-platforms/cloudflare-for-saas/start/advanced-settings/apex-proxying/) enabled.
By default, using an A record instead of CNAME to point to the target is **not a supported setup**!
:::
