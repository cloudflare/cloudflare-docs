...
- Top-level `status`: Hostname activation status. This field shows whether Cloudflare has validated and activated the custom hostname.
- Nested `ssl.status`: Certificate status. This field shows whether the hostname's SSL/TLS certificate has completed issuance and deployment.

Treat the custom hostname as ready for production HTTPS when the top-level `status` is `active`, `ssl.status` is `active`, and the customer's DNS points to your CNAME target or apex proxying records.

If the top-level `status` is `active` but `ssl.status` is not `active`, the hostname is active but its certificate has not completed issuance and deployment. A successful TLS handshake alone does not mean the custom hostname certificate has finished provisioning because Cloudflare may present another matching certificate for that hostname. For more information, refer to [Certificate and hostname priority](/ssl/reference/certificate-and-hostname-priority/).

For non-wildcard custom hostnames, Cloudflare always attempts [HTTP certificate validation](/cloudflare-for-platforms/cloudflare-for-saas/security/certificate-management/issue-and-validate/validate-certificates/http/#non-wildcard-custom-hostnames), even if you selected **TXT** validation.
