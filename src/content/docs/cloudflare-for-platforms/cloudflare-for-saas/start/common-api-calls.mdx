...
When you use the [Create custom hostname endpoint](/api/resources/custom_hostnames/methods/create/), choose one `ssl.method` value: `http`, `txt`, or `email`. This value controls initial certificate issuance. During renewal, Cloudflare may use a different method based on current eligibility. Retrieve the current `ssl.method` and `ssl.validation_records` before taking action. You can explicitly change the method with the [Edit custom hostname endpoint](/api/resources/custom_hostnames/methods/edit/). For more information, refer to [Renew certificates](/cloudflare-for-platforms/cloudflare-for-saas/security/certificate-management/issue-and-validate/renew-certificates/).
