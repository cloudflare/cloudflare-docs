...
Once you have [added a custom hostname](/cloudflare-for-platforms/cloudflare-for-saas/start/getting-started/), you can enable mTLS by using Cloudflare Access. In the [Cloudflare dashboard](https://dash.cloudflare.com/), go to **Zero Trust** and [add mTLS authentication](/cloudflare-one/access-controls/service-credentials/mutual-tls-authentication/) with a few clicks.
...
In the API documentation, refer to [SSL properties of a custom hostname](/api/resources/custom_hostnames/methods/edit/). A `PATCH` request replaces the existing `ssl.settings` object. Retrieve and reuse all current settings to avoid resetting unrelated values.

1. Make a `GET` request to the [Custom Hostname Details](/api/resources/custom_hostnames/methods/get/) endpoint to retrieve the current `ssl.settings` object.
...
2. Copy the complete `ssl.settings` object. Change only `min_tls_version`, then make a `PATCH` request to the [Edit Custom Hostname](/api/resources/custom_hostnames/methods/edit/) endpoint. Omit settings that were not present in the `GET` response.
...
2. Copy the complete `ssl.settings` object. Change only `ciphers`, then make a `PATCH` request to the [Edit Custom Hostname](/api/resources/custom_hostnames/methods/edit/) endpoint. Omit settings that were not present in the `GET` response.
