...
Selecting `ssl.method: txt` uses TXT validation for initial certificate issuance. During an Automated Certificate Management Environment (ACME) renewal, Cloudflare may use automatic HTTP validation for an eligible non-wildcard hostname.

Hostnames with properly configured [Delegated DCV](/cloudflare-for-platforms/cloudflare-for-saas/security/certificate-management/issue-and-validate/validate-certificates/delegated-dcv/) remain TXT-based. Keep the delegation CNAME record in place for future renewals. With manual TXT validation, share each certificate order's current DCV tokens with your customer. For more information, refer to [Renew certificates](/cloudflare-for-platforms/cloudflare-for-saas/security/certificate-management/issue-and-validate/renew-certificates/).
