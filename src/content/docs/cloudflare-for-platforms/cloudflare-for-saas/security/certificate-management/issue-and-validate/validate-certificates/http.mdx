...
For non-wildcard custom hostnames configured with `ssl.method: http`, Cloudflare attempts to complete DCV through [HTTP validation](#http-automatic) after the hostname points to your SaaS target.
...
For normal ACME renewals, Cloudflare preserves the configured validation method. For more information, refer to [Renew certificates](/cloudflare-for-platforms/cloudflare-for-saas/security/certificate-management/issue-and-validate/renew-certificates/).
