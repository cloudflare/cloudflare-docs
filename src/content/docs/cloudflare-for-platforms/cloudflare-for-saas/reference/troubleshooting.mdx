...
If a custom hostname certificate is stuck in **Pending Validation** or has timed out, a [domain control validation (DCV)](/ssl/edge-certificates/changing-dcv-method/dcv-flow/) step has not been completed successfully.

Refer to [DCV troubleshooting](/cloudflare-for-platforms/cloudflare-for-saas/domain-support/hostname-validation/dcv-troubleshooting/) for:

- A breakdown of which errors require action from the **SaaS Provider** versus the **end customer**.
...
If a custom hostname shows a `caa_error` status, refer to [DCV troubleshooting](/cloudflare-for-platforms/cloudflare-for-saas/domain-support/hostname-validation/dcv-troubleshooting/#caa-records) for resolution steps, including the exact CAA records required per certificate authority.
