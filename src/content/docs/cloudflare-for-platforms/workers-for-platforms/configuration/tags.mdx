...
You can also use `limit` (integer, max 1000) to control results per page and `cursor` (string) for pagination. When the response contains a non-empty cursor, pass it as the `cursor` parameter to fetch the next page.

<APIRequest
	method="GET"
	path="/accounts/{account_id}/workers/dispatch/namespaces/{dispatch_namespace}/scripts"
	parameters={{ limit: 100, cursor: "<CURSOR_VALUE>" }}
/>
