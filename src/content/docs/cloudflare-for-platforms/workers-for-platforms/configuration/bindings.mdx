...
Bindings attached during upload remain available to every invocation of that user Worker. To provide a request-scoped platform capability instead, [pass an RPC stub through dynamic dispatch props](/cloudflare-for-platforms/workers-for-platforms/configuration/dynamic-dispatch/#pass-data-and-capabilities-per-request).
