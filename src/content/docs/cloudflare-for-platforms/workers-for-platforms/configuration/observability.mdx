...
Observability lets your platform collect logs and analytics across user Workers in a dispatch namespace. Use it to debug customer code, show usage data to users, detect errors, and understand how traffic moves through your platform.

Use observability when you want to:

- Capture logs for every user Worker in a namespace.
- Send logs to a platform-owned destination.
- Query usage by dispatch namespace, user Worker, or tag.
- Share per-customer usage or error data with your users.
...
On the platform side, Logpush can be enabled for an entire dispatch namespace or a single user Worker. To capture logs for all of the user Workers in a dispatch namespace:
