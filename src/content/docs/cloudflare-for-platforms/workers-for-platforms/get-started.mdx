...
The dispatch Worker receives incoming requests and routes them to the correct user Worker. It uses a dispatch namespace binding to access the namespace.
...
- [Inbound control](/cloudflare-for-platforms/workers-for-platforms/inbound-control/) — Run platform policy before user Workers receive requests, previews, cron triggers, or queue messages
- [Hostname routing](/cloudflare-for-platforms/workers-for-platforms/inbound-control/hostname-routing/) — Route requests by subdomain, path, or custom hostname
- [Bindings](/cloudflare-for-platforms/workers-for-platforms/configuration/bindings/) — Give each customer access to their own [database](/d1/), [key-value store](/kv/), object storage, or custom platform API
- [Outbound control](/cloudflare-for-platforms/workers-for-platforms/outbound-control/) — Configure egress policies and secrets injection for outgoing requests from user Workers
