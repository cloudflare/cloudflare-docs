...
Soft-deletes a profile and all its memories and messages. The data is permanently removed after a retention period.
...
Soft-deletes all memories and messages in a profile that are tagged with the given session ID. Rows from other sessions in the same profile are untouched. Idempotent: deleting a session ID that has no rows is a no-op.
