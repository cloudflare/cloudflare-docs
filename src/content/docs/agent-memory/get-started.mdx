		...
		.withContext("memory", {
			...
			provider: {
				...
				search: async (query: string) => {
					const profile = await this.env.MEMORY.get(MEMORY_PROFILE_NAME);
	...
	async chat(message: string): Promise<string> {
		...
		const result = await generateText({
			...
			messages: await convertToModelMessages(
				this.session.getHistory() as UIMessage[],
			),
...
Wait for the ingestion delay, then ask a different conversation a question that depends on durable memory. This request uses a different Session history but the same Agent Memory profile.

```bash
curl -X POST "http://localhost:8787/chat" \
  -H "Content-Type: application/json" \
  -d '{"conversationId":"second-chat","message":"How should you format code examples for me?"}'
```

The model should call `search_context`, receive recalled memory from Agent Memory, and use that context in its response.
