...
To programmatically retrieve the current list of Cloudflare IP addresses, use the [Cloudflare API](/api/resources/cloudflare_ips/methods/list/).
...
If you want to receive the generic webhook notification on a private IP address or different port, you can either receive and forward the notification using [Workers](/workers/) or set up a [Cloudflare Tunnel](/cloudflare-one/networks/connectors/cloudflare-tunnel/) to route to your connected application.
