...
```dns
TXT yourdomain.com "v=spf1 include:_spf.cloudflare.com ~all"
```
...
If you have existing SPF records, merge them: `v=spf1 include:_spf.cloudflare.com include:_spf.google.com ~all`
:::
...
Email Routing uses `_spf.mx.cloudflare.net` while Email Sending uses `_spf.cloudflare.com`. Both can coexist in the same SPF record.
:::
...
2. Check **Domain Status**:
   ...
   - **Sending SPF**: Properly configured and verified (`_spf.cloudflare.com`)
	...
	try {
		...
		const hasSpf = spfData.Answer?.some(
			(record) =>
				...
				record.data.includes("_spf.cloudflare.com"),
		);
