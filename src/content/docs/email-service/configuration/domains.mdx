...
1. Go to **DNS** > **Records**.
2. Select the locked record and choose **Edit**.
3. Toggle **Unlock record**.
4. Save your changes.

If you want to migrate to a different email provider without immediately interrupting service, unlock the records first, add the new provider's records alongside them, then remove the Email Service records once the new setup is verified.
...
### Clean up leftover DKIM records

If you have stopped using Email Service but still see leftover records such as a DKIM TXT record at `cf2024-1._domainkey.<hostname>`, use the [Disable Email Routing API](/api/resources/email_routing/subresources/dns/methods/delete/) to remove them.
