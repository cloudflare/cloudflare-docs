...
With Wrangler 4.113.0 or later, you can define routing rules that send mail to a [Worker](/email-service/api/route-emails/email-handler/) in your Worker's [Wrangler configuration file](/workers/wrangler/configuration/). Add the inbound addresses to the top-level `addresses` field:
...
addresses = ["invoice@example.com", "*@example.com"]
```

</WranglerConfig>

Each entry is a literal recipient address or a `*@domain` catch-all. Every entry uses the _Send to a Worker_ action to route mail to the Worker you are deploying.

The `addresses` field applies to every [Worker environment](/workers/wrangler/environments/). Wrangler ignores `addresses` values under `env`.
...
When you run [`wrangler deploy`](/workers/wrangler/commands/workers/#deploy), Wrangler reconciles the Worker's managed Email Routing rules with the configured addresses. Wrangler shows the planned changes and asks for confirmation before applying potentially destructive changes.

:::note

Your Wrangler configuration file manages the rules that you define in `addresses`. The dashboard warns you before an edit detaches a managed rule. On the next deploy, Wrangler reports a takeover conflict and asks for confirmation before restoring the configured rule.
