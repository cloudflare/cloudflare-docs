...
2. Go to **Compute** > **Email Service** > **Email Routing** > **Destination Addresses**.
   ...
   <DashButton url="/?to=/:account/email-service/routing/destination-addresses" />
...
If you have more than one destination address linked to the same email pattern, Email Routing will only process the most recent rule. This means only the most recent rule will receive your forwarded emails. To avoid this, do not link more than one destination address to the same email pattern.
:::
...
:::caution[Renaming an Email Worker]
Renaming an Email Worker removes the binding between that Worker and any routes that point to it. After renaming, edit the affected rules to point at the renamed Worker.
:::
...
Once enabled, you can use subaddressing with any of your routing rules. For example, if you send an email to `user+detail@example.com` it will be captured by the `user@example.com` routing rule. The `+detail` part is ignored by Email Routing, but it can be captured next in the processing chain in the logs, a [Worker](/email-service/api/route-emails/email-handler/) or an [Agent application](https://github.com/cloudflare/agents/tree/main/examples/email-agent).
