...
Add custom headers when forwarding. Only headers with an `X-` prefix can be added through `forward()`. Other headers are rejected.
...
Send automatic replies using the Email Service binding. Replies through the Workers API must satisfy the following requirements, otherwise `reply()` throws an exception:
