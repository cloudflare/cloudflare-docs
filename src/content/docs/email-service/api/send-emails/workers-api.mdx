...
import { Tabs, TabItem, WranglerConfig, Aside } from "~/components";
...
<Aside type="note" title="About messageId in response">
The `messageId` returned by the Workers API is an internal tracking identifier, not the RFC 5322 `Message-ID` header that appears in the delivered email. Cloudflare generates the actual `Message-ID` header at send time, but this value is not exposed in the API response. If you need to track messages, use custom `X-` headers (such as `X-Message-ID` or `X-Tracking-ID`) that you generate and control.
</Aside>
...
interface EmailSendResult {
	messageId: string; // Internal tracking ID (not the RFC 5322 Message-ID header)
}
