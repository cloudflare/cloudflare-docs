...
When using `wrangler dev` without [remote bindings](/workers/development-testing/#remote-bindings), `ArrayBuffer` and `ArrayBufferView` content in attachments cannot be serialized by the local simulator. Refer to [local development for email sending](/email-service/local-development/sending/#known-limitations) for the workaround.
:::

:::note[Workers binding vs REST API responses]
The Workers binding returns a single `messageId` per `send()` call. The [REST API](/email-service/api/send-emails/rest-api/) returns recipient-grouped delivery status instead.
:::
...
```ts
// Send to multiple recipients (combined to/cc/bcc max 50)
const response = await env.EMAIL.send({
