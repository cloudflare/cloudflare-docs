...
:::note[Message-ID not included in response]
The API response does not include the `Message-ID` or other platform-controlled headers that Cloudflare generates when sending the email. If you need to track individual messages, use custom `X-` headers (such as `X-Message-ID` or `X-Tracking-ID`) in your request that you can correlate with your own systems.
:::
