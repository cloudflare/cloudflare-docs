...
A normal `250 2.0.0 Ok` response after the message body includes the assigned Message-ID. Use it to correlate the submission with delivery logs in the dashboard. A response without a Message-ID can mean every recipient was suppressed. Refer to [Suppressed recipients](#suppressed-recipients).
...
SMTP accepts a syntactically valid recipient with `250 2.1.5 Ok` during `RCPT TO`. Email Service checks the account's [suppression list](/email-service/concepts/suppressions/) after receiving the message body.
