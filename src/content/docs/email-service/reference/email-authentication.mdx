...
2. Go to **DNS** > **Records**.
3. Look for multiple TXT records starting with `v=spf1`.
4. Delete the incorrect SPF record.
5. Ensure you have only one SPF record that includes: `v=spf1 include:_spf.email.cloudflare.net ~all`
...
1. Log in to the [Cloudflare dashboard](https://dash.cloudflare.com/) and select your account and domain.
2. Go to **DNS** > **Records**.
3. Add a TXT record with the following content:
   - **Name**: `@` (or your root domain)
   - **Content**: `v=spf1 include:_spf.email.cloudflare.net ~all`
4. If you already have an SPF record, modify it to include `include:_spf.email.cloudflare.net`
...
```txt
v=spf1 include:_spf.email.cloudflare.net include:other-service.com ~all
```
...
```txt
"v=spf1 include:_spf.email.cloudflare.net ~all"
```
...
Email Service automatically generates DKIM keys for your domain, but the DNS records must be properly configured:

1. In the [Cloudflare dashboard](https://dash.cloudflare.com/), go to **Compute & AI** > **Email Service**.
2. Select your domain.
3. Go to **Settings** > **Authentication**.
4. Copy the DKIM record details.
5. Go to **DNS** > **Records** and add the DKIM TXT record:
   - **Name**: `cf2024-1._domainkey`
   - **Content**: The DKIM public key provided
...
Verify your DKIM record is configured correctly:
...
2. Check that the record name matches exactly: `cf2024-1._domainkey.yourdomain.com`
