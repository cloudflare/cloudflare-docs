...
6. Return to **Compute** > **Email Service** > **Email Routing** and turn off Email Routing. Cloudflare removes the routing DNS records it still manages — `MX` and `DKIM`. Because you unlocked and edited the `SPF` record in steps 3 and 5, Email Routing no longer manages it; that record remains in place. Refer to [Remove a domain from Email Routing](/email-service/configuration/domains/#remove-a-domain-from-email-routing) for details.
