...
2. Select your domain, then open **Settings** and turn off Email Routing. Cloudflare removes all routing-related DNS records it manages (`MX`, `SPF`, and `DKIM`), as described in [Remove a domain from Email Routing](/email-service/configuration/domains/#remove-a-domain-from-email-routing).
   ...
   :::note
   Disabling Email Routing removes its `MX` records before you add the Google Workspace records in step 3, which causes a brief inbound mail interruption. To switch providers without any interruption, unlock the Email Routing DNS records first and add the Google Workspace records alongside them before disabling Email Routing. Refer to [Manage locked records](/email-service/configuration/domains/#locked-records) for the unlock steps.
   :::
