...
6. Return to **Compute** > **Email Service** > **Email Routing** and turn off Email Routing. Cloudflare removes the routing records it still manages (`MX` and `DKIM`). Your unlocked and edited `SPF` record remains in place.
7. In **DNS** > **Records**, update the `SPF` `TXT` record to remove the Cloudflare include: `v=spf1 include:_spf.google.com ~all`.
