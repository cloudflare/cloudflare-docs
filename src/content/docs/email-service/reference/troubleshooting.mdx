...
The Workers binding throws `E_RECIPIENT_SUPPRESSED` when a recipient is on the [suppression list](/email-service/concepts/suppressions/) for the account. The REST API returns `400` for the same condition. Both interfaces reject the entire multi-recipient request.
...
### Recipient blocked after deletion

To investigate enforcement after deletion:
