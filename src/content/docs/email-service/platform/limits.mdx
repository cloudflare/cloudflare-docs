...
| **Results per page**           | 1,000             | `per_page` on the list endpoint; defaults to 100                              |
| **Items per bulk import**      | 1,000             | Split larger imports across several requests                                  |
| **Bulk import requests**       | 10 per minute     | Per account, on the `/suppressions/bulk` endpoint; exceeding it returns `429` |
