...
The sending domain of a message is the domain of its sender address. This is the `from` address for the [REST API](/email-service/api/send-emails/rest-api/) and [Workers binding](/email-service/api/send-emails/workers-api/), and the envelope `MAIL FROM` address for [SMTP](/email-service/api/send-emails/smtp/).
