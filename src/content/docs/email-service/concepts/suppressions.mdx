...
Each suppression has a scope. The scope controls which sending domains the entry blocks:

| Scope            | Blocks the recipient for                                                    |
| ---------------- | --------------------------------------------------------------------------- |
| `account`        | Every sending domain and subdomain in your account                          |
| `sending_domain` | Mail whose sender address uses one exact domain, such as `mail.example.com` |

The sending domain is the domain of the sender address. This is the `from` address for the [REST API](/email-service/api/send-emails/rest-api/) and [Workers binding](/email-service/api/send-emails/workers-api/), and the envelope `MAIL FROM` address for [SMTP](/email-service/api/send-emails/smtp/).

A `sending_domain` entry matches only its exact domain. An entry for `example.com` does not block mail from `mail.example.com`, and an entry for `mail.example.com` does not block mail from `example.com`.

Email Service suppresses a recipient when an `account` entry exists for the address, or when a `sending_domain` entry exists for the address and the sender domain. One address can have one `account` entry and one entry for each sending domain. Each entry has its own reason and expiration.

Use `sending_domain` entries when one account sends mail for several brands or customers. A bounce or complaint from one customer's mail then does not block the same recipient for your other customers.

### Automatic suppressions and scope

Bounce and complaint suppressions use the `sending_domain` scope. Each entry uses the domain of the sender address of the message that bounced or received the complaint.

Email Service creates an `account` entry instead when it cannot identify the sending domain. For example, this happens when a complaint does not match a delivered message.

Entries created before sending-domain scope was available have the `account` scope. They remain active until they expire or you delete them.
...
Manual entries use the `account` scope by default. Through the API, you can create a manual entry for one sending domain instead. Email Service does not check that your account sends from that domain. An entry for a domain that you do not send from never blocks mail.
...
Email Service creates a suppression without an expiration when the mailbox or domain does not exist. It also does this when a recipient-side issue persists across repeated delivery attempts without a successful delivery. Email Service counts these attempts separately for each sending domain.

Because bounce suppressions use the `sending_domain` scope, a recipient that bounces for one domain can still receive delivery attempts from your other domains. Each domain gets its own suppression when the recipient bounces for it.
...
- Use the `account` scope when a recipient must not receive mail from any of your domains. Use the `sending_domain` scope when the decision applies to one brand or customer only.
