...
flowchart LR
    ...
    E -->|Send to Worker| W[Email Worker]
    E -->|Drop| X[Drop]
    W --> Y{Worker action?}
    Y -->|forward| F
    Y -->|reply| F
...
4. **Action:** The system applies the matched rule's action:
   ...
   - **Send to a Worker**: The message is passed to your [Email Worker](/email-service/api/route-emails/email-handler/). The Worker can call `forward()`, `reply()`, or `setReject()`.
