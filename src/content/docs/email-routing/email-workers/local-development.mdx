...
You can test the behavior of an Email Worker script in local development using Wrangler with [wrangler dev](/workers/wrangler/commands/general/#dev), or using the [Cloudflare Vite plugin](https://developers.cloudflare.com/workers/vite-plugin/).
...
export default {
	async email(message, env: any, ctx: any) {
		...
		const msg = createMimeMessage();
		msg.setSender({
			name: "Thank you for your contact",
