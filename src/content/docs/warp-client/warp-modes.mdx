...
In **DNS only** mode, WARP routes only DNS queries through Cloudflare's 1.1.1.1 resolver and does not tunnel device traffic. DNS queries are always encrypted, available in two flavors:

- **DNS only (HTTPS)** — uses DNS-over-HTTPS (DoH).
- **DNS only (TLS)** — uses DNS-over-TLS (DoT).
...
- **Traffic and DNS (UDP)** — All device traffic is routed through WARP. DNS queries use UDP inside the tunnel.
