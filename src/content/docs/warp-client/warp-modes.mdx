...
1.1.1.1 is Cloudflare’s public DNS resolver. It offers a fast and private way to browse the Internet. It also offers a DNS encryption service via the two flavors below for increased security and privacy.

- **DNS Only (HTTPS)** — DNS queries are encrypted via DNS-over-HTTPS (DoH).
- **DNS Only (TLS)** — DNS queries are encrypted via DNS-over-TLS (DoT).
...
- **Traffic and DNS (UDP)** — All device traffic is routed through WARP with DNS resolution over UDP.
- **Traffic and DNS (TLS)** — All device traffic is routed through WARP with DNS queries encrypted via DNS-over-TLS (DoT).
- **Traffic and DNS (HTTPS)** — All device traffic is routed through WARP with DNS queries encrypted via DNS-over-HTTPS (DoH).
...
Full tunnel without DNS proxying. All device traffic is routed through WARP, but DNS resolution remains managed by the device operating system.
