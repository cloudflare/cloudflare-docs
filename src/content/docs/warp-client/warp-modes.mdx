...
WARP+ Unlimited is our monthly subscription offering that provides access to a larger network of Cloudflare data centers. More cities to connect to means you are likely to be closer to a Cloudflare data center. This can reduce the latency between your device and Cloudflare and improve your browsing speed. As a result, sites load faster, both for those on the Cloudflare network and those that are not.

Currently, WARP+ Unlimited can only be purchased on iOS and Android devices.