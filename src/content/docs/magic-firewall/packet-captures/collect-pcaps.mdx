...
Due to the nature of Cloudflare network, your traffic may traverse various physical machines (known as metals) within a single Cloudflare location (colo).

- Multiple PCAP Files: A single full PCAP capture may produce many small PCAP files, as a capture is taken for each metal your traffic traverses in the colo. 
