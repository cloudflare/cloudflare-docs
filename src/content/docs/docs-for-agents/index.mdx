...
Use this guide to connect AI coding assistants and large language models (LLMs) to Cloudflare documentation, APIs, and platform tools.

## Quick start

Choose the approach that matches how you use AI tools:

- **AI coding agents** (tools that work with code in your project): Set up [Claude Code](/agent-setup/claude-code/), [Cursor](/agent-setup/cursor/), [Codex](/agent-setup/codex/), [GitHub Copilot](/agent-setup/github-copilot/), [OpenCode](/agent-setup/opencode/), or [Windsurf](/agent-setup/windsurf/) with Cloudflare [Skills](#skills) and [MCP servers](#mcp-servers).
- **Chat interfaces** (ChatGPT, Claude.ai, and similar): Append `/index.md` to any docs page URL and paste the Markdown into your conversation.
- **Project rules and custom instructions**: Add a product-level [`llms.txt`](#per-product-endpoints) URL to your AI tool's configuration file for an index of relevant pages.
...
To install skills in any compatible agent, run the following command in your terminal:
...
The [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) is an open standard that connects AI tools to external tools, data, and services. Cloudflare runs managed remote MCP servers that give your agent the ability to search documentation, call the Cloudflare API, and query logs and analytics while it works.
...
Every agent listed supports Skills and MCP. Capabilities, pricing, and context approaches vary. Refer to [Concepts](#concepts) below for definitions of terms used in this table.
...
Skills are reusable instruction sets that teach an agent about a specific domain — for example, how to deploy a Cloudflare Worker or configure a WAF rule. They act as plugins that the agent loads on demand when it encounters a relevant task.
