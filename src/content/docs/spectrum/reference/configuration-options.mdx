...
Spectrum virtual network origins are for TCP and UDP traffic only. For HTTP/HTTPS traffic to private origins, use [Application Services for Private Origins](/dns/private-origins/), which provides WAF, CDN caching, and the full Cloudflare proxy stack.
...
Suggested addition to the Virtual network origin section:
Can we add a note clarifying that virtual network origins are for TCP/UDP traffic only? For HTTP/HTTPS traffic to private origins, customers should use Application Services for Private Origins (https://developers.cloudflare.com/dns/private-origins/) instead. This prevents confusion between the two sibling features.
Suggested text to add after the intro paragraph:
> Virtual network origins are for TCP and UDP traffic only. For HTTP/HTTPS traffic to private origins, use [Application Services for Private Origins](/dns/private-origins/), which provides WAF, CDN caching, and the full Cloudflare proxy stack.
This helps customers choose the right feature for their protocol.