...
Cloudflare automatically runs Early Hints probe requests on all Pages domains, including custom domains. These probes appear in your Cloudflare logs with these field values:

- `ClientRequestSource: earlyHintsCache`
- `ClientRequestUserAgent: nginx-ssl early hints` or `bastion early hints`
...
Probes return `504` when Cloudflare has no `Link` headers cached for a URL. To convert cache misses to hits and reduce `504` volume in your logs, configure `Link` headers in your Pages project using either the [`_headers` file](/pages/configuration/headers/) or automatic `Link` header generation from HTML elements.

### Disable Early Hints on Pages

Early Hints cannot be disabled for Cloudflare Pages. If you need full control over the Early Hints setting, [migrate your project from Pages to Workers](/workers/static-assets/migration-guides/migrate-from-pages/). Workers respects the zone-level Early Hints setting.
