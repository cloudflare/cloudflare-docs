...
Running this command will prompt you to install the [`create-cloudflare`](https://www.npmjs.com/package/create-cloudflare) package, and then ask you to choose a framework for your project.

:::note
To create a Pages project you must specify `--platform=pages`, otherwise C3 will create a Workers project. When `--platform=pages` is specified, C3 only shows frameworks that support the Pages platform.

If you want to create a Pages project without using a framework, use [Direct Upload](/pages/get-started/direct-upload/) or [Git integration](/pages/get-started/git-integration/) instead.
:::

## Web frameworks

The following frameworks support Pages deployment via C3:
...
When you use a framework, `npm create cloudflare` directly uses the framework's own command for generating new projects, which may prompt additional questions. This ensures that the project you create is up-to-date with the latest version of the framework, and you have all the same options when creating your project via `npm create cloudflare` that you would if you created your project using the framework's tooling directly.
...
- `--category` <Type text="string" /> <MetaInfo text="optional" />
  ...
  - The possible values for this option are:
    ...
    - `hello-world`: Hello World example (Workers only)
    - `web-framework`: Framework Starter
    - `demo`: Application Starter (Workers only)
