...
Log in to Wrangler with the [`wrangler login` command](/workers/wrangler/commands/general/#login). Then run the [`pages project create` command](/workers/wrangler/commands/pages/#pages-project-create):
...
From here, you have created an empty project and can now deploy your assets for your first deployment and for all subsequent deployments in your production environment. To do this, run the [`wrangler pages deploy`](/workers/wrangler/commands/general/#deploy) command:
...
If you would like to use Wrangler to obtain a list of all available projects for Direct Upload, use [`pages project list`](/workers/wrangler/commands/pages/#pages-project-list):
