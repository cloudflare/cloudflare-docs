...
In the code above, the `getPlatformProxy` helper function will automatically detect any bindings defined in your project's [Wrangler configuration](/workers/wrangler/configuration) file and emulate those bindings in local development. Review [Wrangler configuration information on bindings](/workers/wrangler/configuration/#bindings) for more information on how to configure bindings in [Wrangler's configuration](/workers/wrangler/configuration) file.

A new type definition for the `Env` type (used by `context.cloudflare.env`) can be generated from [Wrangler configuration](/workers/wrangler/configuration) file with the following command:

```sh
npm run cf-typegen
```

This should be done any time you add new bindings to [Wrangler configuration](/workers/wrangler/configuration) file.
