...
Wrangler configuration is used primarily for local development. Bindings specified in it are not available remotely, unless they are created as [Remote bindings](/workers/functions/bindings/#remote-bindings). @TODO: Link to the correct page when it is created.
