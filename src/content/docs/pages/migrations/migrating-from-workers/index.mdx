...
2. If using a static site tool, a build directory (called `bucket` in the `wrangler.toml / wrangler.json(c)` file) where the static project builds your HTML, CSS, and JavaScript files.
...
When moving to Cloudflare Pages, remove the Workers application and any associated `wrangler.toml / wrangler.json(c)` files or build output. Instead, note and record your `build` command (if you have one), and the `bucket` field, or build directory, from the Wrangler file in your project's directory.
