...
If your project contains an existing [Wrangler configuration](/workers/wrangler/configuration) file that you [previously used for local development](/pages/functions/local-development/), make sure you verify that it matches your project settings in the Cloudflare dashboard before opting-in to deploy your Pages project with [Wrangler configuration](/workers/wrangler/configuration) file. Instead of writing your [Wrangler configuration](/workers/wrangler/configuration) file by hand, Cloudflare recommends using `npx wrangler pages download config` to download your current project settings into a [Wrangler configuration](/workers/wrangler/configuration) file.

:::

Pages Functions can be configured two ways, either via the [Cloudflare dashboard](https://dash.cloudflare.com) or [Wrangler configuration](/workers/wrangler/configuration) file, a configuration file used to customize the development and deployment setup for [Workers](/workers/) and Pages Functions.

This page serves as a reference on how to configure your Pages project via [Wrangler configuration](/workers/wrangler/configuration) file.

If using [Wrangler configuration](/workers/wrangler/configuration) file, you must treat your [Wrangler configuration](/workers/wrangler/configuration) file as the [source of truth](/pages/functions/wrangler-configuration/#source-of-truth) for your Pages project configuration.

:::note[Configuration via [Wrangler configuration](/workers/wrangler/configuration) file is in open beta.]
...
Using [Wrangler configuration](/workers/wrangler/configuration) file to configure your Pages project allows you to:
...
## Example [Wrangler configuration](/workers/wrangler/configuration) file
...
Pages Functions configuration via [Wrangler configuration](/workers/wrangler/configuration) file requires the [V2 build system](/pages/configuration/build-image/#v2-build-system) or later. To update from V1, refer to the [V2 build system migration instructions](/pages/configuration/build-image/#v1-to-v2-migration).

### Wrangler

You must have Wrangler version 3.45.0 or higher to use [Wrangler configuration](/workers/wrangler/configuration) file for your Pages project's configuration. To check your Wrangler version, update Wrangler or install Wrangler, refer to [Install/Update Wrangler](/workers/wrangler/install-and-update/).

## Migrate from dashboard configuration

The migration instructions for Pages projects that do not have a [Wrangler configuration](/workers/wrangler/configuration) file currently are different than those for Pages projects with an existing [Wrangler configuration](/workers/wrangler/configuration) file. Read the instructions based on your situation carefully to avoid errors in production.

### Projects with existing [Wrangler configuration](/workers/wrangler/configuration) file

Before you could use [Wrangler configuration](/workers/wrangler/configuration) file to define your preview and production configuration, it was possible to use [Wrangler configuration](/workers/wrangler/configuration) file to define which [bindings](/pages/functions/bindings/) should be available to your Pages project in local development.

If you have been using [Wrangler configuration](/workers/wrangler/configuration) file for local development, you may already have a file in your Pages project that looks like this:
...
If you would like to use your existing [Wrangler configuration](/workers/wrangler/configuration) file for your Pages project configuration, you must:
...
2. Review your existing [Wrangler configuration](/workers/wrangler/configuration) carefully to make sure it aligns with your desired project configuration before deploying.

If you add the `pages_build_output_dir` key to [Wrangler configuration](/workers/wrangler/configuration) file and deploy your Pages project, Pages will use whatever configuration was defined for local use, which is very likely to be non-production. Do not deploy until you are confident that your [Wrangler configuration](/workers/wrangler/configuration) file is ready for production use.

:::caution[Overwriting configuration]

Running [`wrangler pages download config`](/pages/functions/wrangler-configuration/#projects-without-existing-wranglertoml-file) will overwrite your existing [Wrangler configuration](/workers/wrangler/configuration) file with a generated [Wrangler configuration](/workers/wrangler/configuration) file based on your Cloudflare dashboard configuration. Run this command only if you want to discard your previous [Wrangler configuration](/workers/wrangler/configuration) file that you used for local development and start over with configuration pulled from the Cloudflare dashboard.

:::

You can continue to use your [Wrangler configuration](/workers/wrangler/configuration) file for local development without migrating it for production use by not adding a `pages_build_output_dir` key. If you do not add a `pages_build_output_dir` key and run `wrangler pages deploy`, you will see a warning message telling you that fields are missing and that the file will continue to be used for local development only.

### Projects without existing [Wrangler configuration](/workers/wrangler/configuration) file

If you have an existing Pages project with configuration set up via the Cloudflare dashboard and do not have an existing [Wrangler configuration](/workers/wrangler/configuration) file in your Project, run the `wrangler pages download config` command in your Pages project directory. The `wrangler pages download config` command will download your existing Cloudflare dashboard configuration and generate a valid [Wrangler configuration](/workers/wrangler/configuration) file in your Pages project directory.
...
Review your generated [Wrangler configuration](/workers/wrangler/configuration) file. To start using [Wrangler configuration](/workers/wrangler/configuration) file for your Pages project's configuration, create a new deployment, via [Git integration](/pages/get-started/git-integration/) or [Direct Upload](/pages/get-started/direct-upload/).
...
If you download a [Wrangler configuration](/workers/wrangler/configuration) file from a project configured with "Latest" using the `wrangler pages download` command, your [Wrangler configuration](/workers/wrangler/configuration) file will have the latest compatibility date available at the time you downloaded the configuration file. Wrangler does not support the "Latest" functionality like the dashboard. Compatibility dates must be explicitly set when using [Wrangler configuration](/workers/wrangler/configuration) file.
...
## Differences using [Wrangler configuration](/workers/wrangler/configuration) file for Pages Functions and Workers

If you have used [Workers](/workers), you may already be familiar with [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/). There are a few key differences to be aware of when using [Wrangler configuration](/workers/wrangler/configuration) file with your Pages Functions project:

- The configuration fields **do not match exactly** between Pages Functions [Wrangler configuration](/workers/wrangler/configuration) file and the Workers equivalent. For example, configuration keys like `main`, which are Workers specific, do not apply to a Pages Function's [Wrangler configuration](/workers/wrangler/configuration) file. Some functionality supported by Workers, such as [module aliasing](/workers/wrangler/configuration/#module-aliasing) cannot yet be used by Cloudflare Pages projects.
- The Pages [Wrangler configuration](/workers/wrangler/configuration) file introduces a new key, `pages_build_output_dir`, which is only used for Pages projects.
...
With [Wrangler configuration](/workers/wrangler/configuration) file you can quickly set configuration across your local environment, preview deployments, and production.

### Local development

[Wrangler configuration](/workers/wrangler/configuration) file works locally when using `wrangler pages dev`. This means that you can test out configuration changes quickly without a need to login to the Cloudflare dashboard. Refer to the following config file for an example:
...
This [Wrangler configuration](/workers/wrangler/configuration) file adds the `nodejs_compat` compatibility flag and a KV namespace binding to your Pages project. Running `wrangler pages dev` in a Pages project directory with this [Wrangler configuration](/workers/wrangler/configuration) file will apply the `nodejs_compat` compatibility flag locally, and expose the `KV` binding in your Pages Function code at `context.env.KV`.
...
Once you are ready to deploy your project, you can set the configuration for production and preview deployments by creating a new deployment containing a [Wrangler configuration](/workers/wrangler/configuration) file.
...
- Configure D1 database bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#d1-databases) the same way they are configured with Cloudflare Workers.
...
- Configure Durable Object namespace bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#durable-objects) the same way they are configured with Cloudflare Workers.

:::caution

<Render file="do-note" product="pages" /> Durable Object bindings configured in
a Pages project's [Wrangler configuration](/workers/wrangler/configuration) file
require the `script_name` key. For Workers, the `script_name` key is optional.
...
- Configure environment variables via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#environment-variables) the same way they are configured with Cloudflare Workers.
...
- Configure Hyperdrive bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#hyperdrive) the same way they are configured with Cloudflare Workers.
...
- Configure KV namespace bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#kv-namespaces) the same way they are configured with Cloudflare Workers.
...
- Configure Queues Producer bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#queues) the same way they are configured with Cloudflare Workers.
...
- Configure R2 bucket bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#r2-buckets) the same way they are configured with Cloudflare Workers.
...
- Configure Vectorize bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#vectorize-indexes) the same way they are configured with Cloudflare Workers.
...
- Configure service bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#service-bindings) the same way they are configured with Cloudflare Workers.
...
- Configure Analytics Engine Dataset bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#analytics-engine-datasets) the same way they are configured with Cloudflare Workers.
...
- Configure Workers AI bindings via your [[Wrangler configuration](/workers/wrangler/configuration) file](/workers/wrangler/configuration/#workers-ai) the same way they are configured with Cloudflare Workers.
...
When used in your Pages Functions projects, your [Wrangler configuration](/workers/wrangler/configuration) file is the source of truth. You will be able to see, but not edit, the same fields when you log into the Cloudflare dashboard.

If you decide that you don't want to use [Wrangler configuration](/workers/wrangler/configuration) file for configuration, you can safely delete it and create a new deployment. Configuration values from your last deployment will still apply and you will be able to edit them from the dashboard.
