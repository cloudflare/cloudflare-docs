...
If you are on Windows, you can use this PowerShell alternative (no `jq` required):
...
Replace `<PROJECT_NAME>` with your Pages project name. Keep `$prod_id` empty on the first loop so the active production deployment is detected and preserved.
