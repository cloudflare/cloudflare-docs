...
if [ -n "$failed" ]; then
    echo "The following deployments failed to delete:" >&2
    printf '%s\n' "$failed" >&2
    exit 1
fi
...
These scripts will not delete the active production deployment if one exists.
