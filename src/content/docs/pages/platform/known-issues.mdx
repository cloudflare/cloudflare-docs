...
prod_id=""
while :; do
  ids=$(npx wrangler pages deployment list --project-name research-cloudflare-com --json | jq -r '.[].Id')
  ...
  while IFS= read -r id; do
    if ! npx wrangler pages deployment delete "$id" --project-name research-cloudflare-com --force 2>&1 | tee /tmp/wrangler-del.log | grep -q "Successfully deleted"; then
