...
To deploy your application to a `*.pages.dev` subdomain, you need to specify a directory of static assets to serve, configure the `pages_build_output_dir` in your project’s [Wrangler configuration](/workers/wrangler/configuration) file and set the value to `./public`:
