...
Open the `wrangler.toml / wrangler.json(c)` file, and add the following binding to the file. `bucket_name` should be the exact name of the bucket created earlier, while `binding` can be any custom name referring to the R2 resource:
...
Save the `wrangler.toml / wrangler.json(c)` file, and we are ready to move on to the last step.
