...
To bind the R2 bucket we have created to the cat blog, we need to update [Wrangler configuration](/workers/wrangler/configuration) file.

Open [Wrangler configuration](/workers/wrangler/configuration) file, and add the following binding to the file. `bucket_name` should be the exact name of the bucket created earlier, while `binding` can be any custom name referring to the R2 resource:
...
Save [Wrangler configuration](/workers/wrangler/configuration) file and we are ready to move on to the last step.
