...
describe("resolveSocialImagePath", () => {
  ...
  test("precedence: prop > config > product-group > opengraph > logo > card", () => {
    ...
    expect(
      resolveSocialImagePath({ productGroup: "Developer Platform", exists: none }),
    ...
    expect(resolveSocialImagePath({ exists: none })).toBe("/cf-twitter-card.png");
  });
...
describe("inferContentType", () => {
  ...
  test("otherwise undefined", () => {
    expect(inferContentType("/workers/get-started/", "workers")).toBeUndefined();
...
describe("resolvePageTitle", () => {
  test("no suffix, no override → `Title | Site` (unconditional, no dedup)", () => {
    ...
    expect(
      resolvePageTitle({ title: "Cloudflare Docs", siteTitle: "Cloudflare Docs" }),
...
describe("buildStructuredData", () => {
  ...
  test("@type reflects schemaType", () => {
    const json = JSON.parse(buildStructuredData({ ...base, schemaType: "WebPage" })!);
  ...
  test("description included only when truthy", () => {
    ...
    expect(
      JSON.parse(buildStructuredData({ ...base, description: "d" })!).description,
    ).toBe("d");
  });
  ...
  test("keywords omitted when tags absent/empty, present when populated", () => {
    ...
    expect(JSON.parse(buildStructuredData({ ...base, tags: [] })!).keywords).toBeUndefined();
    expect(
