...
import { aiModelCardSchema } from "~/schemas/ai-model-catalog";
...
export const collections = {
	docs: defineCollection(
		docsCollection({
			...
			schemaFields: {
				...
				// Canonical URL override for pages that duplicate content living at
				// another path (e.g. legacy WAF reference pages). Read by
				// PageHead's `meta.canonical` via `[...slug].astro`.
				canonical: z.string().optional(),
				// Style-guide component demo pages self-report which live component
				// they showcase, so DocsLayout can render a "Component usage" link.
				styleGuide: z.object({ component: z.string() }).optional(),
	...
	"warp-releases": defineCollection({
		loader: glob({
			pattern: "**/*.{json,yml,yaml}",
			base: "./src/content/warp-releases",
		}),
